...
/**
 * Register every `agent-meow:browser-*` IPC handler. Idempotent per process is
...
function registerBrowserIpc({ ipcMain, isPinnedOriginSender, getRegistryForEvent }) {
  ...
  ipcMain.handle("agent-meow:browser-open-or-navigate", (event, args) => {
  ...
  ipcMain.handle("agent-meow:browser-set-active", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-resize", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-screenshot", async (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-execute", async (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-has-view", (event, args) => {
    const g = gateRegistry(event);
  ...
  ipcMain.handle("agent-meow:browser-close", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-go-back", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
  ...
  ipcMain.handle("agent-meow:browser-go-forward", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
    const entry = g.registry.get(args?.conversationId);
    if (!entry) return { ok: false, error: "No browser view" };
  ...
  ipcMain.handle("agent-meow:browser-reload", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
    const entry = g.registry.get(args?.conversationId);
    if (!entry) return { ok: false, error: "No browser view" };
  ...
  ipcMain.handle("agent-meow:open-browser-devtools", (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
    const entry = g.registry.get(args?.conversationId);
    if (!entry) return { ok: false, error: "No browser view" };
  ...
  ipcMain.handle("agent-meow:browser-enable-design-mode", async (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
    const { conversationId } = args ?? {};
    const entry = g.registry.get(conversationId);
  ...
  ipcMain.handle("agent-meow:browser-disable-design-mode", async (event, args) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
    const { conversationId } = args ?? {};
    const entry = g.registry.get(conversationId);
  ...
  ipcMain.handle("agent-meow:browser-signal-design-result", async (event, payload) => {
    const g = gateRegistry(event);
    if (g.error) return { ok: false, error: g.error };
