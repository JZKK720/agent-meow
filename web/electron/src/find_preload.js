...
contextBridge.exposeInMainWorld("agentMeowFind", {
   ...
   */
  query: (text, opts) => {
    ipcRenderer.send("agent-meow:find-query", {
      text: String(text ?? ""),
  ...
  close: () => {
    ipcRenderer.send("agent-meow:find-close");
  },
  /**
   ...
   */
  onResult: (callback) => {
    ipcRenderer.on("agent-meow:find-result", (_event, result) => callback(result));
  },
  /** Fires when Cmd/Ctrl+F re-activates an already-open bar. */
  onActivate: (callback) => {
    ipcRenderer.on("agent-meow:find-activate", () => callback());
  },
});
