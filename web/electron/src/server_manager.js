// Process lifecycle for desktop-managed agent-meow servers and host connections.
//
// This is the only place the desktop spawns long-lived processes. It owns:
//   - hostChildren: the foreground `agent-meow host --server <url>` processes this
//     app started. They are torn down when the app quits (the confirmed
//     lifecycle: the desktop owns what it starts).
//   - ownedLocalServer: a local `agent-meow server` we started ourselves (and so
...
/** The line `agent-meow host` prints once the websocket tunnel is up. */
...
/**
 * Every `agent-meow host` child we have spawned and not yet seen exit — including
 ...
 * from `agent-meow host`'s own messages (HostConnectError: "Authentication
 * failed", "HTTP 401", login-page redirect, or the `agent-meow login` hint). Lets
...
function isAuthError(text) {
  return /authentication failed|http 401|unauthor|login page|agent-meow login/i.test(
    String(text || ""),
  );
}

/**
 * True when `agent-meow host` refused to start because a daemon already serves
...
/**
 * Spawn `agent-meow host --server <url>` and resolve once it reports connected
 ...
 * *adopt* it without spawning a duplicate — `agent-meow host` would otherwise
...
async function ensureHostConnected(cliPath, serverUrl) {
  ...
  // two `agent-meow host` processes for one target.
...
async function connectHost(cliPath, serverUrl, key) {
  ...
  // tunnel, via the fast disk-read + single HTTP probe — not the slow `agent-meow
 ...
 * stored credentials, runs `agent-meow login <url>` (browser/OIDC/Databricks; a
 * no-op when the server needs no auth). Returns ok when already authed, after a
 * successful login, or for a no-auth server; an error (pointing at `agent-meow
...
async function ensureServerAuth(cliPath, serverUrl) {
  ...
  return {
    ok: false,
    error: `Sign-in required — run \`agent-meow login ${serverUrl}\` in a terminal, then try again.`,
  };
}
...
async function startLocalServer(cliPath) {
  ...
  // `agent-meow server status` (a Python cold start). We didn't start it, so no
  // ownership claim.
...
async function shutdown(cliPath) {
  ...
  // during a connect can't leave an orphaned `agent-meow host` process.
  const exits = [];
