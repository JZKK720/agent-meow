...
// page — so it drives the updater over its own `agent-meow:overlay-*` channels
...
//
// It exposes `window.agentMeowDesktop` in the exact shape the web
// `updateBridge()` reads (kind + `updates`), so the reused `UpdateBanner`
// component works unchanged, plus a tiny `agentMeowUpdateOverlay` used only to
...
contextBridge.exposeInMainWorld("agentMeowDesktop", {
  kind: "electron",
  updates: {
    getConfig: () => ipcRenderer.invoke("agent-meow:overlay-get-update-config"),
    getStatus: () => ipcRenderer.invoke("agent-meow:overlay-get-update-status"),
    check: () => ipcRenderer.invoke("agent-meow:overlay-update-check"),
    download: () => ipcRenderer.invoke("agent-meow:overlay-update-download"),
    installNow: () => ipcRenderer.invoke("agent-meow:overlay-update-install"),
    setConfig: (patch) => ipcRenderer.invoke("agent-meow:overlay-set-update-config", patch),
    onStatus: (callback) => {
...
contextBridge.exposeInMainWorld("agentMeowUpdateOverlay", {
  ...
  reportHeight: (height) =>
    ipcRenderer.send("agent-meow:overlay-height", Math.max(0, Math.ceil(height))),
  /** Subscribe to OS/app appearance changes so the card can restyle live. */
  onTheme: (callback) => {
    const listener = (_event, theme) => callback(theme);
    ipcRenderer.on("agent-meow:overlay-theme", listener);
    return () => ipcRenderer.removeListener("agent-meow:overlay-theme", listener);
  },
});
