...
// mirror `agent_meow/server/schemas.py` (`SessionResponse`,
 ...
 * `agent_meow/server/schemas.py:SessionStatusEvent`.
 */
 ...
 *
 * Mirrors `agent_meow.server.schemas.SessionEventInput`.
 */
 ...
 *
 * `queuedItems` is documented in `agent_meow/server/API.md` but is
 ...
 *
 * Mirrors `agent_meow.server.schemas.SessionResponse`.
 */
/**
 ...
 * `agent_meow.server.schemas.ModelUsage`.
 */
...
export interface Session {
  ...
  title: string | null;
  /**
   * Session-scoped guardrails labels (includes `agent_meow.wrapper` /
   * `agent_meow.ui` markers the picker reads, and
   * `agent_meow.fork.source_id` on an unbound coding clone).
   */
 ...
 * `agent_meow.server.schemas.SandboxStatus`. Drives the provisioning
 ...
 * ``agent_meow.server.schemas.SkillSummary``. Just the name +
