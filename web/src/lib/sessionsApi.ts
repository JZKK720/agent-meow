...
// /v1/sessions"). Mirrors `agent_meow/server/routes/sessions.py`.
//
...
/**
 * Wire shape of `ModelUsage` from `agent_meow/server/schemas.py` — one
 ...
 * `agent_meow/server/schemas.py`. Snake-case; converted to the
...
interface SessionResponseWire {
  ...
  items?: SessionItem[];
  // `queued_items` is documented in `agent_meow/server/API.md` but
   ...
   * `agent_meow.server.schemas.SandboxStatus`.
   */
 ...
 * (`agent_meow/server/API.md` §Reconnect Contract), callers should
