 ...
 *   down. The owner reconnects the host (`agent-meow host`); a
 ...
 *
 * 1. `host_offline` — `agent-meow host` re-registers the host
 ...
 *    (`wrapper === "claude-code-native-ui"`) use `agent-meow claude
 *    --resume <id>`; everything else uses the generic `agent-meow run
...
export function buildReconnectCommand({
  ...
  if (state === "host_offline") {
    return ["agent-meow host \\", `  --server ${serverUrl}`].join("\n");
  }
  ...
  return [
    "agent-meow run path/to/agent.yaml \\",
    `  --resume ${conversationId} \\`,
    `  --server ${serverUrl}`,
  ].join("\n");
}
 ...
 *
 * @param wrapper - The conversation's `agent_meow.wrapper` label
 *   (`"claude-code-native-ui"` for `agent-meow claude` sessions). Picks
