...
//
// What we lock:
//   1. host_offline owner → `agent-meow host` (no --resume / YAML),
...
//   4. claude-native local_stranded → `agent-meow claude --resume`.
...
describe("buildReconnectCommand", () => {
  ...
  it("prefers `agent-meow host` for a host_offline claude-native session", () => {
    ...
    expect(cmd).toContain("agent-meow host");
    expect(cmd).not.toContain("agent-meow claude");
  });
  ...
  it("emits the generic run form for a local_stranded session", () => {
    ...
    expect(cmd).toContain("agent-meow run path/to/agent.yaml");
    expect(cmd).toContain("--resume conv_abc123");
    expect(cmd).toContain("--server https://example.databricksapps.com");
    expect(cmd).not.toContain("--profile");
  });

  it("emits `agent-meow claude --resume` for a claude-native local_stranded session", () => {
    const cmd = buildReconnectCommand({
...
describe("<ReconnectSessionDialog />", () => {
  ...
  it("defaults to the Reconnect tab with the host command for a host_offline owner", () => {
    ...
    expect(block.textContent).toContain("agent-meow host");
  ...
  it("shows the run command for a local_stranded session (owner or not)", () => {
    ...
    const block = screen.getByTestId("reconnect-session-command");
    expect(block.textContent).toContain("agent-meow run path/to/agent.yaml");
  ...
  it("shows the claude reattach command for a claude-native local_stranded session", () => {
    ...
    const block = screen.getByTestId("reconnect-session-command");
    expect(block.textContent).toContain("agent-meow claude");
