...
//
//   - `agent_meow.wrapper === "claude-code-native-ui"` → `isClaudeNative`
//   - registered `agent_meow.wrapper` native value    → `isNativeWrapper`
//   - `agent_meow.ui === "terminal"`                  → `isTerminalFirst`
//
...
// label; runner-hosted SDK sessions now stamp `agent_meow.ui` WITHOUT a
// wrapper label (their embedded terminal hosts the agent-meow REPL), so
...
export interface TerminalFirstContextValue {
  /** True when `agent_meow.wrapper === "claude-code-native-ui"`. */
   ...
   * agent-meow REPL terminal are terminal-first but not native).
   */
  isNativeWrapper: boolean;
  /** True when `agent_meow.ui === "terminal"` — gates the toggle + sidebar card. */
