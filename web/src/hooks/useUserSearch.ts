 ...
 * `AgentMeowHostConfig.searchUsers` (see `lib/host.ts`). When no host searcher
...
import { getAgentMeowUserSearch, type UserSuggestion } from "@/lib/host";
...
export function useUserSearch(query: string): UseUserSearchResult {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["agentMeowUserSearch", trimmed],
