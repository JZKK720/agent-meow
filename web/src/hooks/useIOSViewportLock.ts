 ...
 * to `--agentmeow-viewport-height`; the app-shell sizes to that (see index.css).
...
export function useIOSViewportLock(): void {
  useEffect(() => {
    ...
    const apply = () => {
      frame = 0;
      root.style.setProperty("--agentmeow-viewport-height", `${Math.round(viewport.height)}px`);
      resetPan();
    };
    ...
    return () => {
      ...
      root.style.removeProperty("--agentmeow-viewport-height");
    };
  }, []);
}
