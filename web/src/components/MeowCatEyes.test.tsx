...
import { MeowCatEyes } from "./MeowCatEyes";
...
describe("MeowCatEyes", () => {
  it("renders the mascot with image semantics", () => {
    const { container } = render(<MeowCatEyes className="h-18" />);
    const svg = container.querySelector("svg");
    // The new-chat hero is a meaningful image, so the wrapper must override
    // MeowCatIcon's decorative aria-hidden default; losing the override would
  ...
  it("follows last-activity: centered on mount, pointer/caret trading off, focus alone doesn't move it", async () => {
    ...
    const { container } = render(<MeowCatEyes />);
    const svg = container.querySelector("svg");
    if (!svg) throw new Error("MeowCatEyes did not render an svg");
    ...
    const pupils = Array.from(container.querySelectorAll<SVGGElement>("g.meowcat-pupil"));
    expect(pupils).toHaveLength(2);

    // On mount, before any activity, neither pupil has a transform written —
    // the cat rests centered until the user does something.
    await nextFrame();
    ...
    // the cat's gaze to the caret — up and to the LEFT of both eyes, the
  ...
  it("tracks the caret on first input when a field was already focused at mount", async () => {
    // Reproduces the autofocus race: the composer focuses before MeowCatEyes'
    ...
    // Focus BEFORE MeowCatEyes mounts — no focusin reaches the component.
    textarea.focus();
