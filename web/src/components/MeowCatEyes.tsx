...
// Each of the cat's eyes is a fixed near-circular white with a concentric pupil
// drawn on top; the pupil can slide until its rim meets the inner edge of the
// white, i.e. up to (whiteRadius - pupilRadius) away from the eye center.
const VIEWBOX_W = 1024;
const VIEWBOX_H = 1024;
// Radii measured off the main-eye paths in MeowCatIcon.tsx — identical to
// Otto's eye geometry (the eye paths are the same shape, reused for the
// cat mascot so the eye-tracking math carries over unchanged).
...
// ratio (~13% of the white radius) as the previous mascot.
const MAX_OFFSET = Math.min(9.3, WHITE_RADIUS - PUPIL_RADIUS);

// Centers of the cat's two eyes in `g.meowcat-pupil` document order — right
// eye first, then left, matching MeowCatIcon.tsx.
const EYE_CENTERS = [
  { cx: 619.1, cy: 520.6 },
  { cx: 413.8, cy: 520.6 },
];
...
// ranges, etc. carry no caret, so focusing them keeps the cat on the pointer.
...
// Text fields whose caret the cat should follow while focused: <textarea>, a
...
// early, so the cat glanced a line too low):
...
// resolved (whereupon the cat falls back to the pointer).
...
/**
 * The agent-meow cat mascot (橘宝疾风 / MeowCat) with eyes that follow the
 * cursor: each black pupil slides to the inner edge of its white eye on the
 * side nearest whatever the cat is watching.
 *
 * The cat looks at whatever the user last moved: the mouse pointer, or —
 * while a text field (textarea, text input, or contenteditable) is focused
 * — its **text caret**. Moving the mouse pulls the gaze to the pointer even
 * while a field is focused; a genuine caret move (typing, paste/delete,
 * arrow/Home/End navigation, click-to-reposition) pulls it back. The pupils'
 * 90ms transform transition (below) smooths every hand-off.
 *
 ...
 * caret-tracking; the cat starts watching only once the user actually moves
 * the mouse or the caret.
 *
 * The art lives in MeowCatIcon; this component drives its two
 * `g.meowcat-pupil` groups (black disc + glint) through the forwarded ref.
 * Updates are coalesced into a single rAF callback and applied straight to
 * the DOM nodes, so tracking never re-renders React. Respects
 * `prefers-reduced-motion` by leaving the pupils centered.
 */
export function MeowCatEyes({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Class-selector contract with MeowCatIcon (pinned by MeowCatIcon.test.tsx);
    // querySelectorAll fails silently, so a rename would freeze the eyes.
    const pupils = Array.from(svg.querySelectorAll<SVGGElement>("g.meowcat-pupil"));
    ...
    // The focused text field whose caret the cat can watch, or null when no
    // tracked field is focused.
    let activeField: HTMLElement | null = null;
    // What the user last moved decides where the cat looks. null (nothing yet)
    // keeps the pupils centered — the cat only starts tracking on the first
    ...
    const apply = () => {
      ...
      // pointing at a detached node whose rect measures as (0,0) — the cat
      // would aim at the top-left corner. Drop a disconnected field back to
      // the pointer so it rests centered instead.
    ...
    const onMove = (e: PointerEvent) => {
      ...
      // The mouse just moved, so the cat watches the pointer — even if a text
      // field is focused.
      source = "pointer";
      schedule();
    };
    ...
    // the cat's gaze to it. Scoped to the active field so input/click elsewhere
    ...
    // only while the cat is actually watching the caret.
  ...
  return (
    <MeowCatIcon
      ref={svgRef}
      className={className}
      // The hero mascot is meaningful (not decorative), so MeowCatIcon's
      // decorative aria-hidden default is overridden with image semantics.
      role="img"
      aria-label="agent-meow"
      aria-hidden={false}
    />
  );
}