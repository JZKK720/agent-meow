...
/**
 * The `agent-meow://<host>/c/<session_id>` deep link encoded into the share QR
...
function getDeepLink(sessionId: string, rebasePath: (path: string) => string): string {
  ...
  try {
    ...
    return `agent-meow://${host}/c/${sessionId}`;
  } catch {
    // Unparseable transform output: fall back to the current origin's host.
    return `agent-meow://${window.location.host}/c/${sessionId}`;
  }
}
 ...
 * `agent-meow://<host>/c/<id>` deep link so a user can scan it with their phone
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Scan with your phone's camera to open this session in the agent-meow app.
          </DialogDescription>
        </DialogHeader>
        <div className="flex justify-center">
          <div className="rounded-lg bg-white p-3">
            <QRCodeSVG
              ...
              bgColor="#ffffff"
              fgColor="#000000"
              aria-label="QR code to open this session in the agent-meow app"
            />
          </div>
        </div>
        <DialogFooter>
