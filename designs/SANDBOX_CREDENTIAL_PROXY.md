...
> Code: `agent_meow/inner/credential_proxy.py`,
> `agent_meow/inner/egress/proxy.py`, `agent_meow/spec/parser.py`.
...
`agent_meow/inner/egress/`). We extend it to attach credentials. The
...
The parser (`agent_meow/spec/parser.py`, `_parse_credential_proxy`)
...
`agent_meow/inner/credential_proxy.py`:
...
`agent_meow/inner/egress/proxy.py`: `EgressProxy` takes
...
`agent_meow/inner/egress/controller.py` threads `credential_rewrites`
...
- `agent_meow/inner/os_env.py` — `_start_locked` builds a scoped parent
...
- `agent_meow/inner/sandbox.py` — `credential_proxy` field on
