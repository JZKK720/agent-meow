...
`designs/harness-plugin-interface.md` Â§ "Native TUI Harnesses" already names the
...
importing `from agent_meow.<x>_native import run_<x>_native` and calling
...
| enumerations (Â§5) | frozensets/dicts | derive from `native_agents()` / capability flags |
...
2. Populate the built-in contribution with one provider per native agent,
   pointing at the existing `agent_meow.<x>_native` functions.
