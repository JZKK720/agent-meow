...
async function run({
        ...
        pullRequest: {
          closingIssuesReferences: {
            nodes: linkedIssues.map((li) => ({
              number: li.number,
              repository: { nameWithOwner: li.repo || "JZKK720/agent-meow" },
  ...
  const context = {
    repo: { owner: "JZKK720", repo: "agent-meow" },
    payload: { pull_request: {
      number: PR_NUMBER, draft: false,
      user: { login: author },
      // precise fork detection compares head vs base full_name
      head: { repo: { full_name: fork ? "external-contributor/agent-meow" : "JZKK720/agent-meow" } },
      base: { repo: { full_name: "JZKK720/agent-meow" } },
...
(async () => {
  ...
  r = await run({ files: ["agent_meow/db/x.py"], load: { SabhyaC26: 1 } });
  ...
  r = await run({
    files: ["agent_meow/inner/a.py", "agent_meow/tools/b.py"],
  ...
  r = await run({ files: ["agent_meow/inner/foo.py"], fork: false });
  ...
  r = await run({ files: ["agent_meow/inner/foo.py"], author: "dhruv0811" });
