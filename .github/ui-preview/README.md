...
Deploy a live, per-PR preview of the agent-meow web UI as a
...
Unlike agent-meow's production Databricks deploy (`deploy/databricks/`, backed by
...
There is **no LLM or runner baked into the preview** -- agent-meow runs agent
turns on a runner the user connects from their own machine or sandbox
(`meow run … --server <preview-url>`), where the model credentials live. So
