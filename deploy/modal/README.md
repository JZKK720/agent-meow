...
[Modal](https://modal.com) plays two distinct roles for agent-meow:

1. **[Server deploy target](#deploying-the-server)** — run the
   agent-meow server itself on Modal as a single always-on web server
   (`modal_app.py` in this directory).
2. **[Sandbox provider](#sandboxes-for-runner-hosts)** — disposable
   cloud machines for running agent-meow *hosts*, so sessions execute in
...
Run the agent-meow server on Modal as a single always-on web server.
...
`ghcr.io/JZKK720/agent-meow-server:latest`, so a redeploy is an
...
agent-meow hosts — no laptop tethered to a session, no VM to babysit.
...
`modal token new` writes `~/.modal.toml`. Anywhere agent-meow needs to
...
Sandboxes boot from `ghcr.io/JZKK720/agent-meow-host:latest`, an image
published by CI from the `host` target of
[`deploy/docker/Dockerfile`](../docker/Dockerfile) with agent-meow
...
Then point agent-meow at it — `OMNIGENT_MODAL_HOST_IMAGE` for the CLI
...
```bash
meow sandbox connect --provider modal \
...
authentication. The interactive `meow login` browser flow can't
...
- **Claude with an API key** — put `OMNIGENT_ANTHROPIC_API_KEY` in the secret.
  agent-meow resolves it into Claude Code's `apiKeyHelper`; do not also set
...
- **Managed launch hangs then fails.** The server waits up to two
  ...
  inspect the host log inside the sandbox: `/tmp/agent-meow-host.log`.
