...
> Hobby**) with no "never expire" option. agent-meow requests the 24 h
...
**Debian-based** and **single-stage**. The agent-meow host image
...
- **All managed sandboxes share one E2B account + API key.** Cross-user
  isolation between agent-meow users rides on E2B's sandbox boundaries, and
...
- **Sandbox URLs are public by default.** E2B exposes sandbox ports via
  public `*.e2b.app` URLs; agent-meow never opens one (the host dials *out*
