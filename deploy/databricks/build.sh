...
# deployment of agent-meow.
#
# Inputs:
#   SKIP_WEB_UI=1   Skip the web SPA build for API-only deployments.
#
# Outputs:
#   dist/agent-meow-<version>-py3-none-any.whl
...
rm -rf agent_meow/server/static/web-ui dist build agent_meow.egg-info

if [[ "${SKIP_WEB_UI:-}" != "1" ]]; then
    echo "==> Building web SPA into agent_meow/server/static/web-ui/"
    cd web
    npm install
    npm run build
    cd "${REPO_ROOT}"
...
echo "==> Building agent-meow-client wheel"
uv build --wheel --out-dir dist/ sdks/python-client/

echo "==> Building agent-meow-ui-sdk wheel"
uv build --wheel --out-dir dist/ sdks/ui/

echo "==> Building agent-meow wheel"
