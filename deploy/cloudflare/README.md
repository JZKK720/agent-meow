# agent-meow on Cloudflare (Containers + D1 + R2)

Run the agent-meow server on **Cloudflare Containers**, with **D1** as the
...
> Cloudflare D1 isn't yet first-class in agent-meow. It works end to end — it's how
...
- **R2** is the artifact store. Cloudflare container disk is **ephemeral**, so
  ...
  agent-meow's native `S3ArtifactStore`, selected with
