...
description: Run the agent-meow server as a Docker compose stack (server + Postgres) on any Docker host — your laptop, a VPS, EC2 by hand, or as the base layer of any container-platform deploy. Invoke when the user wants to build the image, bring up the compose stack, debug the stack on a host they already have, or extend the stack for a new platform.
---
...
hosts). It is the inverse profile: full agent-meow install plus git +
...
docker compose logs -f agent-meow   # Ctrl-C when you see "Uvicorn running"
```
...
| | |
|---|---|
| `Dockerfile` | Multi-stage build with two final targets. `web-builder` (node:20) runs `npm install && npm run build` on `web/`. `builder` (python:3.12) installs agent-meow into `/opt/venv`; `server-builder` overlays the SPA bundle from `web-builder` and adds psycopg. The default target (`runtime`) copies the venv + `/build/` from `server-builder` and runs `entrypoint.py`. `--target host` builds the host image instead (from `builder`: agent-meow + git/tmux, no SPA/psycopg/entrypoint). |
...
| `docker-compose.yaml` | Two services: `postgres` (16-alpine, persistent volume) and `agent-meow` (built from the Dockerfile, depends on postgres healthcheck). Build context is `../..` (repo root). |
...
docker compose build --no-cache agent-meow
...
|---|---|---|
| Root URL returns `{"service":"agent-meow",…}` instead of the SPA | npm build didn't produce the bundle inside the container | `docker compose exec agent-meow ls /build/agent_meow/server/static/web-ui/` — empty = the `web-builder` stage didn't run cleanly. Rebuild with `--no-cache`. |
