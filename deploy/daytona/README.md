...
>
> 1. The in-sandbox host's dial-back to your agent-meow `server_url` is
...
plain HTTP and WebSocket upgrades — to your real agent-meow server, so a
...
- **All managed sandboxes share one Daytona org + API key.**
  Cross-user isolation between agent-meow users rides entirely on
...
- **No platform lifetime cap.** Unlike Modal's 24-hour limit, Daytona
  sandboxes run until deleted. agent-meow disables Daytona's 15-minute
