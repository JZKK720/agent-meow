...
def test_build_kiro_mcp_config_targets_serve_mcp(tmp_path: Path) -> None:
    ...
    default_cmd = bridge.build_kiro_mcp_config(bridge_dir)["mcpServers"]["agent-meow"]["command"]
...
def test_write_kiro_workspace_mcp_config_merges_preserving_user_servers(tmp_path: Path) -> None:
    """The agent-meow server is merged into <workspace>/.kiro/settings/mcp.json
    ...
    assert set(written["mcpServers"]) == {"user_server", "agent-meow"}
    assert "serve-mcp" in written["mcpServers"]["agent-meow"]["args"]
