...
def test_sandbox_all_symbols_importable() -> None:
    ...
    re-export â€” the failure mode is "looks fine in the source, breaks at
...
def test_sandbox_reexports_are_inner_objects() -> None:
    ...
    Identity (``is``) â€” not equality â€” because anything else means the
...
def test_default_sandbox_for_platform_is_seatbelt_on_macos_regardless_of_binary() -> None:
    ...
    Linux branch, does not probe for ``sandbox-exec`` â€” selection is a
