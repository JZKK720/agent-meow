"""Tests for the generic ACP executor (:mod:`agent_meow.inner.acp_executor`).
...
def test_harness_wrap_requires_command(monkeypatch: pytest.MonkeyPatch) -> None:
    from agent_meow.inner import acp_harness
...
async def test_mcp_relay_starts_and_builds_serve_mcp_entry() -> None:
    ...
    try:
        ...
        assert "agent_meow.claude_native_bridge" in entry["args"]
