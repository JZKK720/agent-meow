...
Covers agent_meow._platform, agent_meow.inner._proc, the harness IPC endpoint
...
from agent_meow import _platform
...
def test_endpoint_uds_variant_shape() -> None:
    from agent_meow.runtime.harnesses.process_manager import _HarnessEndpoint
...
def test_windows_jobobject_is_platform_default() -> None:
    from agent_meow.inner import sandbox
...
def test_windows_jobobject_kill_on_close_terminates_tree() -> None:
    ...
    from agent_meow.inner.windows_jobobject_sandbox import WindowsJobObjectSandboxBackend
...
def test_helper_env_keeps_systemroot_so_child_can_import_asyncio() -> None:
    ...
    # makes any spawned `python -m agent_meow...` die at `import asyncio` with
    # WinError 10106 (Winsock loads providers from %SystemRoot%). The os_env
    # allowlist must carry the Windows system vars.
    from agent_meow.inner.os_env import build_helper_env
    from agent_meow.inner.sandbox import SandboxPolicy
...
def test_parent_death_watchdog_does_not_false_fire_on_windows() -> None:
    ...
    from agent_meow.runner._entry import _parent_is_orphaned
...
def test_host_runner_env_lets_child_import_asyncio_and_resolve_home() -> None:
    ...
    from agent_meow.host.connect import _build_runner_env
