...
from agent_meow.inner.kiro_native_executor import KiroNativeExecutor
...
async def test_kiro_native_executor_injects_latest_user_message(
    ...
    monkeypatch.setattr("agent_meow.inner.kiro_native_executor.inject_user_message", _fake_inject)
...
async def test_kiro_native_executor_surfaces_injection_failure(
    ...
    monkeypatch.setattr("agent_meow.inner.kiro_native_executor.inject_user_message", _fail_inject)
    executor = KiroNativeExecutor(bridge_dir=tmp_path)
...
def test_kiro_native_executor_requires_bridge_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_meow.kiro_native_bridge import KIRO_NATIVE_BRIDGE_DIR_ENV_VAR
