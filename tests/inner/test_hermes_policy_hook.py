...
Omnigent relay tools (surfaced into Hermes as ``mcp_agent_meow_*``) are gated when
...
from agent_meow.inner import hermes_policy_hook
...
def _run(monkeypatch: pytest.MonkeyPatch, tool_name: str) -> tuple[dict, bool]:
    ...
    monkeypatch.setattr("agent_meow.native_policy_hook.post_evaluate_with_retry", _spy, raising=True)
    monkeypatch.setattr(
        "sys.stdin",
...
@pytest.mark.parametrize(
    "tool_name",
    [
        "mcp_agent_meow_sys_session_get_info",  # hermes single-underscore form
        "mcp_agent_meow_sys_os_write",
        "mcp__agent_meow__list_comments",  # native double-underscore form
    ],
)
