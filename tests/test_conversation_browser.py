...
def test_display_server_url_maps_databricks_api_mount(url: str, expected: str) -> None:
    ...
    What this proves: the startup banner shows the workspace ``/agent-meow``
    URL a user recognizes instead of the internal ``/api/2.0/agent-meow``
...
def test_is_workspace_hosted_url(url: str, expected: bool) -> None:
    ...
    server-version row fires for ``/api/2.0/agent-meow`` and nothing else, so
