...
(real stores + mock LLM) so the tests hit the real route ‚Ü?store
...
from agent_meow.server.background_session_titles import BackgroundTitleRequest
from agent_meow.spec.types import SkillSpec
from agent_meow.stores.conversation_store.sqlalchemy_store import (
    SqlAlchemyConversationStore,
)
from agent_meow.stores.host_store import HostStore
from agent_meow.tools.builtins.load_skill import format_skill_meta_text
...
# ‚îÄ‚îÄ POST /v1/sessions with title and labels ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
# ‚îÄ‚îÄ GET /v1/sessions (list) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_list_sessions_kind_filter(
    ...
    # binding ‚Ä?the field the picker harvests for discovery.
...
async def test_list_sessions_includes_workspace_and_host_id(
    ...
    # Register the host first ‚Ä?conversations.host_id is an FK to the
    ...
    # exclude_none drops unset fields entirely ‚Ä?not present, not null.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Sidebar treats this as "something is happening" ‚Ä?wire
...
async def test_session_snapshot_defaults_terminal_pending_false(
    ...
    for a terminal-first session once its terminal has landed ‚Ä?the
...
async def test_session_snapshot_reflects_terminal_pending_cache(
    ...
    the value travels cache ‚Ü?response builder ‚Ü?snapshot.
    """
    from agent_meow.server.routes import sessions as sessions_module
...
async def test_external_session_status_event_lands_in_status_cache(
    ...
    sidebar stuck on "idle" while the chat showed "Working‚Ä?.
    """
    from agent_meow.server.routes import sessions as sessions_module
...
# ‚îÄ‚îÄ POST /v1/sessions/{id}/events external_session_superseded ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_external_session_superseded_publishes_redirect_event(
    ...
    the new one. The event is transient (not persisted) ‚Ä?the handler only
...
async def test_external_session_superseded_drains_pending_inputs(
    ...
    try:
        ...
        # Drained ‚Ä?so it won't reappear from the snapshot on reload, and it was
...
# ‚îÄ‚îÄ POST /v1/sessions/{id}/events external_subagent_start ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_external_subagent_start_mints_child_session(
    ...
    assert child["labels"]["agent_meow.claude_native.description"] == "Trace the auth flow"
...
async def test_external_subagent_start_handles_duplicate_agent_type_and_description(
    ...
    joke"), and a na√Øve ``title = f"{agent_type}:{description}"``
...
async def test_external_subagent_start_is_idempotent_on_subagent_id(
    ...
    children = (await client.get(f"/v1/sessions/{parent['id']}/child_sessions")).json()["data"]
    # Pin "exactly one" rather than ">= 1" ‚Ä?a duplicate would slip
...
async def test_external_subagent_start_adopts_unlabeled_title_collision(
    ...
    the forwarder parked the sub-agent ‚Ä?it silently never appeared in
    ...
    # the claude-native labels ‚Ä?exactly what a crash between
    ...
    # 200/202, NOT 500 ‚Ä?the duplicate-title collision must be adopted,
    ...
    assert child["labels"]["agent_meow.claude_native.subagent_id"] == "a5c7effac5a9a35ab"
    assert child["labels"]["agent_meow.claude_native.description"] == "Trace the auth flow"
...
async def test_external_subagent_start_rejects_missing_required_keys(
    ...
    400 ‚Ä?payload validation is at the route boundary so the handler
...
# ‚îÄ‚îÄ POST /v1/sessions/{id}/events skill slash_command ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_skill_slash_command_persists_visible_item_and_hidden_meta_message(
    ...
    # must come from the visible "/name args" text ‚Ä?the hidden meta
...
async def test_skill_slash_command_keeps_existing_title(
    ...
    invoked later ‚Ä?otherwise every mid-conversation ``/skill`` send
...
async def test_external_user_message_folds_pending_image_into_durable_item(
    ...
    (``external_conversation_item`` with no image block), so the image ‚Ä?
    which the client uploaded and POSTed ‚Ä?would be dropped from
    ...
    try:
        ...
        # ‚Ä?so reloading history shows the image, not just the caption.
        ...
        # The pending entry was drained ‚Ä?it won't double-render on rebind.
        assert pending_inputs.snapshot_for(session["id"]) == []
    finally:
        pending_inputs.reset_for_tests()
...
# ‚îÄ‚îÄ PATCH /v1/sessions/{id} ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_create_session_sets_terminal_launch_args(
    ...
    value travelled route ‚Ü?create_conversation ‚Ü?DB ‚Ü?response
...
async def test_create_session_without_terminal_launch_args_is_null(
    ...
    NULL ‚Ä?a non-native / auto-mode-off session stores nothing.
...
async def test_patch_session_sets_terminal_launch_args(
    ...
    value travelled route ‚Ü?store ‚Ü?DB ‚Ü?response builder. If the
    builder dropped the field or the store didn't persist it, the GET
    snapshot would carry ``None`` and this would fail ‚Ä?the exact gap
...
async def test_patch_session_replaces_terminal_launch_args(
    ...
    appending ‚Ä?the resume last-write-wins contract
    (designs/NATIVE_RUNNER_SERVER_LAUNCH.md).

    If the route/store appended, the final value would be
    ``["--model", "opus", "--verbose"]`` and this would fail ‚Ä?which
...
async def test_patch_session_rejects_invalid_reasoning_effort(
    ...
    unchanged. Mirrors the POST-create validation contract ‚Ä?the two
    ...
    # 400 + typed code: required by DESIGN_PRINCIPLES.md ¬ß5 "fail
    ...
    # The DB row keeps the prior value ‚Ä?validation runs before any
...
async def test_patch_session_sets_external_session_id(
    ...
    # Response body is the updated snapshot ‚Ä?the wrapper bridge
...
async def test_patch_session_external_session_id_rejects_overwrite(
    ...
    it to ``invalid_input``. The original value must survive ‚Ä?a
...
async def test_create_session_returns_null_external_session_id(
    ...
    observed" from "set to empty" ‚Ä?a non-null default would mean
...
# ‚îÄ‚îÄ claude-native session discovery (list + snapshot) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_claude_native_session_discoverable_with_terminal_metadata(
    ...
    UI needs to find and open it ‚Ä?on both the list and the snapshot."""
...
async def test_get_session_agent_name_is_spec_name_after_switch(
    ...
    ``"<builtin> (switch ag_‚Ä?"`` for agent-store disambiguation, but
    clients (REPL toolbar, web sidebar) display ``agent_name``
    verbatim ‚Ä?the snapshot must surface the spec's clean identity
    (e.g. ``"claude-native-ui"``), not the clone row's name.

    Drives the REAL switch route end-to-end: source session ‚Ü?seeded
    bindable built-in ‚Ü?``POST .../switch-agent`` ‚Ü?``GET`` snapshot.
    """
    ...
    # "(switch ‚Ä?" disambiguation suffix ‚Ä?i.e. row name ‚â?spec name.
    ...
    # would display "claude-native-ui (switch ag_‚Ä?" to the user.
    assert snap["agent_name"] == "claude-native-ui"
...
async def test_list_sessions_exposes_pending_elicitations_count(
    ...
    (``record_publish`` + ``resolve``) ‚Ä?the route handler reads
    ...
    index's lookup would surface as a missing or stale badge ‚Ä?
    ...
    # Field present, default 0 ‚Ä?guards against accidentally
...
async def test_list_sessions_pending_count_falls_back_to_row_for_bound_session(
    ...
    only consults the row when the session is runner-bound ‚Ä?this pins that
    ...
    # Bind a runner and seed the persisted count directly ‚Ä?simulating the
    ...
    row = next((r for r in resp.json()["data"] if r["id"] == session_id), None)
    assert row is not None
    # Index empty + row=2 + runner-bound ‚Ü?fallback surfaces the row's count.
...
async def test_get_session_slim_skips_items_and_liveness(
    ...
    # Items read skipped ‚Ä?empty list, not an absent field, so clients
    # that parse `items ?? []` see the same shape either way.
    assert slim_body["items"] == []
    # Liveness lookup skipped ‚Ä?both fields unset (serialized as absent
...
async def test_get_session_replays_pending_elicitations(
    ...
    never render ‚Ä?clicking the sidebar-badged row would land on a
    ...
    assert resp.status_code == 200
    body = resp.json()
    # Field present even when empty ‚Ä?keeps the wire shape stable.
    ...
    # Full event shape survives ‚Ä?UI re-parses through the same SSE
...
# ‚îÄ‚îÄ GET /v1/sessions/{id}/items ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
# ‚îÄ‚îÄ GET /v1/sessions/{id} snapshot fields ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_post_external_function_call_output_caps_oversized_output(
    ...
    one giant SSE frame. This route must bound it ‚Ä?the native analog of the
    ...
    # 2 MiB ‚Ä?comfortably over the 1 MiB cap so truncation is unambiguous.
    ...
    # The broadcast SSE frame is capped too (same item object) ‚Ä?this is the
...
async def test_external_transcript_items_recoverable_via_snapshot_by_item_id(
    ...
    # Equal sets ‚á?deduping snapshot ‚à?live-stream by id loses nothing
    # and duplicates nothing.
    assert streamed_ids == set(snapshot_ids), (
        f"streamed vs snapshot ids diverge ‚Ä?"
...
async def test_post_external_session_status_failed_surfaces_output_and_reauth(
    ...
    *top-level* session sees the reason ‚Ä?not only the sub-agent parent path.
...
async def test_publish_status_keeps_failed_sticky_against_trailing_idle(
    ...
    (‚Ü?``failed``), but the now-quiet pane then makes the runner's
    ...
    sticky-``failed`` guard in ``_publish_status`` ‚Ä?without it, the red
    ...
    # The idle posted right after ``failed`` is absent ‚Ä?the error
...
async def test_post_interrupt_without_data_field_is_accepted(
    ...
    was broken in production ‚Ä?this pins the default.
    """
    published: list[tuple[str, dict[str, Any]]] = []
...
async def test_post_external_output_text_delta_carries_streaming_identifiers(
    ...
    preview with the final item) ‚Ä?including the falsy ``index: 0`` /
    ...
    # All four fields survive ‚Ä?including index 0 and final False, which a
...
async def test_post_external_session_status_rejects_unknown_status(
    ...
    would silently drop the event downstream ‚Ä?exactly the kind of
...
async def test_external_session_usage_broadcasts_parent_subtree_cost_not_own(
    ...
    own ‚á?subtree). The broadcast must match the GET snapshot's subtree total.
    """
    published: list[tuple[str, dict[str, Any]]] = []
    monkeypatch.setattr(
        "agent_meow.server.routes.sessions.session_stream.publish",
    ...
    # live badge regressed to own-cost (the bug) ‚Ä?it would show $1.00 right
...
async def test_external_session_usage_policy_cost_only_post_accepted(
    ...
    at the last S ‚Ä?otherwise the gate would stop seeing in-flight spend, or
...
async def test_external_session_usage_cumulative_cost_is_set_not_added(
    ...
    """
    Successive cumulative-cost posts SET (not accumulate) ‚Ä?native reports
...
async def test_external_session_usage_cost_is_monotonic(
    ...
    are clamped monotonic so such a post is a no-op ‚Ä?it can't reset the gate to
    ...
    # Falsified low report ‚Ä?must be ignored, not stored.
...
async def test_external_session_usage_codex_tokens_priced(
    ...
    monkeypatch.setattr(
        "agent_meow.llms.context_window.fetch_model_pricing",
...
async def test_external_session_usage_codex_cached_tokens_priced_at_cache_rate(
    ...
    billed at the full input rate ‚Ä?the over-report this fix targets.
    """
    ...
    # Without the split it would be 0.002 (1000*1e-6 + 500*2e-6) ‚Ä?so this
...
async def test_external_session_usage_codex_cached_tokens_no_catalog_cache_rate(
    ...
    0.00128) ‚Ä?the over-charge this fixes.
    """
    # No cache_read_per_token ‚á?compute_llm_cost derives it as 0.10x input.
    monkeypatch.setattr(
        "agent_meow.llms.context_window.fetch_model_pricing",
        lambda model: ModelPricing(input_per_token=1e-6, output_per_token=2e-6),
    )
...
async def test_accumulate_session_usage_prices_from_usage_model(
    ...
    ``"harness-model"`` is priceable ‚Ä?the agent's spec model (its name) is
    NOT ‚Ä?so a recorded cost proves the harness-reported model was used, not
    the spec model.
    """
    ...
    usage = _read_session_usage(db_uri, session["id"])
    # cost = 1000*1e-6 + 500*2e-6 = 0.002 ‚Ä?priced via usage.model despite the
...
async def test_accumulate_session_usage_unpriced_without_usage_model(
    ...
    """No ``usage.model`` and an unpriceable spec model ‚á?no cost recorded.

    Guards the fallback chain and the "unpriced ‚á?no ``total_cost_usd`` key"
    contract: when neither the harness nor the catalog can price the turn, the
    cost key stays absent (so the UI shows "‚Ä?, not a misleading $0.00).
    """
    from agent_meow.server.routes import sessions as sessions_routes
...
async def test_accumulate_session_usage_records_per_model_breakdown(
    ...
    flat session ``total_cost_usd`` ‚Ä?the no-double-count invariant the UI
    ...
    # Per-model costs sum to the flat total ‚Ä?proves no double-count / drop.
...
async def test_accumulate_session_usage_unpriced_model_has_tokens_no_cost(
    ...
    Mirrors the flat "priced ‚ü?``total_cost_usd`` key present" contract at the
...
async def test_accumulate_session_usage_concurrent_calls_accumulate_both_deltas(
    ...
    compute their own delta, and overwrite the other's result ‚Ä?silently dropping
    a delta. The fix (increment_session_usage ‚Ä?single atomic transaction with
    ...
    def _call(input_tokens: int, output_tokens: int) -> None:
        ...
        # the concurrency is real ‚Ä?not short-circuited by a shared connection.
        store = SqlAlchemyConversationStore(db_uri)
        sessions_routes._accumulate_session_usage(
            {
                "usage": {
...
async def test_external_session_usage_cost_only_attributes_to_model(
    ...
    ``by_model`` entirely ‚Ä?the TOKEN USAGE panel undercounted the session
    ...
    # Per-model cost reconciles with the flat session total ‚Ä?the exact gap
...
async def test_external_session_usage_policy_cost_only_skips_attribution(
    ...
    so attribution is skipped ‚Ä?only the priced display cost flows into
...
async def test_external_session_usage_unpriced_omits_cost(
    ...
    """
    An unpriced session omits ``total_cost_usd`` everywhere ‚Ä?event and store.
    ...
    ``session.usage`` event must not carry it, so the web UI renders "‚Ä?
    ...
    by the absence.
    """
    # Pricing unavailable for the model ‚Ä?the unpriced path.
    monkeypatch.setattr(
        "agent_meow.llms.context_window.fetch_model_pricing",
        lambda model: None,
    )
...
async def test_session_snapshot_unpriced_cost_is_none(
    ...
    must carry ``None`` (rendered "‚Ä? by the client) rather than ``0.0`` ‚Ä?
...
# PreToolUse hook) ‚Ä?see omnigent/policies/builtins/cost.py. The old
...
async def test_external_session_usage_over_budget_does_not_stop_session(
    ...
    must NOT set the deliberate-stop label ‚Ä?a regression re-adding a
    ...
    assert conv.labels.get("agent_meow.stopped") != "true"
...
async def test_relay_tool_call_ask_approval_persists_checkpoint(
    ...
    crossed cost checkpoint) must be persisted ON ACCEPT ‚Ä?otherwise the
    ...
    """
    from agent_meow.policies.builtins.cost import _ASK_APPROVED_KEY
    ...
    # First tool-call query ‚Ü?ASK parked as a pending elicitation.
    verdict = await _evaluate_tool(client, sid)
    assert verdict["verdict"] == "pending", verdict
    eid = verdict["elicitation_id"]

    # Checkpoint not recorded yet ‚Ä?it must land only on approve.
    assert (store.get_conversation(sid).session_state or {}).get(_ASK_APPROVED_KEY) is None

    # Approve ‚Ü?the deciding policy's checkpoint must persist server-side.
    resp = await client.post(
        f"/v1/sessions/{sid}/events",
...
async def test_relay_tool_call_ask_decline_does_not_record_checkpoint(
    ...
    POLICIES.md ¬ß7.2: a denied ASK leaves no trace. So a decline must NOT
    persist the checkpoint ‚Ä?the next tool call re-asks (the user did not
    ...
    assert resp.status_code == 202, resp.text
    # Declined ‚Ü?checkpoint NOT recorded; the next tool call re-asks.
    assert (store.get_conversation(sid).session_state or {}).get(_ASK_APPROVED_KEY) is None
...
async def test_mcp_relay_tool_call_ask_approval_persists_checkpoint(
    ...
    approved retry ‚Ä?otherwise it is dropped and re-prompts every call. This
    ...
    # First tools/call ‚Ü?over the $0.05 checkpoint ‚Ü?ASK (input_required).
    resp = await client.post(
    ...
    # Retry carrying the approval ‚Ü?the deferred checkpoint must persist
...
async def test_post_external_model_change_dedupes_when_unchanged(
    ...
    This is the web‚ÜíTUI round-trip ‚Ä?the web PATCH set ``model_override``
    ...
    assert resp.status_code == 202, resp.text
    # Already on "opus" ‚Ä?nothing re-published.
    assert [event["type"] for _, event in published] == []
...
async def test_post_external_model_change_does_not_forward_to_runner(
    ...
    back would loop (runner injects ``/model`` ‚Ü?transcript records the
    model ‚Ü?forwarder posts ``external_model_change`` ‚Ü?...). This guards
    that loop boundary ‚Ä?note the contrast with the PATCH path
...
async def test_post_external_model_options_populates_picker_and_publishes(
    ...
    harness actually loaded ‚Ä?regardless of how it authenticated. The route
    ...
    session = await _create_session(
        client,
        agent["id"],
        labels={"agent_meow.ui": "terminal", "agent_meow.wrapper": "pi-native-ui"},
    )
        ...
        json={
            ...
            "data": {
                "models": [
                    ...
                    # No displayName ‚Ü?falls back to the id.
...
async def test_post_external_model_options_empty_evicts_cache(
    ...
    Empty means "no catalog to show" ‚Ä?the entry is evicted so the picker
...
async def test_post_external_model_options_rejects_non_pi_native_session(
    ...
    agent = await create_test_agent(client)
    # A plain (non-native) session ‚Ä?no pi-native wrapper label.
    session = await _create_session(client, agent["id"])
...
async def test_post_external_codex_collaboration_mode_change_persists_label(
    ...
    snapshot = (await client.get(f"/v1/sessions/{session['id']}")).json()
    assert snapshot["labels"]["agent_meow.codex_native.collaboration_mode"] == "plan"
...
async def test_patch_model_override_records_system_note_for_inprocess_session(
    ...
    monkeypatch.setattr(
        "agent_meow.server.routes.sessions._forward_session_change_to_runner",
        _noop_forward,
    )
    agent = await create_test_agent(client)
    session = await _create_session(client, agent["id"])
...
async def test_patch_model_override_skips_note_for_native_session(
    ...
    ``agent_meow.ui == "terminal"``) must NOT get an injected note ‚Ä?
    ...
    label, NOT ``agent_meow.ui`` alone ‚Ä?see
    ``test_patch_model_override_records_note_for_terminal_view_sdk_session``
    for the chat-first SDK session that has ``agent_meow.ui`` but no wrapper
    ...
    assert patch.status_code == 200, patch.text
    # Gate excludes native-wrapper sessions ‚Ä?no transcript note.
...
async def test_patch_model_override_records_note_for_terminal_view_sdk_session(
    ...
    in-process claude-sdk agent whose history Omnigent writes ‚Ä?so a web
    ...
    session = await _create_session(
        client,
        agent["id"],
        # Terminal VIEW only ‚Ä?no native wrapper. Mirrors a polly/debby
    ...
    assert patch.status_code == 200, patch.text
    # Note IS recorded: ``agent_meow.ui == "terminal"`` alone must not suppress
    # it. An empty list here means the gate regressed to keying on
    # ``agent_meow.ui``, re-breaking the polly/debby web ``/model`` feedback.
    assert _model_change_notes(published) == [
...
async def test_patch_model_override_silent_skips_note(
    ...
    """
    A ``silent`` PATCH (bind-time auto-apply) must NOT record a note ‚Ä?only
...
async def test_post_external_mcp_startup_publishes_session_mcp_startup(
    ...
    try:
        ...
        # An empty (settled) map evicts the cache ‚Ä?the snapshot goes back
...
async def test_post_external_mcp_startup_all_ready_evicts_snapshot_cache(
    ...
    nothing ‚Ä?a blank conversation area.
    """
    published: list[tuple[str, dict[str, Any]]] = []
    monkeypatch.setattr(
        "agent_meow.server.routes.sessions.session_stream.publish",
...
async def test_external_user_message_seeds_title_on_claude_native_session(
    ...
    creates sessions without a title ‚Ä?same shape as every other
    ...
    session = await _create_session(
        client,
        agent["id"],
        # No title ‚Ä?claude-native wrapper no longer stamps one.
        labels={
            "agent_meow.ui": "terminal",
            "agent_meow.wrapper": "claude-code-native-ui",
        },
...
async def test_interrupt_on_claude_native_session_skips_idle_publish_on_runner_failure(
    ...
    is the right signal ‚Ä?it tells the user the cancel didn't land.
    ...
    naturally surfaces as "no idle" ‚Ä?the pane keeps changing). This
    test acts as a regression guard against re-adding an AP-side idle
    publish ‚Ä?if someone reintroduces the pre-refactor "publish idle on
    2xx" logic, the 503 path here would start leaking idle.
    """
    from agent_meow.runtime import session_stream, set_runner_client

    def _handler(request: httpx.Request) -> httpx.Response:
        """Return 503 ‚Ä?the bridge-not-ready shape from the runner."""
        del request
    ...
    assert not idle_status, (
        ...
        f"rejected the interrupt ‚Ä?the spinner staying is the correct "
...
async def test_stop_session_forwards_stop_session_event_to_runner(
    ...
    the right body ‚Ä?a regression that dropped the forward, mangled
...
async def test_stop_session_surfaces_runner_failure_as_error(
    ...
    dialog as if the session stopped ‚Ä?the exact silent-failure the
    ...
    # RUNNER_UNAVAILABLE ‚Ü?503. A 202 here would mean the failure was
    # swallowed and the UI would falsely report success.
    assert resp.status_code == 503, (
        f"A runner 503 on stop_session must surface as a non-2xx to the "
        f"client (RUNNER_UNAVAILABLE ‚Ü?503), got {resp.status_code}: {resp.text}"
    )
    ...
    from agent_meow.server.routes.sessions import _interrupt_fenced_sessions
...
async def test_stop_session_no_runner_lifts_stop_fence(
    ...
    try:
        ...
        assert session_id not in _interrupt_fenced_sessions, (
            "a no-runner stop_session must remove the fence it installed ‚Ä?"
...
async def test_interrupt_forward_failure_lifts_stop_fence(
    ...
    If the forward fails the turn keeps running ‚Ä?but the fence used to
    ...
    try:
        ...
        assert resp.status_code == 202, resp.text
        assert session_id not in _interrupt_fenced_sessions, (
            "a failed interrupt forward must remove the fence it installed ‚Ä?"
...
async def test_patch_collaboration_mode_persists_label_and_forwards_event(
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["labels"]["agent_meow.codex_native.collaboration_mode"] == "plan"
...
async def test_patch_collaboration_mode_requires_live_runner_before_persisting(
    ...
    assert "agent_meow.codex_native.collaboration_mode" not in snapshot["labels"]
    assert published == []
...
@pytest.mark.parametrize(
    ...
    [
        # (1) Native + claude-accepted level ‚Ü?POSTs effort_change.
        # The motivating case: dropdown click on a running pane.
        (True, "high", "high", "high"),
        # (2) Non-native + same level ‚Ü?Omnigent server is harness-agnostic
        ...
        # (3) Clear on native session ‚Ü?persisted None gets forwarded
        ...
        # ``minimal``). After refactor Omnigent no longer filters ‚Ä?it
...
async def test_patch_reasoning_effort_forwards_effort_change_event(
    ...
    runner ``/events`` ‚Ä?harness-agnostic on the Omnigent side.
    ...
    * The body is the new ``effort_change`` discriminator with the
      persisted level (or ``None`` for clear) ‚Ä?runner-side dispatch
      decides what to do with it.
    * Omnigent server does not check ``_is_native_terminal_session``
      and does not filter on level ‚Ä?every PATCH that changes effort
      sends the event.
    ...
    # happen anymore ‚Ä?its callsite is gone from agent_meow server. A non-
...
async def test_silent_patch_skips_effort_change_forward(
    ...
    has sent anything. The persisted value is still authoritative ‚Ä?
    ...
    try:
        ...
        # 200 with persisted value ‚Ä?silent only suppresses the live
    ...
    # item into a fresh pane ‚Ä?the bug this skip exists to prevent.
    effort_forwards = [
        f
        for f in captured
...
async def test_patch_reasoning_effort_swallows_runner_failure(
    ...
    def _handler(request: httpx.Request) -> httpx.Response:
        """Record POST + 503 ‚Ä?the runner-route's pane-not-ready shape."""
        if request.method != "POST":
            return httpx.Response(204)
        body: dict[str, Any] | None = None
        if request.content:
    ...
    try:
        ...
        # 200 even though the runner 503'd ‚Ä?clicking the dropdown
    ...
    # enough to talk to the runner ‚Ä?i.e. the failure was swallowed,
...
# ‚îÄ‚îÄ POST /v1/sessions/{id}/events with client-side tools ‚îÄ‚îÄ‚îÄ‚îÄ
...
# ‚îÄ‚îÄ POST /v1/sessions/{id}/events external_codex_subagent_start ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
...
async def test_external_codex_subagent_start_mints_child_session(
    ...
    # tool is derived from agent_nickname ‚Ü?agent_role ‚Ü?"Codex".
    ...
    labels = child["labels"]
    assert labels["agent_meow.codex_native.subagent_thread_id"] == "thread_child_alpha"
    assert labels["agent_meow.codex_native.parent_thread_id"] == "thread_parent"
    assert labels["agent_meow.codex_native.collab_tool_call_id"] == "collab_123"
    assert labels["agent_meow.codex_native.agent_nickname"] == "auth-auditor"
    assert labels["agent_meow.codex_native.agent_role"] == "reviewer"
...
async def test_external_codex_subagent_start_is_idempotent_and_upserts_labels(
    ...
    # First registration ‚Ä?sparse (no nickname yet).
    first = await client.post(
        f"/v1/sessions/{parent['id']}/events",
        json={
            "type": "external_codex_subagent_start",
    ...
    # Second registration ‚Ä?richer (nickname/role added from resume).
    second = await client.post(
        f"/v1/sessions/{parent['id']}/events",
        json={
            "type": "external_codex_subagent_start",
    ...
    # Must return the same child id ‚Ä?no duplicate row minted.
...
async def test_external_codex_subagent_start_adopts_unlabeled_title_collision(
    ...
    # 202, NOT 500 ‚Ä?the collision must be adopted, not escape as an
    ...
    # normal label lookup.
    assert (
        children[0]["labels"]["agent_meow.codex_native.subagent_thread_id"] == "thread_child_gamma"
    )
...
async def test_external_codex_subagent_terminal_status_accepted_without_runner(
    ...
    # Must succeed ‚Ä?no runner delivery is required for Codex-internal children.
...
async def test_native_message_persisted_when_runner_offline(
    ...
    RUNNER_UNAVAILABLE and silently drop the message ‚Ä?the user's input
    ...
    """
    agent = await create_test_agent(client)
    # Native terminal-first session, no host/runner bound ‚Ü?runner offline.
    session = await _create_session(
        client,
        agent["id"],
        labels={"agent_meow.wrapper": "claude-code-native-ui"},
    ...
    # The user's message is durable history ‚Ä?the whole point of the fix.
    user_msgs = [
        i
        for i in items
    ...
    # which needs a bound runner_id ‚Ä?covered by the snapshot unit test.)
...
async def test_non_native_message_still_raises_when_runner_offline(
    ...
    would desync the store from harness state ‚Ä?it must keep raising.
    """
    agent = await create_test_agent(client)
...
async def test_message_forward_failure_surfaces_runner_unavailable(
    ...
    accepted. The message was persisted but never delivered ‚Ä?the runner
