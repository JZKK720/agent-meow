...
from agent_meow.server.routes import session_mcp_servers as mcp_routes
...
async def test_create_mcp_server_supports_single_yaml_bundle(client: httpx.AsyncClient) -> None:
    """Single-file agent-meow YAML bundles are updated inline."""
...
def _single_yaml_bundle(yaml_text: str) -> bytes:
    """Build a tar.gz bundle containing one agent-meow YAML file."""
    buf = io.BytesIO()
