...
def _fake_runner_returning(compact_status: int) -> tuple[httpx.AsyncClient, list[dict[str, Any]]]:
    ...
    the test can assert the agent-meow server actually forwarded the control,
...
async def test_compact_skips_omnigent_compaction_when_runner_handles_it(
    ...
    the agent-meow server skip its own compaction.
    ...
    async def _must_not_run(**_: Any) -> CompactionResult:
        ...
        raise AssertionError(
            ...
            "reported it handled /compact (200). The agent-meow server fell "
    ...
    # handled it, agent-meow returned without running (or raising from) its own
    # compaction.
...
async def test_external_compaction_status_publishes_compaction_sse(
    ...
    post-compaction SessionStart (completed) hooks. agent-meow must translate it
