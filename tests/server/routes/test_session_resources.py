...
async def test_claude_native_message_forwards_to_runner_without_persisting(
    ...
    """
    Claude-native web-chat input is runner injection, not agent-meow persistence.

    This fails if the route regresses to the legacy create-or-steer
    path, which would either start a duplicate agent-meow agent task or make
...
async def test_claude_native_message_surfaces_runner_sse_failure(
    ...
    This fails if the agent-meow route treats the runner's streaming HTTP 200
...
# â”€â”€ Phase 1b: typed collections & single-resource proxy tests â”€â”€â”€â”€
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # INTERNAL_ERROR (â†’500) and a message carrying the raw status.
        (
            500,
            {},
            500,
...
# â”€â”€ Phase 1c: session-scoped file endpoint tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Phase 1d: integration hardening tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Server filesystem proxy tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Server shell proxy test â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Conversation deletion cleanup test â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Resource event persistence tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def test_relay_persists_terminal_resource_created_from_runner() -> None:
    ...
    ``session.resource.created`` on its SSE stream. The agent-meow relay must
...
async def test_native_dispatch_fast_fails_and_consumes_message_on_terminal_error() -> None:
    ...
    The agent-meow server first asks the runner to ensure the native terminal.
    When the runner returns a structured startup failure, agent-meow must not
    ...
    # the input pending forever; two would mean the agent-meow fast-fail path
...
async def test_native_dispatch_reports_malformed_runner_error_body() -> None:
    ...
    instead of the structured native startup error JSON, agent-meow still fast
...
async def test_native_dispatch_transport_error_does_not_fallback_to_forwarding() -> None:
    ...
    If agent-meow cannot reach the runner's terminal ensure endpoint, it must
