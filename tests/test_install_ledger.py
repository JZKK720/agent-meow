...
from agent_meow import install_ledger


def test_install_ledger_round_trip_and_mode(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("AGENT_MEOW_DATA_DIR", str(tmp_path / ".omnigent"))
...
def test_write_install_ledger_from_env_records_profile_and_dep_provenance(
    ...
    monkeypatch.setenv("AGENT_MEOW_DATA_DIR", str(state))
    monkeypatch.setenv("AGENT_MEOW_LEDGER_PROFILE", str(profile))
    monkeypatch.setenv("AGENT_MEOW_LEDGER_DEP_UV", "omnigent")
...
def test_deep_backfill_observes_external_config_and_launch_agents(
    ...
    launch_agent = launch_dir / "ai.agent_meow.local.plist"
