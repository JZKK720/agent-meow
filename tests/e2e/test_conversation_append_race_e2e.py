...
This test runs a REAL agent-meow server subprocess against a REAL SQLite
...
1. The store is constructed in a separate process from the
   test (matches the user's ``agent-meow run`` setup
...
- Spins up an agent-meow server subprocess (:fixture:`ap_server`).
- Imports the agent-meow server's actual ``SqlAlchemyConversationStore``
...
def _find_free_port() -> int:
    """Pick a free TCP port for the agent-meow subprocess to bind."""
    s = socket.socket()
    s.bind(("", 0))
...
def test_concurrent_appends_against_live_omnigent_server_db_no_collision(
    ...
    """
    With a live agent-meow server running, fire N concurrent appends
    ...
    - The lock-escalation fix in
      ...
      (the agent-meow server). On revert, the test surfaces the user's
    ...
    Note: this test does NOT exercise the agent-meow route layer (the
    ...
    # this gets a separate engine in this process (the agent-meow server
