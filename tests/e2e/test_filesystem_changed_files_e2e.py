...
# ``AGENT_MEOW_RUNNER_WORKSPACE``. That leaves the runner with no filesystem
...
def fs_ws_server(
    ...
    The runner is given ``AGENT_MEOW_RUNNER_WORKSPACE=fs_workspace`` (and the
    ...
    env: dict[str, str] = {
        **os.environ,
        ...
        # agent-meow; only the *workspace* (cwd / AGENT_MEOW_RUNNER_WORKSPACE) is
    ...
    runner_proc = subprocess.Popen(
        ...
        env={
            **env,
            "AGENT_MEOW_RUNNER_ID": fs_ws_runner_id,
            ...
            "AGENT_MEOW_RUNNER_WORKSPACE": str(fs_workspace),
        },
        cwd=str(fs_workspace),
...
def test_filesystem_changes_appear_after_agent_write(
    ...
    shared ``live_server``: the runner needs ``AGENT_MEOW_RUNNER_WORKSPACE`` set so
