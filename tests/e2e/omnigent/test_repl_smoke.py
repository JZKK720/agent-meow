"""Phase 0 characterization test â€” REPL smoke under pexpect.

Spawns ``agent-meow run <yaml>`` under a PTY, waits for the
REPL's ``â¯`` input prompt, types a prompt, awaits the turn
...
driven off the visible ``â ¹ working`` activity line and the
``â¯`` input prompt that re-renders when the turn settles â€”
...
a PTY. Likewise the user/assistant turns now render as a ``â¯``
echo and a ``â—† <model>`` header â€” the legacy ``You>`` / ``Agent>``
...
- ``omnigent.cli._run_agent`` REPL entrypoint stops booting
  ...
  handling regression).
- The REPL stops echoing the submitted prompt with its ``â¯``
  marker â€” the submission never reached the Session.
...
# turn streams; ``â¯ `` is the input prompt it re-renders when the
...
# REPL always echoes the prompt, the ``â¯`` marker, spinner
# frames, and a status line â€” even a failed turn produces more
...
def test_repl_smoke_single_prompt(
    ...
    try:
        # Boot readiness: the visible ``â¯`` input prompt. Prefer it
        ...
        # Drain anything rendered between the settled ``â¯`` prompt
        # and Ctrl+D â€” the post-turn render often repaints the
    ...
    observed: dict[str, Any] = {
        ...
        # The ``â¯`` marker is the REPL's deterministic echo of the
        # user's submitted prompt (``RichBlockFormatter.user_message``
        # renders ``â¯ <text>``); its presence with the prompt text
        # proves the submission reached the Session, not just the
        # input buffer. Replaces the removed ``You>`` banner.
        "user_prompt_echoed": "â¯" in combined_stripped and _PROMPT in combined_stripped,
    }
    ...
    assert len(turn.stripped) >= _MIN_STRIPPED_TURN_CHARS, (
        ...
        f"sleepâ†’runningâ†’sleep oscillation.\n\n"
