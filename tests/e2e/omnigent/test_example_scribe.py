...
spec-load â€” no LLM, no credentials â€” modeled on ``test_example_debby.py``.
...
- the reviewer collapses onto claude-sdk (the cross-model fact-check stops being
  independent),
- a sub-agent silently pins a model (re-coupling it to one provider â€” a
...
def test_scribe_has_researcher_and_cross_vendor_reviewer(scribe_spec: AgentSpec) -> None:
    ...
    the whole point of the fact-check â€” it catches claims the author's own model
    ...
    # Reviewer is a different vendor than the brain â†’ the fact-check is independent.
...
def test_scribe_has_os_env(scribe_spec: AgentSpec) -> None:
    ...
    â€” it reads change context (git/gh) and writes docs through them. The shipped
