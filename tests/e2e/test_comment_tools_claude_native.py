...
starts Claude Code in a private tmux window with the agent-meow MCP
...
def _claude_code_session(
    ...
    *,
    ...
    """
    Start Claude Code in a private tmux window with the agent-meow MCP bridge.
...
def test_claude_native_agent_addresses_comments_without_tool_guidance(
    ...
    **What breaks if this test fails:**
    ...
    - ``_execute_comment_tool`` call path broken (server_client / session
      scoping), so the agent-meow server ``PATCH /v1/sessions/{id}/comments`` is
      never hit.
    ...
    # the relay, so comment status changes reach the agent-meow server even
    ...
    with _claude_code_session(session_id, model=None, launch_env=launch_env):
        ...
        # and hit the agent-meow server directly, so the comment status changes appear
        ...
        assert post_statuses.get(comment1_id) == "addressed", (
            ...
            f"If 'draft', the update_comment relay call did not reach the agent-meow server."
        )
