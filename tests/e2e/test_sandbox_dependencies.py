...
def test_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from agent_meow')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from agent_meow."},
        ],
        key=model,
    )
    ...
    assert "hello from agent_meow" in combined, (
        f"Expected cowsay ASCII art with 'hello from agent_meow' "
...
def test_uv_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from agent_meow via uv')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from agent_meow via uv."},
        ],
        key=model,
    )
    ...
    all_output = _tool_call_output_text(body)
    combined = (text + " " + all_output).lower()
    assert "hello from agent_meow via uv" in combined, (
        f"Expected cowsay output with 'hello from agent_meow via uv' -- proves "
