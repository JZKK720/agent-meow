...
from dev.benchmarks.agent_meow.environment import BenchEnvironment, _sse_session_status
from dev.benchmarks.agent_meow.journeys import ALL_JOURNEYS, Journey, run_latency, run_throughput
from dev.benchmarks.agent_meow.measure import RunResult, aggregate, check_thresholds
from dev.benchmarks.agent_meow.schema import SCHEMA_VERSION, build_report
...
def test_aggregate_route_appendix_groups_and_orders() -> None:
    ...
    # 2 ops/run × 2 runs = 4 ops. GET seen 2×/run (�? total �?1.0/op),
    # POST 1×/run (�? �?0.5/op).
    runs = [
        RunResult(
