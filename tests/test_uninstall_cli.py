...
from agent_meow.install_ledger import InstallLedger, new_ledger
...
def test_uninstall_cli_refuses_without_install_signal(monkeypatch) -> None:
    runner = CliRunner()
    monkeypatch.setattr("agent_meow.install_ledger.resolve_uninstall_ledger", lambda: None)
...
def test_uninstall_cli_defaults_to_dry_run_without_destructive_flags(
    ...
    monkeypatch.setattr(cli_module, "_uninstall_script_path", lambda: script)
    monkeypatch.setattr("agent_meow.install_ledger.resolve_uninstall_ledger", lambda: ledger)
