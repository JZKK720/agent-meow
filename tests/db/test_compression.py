...
Covers the frame format, the raw/zstd threshold, and â€?critically â€?that
values written before a column was migrated (unframed ``TEXT``) still decode
unchanged, so the ``TEXT`` â†?``BLOB`` migration needs no backfill.
"""
...
from agent_meow.db.compression import _MIN_COMPRESS_BYTES, decode, encode
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "cafÃ© â€?naÃ¯ve â€?æ—¥æœ¬èª?â€?ðŸŽ‰ " * 5,  # multibyte utf-8
