...
flips ``streamStale`` in the chat store, which swaps the "Workingâ€¦"
shimmer for an "Agent is unresponsive" banner.

This test exercises the full chain: SPA â†’ SSE stream â†’ health poll â†’
banner render. It kills the runner subprocess with SIGKILL while the
LLM is processing, so the tunnel drops instantly â€” no graceful
...
def test_stale_banner_on_runner_crash(
    ...
    longer renders a composer â€” see :func:`seeded_session`.
    ...
    # Wait for the "Workingâ€¦" shimmer â€” proves the SSE stream opened
    ...
    # is async â€” the server's WS route needs to notice the close and
    # deregister). 10 retries Ã— 0.5 s = 5 s budget.
    ...
    # The health poller fires every 10 s. No grace period â€” the
