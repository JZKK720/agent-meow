...
agent-meow#927: when a hook retry re-parks the *same* elicitation id after the
...
def _permission_hook_payload(elicitation_id: str) -> dict:
    ...
    Mirrors the ``agent-meow claude`` wrapper's hook subprocess: the
...
def test_reparked_elicitation_reliably_resurfaces_in_inbox(
    ...
    Regression for agent-meow#927. When the user approves an inbox approval, the
    ...
    agent-meow#927 shape of a single prompt whose hook keeps re-parking the same
    ...
    # A single elicitation id, re-parked repeatedly — the agent-meow#927 scenario
