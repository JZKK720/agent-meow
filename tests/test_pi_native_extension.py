...
def test_inbox_model_change_applies_via_set_model(tmp_path: Path) -> None:
    ...
    and apply it live via ``pi.setModel`` â€?with no error item posted.
    """
    node = shutil.which("node")
    if node is None:
        pytest.skip("node is required for the pi-native extension e2e test")
...
def test_session_start_posts_model_options_from_registry(tmp_path: Path) -> None:
    ...
    extension prefers ``ctx.modelRegistry.getAvailable()`` â€?only models with
    configured auth â€?over ``getAll()`` (Pi's entire built-in catalog), so the
...
def test_session_start_empty_registry_posts_no_model_options(tmp_path: Path) -> None:
    ...
    throws), the extension stays silent rather than posting an empty catalog â€?
