...
def _worker_spec(harness: str, **executor_kwargs: object) -> AgentSpec:
    ...
    return AgentSpec(
        spec_version=1,
        name="worker",
        executor=ExecutorSpec(type="agent-meow", config={"harness": harness}, **executor_kwargs),  # type: ignore[arg-type]
    )
...
def test_resolve_provider_legacy_profile(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(
            type="agent-meow", config={"harness": "codex-native", "profile": "legacy-prof"}
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            SimpleNamespace(executor=SimpleNamespace(type="agent-meow", config={})),
            "claude-native",
            id="structural-stub",
        ),
    ],
...
def test_failed_auth_command_note_never_leaks_the_command(
    ...
    parent = AgentSpec(
        spec_version=1,
        name="orchestrator",
        executor=ExecutorSpec(type="agent-meow", config={"harness": "pi"}),
