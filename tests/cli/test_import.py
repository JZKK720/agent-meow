...
from agent_meow.session_import.models import SessionImportNotFoundError
...
def test_import_command_loads_local_session_and_posts_normalized_items(tmp_path: Path) -> None:
    ...
    with patch("agent_meow.cli._resolve_attach_server", return_value=_BASE):
...
def test_import_command_accepts_opencode_export() -> None:
    ...
    with (
        patch("agent_meow.cli._resolve_attach_server", return_value=_BASE),
        patch("agent_meow.session_import.local._run_opencode_json", return_value=export),
    ):
        result = CliRunner().invoke(
            cli,
...
def test_import_command_reports_opencode_discovery_failure() -> None:
    ...
    with patch(
        "agent_meow.session_import.local._run_opencode_json",
