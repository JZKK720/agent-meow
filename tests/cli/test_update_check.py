...
def test_find_repo_root_ignores_unrelated_ancestor_git(
    ...
    agent-meow was installed under ``~/.local/share/uv/tools/`` â€”
    misclassifying the install as a dev clone and writing
    ``kind: "clone"`` to the update-check cache.

    Layout:

        tmp_path/.git/                      â† unrelated dotfiles-style repo
...
def test_find_repo_root_requires_pyproject_alongside_git(
    ...
    """``.git/`` next to ``omnigent/`` without ``pyproject.toml`` â†’ None.
...
def test_find_repo_root_accepts_git_plus_pyproject(
    ...
    """``.git/`` AND ``pyproject.toml`` directly above ``omnigent/`` â†’ repo root."""
    ...
    monkeypatch.setattr(mod, "__file__", str(fake_file))
    # Returns exactly the repo root â€” the parent of omnigent/.
...
def test_no_repo_root_routes_to_wheel_check(
    ...
    """No clone reachable â†’ dispatcher routes to the wheel-install path.
    ...
    # stubbed it out â€” proves the dispatch is wired correctly.
...
def test_fresh_cache_clears_after_pull(
    ...
    # No notice â€” the user is up to date.
    assert capsys.readouterr().err == ""
...
def _block_build_info_import(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    a fake â€” silently overriding the uv_cache / mtime install-time
    signals most tests are trying to exercise.

    Two things have to be reset every test for the block to work:

    1. ``sys.modules["omnigent._build_info"] = None`` â€” Python's
       documented "this import raises ImportError" sentinel.
    2. ``delattr(agent-meow, "_build_info")`` â€” once a previous test
       ...
       ``agent-meow`` package. Subsequent ``from omnigent import
    ...
    by ``monkeypatch.setitem(sys.modules, ..., fake_module)`` â€” that
...
# substring of the formatted command. Not a real endpoint â€” never hit.
...
def _strip_rich_panel(text: str) -> str:
    ...
    no_box = re.sub(r"[â•­â•®â•¯â•°â”€â”‚]", "", text)
...
def _write_fake_dist_info(
    ...
    *,
    ...
    about â€” the production code already handles missing files.
    ...
    :param dir_mtime_epoch: When provided, ``os.utime`` is used to
        backdate the dist-info dir's mtime to this Unix timestamp â€”
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "git+ssh://****@github.com/JZKK720/agent-meow.git",
            "git+ssh://git@github.com/JZKK720/agent-meow.git",
        ),
        # Same redaction, but the URL was stored without the ``git+``
        # VCS prefix (the shape uv wrote on the machine in the report).
        (
            "ssh://****@github.com/JZKK720/agent-meow.git",
            "ssh://git@github.com/JZKK720/agent-meow.git",
        ),
        # Already-correct SSH user â€” must be left exactly as-is.
        (
            "git+ssh://git@github.com/org/repo.git",
            "git+ssh://git@github.com/org/repo.git",
        ),
        # HTTPS URL with no userinfo â€” nothing to repair.
        (
            "git+https://github.com/org/repo.git",
            "git+https://github.com/org/repo.git",
        ),
        # HTTPS with a partially-redacted ``user:****`` â€” that ``****``
        ...
        # SSH with a real (non-redacted) custom user â€” not the marker,
...
def test_unredact_ssh_userinfo(stored: str, expected: str) -> None:
    ...
    on the remaining cases means the repair became too aggressive â€”
...
def test_read_wheel_info_repairs_redacted_ssh_user(
    ...
    """An end-to-end repair: redacted ``direct_url.json`` â†’ runnable command.
    ...
    ``uv tool install --reinstall git+ssh://git@â€¦`` command. If the
    ...
    """
    redacted_url = "ssh://****@github.com/JZKK720/agent-meow.git"
    dist = _write_fake_dist_info(
        tmp_path,
        installer="uv",
        direct_url={
    ...
    assert info.vcs_url == "git+ssh://git@github.com/JZKK720/agent-meow.git"
    ...
    # runnable form â€” not the broken ``****@`` URL.
    assert suggestion.runnable is True
    assert (
        suggestion.command
        == "uv tool install --reinstall git+ssh://git@github.com/JZKK720/agent-meow.git"
    )
...
def test_read_wheel_info_pip_registry_install(
    ...
    # No direct_url means we know nothing about the source â€” vcs_url
...
def test_read_wheel_info_handles_corrupt_direct_url(
    ...
    """Corrupt direct_url.json is tolerated â€” fields fall back to None."""
    ...
    info = _read_installed_wheel_info()
    # We still get a result back â€” corrupt direct_url just means we
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # pip + registry â€” the canonical upgrade incantation.
        ("pip", None, "pip install -U agent-meow", True),
        # pipx â€” pipx has its own subcommands; we never recommend the
        ...
        # poetry path â€” included for completeness; poetry is rare for
        # CLI tool installs but the format is documented.
        ("poetry", None, "poetry update agent-meow", True),
        # Unknown installer WITH a VCS URL â€” we know the source but
        ...
        # Unknown installer with no source URL â€” honest fallback.
...
def test_pip_invocation_pins_to_running_interpreter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # No interpreter path (frozen / embedded) â†’ bare ``pip`` fallback.
...
def test_wheel_check_no_nag_when_up_to_date(
    ...
    """Latest PyPI release == installed version â†’ no nag.
...
def test_wheel_check_nags_when_newer_release_available(
    ...
    """Cached latest > installed â†’ nag naming the release and ``omni upgrade``."""
    monkeypatch.delenv("OMNIGENT_NO_UPDATE_CHECK", raising=False)
    _point_cache_at(tmp_path, monkeypatch)
    _write_cache(
        _CacheEntry(
    ...
    # Names the new release, the installed version, and the command â€”
...
def test_wheel_check_fires_once_per_release(
    ...
    """Already-notified release â†’ silent (no repeat nag).
...
def test_wheel_check_bails_for_editable_install(
    ...
    """Editable install â†’ no nag and no background refresh.

    There's no PyPI release to compare an editable checkout against â€”
    the right answer is ``git pull``, not a reinstall â€” so the wheel
...
def test_wheel_check_bails_when_distribution_missing(
    ...
    """No installed distribution â†’ silent no-op (running from source)."""
    monkeypatch.delenv("OMNIGENT_NO_UPDATE_CHECK", raising=False)
    monkeypatch.setattr("omnigent.update_check._get_distribution", lambda: None)
    _run_installed_wheel_check()
    assert capsys.readouterr().err == ""
...
def test_wheel_check_refreshes_when_cache_stale(
    ...
    """Stale wheel cache â†’ no nag this run, but a background refresh fires.
...
def test_wheel_check_env_var_disables(
    ...
    Verifies the env-var gate works for the wheel path too â€” not just
...
def test_wheel_check_ignores_clone_kind_cache(
    ...
    clone-kind cache as a "latest version" signal â€” it has none â€” so it
...
def test_is_newer_pep440_ordering() -> None:
    ...
    assert _is_newer("0.10.0", "0.9.0") is True  # lexical "0.10" < "0.9" â€” must not fool us
...
def test_fetch_latest_version_pep691_json(monkeypatch: pytest.MonkeyPatch) -> None:
    """PEP 691 ``versions`` â†’ latest *stable* (pre/dev releases excluded)."""
    import httpx
...
def test_fetch_latest_version_from_files_when_no_versions_key(
    ...
    """An index without a ``versions`` key â†’ derive from wheel/sdist filenames."""
    import httpx
    ...
    body = {
        "files": [
            ...
            {"filename": "omnigent-0.3.0rc1-py3-none-any.whl"},  # prerelease â†’ excluded
...
def test_fetch_latest_version_html_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    """A PEP 503 HTML index (no JSON) â†’ scrape filenames from the links."""
    import httpx
...
def test_fetch_latest_version_none_when_only_prereleases(
    ...
    """Only pre-releases available â†’ no stable release, returns ``None``."""
    import httpx
...
def test_resolve_index_url_precedence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Multiple whitespace/comma-separated URLs â†’ the first (primary) index.
...
def test_resolve_index_url_from_uv_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """No env var â†’ uv.toml's ``index-url`` is used (corp-mirror-in-a-file case)."""
    from omnigent.update_check import _resolve_index_url
...
def test_resolve_index_url_from_pip_config(
    ...
    """uv has nothing â†’ pip.conf's ``[global] index-url`` is used."""
    from omnigent.update_check import _resolve_index_url
...
def test_wheel_info_prefers_build_info_over_uv_cache(
    ...
    semantically correct signal for "how stale is this code" â€” if
    ...
    info = _read_installed_wheel_info()
    assert info is not None
    # Build time, not uv_cache time â€” proves the priority is correct.
...
def test_wheel_info_falls_back_to_uv_cache_when_build_info_missing(
    ...
    monkeypatch.setattr("omnigent.update_check._get_distribution", lambda: dist)
    # _build_info import returns None â€” simulates a source checkout.
...
def test_wheel_info_falls_back_to_mtime_when_only_dist_info_available(
    ...
    """No _build_info, no uv_cache â€” fall back to dist-info mtime."""
...
def test_wheel_info_build_info_empty_sha_does_not_clobber_direct_url_sha(
    ...
    â€” it's the only commit info we have.
    """
...
def test_read_build_info_returns_none_when_module_missing() -> None:
    ...
    None â€” Python's documented "this import fails" sentinel. The
...
def test_read_build_info_tolerates_malformed_module(
    ...
    hand-edited ``_build_info.py`` â€” falling back to other signals
    is always better than crashing the CLI's startup banner.
    """
    import types

    fake_module = types.ModuleType("omnigent._build_info")
    # Missing BUILD_TIME_EPOCH and COMMIT_SHA â€” AttributeError when read.
    monkeypatch.setitem(sys.modules, "omnigent._build_info", fake_module)
...
def test_run_upgrade_command_invokes_subprocess(
    ...
    # shlex.split tokenization is the contract â€” a single
...
def test_run_upgrade_command_returns_minus_one_when_binary_missing(
    ...
    """Binary missing from PATH â†’ ``_run_upgrade_command`` returns -1 and prints why.
    ...
    # non-zero" â€” the latter would be the subprocess's own code.
    assert code == -1
    err = _strip_rich_panel(capsys.readouterr().err)
...
def test_format_version_falls_back_to_bare_version_when_build_info_missing() -> None:
    ...
    hit this path. The line must remain stable across releases â€”
    scripts that grep for "agent-meow X.Y.Z" must keep working.
    """
    # The autouse fixture has already blocked sys.modules['omnigent.build_info']
    # via the None sentinel, so _read_build_info returns None.
    from omnigent.cli import _format_version

    out = _format_version()
    # Exact prefix match â€” if the format ever gains extra content
...
def test_format_version_includes_sha_and_build_time_when_present(
    ...
    assertion catches it â€” bug reports rely on this string being
    copy-pasteable.
    """
    import types
...
def test_format_version_omits_sha_when_build_info_has_empty_sha(
    ...
    bake ``COMMIT_SHA = ""``. The build time is still useful â€” the
...
def test_split_vcs_url_strips_pip_fragment() -> None:
    ...
    assert _split_vcs_url("git+https://github.com/o/omnigent.git#egg=agent-meow") == (
        "https://github.com/o/omnigent.git",
        None,
    )
...
def test_split_vcs_url_ssh_userinfo_is_not_a_revision() -> None:
    ...
    # â€¦but a real trailing revision still parses, even with SSH userinfo.
...
def test_wheel_check_skips_vcs_install(
    ...
    fire forever â€” even on a build that is *ahead* of the latest release.
