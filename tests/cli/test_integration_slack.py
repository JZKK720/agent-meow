...
from agent_meow.integration_daemon import DaemonRecord, IntegrationDaemon
...
def test_slack_start_hint_when_not_installed(data_dir: Path) -> None:
    runner = CliRunner()
    with mock.patch("agent_meow.cli._slack_installed", return_value=False):
...
def test_slack_foreground_runs_subprocess(data_dir: Path) -> None:
    ...
    with (
        mock.patch("agent_meow.cli._slack_installed", return_value=True),
        mock.patch("agent_meow.cli.subprocess.run") as run,
    ):
