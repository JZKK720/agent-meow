"""Tests for agent_meow.cli —bundle env var resolution."""
...
def test_server_uvicorn_log_config_uses_terminal_handler_when_requested(
    ...
    monkeypatch.setattr("agent_meow.process_logging.terminal_supports_color", lambda: True)
...
def _fake_run_claude_native_capture(
    ...
    def _stub(**kwargs: object) -> None:
        ...
        :param kwargs: Whatever ``agent_meow.claude_native.run_claude_native``
...
def _fake_run_codex_native_capture(
    ...
    def _stub(**kwargs: object) -> None:
        ...
        :param kwargs: Whatever ``agent_meow.codex_native.run_codex_native``
            is called with.
        """
        captured.update(kwargs)
...
def test_claude_command_short_r_binds_omnigent_session(
    ...
    id internally —see ``agent_meow.claude_native._resolve_cold_resume_args``
...
def test_codex_command_resume_binds_session_and_passes_unknown_args(
    ...
    monkeypatch.setattr("agent_meow.cli._load_effective_config", dict)
    monkeypatch.setattr(
        "agent_meow.codex_native.run_codex_native",
...
def test_antigravity_command_env_var_passes_command_to_run_antigravity_native(
    ...
    monkeypatch.setattr("agent_meow.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "agent_meow.antigravity_native.run_antigravity_native",
...
def test_pi_config_args_form_base_cli_args_append(
    ...
    monkeypatch.setattr("agent_meow.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "agent_meow.pi_native.run_pi_native",
        lambda **kwargs: captured.update(kwargs),
    )
...
def test_kiro_command_parses_native_options_and_prompt(
    ...
    monkeypatch.setattr(
        "agent_meow.kiro_native.run_kiro_native",
...
def test_pi_config_command_threads_to_harness_path_env_var(
    ...
    monkeypatch.setattr("agent_meow.cli._ensure_backend", lambda s: s or "http://localhost:1")
    monkeypatch.setattr("agent_meow.pi_native.run_pi_native", lambda **kw: None)
...
def test_cli_warns_deprecated_harness_path_env_vars(
    ...
    from agent_meow.cli import _warn_deprecated_harness_path_env_vars
...
def _invoke_bundled_agent_command(
    ...
    monkeypatch.setattr("agent_meow.cli._load_effective_config", dict)
    dispatch = Mock()
    monkeypatch.setattr("agent_meow.cli._dispatch_run", dispatch)
...
def test_first_run_plan_and_polly_command_agree_on_bundled_path(
    ...
    """
    monkeypatch.setattr(
        "agent_meow.onboarding.provider_config.default_provider_for_harness",
...
def test_bundled_agent_multiple_credentials_notice_preserves_first_pick(
    ...
    monkeypatch.setattr("agent_meow.cli.click.prompt", _fail_prompt)
    monkeypatch.setattr("agent_meow.cli.click.confirm", _fail_prompt)
    _write_isolated_provider_config(
        tmp_path,
        {
            "anthropic_first": {
...
def test_bundled_agent_unreadable_global_config_degrades_to_launch(
    ...
    monkeypatch.setattr("agent_meow.cli._load_global_config", _corrupt)
    dispatch = Mock()
    monkeypatch.setattr("agent_meow.cli._dispatch_run", dispatch)
...
def test_start_cli_runner_process_uses_token_bound_runner_id(
    ...
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("agent_meow.cli.secrets.token_urlsafe", lambda _size: "bind-token")
    ...
    monkeypatch.setattr("agent_meow.cli.subprocess.Popen", _Proc)
...
def test_start_cli_runner_process_reports_captured_log_path(
    ...
    monkeypatch.setattr("agent_meow.cli.subprocess.Popen", _ExitedProc)
...
def test_run_without_agent_drops_into_configure_when_unconfigured(
    ...
    configure = Mock()
    monkeypatch.setattr("agent_meow.cli._run_configure_harnesses_interactive", configure)
...
def test_run_without_agent_claude_alias_dispatches_generated_yaml_headlessly(
    ...
    run_chat = Mock()
    run_prompt = Mock()
    monkeypatch.setattr("agent_meow.chat.run_chat", run_chat)
    monkeypatch.setattr("agent_meow.chat.run_prompt", run_prompt)
...
def test_attach_forwards_live_conversation_to_run_attach(
    ...
    monkeypatch.setattr("agent_meow.cli._require_live_conversation", lambda **_kw: None)
    run_attach = Mock()
    monkeypatch.setattr("agent_meow.chat.run_attach", run_attach)
...
def test_attach_nonlive_conversation_errors_loud_without_connecting(
    ...
    monkeypatch.setattr(
        "agent_meow.cli._host_http_json",
...
def test_run_server_resume_by_id_forwards_to_run_attach(
    ...
    monkeypatch.setattr("agent_meow.cli._require_live_conversation", lambda **_kw: None)
    monkeypatch.setattr("agent_meow.chat._redirect_native_resume_if_needed", lambda **_kw: False)
    run_attach = Mock()
    run_chat = Mock()
    monkeypatch.setattr("agent_meow.chat.run_attach", run_attach)
    monkeypatch.setattr("agent_meow.chat.run_chat", run_chat)
...
def test_run_server_resume_native_redirects_before_attach_preflight(
    ...
    monkeypatch.setattr("agent_meow.chat._redirect_native_resume_if_needed", _redirect)
    monkeypatch.setattr("agent_meow.cli._require_live_conversation", _must_not_preflight)
    monkeypatch.setattr("agent_meow.chat.run_attach", _must_not_attach)
...
def test_save_and_load_global_config_round_trips(
    ...
    monkeypatch.setattr("agent_meow.cli._GLOBAL_CONFIG_PATH", config_path)
...
def test_config_list_empty(
    ...
    monkeypatch.setattr("agent_meow.cli._print_credentials_by_harness", lambda: None)
...
def test_save_local_config_migrates_scalar_harness(
    ...
    from agent_meow.cli import _load_local_config
...
def test_config_set_local_writes_project_config(
    ...
    """
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("agent_meow.cli._GLOBAL_CONFIG_PATH", tmp_path / "global.yaml")
...
def test_run_applies_global_config_agent_default(
    ...
    monkeypatch.setattr("agent_meow.cli._dispatch_run", fake_dispatch)
    monkeypatch.setattr("agent_meow.cli._build_resume_parts", lambda: None)
    monkeypatch.setattr(
...
def test_setup_command_replaces_wizard(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "agent_meow.cli._run_configure_harnesses_interactive",
        configure_flow,
    )
    monkeypatch.setattr("agent_meow.cli._run_configure_databricks", configure_databricks)
    monkeypatch.setattr("agent_meow.onboarding.setup.run_onboarding", run_onboarding)
...
def test_setup_uses_compact_branding_on_short_terminals(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_meow.cli._run_configure_harnesses_interactive", configure_flow)
    monkeypatch.setattr("agent_meow.inner.ui.print_landing", print_landing)
    monkeypatch.setattr("agent_meow.inner.ui.print_brandmark", print_brandmark)
    monkeypatch.setattr(
        "agent_meow.cli.shutil.get_terminal_size",
...
def test_node_dependency_problem_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("agent_meow.cli.shutil.which", lambda _: None)
...
def test_node_dependency_problem_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_meow.cli.shutil.which", lambda _: "/usr/bin/node")
    monkeypatch.setattr(
        "agent_meow.cli.subprocess.run",
...
def test_node_dependency_problem_probe_inconclusive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_meow.cli.subprocess.run", _boom)
...
def test_warn_missing_harness_dependencies_silent_when_present(
    ...
    monkeypatch.setattr("agent_meow.cli.shutil.which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr(
        "agent_meow.cli.subprocess.run",
        _fake_node_run("v22.14.0", probe_returncode=0),
    )
...
def test_warn_missing_harness_dependencies_lists_all_gaps(
    ...
    monkeypatch.setattr("agent_meow.cli.shutil.which", _which)
    monkeypatch.setattr(
        "agent_meow.cli.subprocess.run",
        _fake_node_run("v20.12.2", probe_returncode=1),
    )
...
def test_click_subcommands_allowlist_covers_registered_commands() -> None:
    ...
    """
    from agent_meow.cli import _CLICK_SUBCOMMANDS, cli
...
def test_announce_auto_configured_credentials_names_creds_compactly(
    ...
    monkeypatch.setattr("agent_meow.onboarding.ambient.detect_providers", lambda: detected)
...
def test_adopt_ambient_credentials_announces_only_what_was_adopted(
    ...
    monkeypatch.setattr("agent_meow.cli_config._promote_global_auth_to_provider", Mock())
    monkeypatch.setattr(
        "agent_meow.cli_config._adopt_detected_providers", Mock(return_value=["anthropic"])
    )
    monkeypatch.setattr(
        "agent_meow.onboarding.ambient.detect_providers",
        lambda: [
            DetectedProvider(
                name="anthropic", kind="key", family="anthropic", source="$ANTHROPIC_API_KEY"
            )
...
def test_native_terminal_dispatch_specs_cover_registered_native_agents() -> None:
    from agent_meow.harness_plugins import native_agents
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "opencode-native",
            "agent_meow.opencode_native.run_opencode_native",
            {"opencode_args": (), "model": "native-model"},
        ),
        (
            "cursor-native",
            "agent_meow.cursor_native.run_cursor_native",
            {"cursor_args": ("--model", "native-model")},
        ),
        (
            "kimi-native",
            "agent_meow.kimi_native.run_kimi_native",
        ...
        (
            "goose-native",
            "agent_meow.goose_native.run_goose_native",
        ...
        (
            "qwen-native",
            "agent_meow.qwen_native.run_qwen_native",
            {"qwen_args": ("--model", "native-model")},
        ),
        (
            "hermes-native",
            "agent_meow.hermes_native.run_hermes_native",
...
def test_dispatch_native_terminal_harness_launches_registered_wrapper(
    ...
    monkeypatch.setattr("agent_meow.cli._ensure_backend", lambda _s: "http://localhost:0")
    captured: dict[str, object] = {}
...
@pytest.mark.parametrize(
    ...
    [
        ("goose-native", "agent_meow.goose_native.run_goose_native", "goose_args"),
        ("qwen-native", "agent_meow.qwen_native.run_qwen_native", "qwen_args"),
        ("hermes-native", "agent_meow.hermes_native.run_hermes_native", "hermes_args"),
    ],
)
@pytest.mark.parametrize(
...
def test_dispatch_native_terminal_harness_ignores_non_native(
    ...
    monkeypatch.setattr("agent_meow.cli._ensure_backend", _must_not_run)
...
def test_dispatch_native_terminal_harness_continue_resumes_latest(
    ...
    monkeypatch.setattr("agent_meow.chat._resolve_latest_conversation_id", _fake_latest)
    monkeypatch.setattr("agent_meow.chat._remote_headers", lambda **_kw: {})
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "agent_meow.cursor_native.run_cursor_native",
        lambda **kwargs: captured.update(kwargs),
...
def test_dispatch_native_terminal_harness_continue_with_no_prior_fails_loud(
    ...
    """
    monkeypatch.setattr("agent_meow.cli._ensure_backend", lambda _s: "http://localhost:0")
    monkeypatch.setattr("agent_meow.chat._resolve_latest_conversation_id", lambda **_kw: None)
    monkeypatch.setattr("agent_meow.chat._remote_headers", lambda **_kw: {})

    def _must_not_launch(**_kw: object) -> None:
        raise AssertionError("wrapper launched despite no conversation to continue")

    monkeypatch.setattr("agent_meow.cursor_native.run_cursor_native", _must_not_launch)
...
def test_dispatch_native_terminal_harness_explicit_id_skips_latest_lookup(
    ...
    monkeypatch.setattr("agent_meow.chat._resolve_latest_conversation_id", _must_not_lookup)
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "agent_meow.cursor_native.run_cursor_native",
        lambda **kwargs: captured.update(kwargs),
...
def test_manage_qwen_harness_declines_install_returns(
    ...
    import agent_meow.onboarding.interactive as it
    ...
    launch = Mock()
    monkeypatch.setattr("agent_meow.cli_config._launch_qwen_auth", launch)
...
def test_manage_qwen_harness_back_does_not_launch(
    ...
    monkeypatch.setattr("agent_meow.cli_config._qwen_auth_configured", lambda: False)
    monkeypatch.setattr(it, "console", Mock())
...
def test_manage_goose_harness_missing_cli_shows_hint_returns(
    ...
    monkeypatch.setattr(it, "console", Mock())
    launch = Mock()
    monkeypatch.setattr("agent_meow.cli_config._launch_goose_configure", launch)
...
def test_manage_goose_harness_back_does_not_launch(
    ...
    import agent_meow.onboarding.goose_auth as ga
    import agent_meow.onboarding.harness_install as hi
    import agent_meow.onboarding.interactive as it
