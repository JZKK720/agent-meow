...
def _uv_registry_info() -> _InstalledWheelInfo:
    """A registry uv-tool install â†’ ``uv tool upgrade agent-meow`` (runnable)."""
...
def _git_install_info() -> _InstalledWheelInfo:
    """A git/VCS uv-tool install â†’ ``uv tool install --reinstall git+â€¦``."""
    return _InstalledWheelInfo(
        install_time_epoch=0.0,
        installer="uv",
...
def test_upgrade_up_to_date(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    """Latest == installed â†’ reports up to date, exit 0, nothing stopped/run."""
...
def test_upgrade_check_reports_newer_and_exits_nonzero(
    ...
    """``--check`` with a newer release â†’ prints the delta and exits non-zero, no upgrade."""
    ...
    assert "v0.1.0 â†’ v0.2.0" in result.output


def test_upgrade_runs_installer_and_drains_first(
    monkeypatch: pytest.MonkeyPatch, _wheel_install: None
) -> None:
    """A newer release â†’ drain (no force), stop the server, run the uv command."""
    monkeypatch.setattr("omnigent.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
...
def test_upgrade_install_failure_surfaces(
    ...
    """A non-zero installer exit â†’ ClickException naming the status, exit non-zero."""
    monkeypatch.setattr("omnigent.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
...
def test_upgrade_index_unreachable(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    """Index unreachable â†’ clear error, no upgrade attempted."""
...
def test_upgrade_pre_check_detects_release_candidate(
    ...
    assert result.exit_code == 1, result.output
    assert "v0.1.0 â†’ v0.1.1rc1" in result.output
...
def test_upgrade_without_pre_ignores_release_candidate(
    ...
    """Without ``--pre`` the rc is invisible â†’ reports up to date."""
...
def test_count_running_sessions_ignores_idle_connected(
    ...
    sessions = [
        ...
        {"id": "c", "status": "idle"},
        {"id": "d", "status": "running"},
        {"id": "e"},  # missing status â†’ not running
    ]
    monkeypatch.setattr(
...
# â”€â”€ ``omni update`` alias â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_update_is_alias_for_upgrade_same_callback() -> None:
    ...
    identical â€” there is no second implementation to drift from ``upgrade``.
    """
...
def test_update_check_matches_upgrade_check(
    ...
    assert "v0.1.0 â†’ v0.2.0" in update_result.output
...
def test_upgrade_noop_install_reports_failure_not_success(
    ...
    cache). The old code printed "âœ“ Upgraded to v0.2.0" anyway, so the next
    ``--check`` kept reporting the same update â€” the bug this guards against.
    """
    monkeypatch.setattr("omnigent.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
    monkeypatch.setattr("omnigent.update_check._run_upgrade_command", lambda *_a, **_k: 0)
    ...
    assert "âœ“ Upgraded" not in result.output
...
def test_upgrade_git_install_noop_does_not_claim_update(
    ...
    """If the re-pull leaves the commit unchanged, say so â€” don't claim an update."""
    monkeypatch.setattr("omnigent.update_check._read_installed_wheel_info", _git_install_info)
    # Remote unknown (e.g. offline) â†’ re-pull anyway, then verify by commit.
    ...
    assert "âœ“ Updated" not in result.output
...
def test_upgrade_git_confirmed_behind_but_repull_noop_fails(
    ...
    monkeypatch.setattr("omnigent.update_check._read_installed_wheel_info", _git_install_info)
    # Remote HEAD differs from the installed commit ("a"*40) â†’ positively behind.
    monkeypatch.setattr("omnigent.update_check._remote_git_head", lambda _url: "b" * 40)
    monkeypatch.setattr("omnigent.update_check._run_upgrade_command", lambda *_a, **_k: 0)
    # â€¦but the re-pull left the install on the SAME commit (pinned ref / cached).
    monkeypatch.setattr(
        "omnigent.update_check._probe_installed_distribution", lambda: ("0.1.0", "a" * 40)
    )
