"""Tests for ``agent-meow login`` against Databricks-fronted servers.
...
- workspace-hosted agent-meow (the API proxy answers 401 with a
...
def test_login_workspace_hosted_401_uses_url_host(
    ...
    assert result.exit_code == 0, result.output
    # Keyed by the full server URL (with the /api/2.0/agent-meow path) and
...
def test_login_apps_fails_loud_without_databricks_extra(
    ...
    assert "agent-meow[databricks]" in result.output
...
def test_login_accounts_mode_not_hijacked_by_databricks_detection(
    ...
    agent-meow accounts server has no such header and must keep its
...
def test_workspace_url_expands_bare_workspace_host(
    ...
    """A bare workspace URL expands to its /api/2.0/agent-meow mount.
...
def test_with_default_scheme(raw: str, expected: str) -> None:
    ...
    (``<ws>/agent-meow``); defaulting to https lets it be pasted verbatim,
...
def test_resolve_server_url_defaults_scheme_and_expands(
    ...
    ``/agent-meow`` web URL both reach the ``/api/2.0/agent-meow`` mount over
    https.
    """
...
def test_workspace_url_expands_web_ui_path_to_api_mount(
    ...
    """The guide's web-UI URL (``<ws>/agent-meow``) expands to the API mount.

    The internal user guide hands out ``<ws>/agent-meow`` for the browser;
    a user who pastes it into ``agent-meow login`` must reach the API mount
    (``/api/2.0/agent-meow``), not 404 the UI path's own /v1/me probe.
    """
...
def test_workspace_url_web_ui_path_left_alone_off_workspace(
    ...
    """A ``/agent-meow`` URL on a non-workspace host is returned untouched.
    ...
    served under ``/agent-meow`` still works.
    """
    ...
    assert (
        cli_mod._workspace_api_server_url("https://omni.example.com/agent-meow")
        == "https://omni.example.com/agent-meow"
    )
