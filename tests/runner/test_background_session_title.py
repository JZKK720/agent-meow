...
async def test_background_title_uses_isolated_codex_process(
    ...
    monkeypatch.setattr(
        "agent_meow.runner.app._resolve_harness_config",
...
async def test_background_title_maps_claude_native_to_claude_cli(
    ...
    monkeypatch.setattr(
        "agent_meow.runner.app._generate_claude_native_background_title",
...
async def test_claude_native_title_uses_tool_free_print_mode(
    ...
    monkeypatch.setattr(
        "agent_meow.claude_native.resolve_native_claude_config",
        lambda spec=None: None,
    )
    monkeypatch.setattr(
        "agent_meow.claude_launcher.resolve_claude_launch",
...
async def test_background_title_timeout_releases_process(
    ...
    monkeypatch.setattr(
        "agent_meow.runner.app._BACKGROUND_TITLE_INFERENCE_TIMEOUT_SECONDS",
        0.01,
    )
    app = create_runner_app(
        process_manager=process_manager,  # type: ignore[arg-type]
