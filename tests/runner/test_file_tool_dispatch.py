...
from agent_meow.runner.tool_dispatch import _execute_file_tool, execute_tool
...
async def test_send_with_file_ids_copies_then_attaches_input_file(
    ...
    """
    One ``file_ids`` entry copies parentâ†’child, then the child events
    ...
    # Exactly one copy, addressed parentâ†’child with the requested ids.
    assert len(copies) == 1
