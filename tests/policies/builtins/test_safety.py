...
def test_ask_on_os_tools_asks_for_sys_os_tools(tool: str) -> None:
    ...
    If any returns ALLOW, the policy is not matching the agent-meow
...
def test_ask_on_os_tools_asks_for_pi_native_tools(
    ...
    (which match the agent-meow convention, not Claude's ``file_path``)
...
def _request_event(text: str) -> PolicyEvent:
    ...
    The agent-meow server evaluates skill slash commands at the REQUEST phase
