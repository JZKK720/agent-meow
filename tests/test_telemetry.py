...
from agent_meow.telemetry.surface import classify_surface
...
def test_get_installation_id_creates_uuid(tmp_path: Path) -> None:
    ...
    import agent_meow.telemetry.installation_id as _mod
    ...
    with (
        ...
        patch(
            "agent_meow.telemetry.installation_id._telemetry_file_path", return_value=telemetry_file
        ),
    ):
...
def test_get_installation_id_corrupted_file(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "agent_meow.telemetry.installation_id._write_to_disk", side_effect=OSError("disk full")
        ),
    ):
        result = _mod.get_installation_id()
...
def test_host_hello_frame_roundtrip_with_installation_id() -> None:
    ...
    from agent_meow.host.frames import HostHelloFrame, decode_host_frame, encode_host_frame
    from agent_meow.runtime import telemetry as _telemetry_mod
...
def test_build_record_promotes_host_installation_id() -> None:
    ...
    import agent_meow.telemetry.client as _mod
    from agent_meow.telemetry.events import SessionCreatedEvent
...
def test_resolve_harness_none_when_agent_store_uninitialized() -> None:
    ...
    with patch("agent_meow.runtime._globals._agent_store", None):
...
def test_resolve_harness_none_when_agent_not_in_store() -> None:
    ...
    with patch("agent_meow.runtime._globals._agent_store", mock_store):
        assert _resolve_harness(conv) is None
...
def test_resolve_harness_sdk_via_config_key() -> None:
    ...
    with (
        patch("agent_meow.runtime._globals._agent_store", mock_store),
        patch("agent_meow.runtime.get_agent_cache", return_value=mock_cache),
    ):
        assert _resolve_harness(conv) == "claude-sdk"
