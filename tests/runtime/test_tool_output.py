"""
Tests for :func:`~?omnigent.runtime.tool_output.cap_tool_output` â€” the canonical
...
def test_cap_tool_output_passes_through_when_within_cap() -> None:
    ...
    # Exactly at the cap is still within bounds â€” the check is `<=`, so a
...
def test_cap_tool_output_truncates_on_a_character_boundary() -> None:
    ...
    # "â‚¬" is 3 UTF-8 bytes. Sizing the string so the byte cap lands mid-char
    ...
    euros = "â‚¬" * (MAX_TOOL_OUTPUT_BYTES + 1)
