...
* :func:`build_kiro_native_spawn_env` â€” the minimal env handed to the harness
...
* :func:`build_kiro_native_terminal_env` â€” the allowlisted child env for the
...
suite it does not isolate ``TMPDIR`` â€” ``_BRIDGE_ROOT`` is resolved at import
...
def test_spawn_env_sets_only_the_bridge_dir() -> None:
    ...
    ``GOOSE_*``). Asserting an exact one-key set pins that minimality â€” a refactor
    ...
    # It is ``<root>/kiro-native/<hash>`` â€” the segment is present, but the path
...
def test_spawn_env_bridge_dir_is_deterministic_per_session() -> None:
    """Same session id â†’ same bridge dir; a different id â†’ a different dir.
...
def test_terminal_env_keeps_allowlisted_vars_and_adds_bridge_dir() -> None:
    ...
    ``kiro-cli`` needs the terminal/locale context (``TERM``/``LANG``/â€¦) and its
...
def test_terminal_env_drops_non_allowlisted_and_ambient_secrets() -> None:
    """Anything off the allowlist â€” arbitrary exports and provider keys â€” is dropped.
    ...
    # Nothing from the (all-disallowed) source env survives â€” only the harness's
