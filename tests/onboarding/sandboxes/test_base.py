...
def test_run_background_wraps_command_in_sh_c() -> None:
    ...
    in-sandbox ``agent-meow host`` launch died on its ``OMNIGENT_HOST_TOKEN=…``
    prefix.
    """
    launcher = _RecordingLauncher()

    launcher.run_background("sb-1", "FOO=bar agent-meow host --server https://srv")

    [cmd] = launcher.commands
    assert cmd == (
        "setsid nohup sh -c 'FOO=bar agent-meow host --server https://srv' "
        "> /tmp/agent-meow-host.log 2>&1 < /dev/null & echo launched"
    )
...
def test_start_host_env_prefix_is_honored_by_a_real_shell() -> None:
    ...
    real ``sh -c`` (the inner shell of the wrapper) with ``agent-meow host``
    ...
    probe = raw.replace(
        "agent-meow host --server https://srv",
        "sh -c 'printf %s:%s:%s "
