...
description: Reference guide for building new agent-meow harness integrations — covers SDK/subprocess harnesses and native harnesses as separate tracks, each with their own feature matrix, implementation patterns, and prioritized checklist.
---

# Harness integration guide

This skill describes the **feature matrix** every agent-meow harness must
consider. Use it when planning, reviewing, or implementing a new harness.

agent-meow has two distinct harness tracks with different architectures and
feature sets:
...
- **Native harnesses** — wrap a vendor's own TUI or server and mirror its
  output into agent-meow. They observe and relay, rather than drive.

---

## Part 1 — SDK / subprocess harnesses

These harnesses run the vendor model directly and bridge agent-meow tools into
...
|---|---|
| **Connects to agent-meow MCP** | Harness exposes/consumes tools via the MCP protocol (in-proc SDK MCP server) |
...
| **Streaming** | Harness forwards token-level or delta-level streaming to the agent-meow forwarder |
| **agent-meow policies** | Harness enforces agent-meow-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the harness surfaces the approval request in the agent-meow web UI so the user can approve or deny |
...
| **Tool-boundary steer** | agent-meow can inject steering text at tool-call boundaries |
| **Resume/fork from agent-meow transcript** | Rebuild a conversation from a stored agent-meow transcript (replay history, seed prompt, or vendor session ID) |
...
| **Cost tracking** | Harness reports token usage and cost data back to agent-meow for each turn |

### MCP connectivity

The harness must bridge agent-meow's builtin MCP tools so the model can call
...
The harness must support the agent-meow policy engine's three verdicts at two
checkpoints:
...
or tool result in the agent-meow web UI as an approval card, then relay the
...
| Vendor session ID | Relies on the vendor's own session persistence (no agent-meow-side rebuild) |
...
- [ ] Connects to agent-meow MCP (in-proc SDK MCP server or vendor-specific bridge)
- [ ] Model override works (or document vendor lock-in)
- [ ] Auth is configured and documented (setup flow in `omni setup`)
- [ ] Streaming forwards to the agent-meow forwarder
- [ ] agent-meow policies enforce tool-use rules
...
- [ ] Resume/fork rebuilds conversation from agent-meow transcript
...
agent-meow. They relay the vendor's conversation into the agent-meow session.
...
| **Connects to agent-meow MCP** | Whether the native harness connects to the agent-meow MCP server |
...
| **agent-meow policies** | Whether the native harness enforces agent-meow-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the native harness surfaces the approval request in the agent-meow web UI so the user can approve or deny |
| **Interrupt** | User can abort a running turn |
| **Bidirectional sync (TUI->Omni)** | TUI output mirrors into the agent-meow conversation |
| **In-harness session-cmd sync** | Supports `clear`, `fork`, `resume`, `switch` commands from agent-meow |
| **Resume/fork from agent-meow transcript** | Can rebuild conversation from agent-meow transcript (native rebuild, or fresh launch) |
...
| **Cost tracking** | Native harness reports token usage and cost data back to agent-meow for each turn |
...
- [ ] Connects to agent-meow MCP
- [ ] Auth configured (vendor login / config)
- [ ] Streaming forwarder works (deltas preferred; complete-only acceptable)
- [ ] agent-meow policies enforce tool-use rules (ALLOW / ASK / DENY at both tool call and tool result)
- [ ] Native elicitation surfaces tool-approval requests to web UI
- [ ] Interrupt aborts the running turn
- [ ] Bidirectional sync mirrors TUI output into agent-meow conversation
- [ ] Cost tracking reports token usage and cost per turn
...
- [ ] Session commands (clear, fork, resume) work from agent-meow
- [ ] Resume/fork rebuilds from agent-meow transcript
- [ ] Reasoning tokens are forwarded
