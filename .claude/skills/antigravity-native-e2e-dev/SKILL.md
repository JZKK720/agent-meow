...
description: Spin up a live local agent-meow server + runner and exercise the native Antigravity (agy) TUI harness (antigravity-native) end-to-end — launch the real `agy` CLI via `agent-meow antigravity`, drive turns through the web UI, smoke-test, and bug-bash. Load when developing, testing, or debugging the antigravity-native harness (agent_meow/inner/antigravity_native_executor.py, agent_meow/antigravity_native.py, antigravity_native_bridge.py, antigravity_native_rpc.py, antigravity_native_reader.py, antigravity_native_launch.py) or its agy launch / RPC mirror / tmux delivery / OAuth / MCP-relay behavior. NOT the in-process `antigravity` Gemini SDK harness.
---
...
`agy` CLI, installed from `antigravity.google/cli/install.sh`). `agent-meow
...
```
your TTY ── (attach / pexpect) ──► agent-meow antigravity (CLI, local)
                                        │ ensures
                                        ▼
...
2. **Read path = RPC.** `antigravity_native_reader` polls/streams agy's connect-RPC
   trajectory steps and mirrors them into the agent-meow session.
3. **Control = RPC.** Interrupt is `CancelCascadeSteps`; a tool/permission prompt
   is answered via `HandleCascadeUserInteraction` (surfaced as an agent-meow
   elicitation).
...
2. **The `agy` CLI is on PATH** (or at `~/.local/bin/agy`) — the harness can't
   ...
   .venv/bin/python -c "from agent_meow.onboarding.harness_readiness import harness_is_configured; print('antigravity-native ready:', harness_is_configured('antigravity-native'))"
   ```
3. **`agy` is signed in (OAuth).** agy is **OAuth-only** — it has no `agy login`;
   ...
   ```bash
   .venv/bin/python -c "from agent_meow.onboarding.gemini_auth import gemini_login_detected; print('agy oauth token present:', gemini_login_detected())"
...
(`agent-meow antigravity --server ""` also auto-spawns a persistent local server and
...
`agent-meow antigravity` **attaches an interactive TUI**, so run it where you can
...
```bash
.venv/bin/meow antigravity --server "$SERVER" 2>&1   # attaches the agy TUI; leave it running
...
`claude-native-e2e-test` skill's `cuj_driver.py`: spawn `agent-meow antigravity
...
defaults to the agent-meow conversation id):

```bash
.venv/bin/python -c "from agent_meow.antigravity_native_bridge import bridge_dir_for_bridge_id as d; print(d('$CONV'))"
# ~/.agent_meow/antigravity-native/<sha256(bridge_id)[:32]>/
...
#   bridge.json    <- token for the agent-meow MCP relay (sys_* tools)
...
- The **isolated HOME** (`agy-home/`) is why your real `~/.gemini` is never
  ...
  agy's `/mcp` panel should show `✓ agent-meow` with the `sys_*` tools.
...
| agent-meow MCP relay (`sys_*`) | in the agy TUI run `/mcp` → expect `✓ agent-meow`; prompt agy to `sys_session_list` / spawn a sub-agent |
| Permission elicitation | with a tool that needs approval, agy's `request-review` surfaces as an **agent-meow elicitation** (interaction bridge); answer it in the web UI and confirm the tool runs |
...
| Resume | stop, `agent-meow antigravity --server "$SERVER" --resume "$CONV"`; `--resume` (no value) opens the antigravity-native picker |
| Concurrency / leaks | drive several sessions; sweep for orphaned `agy` / tmux after teardown |

## Gotchas (these cost real time)

1. **It's a TUI, not `omni run`.** Use `agent-meow antigravity`. The executor only
...
2. **`config.yaml`'s `server:` defaults to a remote server.** Always pass
   ...
   (allowlist: `agent_meow/spec/_omnigent_compat.py`).
...
- **Executor (write path — types into the TUI):** `agent_meow/inner/antigravity_native_executor.py`
- **Harness wrap (`harness: antigravity-native`):** `agent_meow/inner/antigravity_native_harness.py`
- **CLI launch / daemon-runner / tmux attach:** `agent_meow/antigravity_native.py`
  (`run_antigravity_native`); CLI command `antigravity(...)` in `agent_meow/cli.py`
- **agy argv / auth-mode / permission flag:** `agent_meow/antigravity_native_launch.py`
- **Bridge (state, tmux delivery, isolated HOME, MCP relay):** `agent_meow/antigravity_native_bridge.py`
- **connect-RPC client (port discovery, send/cancel/interaction):** `agent_meow/antigravity_native_rpc.py`
- **RPC read driver (trajectory mirror):** `agent_meow/antigravity_native_reader.py`
- **Steps / interactions / audit:** `agent_meow/antigravity_native_steps.py`,
  `agent_meow/antigravity_native_interactions.py`, `agent_meow/antigravity_native_audit.py`
- **OAuth detection:** `agent_meow/onboarding/gemini_auth.py`
...
- **Permission gating is all-or-nothing + post-hoc.** agy honors only
  ...
  auto-bypasses and the genuine agent-meow gate is the elicitation + post-hoc audit
...
# rm -rf "$(.venv/bin/python -c "from agent_meow.antigravity_native_bridge import bridge_dir_for_bridge_id as d; print(d('$CONV'))")"
```
