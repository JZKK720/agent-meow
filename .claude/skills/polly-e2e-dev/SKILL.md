...
- **guardrails** (`agent_meow.inner.nessie.policies`) — `blast_radius` (deny
...
```bash
uv run --frozen python -c "import agent-meow; print('ok')"   # builds .venv
```
...
- **Guardrail policies:** `agent_meow/inner/nessie/policies.py`
- **Runner-side gate:** `agent_meow/runner/policy.py`; server-side tool-call
  enforcement: `agent_meow/server/routes/sessions.py`
...
`agent_meow.host._daemon_entry` / `runner._entry` / `harnesses._runner`
...
pgrep -af "$(pwd)/.venv/bin/python -m agent-meow" | grep -E "_entry|_runner|_daemon" || echo clean
```
