...
:mod:`~?agent_meow.kimi_native_credentials`) so the running ``kimi`` TUI invokes
...
- ``evaluate-policy`` — the ``PreToolUse`` deny-gate. Mirrors
  :func:`~?agent_meow.claude_native_hook._main_evaluate_policy`: it converts the
  ...
  with :func:`~?agent_meow.native_policy_hook.hook_payload_to_evaluation_request`),
  ...
  runs — agent-meow enforces its deny-policy without silencing the user's
...
- ``permission-request`` — the interactive web-UI approval. Kimi fires
  ...
  :data:`~?agent_meow.kimi_native_bridge.APPROVE_KEY` "Approve once" /
  :data:`~?agent_meow.kimi_native_bridge.DENY_KEY` "Reject"). Fail-safe: on no
...
def _parse_bridge_dir_args(argv: list[str], prog: str) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog=f"agent_meow.kimi_native_hook {prog}")
