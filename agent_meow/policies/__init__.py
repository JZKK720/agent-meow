...
- :class:`EvaluationContext`, :class:`PolicyResult`,
  ...
  evaluate boundary (see :mod:`~?agent_meow.policies.types`).
- :class:`Policy` ABC (:mod:`~?agent_meow.policies.base`) and
...
parking, fail-closed) live in :mod:`~?agent_meow.runtime.policies`.

Agent-author callables should import :class:`EvaluationContext`
and :class:`PolicyResult` from here, not from
``agent_meow.spec.types`` — those are runtime evaluation
...
from agent_meow.policies.schema import (
    PolicyCallable,
