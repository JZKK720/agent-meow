...
if TYPE_CHECKING:
    ...
    # agent_meow.policies.types); PhaseSelector.matches takes
...
class LabelDef:
    ...
    conversation (see POLICIES.md Â§10).
...
class FunctionPolicySpec(PolicySpec):
    """
    A policy backed by a Python callable (see POLICIES.md Â§9.1).
...
class AgentSpec:  # type: ignore[explicit-any]  # params: dict[str, Any] field (see below)
    ...
    :param async_enabled: Whether the LLM-callable async-dispatch
        ...
        ``designs/SERVER_HARNESS_CONTRACT.md`` Â§Async work +
    ...
    :param os_env: The agent's OS environment, e.g.
        ...
        ``"inherit"`` string sentinel from agent_meow inline
    ...
    :param timers: Whether the LLM-callable timer builtins
        ...
        stack uses in-memory asyncio tasks; agent-meow supersedes that
