...
def _parse_os_env(
    ...
    :class:`~?agent_meow.inner.datamodel.OSEnvSpec` for the
...
def _parse_terminals(
    ...
    Native agent-meow YAML mirrors the omnigent-compat ``terminals:`` shape — a
...
def _parse_os_env_sandbox(
    ...
    if raw_type is None:
        ...
        from agent_meow.inner.sandbox import _default_sandbox_for_platform
...
def _parse_egress_rules(raw: object) -> list[str] | None:
    ...
    :func:`~?agent_meow.inner.egress.rules.parse_rule` so syntax
    ...
    if not raw:
        return None
    from agent_meow.inner.egress.rules import parse_rule
