...
from agent_meow.llms.routing import infer_harness_from_model as _infer_harness_from_model
...
def _translate_tools_to_omnigent(spec: AgentSpec) -> dict[str, Tool]:
    ...
    :class:`~?agent_meow.tools.FunctionTool`. Filesystem paths
...
def _sub_spec_to_agent_tool(sub: AgentSpec) -> AgentTool:
    ...
    :returns: A populated :class:`~?agent_meow.tools.AgentTool`.
    """
...
# parser (``agent_meow.spec.parser._parse_guardrails``) is invoked
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    from agent_meow.spec.parser import _parse_guardrails
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    # ``agent_meow.inner.loader.load_agent_def``) wins over ``prompt:``
    ...
    # translation time — the agent-meow path spawns each child as an
...
def _translate_executor_from_def(
    ...
    *,
    ...
    The ``harness`` and ``profile`` fields from agent_meow land in
    ...
    if oa_executor is not None and oa_executor.auth is not None:
        ...
        from agent_meow.spec.parser import _parse_executor_auth
