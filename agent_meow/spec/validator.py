...
from agent_meow.spec.types import AgentSpec, ToolRuntime
...
# ``agent_meow.server.routes.sessions``.
...
def _validate_local_tools(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    # spec load (the validator runs from ``agent_meow.spec``,
    # which is a lower layer than ``agent_meow.tools``).
