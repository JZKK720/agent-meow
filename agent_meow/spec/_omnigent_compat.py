...
if TYPE_CHECKING:
    ...
    from agent_meow.spec.validator import ValidationResult
...
# ``agent_meow.inner.open_responses_sdk.OpenResponsesExecutor``
...
def diagnose_yaml_rejection(path: Path) -> str:
    ...
    Used by ``agent_meow.spec.load`` to produce an actionable error
...
def load_omnigent_yaml(
    ...
    *,
    ...
    :param prune_invalid_sub_agents: When ``True``, sub-agents that
        ...
        See :func:`~?agent_meow.spec.load` for the full rationale — this
    ...
    if prune_invalid_sub_agents:
        ...
        # before the root validation gate below; see agent_meow.spec.load.
        from agent_meow.spec import _prune_invalid_sub_agents
