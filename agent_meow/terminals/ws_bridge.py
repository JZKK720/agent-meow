...
Lives under :mod:`~?agent_meow.terminals` (alongside the
...
- :mod:`~?agent_meow.server.routes.terminal_attach`, when no runner WS
  factory is configured (in-process / test setups, where the tmux
  socket lives on the same filesystem the server can see).
- :mod:`~?agent_meow.runner.app`, where the runner exposes its own WS
