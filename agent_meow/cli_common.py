"""Shared constants and guards for the CLI, importable without ``agent_meow.cli``.

The native coding-agent subcommands live in :mod:`agent_meow.cli_native`, which
...
that imports nothing from ``agent_meow.cli`` —lets both ``cli`` and ``cli_native``
