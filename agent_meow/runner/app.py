...
if TYPE_CHECKING:
    ...
    from agent_meow.codex_native_app_server import CodexAppServerClient
...
from agent_meow.harness_plugins import load_object, model_env_keys, spawn_env_builders
from agent_meow.llms.summarize import (
...
from agent_meow.runner.codex.goal import CodexGoalRunner
...
from agent_meow.runner.session_init_protocol import (
    RunnerSessionInitEnvelope,
    parse_runner_session_init_envelope,
)
from agent_meow.runtime.harnesses.process_manager import HarnessProcessManager, NoLiveHarnessError
from agent_meow.server.schemas import (
    BackgroundSessionTitleRequest,
    BackgroundSessionTitleResponse,
)
from agent_meow.spec.skill_sources import SkillSourceContext, resolve_harness_skills
from agent_meow.spec.types import AgentSpec, LocalToolInfo, SkillSpec
from agent_meow.terminals.control_bridge import bridge_tmux_control_to_websocket
from agent_meow.terminals.ws_bridge import (
    WS_CLOSE_TERMINAL_NOT_FOUND,
    bridge_tmux_pty_to_websocket,
)
from agent_meow.tools.builtins.load_skill import (
    find_skill_by_name,
...
async def _generate_claude_native_background_title(
    ...
    *,
    ...
    from agent_meow.claude_launcher import resolve_claude_launch
...
# ``omnigent attach``) for runner-hosted SDK sessions â€?the SDK mirror of
...
def _publish_tmux_target_for_bridge(
    *,
    ...
    from agent_meow.claude_native_bridge import bridge_dir_for_bridge_id, write_tmux_target
...
async def _refresh_claude_permission_hook_auth(
    *,
    ...
    """
    from agent_meow.claude_native_bridge import update_permission_hook_auth_headers
    ...
    while True:
        ...
        try:
            ...
            raise
        except Exception:  # noqa: BLE001 â€?retain the last still-valid snapshot
...
class _CodexNativeLaunchConfig:
    ...
    :param fork_source_id: SOURCE conversation id stamped on a forked
        clone (``agent_meow.fork.source_id``), used to locate the
    ...
    :param fork_source_external_id: SOURCE Codex thread id stamped on a
        forked clone (``agent_meow.fork.source_external_session_id``),
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a
        native target (``agent_meow.fork.carry_history``); when no source
    ...
    :param bypass_sandbox: ``True`` when the session opted into Codex's
        DANGEROUS full-bypass stance (``agent_meow.codex_native.bypass_sandbox``
...
class _PiNativeLaunchConfig:
    ...
    :param fork_source_external_id: SOURCE Pi session id stamped on a forked
        clone (``agent_meow.fork.source_external_session_id``); consulted only
        when the clone has no native session of its own yet.
    :param fork_carry_history: ``True`` on a forked clone bound to a native
        target (``agent_meow.fork.carry_history``); when no source session
...
class _OpenCodeNativeLaunchConfig:
    ...
    :param fork_carry_history: ``True`` on a forked clone whose prior
        ...
        (``agent_meow.fork.carry_history``); opencode has no native session to
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    from agent_meow.opencode_native_forwarder import OpenCodeNativeForwarder
    ...
    from agent_meow.opencode_native_bridge import xdg_config_home_for_bridge_dir
    ...
    # model can call sys_*/load_skill/web_fetch â€?the real "connects to Omnigent
    ...
    # through Omnigent's policy engine via the forwarder's permission gate â€?
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) â€?we'll rehydrate from the Omnigent transcript
...
def _build_opencode_policy_evaluator(
    *,
    ...
    :returns: An async evaluator returning a verdict mapping, or a deny
        verdict on failure.
    """
    from agent_meow.opencode_native_permissions import OPENCODE_NATIVE_HARNESS
...
def _opencode_native_model_from_spec(agent_spec: Any | None) -> str | None:
    ...
    if agent_spec is None:
        return None
    try:
        from agent_meow.runtime.workflow import _resolve_spec_model
...
def _pi_args_have_provider(args: list[str]) -> bool:
    ...
    Omnigent must not inject the ``omnigent setup`` provider on top â€?the
...
def _build_pi_native_args(
    *,
    ...
    if approve:
        ...
        # terminal to answer it â€?mirroring ensure_claude_workspace_trusted.
        args.append("--approve")
...
async def _resolve_pi_resume_session(
    *,
    ...
    from agent_meow.pi_native_resume import ensure_local_pi_resume_session, mint_pi_session_id
    ...
    model = ""
    try:
        from agent_meow.pi_native_credentials import resolve_pi_native_provider
...
async def _auto_create_pi_terminal(
    ...
    *,
    ...
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agent_meow.pi_native import resolve_pi_executable
    ...
    from agent_meow.pi_native_bridge import extension_path as pi_extension_path
    from agent_meow.runner._entry import _make_auth_token_factory
    ...
    try:
        from agent_meow.runner.tool_dispatch import build_native_relay_tool_schemas
    ...
    from agent_meow.pi_native import pi_supports_approve
    ...
    if not _pi_args_have_provider(launch_config.terminal_launch_args or []):
        from agent_meow.pi_native_credentials import (
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from agent_meow.cursor_native import resolve_cursor_executable
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from agent_meow.cursor_native import is_valid_cursor_chat_id
    ...
    from agent_meow.cursor_native_forwarder import clear_cursor_bridge_state, preseed_resume_state
    from agent_meow.cursor_native_status import clear_cursor_status_state
    from agent_meow.cursor_native_usage import clear_cursor_usage_state
    ...
    if launch_config.fork_carry_history and not preseeded and server_client is not None:
        try:
            from agent_meow.claude_native import _fetch_all_session_items_for_claude_resume
    ...
    from agent_meow.cursor_native_forwarder import supervise_cursor_forwarder
    from agent_meow.cursor_native_permissions import supervise_cursor_transcript_elicitations
    from agent_meow.cursor_native_usage import supervise_cursor_usage_forwarder
    ...
    async def _supervise_cursor_native_bridges() -> None:
        ...
        calls (see :mod:`agent_meow.cursor_native_permissions`) â€?more reliable
        ...
        :mod:`agent_meow.cursor_native_usage`).
        """
...
async def _auto_create_goose_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from agent_meow.goose_native import resolve_goose_executable
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from agent_meow.goose_native_bridge import bridge_dir_for_session_id, write_tmux_target
    from agent_meow.goose_native_forwarder import clear_goose_bridge_state
    ...
    from agent_meow.goose_native_forwarder import supervise_goose_forwarder
    from agent_meow.goose_native_permissions import supervise_goose_approval_mirror
    ...
    async def _supervise_goose_native_bridges() -> None:
        ...
        a web elicitation (see :mod:`agent_meow.goose_native_permissions`).
        """
        await asyncio.gather(
...
async def _auto_create_hermes_terminal(
    ...
    *,
    ...
    ``cwd`` + ``started_at`` floor (see :mod:`agent_meow.hermes_native_forwarder`).
    ...
    :returns: Created terminal resource view.
    """
    from agent_meow.hermes_native import resolve_hermes_executable
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    from agent_meow.hermes_native_forwarder import clear_hermes_bridge_state
    from agent_meow.hermes_native_status import clear_hermes_status_state
        ...
        if _source_db is not None and _source_db.is_file():
            ...
            if _target_db is not None:
                try:
                    ...
                    if _clone_max_id > 0:
                        from agent_meow.hermes_native_forwarder import (
                            _ForwardState,
                            _write_state,
                        )
    ...
    from agent_meow.hermes_native_bridge import read_hermes_home
    from agent_meow.hermes_native_forwarder import supervise_hermes_forwarder
    from agent_meow.hermes_native_permissions import supervise_hermes_approval_mirror
    ...
    async def _supervise_hermes_native_bridges() -> None:
        ...
        elicitation (see :mod:`agent_meow.hermes_native_permissions`).
        """
...
async def _auto_create_kiro_terminal(
    ...
    *,
    ...
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agent_meow.kiro_native import build_kiro_launch
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from agent_meow.kiro_native_bridge import write_tmux_target
    ...
    from agent_meow.kiro_native_permissions import supervise_kiro_permission_mirror
    from agent_meow.kiro_native_session_forwarder import supervise_kiro_session_forwarder
...
async def _persist_qwen_external_session_id(
    ...
    ``fork_conversation`` stamps as ``agent_meow.fork.source_external_session_id``
...
async def _build_qwen_fork_recording(
    ...
    *,
    ...
    """
    from agent_meow.pi_native_resume import fetch_all_session_items_for_pi_resume
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    """
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agent_meow.qwen_native import resolve_qwen_executable
    ...
    from agent_meow.qwen_native_forwarder import clear_qwen_bridge_state
    ...
    # (``agent_meow.fork.source_external_session_id``). qwen is cleaner than
    ...
    # Expose Omnigent's builtin tools (sys_*, load_skill, web_fetch, â€? to qwen
    ...
    # the default in-terminal prompt (the embedded pane shows it) â€?Omnigent-side
    ...
    from agent_meow.qwen_native_bridge import qwen_session_recording_path
    from agent_meow.qwen_native_forwarder import (
        supervise_qwen_compaction_mirror,
        supervise_qwen_forwarder,
    )
    from agent_meow.qwen_native_permissions import supervise_qwen_approval_mirror
    ...
    async def _supervise_qwen_native_bridges() -> None:
        ...
        elicitations (see :mod:`agent_meow.qwen_native_permissions`); the compaction
        ...
        :func:`agent_meow.qwen_native_forwarder.supervise_qwen_compaction_mirror`).
        """
        await asyncio.gather(
            supervise_qwen_forwarder(
                base_url=server_url,
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    :func:`agent_meow.kimi_native_credentials.build_kimi_session_home`) that
    mirrors the user's global ``kimi login`` (symlinked ``oauth`` / providers)
    and adds the Omnigent tool-policy hooks â€?a ``PreToolUse`` deny-gate and a
    ``PermissionRequest`` read-only surface dispatched to
    :mod:`agent_meow.kimi_native_hook`. The hook subprocess reads its routing
    from ``hook_config.json`` in the bridge dir.

    A background forwarder (:func:`agent_meow.kimi_native_forwarder.
    ...
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agent_meow.kimi_native import resolve_kimi_executable
    from agent_meow.kimi_native_bridge import (
        bridge_dir_for_session_id,
        write_hook_config,
        write_tmux_target,
    )
    from agent_meow.kimi_native_credentials import build_kimi_session_home
    from agent_meow.kimi_native_forwarder import clear_kimi_bridge_state, supervise_kimi_forwarder
    from agent_meow.runner._entry import _make_auth_token_factory
    ...
    # ``agent_meow.kimi_native_hook``. The hook subprocess reads the server URL +
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    from agent_meow.inner.codex_executor import _find_codex_cli
    ...
    if (
        ...
        from agent_meow.codex_native import _clone_codex_rollout, _mint_codex_thread_id
        ...
        except Exception:  # noqa: BLE001 â€?best-effort; fall back to stored items
            cloned_rollout = None
            _logger.warning(
    ...
    if (
        ...
        # the same server-itemsâ†’rollout converter the cross-machine cold resume
        ...
        from agent_meow.codex_native import (
            _ensure_local_codex_resume_rollout,
    ...
    if launch_config.external_session_id is not None and original_external_session_id is not None:
        from agent_meow.codex_native import _ensure_local_codex_resume_rollout
    ...
    if launch_config.external_session_id is None:
        ...
        from agent_meow.codex_native_bridge import CodexNativeBridgeState, write_bridge_state
...
async def _codex_discover_thread_and_forward(
    *,
    ...
    from agent_meow.codex_native_forwarder import (
        supervise_forwarder,
    ...
    try:
        ...
        # ``agent_meow.fork.source_external_session_id`` from
...
async def _codex_forward_known_thread(
    *,
    ...
    :returns: None. Runs until cancelled or the app-server connection
        closes.
    """
    from agent_meow.codex_native_forwarder import supervise_forwarder
    from agent_meow.runner._entry import (
        _make_auth_token_factory,
        _RunnerDatabricksAuth,
    )
...
async def _run_antigravity_reader(
    *,
    ...
    """
    from agent_meow.antigravity_native_reader import run_reader_with_bridge
...
async def _auto_create_antigravity_terminal(
    ...
    *,
    ...
    :func:`agent_meow.antigravity_native._launch_antigravity_terminal`, which
    ...
    *attended* (``headless=False``). Auto-bypass comes only from the user's
    ...
    from agent_meow.antigravity_native_launch import build_agy_launch
...
def _mint_runner_agy_conversation_id() -> str:
    ...
    :func:`agent_meow.antigravity_native._mint_agy_conversation_id`.
...
async def _cold_start_agy_conversation(
    ...
    *timeout_s*, or ``StartCascade`` erroring) must NOT abort the auto-create:
    ...
    from agent_meow.antigravity_native_rpc import (
        AntigravityRpcError,
...
def _terminal_tmux_pane(
    ...
    # instance (see agent_meow.inner.terminal). Guard defensively so a registry
...
async def _codex_session_needs_runner_terminal(
    ...
    - **Sub-agent children** (dispatched server-side via
      ...
      e.g. polly run via ``omnigent run --server`` â€?silently never got
...
def _pi_native_model_from_spec(agent_spec: AgentSpec | ResolvedSpec | None) -> str | None:
    ...
    :func:`agent_meow.pi_native_credentials.resolve_pi_native_provider`). The
...
def _cursor_fork_history_preamble(items: list[dict[str, Any]]) -> str:
    ...
    :func:`agent_meow.cursor_native_bridge.wrap_fork_preamble`.
...
def _build_claude_native_base_args(
    *,
    ...
    :param resume_external_session_id: Claude-native session id to
        ...
        :func:`agent_meow.claude_native._clone_claude_transcript`), so
    ...
    """
    from agent_meow.reasoning_effort import CLAUDE_EFFORTS
...
def _claude_terminal_env_unset(
    ...
    API key" menu, whose selected row uses the same ``â¯`` glyph the tmux
    ...
    :param claude_config: The resolved native launch config, or ``None``
        (Claude's own login) â€?which still strips the nested-session marker.
...
def _ensure_orchestrator_skills_in_bundle(
    ...
    unconditionally for every agent â€?every ``omnigent claude`` /
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    from agent_meow.claude_native_forwarder import reset_transcript_forward_state
    from agent_meow.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    if server_client is not None and session_external_id is not None:
        from agent_meow.claude_native import _ensure_local_claude_resume_transcript
        ...
        from agent_meow.claude_native import _clone_claude_transcript
    ...
    # managed-host path. Identity by default. See agent_meow.claude_launcher.
    ...
    # ``claude_native.py``.
    from agent_meow.claude_native_forwarder import supervise_forwarder
...
async def _auto_create_repl_terminal(
    ...
    *,
    ...
    like the claude-/codex-native terminals â€?with the Omnigent REPL as
    the TUI.
    ...
    OIDC token from ``omnigent login`` â†?``~/.databrickscfg``), which
    ...
    :param server_client: Omnigent server client used to stamp the
        ``agent_meow.ui: terminal`` presentation label that makes the web
...
async def _claude_native_bridge_id_for_session(
    *,
    ...
    :returns: Opaque bridge id from
        ``agent_meow.claude_native.bridge_id`` when present, otherwise
...
async def _antigravity_native_terminal_arrives_via_transfer(
    *,
    ...
    :func:`agent_meow.antigravity_native_reader._rotate_session_for_cascade`) binds the
...
def _looks_like_file_path(path: str) -> bool:
    ...
    source extension (see :func:`agent_meow.spec.parser._discover_local_tools`).
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure â€?the POST may never have reached agent_meow.
        return True
...
def create_runner_app(
    *,
    ...
    if terminal_registry is not None:
        from agent_meow.runtime import _globals as _rt_globals
    ...
    # ``agent_meow.runner.pending_approvals`` so the runner-side
    # policy gate (``agent_meow.runner.tool_dispatch``) can register
    ...
    def _has_active_work() -> bool:
        ...
        * Live ``sys_call_async`` tasks in ``_session_async_tasks`` (not
          ``done()``) â€?their results still need to land in the inbox.
        * Live ``sys_timer_set`` tasks in ``_session_timers`` â€?firing
          must POST into the session; shutdown would drop the schedule.
        * Parked approval Futures in ``pending_approvals`` â€?the human
          gate is still open.
    ...
    def _drain_session_streams() -> None:
        ...
        as a clean return â€?no ``runner_disconnected`` failure and no durable
        error label â€?so an idle-reaped session settles quietly rather than
    ...
    from agent_meow.runtime.filesystem_registry import (
        FilesystemRegistry,
    ...
    async def _initialize_session(body: dict[str, Any]) -> JSONResponse:
        ...
        if spec is not None:
            ...
            if harness_name == "pi-native" and spawn_env is None:
                from agent_meow.pi_native_bridge import build_pi_native_spawn_env
            ...
            if harness_name == "cursor-native" and spawn_env is None:
                from agent_meow.cursor_native_bridge import build_cursor_native_spawn_env

                spawn_env = build_cursor_native_spawn_env(session_id)
            if harness_name == "kiro-native" and spawn_env is None:
                from agent_meow.kiro_native_bridge import build_kiro_native_spawn_env
            ...
            if harness_name == "goose-native" and spawn_env is None:
                from agent_meow.goose_native_bridge import build_goose_native_spawn_env
            ...
            if harness_name == "qwen-native" and spawn_env is None:
                from agent_meow.qwen_native_bridge import build_qwen_native_spawn_env

                spawn_env = build_qwen_native_spawn_env(session_id)
            if harness_name == "kimi-native" and spawn_env is None:
                from agent_meow.kimi_native_bridge import build_kimi_native_spawn_env
    ...
    async def _codex_native_bridge_state_for_session(
        ...
        *,
        ...
        lookup first resolves ``agent_meow.codex_native.bridge_id`` from
    ...
    async def _handle_codex_native_interrupt(conv_id: str) -> Response:
        ...
        *own* ``[Request interrupted by user]`` record, which is real.
        ...
        the way the Codex TUI does â€?``turn/interrupt`` with an EMPTY turn
        ...
        """
        from agent_meow.codex_native_app_server import client_for_transport
        from agent_meow.codex_native_bridge import (
            CODEX_NATIVE_BRIDGE_ID_LABEL_KEY,
            bridge_dir_for_bridge_id,
        ...
        if pending_mcp:
            ...
            # owner-only â€?without this post the web band and snapshot stay
    ...
    async def _handle_pi_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from agent_meow.pi_native_bridge import bridge_dir_for_session_id, enqueue_interrupt
    ...
    async def _handle_pi_native_model_change(
        ...
        propagate a web-picked model live â€?without relaunching the pane â€?
        ...
        :returns: 204 when the payload was queued or skipped; 503 if the
            ...
            on the next spawn).
        """
        from agent_meow.pi_native_bridge import bridge_dir_for_session_id, enqueue_model_change
    ...
    async def _teardown_session_terminals(conv_id: str) -> None:
        ...
        from agent_meow.runner.tool_dispatch import _publish_terminal_deleted_event
    ...
    async def _handle_cursor_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from agent_meow.cursor_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_goose_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from agent_meow.goose_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_kiro_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from agent_meow.kiro_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_kimi_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from agent_meow.kimi_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_goose_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from agent_meow.goose_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_kiro_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from agent_meow.kiro_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_kimi_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from agent_meow.kimi_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_hermes_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from agent_meow.hermes_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_hermes_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from agent_meow.hermes_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_qwen_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from agent_meow.qwen_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_qwen_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from agent_meow.qwen_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        ...
        session ``model`` field â†?bridge-state ``model_override``) â€?Omnigent
        creates the session without a model, so the session field alone is
        usually empty â€?ask opencode to compact, and return 200 so the Omnigent
        ...
        """
        from agent_meow.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
    ...
    async def _opencode_native_model_options(conv_id: str) -> list[dict[str, Any]]:
        ...
        from agent_meow.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from agent_meow.opencode_native_client import OpenCodeClient
    ...
    async def _handle_cursor_native_compact(conv_id: str) -> Response:
        ...
        :mod:`agent_meow.cursor_native_forwarder` when it observes cursor write the
        ...
        """
        from agent_meow.cursor_native_bridge import bridge_dir_for_session_id, inject_user_message
    ...
    async def _handle_pi_native_compact(conv_id: str) -> Response:
        ...
        """
        from agent_meow.pi_native_bridge import bridge_dir_for_session_id, enqueue_compact
    ...
    def _inject_codex_compact(socket_path: str, target: str) -> None:
        ...
        sequence that :func:`~agent_meow.claude_native_bridge.inject_slash_command`
        ...
        """
        from agent_meow.claude_native_bridge import _run_tmux
    ...
    async def _handle_qwen_native_compact(conv_id: str) -> Response:
        ...
        :func:`agent_meow.qwen_native_forwarder.supervise_qwen_compaction_mirror` when
        ...
        """
        from agent_meow.qwen_native_bridge import bridge_dir_for_session_id, submit_user_message
    ...
    async def _handle_claude_native_cost_popup(
        ...
        terminal â€?who never sees the web card â€?the Omnigent server forwards a
    ...
    async def _handle_codex_native_cost_popup(
        ...
        harness-agnostic :func:`agent_meow.native_cost_popup.launch_cost_popup`.
        ...
        """
        from agent_meow.native_cost_popup import launch_cost_popup
    ...
    async def _handle_opencode_native_blocked_notice(
        ...
        :returns: 204 once dispatched (or skipped); 503 if launching raised.
        """
        from agent_meow.native_cost_popup import launch_blocked_notice
    ...
    async def _native_cost_popup_config_file(conv_id: str, harness: str) -> Path:
        ...
        """
        from agent_meow.cli_auth import databricks_request_headers
        from agent_meow.opencode_native_bridge import write_cost_popup_config
        from agent_meow.runner._entry import _make_auth_token_factory

        if harness == "claude-native":
            from agent_meow import claude_native_bridge as _cnb
            ...
            from agent_meow import codex_native_bridge as _cxb
    ...
    async def _repop_pending_cost_popup_on_attach(
        ...
        if harness not in ("claude-native", "codex-native", "opencode-native"):
            return
        from agent_meow.native_cost_popup import launch_cost_popup, wait_for_tmux_client
    ...
    async def _ensure_comment_relay_started(
        ...
        *,
        ...
        async def _relay_tool_executor(
            ...
            :class:`~agent_meow.runner.proxy_mcp_manager.ProxyMcpManager`
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if conv not in _session_tool_schemas:
            ...
            if cached_spec is not None:
                try:
                    from agent_meow.tools.manager import (
                        ToolManager,
                    )
        ...
        if cached_spec and cached_spec.mcp_servers:
            from agent_meow.runner.mcp_manager import compute_spec_hash
    ...
    async def _stream_message_to_harness(
        ...
        if harness_name == "claude-native" and spawn_env is None:
            from agent_meow.claude_native_bridge import build_claude_native_spawn_env
    ...
    async def post_session_events(
        ...
        / ``ApprovalEvent``) â€?see
        :class:`agent_meow.runtime.harnesses._scaffold.InboundEventRequest`.
    ...
    async def _recreate_repl_terminal(
        ...
        exits â€?the user pressing Ctrl+C inside the REPL, or ``omnigent
    ...
    async def search_environment_files(
        ...
        """
        from agent_meow.runner.environment_filesystem import (
    ...
    async def list_filesystem_changes(
        ...
        """
        from agent_meow.runtime.filesystem_registry import GitStatusUnavailable
    ...
    async def read_environment_file_diff(
        ...
        from agent_meow.entities.environment_filesystem import InvalidPath
        from agent_meow.runner.environment_filesystem import _validate_path
        ...
        from agent_meow.runner.environment_filesystem import CallerProcessFilesystem
    ...
    # (Omnigent-configured provider OR pi's own ``/login``) â€?a launch-written
    ...
    async def mcp_execute(session_id: str, request: Request) -> JSONResponse:
        ...
        **after** TOOL_CALL policy evaluation.  The Omnigent server owns policy
        ...
        - **Runner-local tools** (bare names: ``sys_os_read``,
          ...
          :func:`~agent_meow.runner.tool_dispatch.execute_tool` using
        ...
        if method == "tools/call":
            ...
            if "__" in tool_name:
                ...
                try:
                    from agent_meow.tools.mcp import McpElicitationRequired
    ...
    def _resolve_summarize_connection(
        ...
        """
        from agent_meow.spec.types import ApiKeyAuth, DatabricksAuth, ProviderAuth
        ...
        if auth is None and not _spec_has_legacy_profile:
            from agent_meow.runtime.workflow import _load_global_auth
    ...
    if (
        ...
        from agent_meow.native_cost_popup import _list_tmux_clients
        from agent_meow.runner.tool_dispatch import _publish_terminal_deleted_event
        from agent_meow.terminals.pane_reaper import NativePaneReaper, PaneRef
...
def create_runner_app_from_env() -> FastAPI:
    ...
    :class:`~agent_meow.runner.transports.tcp.RunnerTCPSubprocess` and
    :class:`~agent_meow.runner.transports.uds.RunnerSubprocess`.  It is
    intentionally lighter than :func:`agent_meow.runner._entry.create_app`
...
_HARNESS_MODEL_ENV_KEY: dict[str, str] = {
    ...
    # :mod:`agent_meow.inner.kimi_executor`; without this mapping a per-session
...
async def _evaluate_agent_start_gate(
    ...
    """
    from agent_meow.runner.policy import RunnerToolPolicyGate
