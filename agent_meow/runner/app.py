...
if TYPE_CHECKING:
    ...
    from agent_meow.runner.cost_advisor import AdvisorTurnResult
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    # model can call sys_*/load_skill/web_fetch — the real "connects to agent-meow
    ...
    # through agent-meow's policy engine via the forwarder's permission gate —
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) — we'll rehydrate from the agent-meow transcript
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    async def _supervise_cursor_native_bridges() -> None:
        ...
        calls (see :mod:`~?agent_meow.cursor_native_permissions`) — more reliable
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    # Expose agent-meow's builtin tools (sys_*, load_skill, web_fetch, …) to qwen
    ...
    # the default in-terminal prompt (the embedded pane shows it) — agent-meow-side
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    and adds the agent-meow tool-policy hooks — a ``PreToolUse`` deny-gate and a
...
def _merge_advisor_note(
    ...
    :param note_item: The advisor's note message item (see
        :func:`~?agent_meow.runner.cost_advisor._advisor_note_item`), e.g.
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure — the POST may never have reached agent-meow.
        return True
...
def create_runner_app(
    *,
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        ...
        session ``model`` field → bridge-state ``model_override``) — agent-meow
        creates the session without a model, so the session field alone is
        usually empty — ask opencode to compact, and return 200 so the agent-meow
    ...
    async def _handle_claude_native_cost_popup(
        ...
        terminal — who never sees the web card — the agent-meow server forwards a
    ...
    async def _run_turn_advisor(
        ...
        from agent_meow.runner.cost_advisor import maybe_run_advisor
    ...
    def _emit_routing_decision(conv: str, result: AdvisorTurnResult | None) -> None:
        ...
        if result is None:
            return
        from agent_meow.runner.cost_advisor import routing_decision_event
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if cached_spec is not None:
            ...
            from agent_meow.runtime.prompt import (
                build_instructions,
            )
