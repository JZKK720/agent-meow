...
The runner-side cost advisor (:mod:`~?agent_meow.runner.cost_advisor`)
...
:class:`~?agent_meow.cost_plan.AdvisorVerdict` (or ``None``) sizing the
...
:class:`~?agent_meow.llms.client.Client`, which the runner already depends
on.
"""
...
from agent_meow.llms.types import MessageOutput, Response, ResponseStreamEvent
...
class LLMClientLike(Protocol):
    ...
    Matches :class:`~?agent_meow.llms.client.Client`'s ``responses.create``
    surface; declared structurally so tests can pass a scripted stub that
    returns canned :class:`~?agent_meow.llms.types.Response` objects without
...
class _ResponsesLike(Protocol):
    ...
    :meth:`~?agent_meow.llms.client._ResponsesNamespace.create` the judge
    uses, so the real :class:`~?agent_meow.llms.client.Client` satisfies
    ...
    async def create(  # type: ignore[explicit-any]  # mirrors Client.responses.create's Any-typed input/kwargs
        ...
        **kwargs: Any,
        ...
        :class:`~?agent_meow.llms.client.Client` (which also streams); the
...
class LLMJudge:
    ...
    Implements the :class:`~?agent_meow.runner.cost_advisor.Judge` protocol.
    ...
    def __init__(self, config: _JudgeConfig, client: LLMClientLike) -> None:
        ...
        :param client: The LLM client the judge calls; the real
            :class:`~?agent_meow.llms.client.Client` in production, a
...
def _resolve_workspace_creds(profile: str | None) -> Any:  # type: ignore[explicit-any]  # WorkspaceCreds (lazy import keeps module import side-effect free)
    ...
    :returns: The resolved
        :class:`~?agent_meow.runtime.credentials.databricks.WorkspaceCreds`.
...
def build_llm_judge(  # type: ignore[explicit-any]  # executor_config is a YAML-shaped dict
    *,
    ...
    :param databricks_profile: The Databricks profile the brain's gateway
        ...
        :func:`~?agent_meow.runner.cost_advisor._databricks_profile_for_spec`),
        or ``None``. Used only when the judge model is a Databricks id and
        *connection* is ``None``.
    :param client: LLM client override; ``None`` builds the real
        :class:`~?agent_meow.llms.client.Client`. Tests pass a scripted stub —
...
def resolve_advisor_mode(spec_mode: str, override: str | None) -> str | None:
    ...
    :param spec_mode: The mode the spec marker configured, one of
        :data:`~?agent_meow.cost_plan.ADVISOR_MODES` (``"advise"`` /
