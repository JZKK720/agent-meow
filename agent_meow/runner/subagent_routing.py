...
* **Runner side** 鈥?a loopback HTTP relay (:func:`start_subagent_router`)
...
* **Server side** 鈥?the policy (:func:`resolve_subagent_route`), which
...
1. harness hook timeout 鈥?12s (``claude_native_bridge``'s PreToolUse entry,
...
2. hook script HTTP request 鈥?:data:`HOOK_REQUEST_TIMEOUT_S` (8s, defined
   as ``REQUEST_TIMEOUT_S`` in ``agent_meow.inner.hook_scripts.subagent_router``,
   which stays stdlib-only and cannot import this module)
3. runner loopback relay wait 鈥?:data:`RELAY_TIMEOUT_S` (7s)
4. server relay hop 鈥?:data:`SERVER_HOP_TIMEOUT_S` (6s), inside which the
...
if TYPE_CHECKING:
    ...
    from agent_meow.entities.conversation import RoutingDecisionData
...
#: the whole server-side route runs 鈥?candidate preparation and then the
...
#: landing screen, and that same prompt is then submitted inside the harness 鈥?#: where the first-prompt hook would score it a second time, seconds later, for
...
#: skip the router and let it inherit the parent's 鈥?possibly expensive 鈥?#: model, route it on this placeholder so it lands on the router's floor arm.
...
#: arm 鈥?a cheap default, not per-spawn intelligence.
...
# 鈹€鈹€ Enablement gate 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
def routing_enabled(
    ...
    *,
    ...
    鈥?the server routes children of a routed parent), and, where a
    ...
    :param caps: ``RuntimeCaps``-shaped object that must be able to route.
        ``None`` skips that check 鈥?the runner process holds no routing
    ...
    from agent_meow.server.routing_backend import routing_available
...
class SessionRoutingClass:
    ...
    byte-for-byte a plain codex session 鈥?no catalog replacement, no
    ...
    bundled catalog has no entry for, plus 鈥?on a native terminal 鈥?the
    ...
    :param turn_routing: The session still needs the ``UserPromptSubmit``
        ...
        the session 鈥?a web create that routed the model before the pane
        launched, or an earlier turn 鈥?because the hook's only remaining
...
def subagent_routing_enabled(subagent_routing_override: str | None) -> bool:
    ...
    the harness 鈥?``"off"``, unset, and a legacy row still carrying the
    ...
    :param subagent_routing_override: The session's
        ``subagent_routing_override`` 鈥?``"on"``, ``"off"``, or ``None``.
...
class SubagentRouteDecision:
    ...
    ``"databricks-claude-sonnet-5"``) 鈥?never a harness's own tool
    ...
    :param action: ``"allow"`` (spawn unchanged), ``"rewrite"`` (same
        harness, injected model), ``"redirect"`` (cross-harness 鈥?deny
    ...
    :param decision_id: Identity shared by the response and the transcript
        item.
    :param router_source: Which router answered 鈥?``"databricks-aigw"`` or
...
# 鈹€鈹€ Policy 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
def _harness_family(harness: str) -> str | None:
    from agent_meow.server.smart_routing import _HARNESS_FAMILY
...
def model_in_family(family: str | None, model: str) -> bool:
    ...
    if family is None or family == "pi":
        return True
    from agent_meow.model_catalog import model_family_token
...
def candidate_models(
    ...
    *,
    ...
    """Build the harness 鈫?models map offered to the router.
    ...
    cross-harness counterpart 鈥?the only case where a ``redirect`` verdict
    can fire.
    ...
    :param catalog: Live per-session model catalog keyed by worker name
        (:func:`agent_meow.server.smart_routing.fetch_runner_models`). Preferred
    ...
    :returns: Harness 鈫?model ids, cheapest first, empty entries dropped.
    """
...
def _with_unadvertised_arms(models: list[str], family: str | None) -> list[str]:
    ...
    it and the row would otherwise hide it 鈥?leaving a codex session unable
    ...
    if not models or family != _UNADVERTISED_ARM_FAMILY:
        return models
    from agent_meow.codex_model_vocabulary import EXTENDED_CATALOG_MODELS
...
def _routing_task(req: SubagentRouteRequest) -> str:
    ...
    routes 鈥?on :data:`_PLACEHOLDER_TASK` 鈥?so it lands on the router's floor
...
async def resolve_subagent_route(
    ...
    *,
    ...
    :param caps: ``RuntimeCaps``-shaped object. ``None`` reads the
        process-global caps.
    :param available_models: Candidate harness 鈫?models map. ``None``
...
async def _decide(
    ...
    *,
    ...
    # floor arm rather than inheriting the parent's 鈥?possibly expensive 鈥?    # model. Mirrors ucode PR 251's ``default_task_label``. This reverses the
    ...
    from agent_meow.server.routing_backend import (
        backends_from_caps,
        route_with_fallback,
        select_router,
    )
    ...
    # words it. Fail-open still preserves the ask 鈥?a router outage returns an
    ...
    try:
    ...
    except Exception as exc:  # noqa: BLE001 鈥?router outages are a normal path here
        _logger.warning(
        ...
        from agent_meow.server.smart_routing import failure_detail
...
def _requested_match(req: SubagentRouteRequest, model: str | None) -> bool:
    ...
    if not req.requested_model or not model:
        return False
    from agent_meow.server.smart_routing import _bare_id
...
def _ask_note(req: SubagentRouteRequest, model: str) -> str:
    ...
    "honored" is only ever claimed on a match 鈥?the router strictly decides,
    ...
    if _requested_match(req, model):
        return f" Spawn requested {req.requested_model}; honored 鈥?the router picked the same arm."
    return f" Spawn requested {req.requested_model}; overridden 鈥?the router picked {model}."
...
# 鈹€鈹€ Transcript persistence 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
async def persist_subagent_decision(
    ...
    """
    from agent_meow.entities.conversation import NewConversationItem
...
def write_advertisement(
    ...
    *,
    ...
    :param filename: Advertisement file name. Defaults to this module's
        ``subagent_router.json``; ``agent_meow.runner.turn_routing`` passes
    ...
    # The file carries a bearer token, so it is never world-readable 鈥?not even
...
def _handler_factory(
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            except Exception:  # noqa: BLE001 鈥?never wedge the spawn path
                _logger.warning(
...
# 鈹€鈹€ Per-session router lifecycle (runner side) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

_session_routers: dict[str, SubagentRouter] = {}
# Models the relay actually handed back, per session 鈥?the ledger the
...
def session_routing_class(session_id: str) -> SessionRoutingClass:
    ...
    :returns: The recorded class, or :data:`PLAIN_SESSION` when none was
        recorded 鈥?an unknown session must not pay any routing-path cost.
    """
    with _lifecycle_lock:
...
def ensure_session_router(
    ...
    *,
    ...
    start runs under ``_lifecycle_lock`` 鈥?releasing it between the lookup
    ...
    :param server_client: Runner鈫抯erver client used to relay verdicts.
...
def ensure_session_router_quietly(
    ...
    *,
    ...
    advertisement get a router at all 鈥?every other harness (pi, copilot,
    goose, 鈥? is handed no router env vars, so starting one for it would
    ...
    :param routing_class: The session's Smart Routing class, which decides
        ...
        gets one, pinned or auto-harness 鈥?on codex the advertisement is
    ...
    try:
        ...
        # bridge root fails its ownership check 鈥?never a reason to fail
...
def shutdown_session_router(session_id: str, router: SubagentRouter | None = None) -> None:
    ...
    to call twice 鈥?:meth:`SubagentRouter.close` is idempotent 鈥?so every
...
def _prune_router_dirs(router: SubagentRouter) -> None:
    ...
    """
    from agent_meow.claude_native_bridge import subagent_router_bridge_root
...
def router_dir_for_session(session_id: str) -> Path:
    ...
    """
    from agent_meow.claude_native_bridge import ensure_secure_dir, subagent_router_bridge_root
...
def router_env(session_id: str, router_dir: Path, harness: str | None = None) -> dict[str, str]:
    ...
    generated routing ``hooks.json`` on. Only *harness*'s own vars are set 鈥?    a codex executor spawned beneath a claude session would otherwise see
    ...
    from agent_meow.inner.hook_scripts.subagent_router import (
        ROUTER_DIR_ENV_VAR,
        SESSION_ID_ENV_VAR,
    )
