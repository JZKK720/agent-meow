...
:class:`~?agent_meow.runner.cost_judge.Judge` over the turn's user query
and produces ONE :class:`~?agent_meow.cost_plan.AdvisorVerdict` sizing the
...
:func:`~?agent_meow.runner.cost_judge.resolve_advisor_mode`).
"""
...
from agent_meow.runner.cost_judge import build_llm_judge, resolve_advisor_mode
...
def _databricks_profile_for_spec(spec: Any) -> str | None:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    (:func:`~?agent_meow.runtime.workflow._build_claude_sdk_spawn_env`):
...
class Judge(Protocol):
    ...
    The production :class:`~?agent_meow.runner.cost_judge.LLMJudge`
...
def routing_decision_event(verdict: AdvisorVerdict) -> dict[str, Any]:  # type: ignore[explicit-any]  # JSON-shaped SSE event
    ...
    The item type is in :data:`~?agent_meow.entities.conversation.NON_CONTENT_ITEM_TYPES`
...
async def maybe_run_advisor(
    *,
    ...
    :param judge: Judge override; ``None`` builds the production
        :class:`~?agent_meow.runner.cost_judge.LLMJudge` from the spec's
        advisor config.
...
async def _persist_verdict_label(
    ...
    (see ``update_session`` in :mod:`~?agent_meow.server.routes.sessions`).
