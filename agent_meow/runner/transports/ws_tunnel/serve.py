...
# Reconnect backoff: 0.5 s initial, 10 s cap, Â±50% jitter. The
...
# (``agent_meow.chat._wait_for_remote_runner``, currently 60 s).
...
# login page (``https://<workspace>/oidc/oauth2/v2.0/authorize?â€¦``).
...
def _websocket_auth_redirect_url(exc: BaseException) -> str | None:
    ...
    (``https://<workspace>/oidc/oauth2/v2.0/authorize?â€¦``) and the
