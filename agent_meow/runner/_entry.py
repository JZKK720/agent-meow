...
if TYPE_CHECKING:
    ...
    from agent_meow.runner.transports.ws_tunnel.serve import _ASGIApp
...
def _is_login_redirect_or_unauthorized(response: httpx.Response) -> bool:
    ...
    if response.status_code in (401, 403):
        ...
        # in addition to the 302â†?oidc/ bounce; treat both as re-auth signals.
        return True
...
def _make_auth_token_factory(
    ...
    *,
    ...
    Resolution order:
      ...
      4. Databricks OAuth token (refreshed via the SDK) â€?host-keyed
...
def _make_managed_mint_factory(
    ...
    :returns: A sync callable returning a fresh owner JWT, or ``None`` only
        ...
        Databricks Apps OAuth redirect before the request reaches the app) â€?
    ...
    # an Apps OAuth redirect that happens before the request reaches agent_meow.
...
def _runner_parent_pid_from_env() -> int | None:
    ...
    """
    from agent_meow.runner.identity import RUNNER_PARENT_PID_ENV_VAR
...
def _runner_workspace_from_env() -> Path | None:
    ...
    """
    from agent_meow.runner.identity import RUNNER_WORKSPACE_ENV_VAR
...
def _runner_isolate_session_from_env() -> bool:
    ...
    """
    from agent_meow.runner.identity import RUNNER_ISOLATE_SESSION_ENV_VAR
...
def create_app(
    ...
    from agent_meow.inner.terminal import reap_orphaned_terminals
    ...
    async def _start_pm() -> None:
        ...
        if prewarm_path and mcp_manager is not None:
            try:
                from agent_meow.spec import load as _load_spec
...
async def _run_tunnel_from_env() -> None:
    ...
    :returns: None.
    """
    from agent_meow.runner.identity import get_stable_runner_id
    from agent_meow.runner.transports.ws_tunnel.serve import serve_tunnel
    ...
    # drains its session streams and closes cleanly â€?the server then sees an
    ...
    if idle_timeout_s > 0:
        ...
        def _request_idle_shutdown() -> None:
            ...
            wait below unwinds. Does NOT set stop_event â€?that would trip the
    ...
    try:
        ...
        if tunnel_task in done:
            ...
            # is done). Wait for the tunnel to finish that graceful close â€?
            # bounded â€?instead of falling straight into the finally, which
...
def main() -> None:
    ...
    try:
        ...
        if not str(exc).startswith(RUNNER_TUNNEL_REJECTION_PREFIX):
            raise
        # A fatal server rejection is an expected, actionable exit â€?log the
