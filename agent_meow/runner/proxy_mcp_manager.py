...
:class:`~?agent_meow.server.mcp_pool.ServerMcpPool` and enforces
...
interface as :class:`~?agent_meow.runner.mcp_manager.RunnerMcpManager` so
...
class ProxyMcpManager:
    ...
    Drop-in substitute for :class:`~?agent_meow.runner.mcp_manager.RunnerMcpManager`
    used in agent-meow mode.  The runner creates one instance per session on first MCP use
    and passes it to :func:`~?agent_meow.runner.tool_dispatch.execute_tool` in place
    of a direct manager.
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        ...
        :meth:`~?agent_meow.runner.mcp_manager.RunnerMcpManager.schemas_for`.
    ...
    async def call_tool(
        ...
        **ASK policy / approval flow**: when the agent-meow server returns an
        ...
        for _attempt in range(2):
            ...
            # ── ASK: agent-meow server returned InputRequiredResult ───────────────
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op — the agent-meow server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op — the agent-meow server owns and manages MCP connections."""
