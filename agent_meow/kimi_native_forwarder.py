...
injects web-UI turns into it (see :mod:`~?agent_meow.kimi_native_bridge`). The TUI's
reply renders live in the embedded terminal, but — unlike the SDK ``KimiExecutor``
— nothing flows the assistant's response back into agent-meow's conversation
transcript (the chat bubbles). This module closes that gap, the kimi analog of
:mod:`~?agent_meow.cursor_native_forwarder`.
...
``/v1/sessions/{id}/events`` (the same shape :mod:`~?agent_meow.kimi_native_hook`
