...
- carries a ``config.toml`` that is the user's config text with two agent-meow
  ...
  read-only surface, both dispatched to :mod:`~?agent_meow.kimi_native_hook`.
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    ...
    Both hooks dispatch to :mod:`~?agent_meow.kimi_native_hook` with the bridge
    ...
    # from the path, importing only the interpreter's own agent_meow. Mirrors
    # claude-native's ``python -I -m agent_meow.claude_native_hook``.
    base = f"{shlex.quote(python)} -I -m agent_meow.kimi_native_hook"
