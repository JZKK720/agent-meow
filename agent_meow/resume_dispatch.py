...
def run_resume(
    *,
    ...
    :param server: Optional remote agent-meow server URL. Required in the
...
def _dispatch_by_runtime(
    *,
    ...
    :param server: Optional remote agent-meow server URL. ``None`` when
...
def _read_wrapper_label_local(*, conv_id: str) -> str | None:
    ...
    """
    from agent_meow.chat import _omnigent_persistent_dir
    ...
    if conversation is None:
        raise click.ClickException(
            ...
            "Pass --server if the conversation lives on a remote agent-meow server.",
        )
