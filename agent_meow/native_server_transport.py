...
(:class:`~?agent_meow.opencode_http_transport.OpenCodeHttpTransport`);
:class:`NativeServerTransport` is the seam so the orchestration in
:class:`~?agent_meow.native_server_harness.NativeServerHarness` stays
protocol-agnostic.
"""
...
class NativeLaunchConfig:
    ...
    :param omnigent_session_id: agent-meow conversation id, e.g.
        ``"conv_abc123"``.
...
class NativeServerTransport(Protocol):
    ...
    :class:`~?agent_meow.native_server_harness.NativeServerHarness` calls only
    these methods.
    """
