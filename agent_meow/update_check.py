...
* **Dev clone** (``git clone … && uv sync`` / ``pip install -e .``): we
  ...
  to ``~/.agent_meow/.update_check.json`` so the (potentially slow)
    ...
    with contextlib.suppress(OSError, ValueError):
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from agent_meow.update_check import refresh_update_cache as r; r()",
            ],
...
def _find_repo_root() -> Path | None:
    ...
    ``~/.local/share/uv/tools/agent_meow/…``, the unbounded walk-up
    ...
    """
    package_dir = Path(__file__).resolve().parent  # <candidate>/agent_meow/
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    The ``agent_meow/_build_info.py`` module is generated by
    ...
    """
    try:
        from agent_meow import _build_info  # type: ignore[attr-defined]
    except ImportError:
        return None
    try:
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    ``agent-meow`` and cached its metadata — re-reading in-process returns the
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    probe = (
        ...
        "    print('')\n"
        "try:\n"
        "    from agent_meow.update_check import _read_installed_wheel_info as r\n"
        "    i = r()\n"
