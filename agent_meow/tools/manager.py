...
from agent_meow.spec.types import SharePolicy, ToolRuntime
from agent_meow.tools._srt import is_srt_available
...
from agent_meow.tools.local import load_local_python_tools
...
class ToolManager:
    ...
    def _create_web_fetch(self) -> Tool:
        ...
        """
        from agent_meow.tools.builtins.web_fetch import WebFetchTool
    ...
    def _register_task_lifecycle_tools(self) -> None:
        ...
        """
        from agent_meow.tools.builtins.async_inbox import SysCancelTaskTool
    ...
    def _register_browser_tools(self) -> None:
        ...
        not carry.
        """
        from agent_meow.tools.builtins.browser import (
            BrowserClickTool,
            BrowserNavigateTool,
    ...
    def _register_terminal_tools(self) -> None:
        ...
        :func:`agent_meow.runtime.get_terminal_registry`) so state
    ...
    def _register_local_tools(self, workdir: Path | None) -> None:
        ...
        if workdir is not None:
            for tool in load_local_python_tools(
                ...
                if not is_valid_tool_name(tool.name()):
                    ...
                    continue
                self._tools[tool.name()] = tool
        # Omnigent-style callable tools â€?sibling loader for the
        # ``omnigent-python-callable`` language entries the YAML
        # translator emits. See
        # :mod:`agent_meow.tools.local_callable` for the reasoning;
        ...
        from agent_meow.tools.local_callable import load_local_callable_tools

        for callable_tool in load_local_callable_tools(server_local_tools):
            if not is_valid_tool_name(callable_tool.name()):
                _logger.warning(
                    "Omnigent callable tool %r has invalid name â€?skipping",
                    callable_tool.name(),
                )
                continue
