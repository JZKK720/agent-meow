...
``agent_meow_client.tools`` — import them from there.

Imports are lazy so that loading ``agent_meow.tools`` does not pull
...
if TYPE_CHECKING:
    ...
    from agent_meow.tools.local import LocalPythonTool
...
# `agent_meow_client.tools` package (see sdks/python-client/). Import
# them from there — they used to be re-exported here but the
# duplicate surface was removed in the SDK carve-out.
_LAZY_EXPORTS: dict[str, tuple[str, str]] = {
    "Tool": ("agent_meow.tools.base", "Tool"),
    "ToolContext": ("agent_meow.tools.base", "ToolContext"),
    "ClientSideTool": ("agent_meow.tools.client_specified", "ClientSideTool"),
    "ClientSideToolSpec": (
        "agent_meow.tools.client_specified",
        "ClientSideToolSpec",
    ),
    "LocalPythonTool": ("agent_meow.tools.local", "LocalPythonTool"),
    "ToolManager": ("agent_meow.tools.manager", "ToolManager"),
}
...
def __getattr__(name: str) -> Any:
    ...
    ``tool`` decorator from ``agent_meow_client`` (not here),
