...
:class:`~?agent_meow.tools.manager.ToolManager` so the LLM can see and
call ``sys_advise_models``.  Execution is handled server-side: the
agent-meow server intercepts the ``tools/call`` in
:func:`~?agent_meow.server.routes.sessions._handle_advise_models_mcp`
...
- ``RuntimeCaps.routing_client`` is configured
  (``AGENT_MEOW_SMART_ROUTING=1`` + ``llm:`` config block).
"""
...
class SysAdviseModelsTool(Tool):
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "Advisory only. Available when AGENT_MEOW_SMART_ROUTING=1."
        )
