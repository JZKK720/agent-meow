...
:mod:`~?agent_meow.tools.builtins.spawn`). They ship as schema-only
:class:`~?agent_meow.tools.base.Tool` subclasses — the base-class
...
class SysAgentGetTool(Tool):
    ...
    :class:`~?agent_meow.server.schemas.AgentObject`. Returns
...
class SysAgentListTool(Tool):
    ...
    - **local configs**: agent config YAMLs authored locally with
      ``sys_os_write`` (e.g. following the ``build-agent-meow`` skill) — a
