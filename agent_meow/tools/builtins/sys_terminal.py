...
:class:`~?agent_meow.terminals.TerminalRegistry`. The registry owns
...
on AP-side :class:`~?agent_meow.tools.manager.ToolManager` and flow
...
class SysTerminalLaunchTool(Tool):
    ...
    def _resolve_cwd(
        ...
        3. ``spec.os_env.cwd`` if it's set to a meaningful path
           (not ``None``, ``""``, ``"."``, or ``"./"``).
        4. ``ctx.workspace`` — the per-task workspace agent-meow creates
