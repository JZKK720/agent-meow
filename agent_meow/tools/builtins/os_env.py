...
inner :mod:`~?agent_meow.inner.session` when the agent's
``os_env`` is set. They wrap a shared
:class:`~?agent_meow.inner.os_env.OSEnvironment` instance so the
...
:class:`~?agent_meow.tools.manager.ToolManager` (so policies,
...
(:func:`~?agent_meow.inner.os_env.create_os_environment`,
:class:`OSEnvSpec`) which won't ship in the AP-only deployment
once :mod:`~?agent_meow.inner` retires. Keeping the dependency
...
from agent_meow.inner.os_env import _DEFAULT_READ_LIMIT, OSEnvironment
...
# ── JSON Schemas ──────────────────────────────────────────────
# Mirror the inner :mod:`~?agent_meow.inner.session` schemas
...
class _OSEnvBackedTool(Tool):
    ...
    :class:`~?agent_meow.tools.manager.ToolManager` constructs
...
def build_os_env_tools(os_env: OSEnvironment) -> list[Tool]:
    ...
    Convenience for the
    :class:`~?agent_meow.tools.manager.ToolManager`'s registration
