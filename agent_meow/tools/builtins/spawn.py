...
# paths strip this marker and expose ``agent_meow.closed=true`` instead.
...
class SysSessionSendTool(Tool):
    ...
    :class:`~?agent_meow.runtime.workflow._AsyncToolHandle`:
...
class SysSessionShareTool(Tool):
    ...
    (:class:`~?agent_meow.spec.types.SharePolicy`), which is its sole gate:
