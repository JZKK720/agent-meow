...
Extracted from :mod:`agent_meow.cli` to keep that module under the repo's
per-file size budget; the ``config`` / ``setup`` Click commands stay in
``agent_meow.cli`` and reach these helpers via :func:`register`.

The three config-load helpers these functions need (``_load_global_config``,
``_save_global_config``, ``_load_effective_config``) live in ``agent_meow.cli`` and
are used ~20x each there, so they stay put; this module reaches them through
call-time proxies below, so importing this module never imports ``agent_meow.cli``
(no cycle) and a test's ``monkeypatch`` of ``agent_meow.cli.<helper>`` is still
honoured.
"""
...
if TYPE_CHECKING:
    ...
    from agent_meow.onboarding.provider_config import ProviderEntry
...
# Brand shown for an auto-configured CLI login in the credentials callout 鈥?
...
# a copy of ``undici`` that calls ``worker_threads.markAsUncloneable`` 鈥?a
...
def _node_version(node_path: str) -> str | None:
    ...
    version is non-fatal 鈥?the caller still reports the underlying problem.
...
def _node_dependency_problem() -> str | None:
    ...
    :returns: A human-readable description suitable for a warning bullet,
        ...
        probe also yields ``None`` 鈥?setup should not block on it.
    """
    node = shutil.which("node")
    if node is None:
        return f"node not found 鈥?Claude, Codex, and Pi need {_NODE_MIN_VERSION_HINT}."
    # Probe the exact API the bundled undici calls. Exit 0 鈬?capability
    # present; exit 1 鈬?too old; we treat any other failure as inconclusive.
    probe = (
        "process.exit("
    ...
    return f"Node.js is too old{detected} 鈥?Claude, Codex, and Pi need {_NODE_MIN_VERSION_HINT}."
...
def _isolated_databricks_cfg() -> collections.abc.Generator[None, None, None]:
    ...
    the direct configparser writes in ``agent_meow.onboarding.setup``
    ...
    from agent_meow.onboarding.internal_beta import DEFAULT_PROFILES
    from agent_meow.onboarding.setup import CONFLICTING_ENV_VARS
    ...
    def _on_signal(signum: int, _frame: types.FrameType | None) -> None:
        ...
        # (e.g. Click's Ctrl-C 鈫?Abort) is preserved rather than falling
...
def _run_configure_databricks() -> None:
    ...
    # gateway workspace(s) only 鈥?not the MCP-only profiles, which are
...
def _warn_missing_harness_dependencies() -> None:
    ...
    hard failure would block a valid flow 鈥?but ``omnigent claude`` /
    ``codex`` do need both, hence the prominent notice.

    :returns: None. Side effect: writes a yellow warning block to stderr
        via :mod:`agent_meow.inner.ui` when one or more dependencies are
        missing.
    """
    problems: list[str] = []
    ...
    if shutil.which("tmux") is None:
        problems.append(
            "tmux not found 鈥?native Claude/Codex need tmux (macOS: `brew install tmux`)."
        )
    if not problems:
        return
    ui.warn("Some harnesses need external tools:")
    for problem in problems:
        ui.err_console.print(f"  鈥?{problem}", style="omni.warning", markup=False)
...
def _print_credentials_by_harness() -> None:
    ...
    family, with the per-family default marked 鈥?via
    :func:`render_provider_listing_by_harness`.

    :returns: None. Side effect: writes the listing to the onboarding
        console.
    """
    from agent_meow.onboarding.configure_models import render_provider_listing_by_harness
...
def _existing_key_name_for_ref(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    update such a key in place rather than writing a second, identical entry 鈥?
    ...
    """
    from agent_meow.onboarding.provider_config import KEY_KIND, load_providers
...
def _unique_provider_name(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    has one), this derives a fresh name 鈥?``anthropic`` 鈫?``anthropic-2`` 鈫?
    ``anthropic-3`` 鈥?so both coexist.
...
def _resolve_key_provider_name(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    """Pick the entry name for an API key being added 鈥?update vs keep-both.
...
def _credential_source_hint(entry: ProviderEntry, family: str) -> str | None:
    ...
    if raw is None and family == PI_SURFACE:
        # The pi surface carries no family block of its own 鈥?pi consumes
...
def _family_key_count(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    key serves a harness 鈥?a lone key needs no source qualifier.
...
def _configure_harness_add(family: str | None = None) -> str | None:
    ...
    :param family: When set (``"anthropic"`` / ``"openai"`` / ``"pi"``),
        the add menu is scoped to credentials that can drive that harness 鈥?
    ...
    from agent_meow.onboarding.ambient import detect_providers
    ...
    from agent_meow.onboarding.interactive import console, prompt_text, select
    from agent_meow.onboarding.provider_config import (
        ANTHROPIC_FAMILY,
        BEDROCK_KIND,
        CHAT_WIRE_API,
    ...
    # A flat, credential-aware menu: the user picks "OpenAI 鈥?API key" or
    # "Claude 鈥?subscription" directly (rather than a bare kind then
    ...
    # gets its own add option. This is the only way back after Remove 鈥?
    ...
    # Base options first, then one row per detected config provider 鈥?the
    # selection index maps back into cli_config_dets below.
    base_option_count = len(options)
    options = options + [
        AddOption(
            label=f"\N{GEAR}\N{VARIATION SELECTOR-16} {d.display_name or d.name} 鈥?"
    ...
    if choice < 0:  # Esc 鈥?abort the add
        return None
    ...
    if kind == CLI_CONFIG_KIND:
        ...
        # order, after the base options 鈥?map the selection back to its
        ...
        # after all" 鈥?drop any standing dismissal so it behaves like an
        ...
        if chosen.provider is not None:
            ...
            # Preset: the preferred name is the provider id 鈥?but the final name
            ...
            # "Other provider 鈥?API key": pick from the remaining catalog,
            ...
            if _other_choice < 0:  # Esc 鈥?abort the add
                return None
        ...
        if (
            ...
            and click.confirm(
                f"Detected {detected[provider].source} in the environment 鈥?use it?",
                default=True,
            )
        ):
        ...
        # Default model 鈥?free-form text entry. The bundled catalog lags new
        ...
        # id. Blank 鈫?the default (or no pin when unknown). Always persisting
        ...
        # A third-party OpenAI-compatible vendor (OpenRouter, Groq, 鈥? is
        ...
        # traffic to api.openai.com 鈥?the reason an OpenRouter key failed.
        ...
        from agent_meow.onboarding.harness_install import harness_install_spec, harness_login
        ...
        # A harness has at most ONE subscription 鈥?the CLI's own login. If one
        # is already configured for this CLI (under any name, including an
        # ambient login adopted as e.g. ``claude``), adding another just
        # duplicates it 鈥?the ``claude`` + ``claude-subscription`` bug. Offer to
        ...
        if existing_subs:
            ...
            if replace != 0:  # "Keep the current one" or Esc 鈥?abort the add
                return None
        ...
        # once the CLI is actually authenticated 鈥?otherwise we'd persist a
        ...
        console.print(f"  [dim]Signing in to {disp} (its login will open)鈥/dim]")
        if not harness_login(login_family):
            return f"鉁?{disp} login not completed 鈥?subscription not added"
        # Login succeeded 鈥?drop the existing subscription(s) for this CLI so the
        ...
        # Subscription name is derived from the CLI login 鈥?no prompt.
        ...
        secret_store.store_secret(name, pasted)
        # Which harness surfaces 鈥?one clear pick instead of two y/n prompts.
        # (These are *harness* surfaces: Codex/OpenAI 鈫?codex + openai-agents;
        # Claude/Anthropic 鈫?claude-sdk + native-claude.)
        ...
        if surface_choice < 0:  # Esc 鈥?abort the add
            return None
        families = (
        ...
        # exact "OpenRouter doesn't work but LiteLLM does" symptom), so ask 鈥?
        ...
        if OPENAI_FAMILY in families:
            ...
            if wire_choice < 0:  # Esc 鈥?abort the add
                return None
        ...
        if os.environ.get("AWS_BEARER_TOKEN_BEDROCK") and click.confirm(
            "Detected AWS_BEARER_TOKEN_BEDROCK in the environment 鈥?use it?", default=True
        ):
        ...
        if not databricks_sdk_installed():
            ...
            return (
                "鉁?Databricks routing needs the databricks extra 鈥?"
        ...
        # `clear_on_exit`) 鈥?entering the Databricks option should NOT blank the
        ...
        # Ask only for the workspace URL 鈥?never a profile name. The flow
        ...
        from agent_meow.onboarding.databricks_config import normalize_workspace_url
        from agent_meow.onboarding.interactive import clear_screen
        from agent_meow.onboarding.setup import login_databricks_workspace
        from agent_meow.onboarding.ucode_setup import (
        ...
        if not workspace_url:  # blank 鈥?abort the add
            return None
        ...
        if normalized_workspace_url != workspace_url.rstrip("/"):
            console.print(
                f"  [dim]Using {normalized_workspace_url} 鈥?ignored the extra "
        ...
        #    name) and 2. run `ucode configure` against it for model serving 鈥?
        ...
        # Fail loud if ucode didn't actually record state for the workspace 鈥?
        ...
        # "鉁?Added databricks" status) renders on a clean screen.
        clear_screen()
        # Databricks name is fixed 鈥?no prompt. The provider keys on the
        # profile; runtime resolves profile 鈫?workspace URL 鈫?ucode state.
        name = "databricks"
    ...
    # Persist the entry (deep-merge 鈥?doesn't disturb sibling entries).
    _save_global_config(
    ...
    # so a first provider "just works". An existing default is left alone 鈥?
    ...
    # become the default for THAT surface 鈥?defaulting the other harnesses too
    ...
    if became_default:
        labels = " 路 ".join(family_label(f) for f in became_default)
        return f"鉁?Added {name} 鈥?default for {labels}"
    return f"鉁?Added {name}"
...
def _adopt_detected_providers() -> list[str]:
    ...
    editable entry 鈥?so the tree shows one uniform provider list with no
...
def _promote_global_auth_to_provider() -> str | None:
    ...
    ``auth: {type: databricks}`` block 鈥?which ``configure harnesses`` does not
    read 鈥?so the readout showed no Databricks provider (and an ambient CLI
    ...
    Becomes the default only for families with no existing **provider** default 鈥?
    ...
    """
    from agent_meow.onboarding.configure_models import build_databricks_provider_entry
    from agent_meow.onboarding.provider_config import (
        load_providers,
        provider_entry_settings,
        provider_families,
    ...
    if name in load_providers(config):
        return None  # already a first-class provider 鈥?nothing to backfill
    ...
    for fam in sorted(provider_families(parsed)):
        ...
        # already drives the surface 鈥?explicitly or via pi's fallback 鈥?
...
def _compact_credential_label(det: DetectedProvider) -> str:
    ...
    harness only ever has one), this names the *brand* behind a login 鈥?
    ``"Claude Subscription"`` / ``"ChatGPT Subscription"`` 鈥?so a single
    ...
    """
    from agent_meow.onboarding.ambient import SUBSCRIPTION_KIND
...
def _announce_auto_configured_credentials(adopted: list[str]) -> None:
    """Print the "found existing credentials 鈫?auto-configured" callout.
    ...
    Claude Subscription, ChatGPT Subscription``) 鈥?so a user who never ran an
    ...
    :param adopted: Provider names just persisted by
        ...
        A name with no matching live detection is skipped (defensive 鈥?the
...
def _adopt_ambient_credentials(progress: RunnerStartupProgress | None = None) -> list[str]:
    ...
    The callout is scoped to *machine* credentials 鈥?the ambient detections 鈥?
    ...
    # Clear the search spinner (if any) before printing 鈥?the callout writes to
...
class _HarnessMenuRow:
    ...
    :param label: Display text, e.g. ``"馃攽 anthropic   鉁?default"``.
    :param action: The action on Enter 鈥?``"set_default"`` / ``"add"`` /
        ``"remove"`` / ``"back"``.
...
def _credential_label(name: str, entry: ProviderEntry) -> str:
    ...
    endpoint as its display name 鈥?so menus and summaries avoid raw provider
...
def _harness_credential_rows(config: dict[str, Any], family: str) -> list[_HarnessMenuRow]:  # type: ignore[explicit-any]
    ...
    current default is marked with a green 鉁? ``+ Add a credential`` runs the
    add flow; ``鈫?Back`` returns to the harness picker (as do Esc / ``q``).
    ...
    # so the 鉁?always marks the credential the harness would actually use.
    ...
    for name, entry in serving:
        ...
        # The current default renders bold-green with a 鉁?so it stands out in
...
def _prompt_install_harness(family: str) -> bool:
    ...
    three choices: install it now (``npm install -g 鈥`), go back, or print the
    ...
    :returns: ``True`` only when the CLI is installed afterward (user chose
        ...
        themselves, the install fails, or they Esc 鈥?the caller returns to the
        harness picker.
    """
    from agent_meow.onboarding.configure_models import family_label
    ...
    if choice == 0:
        console.print(f"  [dim]Installing {label} 鈥?running `{cmd}`鈥/dim]")
        if install_harness_cli(family):
            console.print(f"  [green]鉁?{label} installed[/green]")
            return True
        console.print(
...
def _manage_harness_providers(family: str) -> None:
    ...
    """
    from agent_meow.onboarding.configure_models import family_label
    from agent_meow.onboarding.harness_install import harness_cli_installed
    ...
    # harness picker 鈥?there's nothing to configure for a harness you can't run.
    ...
    # menu shows only the latest result 鈥?not an accumulating stack of "鉁?鈥?.
    status: str | None = None
    while True:
        rows = _harness_credential_rows(_load_global_config(), family)
        idx = select(
            f"{family_label(family)} 鈥?select or add a credential",
            [r.label for r in rows],
            clear_on_exit=True,
            status=status,
        )
        if idx < 0:  # Esc / q 鈥?back to the harness picker
            return
        row = rows[idx]
        if row.action == "back":
            return
...
def _prompt_install_cursor() -> str | None:
    ...
    from agent_meow.onboarding.cursor_auth import CURSOR_EXTRA, install_cursor_sdk
    from agent_meow.onboarding.extra_install import extra_install_display
    from agent_meow.onboarding.interactive import console, select
    ...
    choice = select(
        ...
        descriptions=[
            ...
            "Skip the install 鈥?store the key now; the SDK can be added later.",
    ...
    if choice == 0:
        console.print(f"  [dim]Installing the cursor extra 鈥?running `{cmd_markup}`鈥/dim]")
        if install_cursor_sdk():
            console.print("  [green]鉁?cursor-sdk installed[/green]")
            return "鉁?cursor-sdk installed"
        console.print(f"  [red]Install failed.[/red] Run it manually: [bold]{cmd_markup}[/bold]")
        return "鉁?Install failed 鈥?set the key anyway, or install by hand"
    if choice < 0:
...
def _manage_cursor_sdk_harness() -> None:
    ...
    Cursor's own backend with a ``CURSOR_API_KEY`` 鈥?the SDK requires one (a
    ...
    (which gate on the CLI), declining still drops into the key menu 鈥?the
    ...
    while True:
        ...
        header = "Cursor 鈥?API key configured" if key_set else "Cursor 鈥?no API key yet"
        ...
        if action == "set_key":
            ...
            status = "鉁?Removed Cursor API key"
...
def _set_cursor_api_key() -> str | None:
    ...
    # exported with a trailing newline (a common ``export $(鈥?`` mishap)
    # validates and resolves cleanly 鈥?matching the pasted-key branch's
    ...
    if detected and click.confirm(
        "Detected CURSOR_API_KEY in the environment 鈥?use it?", default=True
    ):
        ...
        return "鉁?Cursor API key set (from $CURSOR_API_KEY)"
    ...
    return "鉁?Cursor API key stored"
...
def _manage_cursor_native_harness() -> None:
    ...
    while True:
        ...
        header = "Cursor CLI 鈥?logged in" if logged_in else "Cursor CLI 鈥?not logged in yet"
        ...
        if rows[idx].action == "login":
            status = (
                "鉁?Cursor CLI logged in" if harness_login(CURSOR_KEY) else "Login not detected"
            )
        elif rows[idx].action == "logout":
            status = "鉁?Cursor CLI logged out" if harness_logout(CURSOR_KEY) else "Logout failed"


def _manage_cursor_harness() -> None:
    """Configure Cursor CLI and SDK from one consolidated setup entry."""
    from agent_meow.onboarding.cursor_auth import cursor_api_key_configured
    from agent_meow.onboarding.harness_install import (
        CURSOR_KEY,
        harness_cli_installed,
        harness_cli_logged_in,
    ...
    while True:
        ...
        rows = [
            _HarnessMenuRow(f"Cursor CLI 鈥?{cli_status}", action="cli"),
            _HarnessMenuRow(f"Cursor SDK 鈥?{sdk_status}", action="sdk"),
            _HarnessMenuRow("鈫?Back", action="back"),
        ]
...
def _prompt_install_antigravity() -> str | None:
    ...
    Mirrors :func:`_prompt_install_harness` 鈥?a three-choice ``select`` (install now /
    set key anyway / print command) 鈥?but does NOT gate key management on the SDK:
    ...
    from agent_meow.onboarding.antigravity_auth import ANTIGRAVITY_EXTRA, install_antigravity_sdk
    from agent_meow.onboarding.extra_install import extra_install_display
    from agent_meow.onboarding.interactive import console, select
    ...
    if choice == 0:
        console.print(f"  [dim]Installing the antigravity extra 鈥?running `{cmd_markup}`鈥/dim]")
        if install_antigravity_sdk():
            console.print("  [green]鉁?google-antigravity installed[/green]")
            return "鉁?google-antigravity installed"
        console.print(f"  [red]Install failed.[/red] Run it manually: [bold]{cmd_markup}[/bold]")
        return "鉁?Install failed 鈥?set the key anyway, or install by hand"
    if choice < 0:
        return _SOFT_INSTALL_ABORT
...
def _manage_antigravity_harness() -> None:
    ...
    API key 鈥?stored in the secret store, referenced from the ``antigravity:``
    config block 鈥?mirroring how the other harnesses persist api keys.
    ...
    while True:
        ...
        header = (
            "Antigravity 鈥?Gemini API key configured"
            if key_set
            else "Antigravity 鈥?no Gemini API key yet"
        )
        idx = select(header, [r.label for r in rows], clear_on_exit=True, status=status)
        if idx < 0:  # Esc / q
            return
        ...
        if action == "set_key":
            ...
            status = "鉁?Removed Gemini API key"
...
def _set_antigravity_api_key() -> str | None:
    ...
    if detected_var is not None and click.confirm(
        f"Detected {detected_var} in the environment 鈥?use it?", default=True
    ):
        ...
        return f"鉁?Gemini API key set (from ${detected_var})"
...
def _qwen_auth_configured() -> bool:
    ...
    docs/QWEN_FOLLOWUPS.md). Returns ``False`` for a fresh install with no auth 鈥?
...
def _print_qwen_auth_help() -> None:
    ...
    console.print(
        ...
        "    鈥?Interactive: run [bold]qwen[/bold] and use [bold]/auth[/bold] "
        "(API key or Alibaba Cloud Coding Plan)\n"
        "    鈥?Headless / ACP: set [bold]OPENAI_API_KEY[/bold] + "
        "[bold]OPENAI_BASE_URL[/bold] + [bold]OPENAI_MODEL[/bold]\n"
        "    鈥?Coding Plan: [bold]BAILIAN_CODING_PLAN_API_KEY[/bold] + the "
        "Coding Plan base URL\n"
        "    鈥?OpenRouter: [bold]OPENROUTER_API_KEY[/bold] + "
...
def _manage_qwen_harness() -> None:
    ...
    to launch ``qwen`` for ``/auth`` 鈥?it does **not** pretend to run a ``qwen
    login``
    ...
    Like the CLI-backed harnesses, a missing CLI gates the drill-in 鈥?there's
    ...
    if not harness_cli_installed(QWEN_KEY):
        ...
        if choice == 0:
            console.print(f"  [dim]Installing Qwen Code 鈥?running `{cmd}`鈥/dim]")
            if install_harness_cli(QWEN_KEY):
                console.print("  [green]鉁?Qwen Code installed[/green]")
            else:
                console.print(
    ...
    while True:
        ...
        header = (
            "Qwen Code 鈥?authentication detected"
            if configured
            else "Qwen Code 鈥?not authenticated yet"
        )
        rows: list[_HarnessMenuRow] = [
...
def _print_goose_auth_help() -> None:
    ...
    console.print(
        ...
        "    鈥?Interactive: run [bold]goose configure[/bold] to pick a provider "
        "and store its key (keyring or ~/.config/goose/config.yaml)\n"
        "    鈥?Env override: set [bold]GOOSE_PROVIDER[/bold] + [bold]GOOSE_MODEL[/bold] "
...
def _launch_goose_configure() -> str | None:
    ...
    """
    from agent_meow.onboarding.goose_auth import goose_config_summary
    from agent_meow.onboarding.harness_install import (
        GOOSE_KEY,
        harness_cli_installed,
        harness_install_spec,
    ...
    if not harness_cli_installed(GOOSE_KEY):
        return "鉁?goose CLI not found"
    spec = harness_install_spec(GOOSE_KEY)
    assert spec is not None
    console.print(
        "  [dim]Launching [bold]goose configure[/bold] 鈥?pick a provider and "
    ...
    if summary.provider:
        ...
        return f"鉁?provider configured: {summary.provider}{model}"
...
def _manage_goose_harness() -> None:
    ...
    Goose owns its own auth (keyring / ``~/.config/goose/config.yaml``) 鈥?Omnigent
    stores no Goose credential 鈥?so, like the Qwen drill-in, this reports
    ...
    ``goose`` (ACP) harness 鈥?both launch the same ``goose`` binary and read the
    same config.
    ...
    # auto-install 鈥?show the hint and return.
    if not harness_cli_installed(GOOSE_KEY):
        spec = harness_install_spec(GOOSE_KEY)
    ...
    while True:
        summary = goose_config_summary()
        if summary.provider:
            model = f" 路 {summary.model}" if summary.model else ""
            header = f"Goose 鈥?provider configured: {summary.provider}{model}"
        else:
            header = "Goose 鈥?no provider configured yet"
        rows: list[_HarnessMenuRow] = [
...
def _print_acp_examples() -> None:
    ...
    console.print(
        "\n  [bold]Custom ACP agents[/bold] 鈥?connect any agent that speaks the "
        "Agent Client Protocol ([underline]agentclientprotocol.com[/underline]).\n"
        "  Omnigent stores no credential 鈥?log into each agent via its own CLI first.\n\n"
        "  Example commands to paste:\n"
        "    鈥?Gemini CLI     [bold]gemini --experimental-acp[/bold]\n"
        "    鈥?Qwen Code      [bold]qwen --acp[/bold]\n"
        "    鈥?Goose          [bold]goose acp[/bold]\n"
        "    鈥?Claude Code    [bold]npx -y @zed-industries/claude-code-acp[/bold]\n"
    )
...
def _add_acp_agent() -> None:
    ...
    """
    from agent_meow.onboarding.acp_auth import (
        AcpAgentEntry,
        acp_agents,
        acp_agents_settings,
        slugify,
    )
    from agent_meow.onboarding.interactive import console, prompt_text

    _print_acp_examples()
    name = prompt_text("Agent name (e.g. Gemini CLI)").strip()
    if not name:
        console.print("  [yellow]No name entered 鈥?nothing added.[/yellow]")
        return
    command = prompt_text("Command to launch (e.g. gemini --experimental-acp)").strip()
    if not command:
        console.print("  [yellow]No command entered 鈥?nothing added.[/yellow]")
        return
    model = (prompt_text("Model (optional 鈥?Enter to skip)", default="") or "").strip() or None
...
def _manage_acp_agent(slug: str) -> None:
    ...
    A single-shot menu (Remove / Back) 鈥?Omnigent stores no credential, so there
    is nothing else to manage per agent yet.

    :param slug: The agent's slug (see :func:`agent_meow.onboarding.acp_auth.slugify`).
    """
    from agent_meow.onboarding.acp_auth import acp_agents, acp_agents_settings
    from agent_meow.onboarding.interactive import console, select
    ...
    if agent is None:
        return
    suffix = f"  路  {agent.model}" if agent.model else ""
    header = f"{agent.name} 鈥?{agent.command}{suffix}"
    rows: list[_HarnessMenuRow] = [
    ...
    console.print(f"  鉁?Removed {agent.name}")
...
def _manage_hermes_harness() -> None:
    ...
    picker) and is installed via a curl script from Nous Research 鈥?Omnigent
    ...
    if not harness_cli_installed(HERMES_KEY):
        ...
        if choice == 0:
            console.print(f"  [dim]Installing Hermes 鈥?running `{hint}`鈥/dim]")
            if install_harness_cli(HERMES_KEY):
                console.print("  [green]鉁?Hermes installed[/green]")
            else:
                console.print(
                    f"  [red]Install failed.[/red] Run it manually, then re-open: "
                    f"[bold]{hint}[/bold]"
    ...
    while True:
        ...
        if action == "model":
            ...
            try:
                ...
                status = "鉁?hermes model completed"
            except FileNotFoundError:
                status = "鉁?hermes binary not found"
...
def _manage_kiro_harness() -> None:
    ...
    Identity Center) and is installed via Kiro's curl installer 鈥?Omnigent stores
    ...
    while True:
        ...
        if action == "login":
            ...
            try:
                ...
                status = "鉁?kiro-cli login completed"
            except FileNotFoundError:
                status = "鉁?kiro-cli binary not found"
...
def _print_kimi_auth_help() -> None:
    ...
    OpenAI-compatible endpoint, a Databricks gateway, 鈥? in
    ``~/.kimi/config.toml``. Omnigent has no per-spawn provider override for
    upstream kimi, so all of this lives in the kimi CLI's own config 鈥?
    ...
    console.print(
        ...
        "~/.kimi/config.toml):\n"
        "    鈥?Default provider: run [bold]kimi login[/bold] "
        "(Moonshot OAuth, or paste a Moonshot API key)\n"
        "    鈥?Other providers: run [bold]kimi provider add[/bold] "
        "(OpenAI-compatible endpoint, gateway, 鈥?, then pin that model id in "
        "the agent spec\n"
        "    鈥?Omnigent stores no kimi credential and cannot thread one per "
        "spawn 鈥?configure it once in the kimi CLI\n"
    )
...
def _manage_kimi_harness() -> None:
    ...
    reports ``False`` for it 鈥?meaning ``harness_login`` runs ``kimi login``
    ...
    Like the other CLI-backed harnesses, a missing CLI gates the drill-in 鈥?
    ...
    # npm), so there's no in-process auto-install 鈥?name the command and let
    ...
    while True:
        ...
        idx = select(
            "Kimi Code 鈥?authentication is managed by the kimi CLI",
            [r.label for r in rows],
            clear_on_exit=True,
            status=status,
        )
        ...
        if action == "login":
            ...
            # don't assert success 鈥?just confirm the flow finished.
            console.print("  [dim]Signing in to Kimi (its login will open)鈥/dim]")
            harness_login(KIMI_KEY)
            status = "kimi login flow finished 鈥?kimi stores its own credentials"
        elif action == "logout":
            console.print("  [dim]Signing out of Kimi鈥/dim]")
...
def _prompt_install_copilot() -> str | None:
    ...
    from agent_meow.onboarding.copilot_auth import COPILOT_EXTRA, install_copilot_sdk
    from agent_meow.onboarding.extra_install import extra_install_display
    from agent_meow.onboarding.interactive import console, select
    ...
    choice = select(
        ...
        descriptions=[
            f"Runs `{cmd_markup}`, then continues.",
            "Skip the install 鈥?store the token now; the SDK can be added later.",
            "Print the command so you can install it yourself, then continue.",
        ],
        default=0,
        clear_on_exit=True,
    ...
    if choice == 0:
        console.print(f"  [dim]Installing the copilot extra 鈥?running `{cmd_markup}`鈥/dim]")
        if install_copilot_sdk():
            console.print("  [green]鉁?github-copilot-sdk installed[/green]")
            return "鉁?github-copilot-sdk installed"
        console.print(f"  [red]Install failed.[/red] Run it manually: [bold]{cmd_markup}[/bold]")
        return "鉁?Install failed 鈥?set the token anyway, or install by hand"
    if choice < 0:
        return _SOFT_INSTALL_ABORT
    if choice == 2:  # run it yourself
...
def _manage_copilot_harness() -> None:
    ...
    GitHub's Copilot backend with a GitHub token 鈥?the SDK requires one and it
    ...
    menu 鈥?the ``copilot:`` token is independently storable. Mirrors cursor /
    antigravity.
    ...
    while True:
        ...
        header = (
            "Copilot 鈥?GitHub token configured" if token_set else "Copilot 鈥?no GitHub token yet"
        )
        idx = select(header, [r.label for r in rows], clear_on_exit=True, status=status)
        if idx < 0:  # Esc / q
            return
        ...
        if action == "set_key":
            ...
            status = "鉁?Removed Copilot GitHub token"
...
def _set_copilot_github_token() -> str | None:
    ...
    The token shape is checked softly (a classic ``ghp_`` PAT 鈥?which Copilot
    rejects 鈥?or a wrong paste is flagged but can be forced). The token is never
    echoed.
    ...
    if detected_var is not None and click.confirm(
        ...
        return f"鉁?Copilot GitHub token set (from ${detected_var})"
    ...
    return "鉁?Copilot GitHub token stored"
...
def _manage_credential(provider: str, family: str) -> str | None:
    ...
    when it is not already this harness's default), ``Remove``, and ``鈫?Back``.
    Make-default / remove return to level 2 with a confirmation; ``鈫?Back`` /
    ...
    # surface's *effective* default (matching the 鉁?on the level-2 row) 鈥?
    ...
    if idx < 0:  # Esc / q 鈥?back to the credential list, no change
        return None
    row = rows[idx]
    if row.action == "back":
        return None
    ...
    # our config 鈥?so removing it means signing out of that CLI (otherwise the
    ...
    # harness configs outside ~/.omnigent/config.yaml 鈥?so removing it
...
def _remove_subscription(provider: str, family: str) -> str | None:
    ...
    our entry would leave that login in place 鈥?so it would still drive the
    ...
    choice = select(
        ...
        [f"Yes 鈥?sign out of {disp} and remove", "No 鈥?keep it"],
        descriptions=[
    ...
    # Drop our entry regardless 鈥?the user asked to remove it. If logout failed
    # we say so, since the standalone login may persist (and be re-detected).
    _remove_credential(provider)
    if signed_out:
        return f"鉁?Signed out of {disp} and removed"
    return (
        f"鉁?Removed {disp} subscription 鈥?note: `{logout_cmd}` did not complete, "
...
def _remove_databricks_provider(provider: str) -> str:
    ...
    ``~/.omnigent/config.yaml`` 鈥?most damagingly, for Codex < 0.134.0 it
    ...
    the provider therefore undoes that wiring as part of the removal 鈥?no
    ...
    serves 鈥?a databricks entry routes both Claude and Codex.
    ...
    try:
        ...
        # The entry removal below still proceeds 鈥?the user asked for it 鈥?
        # but say exactly what was left behind instead of failing silently.
        cleanup_note = f" 鈥?ucode cleanup incomplete: {exc}"
    else:
        cleaned: list[str] = []
        ...
        if cleaned:
            cleanup_note = f" 鈥?{', '.join(cleaned)}"
    removed_msg = _remove_credential(provider) or f"鉁?Removed {provider}"
...
def _set_harness_default(provider: str, family: str) -> str | None:
    ...
    :param family: The harness surface to scope the default to,
        ``"anthropic"``, ``"openai"``, or ``"pi"`` 鈥?leaving the other
    ...
    return f"鉁?{label} is now the {family_label(family)} default"
...
def _clear_detection_dismissal(name: str) -> None:
    ...
    dismissed) ambient credential 鈥?e.g. picking the detected codex
    config.toml provider from the add menu 鈥?so the detection behaves like
...
def _remove_credential(provider: str) -> str | None:
    ...
    The stored secret (if any) is left in place 鈥?removing a credential does
    ...
    # "Remove doesn't remove" bug). Subscriptions are exempt 鈥?their removal
    ...
    if backing is not None:
        return f"鉁?Removed {label} 鈥?it stays on your machine but won't be auto-configured again"
    return f"鉁?Removed {label}"
...
def _launch_opencode_auth_login() -> str | None:
    ...
    if not harness_cli_installed(OPENCODE_KEY):
        return "鉁?opencode CLI not found"
    spec = harness_install_spec(OPENCODE_KEY)
    assert spec is not None
    console.print(
        "  [dim]Launching [bold]opencode auth login[/bold] 鈥?pick a provider and "
    ...
    if summary.has_provider:
        return f"鉁?providers: {summary.describe()}"
...
def _run_opencode_auth_list() -> None:
    ...
    from agent_meow.onboarding.harness_install import OPENCODE_KEY, harness_install_spec
...
def _set_opencode_default_model(current: str | None) -> str | None:
    ...
    given 鈥?written into the per-session ``opencode.json`` at spawn so the TUI
    ...
    """
    from agent_meow.onboarding.interactive import console, select
    from agent_meow.onboarding.opencode_auth import reachable_provider_ids

    models = _list_opencode_models()
    if not models:
        return "鉁?no models 鈥?sign in to a provider first (opencode auth login)"
    # `opencode models` can list hundreds of `provider/model` ids across every
    # provider on models.dev 鈥?too long for the picker (it overflows the
    ...
    if idx == clear_index:
        ...
        console.print("  [green]鉁?default model cleared[/green]")
        return "鉁?default model cleared"
    chosen = models[idx]
    _save_global_config({"opencode_model": chosen})
    console.print(f"  [green]鉁?default model set to[/green] [bold]{chosen}[/bold]")
    return f"鉁?default model: {chosen}"
...
def _print_opencode_auth_help() -> None:
    ...
    console.print(
        ...
        "    鈥?[bold]opencode auth login[/bold] 鈥?sign in to a provider (OpenAI, Anthropic, 鈥?;\n"
        "      stored in ~/.local/share/opencode/auth.json.\n"
        "    鈥?Provider env vars (OPENAI_API_KEY / ANTHROPIC_API_KEY / 鈥? are auto-detected.\n"
        "    鈥?Databricks gateway: set an agent ``profile`` (configured under Claude / Codex);\n"
...
def _manage_opencode_harness() -> None:
    ...
    OpenCode owns its own provider auth 鈥?``opencode auth login`` (stored in
    ``~/.local/share/opencode/auth.json``) or ambient provider env vars 鈥?so,
    ...
    synthesized into opencode's per-session config instead 鈥?set under
    Claude / Codex.)
    ...
    if not harness_cli_installed(OPENCODE_KEY):
        ...
        if choice == 0:
            console.print(f"  [dim]Installing OpenCode 鈥?running `{cmd}`鈥/dim]")
            if install_harness_cli(OPENCODE_KEY):
                console.print("  [green]鉁?OpenCode installed[/green]")
            else:
                console.print(
                    f"  [red]Install failed.[/red] Run it manually, then re-open: "
                    f"[bold]{cmd}[/bold]"
    ...
    # OpenCode owns its provider auth (``opencode auth login`` 鈫?auth.json) or
    # ambient env keys; Omnigent stores nothing. Report what's reachable and
    # offer to run its native login 鈥?like the Goose/Qwen drill-ins.
    status: str | None = None
    while True:
        from agent_meow.onboarding.opencode_auth import opencode_auth_summary
        ...
        header = (
            f"OpenCode 鈥?providers: {summary.describe()}"
            if summary.has_provider
            else "OpenCode 鈥?no provider configured yet"
        )
        model_label = (
...
def _run_configure_harnesses_interactive() -> None:
    ...
    provider and adopts any ambient-detected credential 鈥?announcing the
    newly auto-configured machine credentials in a callout 鈥?then loops on
    the level-1 harness overview. Every harness is shown on a single compact
    row 鈥?the harness name on the left, then an aligned ``鉁揱`/``鉁梎` status
    column (the configured credential, or "Not installed" / "Not configured")
    鈥?in 0.3 priority order: Claude, Codex, Cursor, OpenCode,
    ...
    # Surface missing external tooling (Node 鈮?2.10 / tmux) the harnesses need,
    ...
    # it (skipped by 鈫?鈫?. Drilling in (level 2) keeps add/manage off this
    ...
    # Sentinel marking the Antigravity row 鈥?it is not a provider family (Gemini
    ...
    # Sentinel marking the Qwen Code row 鈥?like Antigravity/Cursor it is not a
    ...
    _QWEN = "\x00qwen"
    # Sentinel marking the OpenCode row 鈥?native-server harness with no Omnigent
    ...
    # Sentinel marking the Goose row 鈥?like Qwen/Antigravity/Cursor it is not a
    ...
    _GOOSE = "\x00goose"
    # Sentinel marking the Hermes row 鈥?like Goose it owns its own auth via
    # ``hermes model`` and is installed via a curl installer.
    _HERMES = "\x00hermes"
    # Sentinel marking the Kiro row 鈥?like Goose/Hermes it owns its own auth (via
    ...
    _KIRO = "\x00kiro"
    # Sentinel marking the Kimi Code row 鈥?like Cursor/Antigravity/Qwen it is
    # not a provider family. Auth lives entirely in the kimi CLI (``kimi login``
    # / ``kimi provider add`` 鈫?~/.kimi/config.toml), so it dispatches to its
    ...
    # (``_ACP_AGENT_PREFIX + slug`` 鈫?per-agent remove drill-in); a single
    # ``_ACP_ADD`` row jumps straight into the add flow. Not a provider family 鈥?
    ...
    # launchable harness (green 鉁?; "missing" is an absent CLI/SDK (red 鉁?;
    # "warn" is installed-but-unconfigured (yellow 鉁?鈥?present, not usable
    ...
    # names, so every 鉁?鉁?lines up in a single column.
    status_styles = {
        "ready": ("鉁?, "green"),
        "missing": ("鉁?, "red"),
        "warn": ("鉁?, "yellow"),
    ...
    def build_harness_rows() -> list[tuple[str, str, str, str, str]]:
        # One visible row per harness, in 0.3 priority order. No folding 鈥?every
        ...
        from agent_meow.onboarding.hermes_auth import hermes_config_summary
        from agent_meow.onboarding.opencode_auth import opencode_auth_summary
        ...
        if not harness_cli_installed(CURSOR_KEY):
            ...
            rows.append(
                (
                    CURSOR_KEY,
                    "Cursor",
                    "CLI not installed 路 SDK ready" if cursor_sdk_ready else "CLI not installed",
                    "missing",
            ...
            rows.append(
                (
                    CURSOR_KEY,
                    "Cursor",
                    "CLI needs login 路 SDK ready" if cursor_sdk_ready else "CLI needs login",
                    "warn",
                    "Open to run `cursor-agent login`.",
                ),
            )

        # OpenCode 鈥?its own provider auth (login or env keys); the status is
        ...
        # Hermes 鈥?curl-installed; its provider/model live in
        ...
        # Antigravity 鈥?Gemini key (antigravity-sdk extra is soft, like Cursor).
        ...
        # Qwen Code 鈥?no CLI login; auth via OpenAI-compatible env vars or the
        ...
        # Goose 鈥?its own provider config via `goose configure`.
        if not harness_cli_installed(GOOSE_KEY):
        ...
        # Copilot 鈥?GitHub token (github-copilot-sdk extra is soft).
        ...
        # Kiro 鈥?native CLI, own auth via `kiro-cli login`; there is no
        ...
        # Kimi Code 鈥?native CLI, own auth via `kimi login`; there is no local
        ...
        # Custom ACP agents 鈥?the generic `acp` harness driving any user-configured
        # ACP-agent command. Each configured agent gets its own overview row
        # (select 鈫?per-agent remove drill-in) so it sits alongside the built-in
        # harnesses, followed by an "Add" row that jumps straight into the add
        # flow. Not gated on a binary 鈥?each agent owns its own install.
        ...
        rows.append(
            (
                ...
                "action",
                "Add an ACP agent (gemini, qwen, goose, 鈥?.",
            )
        )
        return rows
    ...
    while True:
        ...
        # so every 鉁?鉁?glyph lines up vertically (the earlier right-aligned
        ...
        # _render_menu prefixes selected rows with ``"    鉂? "`` (7 cells).
        ...
        max_status_width = max(8, min(30, term_width - 7 - name_col - len("鉁?")))
        options: list[str] = []
