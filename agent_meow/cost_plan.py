...
(:mod:`~?agent_meow.runner.cost_advisor`) and the session label that
...
canonicalizer from :mod:`~?agent_meow.model_override`, so tier ranking and
...
# ``create_session`` in :mod:`~?agent_meow.server.routes.sessions`).
...
class AdvisorVerdict:
    ...
    :param rationale: One-sentence judge explanation, surfaced in the
        ...
        always produces a string (:mod:`~?agent_meow.runner.cost_judge`
