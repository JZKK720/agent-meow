...
codex names the same model ``gpt-5.6-luna`` 鈥?the version segment is dotted
...
catalog 鈥?see :data:`EXTENDED_CATALOG_MODELS`. :func:`codex_spawn_model`
...
someone can read it 鈥?the same posture the claude side takes when a routed
...
#: :data:`agent_meow.claude_model_vocabulary._CATALOG_PREFIXES`, and equal to
#: :data:`agent_meow.server.smart_routing.MODEL_ID_PREFIXES` (both asserted by
...
#: A bare gpt id, split into family, version digits, and optional tier 鈥?#: ``gpt-5-6-luna`` 鈫?``("gpt", "5", "6", "luna")``. Codex spells the
...
#: to make them spawnable. Bare id 鈫?the exact slug the entry is written
...
def clamp_spawn_effort(effort: str | None, model: str | None) -> str | None:
    ...
    :param effort: The spawn's requested effort, or ``None`` when it named
        ...
        already inside the ladder 鈥?nothing to clamp).
