...
# codex-native ``agent_meow.codex_native.bridge_id`` label.
OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY = "agent_meow.opencode_native.bridge_id"
...
# Token file the shared ``agent_meow.claude_native_bridge serve-mcp`` reads to
...
# ``policy_hook.json``; consumed by ``agent_meow.native_cost_popup``).
...
def write_relay_bridge_config(bridge_dir: Path) -> None:
    ...
    The shared ``agent_meow.claude_native_bridge serve-mcp`` stdio server (which
...
def write_cost_popup_config(
    ...
    ``agent_meow.native_cost_popup`` subprocess inside a ``tmux display-popup`` on
