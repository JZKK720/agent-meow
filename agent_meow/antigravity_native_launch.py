...
from agent_meow.onboarding.gemini_auth import gemini_auth_has_credential
...
def should_skip_permissions(
    ...
    * the launch is **headless** — no interactive client will attach to answer
    ...
    :mod:`~?agent_meow.antigravity_native_reader` /
    :mod:`~?agent_meow.antigravity_native_interactions`). agy exposes no firing
    ...
    .. warning:: The ``headless`` argument is currently derived from the
       ...
       :mod:`~?agent_meow.antigravity_native`). That is sound only while
