...
``agent_meow.cli`` after the ``cli`` group and its shared helpers exist. The
command bodies reach those helpers through thin proxies that resolve them on
the ``agent_meow.cli`` module at call time, so this module never needs a
top-level ``import agent_meow.cli`` (no import cycle) and a test's
``monkeypatch`` of an ``agent_meow.cli`` helper still takes effect. Only
decorator-time state (:data:`agent_meow.cli_common.RESUME_PICKER_SENTINEL`,
:data:`agent_meow.cli_common.CLAUDE_STARTUP_PROFILE_ENV_VAR`) is imported at module
scope, from the leaf :mod:`agent_meow.cli_common`.
"""
...
def register_native_commands(cli: click.Group) -> None:
    ...
    # monkeypatch of ``agent_meow.cli.<helper>`` is honored. Binding the function
    ...
    def claude(
        ...
        from agent_meow.harness_startup_config import resolve_harness_command
