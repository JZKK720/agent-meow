...
def build_hooks_config(
    ...
    ``agent_meow.cursor_native_usage record-usage``, which appends the usage to
...
def capture_cursor_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`~?agent_meow.cursor_native_permissions`) to detect cursor-agent's native
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`~?agent_meow.claude_native_bridge.kill_session`.
