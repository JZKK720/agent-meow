...
Usage::
    ...
    from agent_meow.llms import Client
...
def __getattr__(name: str) -> Any:
    ...
    Eager top-level imports here pull in ``agent_meow.llms.client``,
    which imports ``agent_meow.reasoning_effort``, which imports
    ``agent_meow.llms.errors`` -- a submodule load that re-triggers
    ...
    raise AttributeError(f"module 'agent_meow.lms' has no attribute {name!r}")
