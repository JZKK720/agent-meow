...
ANTIGRAVITY_NATIVE_BRIDGE_ID_LABEL_KEY = "agent_meow.antigravity_native.bridge_id"
...
def ensure_agy_onboarding_complete() -> None:
    ...
    (see :func:`~?agent_meow.runner.app._auto_create_antigravity_terminal` and the CLI
...
class AntigravityNativeBridgeState:
    ...
    .. note::
        ...
        driver that superseded it (:mod:`~?agent_meow.antigravity_native_reader`) keeps
    ...
    .. note::
        ...
        :func:`~?agent_meow.antigravity_native_rpc.resolve_language_server_port`).
...
# ── agent-meow MCP relay wiring (sys_* tools) ──────────────────────────────────
#
...
# ``agent_meow.claude_native_bridge serve-mcp`` stdio server — into agy.
#
...
# ``agent_meow.onboarding.gemini_auth``: macOS writes ``oauth_creds.json`` while
...
def build_mcp_config(
    ...
    *,
    ...
    Mirrors cursor #742's :func:`~?agent_meow.cursor_native_bridge.build_mcp_config`
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    Mirrors :func:`~?agent_meow.cursor_native_bridge.write_mcp_bridge_config`.
...
def write_mcp_config(
    ...
    *,
    ...
    config. Mirrors cursor #742's :func:`~?agent_meow.cursor_native_bridge.write_mcp_config`,
...
def send_interaction_keys_via_tui(
    ...
    **in-process permission / question prompt**. When agy runs attended (the
    ...
    :func:`~?agent_meow.antigravity_native_rpc.handle_user_interaction` flips the
    ...
    :func:`~?agent_meow.cursor_native_bridge.send_cursor_pane_keys`.
