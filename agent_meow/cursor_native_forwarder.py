...
:mod:`~?agent_meow.claude_native_forwarder` (which tails Claude Code's JSONL
transcript) and :mod:`~?agent_meow.codex_native_forwarder` (which subscribes to the
...
cursor-native (see ``_publish_turn_status`` in :mod:`~?agent_meow.runner.app`),
...
:mod:`~?agent_meow.cursor_native_status`) — the SAME server contract
...
from agent_meow import cursor_native_status
from agent_meow._native_post_delivery import post_may_have_been_delivered
from agent_meow.cursor_native_bridge import FORK_HISTORY_CLOSE_TAG, FORK_HISTORY_OPEN_TAG
...
#: ``agent_meow.db.db_models.SqlConversationItem``). cursor's content-address blob
...
# real text — the copied history already lives in the agent-meow timeline, so
...
def _read_last_used_model(store_path: Path) -> str | None:
    ...
    catalog (:func:`~?agent_meow.cursor_native.cursor_base_model_options`) and the
    ...
    :func:`~?agent_meow.cursor_native_bridge.inject_model_command`).
...
async def _post_external_session_status(
    ...
    (:func:`~?agent_meow.cursor_native_status.record_turn_end`); this turns that
...
async def _post_external_compaction_status(
    ...
    :func:`~?agent_meow.claude_native_forwarder._post_external_compaction_status`.
...
async def supervise_cursor_forwarder(
    *,
    ...
    Mirrors :func:`~?agent_meow.claude_native_forwarder.supervise_forwarder`: the
