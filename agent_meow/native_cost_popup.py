...
verdict to the agent-meow server's elicitation-resolve endpoint — the **same**
...
:func:`~?agent_meow.claude_native_bridge.display_cost_approval_popup`) as::

    python -I -m agent_meow.native_cost_popup \
...
def launch_blocked_notice(
    ...
    *,
    ...
    if not clients:
        return
    python = python_executable or sys.executable
    argv = [python, "-I", "-m", "agent_meow.native_cost_popup", "--notice", "--message", message]
    if policy_name:
        argv += ["--policy-name", policy_name]
    inner_cmd = shlex.join(argv)
    for client in clients:
...
def _post_verdict(
    *,
    ...
    MCP :class:`~?agent_meow.server.schemas.ElicitationResult` body — the
...
def main(argv: list[str] | None = None) -> int:
    ...
    """
    parser = argparse.ArgumentParser(prog="agent_meow.native_cost_popup")
    parser.add_argument(
        "--notice",
        action="store_true",
