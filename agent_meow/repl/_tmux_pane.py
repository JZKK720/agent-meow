...
def _resolve_omnigent_argv() -> list[str]:
    ...
    3. Fallback — ``[sys.executable, "-m", "agent_meow.cli"]``.
       Always works because if Python is running this code, the
       ``agent_meow.cli`` module is importable. Trades aesthetics
       (the binding shows ``python -m agent_meow.cli``) for
    ...
    :returns: A list of one or two argv elements. Length-1 means
        ...
        ``[python, "-m", "agent_meow.cli"]``. The caller appends
        the subcommand and its args (e.g. ``["pane-split", "-v",
        ...]``) to this prefix.
    """
    argv0 = sys.argv[0] if sys.argv else ""
    # When the process was launched via ``python -m agent_meow.cli``
    ...
    if argv0.endswith(".py"):
        return [sys.executable, "-m", "agent_meow.cli"]
    ...
    # ``agent_meow.cli`` module is importable since we're running
...
def _user_args_after_launcher(launch_argv: list[str]) -> list[str]:
    ...
    - ``[<python>, -m, agent_meow.cli, run, …]`` (python-m fallback)
...
def register_pane(
    *,
    ...
    # ``-m agent_meow.cli`` markers each time is exactly what this
    # repairs.
