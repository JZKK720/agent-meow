...
def _is_recoverable_sse_transport_error(exc: BaseException) -> bool:
    ...
    """
    # Import lazily â€?``httpx`` is a project dependency, but keeping
...
# Single source of truth so every ``fmt.welcome(...)`` call â€?the
# initial banner, ``/new``, ``/switch`` â€?shows the same set. The
...
# (``{model Â· state} â€?hints â€?state: sleeping``) fits the e2e PTY
...
# (see ``server/API.md`` Â§ conversation items â€?asking for more
...
# Sub-agent tree (state badge + ``â†“`` menu). The depth cap mirrors web's
...
def _load_startup_theme() -> TerminalTheme:
    ...
    if persisted_theme is not None:
        # Theme was previously saved â€?use it directly.
        try:
    ...
    from agent_meow.repl._theme_picker import startup_theme_picker
...
# write â€?see the boot sequence's rationale for not using ``host.output``).
_ANSI_DIM = "\033[2m"
_ANSI_RESET = "\033[0m"
...
class _StartupHeader:
    ...
    :param credential: The launch harness's credential as glyph + label,
        e.g. ``"ðŸ§± Databricks (my-ws)"`` â€?a subscription renders
    ...
    :param creds_line: The per-family creds disclosure shown beneath the
        box for multi-vendor agents, e.g. ``"Claude â†?Subscription
        Â·   Codex â†?Subscription"``; ``None`` for single-family
...
def _summarize_description(description: str | None) -> str | None:
    ...
    :param description: The raw spec ``description``, e.g. polly's
        ``"Multi-agent coding orchestrator. polly never â€?``; ``None``
        when absent.
    ...
    if len(first) > max_len:
        first = first[: max_len - 1].rstrip() + "â€?
    return first
...
def _header_glyph(kind: str) -> str:
    ...
    The header drops the subscription ADMISSION TICKETS glyph â€?its red
    rendering is too loud for the banner box â€?while every other kind
    ...
    :returns: The kind's glyph (e.g. ``"ðŸ”‘"``), or ``""`` for the
        subscription kind.
    """
    from agent_meow.onboarding.configure_models import kind_glyph
    from agent_meow.onboarding.provider_config import SUBSCRIPTION_KIND
...
def _build_startup_header(
    ...
    credential. This function does no exception handling â€?a failure is
    ...
    if len(families) > 1:
        parts: list[str] = []
        for fam in families:
            # Effective per-surface default â€?for the pi surface this is
            ...
            if entry is None:
                # No default for this surface â€?but a launch falls back to the
                ...
                if fallback is None:
                    ...
                    label = (
                        f"no default â†?will use {_header_glyph(fallback.kind)} {cred_text}"
                    ).strip()
            else:
                cred_text = credential_label(
                    entry.kind,
            ...
            parts.append(f"{family_label(fam)} â†?{label}")
        creds_line = "   Â·   ".join(parts)
...
def _render_startup_banner_ansi(
    ...
    *,
    ...
    inline as ``"<url>  Â·  server <ver>"``; a per-family creds line is
    appended beneath the box for multi-vendor agents. When *header* is
    ``None`` the box keeps its minimal form â€?just the name, with the
    ...
    """
    from agent_meow.conversation_browser import display_server_url, is_workspace_hosted_url
    from agent_meow.inner.banner import BannerLine, startup_banner_strings
    ...
    # Suppress the version on Databricks workspace mounts â€?a workspace build
    ...
    # Server URL + installed version on one row: "<url>  Â·  server <ver>".
    # The URL is shown for ANY server target, loopback included â€?a local
    ...
    if display_url is not None:
        url_row = display_url
        if version:
            url_row = f"{display_url}  Â·  server {version}"
...
async def _fetch_server_version(client: OmnigentClient) -> str | None:
    ...
    Tries ``GET /v1/info`` â†?``server_version`` first, then falls back to
    the long-standing ``GET /api/version`` â†?``version`` endpoint. The
    ...
    NOT universally unauthed â€?a hosted deployment (behind OIDC / accounts /
    ...
    *per phase* (connect / read / write each 1.0s) so the worst case a
    ...
    banner stays small â€?the connect phase, the dominant cost for an
    unreachable host, fails within a second (and a dead host fails the first
    request, so the fallback adds no latency there). Any failure â€?
    unreachable, slow, 401/4xx/5xx, non-JSON, or a server too old to report
    either field â€?returns ``None`` and the banner simply omits the version
    ...
    for path, key in (("/v1/info", "server_version"), ("/api/version", "version")):
        ...
        except Exception:  # noqa: BLE001 â€?startup-UI boundary: never block boot on a banner detail
            return None
...
def _is_remote_server_url(url: str | None) -> bool:
    ...
    process â€?possibly on another machine â€?and showing the URL
    ...
    try:
        ...
        # Hostname (not an IP literal) â€?treat as remote.
        return True
...
def _humanize_agent_name(agent_name: str) -> str:
    ...
    Centralized so every banner-rendering site agrees â€?the
    initial ``run_repl`` banner, the ``/new`` reset banner,
    the ``/switch`` redraw banner, and the
    "Resumed conversation â€? line all stay consistent.
...
class _ApprovalVerdict(enum.Enum):
    ...
    - ``APPROVE_ONCE`` â€?allow this one request only.
    - ``APPROVE_ALWAYS`` â€?allow this request AND remember the
      decision for the rest of the REPL session. Future asks
      from the same policy at the same phase auto-approve
      without prompting.
    - ``REFUSE`` â€?refuse this request. Fail-closed default
      per POLICIES.md Â§13; anything not explicitly approve is
      a refusal.
    """
...
# Anything outside these sets is a REFUSE â€?fail-closed per
# POLICIES.md Â§13.
...
class _ApprovalState:
    ...
    multiple REPL sessions in the same process isolated â€?
    ...
    2. The session auto-approve cache: a set of
       ...
       prompt and auto-approve. Scoped to this REPL run â€?
    ...
    def __init__(self) -> None:
        ...
        # Current ASK's identity â€?captured on ``begin`` so
        ...
        # (policy_name, phase) â†?"approve always" cache.
    ...
    def is_pre_approved(self, policy_name: str, phase: str) -> bool:
        ...
        Called by the approval hook BEFORE rendering anything â€?
    ...
    def remember_always(self, policy_name: str, phase: str) -> None:
        ...
        Idempotent â€?adding a duplicate entry is a no-op. The
    ...
    def begin(
        ...
        """
        Start a new approval â€?create the future the hook awaits.
...
class _FieldInputState:
    ...
    Same future-based pattern as :class:`_ApprovalState` â€?no direct
    ...
    def __init__(self) -> None:
        ...
        # prompting rather than advancing to â€?and re-prompting for â€?
...
def _build_elicitation_content_from_schema(
    ...
    See :func:`agent_meow.tools._elicitation_schema.build_accept_content_from_schema`
    ...
    """
    from agent_meow.tools._elicitation_schema import build_accept_content_from_schema
...
def _make_elicitation_prompt(
    ...
    (``response.elicitation_request`` SSE event â€?today the
    ...
    - Pre-approved: the user previously said "always" for this
      ...
      an auto-accept fired â€?silent auto-acceptance would be
    ...
    This hook does NOT touch stdin or call :func:`input` â€?
    ...
    Â§7 + ``designs/SERVER_HARNESS_CONTRACT.md`` Â§"Universal
    API additions".

    The bool return is collapsed to MCP ``action`` by the SDK:
    ``True`` â†?``"accept"``, ``False`` â†?``"decline"``. The
    REPL's three-way verdict (once / always / refuse) maps to
    bool the same way â€?"always" still accepts the current
    ...
    async def _on_elicitation_request(ctx: ElicitationRequestCtx) -> bool:
        ...
        if state.is_pre_approved(ctx.policy_name, ctx.phase):
            # Audit line â€?don't be silent when auto-approving,
            # the user might have forgotten they flipped it on.
            host.output(
                Text.from_markup(
                    f"   [{fmt.muted}]auto-approved Â· "
                    f"{ctx.policy_name} Â· {ctx.phase}[/{fmt.muted}]",
                ),
            )
            return True
        ...
        host.output(
            Text.from_markup(
                f"\n [{fmt.warning}]âš?approval required Â· {ctx.phase}[/{fmt.warning}]",
            ),
        )
        host.output(
            Text.from_markup(
        ...
        if _is_external_url:
            # External URL (OAuth, MCP server, etc.) â€?show the link,
            ...
            # Our own URL or form mode â€?use keyboard y/a/n.
            host.output(
                Text.from_markup(
...
def _server_event_to_sdk_event(event: object) -> object | None:
    ...
    # as-is (not translated to SDK events) â€?the adapter handles
...
class _SessionsChatReplAdapter:
    ...
    def __init__(
        ...
        :param agent_name: Human-readable agent name for
            display.
        :param tool_callables: Optional name â†?callable mapping
        ...
        # like the web UI co-drive, and never PATCHes the runner binding â€?
        ...
        # runner-bind PATCH â€?including the periodic ``_runner_recover_watch``
        # watchdog â€?so observing a sub-agent never hijacks its runner or
        # disturbs the owned session's binding.
        self._readonly_view = False
        # Set while CO-DRIVING a sub-agent interactively from the â†?selector:
        ...
        # sends never re-roots the selector â€?Left-arrow still returns to the
        ...
        # event â€?always, regardless of whether send() is active.
        ...
        async with self._ensure_session_lock:
            ...
            if self._session_id is None:
                ...
                session = await self._client.sessions.create(
                    ...
                    # CLI sessions. Doesn't drive any behavior â€?
                ...
                if pending_model_override is not None and session.model_override is None:
                    ...
                    # ``silent`` skips the tmux ``/model`` forward â€?
                    ...
                    except Exception:  # noqa: BLE001 â€?REPL boundary; log and clear
                        _log.warning(
    ...
    async def _bind_runner_if_needed(self) -> None:
        ...
        # holds while observing a sub-agent read-only â€?binding there would
    ...
    async def switch_to_session(self, new_session_id: str) -> str:
        ...
        sessionâ†”runner invariant holds, cancels the SSE pump bound to
        ...
        # co-drive) left over from diving into a sub-agent â€?otherwise the bind
    ...
    async def view_session(
        ...
        id. It never unbinds the prior session nor binds the target â€?so an
        ...
        :param read_only: ``True`` while observing a sub-agent (suppresses
            ...
            watchdog resumes.
        :param interactive: ``True`` to CO-DRIVE the child â€?the user can send
    ...
    async def _stream_pump(self) -> None:
        ...
        while True:
            try:
                ...
                raise
            except Exception as exc:  # noqa: BLE001 â€?reconnect on any error
                # Recoverable transport errors (peer closed mid-chunk,
                # read timeout, transient network error) are normal
                # background noise â€?the session continues server-side
    ...
    async def send(
        ...
        *,
        ...
        ``_on_event`` callback â€?there is no queue or drain loop.
        ...
        # â”€â”€ C1: File attachments â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if files:
        ...
        try:
            ...
            while not self._turn_done.is_set():
                try:
                    ...
                    # asyncio.shield() is needed â€?shield leaks orphaned
    ...
    async def _handle_elicitation(
        ...
        try:
            ...
            if exc.code == "not_found":
                ...
                # The harness already received the verdict â€?treat as no-op.
                return
            raise
    ...
    async def _prompt_schema_fields(
        ...
        for key, prop in properties.items():
            ...
            if description:
                label_parts.append(f" â€?{description}")
            hint_parts: list[str] = []
            ...
            # key) that ``Text.from_markup`` would parse as Rich tags â€?
            ...
            # whole form. The user aborts the turn with Esc (â†?``aborted``).
            while True:
                ...
                if not stripped:
                    if default is not None:
                        ...
                        host.output(
                            Text(
                                f"     â†?{key} is required â€?enter a value (Esc cancels)",
                                style=fmt.warning,
                            ),
                        )
                        continue
                ...
                if prop_type == "boolean":
                    ...
                    try:
                        ...
                        host.output(
                            Text(
                                f"     â†?expected a whole number, got {stripped!r}",
                                style=fmt.warning,
                            ),
                        )
                        continue
                    ...
                    try:
                        ...
                        host.output(
                            Text(
                                f"     â†?expected a number, got {stripped!r}",
                                style=fmt.warning,
                            ),
                        )
                        continue
                ...
                if one_of and isinstance(one_of, list):
                    ...
                    if val not in valid:
                        host.output(
                            Text(
                                f"     â†?choose one of: {', '.join(str(v) for v in valid)}",
                                style=fmt.warning,
                            ),
                        )
                        continue
                    ...
                    if val not in enum_vals:
                        host.output(
                            Text(
                                f"     â†?choose one of: {', '.join(str(v) for v in enum_vals)}",
                                style=fmt.warning,
                            ),
                        )
                        continue
    ...
    async def _unbind_runner_soft(self, session_id: str) -> None:
        ...
        working against unpatched deployments â€?the 1:1 sessionâ†”runner
    ...
    def reset(self) -> None:
        """
        Legacy hook â€?no-op in sessions mode.
    ...
    async def start_new_conversation(self) -> None:
        ...
        old session (1:1 sessionâ†”runner invariant), cancels the SSE
        pump, and clears local state. Session creation stays lazy â€?
        ...
        """
        # A fresh session is owned, not observed â€?clear any read-only-view
    ...
    def resume_from_response(self, response_id: str) -> None:  # noqa: ARG002 â€?legacy hook accepted but ignored in sessions mode
        """
        Legacy hook â€?no-op in sessions mode.
...
async def run_repl(
    ...
    *,
    ...
    """The entire REPL â€?using the framework.
    ...
    :param log_dir: When set, write a JSON dump of the active
        ...
        items. Failures are logged to stderr but do NOT propagate â€?
    ...
    :param harness: The launch harness derived from the local spec,
        e.g. ``"claude-sdk"`` â€?used to name the model + credential in
    ...
    :param agent_description: The agent spec's ``description``, surfaced
        ...
        polly's ``"Multi-agent coding orchestrator. â€?``. ``None``
    ...
    # this the bar only showed "esc cancel Â· ctrl+c exit",
    ...
    host = TerminalHost(
        ...
        # both completers â€?only one fires per keystroke because
        # their trigger characters (``/`` vs ``@``) don't overlap.
        completer=merge_completers([_SlashCommandCompleter(), FileMentionCompleter()]),
        theme=theme,
    )

    # â”€â”€ Debug event pipeline (--debug-events) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    from agent_meow.repl._event_tape import EventTape, PipelineCounters, TapeEntry
    ...
    # turn in this REPL benefits â€?no per-call re-registration.
    ...
    # name â†?callable indirection is what SessionsChat expects.
    tool_callables: dict[str, object] | None = None
    ...
    # a duplicate "â—?agent + text" block. See :class:`_TurnProseTracker`.
    ...
    def _flush_inflight_assistant_text() -> list[FormattedItem]:
        ...
        deltas â€?never between consecutive text blocks. The formatter's
        ...
        re-renders the whole turn's prose â€?prefixed with a fresh
        ``â—†`` â€?on every later delta and tool round (the "growing
    ...
    def _spawn_metadata_refresh() -> None:
        ...
        adapter state is always derived from a snapshot â€?never applied
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, _StatusEv):
            ...
            if event.status == "running":
                ...
                # but the event is transient (no replay) â€?one landing in
                # a stream-pump reconnect gap or before this REPL
                # attached is lost â€?so also re-sync at each turn start.
                ...
                # no response.failed / ErrorEvent ever arrives â€?the only
                ...
                # host.update_context_usage) over the local estimate â€?
        ...
        if isinstance(event, _AgentChangedEv):
            ...
            # event is a trigger, not a data source â€?it carries no
            # llm_model / harness / context_window, and state must come
            # from one place â€?so re-derive from a fresh snapshot (the
        ...
        from agent_meow.server.schemas import ElicitationResolvedEvent
        ...
        if isinstance(event, ElicitationResolvedEvent):
            if approval_state.pending:
                ...
                host.output(
                    Text.from_markup(
                        f"   [{fmt.muted}]â€?resolved via approval page[/{fmt.muted}]",
                    ),
                )
            if tape_entry is not None:
                _maybe_log_tape_entry(tape_entry)
        ...
        # to the host registry (state badge + â†?menu) before the generic
        ...
        if isinstance(sdk_ev, _CIP):
            items_out = [
                Text.from_markup(f"  [{fmt.muted}]Compacting conversation contextâ€¦[/{fmt.muted}]")
            ]
            if tape_entry is not None:
                _event_tape.update_format(tape_entry, items_out)  # type: ignore[union-attr]
            for item in items_out:
        ...
        if isinstance(sdk_ev, _OIDE):
            ...
            if isinstance(item, dict):
                ...
                if plan.flush_inflight_text and not should_render:
                    ...
                    # the entry â€?a stale one could wrongly suppress a
    ...
    def _maybe_log_tape_entry(entry: TapeEntry) -> None:
        ...
        if _event_log_fh is not None:
            from agent_meow.repl._event_tape import log_entry_jsonl
    ...
    async def on_input(text: str, attachments: list[PendingAttachment] | None = None) -> None:
        ...
        if field_input_state.pending:
            ...
            host.output(
                Text(f"   â€?{field_name}: {text}", style=fmt.muted),
            )
        ...
        # transcript makes sense on scrollback â€?otherwise a
        ...
        if approval_state.pending:
            ...
            host.output(
                Text.from_markup(
                    f"   [{fmt.muted}]â€?{verdict_label}[/{fmt.muted}]",
                ),
            )
        ...
        # escapes â€?it sends a literal leading "!" as an ordinary prompt.
        if text.startswith("!") and not text.startswith("!!"):
            cmd = text[1:].strip()
            if not cmd:
                host.output(
                    Text.from_markup(
                        f"   [{fmt.muted}]! <command> runs a shell command Â· "
        ...
        # contain more path separators â€?don't treat those as commands.
        first_token = text.split()[0] if text.split() else ""
        if first_token.startswith("/") and "/" not in first_token[1:]:
            # Starting a new conversation orphans any buffered "!" output â€?it
        ...
        # While observing a sub-agent read-only (dived in via â†?on a CLOSED /
        # non-chattable child), refuse plain message sends â€?there's no live
        ...
        # press â†?to return to the top-level session.
        if getattr(session, "_readonly_view", False) and not getattr(
            session, "_interactive_child", False
        ):
            host.output(
                Text.from_markup(
                    f"   [{fmt.muted}]read-only view (closed sub-agent) â€?press â†?to "
        ...
        # â€?the ``ðŸ“Ž`` chip shows them instead.  But the LLM needs
        ...
        if is_streaming:
            ...
            # has visual breathing room â€?otherwise it wedges
        ...
        # in the SDK formatter already prefixes ``â—?model`` with
        ...
        try:
            ...
            # Best-effort â€?server may already have finished.
            with contextlib.suppress(Exception):
            ...
            raise
        except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: any uncaught error here would be swallowed by prompt-toolkit's background runner, leaving the user staring at a silent prompt (see comment below for the concrete incident this guards against)
            ...
            # it silently â€?leaving the user staring at the prompt
    ...
    # Ctrl+O debug overview. Registered here â€?not inside the SDK â€?
    ...
    # or when the user isn't running inside tmux to begin with â€?
    ...
    host.add_overlay(
        Overlay(
            ...
            title=f" Debug overview â€?{ui_name}",
            actions=(
    ...
    # â”€â”€ â†?Sub-agents menu â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # While sub-agents are running, the toolbar reads ``state: N agents
    # running`` instead of ``sleeping`` (see ``build_toolbar``) and a
    # ``â†?agents`` hint advertises the menu. Pressing Down on an empty input
    ...
    # The session the tree is rooted at â€?the originally-launched "main"
    ...
    # though no sub-agents are registered yet â€?see ``_subagent_poll_loop``.
    ...
    def _sync_subagent_root() -> None:
        ...
        # observing a sub-agent â€?either read-only OR co-driving it
        # interactively â€?freeze the root: never self-heal off
    ...
    async def _subagent_poll_loop() -> None:
        ...
        # statuses stay current â€?the SSE stream only carries the active
        ...
        # is live work to track â€?an active sub-agent, or a child the user has
        # dived into (whose own stream can't refresh its row) â€?OR when the root
        ...
        while True:
            try:
                ...
                raise
            except Exception:  # noqa: BLE001 â€?best-effort background poll; never crash the REPL
                pass
            await asyncio.sleep(_SUBAGENT_POLL_SECONDS)

    async def _open_subagent_by_id(target_id: str) -> None:
        # Invoked by the host when the user picks a row in the inline â†?menu
        ...
        # co-drive target â€?the user can type to chat with it, POSTing to the
        ...
        except Exception as exc:  # noqa: BLE001 â€?REPL boundary: render the failure, stay alive
            host.output(
                Text.from_markup(f"  [bold red]Failed to open {target_id[:16]}â€? {exc}[/]")
            )
            return
        ...
        if interactive:
            host.output(
                Text.from_markup(
                    f"  [{fmt.muted}]interactive â€?type to chat with this sub-agent; "
                    f"â†?back to main[/{fmt.muted}]"
                )
            )
        elif not returning_to_root:
            host.output(
                Text.from_markup(
                    f"  [{fmt.muted}]read-only (closed sub-agent) â€?â†?back to main[/{fmt.muted}]"
                )
            )
        await _refresh_subagents()
    ...
    # â”€â”€ Ctrl+E event tape overlay (--debug-events only) â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # the full detail panel â€?pipeline journey + raw JSON payload.
    if debug_events and _event_tape is not None:
        ...
        async def _tape_targets() -> list[OverlayTarget]:
            ...
            """
            from agent_meow.repl._event_tape import _OverlayTargetLike
    ...
    async with host:
        # â”€â”€ Open JSONL event log when --debug-events is on â”€â”€â”€â”€
        ...
        if debug_events:
            ...
            from agent_meow.repl._event_tape import open_event_log
            ...
            _event_log_fh = open(_event_log_path, "a")  # noqa: SIM115 â€?closed in finally below

        # Mirror the legacy CLI's mascot-art startup banner so the
        # Omnigent REPL feels identical at boot. Raw stdout write
        # (matching ``omnigent/inner/cli.py:2962``) â€?the banner
        ...
        if attach_only:
            ...
            # hint â€?this is a co-drive client joining the host's live session,
            ...
            except Exception:  # noqa: BLE001 â€?startup-UI boundary: a config read must never block REPL boot
        ...
        #   - the server is a Databricks workspace mount â€?a workspace build
        ...
        # was just printed above â€?a second banner would
        ...
        # text (the "Resumed conversation â€? line) matches
        ...
        if resume_conversation_id is not None:
            ...
            except Exception as exc:  # noqa: BLE001 â€?REPL boundary: never crash on resume failure; render and proceed
                _log.exception("Failed to resume conversation %s", resume_conversation_id)
                host.output(
                    Text.from_markup(
                        f"  [bold red]Failed to resume {resume_conversation_id[:16]}â€? {exc}[/]"
                    )
                )
        # Hold a reference to the auto-send task for the lifetime of
        # ``host.run`` â€?``asyncio.create_task`` only weakly roots its
        ...
        # Background poll that keeps the sub-agent tree (badge + â†?menu)
...
async def _maybe_write_session_log(
    ...
    :param client: Connected OmnigentClient â€?REUSED, not opened
    ...
    """
    from agent_meow.repl._session_log import write_session_log
    ...
    except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: same rationale as above
...
# Single registry: name â†?(help string, handler).
#
# Every handler MUST accept the same 5 positional parameters â€?
# ``(arg, session, client, host, fmt)`` â€?because the dispatcher
...
async def _cmd_help(
    arg: str,  # noqa: ARG001 â€?dispatch-contract params (see COMMANDS docstring)
...
async def _cmd_theme(
    arg: str,
    session: Session,  # noqa: ARG001 â€?dispatch-contract params
    client: OmnigentClient,  # noqa: ARG001 â€?dispatch-contract params
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    if not value:
        ...
        host.output(
            Text.from_markup(
                f"  [{fmt.muted}]usage: /theme light Â· /theme dark Â· "
    ...
    if value not in _EFFORT_VALUES:
        host.output(
            Text.from_markup(
                ...
                f"{value} Â· expected none, minimal, low, medium, high, xhigh, max, or default[/]"
            )
        )
        return
...
def _model_readout_harness(active_model: str | None) -> str:
    ...
    unrecognised â€?that yields the anthropic-family default, the most
    ...
    :returns: A canonical harness name, e.g. ``"claude-sdk"`` or
        ``"openai-agents"``.
    """
    from agent_meow.llms.routing import infer_harness_from_model
...
def _session_readout_harness(session: Session) -> str:
    ...
    ``session.harness``) so the readout reflects the real provider family â€?
...
def _build_model_readout_lines(
    ...
    Renders one ``Active:`` line â€?``<model> Â· <glyph friendly-provider>
    Â· <source>`` via :func:`describe_active_credential` â€?using the kind
    glyph in place of the kind word so a databricks provider named
    ``databricks`` doesn't render as the redundant ``databricks Â·
    ...
    provider â€?switching the active provider mid-session is not wired, so
    ...
    ambient ``$ANTHROPIC_API_KEY`` does *not* shadow it â€?warning the
    ...
    :param config: The parsed effective config mapping (``providers:``
        block), e.g. from
        :func:`agent_meow.onboarding.provider_config.load_config`.
    ...
    :returns: Plain (un-markup) display lines, e.g.
        ``["Active:  claude-sonnet-4-6  Â·  ðŸ”‘ Anthropic API Key  Â·  $ANTHROPIC_API_KEY",
        "Also configured:  ðŸ§± Databricks", "  /model <name> changes the model. â€?]``.
    """
    from agent_meow.onboarding.configure_models import (
        credential_label,
        kind_glyph,
    ...
    if cred is None:
        # Nothing resolves for this harness's surface â€?not in the explicit
        ...
        if model_override is not None:
            lines.append(f"Active:  {model_override}  Â·  (provider unresolved)")
        else:
            lines.append("Active:  None  Â·  None")
            lines.append(
                "no model configured â€?run `omnigent setup --no-internal-beta` to add one"
            )
        lines.append("usage: /model <name> Â· /model default | off | reset to clear")
        return lines

    # One clean "Active" line: <model> Â· <glyph friendly-provider> Â· <source>.
    # The kind glyph stands in for the kind word, so a databricks provider
    # named "databricks" no longer renders as the redundant "databricks Â·
    ...
    if cred.model:
        ...
        model_label = "(Databricks profile picks the model â€?pin one with /model <name>)"
    elif cred.kind == "subscription":
        model_label = "(CLI login picks the model â€?pin one with /model <name>)"
    else:
        model_label = "(no model pinned â€?set one with /model <name>)"
    glyph = kind_glyph(cred.kind)
    # credential_label is the single source of truth shared with `configure
    # harnesses` â€?a subscription reads "Subscription" (not the brand name
    ...
    lines.append(f"Active:  {model_label}  Â·  {provider_label}  Â·  {cred.source}")
    ...
    # family â€?filter its alternates on the pi surface instead, which every
    ...
    if others:
        ...
        lines.append("Also configured:  " + "  Â·  ".join(items))
...
def _match_configured_provider(config: dict[str, object], token: str) -> str | None:
    ...
    shows â€?``"Anthropic"`` resolves to the configured ``"anthropic"``).
    ...
    """
    from agent_meow.onboarding.configure_models import provider_display_name
...
def _model_validation_warning(model: str) -> str | None:
    ...
    (:func:`agent_meow.onboarding.providers.get_chat_models`). An unknown
    ...
    try:
        ...
        # ``qwen/qwen3.7-plus`` via OpenRouter) â€?the gateway, not our
        # catalog, owns the naming. Inform, don't alarm.
        return f"{model!r} isn't a catalog model â€?fine for gateway / OSS models; using it as-is."
...
async def _cmd_model(
    ...
    No-arg shows the active credential (model Â· provider Â· source) via
    ...
    with guidance â€?switching the active provider mid-session is not wired
    ...
    def _emit_model_readout() -> None:
        from agent_meow.onboarding.detected import effective_config_with_detected
        from agent_meow.onboarding.provider_config import load_config
    ...
    value = arg.strip()
    # Bare `/model` and the display keywords both just show the readout â€?never
    ...
    # cannot switch the provider â€?that's resolved server-side from the
    ...
    from agent_meow.onboarding.configure_models import kind_glyph, provider_display_name
    from agent_meow.onboarding.detected import effective_config_with_detected
    from agent_meow.onboarding.provider_config import (
    ...
    if "/" not in value and matched is not None and matched == active_name:
        # Bare active-provider name â†?resolve its configured default model.
        resolved = _resolve_provider_default_model(config, matched)
        if resolved is None:
            # databricks / subscription pick their own model â€?nothing to set.
            host.output(
                Text.from_markup(
        ...
        host.output(
            Text.from_markup(
                f"  [{fmt.muted}]resolved provider {value!r} â†?{target}[/{fmt.muted}]"
            )
        )

    # Validate against the catalog â€?inform, but do NOT block (gateways and
...
async def _start_new_conversation(
    session: Session,
    host: TerminalHost,
    fmt: RichBlockFormatter,  # noqa: ARG001 â€?reserved for future banner styling
) -> bool:
    ...
    if callable(starter):
        try:
            await starter()
        except Exception as exc:  # noqa: BLE001 â€?REPL boundary
    ...
    # in the badge / â†?menu under the fresh session.
...
async def _cmd_new(
    arg: str,  # noqa: ARG001 â€?dispatch-contract params
    session: Session,
    client: OmnigentClient,  # noqa: ARG001
    host: TerminalHost,
    fmt: RichBlockFormatter,
...
async def _cmd_switch(
    ...
    if not arg:
        ...
        try:
            ...
            # has prior-conversation transcript on screen â€?
        ...
        except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: render network/server errors as inline text so the REPL stays responsive instead of crashing
...
async def _attach_to_conversation(
    ...
    *,
    ...
    uses â€?so a resumed conversation looks identical to the
    ...
    - The ``/switch <id>`` slash command (interactive switch
      mid-session) â€?passes ``redraw_screen=True`` because the
    ...
    - ``run_repl(resume_conversation_id=...)`` on startup
      ...
      designs/RUN_OMNIGENT_SESSION_RESUMPTION.md) â€?passes
    ...
    Both call sites render the FULL conversation, not a tail â€?
    ...
    :param ui_name: The display-formatted agent name shown in
        ...
        banner â€?typically via :func:`_humanize_agent_name`.
    :param redraw_screen: When True, clear the screen and
        ...
        conversation â€? line â€?appropriate when the banner is
        already on screen.
    ...
    # local REPL right away â€?without this, they only surface after
    ...
    if last_response_id is None:
        ...
        # as the resume target, so this is the normal new-session case â€?the
    ...
    host.output(
        Text.from_markup(
            f"  [{fmt.muted}]Resumed conversation {conversation_id[:16]}â€¦[/{fmt.muted}]\n"
        )
    )

    # Pre-pass: build a call_id â†?tool metadata lookup so
    ...
    # ``items`` is already fetched above â€?no extra API call needed.
    cw = getattr(session, "context_window", None)
    if cw:
        ...
        if last_total is not None:
            ...
            # will see as input_tokens on the next turn â€?far more
            # accurate than a local count_tokens() estimate.
            tokens = last_total
        else:
            from agent_meow.runtime.compaction import count_tokens
...
async def _cmd_fork(
    ...
    then switches the REPL to the fork **in-place** â€?no screen
    ...
    except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: render server errors inline
...
async def _cmd_history(
    ...
    if sessions_api_conv_id is not None:
        ...
        except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: surface server errors inline
            host.output(Text.from_markup(f"  [bold red]Error: {exc}[/]"))
        return
...
_CONTEXT_COIN_USED: str = "â–?
_CONTEXT_COIN_FREE: str = "â–?
_CONTEXT_COIN_BUF: str = "â–?
...
async def _fetch_context_items(
    ...
    if sessions_api_conv_id is not None:
        ...
        except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary: surface inline
    ...
    if session.current_response_id:
        try:
            ...
            if resp.conversation:
                items = await _list_all_conversation_items(client, resp.conversation.id)
                return _ContextItems(items=items, error=None)
        except Exception as exc:  # noqa: BLE001 â€?REPL UI boundary
            return _ContextItems(items=[], error=str(exc))
...
async def _refresh_session_metadata(
    ...
    agent â€?and with it ``llm_model`` / ``harness`` /
    ``context_window`` / ``model_override`` â€?can change between turns
    ...
    trigger), but the event is transient SSE-only with no replay â€?one
    landing in a stream-pump reconnect gap or before this REPL attached
    is lost â€?so each turn start re-fires the refresh as the catch-up
    ...
    except Exception:  # noqa: BLE001 â€?background refresh at the REPL UI boundary: a stale toolbar beats an unhandled-task traceback
        return
    hydrate(snap)
    ...
    if new_name != old_name:
        ...
        host.output(
            Text.from_markup(
                f"  [{fmt.muted}]Agent switched: {_humanize_agent_name(old_name)} â†?"
...
def _render_context_tree(
    ...
    # read e.g. "920,150 tokens (72%)" â€?a count that is 92% of the window.)
...
async def _cmd_compact(
    ...
    if not callable(compact):
        ...
        return
    # Progress messages ("Compactingâ€? / "Compaction complete.") arrive
    ...
    # â€?doing so would duplicate them for explicit /compact calls.
    try:
        result = compact()
        if inspect.isawaitable(result):
            await result
    except Exception as exc:  # noqa: BLE001 â€?REPL boundary: keep prompt alive
...
async def _cmd_context(
    ...
    # returned in SessionResponse â€?avoids requiring litellm client-side.
    context_window: int | None = getattr(session, "context_window", None)

    # Use the provider-reported token count from the most recently
    # completed response when available â€?it includes the system prompt,
    ...
    if live_tokens is not None:
        ...
        # recognised LLM identifier â€?good enough for an estimate.
...
async def _cmd_logs(
    ...
    from agent_meow.cli_diagnostics import current_cli_log_path
    from agent_meow.repl._session_log import write_logs_zip, write_session_log
    ...
    if session_id:
        ...
        except Exception as exc:  # noqa: BLE001 â€?slash-command UI boundary
...
async def _cmd_report(
    arg: str,
    session: Session,
    client: OmnigentClient,  # noqa: ARG001 â€?dispatch-contract param
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    if opened:
        host.output(
            Text.from_markup(f"  [{fmt.muted}]Opening GitHub issue in browserâ€¦[/{fmt.muted}]")
        )
    else:
        host.output(
            Text.from_markup(
...
async def _list_all_conversation_items(
    ...
    while True:
        ...
        except Exception:  # noqa: BLE001 â€?overlay builder: any per-page error falls back to whatever was already fetched; partial sidebar beats no sidebar
            break
...
def _should_discover_subagents(
    ...
    * ``has_active_subagents`` â€?a sub-agent anywhere in the tree is still
      running, so its (and any grandchild's) status keeps changing; or
    * ``observing_subagent`` â€?the user has dived into a child (read-only or
    ...
    * ``last_polled_root != root_id`` â€?the root just changed: a one-shot
...
def _apply_child_session_event(
    ...
    *,
    ...
    when the event's carrier ``conversation_id`` is the active session â€?so a
...
async def _refresh_subagent_tree(
    ...
    *,
    ...
    ``GET â€?child_sessions`` breadth-first capped at ``MAX_TREE_DEPTH`` and tags
    ...
    except Exception:  # noqa: BLE001 â€?best-effort poll: a failed fetch leaves the prior tree in place rather than crashing the REPL
        return
...
async def _collect_overview_targets(
    ...
    ``sys_session_send`` (continuation path) â€?the tool outputs include a persistent
    ...
    :param client: Agent-plane HTTP client used for the items
        fetch.
    :param session: REPL session â€?only
    ...
    """
    targets: list[OverlayTarget] = [OverlayTarget(key="main", label="main", icon="ðŸ¤–")]
    ...
    if sessions_api_conv_id is not None:
        ...
        except Exception:  # noqa: BLE001 â€?overlay builder: any network/server error falls back to the base targets list; the overlay must open even under partial failure
            return targets
    ...
    # Recreate rather than mutate â€?dataclasses are frozen-ish in
    # intent even when the runtime allows assignment.
    targets[0] = OverlayTarget(key=conv_id, label="main", icon="ðŸ¤–")
    ...
    # Dedupe by conversation_id â€?repeated sys_session_send calls (continuation path)
    ...
    for item in items:
        ...
        if not isinstance(sa_agent, str) or not isinstance(sa_title, str):
            # Malformed payload â€?skip rather than emit a "?:?"
    ...
    # Terminals â€?inferred from the agent's tool history rather
    ...
    # appears here. Acceptable for an MVP â€?see the design
    ...
    for info in terminals:
        # ``ðŸ’»`` (U+1F4BB PERSONAL COMPUTER, "laptop") is
        # wcswidth-wide (2 cells) AND reads as a computer
        # â€?keeps the visual category consistent with the
        # F20 overview pane the legacy CLI had. Avoid the
        # otherwise-tempting ``ðŸ–¥`` (U+1F5A5 DESKTOP COMPUTER):
...
async def _collect_terminals_for_conversations(
    ...
    *,
    ...
    async def fetch_items(cid: str) -> tuple[str, list[dict[str, object]]]:
        if cid in seed:
            return cid, seed[cid]
        # Paginate past the per-request 100-item cap â€?same
...
# sidebar targets from main / sub-agent targets â€?both of those
...
class _TerminalInfo:
    ...
    calls" â€?a process that crashed outside the agent's tool
    ...
    :param session: Session key the launch call passed,
        ...
        terminal.
    :param socket: Tmux socket path from the launch output â€?
    ...
    :param conv_id: The conversation that owns this terminal
        â€?main agent's conversation for parent-spawned
...
def _terminal_target_key(info: _TerminalInfo) -> str:
    ...
    + target are NOT encoded â€?the builder re-walks the owning
...
def _decode_terminal_target_key(key: str) -> tuple[str, str, str] | None:
    ...
    :returns: ``(conv_id, name, session)`` if *key* is a
        terminal key, ``None`` otherwise â€?non-terminal keys
...
def _parse_terminal_tool_output(raw: object) -> dict[str, object] | None:
    ...
    :param raw: The ``function_call_output.output`` value as
        persisted â€?a string in both cases.
...
def _reconstruct_terminals_from_items(
    ...
    *,
    ...
    decode skip the item rather than crash â€?the inferred view
    ...
    for item in items:
        ...
        if tool_name == "sys_terminal_launch":
            ...
            # "already_running" (idempotent re-launch â€?same
            ...
            if payload.get("status") == "closed":
                live.pop(key, None)
            # ``not_found`` closes are no-ops â€?the LLM tried
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    Four guards short-circuit cleanly without raising â€?the
    ...
    4. The agent's tmux session is dead at runtime (user
       ...
       the pane â†?window â†?session â†?tmux server). The walker
       still reports the terminal as live because no
       ``sys_terminal_close`` was recorded â€?the agent didn't
    ...
    :param client: Omnigent HTTP client â€?used to re-walk the
    ...
    if decoded is None:
        # Not a terminal target â€?silently ignore. Pressing
    ...
    if not os.environ.get("TMUX"):
        ...
        # command from the panel and paste it themselves â€?
    ...
    # launch outputs land past position 99 â€?same bug shape as
    ...
    except Exception:  # noqa: BLE001 â€?overlay action: a per-conversation fetch error becomes a stderr hint instead of crashing the overlay
        import sys as _sys
    ...
    if not _tmux_session_alive(info.socket, info.target):
        print(
            ...
            f"the agent's pane â†?window â†?session â†?tmux server). The "
    ...
    # command that the new window's pane will run â€?we hand
...
def _terminal_attach_command(info: _TerminalInfo) -> str:
    ...
    :meth:`agent_meow.inner.cli._terminal_attach_command`
...
async def _build_terminal_overview(
    ...
    *,
    ...
    key stay short â€?the socket isn't on the key, it's read
    fresh from the persisted launch output.

    The panel mirrors the legacy CLI's
    :meth:`agent_meow.inner.cli._render_overview_terminal_text`
    output (``cli.py:2232``):
    ...
    :param target: The selected :class:`OverlayTarget` â€?
    ...
    # are still findable here â€?the user-reported 2026-04-30
    ...
    except Exception as exc:  # noqa: BLE001 â€?overlay content builder: any items-fetch error surfaces as a diagnostic line; the panel still renders
        parts.append(
            Text.from_markup(
    ...
    if not matches:
        # Terminal isn't in the live set â€?either it was closed
    ...
    # the recovered socket â€?the inferred-from-tool-history
    ...
    if is_alive:
        ...
        parts.append(
            Text.from_markup(
                ...
                f"agent doesn't know â€?no ``sys_terminal_close`` was "
                f"recorded â€?so the sidebar still shows the row. Ask the "
...
def _tmux_session_alive(socket: str, target: str) -> bool:
    ...
    runtime liveness of an agent-launched tmux session â€?the
    inferred-from-tool-history view alone can't catch sessions
    the agent didn't formally close (e.g. the user attached,
    typed ``exit`` in the bash pane, killing the pane â†?window
    â†?session â†?tmux server). The agent never knew, so no
    ...
    The Attach command stays printed regardless â€?the user can
    ...
    :returns: ``True`` only when ``tmux has-session`` exits
        zero â€?i.e. the session is reachable on the socket.
    """
    import subprocess as _subprocess
...
def _parse_sub_agent_handle(raw: str) -> dict[str, object] | None:
    ...
    as a raw JSON string of the handle dict â€?
    ``{"kind": "sub_agent", "conversation_id": ..., ...}``.

    Harnesses that route tools through an MCP server â€?notably the
    claude-sdk harness's MCP bridge â€?wrap the same payload as an
    ...
    if isinstance(payload, list):
        ...
        # spec, but sys_session_send emits exactly one â€?return the
...
async def _build_debug_overview(
    ...
    *,
    ...
    2. **Event stream** â€?all items from the conversation,
       ...
       follows: ``message`` (user) â†?``user_message``;
       ``message`` (assistant) â†?``assistant_message``;
       ``function_call`` â†?``tool_call_request``;
       ``function_call_output`` â†?``tool_call_complete``.
       Reasoning items are shown as ``reasoning``.
    3. **Fallback** â€?when no conversation exists yet (fresh
       REPL, no turns), a one-liner explaining the state.

    Errors from the ``/v1/sessions`` fetch surface inside
    the overlay as a red line rather than propagating â€?Ctrl+O is
    ...
    :param agent_name: Registered agent name for the header, e.g.
        ``"coding_supervisor"``.
    :param fmt: The REPL's :class:`RichBlockFormatter` â€?reused so
    ...
    # Terminal targets short-circuit to a dedicated renderer â€?
    ...
    if is_main:
        if target is not None and target.key != "main":
            ...
            # building the target list â€?skip the extra round-trip.
            conv_id = target.key
        elif response_id:
            try:
                resp = await client.responses.get(response_id)
                conv_id = resp.conversation.id if resp.conversation else None
            except Exception as exc:  # noqa: BLE001 â€?overlay content builder: capture any lookup error as a displayable string; the overlay panel must still render even under partial failure
    ...
    # the overlay can render guardrails label state â€?the legacy
    ...
    try:
        ...
        labels = snap.labels
    except Exception as exc:  # noqa: BLE001 â€?overlay content builder
    ...
    # pair â€?matching the omnigent event view. When an output
...
def _render_overview_event(
    ...
    :param call_id_to_name: Precomputed ``call_id â†?tool_name``
        lookup built from the same items list â€?used to print
    ...
    # Missing ``type`` is an API violation â€?every conversation
    ...
    if itype == "function_call_output":
        ...
        # that case â€?``(missing call_id)`` tells the reader the
        ...
        if summary:
            ...
            for line in text.split("\n"):
                if line.strip():
                    lines.append(Text.from_markup(f"    [{fmt.muted}]{line}[/{fmt.muted}]"))
        return lines
    # Unknown item type â€?surface it rather than silently dropping,
...
def _render_overview_message_event(
    ...
    :returns: Rich renderables â€?header + continuation lines.
    """
    from rich.text import Text
...
def _build_call_id_to_name_lookup(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    :returns: Map from ``call_id`` to tool name. Items missing
        either field are skipped silently â€?callers fall back to
...
def _coerce_arguments_dict(raw: object) -> dict[str, object]:
    ...
    :returns: Parsed arguments dict, e.g. ``{"file_path": "/x.py"}``.
        Empty dict on any decode failure or non-object payload â€?
        the caller renders ``â?name()`` instead of raising.
    """
...
def _render_message_history_item(
    ...
    (same ``â¯`` echo shown live). Assistant messages emit a
...
# â”€â”€ Slash-command autocomplete â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


# Hidden from the popup so it doesn't show duplicate rows for the
# same handler â€?``/help`` already lists the canonical names.
...
def register_skill_commands(skills: list[SkillSpec]) -> list[str]:
    ...
    Skills marked ``user-invocable: false`` are skipped â€?they are
    ...
    for skill in skills:
        ...
        if not _SKILL_COMMAND_NAME_RE.match(skill.name):
            ...
            # uninvocable or colliding command â€?skip + warn rather than
        ...
        def _make_handler(sk: SkillSpec) -> SlashCommandHandler:
            ...
            async def _skill_handler(
                ...
                if arg:
                    ...
                    host.output(
                        Text.from_markup(f"  [{fmt.muted}]Loading skill {sk.name}â€¦[/{fmt.muted}]")
                    )
                host.start_timer()
                await asyncio.sleep(0)
...
class _SlashCommandCompleter(Completer):
    ...
    def get_completions(
        self,
        document: Document,
        complete_event: CompleteEvent,  # noqa: ARG002 â€?Completer protocol contract
    ) -> Iterable[Completion]:
        """
        ...
        :param complete_event: prompt-toolkit trigger metadata
            (manual vs. while-typing). Unused â€?the popup is cheap
        ...
        # description â€?kept identical to the web menu, which never shows
        # descriptions inline.
        #
        ...
        # tier keeps ``COMMANDS`` insertion order (an empty query â€?lone
        # "/" â€?puts every command in the prefix tier, unchanged).
...
# â”€â”€ "!" shell passthrough â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _resolve_cd(cmd: str, cwd: str) -> str | None:
    ...
    ``cd`` with no argument resolves to home. Only a lone ``cd`` is handled â€?
    a ``cd`` inside a compound command (``cd x && â€¦``) runs in its own subshell
...
def _write_bang_overflow(cmd: str, stdout: str, stderr: str) -> str | None:
    ...
    The overflow is measured on the ANSI-stripped text â€?the same form the
    context builder caps â€?so heavily-styled output doesn't trip the spill when
...
def _build_bang_context(
    ...
    if not out and not err:
        parts += ["", "(no output)"]
    if overflow_path:
        parts += ["", f"(output truncated above â€?full output saved to: {overflow_path})"]
    return "\n".join(parts)
...
async def _run_bang_command(
    ...
    if timed_out:
        ...
        host.output(
            _RText.from_markup(
                f"   [{fmt.warning}]â?killed after {secs}s (timeout)[/{fmt.warning}]"
            )
        )
        status = f"timed out and was killed after {secs}s"
    else:
        style = fmt.muted if code == 0 else fmt.warning
        host.output(_RText.from_markup(f"   [{style}]exit {code} Â· {elapsed:.1f}s[/{style}]"))
        status = f"exit: {code}"
...
async def handle_slash_command(
    ...
    if entry:
        ...
        host.output(
            Text.from_markup(
                f"  [{fmt.muted}]Unknown command: {cmd} Â· /help for list[/{fmt.muted}]"
            )
        )
