...
* the ``Agent`` / ``Task`` tool's ``model`` parameter 鈥?a closed enum
  (``sonnet``, ``opus``, ``haiku``, ``fable``), so a catalog id fails
  schema validation and the spawn dies before it starts;
* the ``/model`` slash command 鈥?an alias (or the custom slot's exact id)
...
inverting that mapping is exact 鈥?and only exact: a family segment alone
...
#: Alias 鈫?env var Claude Code reads to pin that alias to one model id.
...
#: offline, compared BYTE-EXACTLY (case included) against this value 鈥?so
...
#: with, e.g. ``"Sonnet 5"``. Cosmetic 鈥?the slot's id is what ``/model``
#: takes 鈥?so it is not part of the vocabulary below.
...
#: :data:`agent_meow.server.smart_routing.MODEL_ID_PREFIXES` (asserted by
...
def alias_pins(env: Mapping[str, str] | None = None) -> dict[str, str]:
    """Read the session's alias 鈫?model-id pinning.

    :param env: Environment mapping. ``None`` reads :data:`os.environ`.
    :returns: Alias 鈫?pinned model id, for the aliases that are pinned.
    """
...
def model_vocabulary_env(options: Iterable[Mapping[str, Any]]) -> dict[str, str]:
    ...
    ``opus`` / ``sonnet`` rows) pin nothing 鈥?Claude resolves those
    itself 鈥?so they are skipped rather than read as a pin onto an alias.
...
def claude_model_alias(
    ...
    gateway rejects, and a MISMATCHED pin is worse 鈥?the alias resolves
    ...
    :param env: Environment mapping holding the alias pinning. ``None``
        reads :data:`os.environ` 鈥?a hook subprocess inherits the CLI's.
...
def claude_model_command_arg(
    ...
    :returns: The ``/model`` argument, or ``None`` when the id maps to
        nothing the command accepts (the caller must skip the switch 鈥?        an unaccepted value silently keeps the current model).
    """
    if not isinstance(model, str) or not model.strip():
        return None
    environ = os.environ if env is None else env
