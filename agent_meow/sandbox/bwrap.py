...
:mod:`~?agent_meow.inner.bwrap_sandbox` calls
...
:mod:`~?agent_meow.sandbox.seatbelt`).

The bwrap backend is the Linux platform default when the ``bwrap``
binary is on ``PATH`` (selected by
:func:`~?agent_meow.inner.sandbox._default_sandbox_for_platform`). When
...
from agent_meow.inner.bwrap_sandbox import BwrapSandboxBackend
