...
def copilot_sdk_installed() -> bool:
    ...
    (:mod:`~?agent_meow.inner.copilot_executor`), so a token can be set with no SDK;
    setup uses this to detect that and offer to install it. The
    ``github-copilot-sdk`` package is imported as ``copilot``. Mirrors
    :func:`~?agent_meow.onboarding.cursor_auth.cursor_sdk_installed` /
...
def install_copilot_sdk() -> bool:
    ...
    visible. Mirrors :func:`~?agent_meow.onboarding.cursor_auth.install_cursor_sdk`.
