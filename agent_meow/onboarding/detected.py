...
:mod:`~?agent_meow.onboarding.ambient` discovers credentials already on the
...
``providers:`` shape consumed by :mod:`~?agent_meow.onboarding.provider_config`,
...
def codex_config_provider_dismissed(config: dict[str, object]) -> bool:
    ...
    :returns: ``True`` when ``~/.codex/config.toml`` selects a custom,
        ...
        dismissed in *config*.
    """
    from agent_meow.onboarding.ambient import codex_config_detection
...
def synthesize_detected_entries(
    ...
    :param detected: Detections from
        :func:`~?agent_meow.onboarding.ambient.detect_providers`, in priority
        order.
...
def effective_config_with_detected(
    ...
    :param detected: Detections to merge; defaults to a live
        :func:`~?agent_meow.onboarding.ambient.detect_providers` call.
