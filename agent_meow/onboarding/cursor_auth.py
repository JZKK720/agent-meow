...
(:func:`~?agent_meow.runtime.workflow._load_global_auth`) — a Cursor key parked
...
def cursor_sdk_installed() -> bool:
    ...
    (:mod:`~?agent_meow.inner.cursor_executor`), so a key can be set with no SDK;
    setup uses this to detect that and offer to install it. Mirrors
    :func:`~?agent_meow.onboarding.databricks_config.databricks_sdk_installed` /
    :func:`~?agent_meow.onboarding.antigravity_auth.antigravity_sdk_installed`:
...
def cursor_install_command() -> list[str]:
    ...
    Delegates to :func:`~?agent_meow.onboarding.extra_install.extra_install_command`
...
def install_cursor_sdk() -> bool:
    ...
    visible. Mirrors
    :func:`~?agent_meow.onboarding.antigravity_auth.install_antigravity_sdk`.
...
def cursor_api_key_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`~?agent_meow.cli._save_global_config` (a shallow update, so
