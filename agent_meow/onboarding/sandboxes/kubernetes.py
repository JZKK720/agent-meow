...
# in :mod:`agent_meow.server.managed_hosts` (which validate config-sourced names);
...
def _validate_k8s_name_env(
    ...
    :param kind: ``"label"`` (namespace, �?3 chars, no dots) or ``"subdomain"``
        (Secret / ServiceAccount, dot-separated labels, �?53 chars).
...
def _token_secret_name(pod_name: str) -> str:
    ...
    :param pod_name: The Pod name (�?3 chars), so the ``-token`` suffix keeps
