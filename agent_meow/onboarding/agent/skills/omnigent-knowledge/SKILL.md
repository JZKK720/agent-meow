...
description: Deep reference on agent-meow config format, executor types, skill/tool structure, and conventions. Load when you need to look up how the platform works.
---
...
```
my-agent/
â”œâ”€â”€ config.yaml          # REQUIRED â€?agent spec
â”œâ”€â”€ AGENTS.md            # Recommended â€?instructions/personality
â”œâ”€â”€ skills/              # Optional â€?load-on-demand skills
â”?  â””â”€â”€ <skill-name>/
â”?      â””â”€â”€ SKILL.md
â”œâ”€â”€ tools/               # Optional â€?packaged tools
â”?  â”œâ”€â”€ python/          # Local Python tools (auto-discovered *.py)
â”?  â”œâ”€â”€ typescript/      # Local TypeScript tools (auto-discovered *.ts)
â”?  â””â”€â”€ mcp/             # MCP server declarations (*.yaml)
â””â”€â”€ agents/              # Optional â€?sub-agent directories (recursive)
    â””â”€â”€ <agent-name>/
        â”œâ”€â”€ config.yaml
        â””â”€â”€ ...
```
...
# Instructions â€?path to a file or inline text.
...
executor:
  ...
  # type: agents_sdk   â€?OpenAI Agents SDK, in-process
  # type: omnigent     â€?subprocess harness; requires config.harness below

  # Only for type: omnigent â€?pick the harness that runs the loop.
  ...
  # Model is OPTIONAL â€?omit to use the configured provider's default.
...
# os_env â€?grant filesystem/shell access (harness agents). Exposes
...
# guardrails â€?runtime policy gates (optional).
guardrails:
...
tools:
  ...
  # Built-in tools â€?string name or dict with config
  builtins:
...
**There is no `llm` executor type** â€?the only valid values are
`claude_sdk`, `agents_sdk`, and `omnigent`. For **most new simple agents**,
use `claude_sdk` (or `agents_sdk`) â€?in-process, no extra config. Use
...
|------------------|------------|
| `claude-native` (alias `claude`) | Claude Code â€?full coding tools, native permissions |
...
- Keep it focused â€?the model reads this on every turn
...
list â€?new tools may be added at any time.

**Tool recommendation guide:**

- "I want a research agent" â†?`web_search` + `web_fetch`
- "I want a coding agent" â†?`terminal_run` + `upload_file`
- "I want a data analysis agent" â†?`terminal_run` + `upload_file` + `download_file`
- "I want a conversational assistant" â†?no tools needed (or `web_search` for current info)
- "I want an agent that can access external APIs" â†?consider MCP servers (see below)

### MCP servers (external tool integrations)

MCP (Model Context Protocol) lets agents connect to external services â€?
...
- https://modelcontextprotocol.io â€?official MCP directory
- https://github.com/modelcontextprotocol â€?official GitHub org
...
separate tool â€?one file may export many tools. The decorator
...
- Decorate a **module-level** function â€?not a class method,
...
- Type hints on parameters drive the LLM-facing JSON schema. Use
  concrete types â€?`Any` and `object` produce permissive schemas
  with no validation.
...
- Pydantic `BaseModel` arguments are first-class â€?they get
...
This is the simplest valid agent â€?a name, an executor, and instructions.
...
`executor.type: omnigent` â€?that's what provides the spawn tools. Each
...
- **researcher** â€?searches the web for information
- **fact-checker** â€?verifies claims with evidence
...
delivers as a system message when ready â€?`check_task` polls,
