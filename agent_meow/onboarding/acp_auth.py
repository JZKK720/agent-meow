...
The generic ``acp`` harness (see :func:`agent_meow.runtime.workflow._build_acp_spawn_env`
and :mod:`agent_meow.inner.acp_harness`) drives *any* agent that speaks the Agent
...
:mod:`agent_meow.onboarding.cursor_auth`): the CLI orchestrates writes through
:func:`agent_meow.cli._save_global_config` so there is no cliâ†”onboarding cycle.
"""
...
class AcpAgentEntry:
    ...
    :param model: Optional model id (only honored by agents that accept a model
        in ``session/new``; see :class:`agent_meow.inner.acp_executor.AcpAgentConfig`).
