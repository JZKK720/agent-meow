...
:mod:`~?agent_meow.cli`; this module owns the look/feel and the
...
from agent_meow.onboarding.interactive import ACCENT, console
...
# see agent_meow.inner.banner._display_width.)
...
def key_providers() -> list[str]:
    ...
    :returns: Catalog provider names, e.g. ``["openai", "anthropic",
        "openrouter", ...]``.
    """
    from agent_meow.onboarding.providers import get_all_providers
...
def render_provider_listing_by_harness(
    ...
    :param providers: Parsed provider entries keyed by name (from
        :func:`~?agent_meow.onboarding.provider_config.load_providers`).
    :returns: None. Side effect: writes the listing to the shared console.
    """
    from agent_meow.onboarding.provider_config import provider_families
...
def render_provider_listing(
    ...
    Prints (via the shared :data:`~?agent_meow.onboarding.interactive.console`)
    ...
    :param detected: Ambient detections from
        :func:`~?agent_meow.onboarding.ambient.detect_providers`; entries
