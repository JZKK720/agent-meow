...
import agent_meow.onboarding.gemini_auth as _gemini_auth
from agent_meow._platform import resolve_cli_binary
from agent_meow.harness_aliases import HARNESS_ALIASES, canonicalize_harness
from agent_meow.harness_availability import (
    CODEX_CANONICAL_HARNESSES,
    HarnessAvailability,
)
from agent_meow.harness_plugins import harness_install_keys, valid_harnesses
...
# ``agent_meow.onboarding.gemini_auth.gemini_login_detected`` and have the patch
...
def _install_key(canonical: str) -> str:
    ...
    :returns: ``"anthropic"`` / ``"openai"`` for the claude/codex CLIs,
        :data:`~agent_meow.onboarding.harness_install.KIMI_KEY` for kimi,
        :data:`~agent_meow.onboarding.harness_install.OPENCODE_KEY` for
        opencode-native,
        :data:`~agent_meow.onboarding.harness_install.QWEN_KEY` for qwen, or
        :data:`~agent_meow.onboarding.harness_install.PI_KEY` for pi.
    """
...
def harness_is_configured(harness: str) -> bool:
    ...
    if canonical == "acp":
        ...
        try:
            from agent_meow.onboarding.acp_auth import acp_agents
    ...
    if canonical == CURSOR_KEY:
        ...
        # (:mod:`agent_meow.inner.cursor_executor`); gating here would only
...
def _harness_availability(canonical: str) -> HarnessAvailability:
    ...
    if _is_codex_family_harness(canonical):
        from agent_meow.codex_native import _codex_auth_unavailable_reason
