...
# :mod:`~?agent_meow.onboarding.gemini_auth`, not a ``gemini:`` family here).
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # (file-based, checked in :mod:`~?agent_meow.onboarding.gemini_auth`) and the
...
def _cli_config_serves_pi(entry: ProviderEntry) -> bool:
    ...
    natively, and :func:`~?agent_meow.pi_native_credentials._cli_config_pi_provider`
    ...
    The capability check lives in :mod:`~?agent_meow.pi_native_credentials` (the
    ...
    if entry.kind != CLI_CONFIG_KIND:
        return False
    from agent_meow.pi_native_credentials import cli_config_pi_provider_capable
