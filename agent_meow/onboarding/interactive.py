...
selection primitives used by ``agent-meow setup --no-internal-beta`` (and any
future configure noun). The look reproduces
:mod:`~?agent_meow.repl._theme_picker`:

- a bold accent header line,
- one row per option (selected 鈫?``鉂? <label>`` in bold accent, others
  normal weight; non-selectable sub-lines dim italic and indented beneath),
- a muted footer hint line (``鈫?鈫?move  路  Enter select  路  Esc back``),
...
reusable version. The duplication is intentional for now 鈥?a future
...
def clear_screen() -> None:
    ...
    # the cursor 鈥?together they wipe the leaked subprocess output.
...
def _render_menu(
    ...
    *,
    ...
    A bold accent header, one row per option (selected 鈫?bold accent with the
    ``鉂痐` pointer, others normal weight aligned under it), an optional dim
    ...
    ``default: 鈥` summary) with a blank line after 鈥?no ``鉂痐` pointer, and
    鈫?鈫?skip them so the cursor only lands on real choices.
    ...
    :param status: Optional transient status line (e.g. ``"鉁?added X"``)
        rendered green above the title. Being part of the frame, it is
        erased with the frame on ``clear_on_exit`` 鈥?so a re-rendering loop
    ...
    if win_start > 0:
        render_console.print(Text.from_markup(f"       [{MUTED}]鈫?{win_start} more[/]"))
    ...
    for i in range(win_start, win_end):
        ...
        if not selectable[i]:
            ...
            # "+N more" summary): indented, no pointer. 鈫?鈫?skip them. Their
            # styling follows the cursor 鈥?the label's own emphasis (e.g. a
            ...
            # real choice, or the menu ends) 鈥?so consecutive sub-lines stay
            # together and the gap separates one choice's block from the next.
            if i + 1 >= len(options) or selectable[i + 1]:
                render_console.print()
        elif i == selected:
            # Highlighted choice: bold accent with the 鉂?pointer.
            last_choice = i
            render_console.print(Text.from_markup(f"    [bold {ACCENT}]鉂? {label}[/]"))
        else:
    ...
    if win_end < n_options:
        render_console.print(Text.from_markup(f"       [{MUTED}]鈫?{n_options - win_end} more[/]"))
    ...
    hint = (
        "鈫?鈫?nav  路  Enter select  路  Esc exit"
        if compact
        else "鈫?鈫?move  路  Enter select  路  Esc back"
    )
...
def _step_selectable(selectable: list[bool], start: int, step: int) -> int:
    ...
    Skips non-selectable (header/separator) rows so 鈫?鈫?glide over them.
...
def _select_fallback(
    ...
    *,
    ...
    # Map each printed number 鈫?original options index; print headers as
    ...
    while True:
        ...
        # "q" is the fallback's abort, mirroring Esc on the TTY path 鈥?it
...
def select(
    ...
    *,
    ...
    On a TTY this draws the menu via raw termios (accent ``鉂痐` pointer,
    dimmed others, footer hints) and redraws in place on 鈫?鈫? Enter
    ...
    鈫?鈫?skip over (e.g. ``"Claude"`` / ``"Codex"`` labels above their
    ...
    :param selectable: Optional parallel ``bool`` mask; ``False`` marks a
        row as a non-selectable header/separator. ``None`` 鈫?all rows
        selectable.
    :param clear_on_exit: When ``True`` (TTY only), erase the rendered menu
        frame on return instead of leaving it in the scrollback 鈥?so a
    ...
    :param compact: When ``True`` (TTY only), render the dense top-level
        ...
        the footer reads ``nav 路 select 路 Esc exit`` (Esc exits rather
    ...
    :returns: The chosen zero-based index into *options* (always a
        selectable row), or ``-1`` when the user aborts 鈥?Esc / Ctrl-C /
    ...
    try:
        ...
        # Cannot enter raw mode 鈥?degrade to the numbered fallback.
        return _select_fallback(title, options, default=default, selectable=mask)
    ...
    try:
        ...
        while True:
            ...
            if ch in (b"\x03", b"\x04"):
                # Ctrl-C / Ctrl-D 鈥?abort the menu.
                cancelled = True
                break
            if ch == b"\x1b":
...
def prompt_text(
    ...
    *,
    ...
    if received_hidden_input and value:
        ...
        console.print(f"  [{MUTED}]鉁?received ({count} {unit})[/{MUTED}]")
