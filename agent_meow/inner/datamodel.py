...
# whose concrete types live in other modules (agent_meow.tools,
# agent_meow.policies, ...). Importing them here would create import cycles,
...
class History:
    ...
    def get_context_window(self, max_tokens: int | None = None) -> list[Message]:  # noqa: ARG002
        ...
        :mod:`~?agent_meow.runtime.compaction`, which operates on the
...
class OSEnvSandboxSpec:
    ...
    # :func:`~?agent_meow.inner.sandbox._default_sandbox_for_platform`,
    ...
    #   the rest of the tree first (see :mod:`~?agent_meow.inner._cwd_scan`).
    ...
    # see :data:`~?agent_meow.inner.os_env._DEFAULT_ENV_PASSTHROUGH`).
...
class AgentDef:
    ...
    :param instructions: Resolved system-prompt text loaded from
        ...
        See :func:`~?agent_meow.inner.loader.load_agent_def` for
