...
def _read_contained_file(root: Path, value: str) -> str | None:
    ...
    Mirrors :func:`~?agent_meow.spec.parser._read_contained_file`: resolves
...
def _resolve_instructions(
    ...
    Mirrors :func:`~?agent_meow.spec.parser._resolve_instructions`
...
def _effective_terminal_sandbox(
    ...
    :func:`~?agent_meow.inner.terminal.build_terminal_os_env_spec`: if the
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    # (``agent_meow.spec.parser._parse_credential_proxy``) rather than a
...
def load_agent_def_from_path(path_str: str) -> AgentDef:
    ...
    from agent_meow.spec import _find_omnigent_yaml_in_dir

    if path.is_dir():
        resolved_dir = path
        cleanup: tempfile.TemporaryDirectory[str] | None = None
    else:
        from agent_meow.spec.tar_utils import extract_safe
