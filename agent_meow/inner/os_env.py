...
from agent_meow._platform import IS_WINDOWS, WINDOWS_ENV_PASSTHROUGH
...
_DEFAULT_ENV_PASSTHROUGH: tuple[str, ...] = (
    ...
    "PYTHONFAULTHANDLER",
    # agent-meow session marker: always pass the "inside agent-meow" marker
...
def build_helper_env(
    ...
    (:data:`~?agent_meow.runner.identity.RUNNER_AUTH_SECRET_ENV_VARS`): the
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        raise SystemExit("usage: python -m agent_meow.inner.os_env <helper|launch> ...")
    ...
    if command == "helper":
        ...
        if len(args) != 3 or args[1] not in ("--config-fd", "--config-file"):
            raise SystemExit(
                "usage: python -m agent_meow.inner.os_env helper "
    ...
    if command == "launch":
        if len(args) < 3:
            raise SystemExit(
                "usage: python -m agent_meow.inner.os_env launch <sandbox> <target> [args...]"
            )
