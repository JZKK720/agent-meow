...
from agent_meow.llms.errors import is_context_length_exceeded as _is_context_length_exceeded
from agent_meow.reasoning_effort import OPENAI_AGENTS_EFFORTS, validate_effort
...
# Tool executor callable wired in by ``agent_meow.Session``. The result
# is JSON-ish (dict[str, Any]) but the static type leaks ``Any`` through
# the kwargs dict. Constrained here to match agent_meow.executor.
...
def _normalize_responses_items_for_chat(
    ...
    :func:`~?agent_meow.inner.open_responses_sdk._convert_messages_to_responses`
...
def _normalize_content_blocks_for_chat(
    ...
    Background: :mod:`~?agent_meow.runtime.content_resolver` resolves uploaded
