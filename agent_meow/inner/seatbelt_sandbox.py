...
from the resolved :class:`~?agent_meow.inner.sandbox.SandboxPolicy`.
...
This is the **macOS parity** for :mod:`~?agent_meow.inner.bwrap_sandbox`.
...
- The same egress filtering pipeline: parent-side
  :class:`~?agent_meow.inner.egress.proxy.EgressProxy`, Unix socket
  bridge in the scratch tmpdir, in-helper TCPâ†’Unix relay via
  :func:`~?agent_meow.inner.egress.relay.start_relay`, and
...
- The per-helper scratch tmpdir (added by the spawn site via
  :func:`~?agent_meow.inner.sandbox.with_additional_write_roots`) is
...
# above the cwd-scan cap (50k entries Ã— ~100 bytes/rule worst case)
...
class SeatbeltSandboxBackend(SandboxBackend):
    ...
    def wrap_launcher_argv(
        ...
        :param policy: The :class:`SandboxPolicy` produced by
            ...
            :func:`~?agent_meow.inner.sandbox.with_additional_write_roots`
...
def _build_profile(
    ...
    *,
    ...
    # ``agent_meow.inner.os_env`` even starts. The bwrap backend
...
def _seatbelt_safe_roots(
    ...
    Mirrors :func:`~?agent_meow.inner.bwrap_sandbox._bwrap_safe_roots`
...
def _interpreter_safe_roots(argv: Sequence[str]) -> list[Path]:
    ...
    Mirrors :func:`~?agent_meow.inner.bwrap_sandbox._interpreter_safe_roots`:
...
def _scratch_tmpdir(write_roots: list[Path]) -> Path | None:
    ...
    :func:`~?agent_meow.inner.sandbox.create_private_tmpdir` returns a
    path under ``tempfile.gettempdir()`` (typically ``/var/folders/.../T``
    on macOS). The helper-spawn site adds it to ``write_roots`` via
    :func:`~?agent_meow.inner.sandbox.with_additional_write_roots`, so
