...
from agent_meow.runner.identity import RUNNER_AUTH_SECRET_ENV_VARS
...
class SandboxPolicy:
    ...
    :param env_passthrough: User-declared environment-variable names
        ...
        (:data:`agent_meow.inner.os_env._DEFAULT_ENV_PASSTHROUGH`).
        Carried on the policy so every backend that spawns the
        helper applies the same allowlist; the parent-side
        :func:`agent_meow.inner.os_env.build_helper_env` reads
    ...
    dropped —see :func:`agent_meow.inner.os_env._HelperProcessClient.
...
class SandboxBackend(ABC):
    ...
    def wrap_launcher_argv(
        ...
        Called by :class:`agent_meow.inner.os_env._HelperProcessClient`
        ...
        :param argv: The unwrapped command argv the parent intends to
            ...
            "agent_meow.inner.os_env", "helper", "<encoded>"]``.
    ...
    def post_spawn(self, policy: SandboxPolicy, pid: int) -> ContainmentHandle | None:
        ...
        Called by :class:`agent_meow.inner.os_env._HelperProcessClient` on
...
def _resolve_grant_root(cwd: Path, root: str) -> Path:
    ...
    :func:`agent_meow.inner.bwrap_sandbox._resolve_root`). Relative entries
    ...
    *argv* and returns its exit code.
    ...
    if (
        ...
        try:
            ...
            # ``agent_meow.inner.sandbox`` imports succeed even when
            ...
            inline = (
                ...
                "from agent_meow.inner.sandbox import run_launcher; "
...
def get_backend(type_name: str) -> SandboxBackend:
    ...
    (:meth:`agent_meow.inner.os_env._HelperProcessClient._start_locked`)
...
def _default_sandbox_for_platform() -> OSEnvSandboxSpec:
    ...
    :meth:`agent_meow.inner.bwrap_sandbox.BwrapSandboxBackend.resolve`,
