...
:mod:`agent_meow.claude_model_vocabulary`): the Claude-native hook runs as
a per-spawn subprocess (``python -I -m
agent_meow.inner.hook_scripts.claude_router_hook``) and blocks the spawn,
...
Every ``explicit-any`` type-ignore below marks the same thing 鈥?hook
payloads, request bodies and hook outputs are untrusted JSON with no
schema this process can import 鈥?so the per-site justifications are
omitted.
"""
...
from agent_meow.claude_model_vocabulary import (
...
# so both are matched. Also used verbatim as the settings/SDK matcher 鈥?# Claude Code reads a pipe-separated list as exact alternatives.
...
# ``agent_meow.runner.subagent_routing``: larger than the runner's own wait so
...
# ``agent_meow.server.smart_routing.ROUTING_REQUEST_TIMEOUT_S``. A tighter budget
# discards verdicts that did arrive.
REQUEST_TIMEOUT_S = 15.0

# Headroom hop 1 adds over hop 2. Enough for interpreter start-up and the
# script's fail-open branch, and no more 鈥?every second here is a second the
...
# than ``REQUEST_TIMEOUT_S`` 鈥?a hook killed at the same instant its HTTP call
...
def _advertiser_alive(pid: Any) -> bool:  # type: ignore[explicit-any]
    ...
    the advertisement was not written by us 鈥?rejected rather than
...
def claude_requested_model_resolver(
    ...
    :returns: Callable mapping a spawn's ``model`` to a catalog id, or
        ...
        ``"default"``, an unpinned alias) 鈥?the body then claims no ask.
    """
...
def mcp_tool_name(bare: str, harness: str | None) -> str:
    ...
    :param harness: Requesting harness, e.g. ``"claude-native"``. ``None``
        or an unrecognized value keeps the bare name 鈥?an invented prefix
...
def _mcp_discovery_note(harness: str | None) -> str:
    ...
    if harness in _MCP_NAMESPACED_HARNESSES:
        return (
            ...
            "鈥?call them by the bare names above with the server as the namespace, and "
...
def _no_spawn_tool_instruction(model: str, picked: str) -> str:
    ...
    return (
        ...
        "approved 鈥?do the work yourself on your current model instead of spawning, "
        "then continue."
    )
...
def routed_spawn_instruction(
    ...
    *,
    ...
    Names ``sys_session_create`` 鈥?the tool a routed harness actually holds
    (``spawn: True`` grants it) 鈥?with parameters that exist on its schema,
    ...
    ``mcp__agent_meow__sys_session_create``) reported the tool as nonexistent
    ...
    return (
        ...
        f"and the sub-task is approved 鈥?start it with {create} "
...
def smart_routing_spawn_note(harness: str) -> str:
    ...
    reason name the same tool in the same spelling 鈥?a session told about
    ...
    return (
        ...
        "that is an approved re-route, not an error or a permission problem 鈥?make "
...
def decision_to_hook_output(
    ...
    *,
    ...
    :param model_translator: Converts the decision's servable model id
        ...
        allowed unchanged 鈥?a degraded model beats a dead spawn, and an
    ...
    if action == "redirect":
        ...
        # A redirect without a target can't be followed 鈥?fail open.
        return None
    if action == "deny":
...
def route_pre_tool_use(
    ...
    *,
    ...
    :returns: Hook output, or ``None`` for "no opinion" 鈥?every failure
