...
(:func:`~?agent_meow.antigravity_native_bridge.inject_user_message_via_tui`), exactly
...
from agent_meow.reasoning_effort import ANTIGRAVITY_EFFORTS, validate_effort_or_llm_error
...
class AntigravityNativeExecutor(Executor):
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        (:func:`~?agent_meow.antigravity_native_rpc.cancel_cascade_steps`).
        ...
        .. note:: **Scope — RUNNING cascades only (live-verified, C3).**
           ...
           **NO-OP on a step that is WAITING for a user interaction**
           ...
           (:mod:`~?agent_meow.antigravity_native_interactions`), NOT here — this
    ...
    async def run_turn(
        ...
        (:func:`~?agent_meow.antigravity_native_rpc.send_user_cascade_message`), which
    ...
    async def _deliver(self, text: str) -> str | None:
        ...
        :func:`~?agent_meow.antigravity_native_bridge.inject_user_message_via_tui`)
        ...
        discovers/binds — see :mod:`~?agent_meow.antigravity_native_reader`).
...
def _latest_requested_model(steps: list[dict[str, object]]) -> str | None:
    ...
    field :func:`~?agent_meow.antigravity_native_rpc.send_user_cascade_message` sends
...
def _recommended_model(catalog: dict[str, object]) -> str | None:
    ...
    :param catalog: The parsed ``GetAvailableModels`` response as returned by
        :func:`~?agent_meow.antigravity_native_rpc.get_available_models`.
...
def _content_to_text(content: EnqueuedContent, bridge_dir: Path) -> str:
    ...
    if isinstance(content, list):
        from agent_meow.inner.native_attachments import materialize_attachment
