...
around a :class:`~?agent_meow.inner.claude_sdk_executor.ClaudeSDKExecutor`
...
# Set by the agent-meow workflow layer when executor.auth: {type: api_key, …} is
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing — agent-meow omits the env var when ``llm.retry`` matches
...
def _build_claude_sdk_executor() -> Executor:
    ...
    return ClaudeSDKExecutor(
        ...
        # AGENT_MEOW_RUNNER_WORKSPACE (the folder the user launched in, and
