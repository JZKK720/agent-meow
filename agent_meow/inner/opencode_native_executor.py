"""Executor that bridges agent-meow web turns into a native OpenCode session.

Built on :class:`~?agent_meow.native_server_harness.NativeServerHarness`: the
runner owns the ``opencode serve`` process + SSE forwarder, and this
executor injects the latest web turn over the
:class:`~?agent_meow.opencode_http_transport.OpenCodeHttpTransport` using the
...
from agent_meow.opencode_http_transport import OpenCodeHttpTransport
