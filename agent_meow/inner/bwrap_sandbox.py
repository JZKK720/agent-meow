...
:func:`~?agent_meow.inner.sandbox._default_sandbox_for_platform`); spec
...
- The per-helper scratch tmpdir created by
  :func:`~?agent_meow.inner.sandbox.create_private_tmpdir` is
...
1. :data:`~?agent_meow.inner._seccomp.BASELINE_DENYLIST_SYSCALLS` — the
...
class BwrapSandboxBackend(SandboxBackend):
    ...
    def wrap_launcher_argv(
        ...
        :func:`~?agent_meow.inner.sandbox.run_launcher`.
        """
    ...
    def activate(self, policy: SandboxPolicy) -> None:
        ...
        1. The shared k8s-derived baseline via
           :func:`~?agent_meow.inner._seccomp.apply_baseline_denylist`.
...
def _bwrap_extra_seccomp_rules() -> list[SeccompRule]:
    ...
    on top of :func:`~?agent_meow.inner._seccomp.apply_baseline_denylist`.
...
def _resolve_root(cwd: Path, root: str) -> Path:
    ...
    See :data:`~?agent_meow.inner.seatbelt_sandbox._resolve_root`
...
def _dotfile_and_symlink_mask_args(
    ...
    Thin emitter over :func:`~?agent_meow.inner._cwd_scan.scan_cwd_mask_entries`:
    ...
    See :mod:`~?agent_meow.inner._cwd_scan` for the masking-decision
