...
* keep ``agent_meow.repl._resume_picker`` decoupled from the
  ``agent_meow.claude_native`` import graph (which pulls in tmux /
...
# ``labels["agent_meow.ui"] == "terminal"``). Stamped at creation for the
