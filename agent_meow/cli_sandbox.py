...
``agent-meow sandbox …`` bootstraps an agent-meow host inside a sandbox
...
:mod:`~?agent_meow.onboarding.sandboxes`.
"""
...
from agent_meow.onboarding.sandboxes import (
    SandboxLauncher,
    available_providers,
    get_launcher,
)
...
def _omnigent_repo_root() -> Path:
    ...
    1. Walk up from the current working directory looking for a parent
       ...
       This succeeds when the user runs ``agent-meow sandbox …`` from
       inside a checkout.
    ...
    # ``cli_sandbox.py`` lives at ``<repo>/agent_meow/cli_sandbox.py``;
    # parents[1] is the repo root that hosts ``sdks/`` and
    # ``agent_meow/``. Editable installs (``pip install -e .``) point
...
def sandbox_auth(
    ...
    """
    from agent_meow.onboarding.sandboxes import derive_workspace, login_app_oauth_in_sandbox
...
def sandbox_connect(
    ...
    Runs ``agent-meow host --server <url>`` inside the sandbox — the
    ...
    """
    from agent_meow.onboarding.sandboxes import connect_sandbox_host, derive_workspace
...
def lakebox(ctx: click.Context) -> None:
    """
    Alias for ``agent-meow sandbox … --provider lakebox``.
    ...
    # `agent-meow lakebox <sub>` ≡ `agent-meow sandbox <sub> --provider
