...
- **In-process runner**: the server passes
  :func:`~?agent_meow.runtime.get_terminal_registry` into
  :func:`~?agent_meow.runner.create_runner_app`, so the registry is
...
:func:`~?agent_meow.runtime.set_runner_ws_factory` in the server
...
from agent_meow.server.auth import LEVEL_OWNER, LEVEL_READ, AuthProvider
from agent_meow.server.routes._auth_helpers import require_access
...
def create_terminal_attach_router(
    *,
    ...
    :func:`~?agent_meow.server.app.create_app`. The list/CRUD endpoints
