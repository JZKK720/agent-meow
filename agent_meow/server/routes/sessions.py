...
from agent_meow.db.utils import generate_agent_id, generate_task_id
...
from agent_meow.model_override import model_family_mismatch, validate_model_override
...
from agent_meow.policies.types import (
    ElicitationRequest,
    EvaluationContext,
    PolicyAction,
    PolicyResult,
)
from agent_meow.reasoning_effort import (
    EFFORT_CLEAR_VALUES,
    EFFORT_VALUES,
    validate_effort,
)
...
from agent_meow.runner.routing import RunnerRouter
...
from agent_meow.runtime.policies.builder import build_policy_engine, load_session_usage
...
from agent_meow.server.bundles import bundle_location, validate_agent_bundle
from agent_meow.server.host_registry import HostConnection, HostRegistry, RunnerExitReports
...
from agent_meow.server.permissions import check_session_access
...
from agent_meow.server.routes._codex_elicitation import parse_codex_elicitation_request
from agent_meow.server.routes._content_type import (
    require_json_content_type,
    require_json_or_multipart_content_type,
)
from agent_meow.server.routes._host_worktree import CreatedWorktree
...
from agent_meow.stores import AgentStore, ConversationStore
...
from agent_meow.stores.host_store import Host, HostStore
from agent_meow.stores.permission_store import PermissionStore
from agent_meow.tools.client_specified import parse_client_side_tool_specs
...
# sub-agents from agent_meow-spawned ones at the data layer.
_CLAUDE_NATIVE_SUBAGENT_WRAPPER_LABEL_VALUE = "claude-code-native-ui-subagent"
_CLAUDE_NATIVE_SUBAGENT_ID_LABEL_KEY = "agent_meow.claude_native.subagent_id"
...
_CODEX_NATIVE_SUBAGENT_THREAD_ID_LABEL_KEY = "agent_meow.codex_native.subagent_thread_id"
_CODEX_NATIVE_SUBAGENT_PARENT_THREAD_ID_LABEL_KEY = "agent_meow.codex_native.parent_thread_id"
_CODEX_NATIVE_SUBAGENT_TOOL_CALL_ID_LABEL_KEY = "agent_meow.codex_native.collab_tool_call_id"
_CODEX_NATIVE_SUBAGENT_PROMPT_LABEL_KEY = "agent_meow.codex_native.prompt"
_CODEX_NATIVE_SUBAGENT_NICKNAME_LABEL_KEY = "agent_meow.codex_native.agent_nickname"
_CODEX_NATIVE_SUBAGENT_ROLE_LABEL_KEY = "agent_meow.codex_native.agent_role"
# Current Codex collaboration mode kind (``"plan"`` or ``"default"``)
# mirrored from app-server ``thread/settings/updated``.
_CODEX_NATIVE_COLLABORATION_MODE_LABEL_KEY = "agent_meow.codex_native.collaboration_mode"
...
_LAST_CONTEXT_TOKENS_LABEL_KEY: str = "agent_meow.last_context_tokens"
_LAST_CONTEXT_WINDOW_LABEL_KEY: str = "agent_meow.last_context_window"
...
_LAST_TASK_ERROR_CODE_LABEL_KEY: str = "agent_meow.last_task_error_code"
_LAST_TASK_ERROR_MESSAGE_LABEL_KEY: str = "agent_meow.last_task_error_message"
...
def _publish_and_persist_resource_event(
    ...
    """
    from agent_meow.entities.conversation import ResourceEventData
...
def _permission_level_from_grants(
    ...
    Mirrors :func:`~?agent_meow.server.routes._auth_helpers._get_permission_level_sync`
...
def _owner_from_grants(grants: list[SessionPermission]) -> str | None:
    ...
    Mirrors :func:`~?agent_meow.server.routes._auth_helpers.get_session_owner_id`
    but operates on grants already held in memory so callers can batch the
    permission-store query across many sessions at once.
...
def _resolve_llm_model(conv: Conversation | None) -> str | None:
    ...
    try:
        ...
        from agent_meow.runtime._globals import _agent_store
    ...
    **only when the turn is priced** (catalog pricing available or a
    ...
    if llm_model:
        if has_provider_cost:
            ...
            from agent_meow.llms.context_window import compute_llm_cost, fetch_model_pricing
...
async def _get_runner_client(
    ...
    """
    from agent_meow.runtime import get_runner_client
...
async def _provision_managed_sandbox(
    *,
    ...
    """
    from agent_meow.server.managed_hosts import launch_managed_host, relaunch_managed_host
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    :param tunnel_registry: Runner-tunnel registry used to await the
        ...
        wirings (the rendezvous then settles at frame-send).
    """
    from agent_meow.server.managed_hosts import terminate_managed_host
...
async def _run_managed_wake(
    *,
    ...
    """
    from agent_meow.server.managed_hosts import resume_managed_host
...
async def _get_runner_client_for_resource_access(
    ...
    """
    from agent_meow.runtime import get_runner_client, get_runner_router
...
async def _forward_session_change_to_runner(
    ...
    * Explicit ``compact`` — the caller inspects the returned status
    ...
    skipped — the persisted value on the agent-meow side is the authoritative
    ...
    in the agent-meow log — otherwise the POST succeeds at the httpx layer
...
async def _stop_session_host_runner(
    ...
    if conn is None:
        ...
        return
    from agent_meow.host.frames import HostStopRunnerFrame, encode_host_frame
...
async def _forward_event_to_runner(
    ...
    if effective_runner_override is None and _routing_enabled and body.type == "message":
        from agent_meow.server.smart_routing import route_turn
...
async def _dispatch_session_event_to_runner(
    ...
    * **All other cases**: persist the item AP-side (invariant I1:
    ...
    :mod:`~?agent_meow.runtime.pending_inputs`). It is rolled back if the
    ...
    :param created_by: Authenticated identity of the posting actor,
        ...
        :func:`~?agent_meow.runtime.pending_inputs.record` and applied
...
def _agent_provider_family(agent: Agent) -> str | None:
    ...
    """
    from agent_meow.onboarding.provider_config import provider_family_for_harness
...
def configure_subagent_block_notifier(
    ...
    :mod:`~?agent_meow.runtime.pending_elicitations` so a sub-agent that
    ...
    """
    from agent_meow.runtime import pending_elicitations as _pending_elicitations
    from agent_meow.runtime.subagent_block_notifier import SubagentBlockNotifier
...
async def _create_session_worktree(
    *,
    ...
    :raises OmnigentError: ``invalid_input`` for a bad branch name,
        ...
        is configured.
    """
    from agent_meow.host.git_worktree import WorktreeError, validate_branch_name
...
def _child_session_summary_from_conversation(
    ...
    :func:`~?agent_meow.tools.builtins.spawn._spawn_one`, plus the
...
async def _handle_advise_models_mcp(
    ...
    *,
    ...
    from agent_meow.server.smart_routing import fetch_runner_models, infer_models
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The agent-meow server's role here is routing only —
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    if runner_client is None:
        return _mcp_error_response(rpc_id, -32000, f"No runner bound for session {session_id!r}")
    try:
        from agent_meow.runner.tool_dispatch import MCP_PROXY_FORWARD_TIMEOUT_S
...
def create_sessions_router(
    ...
    async def create_session(
        ...
        if launch_host_id is not None and resp.runner_id is None:
            ...
            if host_registry is not None and host_store_inst is not None:
                ...
                from agent_meow.runner.identity import token_bound_runner_id
                from agent_meow.server.routes._host_launch import resolve_host_launch
    ...
    from agent_meow.server.routes.codex.sessions import register_codex_session_routes
    ...
    async def update_session(
        ...
        # dispatch — agent-meow server stays harness-agnostic; the runner
    ...
    async def fork_session(
        ...
        # the copied agent-meow transcript on its own — mark the fork so the
    ...
    async def evaluate_policy(
        ...
        command hooks (via ``agent_meow.claude_native_hook``) to
    ...
    async def antigravity_elicitation_request_hook(
        ...
        :class:`~?agent_meow.server.schemas.ElicitationResult` so the bridge
        ...
        :class:`~?agent_meow.server.schemas.ElicitationRequestParams` dict.
    ...
    async def get_elicitation(
        ...
        :mod:`~?agent_meow.runtime.pending_elicitations` index — no
    ...
    def _policy_description(spec: PolicySpec) -> str | None:
        ...
        if isinstance(spec, FunctionPolicySpec) and spec.function:
            from agent_meow.policies.registry import get_entry
...
async def _fetch_model_options(
    ...
    * **codex-native** — a *live*, account-scoped catalog only the bound runner
    ...
    if wrapper == _CURSOR_NATIVE_WRAPPER_LABEL_VALUE:
        from agent_meow.cursor_native import cursor_base_model_options

        return cursor_base_model_options()
    if wrapper == _KIRO_NATIVE_WRAPPER_LABEL_VALUE:
        from agent_meow.kiro_native import kiro_base_model_options
