...
from agent_meow.db.db_models import InvalidUuidError, uuid_to_bytes
...
from agent_meow.host.identity import MANAGED_HOST_TOKEN_HEADER
...
from agent_meow.server.host_registry import (
    HostConnection,
    HostRegistry,
    RunnerExitReports,
)
...
def create_host_tunnel_router(
    ...
    *,
    ...
    async def tunnel(ws: WebSocket, host_id: str) -> None:
        ...
        # bypass the app's StatementError�?04 handler.
        try:
...
async def _ping_loop(
    ...
    (:data:`agent_meow.stores.host_store.HOST_LIVENESS_TTL_S`): when a
