"""Which router answers a routing call 鈥?the AI Gateway's, or the built-in judge's.
...
(:class:`~agent_meow.server.smart_routing.LLMRoutingClient`) names models from the
...
if TYPE_CHECKING:
    ...
    from agent_meow.server.smart_routing import RoutingClient, RoutingResult
...
class RoutingBackends:
    ...
    Both may be configured at once 鈥?that is the normal Databricks posture, and
...
def usable_external(backends: RoutingBackends) -> RoutingClient | None:
    ...
    as "no external router" from then on 鈥?the deployment's real posture for
...
def select_router(
    ...
    *,
    ...
    :param gateway_backed: Whether EVERY harness family involved in this call
        ...
        out of play 鈥?its picks are gateway catalog ids the pane cannot reach.
...
async def route_with_fallback(
    ...
    *,
    ...
    The external service is still preferred wherever it can serve 鈥?that is the
    ...
    :param available_models: Harness id 鈫?candidate model ids.
    ...
    :raises Exception: Whatever the client that answered last raised 鈥?the
    ...
    try:
        result = await choice.client.route(message, available_models)
    except Exception:  # noqa: BLE001 鈥?the judge is the fallback for any failure
        _logger.warning(
...
def reported_gateway_inference(
    ...
    Read from this replica's live :class:`~agent_meow.server.host_registry.HostRegistry`,
    which the host fills on its connect handshake 鈥?nothing about gateway
    ...
    if not isinstance(host_id, str):
        return None
    from agent_meow.server.routes._sessions.common import get_server_host_registry
...
def gateway_backs_all(
    ...
    """
    from agent_meow.gateway_inference import not_gateway_backed
...
def backends_from_caps(caps: Any) -> RoutingBackends:  # type: ignore[explicit-any]  # RuntimeCaps-shaped
    ...
    Prefers the explicit :attr:`~agent_meow.runtime.caps.RuntimeCaps.routing_backends`.
    Absent it, the legacy single ``routing_client`` is classified by type 鈥?an
    :class:`~agent_meow.server.smart_routing.ExternalRoutingClient` is the external
...
def _managed_llm_capability(caps: Any) -> bool:  # type: ignore[explicit-any]  # RuntimeCaps-shaped
    ...
    :class:`~agent_meow.server.smart_routing.RoutingClient` later, so routing is
...
def routing_sources(caps: Any) -> dict[str, bool]:  # type: ignore[explicit-any]  # RuntimeCaps
    ...
    decline. Nothing about that is persisted 鈥?a restart re-probes.

    :param caps: A ``RuntimeCaps`` (or structural equivalent), or ``None``.
    :returns: ``{"external": ..., "oss": ...}`` 鈥?``"external"`` is the
...
def routing_available(caps: Any) -> bool:  # type: ignore[explicit-any]  # RuntimeCaps-shaped
    ...
    answer", so it can never disagree with :func:`routing_sources` 鈥?the drift
