...
# agent_meow.resources.examples (see pyproject package-data), so they resolve
...
def _normalize_tarinfo(tarinfo: tarfile.TarInfo) -> tarfile.TarInfo:
    ...
    ``agent_meow.spec.tar_utils.extract_safe``), so on-disk permissions
    don't depend on it.
...
def _build_opencode_native_bundle() -> bytes:
    ...
    from agent_meow.opencode_native import _materialize_opencode_agent_spec
    from agent_meow.spec import materialize_bundle
...
def _build_pi_native_bundle() -> bytes:
    ...
    from agent_meow.pi_native import _materialize_pi_agent_spec
    from agent_meow.spec import materialize_bundle
...
def _build_antigravity_native_bundle() -> bytes:
    ...
    from agent_meow.antigravity_native import _materialize_antigravity_agent_spec
    from agent_meow.spec import materialize_bundle
...
def _build_qwen_native_bundle() -> bytes:
    ...
    from agent_meow.qwen_native import _materialize_qwen_agent_spec
    from agent_meow.spec import materialize_bundle
...
def _build_kimi_native_bundle() -> bytes:
    ...
    from agent_meow.kimi_native import _materialize_kimi_agent_spec
    from agent_meow.spec import materialize_bundle
...
def create_app(
    ...
    if auth_provider is not None and account_store is not None:
        ...
        if isinstance(auth_provider, UnifiedAuthProvider) and auth_provider._source == "accounts":
            from agent_meow.server.accounts_bootstrap import bootstrap_admin
    ...
    async def _lifespan(
        ...
        (see ``designs/SERVER_HARNESS_CONTRACT.md`` Â§Process
        management).
        ...
        ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.4.
        ...
        # ``pending_elicitations.record_publish`` to post a ``[System: â€¦]``
        ...
        from agent_meow.runtime import set_resource_registry
        ...
        from agent_meow.server._runner_ws_tunnel import make_tunnel_ws_factory
        ...
        from agent_meow.policies.registry import load_registry
    ...
    async def info() -> dict[str, bool | str | list[str] | None]:
        ...
        databricks_features = (
            importlib.util.find_spec("agent_meow.onboarding.sandboxes.lakebox") is not None
        )
        ...
        try:
            from agent_meow.runtime._globals import _caps
    ...
    # WS tunnel endpoint for runners (RUNNER.md Â§2-3).
    app.include_router(
