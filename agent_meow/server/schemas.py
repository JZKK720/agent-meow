...
class ChildSessionSummary(BaseModel):
    ...
    :param title: Sub-agent title, ``"{agent_type}:{session_name}"``
        as written by :func:`~?agent_meow.tools.builtins.spawn._spawn_one`,
    ...
    :param pending_elicitations_count: Number of approval / input
        ...
        server's :mod:`~?agent_meow.runtime.pending_elicitations`
...
class SessionCreateRequest(BaseModel):
    ...
    def _check_managed_host_fields(self) -> SessionCreateRequest:
        ...
        from agent_meow.server.managed_hosts import is_repo_workspace, parse_repo_workspace
...
class SessionResponse(BaseModel):
    ...
    :param pending_elicitations: Outstanding approval prompts on
        ...
        :mod:`~?agent_meow.runtime.pending_elicitations` index.
    :param pending_inputs: Un-consumed web-composer user messages on
        ...
        :mod:`~?agent_meow.runtime.pending_inputs` index.
    ...
    # Source: :mod:`~?agent_meow.runtime.pending_inputs`.
...
class SessionCollaborationModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``agent_meow.codex_native.collaboration_mode`` on the conversation labels,
...
class SessionInputConsumedPayload(BaseModel):
    ...
    :param type: The item type discriminator — ``"message"`` for
        ...
        results, etc. Mirrors
        :class:`~?agent_meow.server.schemas.SessionEventInput`'s
        ``type`` field.
    ...
    :param cleared_pending_id: When this consumed message drains a
        :mod:`~?agent_meow.runtime.pending_inputs` entry (a native-
...
class ElicitationRequestParams(BaseModel):
    ...
    # ``elicitation/create`` traversing harness → agent-meow → client)
...
class ElicitationRequestEvent(_SSEEventBase):
    ...
    ``type == "approval"`` and
    :class:`~?agent_meow.server.schemas.ElicitationResult` fields in
...
class CreatedEvent(_SSEEventBase):
    ...
    :class:`~?agent_meow.server.schemas.ResponseObject` (status will
...
class CompletedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`~?agent_meow.server.schemas.ResponseObject`.
...
class FailedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`~?agent_meow.server.schemas.ResponseObject` whose
...
def is_known_event(name: str) -> bool:
    ...
    patch :func:`~?agent_meow.runtime.session_stream.publish` to
