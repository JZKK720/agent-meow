...
|---|---|---|
| Agent Management (`/api/agents`) | agent-meow (ours) | No external reference —this is our own API. |
...
|---|---|---|
| Sessions (`/v1/sessions`) | agent-meow (ours) | No external reference. Purpose-built for agent-native workflows: live tail, queued input, interrupt. |
...
Fields:
  ...
  terminal_pending (boolean, default `false`)
    ...
    resource exists. Set by two paths: (1) directly by the agent-meow server at
  ...
  type (string, required)
    Event/input discriminator. Recognized values:
      ...
      - "compact"               —explicit context compaction. Forwarded
                                  ...
                                  its own context) and the agent-meow server
...
- `metadata` on sessions / tasks —caller-attached key-value pairs (max 16 keys, keys �?4 chars,
  values �?12 chars).
