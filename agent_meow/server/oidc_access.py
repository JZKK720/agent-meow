...
1. **Domain allowlist.** The email's domain is in the effective
   ...
   frozen at startup in :class:`~?agent_meow.server.oidc.OIDCConfig`)
...
3. **Individual invite.** The email was pre-authorized via an opt-in
   ...
   :mod:`~?agent_meow.server.accounts_store` /
...
from agent_meow.server.admin_list import AdminList, MtimeCachedIdentitySet, resolve_data_dir
...
class InvitedEmailLookup(Protocol):
    ...
    Implemented by
    :class:`~?agent_meow.server.accounts_store.SqlAlchemyAccountStore`
