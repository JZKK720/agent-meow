...
The server's :mod:`~?agent_meow.server.routes.terminal_attach` route
...
def make_tunnel_ws_factory(
    ...
    Install via :func:`~?agent_meow.runtime.set_runner_ws_factory`. The
    factory shape (callable ``(runner_path) -> async-context``) is
    what :mod:`~?agent_meow.server.routes.terminal_attach` already
    expects.
...
class _TunneledWSConn:
    ...
    :func:`~?agent_meow.server.routes.terminal_attach._shuttle_ws_frames`
