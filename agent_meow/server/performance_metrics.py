...
from agent_meow.process_logging import log_record_display_fields
...
class RequestDurationAccessFormatter(AccessFormatter):
    """
    Uvicorn access formatter that appends agent-meow request duration.
...
def _create_request_otel_instruments(meter: MeterLike) -> _RequestOtelInstruments:
    ...
    return _RequestOtelInstruments(
        started=meter.create_counter(
            "agent_meow.server.http.requests.started",
            unit="{request}",
            description="HTTP requests started by the agent-meow server.",
        ),
        completed=meter.create_counter(
            "agent_meow.server.http.requests.completed",
            unit="{request}",
            description="HTTP requests completed by the agent-meow server.",
        ),
        failed=meter.create_counter(
            "agent_meow.server.http.requests.failed",
            unit="{request}",
...
def _create_processing_otel_instruments(
    ...
    return _ProcessingOtelInstruments(
        ...
        avg_30s_ms=meter.create_gauge(
            "agent_meow.server.http.request.processing.avg_30s",
            unit="ms",
            description=(
