...
:class:`~?agent_meow.runner.mcp_manager.RunnerMcpManager` but used by the
``POST /v1/sessions/{session_id}/mcp`` MCP-proxy endpoint to proxy calls
with policy enforcement.

Reuses :class:`~?agent_meow.tools.mcp.McpServerConnection` for transport
and :func:`~?agent_meow.runner.mcp_manager.compute_spec_hash` for stable
pool keying.
"""
...
from agent_meow.runner.mcp_manager import (  # noqa: F401 — McpSchemasResult re-exported for callers
    McpSchemasResult,
    compute_spec_hash,
)
from agent_meow.spec.types import AgentSpec, MCPServerConfig
from agent_meow.tools.base import is_valid_tool_name
from agent_meow.tools.mcp import McpServerConnection
...
class ServerMcpPool:
    ...
    async def call_tool(
        ...
        :returns: Tool result as a string (same shape as
            :meth:`~?agent_meow.tools.mcp.McpServerConnection.call_tool`).
