...
(:mod:`agent_meow.server.routes.dictation`) with an on-server recognizer
...
def _hermes_stt_url() -> str | None:
    """Return the Hermes STT URL (or None when not configured)."""
...
class _HermesStream:
    ...
    def _send(self) -> None:
        ...
        # Send the Hermes key for auth (STT always routes to Hermes now).
