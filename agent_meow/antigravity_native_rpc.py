...
RPC read driver (:mod:`~?agent_meow.antigravity_native_reader`, which polls/streams
trajectory steps to mirror agy's conversation) and the native executor
(:mod:`~?agent_meow.inner.antigravity_native_executor`, which delivers web/mobile
...
def get_all_cascade_trajectories(port: int) -> dict[str, object]:
    ...
    reader's :func:`~?agent_meow.antigravity_native_reader._detect_rotated_cascade`
...
def _list_agy_pids_from_proc() -> list[int]:
    ...
    :func:`~?agent_meow.antigravity_native_launch.agy_binary_path` resolves to an
...
async def interrupt_turn(port: int, conversation_id: str) -> bool:
    ...
    DENY/ASK (see :mod:`~?agent_meow.antigravity_native_audit`). It is **best-effort
