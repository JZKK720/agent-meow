...
class SqlAgent(OmnigentBase):
    ...
    # Enum stored as a stable int code (see agent_meow.db.enum_codecs
    # AGENT_KIND: template=1, session=2). The store converts to/from the
    # string name at the rowâ†”entity boundary.
...
class SqlAccountToken(OmnigentBase):
    ...
    # the string name at the rowâ†”entity boundary.
...
class SqlConversation(ConversationBase):
    ...
    # created without an agent binding. Indexed for the agentâ†’conversation
    ...
    __table_args__ = (
        ...
        # Agentâ†’conversation reverse lookup and the agent_id / has_agent_id /
...
class SqlComment(OmnigentBase):
    ...
    :param updated_at: Unix epoch **microseconds** of the last
        ...
        JavaScript. ``BigInteger`` because epoch-Âµs overflows a
...
class SqlPolicy(OmnigentBase):
    ...
    # agent_meow.db.enum_codecs POLICY_TYPE: python=1, url=2).
    type: Mapped[int] = mapped_column(SmallInteger)
    ...
    # code (see agent_meow.db.enum_codecs POLICY_SCOPE: default=1, session=2).
...
class SqlScheduledTask(OmnigentBase):
    ...
    :param execution_target: Where a firing runs â€?
        ...
        (see agent_meow.db.enum_codecs SCHEDULED_TASK_EXECUTION_TARGET); the store
        converts to/from the string name at the rowâ†”entity boundary. Defaults to
        ``connected_host``.
    ...
    :param state: Lifecycle state â€?``active``/``paused``/``deleted``.
        ...
        rowâ†”entity boundary. Defaults to ``active``.
    ...
    # store converts to/from the string name at the rowâ†”entity boundary.
