...
``agent_meow_conversation_metadata``. That forced ``list_conversations`` to
...
``agent_meow_conversation_metadata`` via a portable correlated subquery, drops
...
def upgrade() -> None:
    ...
    then drop it from ``agent_meow_conversation_metadata``.
...
def downgrade() -> None:
    ...
    op.execute(
        """
        UPDATE agent_meow_conversation_metadata
        SET archived = COALESCE(
            (SELECT c.archived
             FROM conversations c
             WHERE c.workspace_id = agent_meow_conversation_metadata.workspace_id
               AND c.id = agent_meow_conversation_metadata.id),
            FALSE
        )
        """
    )
