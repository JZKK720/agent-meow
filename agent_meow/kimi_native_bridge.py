...
# read by :mod:`agent_meow.kimi_native_hook` (PreToolUse deny-gate + the
...
def write_hook_config(
    ...
    *,
    ...
    :func:`agent_meow.claude_native_bridge` ``permission_hook.json`` plumbing.
...
def inject_approval_keystroke(
    ...
    ``1/2/3/4 choose Â· â†?confirm``; the web-UI Approve/Deny buttons map to
    ...
    # lists "choose" and "confirm" separately, so a digit selects and â†?commits).
