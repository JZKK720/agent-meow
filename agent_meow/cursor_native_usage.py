...
the SQLite chat store (tailed by :mod:`~?agent_meow.cursor_native_forwarder`) and
...
:func:`~?agent_meow.cursor_native_bridge.write_hooks_config`) whose command runs
...
def _cli_record_usage(bridge_dir: Path) -> int:
    ...
    (:func:`~?agent_meow.cursor_native_status.record_turn_end`) on EVERY firing — even
...
async def forward_cursor_usage_to_session(
    *,
    ...
    from agent_meow._native_post_delivery import post_external_session_status
...
async def supervise_cursor_usage_forwarder(
    *,
    ...
    Mirrors :func:`~?agent_meow.cursor_native_forwarder.supervise_cursor_forwarder`:
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="agent_meow.cursor_native_usage")
