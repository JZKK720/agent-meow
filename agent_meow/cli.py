"""CLI entry point for agent_meow."""
...
from agent_meow._platform import IS_WINDOWS, resolve_repo_symlink
from agent_meow.cli_common import (
    RESUME_PICKER_SENTINEL as _RESUME_PICKER_SENTINEL,
)

# Interactive harness/credential configuration lives in agent_meow.cli_config; the
# config/setup Click commands and the first-run plan call these entry points.
from agent_meow.cli_config import (
...
from agent_meow.cli_native import register_native_commands as _register_native_commands
from agent_meow.cli_sandbox import lakebox as _lakebox_alias_group
from agent_meow.cli_sandbox import sandbox as _sandbox_group
from agent_meow.config import (
...
from agent_meow.inner import _proc, ui
from agent_meow.integration_daemon import IntegrationDaemon
from agent_meow.onboarding.sandboxes import available_providers as _sandbox_providers
from agent_meow.process_logging import LOG_LEVEL_ENV_VAR, LOG_TO_STDERR_ENV_VAR

if TYPE_CHECKING:
    import httpx

    from agent_meow.update_check import _InstalledWheelInfo
...
def _build_external_routing_client(
    ...
    answer) â€?e.g. ``"databricks-"`` when serving-endpoint names carry that
    ...
    from agent_meow.server.smart_routing import _bearer_auth
    ...
    from agent_meow.server.smart_routing import ExternalRoutingClient
...
def _build_local_llm_routing_client(
    ...
    if policy_client is None:
        return None
    from agent_meow.server.smart_routing import LLMRoutingClient
...
def _server_uvicorn_log_config(
    ...
    *,
    ...
    log_config["formatters"]["default"] = {
        "()": "agent_meow.process_logging.TerminalLogFormatter",
        "fmt": DEFAULT_LOG_FORMAT,
        "datefmt": DEFAULT_LOG_DATEFMT,
        "use_colors": use_terminal_colors,
    }
    log_config["formatters"]["access"] = {
        "()": "agent_meow.server.performance_metrics.RequestDurationAccessFormatter",
    ...
    if log_path is not None:
        ...
        if mirror:
            log_config["handlers"]["server_terminal"] = {
                "()": "agent_meow.process_logging.terminal_stream_handler",
                "formatter": "default",
                "level": level_name,
            }
...
def _migrate_legacy_state_dir() -> None:
    ...
    try:
        ...
        return
    click.echo(f"Migrated per-user state from {legacy_src} to ~/.agent_meow.", err=True)
...
def _load_effective_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    winning per-field) via :func:`agent_meow.config._merge_effective_config`
    so a project's per-harness overrides augment â€?rather than replace â€?
...
def _bundled_example_path(name: str) -> str:
    ...
    Located via the packaged ``agent_meow.resources.examples`` (symlinks to
    ...
    resource = importlib.resources.files("agent_meow.resources.examples").joinpath(name)
...
def _pick_first_run_harness() -> _FirstRunPlan | None:
    ...
    from agent_meow.onboarding.harness_install import KIMI_KEY, harness_cli_installed
...
def _resolve_auto_open_conversation_from_config(cfg: dict[str, Any]) -> bool:  # type: ignore[explicit-any]
    ...
    ``omnigent run`` does not use this resolver â€?it defaults the
...
def _normalize_harness_scalar_on_write(
    ...
    absent, or not a string. Behavior is unchanged by the migration â€?the
...
def _save_global_config(  # type: ignore[explicit-any]
    ...
    *deep_merge_keys*, the incoming mapping is instead merged one level
    ...
    :func:`~agent_meow.onboarding.provider_config.set_default_provider`,
...
def _materialize_bundled_example(name: str) -> Path:
    ...
    resource = resources.files("agent_meow.resources.examples").joinpath(name)
...
def _default_artifact_location() -> str:
    """Default artifact dir for ``omnigent server`` â€?``<data_dir>/artifacts``.
...
def _maybe_prompt_first_admin(account_store: Any, auth_provider: Any, *, auto_open: bool) -> None:  # type: ignore[explicit-any]  # SqlAlchemyAccountStore | None, AuthProvider
    ...
    from agent_meow.server.routes.accounts_auth import _MIN_PASSWORD_LENGTH
...
def _preregister_agent(  # type: ignore[explicit-any]  # agent_store / artifact_store / agent_cache typed Any to avoid import cycle
    ...
    :func:`agent_meow.spec.materialize_bundle`, tars it, validates
    ...
    from agent_meow.db.utils import generate_agent_id
    ...
    # :func:`agent_meow.inner.cli._omnigent_register_yaml_bundle` â€?
    ...
    # (``ondelete=CASCADE`` in
    # :class:`agent_meow.db.db_models.SqlTask`), wiping every
...
def _format_version() -> str:
    ...
    from agent_meow.update_check import _read_build_info
...
def _print_version_callback(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
    ...
    it ``_read_build_info()`` â€?during ``agent_meow.cli`` import. The
...
def _warn_deprecated_harness_path_env_vars() -> None:
    ...
    if not sys.stderr.isatty():
        return
    from agent_meow.harness_startup_config import legacy_harness_path_env_vars_set
...
def main() -> None:
    ...
    # Installed first so crashes anywhere below â€?argv shorthands, Click
    # dispatch, imports â€?are all caught. See omnigent/crash_handler.py.
    ...
    # Shorthand: ``omnigent --harness claude [opts]`` â†?
    ...
    # Shorthand: ``omnigent myagent.yaml [opts]`` â†?``run myagent.yaml [opts]``.
    ...
    from agent_meow.cli_diagnostics import (
        log_cli_error_hint,
        log_cli_exception,
        print_setup_hint,
        setup_cli_logging,
    ...
    # ``omnigent setup`` IS the setup wizard â€?if it fails, telling the
    ...
    if not _should_skip_update_check(argv) and sys.stderr.isatty():
        from agent_meow.update_check import maybe_show_update_notice
    ...
    try:
        ...
        # Keep the diagnostics log line ("Details logged to â€?) â€?the
        # always-on CLI log has more context than this single crash â€?then
        ...
        from agent_meow.crash_handler import handle_crash
...
def _is_removed_ad_hoc_invocation(argv: list[str]) -> bool:
    ...
    - The first non-flag token isn't a known click subcommand and is
      ...
      ``omnigent "what does this repo do?"``) â€?the free-text shape
...
def _daemon_host_online(record: _HostDaemonRecord, *, timeout_s: float = 2.0) -> bool:
    ...
    """
    from agent_meow.claude_native_bridge import url_component
...
def _load_existing_host_id() -> str | None:
    ...
    from agent_meow.host.identity import CONFIG_PATH
...
def _load_or_create_host_id() -> str | None:
    ...
    if host_id is not None:
        return host_id
    from agent_meow.host.identity import CONFIG_PATH, load_or_create_host_identity
...
def _ensure_host_daemon(server_url: str | None) -> bool:
    ...
    args = [sys.executable, "-m", "agent_meow.host._daemon_entry", *mode_args]
...
def _build_host_daemon_env(
    *,
    ...
    through :func:`agent_meow.host.connect._build_runner_env`, so these
...
def _ensure_databricks_server_auth(server: str, *, non_interactive: bool = False) -> None:
    ...
    from agent_meow.chat import _remote_headers
    ...
    from agent_meow.cli_auth import load_databricks_org_id
...
def _start_cli_runner_process(
    *,
    ...
    :param runner_id: Optional runner id to advertise. ``None``
        ...
        :func:`agent_meow.runner.identity.get_stable_runner_id`
    ...
    if runner_proc.poll() is not None:
        from agent_meow._runner_startup import format_runner_log_tail
...
def _adopt_cli_runner_process(proc: subprocess.Popen[bytes]) -> None:
    ...
    :returns: None.
    """
    from agent_meow.runner.identity import RUNNER_ADOPT_SIGNAL
...
def server(
    ...
    # dedicated `omnigent server --port 9001 --database-uri â€¦` on
    ...
    from agent_meow.spec import parse_default_policies, parse_server_llm
    ...
    # Hidden by default â€?managed deployments override
    ...
    from agent_meow.process_logging import configure_process_logging
    ...
    from agent_meow.server.app import _WEB_UI_DIST
    ...
    class _ShutdownSignalingServer(uvicorn.server.Server):
        ...
        uvicorn calls ``Server.shutdown()`` in this order:
          ...
          2. ``asyncio.wait_for(_wait_tasks_to_complete(), timeout=â€?``
          3. force-cancel remaining tasks on timeout
          4. run the ASGI lifespan shutdown handler

        The ASGI lifespan ``finally`` block runs at step 4 â€?too late. SSE
        ...
        async def shutdown(self, sockets=None) -> None:  # type: ignore[override]
            ...
            from agent_meow.runtime import session_stream as _session_stream
...
def _uninstall_script_path() -> Path:
    ...
    if repo_script.exists():
        return repo_script
    try:
        resource = resources.files("agent_meow.resources.scripts").joinpath("uninstall_oss.sh")
...
def _maybe_fast_backfill_install_ledger(argv: Sequence[str]) -> None:
    ...
    with contextlib.suppress(Exception):
        from agent_meow.install_ledger import backfill_install_ledger
...
def _internal_write_ledger(from_env: bool) -> None:
    ...
    del from_env
    from agent_meow.install_ledger import ledger_path, write_install_ledger_from_env
...
def doctor(
    ...
    from agent_meow.install_ledger import backfill_install_ledger, backfill_ledger_path
...
def uninstall(
    ...
    from agent_meow.install_ledger import resolve_uninstall_ledger
...
def upgrade(check_only: bool, force: bool, pre: bool) -> None:
    ...
    raise click.ClickException(
        ...
        "Reinstall it explicitly â€?e.g. `uv tool upgrade --reinstall omnigent` or "
...
def _expand_config_env_vars(  # type: ignore[explicit-any]  # raw is parsed YAML (heterogeneous values)
    ...
    :param expand_fn: Callable that expands env var references
        ...
        :func:`agent_meow.spec.expand_env_vars`.
...
def polly(run_args: tuple[str, ...]) -> None:
    ...
    Shorthand for ``omnigent run`` on the packaged polly agent â€?the same
...
def import_session_command(
    ...
    from agent_meow.chat import _remote_headers
    from agent_meow.session_import import (
        ImportSource,
        SessionImportNotFoundError,
    )
    from agent_meow.session_import.local import (
...
# ``agent_meow.chat.run_chat`` for how local-agent options get baked
...
_DEFAULT_HARNESS_PROMPTS = {
    "claude-sdk": (
        "You are Claude Code, running through agent_meow. "
        "Help the user with software engineering tasks."
    ),
    "codex": (
        "You are Codex, running through agent_meow. Help the user with software engineering tasks."
    ),
    "cursor": (
        "You are Cursor, running through agent_meow. Help the user with software engineering tasks."
    ),
    "kimi": (
        "You are Kimi Code, running through agent_meow. "
        "Help the user with software engineering tasks."
    ),
    "qwen": (
        "You are Qwen Code, running through agent_meow. "
        "Help the user with software engineering tasks."
    ),
    "goose": (
        "You are Goose, running through agent_meow. Help the user with software engineering tasks."
    ),
}
_DEFAULT_HARNESS_PROMPT = "You are a helpful coding agent running through agent_meow."
...
def _materialize_harness_launcher_file(
    *,
    ...
    request â€?routing file/shell operations through the Omnigent
...
def _dispatch_native_terminal_harness(
    *,
    ...
    Omnigent turn per message â€?which persists its own user item â€?*while*
    ...
    if session_id is None and not resume_picker and resume_latest:
        from agent_meow.chat import _remote_headers, _resolve_latest_conversation_id
...
def _dispatch_run(
    *,
    ...
    if target is None:
        if server_from_cli and server is not None and harness is None:
            ...
            if resume_conversation_id is not None and is_interactive_shape:
                from agent_meow.chat import _redirect_native_resume_if_needed, run_attach
            ...
            from agent_meow.chat import run_chat
    ...
    if prompt is not None:
        ...
        from agent_meow.chat import run_prompt
...
def attach(
    ...
    if base_url is None:
        raise click.ClickException(
            ...
            "server â€?start one with `omnigent run`, or point at one with "
            "`--server <url>`."
        )
    ...
    from agent_meow.chat import run_attach
...
def run(
    ...
    if harness is None and not direct_server_cli:
        from agent_meow.harness_startup_config import resolve_harness_config
...
def host(ctx: click.Context, server: str | None, non_interactive: bool) -> None:
    ...
    :param non_interactive: When ``True``, never launch the browser login
        for an un-authed remote server â€?fail with the ``omnigent login``
        hint instead.
    """
    ...
    from agent_meow.host.connect import run_host_process
    ...
    # already started by `omnigent server` or a prior host/run daemon) â€?
    ...
    try:
        ...
        # a server started by `omnigent server` or another daemon â€?killing
...
def _format_harness_for_display(
    ...
    :param value: The raw ``harness`` config value â€?scalar string or mapping.
...
def _resolve_harness_startup_args(
    ...
    *cli_args* append *after* so a per-invocation flag wins for last-wins CLIs.
    ...
    """
    from agent_meow.harness_startup_config import resolve_harness_args
...
def _print_config_defaults() -> None:
    ...
    if not global_cfg and not local_cfg:
        click.echo(
            "  (none set â€?`omnigent config set key=value` for project,\n"
...
# â”€â”€ Integrations (Slack, â€? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# Slack socket-mode bot: a separate `omnigent-slack` package (heavy deps â€?
# slack_bolt/aiohttp â€?kept out of the core CLI install). The CLI launches it
...
def _slack_cwd() -> Path | None:
    ...
    a ``.env``; otherwise ``None`` (a wheel install has no such dir â€?config
...
def slack(ctx: click.Context) -> None:
    ...
    Config (Slack tokens, OMNIGENT_SERVER_URL, â€? comes from the environment
    and the integration's .env file â€?see integrations/slack/.env.example.
    """
    if ctx.invoked_subcommand is not None:
        return
...
def slack_start() -> None:
    ...
    # leaves nothing on the terminal â€?confirm it survives a short grace and
...
def setup(internal_beta: bool) -> None:
    ...
    if internal_beta:
        ...
        try:
            import agent_meow.onboarding.internal_beta  # noqa: F401
        except ImportError:
            raise click.ClickException(
        ...
        _warn_missing_harness_dependencies()
        from agent_meow.onboarding.internal_beta import _INTERNAL_BETA_DEFAULT_SERVER
        from agent_meow.onboarding.sandboxes.lakebox import install_demo_databricks_cli
        from agent_meow.onboarding.setup import run_onboarding
...
# `omnigent lakebox` is kept as an alias for `omnigent sandbox â€?
...
def debug_db_upgrade(url: str) -> None:
    ...
    from agent_meow.db.utils import _run_migrations
...
def debug_logs(
    ...
    if not existing_dirs:
        ...
        raise click.ClickException(f"No {requested_log_type} logs found â€?none of {dirs} exist.")
...
def _cached_workspace_bearer(workspace_host: str) -> str | None:
    ...
    """
    from agent_meow.onboarding.databricks_config import databricks_sdk_installed
...
def _workspace_api_server_url(server: str) -> str:
    ...
    :returns: The normalized base URL without a trailing slash, e.g.
        ``"https://example.databricks.com/api/2.0/omnigent"`` â€?or the
    ...
    from agent_meow.conversation_browser import (
        WORKSPACE_API_PATH,
        WORKSPACE_UI_PATH,
        display_server_url,
    )
    ...
    click.echo(
        ...
        "unauthenticated requests â€?if omnigent is hosted on this "
...
def _resolve_server_url(server: str) -> str:
    ...
    then expand a bare Databricks workspace URL â€?or the ``/omnigent``
...
def _databricks_workspace_token(workspace_host: str) -> str | None:
    ...
    """
    from agent_meow.inner.databricks_executor import (
        DatabricksAuthError,
...
def login(server_url: str) -> None:
    ...
    if probe.status_code == 401:
        ...
        # 401 with non-JSON body â€?probably not an Omnigent server.
...
def _strip_resume_flags(argv: list[str]) -> list[str]:
    ...
    :param argv: Parent's launch argv, e.g.
        ``["python", "-m", "agent_meow.cli", "run", "agent.yaml",
...
def _ensure_bundled_agent_brain_credential(name: str) -> None:
    ...
    adoption (see :func:`agent_meow.setup`).
    ...
    try:
        ...
        if len(candidates) > 1:
            message = (
                f"No default {family_name} credential set â€?"
...
def _run_bundled_agent(name: str, run_args: tuple[str, ...]) -> None:
    ...
    ``prog_name`` is pinned to ``"omnigent run"`` so context-derived output â€?
...
# Native coding-agent (TUI) subcommands (claude/codex/pi/â€? live in
# agent_meow.cli_native; register them on the group here, at module bottom, so the
