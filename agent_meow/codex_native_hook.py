...
:mod:`~?agent_meow.codex_native_app_server`). Codex spawns this module as
...
:mod:`~?agent_meow.native_policy_hook`.
"""
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    :func:`~?agent_meow.native_policy_hook.hook_payload_to_evaluation_request`,
    ...
    :mod:`~?agent_meow.codex_native_app_server`, not here, because a
    ...
    if config is None:
        # No agent-meow server configured for this session — nothing to enforce.
        return 0
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m agent_meow.codex_native_hook evaluate-policy")
