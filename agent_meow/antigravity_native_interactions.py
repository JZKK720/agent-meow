...
* ``get_steps`` — re-reads the freshest trajectory steps (production wraps
  :func:`~?agent_meow.antigravity_native_rpc.get_trajectory_steps`).
...
* ``deliver`` — defaults to :func:`_deliver_via_rpc`, which offloads the blocking
  :func:`~?agent_meow.antigravity_native_rpc.handle_user_interaction` to a worker
...
:func:`~?agent_meow.codex_native_elicitation.codex_elicitation_id`), so a
...
from agent_meow.antigravity_native_steps import PendingInteraction, pending_interaction
...
class Deliver(Protocol):
    ...
    Matches :func:`~?agent_meow.antigravity_native_rpc.handle_user_interaction`
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    :func:`~?agent_meow.antigravity_native_bridge.send_interaction_keys_via_tui`) to a
    ...
    from agent_meow.antigravity_native_bridge import send_interaction_keys_via_tui
    from agent_meow.inner.antigravity_native_executor import _bridge_dir_from_env
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    ...
    :func:`~?agent_meow.codex_native_elicitation.codex_elicitation_id`.
...
async def _deliver_via_rpc(
    ...
    *,
    ...
    :func:`~?agent_meow.antigravity_native_rpc.handle_user_interaction` is synchronous
...
async def bridge_interaction(
    ...
    *,
    ...
    :param deliver: Delivers the built payload to agy; defaults to
        ...
        :func:`~?agent_meow.antigravity_native_rpc.handle_user_interaction` to a
        worker thread.
