...
from agent_meow.onboarding.ucode_state import UcodeAgentState, read_ucode_state
...
from agent_meow.runtime.prompt import build_instructions, history_to_input_items
...
def _get_runner_client_for_compaction(
    ...
    if conversation_id is None:
        return None
    from agent_meow.runtime import get_runner_router
...
def _catalog_default_model(family_name: str) -> str | None:
    ...
    :func:`~?agent_meow.onboarding.providers.default_chat_model` for the rule
...
def _apply_cli_config_databricks_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    (:func:`~?agent_meow.pi_native_credentials._cli_config_pi_provider`) �?which
    ...
    from agent_meow.pi_native_credentials import _cli_config_pi_provider
...
def _apply_harness_path_override(
    ...
    from agent_meow.harness_startup_config import (
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    :func:`~?agent_meow.inner.executor_factory.create_executor`
...
def _build_acp_spawn_env(
    ...
    *,
    ...
    from agent_meow.onboarding.acp_auth import acp_agents, resolve_acp_agent
...
def _build_cursor_spawn_env(
    ...
    *,
    ...
    if isinstance(spec.executor.auth, ApiKeyAuth):
        ...
        from agent_meow.onboarding.cursor_auth import resolve_cursor_api_key
...
def _build_kimi_spawn_env(
    ...
    *,
    ...
    defined in :mod:`~?agent_meow.inner.kimi_harness`.
...
async def compact_conversation_now(
    *,
    ...
    :returns: The :class:`CompactionResult` returned by
        :func:`~?agent_meow.runtime.compaction.compact`.
    """
    ...
    from agent_meow.llms.context_window import get_model_context_window
...
def _route_bare_model_for_compaction(llm_config: LLMConfig) -> LLMConfig:
    ...
    whose :func:`~agent_meow.llms.routing.parse_model_string` defaults any
...
def _find_spec_by_name(
    ...
    from agent_meow.tools.builtins.web_fetch import RESEARCHER_NAME

    if name == RESEARCHER_NAME:
        owner = _find_web_fetch_owner(spec)
        if owner is not None:
            from agent_meow.tools.builtins.web_fetch import build_researcher_spec
