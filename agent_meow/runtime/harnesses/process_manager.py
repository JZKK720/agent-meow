...
Owns the contract from Â§Process management of
...
from agent_meow.harness_plugins import missing_install_packages
from agent_meow.inner import _proc
from agent_meow.inner._subprocess_lifecycle import close_subprocess_transport
...
# (see Â§What's deferred in the design doc).
_DIR_MODE = 0o700
...
# ness (Â§Heartbeats: ~5s cadence, ~3 missed = dead). The client
...
class HarnessProcessManager:
    ...
    async def forward_cancel(
        ...
        ``designs/session_rearchitecture.md`` Â§3 / Â§7
        "Flow: interrupt".
