...
from agent_meow.inner.tracing import TracingContext, is_tracing_enabled
from agent_meow.policies.types import FAIL_CLOSED_PHASES
from agent_meow.runtime.harnesses._scaffold import HarnessApp, PolicyVerdictPayload, TurnContext
...
def _strip_mcp_tool_prefix(name: str) -> str:
    ...
    Mirrors :func:`~?agent_meow.runtime.workflow._strip_mcp_tool_prefix`
...
class ExecutorAdapter(HarnessApp):
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        ...
        from agent_meow.runtime.telemetry import current_session_id, session_scope
        ...
        try:
            ...
            if tctx:
                try:
                    from agent_meow.runtime.telemetry import trace_context_for_response
                ...
                async for event in executor.run_turn(
                    ...
                    if ctx.cancelled.is_set():
                        if tctx is not None and agent_span is not None:
                            from agent_meow.runtime.telemetry import record_cancellation
                    ...
                    if tctx is not None:
                        if isinstance(event, ToolCallRequest):
                            ...
                            if event.usage is not None:
                                from agent_meow.runtime.telemetry import record_llm_usage
    ...
    async def _stable_elicitation_handler(
        ...
        Called by :class:`~?agent_meow.inner.claude_sdk_executor.ClaudeSDKExecutor`
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    ...
    from agent_meow.llms.errors import is_context_length_exceeded
...
def _normalize_tool_schemas(
    ...
    agent-meow emits :meth:`~?agent_meow.tools.base.Tool.get_schema` in
    OpenAI Chat Completions shape: ``{"type": "function",
    "function": {"name", "description", "parameters"}}``. The
    inner :class:`Executor` ABC and its
    :func:`~?agent_meow.inner.claude_sdk_executor._build_mcp_tools`
