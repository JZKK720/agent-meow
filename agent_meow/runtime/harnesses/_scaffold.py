...
# agent-meow server → runner → harness). Held at one day (86400s) — matching
...
# Default 600s: context compaction's summarizing LLM call runs as a
# single long await that emits nothing until it returns, so on a
# near-full context it can exceed a shorter window and trip the watchdog,
# wedging the session in a "Prompt is too long" -> compaction -> timeout
# loop. Env var name kept for the ops knob; ``<= 0`` disables.
...
class PolicyVerdictPayload:
    ...
    evaluates the policy on the agent-meow server and sends the verdict back
...
class TurnContext:
    ...
    :param response_id: Server-allocated id for this turn,
        ...
        ``response.created`` envelope so agent-meow can correlate
    ...
    def emit(self, event: HarnessStreamEvent) -> None:
        ...
        :param event: A typed event from
            :data:`~?agent_meow.server.schemas.ServerStreamEvent`,
            e.g.
    ...
    async def dispatch_tool(self, call_id: str, name: str, arguments: str, agent: str) -> str:
        ...
        the parked Future when agent-meow delivers an event carrying the
        matching ``call_id``. See §How agent-meow resolves action_required
        tool calls in the design doc.

        :param call_id: Unique identifier for this tool call,
            e.g. ``"call_abc123"``. agent-meow echoes it back in the
        ...
        :param agent: Agent name that invoked the tool — required
            ...
            :class:`~?agent_meow.entities.conversation.FunctionCallData`.
        :returns: The tool's output string from
            :class:`~?agent_meow.server.schemas.ToolResult`.
    ...
    async def evaluate_policy(
        ...
        :returns: The verdict payload from the agent-meow server.
        :raises asyncio.CancelledError: If the turn is cancelled.
        """
...
class HarnessApp:
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        allowlist at
        :data:`~?agent_meow.runtime.harnesses._client_executor._RETRYABLE_HARNESS_ERROR_CODES`
        ...
        :class:`~?agent_meow.runtime.harnesses._executor_adapter.ExecutorAdapter`)
    ...
    def build(self) -> FastAPI:
        ...
        :returns: A configured :class:`FastAPI` instance ready to
            be served by ``agent_meow.runtime.harnesses._runner``.
        """
    ...
    async def _build_terminal_event(
        ...
        if status_value == "completed":
            ...
            from agent_meow.server.schemas import CancelledEvent
...
# subclass — see
# :meth:`~?agent_meow.runtime.harnesses._executor_adapter.ExecutorAdapter._build_error_detail`.
