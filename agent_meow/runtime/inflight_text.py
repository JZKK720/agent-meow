...
* :func:`~?agent_meow.runtime.session_stream` is pure fan-out with no
...
:func:`~?agent_meow.runtime.session_stream.publish` (the single SSE
chokepoint, same as :mod:`~?agent_meow.runtime.pending_elicitations`):
...
# message_id is retired, and :func:`~?agent_meow.runtime.session_stream.publish`
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> bool:
    ...
    * a ``session.status`` event whose status is terminal (see
    ...
    :param event: The event dict as passed to
        :func:`~?agent_meow.runtime.session_stream.publish`. Reads
    ...
    :returns: ``True`` when the event must be WITHHELD from the live
        ...
        (:func:`~?agent_meow.runtime.session_stream.publish`) must skip
