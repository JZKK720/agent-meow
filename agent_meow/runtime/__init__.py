...
def set_runner_router(router: RunnerRouter | None) -> None:
    ...
    :param router: :class:`~?agent_meow.runner.routing.RunnerRouter`,
...
def get_runner_router() -> RunnerRouter | None:
    ...
    :returns: :class:`~?agent_meow.runner.routing.RunnerRouter` or
        ``None``.
    """
...
def get_harness_process_manager() -> HarnessProcessManager:
    ...
    if pm is None:
        raise RuntimeError(
            "HarnessProcessManager not initialized — agent-meow lifespan startup "
