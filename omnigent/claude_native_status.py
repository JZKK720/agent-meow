...
def _write_context_atomic(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    malformed â€” there's nothing useful to record.
    ...
    # event, so the agent-meow relay's cost accumulation never runs for it).
    ...
    # on every render â€” including right after an in-pane ``/model`` switch.
