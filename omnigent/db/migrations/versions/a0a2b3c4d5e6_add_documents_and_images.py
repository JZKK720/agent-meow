...
- ``documents``: per-session rich-text documents (markdown + ProseMirror
  JSON). Managed by :class:`~?omnigent.stores.document_store.DocumentStore`.
- ``images``: per-session image metadata (binary lives in the
  ArtifactStore). Managed by :class:`~?omnigent.stores.image_store.ImageStore`.
"""
...
revision = "a0a2b3c4d5e6"
down_revision = "d4f2a1b6c8e9"
branch_labels = None
depends_on = None
