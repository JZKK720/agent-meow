...
class SqlConversation(Base):
    ...
    :param root_conversation_id: Id of the root (top-level)
        ...
        ``conversation_id`` lives in the caller's tree in O(1) â€”
    ...
    :param runner_id: Runner the conversation is pinned to (hard
        affinity per ``designs/RUNNER.md`` Â§5). ``None`` until the
        ...
        runner records are not persisted in v1 â€” the registry is
        purely in-memory.
    :param external_session_id: Runtime-native session id this
        ...
        across runtimes â€” at most one external session per
    ...
    # orphaning it â€” and host_id -> NULL keeps the
    ...
    # migration that adds this column. Low-cardinality, so no index â€”
...
class SqlDocument(Base):
    ...
    :param format: Content format identifier â€” ``"markdown"`` or
