...
async def _execute_doc_tool(
    ...
    *,
    ...
    - ``doc_create`` ΓåÆ ``POST /v1/sessions/{id}/resources/documents``
    - ``doc_get`` ΓåÆ ``GET /v1/sessions/{id}/resources/documents/{doc_id}``
    - ``doc_list`` ΓåÆ ``GET /v1/sessions/{id}/resources/documents``
    - ``doc_update`` ΓåÆ ``PATCH /v1/sessions/{id}/resources/documents/{doc_id}``
    - ``doc_generate`` ΓåÆ generates markdown from topic/outline by calling
    ...
    if tool_name == "doc_generate":
        ...
        content_parts.append(
            ...
            "a placeholder ΓÇö use `doc_update` to fill in the full content.\n"
        )
        gen_payload = {
            "title": topic,
            "format": "markdown",
    ...
    # ΓöÇΓöÇ officecli-backed Office document operations ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def _execute_office_cli_tool(
    ...
    *,
    ...
    if tool_name == "doc_edit_office":
        ...
        # The doc resource is markdown/office ΓÇö we need the original file.
...
# ΓöÇΓöÇ agent-meow Images surface dispatch ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def _execute_image_tool(
    ...
    *,
    ...
    - ``image_list`` ΓåÆ ``GET /v1/sessions/{id}/resources/images``
    - ``image_get`` ΓåÆ metadata via the list endpoint (the per-id GET
      returns binary, so we filter the list)
    - ``image_upload`` ΓåÆ reads a local file and ``POST``s it as multipart
    - ``image_edit`` ΓåÆ ``PATCH .../images/{id}/edit`` with Fabric.js JSON
    - ``image_generate`` ΓåÆ v1 stub (returns a not-yet-wired message)
...
# ΓöÇΓöÇ agent-meow Image generation/editing helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _resolve_image_provider() -> str | None:
    ...
    if os.environ.get("A1111_API_URL"):
        return "a1111"
    # ComfyUI is detected at call time via the MCP manager ΓÇö return a hint.
    ...
    *,
    ...
    try:
        if provider == "hosted":
            ...
            # comfyui via MCP ΓÇö handled by the MCP manager, not here.
            return json.dumps({
                "error": (
                    "image_generate: ComfyUI MCP routing requires an MCP "
                    "server declared in tools.mcp_servers ΓÇö call it directly "
    ...
    *,
    ...
    try:
        if provider == "a1111":
            ...
            # hosted API ΓÇö use img2img if available
            api_url = os.environ.get("IMAGE_GEN_API_URL", "").rstrip("/")
            api_key = os.environ.get("IMAGE_GEN_API_KEY", "")
            headers = {}
            if api_key:
...
# ΓöÇΓöÇ agent-meow Video surface dispatch ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def _execute_video_tool(
    ...
    *,
    ...
    - ``video_list`` ΓåÆ ``GET /v1/sessions/{id}/resources/videos``
    - ``video_get`` ΓåÆ metadata via the list endpoint (the per-id GET
      returns binary, so we filter the list)
    - ``video_generate`` ΓåÆ calls the Pixelle-Video FastAPI gateway
...
def _resolve_video_provider() -> str | None:
    ...
    are served via fal.ai or a local ComfyUI/Pixelle gateway ΓÇö there is no
...
async def _execute_video_generate(
    ...
    *,
    ...
    - ``fal``: fal.ai hosted ΓÇö Wan2.1, HunyuanVideo, LTX-Video, Veo, Kling,
      MiniMax, Seedance. User picks model via ``VIDEO_GEN_MODEL``.
    - ``happy-horse``: Happy Horse 1.0 ΓÇö 15B unified Transformer, native
    ...
    try:
        ...
        # openmontage is routed via MCP ΓÇö declared in tools.mcp_servers.
        return json.dumps({
            "error": (
...
async def _video_generate_happy_horse(
    ...
    *,
    ...
    """Generate via Happy Horse 1.0 ΓÇö 15B unified Transformer, native audio-video."""
    import asyncio
    import os
...
# ΓöÇΓöÇ agent-meow Voice surface dispatch ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def _execute_voice_tool(
    ...
    *,
    ...
    - ``transcribe_audio`` ΓåÆ shells out to ``handy --transcribe-file <path>
      --json`` (or the CLI configured via ``HANDY_CLI_PATH``). Returns the
      transcribed text.
    - ``transcribe_audio_high_quality`` ΓåÆ calls a VibeVoice-ASR vLLM endpoint
      (``VIBEVOICE_ASR_URL``) for diarized long-form transcription.
    - ``text_to_speech`` / ``speak`` ΓåÆ calls Voicebox REST ``POST /speak``
      (when ``VOICEBOX_URL`` is set ΓÇö preferred, provides voice cloning,
    ...
    if tool_name in ("text_to_speech", "speak"):
        ...
        # Voicebox is the preferred TTS path ΓÇö it provides voice cloning,
        ...
        if not voicebox_url and not tts_url:
            return json.dumps({
                "error": (
                    ...
                    "  1. Set VOICEBOX_URL=http://127.0.0.1:17493 (Voicebox ΓÇö "
