...
class McpSchemasResult:
    ...
    tool_names: set[str]
    failures: dict[str, str]  # server_name â†’ error message
...
class RunnerMcpManager:
    ...
    def _build_elicitation_callback(
        ...
        :returns: Async callback ``(session_id, params) â†’
            ElicitResult``.
        """
        ...
        async def _elicit(
            ...
            if server_client is None:
                _logger.warning(
                    "MCP elicitation callback: no agent-meow server client available â€” declining",
                )
            ...
            try:
                ...
                _logger.warning(
                    "MCP elicitation callback: agent-meow server POST failed (%s) â€” declining",
                    exc,
                )
                return ElicitResult(action="decline")
