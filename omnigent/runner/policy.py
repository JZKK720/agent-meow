...
Pre-refactor the agent-meow server's :class:`PolicyEngine` enforced
``function``-type policies (POLICIES.md Â§9.1) on every tool dispatch.
...
- ``label`` and ``prompt`` types stay server-side â€” they need the
...
ASK verdicts: the gate itself doesn't own an elicitation channel â€”
it just surfaces ASK to the caller. The caller (typically
``runner.tool_dispatch.execute_tool``) escalates by POSTing
``evaluate_policy=True`` to the agent-meow server, which independently
...
evaluation (runner + server) is by design â€” the runner needs the
...
class PolicyVerdict:
    ...
    on ASK (and escalate to the agent-meow server) rather than treating
    ...
    :param action: ``"allow"`` (tool may proceed), ``"deny"`` (tool
        ...
        the agent-meow server for a user verdict; ``policy_name`` and
    ...
    :param data: Replacement payload from the policy. On ALLOW
        ...
        event content â€” e.g. PII-redacted tool arguments
...
# Singleton ALLOW verdict â€” frozen dataclass, no state, allocate
...
class RunnerToolPolicyGate:
    ...
    async def evaluate_tool_result(
        ...
        output already exists â€” escalating to the user would block
    ...
    async def _evaluate_policies(
        ...
        - DENY short-circuits immediately â€” no later policy can
          override a denial.
        - ASK is recorded but evaluation continues â€” a later policy
        ...
        # wins â€” callers that need chained transforms compose in one callable.
...
def format_deny_text(policy_name: str, reason: str | None) -> str:
    ...
    denials â€” when an ASK round-trip returns refused / times
