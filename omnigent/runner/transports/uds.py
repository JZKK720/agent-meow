...
process via real bytes on the wire â€” no in-memory shortcut.
...
(per RUNNER.md Â§7 Phase 2 "CLI integration") is a separate piece in
the CLI module and not implemented in this session â€” see RUNNER.md
Â§11 autonomous decisions log.
"""
...
class RunnerSubprocess:
    ...
    a separate process while keeping the serverâ†”runner wire local.
    ...
    def __enter__(self) -> RunnerSubprocess:
        ...
        self._process = subprocess.Popen(
            ...
            # torn down on shutdown â€” uvicorn spawns child workers we need
            # to clean up too.
            **_proc.spawn_kwargs(),
            env=env,
        )
