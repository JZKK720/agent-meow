...
Routes all MCP calls through the agent-meow server's
...
The agent-meow server holds the live connections in its
...
- **AP mode** (out-of-process runner paired with agent-meow server):
  ``ProxyMcpManager(session_id, server_client)`` â€” all tool calls flow
  through the agent-meow server, policy enforced centrally.
- **No-AP / test mode**:
  ``RunnerMcpManager(stdio_cwd=...)`` â€” direct MCP connections, runner
...
class ProxyMcpManager:
    """Routes runner-side MCP calls through the agent-meow server MCP proxy endpoint.

    Drop-in substitute for :class:`~?omnigent.runner.mcp_manager.RunnerMcpManager`
    used in agent-meow mode.  The runner creates one instance per session on first MCP use
    ...
    :param ap_client: An :class:`httpx.AsyncClient` pointed at the agent-meow server.
        Must already carry the runner's service auth (e.g. Databricks bearer
        token) so requests are accepted by the agent-meow server's auth middleware.
    """
    ...
    def __init__(
        ...
        :param ap_client: HTTP client pointed at the agent-meow server.
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        """Fetch tool schemas from the agent-meow server MCP proxy (``tools/list``).

        Sends a ``tools/list`` JSON-RPC 2.0 request to the agent-meow server's MCP
        ...
        the agent-meow server (e.g. ``github__search``).  This ensures the harness
        ...
        except Exception as exc:  # noqa: BLE001 â€” network + HTTP errors all surface as failures
            _logger.warning(
        ...
        for tool in tools_list:
            ...
            if not name:
                continue
            # The agent-meow server returns ``inputSchema`` (JSON Schema from MCP).
    ...
    async def call_tool(
        ...
        """Dispatch a tool call via the agent-meow server MCP proxy (``tools/call``).

        Sends a ``tools/call`` JSON-RPC 2.0 request.  The agent-meow server enforces
        TOOL_CALL and TOOL_RESULT policies before forwarding to the real MCP
        server.

        **ASK policy / approval flow**: when the agent-meow server returns an
        ...
        :param spec: Ignored â€” accepted for interface parity with
        ...
        :returns: Tool output string.  On denial the agent-meow server returns an
        ...
        """
        del spec  # agent-meow server resolves spec from session context
        ...
        for _attempt in range(2):
            ...
            # â”€â”€ ASK: agent-meow server returned InputRequiredResult â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if result.get("resultType") == "input_required":
                ...
                # The agent-meow server uses the elicitation_id as the key in
                # inputRequests (MRTR spec: keys are server-assigned and the
                # client CAN read them â€” only requestState is opaque).
            ...
            # â”€â”€ Normal result (ALLOW or post-approval execution) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op â€” the agent-meow server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op â€” the agent-meow server owns and manages MCP connections."""
