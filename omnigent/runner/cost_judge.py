...
- **Per-turn, single verdict.** One brain runs the whole turn, so the
  ...
  parts need â€” the brain handles the whole turn at that level.
- **Conversational turns â†’ ``None``.** "ok", "thanks", "continue",
  ...
  not an error.
- **Difficulty â†’ tier.** Difficult coding / architecture / tricky
  debugging â†’ expensive; medium knowledge work â†’ medium; trivial â†’
...
class _JudgeConfig:
    ...
    :param tiers: Models-only tier catalog, e.g.
        ``{"cheap": ("m1",), "expensive": ("m2",)}`` â€” both the menu the
...
class LLMJudge:
    """
    Per-turn LLM judge: one call â†’ single brain-model verdict | None.
    ...
    selection stands); a broken judge also yields ``None`` (fail-open) â€”
    ...
    async def _invoke_judge(self, query: str) -> dict[str, Any] | None:  # type: ignore[explicit-any]  # parsed JSON verdict
        ...
        :returns: The parsed JSON dict (``{"tier": ..., "model": ...}``
            ...
            the output was unparseable after one retry â€” both are judge
        ...
        for _attempt in range(_JUDGE_ATTEMPTS):
            ...
            except Exception as exc:  # noqa: BLE001 â€” fail-open: any judge error â†’ no verdict
                last_error = exc
        _logger.warning(
...
# â”€â”€ Prompt (module-level so it is reviewable) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
- expensive: genuinely hard, multi-step, or high-stakes engineering â€” \
deep refactors, architecture/design, tricky debugging, security review, \
anything requiring careful multi-file reasoning.
- medium: ordinary knowledge work â€” focused code changes, writing or \
explaining a moderate amount, summarizing a document, routine analysis.
- cheap: trivial, mechanical, or very short tasks â€” a one-line lookup, a \
...
3. If the message is purely CONVERSATIONAL â€” an acknowledgement, thanks, \
a status check, "continue", small talk, or anything with no real work to \
do â€” return the null verdict so the prior selection stays in force.
...
def _resolve_judge_model(  # type: ignore[explicit-any]  # executor_config is a YAML-shaped dict
    ...
    :raises ValueError: When no configured tier has any model (the
        catalog is unusable â€” config validation should have caught this).
    """
...
class _RoutedJudgeCall:
    ...
    :param connection: Connection overrides for the call, e.g.
        ``{"base_url": "https://â€¦/serving-endpoints", "api_key": "â€¦"}``,
...
def _route_databricks_judge_model(
    ...
    (``api.openai.com``) and the judge would fail open on EVERY turn â€”
    ...
    host/token from *databricks_profile* â€” the same profile the brain's
...
def build_llm_judge(  # type: ignore[explicit-any]  # executor_config is a YAML-shaped dict
    *,
    ...
    :param client: LLM client override; ``None`` builds the real
        :class:`~?omnigent.llms.client.Client`. Tests pass a scripted stub â€”
...
def resolve_advisor_mode(spec_mode: str, override: str | None) -> str | None:
    ...
    verdict) even on an advise-default spec â€” that is the shadowâ†’apply
    ...
    :param override: The session's ``cost_control_mode_override`` â€”
    ...
    if override == "off":
        return None
    if override == "on":
        # "on" = apply: escalate adviseâ†’optimize so the user toggle has the
