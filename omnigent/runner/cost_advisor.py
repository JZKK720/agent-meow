...
- in **optimize** mode, reports the model the caller must run the brain
  on this turn (the runner sets it on the harness request â€” claude-sdk
...
advisor â€” the verdict is still recorded (``applied=False``) but the
...
class AdvisorConfig:
    ...
    :param mode: Mode the advisor runs in: ``"optimize"`` (apply the
        verdict to the brain) or ``"advise"`` (shadow â€” record only).
    """
...
def parse_advisor_config(executor_config: Mapping[str, Any] | None) -> AdvisorConfig | None:
    ...
    :returns: The parsed config, or ``None`` when the marker is absent,
        ``null``, or an explicit ``false`` opt-out (advisor off â€” the
        default).
    :raises ValueError: When the marker is present but malformed
        (non-mapping, EMPTY mapping, unknown ``mode``, empty or
        non-string ``tiers``). Opting in with a broken config â€” including
        ``cost_optimize: {}`` â€” fails loud rather than silently running
        unadvised.
    """
...
def _databricks_profile_for_spec(spec: Any) -> str | None:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    ``auth:`` block â€” so the judge call rides the same Databricks gateway
    ...
    :returns: The profile name, e.g. ``"my-workspace"``, or ``None``
        (no Databricks routing configured, or resolution failed â€” the
    ...
    try:
        ...
        if auth is not None:
            # Explicit non-Databricks auth (e.g. api_key) â€” no profile.
            return None
        return str(legacy) if legacy else None
    except Exception:  # noqa: BLE001 â€” advisor must never block the turn
        _logger.warning(
...
class Judge(Protocol):
    ...
    Implementations decide one :class:`AdvisorVerdict` for the turn â€” or
    ...
    async def judge(self, *, query: str, turn_anchor: str) -> AdvisorVerdict | None:
        ...
        :returns: The verdict to persist (and, optimize mode, apply), or
            ``None`` for a conversational turn â€” the advisor then leaves
...
def routing_decision_event(verdict: AdvisorVerdict) -> dict[str, Any]:  # type: ignore[explicit-any]  # JSON-shaped SSE event
    ...
    so the relay has no turn response_id yet â€” it stamps a fresh
...
async def maybe_run_advisor(
    *,
    ...
    Judges the turn, persists the ``cost_control.plan`` label, and â€” in
    optimize mode on a claude-sdk brain with no user pin â€” reports the
    ...
    - the judge returns ``None`` (conversational turn) â€” BY DESIGN the
      label write and application are skipped, so the prior turn's
      selection stays;
    - the label persist fails â€” treated like a conversational turn (no
    ...
    :param turn_anchor: Caller-sampled anchor for the verdict (item id or
        ISO timestamp) â€” the advisor never reads the clock itself.
    ...
    if judge is not None:
        ...
        effective_judge = build_llm_judge(
            ...
            # Ride the same Databricks gateway as the brain â€” a bare
...
def _runner_identity_headers() -> dict[str, str]:
    ...
    :returns: ``{X-agent-meow-Runner-Tunnel-Token: <token>}`` when the
        ...
        CLI / host-daemon spawn path), else ``{}`` â€” single-user local
...
async def _persist_verdict_label(
    ...
    :returns: ``True`` on success; ``False`` (with a warning logged) when
        the PATCH failed â€” the caller then applies nothing so the
    ...
    if resp.status_code >= 400:
        ...
        # longer depends on it â€” it rides the routing_decision transcript
        # item â€” but the telemetry label and its failure mode must stay
