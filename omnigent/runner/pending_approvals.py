...
# (86400s) â€” matching the deciding policy's default ``ask_timeout``: an ASK
...
# as DENY) any prompt a user didn't answer within two minutes â€” the
...
# Module-global registry: elicitation_id â†’ asyncio.Future[bool].
# True = approved, False = declined/timed-out. Future is owned by the
# caller that registered it â€” this module is just the routing table
...
# than one parked at once â€” e.g. parallel tool calls that each tripped a
...
# ``sys_session_send`` to a blocked child â€” that message would otherwise
...
def cleanup(elicitation_id: str) -> None:
    ...
    Idempotent â€” popping an unknown id is a no-op. Callers must
...
async def wait_for_user_approval(
    *,
    ...
    Centralizes the register â†’ wait_for â†’ cleanup â†’ publish
    sequence so every ASK-escalation site emits a
    ``response.elicitation_resolved`` event on the way out â€” the
...
def reset_for_tests() -> None:
    """
    Clear the registry. For test isolation only â€” leaked Futures
