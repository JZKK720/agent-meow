"""Hermes-native tool-approval mirror (TUI â†’ web elicitation).
...
1. poll ``capture-pane`` and detect the approval PANEL â€” the interactive TUI
   renders a prompt_toolkit panel titled ``âš ï¸  Dangerous Command`` with NUMBERED
   choices (``â¯ 1. Allow once`` â€¦ ``4. Deny``), NOT the legacy ``Choice
...
3. on the verdict, send the choice's DIGIT key (e.g. ``1`` = Allow once, ``4`` =
   Deny) into the pane â€” Hermes' number-key binding selects AND confirms in one
   press,
...
This does NOT suppress Hermes' own gate â€” its panel stays the source of truth
...
# ``Choice [o/s/a/D]:`` prompt â€” that path is fail-closed while prompt_toolkit
# owns the terminal. The panel is titled ``âš ï¸  Dangerous Command`` and lists
# NUMBERED choices (``â¯ 1. Allow once`` â€¦ ``4. Deny``); pressing the number both
...
# A numbered choice row, e.g. "â¯ 1. Allow once" / "  4. Deny" (box borders ignored).
...
def hermes_permission_elicitation_id(session_id: str, token: str) -> str:
    """Return the deterministic agent-meow elicitation id for a Hermes prompt.
...
def _strip_border(line: str) -> str:
    ...
    return line.strip().strip("â”‚").strip()


def parse_hermes_approval_prompt(pane: str) -> HermesApprovalPrompt | None:
    """Parse a Hermes ``âš ï¸  Dangerous Command`` approval panel from pane text.
    ...
    and reads the digit key for each from the panel itself â€” robust to whether the
...
async def _run_one_approval(
    ...
    *,
    ...
    if not response.content:
        # Empty 2xx â†’ resolved elsewhere (TUI answered) or timeout: no keystroke.
        return
    try:
