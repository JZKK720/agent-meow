...
* **conversation-ownership discovery** (:func:`resolve_language_server_port` /
  :func:`get_trajectory_steps`) â€” the read side the reader binds to;
* **turn delivery** via ``SendUserCascadeMessage``
  (:func:`send_user_cascade_message`), which agy records as a real ``USER_INPUT``
  turn â€” used by the executor instead of agy's ``SendAgentMessage`` RPC (which
...
* Ownership probe: ``POST .../GetConversationMetadata`` with REQUEST body
  ...
  ("trajectory not found") for an unknown one â€” so a caller can confirm which
...
# answer, step reads, cancel) â€” generous headroom over the tight discovery-probe
...
# caller does not retry â€” the interaction bridge only retries on "input not
...
# the READ timeout is disabled (``None``) â€” a tight per-read deadline would abort
...
# bit 0 (``0x01``) marks a COMPRESSED payload (agy sends uncompressed â€” a set bit
...
def _async_client(timeout: httpx.Timeout | float) -> httpx.AsyncClient:
    ...
    :param timeout: Per-request timeout â€” a flat seconds float, or an
...
def _post_rpc_raising(port: int, method: str, body: dict[str, object]) -> None:
    ...
    caller has a single :class:`AntigravityRpcError` type to catch â€” transport
...
def send_user_cascade_message(
    ...
    *,
    ...
    driver keys on â€” in contrast to ``SendAgentMessage`` which records as a
    ``SYSTEM_MESSAGE`` and is therefore unsuitable.

    Two shape constraints (live-verified against agy 1.0.10, see Â§10.1 of the
    design doc):

    * The turn text MUST be in ``items[].text`` (a list of objects) â€” a flat
...
def start_cascade(
    ...
    * ``source`` is the ONLY required field. ``cascadeId`` is optional â€” when
      omitted agy mints and returns one â€” but the runner PROVIDES it so it owns
...
def get_available_models(port: int) -> dict[str, object]:
    ...
    (unwrapped from the response envelope â€” see below). Used to resolve a model
    enum string at runtime for :func:`send_user_cascade_message` (the model is
    required per-turn and must not be hardcoded â€” ``planModel`` values are
    ...
    current model is already known from a prior turn's step data â€” call this only
    ...
    # future agy ever drops the envelope (defensive â€” keeps callers working).
...
def get_all_cascade_trajectories(port: int) -> dict[str, object]:
    ...
    ``lastModifiedTime`` ABSENT (``null``) until it is actually used â€” so the
...
async def stream_agent_state_updates(
    ...
    Framing (live-verified, agy 1.0.10 â€” see design Â§10.2). The request body is a
    ...
    * ``flag == 0x00`` â€” DATA: ``payload`` is the JSON envelope ``{"update":
      {...}}``; the unwrapped ``update`` object is yielded.
    * ``flag & 0x02`` (:data:`_FRAME_FLAG_TRAILER`) â€” end-of-stream trailer:
    ...
    * ``flag & 0x01`` (:data:`_FRAME_FLAG_COMPRESSED`) â€” compressed payload: agy
      sends uncompressed, so a set bit means a decode mismatch â€” raises
    ...
    5-byte header AND the full declared payload â€” otherwise the loop awaits more
    ...
    async with (
        ...
        # and return cleanly â€” which the reader treats as a normal end-of-stream
        ...
        async for chunk in response.aiter_bytes():
            ...
            while len(buffer) >= _FRAME_HEADER_LEN:
                ...
                if len(buffer) < frame_end:
                    break  # full payload not arrived yet â€” await more bytes
                ...
                if flag & _FRAME_FLAG_TRAILER:
                    ...
                    # HTTP status â€” the 200 + headers were already flushed), so a
                ...
                if isinstance(parsed, dict):
                    ...
                    # (defensive â€” keeps the reader working).
...
def _pane_pid(socket_path: Path, tmux_target: str) -> int | None:
    ...
    ancestor) on a sandboxed launch â€” :func:`_agy_pid_in_pane_subtree` resolves
...
class PaneAgyResolution(NamedTuple):
    ...
    Distinguishes the THREE states the cold-start must treat differently â€” a bare
...
def resolve_pane_agy_rpc_port_state(socket_path: Path, tmux_target: str) -> PaneAgyResolution:
    ...
    * ``(agy_found=True, port=<int>)`` â€” our agy is up and its port is resolved.
    * ``(agy_found=True, port=None)`` â€” our agy IS in the pane subtree but its port
    ...
    * ``(agy_found=False, port=None)`` â€” no agy is running in this pane yet (no
      pane pid, or agy has not been ``exec``-ed â€” the CLI ``tmux_start_on_attach``
    ...
    if pane_pid is None:
        # No pane pid (tmux gone / target absent) â€” agy is not reachable here.
...
def resolve_cold_start_agy_rpc_port(
    ...
    1. **Pane present, our agy found, port resolved** â†’ that scoped port.
    2. **Pane present, our agy found, port not resolvable** (e.g. restricted
       ``/proc`` where lsof cannot attribute the listener) â†’ the lowest candidate
    ...
    3. **Pane present, NO agy found yet** (the CLI ``tmux_start_on_attach``
       early-poll window: the pane is still the shell, agy not yet ``exec``-ed) â†’
       ...
       durably cross-bind this session â€” the exact defect this scoping prevents.

    With **no pane supplied** (``tmux_socket``/``tmux_target`` is ``None`` â€” e.g. a
    ...
    if tmux_socket is not None and tmux_target is not None:
        ...
        if not resolution.agy_found:
            # state 3: our agy is not up yet (CLI early-poll). Keep polling â€” do
        ...
        # /proc; one-agy-per-pod) â€” the candidate scan below is safe and necessary.
        _logger.debug(
            "agy cold-start: pane agy found for target=%s but its port is not "
            "lsof-attributable; using the host-wide candidate scan (safe â€” agy is "
            "up in this pane)",
            tmux_target,
        )
