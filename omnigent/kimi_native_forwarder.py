"""Mirror a kimi-native TUI session's transcript into the agent-meow web chat.
...
reply renders live in the embedded terminal, but â€” unlike the SDK ``KimiExecutor``
â€” nothing flows the assistant's response back into agent-meow's conversation
...
``$KIMI_CODE_HOME/sessions/<wd_â€¦>/<session_â€¦>/agents/main/wire.jsonl``. The
...
- ``{"type": "turn.prompt", "input": [{"type":"text","text":â€¦}], "origin": {"kind":"user"}}``
  â†’ a user message.
- ``{"type": "context.append_loop_event", "event": {"type": "content.part",
  "part": {"type": "text", "text": â€¦}, "uuid": â€¦}}`` â†’ an assistant message.
  (``part.type == "think"`` is reasoning and is skipped for v1; ``tool.call`` /
  ``tool.result`` events are likewise skipped â€” the embedded terminal shows them.)
...
def _workdirs_for_sessions(kimi_home: Path) -> dict[str, str]:
    """Map each session dir â†’ its ``workDir`` from ``session_index.jsonl``.
...
def _discover_wire(kimi_home: Path, workspace: str, launch_epoch_ms: int) -> Path | None:
    ...
    for wire in sessions_root.glob("*/session_*/agents/main/wire.jsonl"):
        # session_index keys on the session dir (â€¦/<wd_â€¦>/<session_â€¦>).
        session_dir = str(wire.parent.parent.parent)
        work_dir = workdirs.get(session_dir)
        # When the index doesn't list it yet, fall back to recency alone â€” a
...
async def supervise_kimi_forwarder(
    *,
    ...
    restarts on any other exception with exponential backoff â€” mirrors
