...
Per ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.2, this rewrite swaps to:

- One terminal abstraction (``inner.terminal.TerminalInstance``) for
  the whole project â€” tmux-based.
...
- No idle reaper: terminals are explicit-launch only and the LLM is
  expected to ``sys_terminal_close`` when done. agent-meow shutdown still
  closes everything; per-conversation cleanup runs from the workflow's
  finally block.

The registry is constructed once at agent-meow startup
...
# tmux session can't block agent-meow shutdown or workflow finalization
# indefinitely. 5s mirrors the legacy ``_RELEASE_GRACE_S`` in
# ``runtime/harnesses/process_manager.py`` â€” long enough for a
...
def conversation_link_for_id(
    ...
    *,
    ...
    :param conversation_id: Conversation/session id, e.g.
        ``"conv_abc123"``.
    :param base_url: Optional agent-meow server base URL, e.g.
    ...
    # APIâ†’UI mount swap (``/api/2.0/agent-meow`` â†’ ``/agent-meow``) and the
    # ``?o=<org>`` selector â€” keeping the terminal status-bar link in
...
class TerminalRegistry:
    ...
    def __init__(self, *, conversation_link_base_url: str | None = None) -> None:
        ...
        :param conversation_link_base_url: Optional agent-meow server base URL
        ...
        # tmux subprocess invocations on the same instance â€” without it,
        ...
        # between them â€” plenty of room for another send to slip in).
        # See ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§9.1.
        self._instance_locks: dict[tuple[str, str, str], threading.Lock] = {}
        # Threading lock â€” see module docstring for the rationale.
    ...
    async def launch(
        ...
        *,
        ...
        without spawning a new tmux session â€” matches the legacy
    ...
    def get_instance_lock(
        ...
        :returns: A :class:`threading.Lock` to acquire around
            ...
            the LLM rather than skipping the lock â€” the
    ...
    def get(
        ...
        Sync because it doesn't touch tmux â€” just reads the map.
    ...
    def list_for_conversation(
        ...
        Snapshot semantics â€” the list reflects state at call time.
    ...
    def transfer(
        ...
        process and tmux pane keep running, but the agent-meow session that owns
    ...
    async def close(
        ...
        ``sys_terminal_close`` tool's contract â€” the LLM may close
    ...
    async def cleanup_conversation(self, conversation_id: str) -> None:
        ...
        Per ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.4, this is **not**
        ...
        workflow consumes â€” not for fire-and-forget cleanup.
    ...
    async def shutdown(self) -> None:
        ...
        bounded by ``_CLOSE_TIMEOUT_S`` per instance. Best-effort â€”
        a stuck instance shouldn't block the rest of agent-meow shutdown.
        """
        with self._lock:
            slots = list(self._by_conversation.items())
            self._by_conversation.clear()
            # AP-shutdown clears all instance locks too â€” every
    ...
    def active_conversation_ids(self) -> list[str]:
        ...
        Used by tests. Snapshot semantics â€” the list reflects state
        at call time.
