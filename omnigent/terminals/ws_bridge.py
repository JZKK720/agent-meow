"""Shared PTY â†” WebSocket bridge for tmux ``attach`` sessions.
...
location avoids a runner â†’ ``server.routes`` dependency.
...
- **Server â†’ client**: every PTY read becomes a *binary* WS frame.
- **Client â†’ server**:
...
# 4404 tells the client's reconnect loop to stop â€” sent on a
...
# deadlock â€” on macOS this surfaces as a hard crash of the forked child.
...
# process's own controlling terminal â€” so its capabilities, not the
...
# support clear") â€” the web terminal renders that error instead of
# the pane.
...
def _fork_exec_pty(tmux_path: str, argv: list[str], env: dict[str, str]) -> _SpawnedPty:
    ...
    fork of a large runner process does not stall the event loop â€” and
    therefore the WS tunnel heartbeat â€” while the kernel works.
    ...
    if pid == 0:
        ...
        # (not ``execvpe``) so there is no Python-level PATH search â€” the
...
async def _tmux_session_alive(socket_path: str, tmux_target: str) -> bool:
    ...
    exit (Claude quit / the session was killed). The pane-dead flag â€” not bare
    session existence â€” is the signal because the claude-native terminal opts
    ...
    # which silently falls back to another pane). rc != 0 â†’ session gone; a "1"
    # line â†’ the inner process exited but the session was kept alive by
...
async def _forward_pty_to_ws(
    ...
    *,
    ...
    ``send_bytes`` â€” never waiting for more, so a lone keystroke flushes
...
async def bridge_tmux_pty_to_websocket(
    ...
    *,
    ...
    :param on_client_interaction: Optional callback fired on every client
        interaction with the pane â€” attach (connect), detach
    ...
    # to tmux regardless of this process's ambient TERM â€” see
    ...
    async def _ws_to_pty() -> None:
        ...
        try:
            while True:
                ...
                # Every received frame is a client interaction â€” a
                # keystroke/focus/mouse byte, a resize, or a disconnect â€”
                ...
                if text is not None:
                    ...
                    if pane_check_due:
                        ...
                        if is_dead is True:
                            ...
                            return
                        # is_dead is False (live) or None (inconclusive) â†’ continue
    ...
    try:
        ...
        # apart â€” a detach must not be reported as terminal-gone, or the
