"""Shared tmux control-mode (``tmux -C``) â†” WebSocket bridge.
...
- ``%output <pane-id> <octal-escaped-bytes>`` â€” the raw bytes the program in a
...
- ``%begin <t> <n> <flags>`` â€¦ ``%end``/``%error <t> <n> <flags>`` â€” bracketed
  reply blocks for commands the bridge sends, correlated by command number.
- ``%exit`` / ``%window-close`` / ``%layout-change`` â€” lifecycle + structure.
...
# ``_forward_pty_to_ws`` is queue-driven and transport-agnostic â€” it drains
# everything already queued into one bounded ``send_bytes`` â€” so the control
...
# tmux's clientâ†’server protocol rejects a single command larger than its 16KB
# imsg cap. ``send-keys -H`` expands each input byte to 3 chars ("xx "), so cap
# the bytes per send-keys invocation well under the limit. 1024 bytes â‰ˆ 3KB
...
async def _run_tmux_capture(socket_path: str, tmux_target: str) -> bytes | None:
    ...
    not to column 0, so every row starts where the previous one ended â€” the
    ...
    **Scrollback**, conditioned on the screen mode:
    ...
    - **Alternate screen** (claude, codex, vim): capture the visible screen
      ...
      stale *primary*-screen history from before the app switched buffers â€”
      lines that were never part of the app's UI â€” corrupting the seed. tmux's
    ...
    if proc.returncode != 0:
        return None
    # ``capture-pane -p`` emits one LF per row â€” INCLUDING a trailing LF after
...
async def _capture_pane_metadata(
    ...
    ``None`` on any failure â€” the caller degrades gracefully (skips the
...
async def bridge_tmux_control_to_websocket(
    ...
    *,
    ...
    """
    # Attaching reflows the pane to this client's size â€” stamp it as a client
    ...
    # pre-attach content is missing. Failure is non-fatal â€” a live pane redraw
    ...
    # Decoded ``%output`` payloads flow reader â†’ forwarder through this queue
    ...
    # xterm's synchronous paint path â€” see the PTY bridge).
    ...
    def _handle_control_line(line: bytes) -> bool:
        ...
        window-close). Pure parsing â€” the actual browser send is the
        forwarder's job.
        ...
        if line.startswith(b"%window-close"):
            ...
            # probe. (%pane-mode-changed â€” copy-mode enter/leave â€” is
        ...
        # forwarding â€” the browser xterm renders purely from %output.
        return True
    ...
    async def _read_control() -> None:
        ...
        can pull many buffered ``%output`` lines at once â€” letting the
        forwarder coalesce them â€” and so an oversized line can't raise
        ...
        try:
            while True:
                ...
                if not data:
                    # tmux control client closed its stdout â€” server/session gone.
                    return
                buffer += data
    ...
    # instant the browser reports its real dimensions â€” two spurious SIGWINCHes
    ...
    # window-close) â€” the signal the close-code logic keys on. The forwarder
    ...
    try:
        ...
        # burst-then-exit program's tail isn't dropped mid-drain â€” the exact
        ...
        if control_ended_first and not forward_task.done():
            # Suppress everything here (TimeoutError â†’ drain took too long, fall
            # through to cancel; any other error â†’ the forwarder itself raised,
        ...
        # Detach reflows the pane back to remaining clients â€” stamp it.
        if on_client_interaction is not None:
            on_client_interaction()
