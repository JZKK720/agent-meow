...
:func:`inject_user_message` (tmux bracketed paste + a single Enter) â€” the goose
...
``goose configure``), and agent-meow does not mutate user config in v1.
"""
...
# in a row marks the input box ready. See KTD3 / R2 in the plan â€” refine with a
...
def build_goose_native_spawn_env(
    ...
    *,
    ...
    pane is cheaper to scrape, and â€” since ``goose session`` exposes no
    ``--provider``/``--model`` flags â€” pins the provider/model via env when the
...
def inject_user_message(
    ...
    *,
    ...
    Enter and inserts a newline on Ctrl+J, so exactly one Enter is sent â€” a
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    Terminates ``goose`` and the pane outright â€” the analog of the user manually
