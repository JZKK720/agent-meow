"""TUIâ†’web forwarder for the cursor-native harness.
...
is then the only surface that reflects the agent's work â€” the agent-meow
...
This module is that missing mirror â€” the cursor analog of
...
``external_conversation_item`` events â€” which also seeds the session title from
...
"Workingâ€¦" spinner, though â€” it never wakes a parent orchestrator. So this
...
:mod:`~?omnigent.cursor_native_status`) â€” the SAME server contract
...
#: and overflows the column â€” every mirror POST then 500s and, because the poll
...
#: theory alias two blobs onto one id â€” that only groups two messages under one UI
...
#: â€” mirroring nothing after it and flooding the app. A connection-level failure
...
#: ~0.7s poll cadence this is a few seconds of retrying â€” enough to ride out a
...
# cursor wraps the real prompt the user typed in ``<user_query>â€¦</user_query>``
# and prepends a large ``<user_info>â€¦`` context dump as a separate user blob.
...
# first user message, fenced in <omnigent_fork_history>â€¦</omnigent_fork_history>
...
# real text â€” the copied history already lives in the agent-meow timeline, so
...
# signal that the compaction the web UI requested has actually completed â€”
# cursor-agent has no compaction hook the way Claude Code does â€” so the
...
class _ForwardState:
    ...
    :param last_rowid: Highest ``blobs.rowid`` already processed for
        ...
        append-only and content-addressed, so rowids only grow â€” tracking the
...
class _ModelMirrorState:
    """In-memory dedupe for terminalâ†’web model-change mirroring.
...
def _write_state(bridge_dir: Path, state: _ForwardState) -> bool:
    ...
    :returns: ``True`` on success. A failure is logged (not silently swallowed)
        and returns ``False`` â€” the in-memory cursor still guards against
...
def _chat_claimed_by_other(bridge_dir: Path, store_path: Path, my_launch_ms: int) -> bool:
    ...
    launched in the same cwd discover the SAME store â€” without this guard both
...
def preseed_resume_state(
    ...
    at higher rowids â€” it does not re-write prior turns as new blobs. Seeding
    ...
    duplicates â€” the e2e gate (cursor-sdk-e2e-dev) is the guard for that drift.
...
def _discover_store(workspace: str, launch_epoch_ms: int) -> Path | None:
    ...
    but bind ONLY when exactly one chat across them qualifies â€” never guessing
    ...
    # mirroring the wrong conversation â€” silent cross-talk is worse than a brief
    # delay.
    if not root.is_dir():
        return None
    matches: list[Path] = []
...
def _unwrap_user_query(text: str) -> str | None:
    ...
    A real user turn is wrapped in ``<user_query>â€¦</user_query>``; the large
    ``<user_info>â€¦`` context dump cursor prepends has no such wrapper and is not
...
def _read_blob_rows(store_path: Path, last_rowid: int) -> list[tuple[int, str, object]]:
    ...
    is wrong â€” it tells SQLite the file never changes and to ignore the WAL,
...
def _read_last_used_model(store_path: Path) -> str | None:
    ...
    hex-encoded JSON blob carrying ``lastUsedModel`` â€” the *base* model id (e.g.
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    from what we last posted â€” *including the very first observation*. Unlike
    ...
    UI pill â€” meaningless for cursor. Surfacing the real model immediately fixes
    ...
    clobber (see ``nativeModelFamilyForSession`` in the web store â€” cursor is
...
async def _patch_external_session_id(
    ...
    """PATCH the agent-meow session with the cursor chat id for cold-resume.
    ...
    try:
        ...
        _logger.warning(
            "Transient error PATCHing external_session_id; session=%s â€” will not retry",
            session_id,
        )
...
async def _post_external_compaction_status(
    ...
    web UI already renders, upgrading the "Compacting conversationâ€¦" spinner
...
async def _persist_native_compaction_item(
    ...
    *,
    ...
    compaction_data = {
        "summary": "[Cursor compaction â€” context was compacted via /summarize]",
...
async def forward_cursor_store_to_session(
    *,
    ...
    wedge the mirror â€” or flood the app â€” indefinitely; an *ambiguous* failure
    ...
    async with httpx.AsyncClient(
        ...
        while True:
            try:
                    ...
                    if await asyncio.to_thread(
                        ...
                        for item in items:
                            if item.item_type == "compaction_completed":
                                ...
                                # its "Compactingâ€¦" spinner upgrades to the
                                # permanent marker. Best-effort â€” a failed post
                                ...
                                # for good, since the cursor persists past it) â€”
                            ...
                            if item.item_type:
                                try:
                                    ...
                                    if post_may_have_been_delivered(exc):
                                        ...
                                        # duplicate the web bubble â€” skip past it.
                                        _logger.warning(
                                        ...
                                        # poison item can't wedge the mirror â€” and
                                        # flood the app â€” forever.
                ...
                # an ``external_session_status: idle`` edge â€” the signal that wakes
