...
difficulty (difficult coding â†’ expensive tier, medium knowledge work â†’
medium, trivial â†’ cheap). The verdict names a single tier + a single
...
and which worker models â€” that is correctness (the model-family guard),
...
def tier_rank(tier: str) -> int:
    ...
    :raises ValueError: When *tier* is not a known tier name â€” an
...
def parse_verdict(labels: Mapping[str, str]) -> AdvisorVerdict | None:
    ...
    sessions keep loading â€” the advisor simply has no v3 verdict to
