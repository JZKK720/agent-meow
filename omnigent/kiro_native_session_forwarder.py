...
into the agent-meow conversation with ``external_conversation_item`` events.
"""
...
# ponytail: grows by one entry per distinct concurrent collision â€” too rare to
...
def _discover_kiro_session_jsonl(
    *,
    ...
    """Find this agent-meow session's Kiro JSONL file â€” only when it's unambiguous.
    ...
    whichever session most recently emitted a turn â€” i.e. it can bind the *other*
...
async def _patch_external_session_id(
    ...
    *,
    ...
    """Persist Kiro's native CLI session id onto the agent-meow session."""
