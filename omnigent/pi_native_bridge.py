...
def clear_inbox(bridge_dir: Path) -> None:
    ...
    terminal (re)launch â€” mirroring codex-native's ``clear_bridge_state``.
...
def _enqueue_payload(bridge_dir: Path, item_id: str, payload: dict[str, Any]) -> None:
    ...
    # with no time ordering â€” and an ``interrupt_`` id sorts before a ``msg_``
...
def write_extension_files(
    ...
    *,
    ...
    :param auth_headers: HTTP headers the extension should use when posting
        terminal-originated events back to agent-meow.
    :param tools: Flat agent-meow tool schemas
        ...
        runner uses), so the Pi agent can invoke agent-meow ``sys_*`` tools with
...
def refresh_config_auth_headers(bridge_dir: Path, auth_headers: dict[str, str]) -> bool:
    ...
    â€” e.g. at the start of each turn â€” keeps its ``/policies/evaluate`` and
