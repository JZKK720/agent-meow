...
â€” it has no ``agy login`` / ``agy auth status`` subcommand. *Where* and *how* it
persists the resulting token is **platform-specific** (verified live against agy
1.0.10):

- macOS writes ``~/.gemini/oauth_creds.json`` â€” a flat OAuth2 object::

      {"access_token": "ya29.â€¦", "refresh_token": "1//0gâ€¦",
       "expiry_date": â€¦, "id_token": "eyJâ€¦", "scope": "â€¦", "token_type": "Bearer"}

- Linux writes ``~/.gemini/antigravity-cli/antigravity-oauth-token`` â€” the OAuth
  object **nested under** ``token``::

      {"auth_method": "oauth",
       "token": {"access_token": "ya29.â€¦", "refresh_token": "1//0gâ€¦",
                 "token_type": "Bearer", "expiry": "â€¦"}}

Detection is file-based and subprocess-free: it checks **both** locations and
treats a non-empty ``access_token`` / ``refresh_token`` string â€” flat (macOS) or
nested under ``token`` (Linux) â€” as a completed login, so a logged-in user is
...
server-side revocation â€” its only job is to reject the "no-credential /
...
machine carrying *both* files â€” e.g. a home directory migrated across OSes â€” a
...
def _file_carries_token(path: Path) -> bool:
    ...
    try:
        ...
        # exists but holds non-UTF-8 bytes (a corrupt creds file) â€” treat it as
...
def gemini_auth_has_credential(creds_path: Path | None = None) -> bool:
    ...
    usable token â€” so a logged-in user is recognized on both macOS
    ...
    ``token``. This cannot detect server-side revocation â€” for that, see
