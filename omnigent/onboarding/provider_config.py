...
tools key defaults by purpose â€” Zed's per-feature model pointers,
...
name: <name>}`` â€” independent of these defaults.)
...
- an explicit **kind** per provider entry â€” one of ``key`` /
  ``subscription`` / ``gateway`` / ``local`` / ``databricks`` â€” so the
...
- an **introspection** helper (:func:`describe_active_credential`) that
  ...
  for the ``/model`` readout and the ``models`` command â€” never exposing
  the secret value itself.

Each inline-credential provider (``key`` / ``gateway`` / ``local``) groups
one or more *families*:

- ``anthropic`` â€” the Anthropic Messages API surface, consumed by the
  Claude SDK harness and native Claude Code.
- ``openai`` â€” the OpenAI Responses/Chat surface, consumed by the Codex
  harness, native Codex, and the OpenAI-Agents SDK harness.
- ``gemini`` â€” the Google Gemini surface, the credential the native
  Antigravity (``agy``) onboarding adopts as a ``key``-kind ``gemini:``
  provider from a detected ``GEMINI_API_KEY``.

The ``pi`` harness consumes the ``anthropic`` / ``openai`` families only â€”
never ``gemini``.
"""
...
# Google Gemini surface â€” the credential the native Antigravity (``agy``)
...
# not ``_VALID_FAMILIES`` â€” else a machine whose only credential is a gemini
# key resolves pi to a credential pi cannot use.
_PI_FALLBACK_FAMILIES = (ANTHROPIC_FAMILY, OPENAI_FAMILY)

# The pi harness's *default scope*. pi is not a model family â€” a provider
# entry never carries a ``pi:`` block â€” but defaults are scoped per harness
...
# too (a Databricks AI Gateway is pi-consumable â€” Pi speaks its Anthropic
...
# - ``subscription``: a logged-in CLI (``claude`` / ``codex``) â€” no families,
...
# (``key`` / ``gateway`` / ``local``) â€” as opposed to ``subscription`` (a
...
# ``pi`` harness consumes both families and so is absent here â€” callers
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # per-spawn provider override flag, so agent-meow cannot thread a generic
    ...
    # codex-native) so a same-agent qwenâ†’qwen fork/switch reads as same-family.
    "qwen": OPENAI_FAMILY,
...
def provider_family_for_harness(harness: str | None) -> str | None:
    ...
    (``"claude_sdk"`` â†’ ``"claude-sdk"``, ``"agents_sdk"`` â†’
    ``"openai-agents"``).
...
class FamilyConfig:
    ...
    ``auth_command``. The secret is resolved lazily â€” see
    :meth:`Provider.family` and :func:`resolve_secret` â€” so a family the
    ...
    :param api_key_ref: A reference to a secret stored outside the config
        ...
        ``"keychain:<name>"`` (read from the agent-meow secret store â€” see
...
class ProviderEntry:
    ...
    - ``subscription`` carries :attr:`cli` (``"claude"`` / ``"codex"``)
      and no families/base_url â€” the CLI's own login supplies auth.
    - ``databricks`` carries :attr:`profile` and no families â€” auth is
      resolved from ``~/.databrickscfg`` + ucode state by the runtime.
    - ``cli-config`` carries :attr:`cli` (``"codex"`` only today) and
      :attr:`model_provider` â€” the provider definition and credential live
    ...
    :param families: Parsed families keyed by family name
        ...
        here may still hold raw ``$VAR`` references â€” they are expanded
    ...
    :param default_families: The set of model families this provider is
        ...
        ``true`` â†’ every family it serves (:func:`provider_families`); a
        family name (``default: openai``) or list (``default: [anthropic]``)
        â†’ just those. Empty when not a default. Per-family scoping lets a
    ...
    def default(self) -> bool:
        ...
        :returns: ``True`` when the provider is a default for â‰¥1 family.
        """
...
class ResolvedCredential:
    ...
    and the ``models`` command. It never carries the secret value â€” only a
...
def resolve_secret(ref: str) -> str:
    ...
    - ``"env:<VAR>"`` â€” read ``<VAR>`` from the environment.
    - a bare inline ``$VAR`` / ``${VAR}`` reference â€” expanded via
      :func:`os.path.expandvars` with an unresolved-variable check.
    - ``"keychain:<name>"`` â€” read ``<name>`` from the agent-meow secret
      store (OS keychain, else a ``0600`` JSON file). The store is
      populated by ``agent-meow setup --no-internal-beta`` â€” see
...
def _expand_family(provider_name: str, family_name: str, family: FamilyConfig) -> FamilyConfig:
    ...
    # auth_command (or, defensively, nothing) â€” leave the secret slot empty.
...
def _parse_family(provider_name: str, family_name: str, raw: dict[str, object]) -> FamilyConfig:
    ...
    ``api_key_ref`` are left **unresolved** here â€” resolved lazily by
    ...
    # bearer token â€” useless as a GEMINI_API_KEY â€” so an ``auth_command`` gemini
...
def _parse_default_families(
    ...
    - ``True`` / ``"true"`` â†’ every family the provider serves.
    - ``False`` / ``"false"`` / absent / ``""`` â†’ not a default.
    - a family name (``"openai"``) â†’ just that family.
    - a list of family names (``["anthropic", "openai"]``) â†’ those.
    ...
    per family) are valid â€” if ``true`` claimed pi on both, they would
    ...
    # the Gemini surface, so it must NOT accept a pi scope â€” this mirrors
...
def _default_raw_value(default_families: frozenset[str], served: set[str]) -> object:
    ...
    Inverse of :func:`_parse_default_families`: the whole served set â†’
    ``True`` (stays correct if the provider later serves more families is
    NOT a concern â€” we re-render on each write); a single family â†’ its
    name; several â†’ a sorted list; none â†’ ``None`` (caller omits the key).
    ...
    # includes pi must stay explicit â€” rendering it as ``True`` would drop
...
def _parse_provider(name: str, raw: dict[str, object]) -> ProviderEntry:
    ...
    # The (possibly family-scoped) default flag â€” resolved per-kind below
    ...
    # real bool / family-name / list â€” :func:`_parse_default_families`
    ...
    if kind == SUBSCRIPTION_KIND:
        ...
        # A subscription serves the family its CLI implies (claudeâ†’anthropic,
        # codexâ†’openai); an unknown CLI serves nothing.
        served = (
            {ANTHROPIC_FAMILY}
            if cli_raw == "claude"
        ...
        return ProviderEntry(
            ...
            # pi â€” naming "pi" in its default scope is a config error.
    ...
    if kind == CLI_CONFIG_KIND:
        ...
        return ProviderEntry(
            ...
            # its Anthropic surface natively). This is allowed structurally â€”
            # without reading the ambient ~/.codex/config.toml at parse â€” so a
            # user can pin piâ†’Databricks; whether the pinned provider is a *real*
    ...
    # GEMINI_API_KEY or OAuth â€” never a proxy). A ``gateway`` / ``local`` may
    # *carry* a gemini block alongside a real family (we ignore it for the Gemini
    # surface â€” see :func:`provider_families`), but one whose ONLY family is
...
def harness_family(harness: str) -> str | None:
    ...
    Public accessor for the canonical harnessâ†’family map. ``None`` means
...
def _cli_config_serves_pi(entry: ProviderEntry) -> bool:
    ...
    routes it too â€” see ``configure_agent_harness_with_provider``). So a
...
def provider_families(entry: ProviderEntry) -> frozenset[str]:
    ...
    - ``subscription`` / ``cli-config``: derived from the CLI â€” ``claude``
    ...
    - ``databricks``: both families plus pi â€” ucode routes the Claude,
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is native-``agent-meow claude`` only â€” the in-process /
    ...
    if entry.kind in (KEY_KIND, GATEWAY_KIND, LOCAL_KIND):
        ...
        # the Gemini surface â€” only a ``key`` may. Stripping it here (rather than
        ...
        # pi consumes the anthropic / openai families only â€” a gemini-only key
    ...
    if entry.kind in (SUBSCRIPTION_KIND, CLI_CONFIG_KIND):
        ...
        if entry.cli == "codex":
            ...
            # KIND level â€” structurally, without reading the ambient
            # ~/.codex/config.toml â€” so ``provider_families`` stays pure (the
            ...
            # serves pi â€” a CLI login is unusable outside its own CLI.
            if entry.kind == CLI_CONFIG_KIND:
...
def first_available_provider(config: dict[str, object], family: str) -> ProviderEntry | None:
    ...
    include *family* regardless of default status â€” the credential a launch
...
def default_provider_for_harness(config: dict[str, object], harness: str) -> ProviderEntry | None:
    ...
    Maps the harness to its family (claude-sdk/native-claudeâ†’anthropic;
    codex/native-codex/openai-agentsâ†’openai) and returns that family's
    ...
    claude`` only â€” routing pi to either fails). A ``cli-config`` default is
    ...
    if explicit is not None:
        return explicit
    # Fall back across the pi-capable surfaces â€” prefer anthropic's default,
    ...
    for fam in _PI_FALLBACK_FAMILIES:
        ...
        # raises for it). Neither can serve pi, so skip them and fall through â€”
...
def surface_default_model(entry: ProviderEntry, surface: str) -> str | None:
    ...
    :data:`PI_SURFACE` surface â€” pi consumes whichever family supplies its
    credential, anthropic preferred (see :func:`_family_for_harness`) â€”
    ...
    :returns: The default model id, e.g. ``"claude-sonnet-4-6"``, or
        ...
        ``subscription`` / ``databricks`` kinds, always â€” the CLI /
...
def _family_for_harness(provider: ProviderEntry, harness: str) -> str | None:
    ...
    Uses the canonical harnessâ†’family map; for the ``pi`` harness (which
...
def describe_active_credential(
    ...
    defaults â€” see :func:`default_provider_for_harness`), resolves the
...
def set_default_provider(
    ...
    only that family/those families from every other provider's default â€”
    so a Claude (``anthropic``) default and a Codex (``openai``) default
    ...
    ``providers:`` block â€” the caller writes the return value back wholesale.
    ...
    :param family: The single scope to make the default for â€” ``"anthropic"``,
...
def provider_entry_settings(
    ...
    is set, the entry carries ``default: true`` â€” but the caller must still
