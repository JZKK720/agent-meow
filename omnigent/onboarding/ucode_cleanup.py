...
- **Codex < 0.134.0 (ucode's legacy layout):** ucode deep-merges into the
  real ``~/.codex/config.toml`` â€” a top-level ``profile = "ucode"`` plus
  ``[profiles.ucode]`` and ``[model_providers.ucode-databricks]`` tables â€”
  which makes every *bare* ``codex`` run (and agent-meow' inner codex, which
...
and files â€” identified by ucode's fixed names (the ``ucode`` profile, the
``ucode-databricks`` model provider, the ``UCODE_WEB_SEARCH_MODEL`` env
marker) â€” and leave everything user-owned untouched. Surgical key removal
...
# ucode's ``agents/claude.py``) â€” used to tell ucode's entry apart from a
...
def strip_ucode_codex_config(config_path: Path | None = None) -> bool:
    ...
    ``model_providers`` parent tables are dropped too. Everything else â€”
    the user's own keys, tables, comments, and formatting â€” is preserved
    ...
    :returns: ``True`` when the file existed, contained ucode-managed keys,
        ...
        strip (missing file or no ucode keys â€” the file is left untouched).
    :raises OmnigentError: If the file exists but is not valid TOML â€”
...
def remove_ucode_sidecars(sidecar_paths: list[Path] | None = None) -> list[Path]:
    ...
    deletion is unconditional â€” no ownership check needed. Missing files
    are skipped.
...
def _remove_web_search_via_claude_cli() -> bool:
    ...
    Removal goes through the claude CLI â€” the same interface ucode used to
    register the entry â€” rather than editing ``~/.claude.json`` directly,
...
def remove_ucode_web_search_mcp(claude_user_config_path: Path | None = None) -> bool:
    ...
    ``web_search`` entry exists AND is ucode's â€” its ``env`` carries
    ``UCODE_WEB_SEARCH_MODEL``, or its command resolves to a ``ucode``
    binary â€” so a ``web_search`` server the user registered themselves is
    ...
    :returns: ``True`` when a ucode-owned entry was found and removed;
        ``False`` otherwise â€” no entry, an entry the user owns, an
