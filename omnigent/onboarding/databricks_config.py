...
def normalize_workspace_url(raw: str) -> str:
    ...
    carries a path and query the workspace host does not â€” e.g.
    ...
    :returns: ``scheme://host`` with no path, query, fragment, or trailing
        ...
        slash â€” matching the prior ``rstrip("/")`` behavior so callers that
...
# make the line's width â€” and therefore its ruff formatting â€” depend on
...
def databricks_sdk_installed() -> bool:
    ...
    The SDK is not part of the default install â€” it ships in the
...
# endpoint name â€” the gateway rejects Anthropic-direct ids like the CLI's
# own ``opus[1m]`` default.
...
def get_workspace_url_for_profile(profile: str) -> str | None:
    ...
    :mod:`configparser`, then falls back to agent-meow' built-in setup
