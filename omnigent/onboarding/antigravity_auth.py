...
resolved with the shared :func:`resolve_secret`. A dedicated block â€” not the
global ``auth:`` block, which the other SDK harnesses inherit â€” keeps a Gemini
...
# ``AIzaSyâ€¦``); newer Google API keys start with ``AQ``. Used for a *soft* paste
# check â€” a non-matching key may be forced through, so a prefix change never
# locks anyone out.
...
def looks_like_gemini_api_key(value: str) -> bool:
    ...
    :param value: A pasted candidate, e.g. ``"AIzaSyAbC123"`` or ``"AQâ€¦"``.
...
def antigravity_api_key_configured(config: dict[str, object] | None = None) -> bool:
    ...
    ``True`` only when the reference resolves â€” a dangling reference reads as
