"""
Databricks profile onboarding for agent-meow.
...
class ProfileSpec:
    """
    One Databricks profile agent-meow depends on.
...
# â”€â”€ Aliasing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _alias_source_for(host: str, target: str, existing: dict[str, str]) -> str | None:
    ...
    profile sections pointing at the same workspace share the login â€”
...
def _alias_profile(source: str, target: str) -> None:
    ...
    the file (which would be a programmer error â€” callers must check
...
def _remove_profile_section(name: str) -> bool:
    ...
    when ``name`` already exists with a different host â€” the CLI
...
# â”€â”€ Compute actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _compute_actions(existing: dict[str, str]) -> _Actions:
    ...
    for spec in DEFAULT_PROFILES:
        ...
        if host is not None and _host_matches(host, spec.host):
            ...
            continue
        if host is not None:
            # Profile name matches but the host doesn't â€” needs an
...
# â”€â”€ OAuth login â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _login_profile(cli: str, spec: ProfileSpec, console: Console) -> bool:
    ...
    opens on the same ``localhost:8020`` redirect URI â€” without this
    ...
    """
    console.print(f"\n  [bold]â†’ {spec.name}[/bold]  [dim]{spec.purpose}[/dim]")
    ...
    if result.returncode != 0:
        ...
        return False
    console.print(f"  [green]âœ“ {spec.name}[/green]")
    time.sleep(_OAUTH_BETWEEN_LOGIN_SLEEP_SECONDS)
    return True


# â”€â”€ Top-level entry points â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            "[bold]agent-meow onboarding[/bold]\n\n"
            "Setting up the Databricks profiles agent-meow needs "
    ...
    if actions.aliasable:
        ...
        if n:
            console.print(f"\n  [green]âœ“ aliased {n} existing profile(s)[/green]")
        existing = _existing_profile_hosts()
        actions = _compute_actions(existing)
...
def _handle_wrong_host(cli: str, spec: ProfileSpec, current_host: str, console: Console) -> None:
    ...
    (leave the user's profile alone â€” they'll need to rename it manually
    ...
    if not sys.stdin.isatty():
        console.print(
            f"  [dim](non-TTY â€” skipping. Rename your existing `{spec.name}` and re-run.)[/dim]"
        )
        return
    try:
...
def _derive_workspace_profile_name(workspace_url: str, existing: dict[str, str]) -> str:
    ...
    Resolution order. Reusing an *existing* login is the priority â€” the
    whole point of the flow is to avoid extra browser sign-in windows â€” so
    ...
    1. An existing profile in ``~/.databrickscfg`` already pointing at this
       host â€” reuse its name, so the host-keyed OAuth token cache is reused
    ...
    2. A bundled :data:`DEFAULT_PROFILES` spec whose host matches â€” use its
       canonical name (e.g. the OSS gateway URL â†’ ``"oss"``) when the user
    ...
    3. Otherwise derive a readable name from the workspace host's first DNS
       label, e.g. ``https://my-ws.cloud.databricks.com`` â†’ ``"my-ws"``.
    ...
    # the OSS build â€” without it there are no bundled canonical names and
...
def login_databricks_workspace(workspace_url: str, *, console: Console | None = None) -> str:
    ...
    points at a *different* host, that stale section is dropped first â€” the
    ...
    if cli is None:
        ...
        raise click.ClickException(
            "`databricks` CLI not on PATH â€” required to authenticate the workspace.\n"
            f"  Install with:\n    {installer}"
        )
    ...
    if current_host is not None and _host_matches(current_host, workspace_url):
        console.print(f"  [green]âœ“ using existing Databricks profile `{profile}`[/green]")
        return profile
...
def maybe_run_onboarding() -> None:
    ...
    - All three configured â†’ no-op.
    - Some same-host profiles exist under different names â†’ silently
      alias, print one-line note, no prompt.
    - Some still need OAuth â†’ ``Y/n`` prompt; on ``Y`` delegate to
      :func:`run_onboarding`.

    Skipped when ``$OMNIGENT_SKIP_ONBOARD`` is set or stdin isn't a
    TTY (CI / piped input â€” can't safely launch OAuth).
    """
    if os.environ.get(SKIP_ENV_VAR):
        return
    if not sys.stdin.isatty():
    ...
    console.print(
        f"\n  [yellow]agent-meow needs Databricks profiles: {', '.join(missing_parts)}[/yellow]"
    )
    try:
