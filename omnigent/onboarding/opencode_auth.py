...
Like :mod:`~?omnigent.onboarding.goose_auth`, agent-meow stores **no** OpenCode
...
(``OPENAI_API_KEY`` / ``ANTHROPIC_API_KEY`` / â€¦). This module is a thin,
read-only reporter so ``agent-meow setup`` can show which providers OpenCode can
reach and offer to run its native login â€” without ever touching its secrets.

It reads ``auth.json`` directly (a JSON object keyed by provider id â€” see
...
# Common OpenCode providers â†’ (provider id, display label, env var). The
...
class OpenCodeAuthSummary:
    ...
    def describe(self) -> str:
        ...
        return " Â· ".join(parts) if parts else "no provider configured yet"
