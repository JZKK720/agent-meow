...
_DIM = "\033[90m"
_BOLD = "\033[1m"
# Brand accent â€” Otto's magenta-pink (#F43BA6), matching omnigent.inner.ui so
...
def _prompt_global_auth() -> tuple[dict[str, str], None] | tuple[None, None]:
    ...
    - ``api_key`` â€” direct OpenAI-compatible bearer token. Prompts for
      the actual key value (not an env-var reference) and an optional
      custom endpoint URL.
    - ``databricks`` â€” Databricks profile from ``~/.databrickscfg``.
        ...
        try:
            if sub == 0:
                ...
                if profiles:
                    ...
                    db_label = (
                        f"Databricks\n"
                        f"        {_DIM}no profiles in ~/.databrickscfg â€” "
...
def _store_default_config(yaml_path: Path, supervisor: _SupervisorConfig | None = None) -> None:
    ...
    console.print(f"  [green]âœ“ stored default_agent in {_GLOBAL_CONFIG_PATH}[/green]")
...
def _finish_new_setup(
    ...
    console.print(
        ...
        "  [dim]See examples:[/dim] omnigent/examples/ in the repo, or agent-meow run --help\n"
    )
...
def run_wizard_and_launch() -> None:
    ...
    1. **Server URL** â€” the agent-meow server to connect to (optional;
       blank means run locally).
    2. **Auth** â€” ``api_key`` (bearer token + optional base URL) or
    ...
    3. **Agent YAML** â€” path to the agent spec file that becomes
    ...
    # â”€â”€ Step 1: Server URL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _section()
    console.print("  [bold]Step 1 / 3 â€” Server URL[/bold]")
    server_url = _prompt_server_url(existing_server)
    if server_url:
        save_settings["server"] = server_url

    # â”€â”€ Step 2: LLM executor auth â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    _section()
    console.print("  [bold]Step 2 / 3 â€” LLM auth[/bold]")
    ...
    # â”€â”€ Step 3: Default agent YAML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _section()
    console.print("  [bold]Step 3 / 3 â€” Default agent YAML[/bold]")
    console.print()
    if existing_agent:
        ...
        console.print(
            "  [dim]Leave blank to skip â€” run ``agent-meow run <yaml>`` directly later.[/dim]"
        )
        console.print()
    try:
    ...
    if save_settings:
        ...
        console.print(f"  [green]âœ“ Config saved to {_GLOBAL_CONFIG_PATH}[/green]")
    else:
        console.print("  [dim]No changes â€” config unchanged.[/dim]")
    console.print()
