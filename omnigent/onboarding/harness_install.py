...
# ``~/.config/goose/config.yaml``) with no agent-meow-managed credential, and ships
...
# agent-meow-managed credentials). The ``hermes`` binary must be on PATH.
HERMES_KEY = "hermes"
...
def harness_setup_hint(harness: str | None) -> str:
    ...
    model credential, both of which ``agent-meow setup`` handles â€” so they route
...
def harness_cli_installed(key: str) -> bool:
    ...
    ucode and the npm install-prompt UX â€” even though the SDK-based
...
def harness_install_command(key: str) -> list[str]:
    """Return the argv that installs the harness CLI, e.g. ``npm install -g â€¦``.
