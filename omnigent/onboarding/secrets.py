...
- **A ``0600`` JSON file** at ``<config_home>/secrets.json`` (a flat
  ...
  unlocked keyring backend). The file is created with â€” and re-chmod'd
  to â€” mode ``0600`` so other users on a shared host cannot read it.
...
:mod:`~?omnigent.onboarding.provider_config` â€” that module imports *this*
...
# request" (locked / headless / no backend) â€” we fall back to the file
...
def active_backend() -> str:
    ...
    would succeed â€” a transient keyring failure falls back to the file
...
def store_secret(name: str, value: str) -> None:
    ...
    if _use_keyring():
        try:
            ...
            # (locked, headless, no backend) â€” fall through to the file.
            pass
