...
Unlike :mod:`~?omnigent.onboarding.cursor_auth`, agent-meow manages **no** Goose
credentials: Goose owns its own auth via ``goose configure`` (keyring or
``~/.config/goose/config.yaml``). This module is a thin, read-only reporter â€”
...
across platforms and config formats â€” ``goose configure`` may store provider/key
...
def goose_config_summary() -> GooseConfigSummary:
    ...
    2. ``goose info -v`` â€” Goose's own resolved config (handles the keyring /
