...
backend, authenticated by a **GitHub token** â€” never the Databricks AI gateway.
...
providers' secrets â€” in the agent-meow secret store (OS keychain, else a ``0600``
JSON file; see :mod:`~?omnigent.onboarding.secrets`) â€” and referenced from a
...
SDK harnesses inherit when their spec declares no auth â€” a Copilot token parked
...
# GitHub token is stored â€” stable so the setup flow and the resolver agree.
...
# Token-shape prefixes Copilot accepts. The check is deliberately *soft* â€” a
# user may force a non-matching value through â€” so a future prefix change can
...
# imported as ``copilot``) â€” not in the default install, so the ``copilot:``
...
# ``ANTIGRAVITY_EXTRA``. The name carries literal brackets â€” markup-rendered
...
def resolve_copilot_github_token(config: dict[str, object] | None = None) -> str | None:
    ...
    env var) returns ``None`` so the caller â€” the copilot spawn-env builder and
    the setup readout â€” can fall back to an inherited ``GH_TOKEN`` instead of
    crashing a run.
...
def copilot_github_token_configured(config: dict[str, object] | None = None) -> bool:
    ...
    resolves â€” a dangling reference reads as not-configured so the setup readout
