...
public URL (``sandbox.get_host(port)``) â€” there is no localâ†’sandbox
path â€” so the in-sandbox Databricks App OAuth flow doesn't apply and
...
- **Templates, not registry images.** Unlike every other launcher, E2B
  cannot boot an arbitrary registry image at create time â€” it boots from
  a pre-built E2B *template*. The agent-meow host image must therefore be
  ...
  ``template`` field names that template â€” it is NOT a
...
- **API-key auth.** ``E2B_API_KEY`` is read from the CLI/server process
  environment by the SDK, 12-factor â€” like the other providers' keys.
"""
...
"""Environment variable overriding :data:`DEFAULT_E2B_TEMPLATE` â€” the
...
values are injected into every sandbox this launcher creates â€” typically
the harness LLM credentials (``ANTHROPIC_API_KEY``, ``OPENAI_API_KEY``,
gateway base URLs, â€¦) and ``GIT_TOKEN`` that the in-sandbox host forwards
...
per account â€” 24 h on Pro, 1 h on Hobby â€” and the SDK default is only
...
# doesn't state the limit â€” E2B's Hobby maximum (1 h).
...
def _is_missing_template_error(message: str) -> bool:
    ...
    ``SandboxException`` â€” ``"404: template '<name>' not found"`` â€” NOT a
...
def _template_build_hint(template: str, exc: Exception) -> click.ClickException:
    ...
    step â€” for both a missing template and an incompatible one.
    ...
    return click.ClickException(
        ...
        "incompatible. Build the agent-meow host image into an E2B template first "
        "(`e2b template build` â€” see deploy/e2b/README.md), or set the correct "
...
class _E2BRemoteProcess(RemoteProcess):
    ...
    queue â€” the queue is the combined-output stream the
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            # a transport error â€” CommandExitException IS a CommandResult,
...
class E2BSandboxLauncher(SandboxLauncher):
    ...
    # E2B exposes sandbox ports OUTWARD only (get_host(port) â†’ public
    # URL); there is no localâ†’sandbox path for the App OAuth callback.
    ...
    def __init__(self, *, template: str | None = None, env: Sequence[str] | None = None) -> None:
        ...
        :param template: Optional E2B template NAME (or id) to provision
            sandboxes from â€” the server's managed-host
            ``sandbox.e2b.template`` config. This is an E2B template the
            agent-meow host image was built into (``e2b template build``),
        ...
        :param env: Optional names of server-process environment
            ...
            ``["OPENAI_API_KEY", "GIT_TOKEN"]`` â€” the server's
    ...
    def _resolve(self, sandbox_id: str) -> Sandbox:
        ...
        # preflight â€” keep the missing-SDK error the friendly install hint.
        _ensure_sdk()
        ...
        if handle is None:
            try:
                ...
                raise click.ClickException(
                    f"E2B sandbox '{sandbox_id}' not found â€” it may have passed its "
    ...
    def _resolve_sandbox_env(self) -> dict[str, str]:
        ...
        :returns: Name â†’ value mapping for ``Sandbox.create(envs=â€¦)``.
    ...
    def provision(self, name: str) -> str:
        ...
        click.echo(f"â–¸ Creating E2B sandbox '{name}' from template '{template}'")
        sandbox = self._create_sandbox(template, resolve_max_lifetime_s(), name, env_vars)
        sandbox_id = sandbox.sandbox_id
        self._sandboxes[sandbox_id] = sandbox
        click.echo(f"  â†’ created {sandbox_id}")
        return sandbox_id
    ...
    def _create_sandbox(
        ...
        try:
            ...
            # which does NOT extend SandboxException â€” catch it explicitly so
            # it surfaces as a credential hint instead of escaping raw.
            raise click.ClickException(
                "E2B authentication failed â€” check E2B_API_KEY (create a key at "
            ...
            if cap is not None and cap < timeout:
                pass  # account-cap rejection â†’ fall through to the clamp-retry
            elif _is_missing_template_error(str(exc)):
                # The most common first-run failure: the host image was never
                # built into an E2B template (E2B returns "404: template â€¦ not
                # found" as a plain SandboxException). Point at the build step.
                raise _template_build_hint(template, exc) from exc
            else:
                # SDK boundary: surface the provider's reason (quota, â€¦) as the
        ...
        # rather than clamping â€” retry once at the cap.
        ui.console.print(
            f"  â†’ requested {timeout // 3600}h lifetime exceeds this E2B account's "
    ...
    def attach(self, sandbox_id: str) -> None:
        ...
        """
        click.echo(f"â–¸ Reusing existing E2B sandbox '{sandbox_id}'")
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        try:
            ...
            ui.console.print(
                f"  â†’ warning: could not extend the lifetime of '{sandbox_id}' "
            ...
            # create, which rejects it â€” verified live on a capped account),
            # so report the REQUEST rather than claim a grant.
            click.echo(
                f"  â†’ requested a {lifetime // 3600}h lifetime extension "
    ...
    def stream_exec(self, sandbox_id: str, command: str, *, pty: bool = False) -> RemoteProcess:
        ...
        routes both into one queue, so the *pty* flag is unused â€” the
        ...
        handle = self._resolve(sandbox_id)
        try:
            # timeout=0 disables the SDK's default 60s per-command cap â€” this
