...
Supports both server-managed hosts (``host_type="managed"`` sessions â€”
``prepare`` / ``provision`` / ``run`` / ``terminate``) and the CLI
bootstrap flow (``agent-meow sandbox create`` / ``connect`` â€” file
...
local-to-sandbox port forwarding that Daytona doesn't have â€” the flow
...
- **No hard lifetime cap, but idle auto-stop.** Daytona stops sandboxes
  after 15 idle minutes BY DEFAULT â€” fatal for a session host that may
  sit between turns â€” so :meth:`DaytonaSandboxLauncher.provision`
...
- **No inbound port forwarding.** Daytona preview links expose sandbox
  ports publicly but provide no localâ†’sandbox path, so
...
launcher creates â€” typically the harness LLM credentials
(``ANTHROPIC_API_KEY``, ``OPENAI_API_KEY``, gateway base URLs, â€¦) and
...
# agent-meow host must survive arbitrary idle gaps between turns, so
...
# a deletion another cleanup path already started). 3 attempts Ã— 2 s
...
def _drive_foreground_pty(pty: PtyHandle, sandbox_id: str, command: str) -> int:
    ...
    if result.exit_code is None:
        ...
        # before the close frame carried an exit code â€” fail loud
...
class DaytonaSandboxLauncher(SandboxLauncher):
    ...
    # Daytona preview links are sandboxâ†’public only; there is no
    # localâ†’sandbox path for the App OAuth callback port.
    ...
    def __init__(self, *, image: str | None = None, env: Sequence[str] | None = None) -> None:
        ...
        :param image: Optional registry image reference to provision
            ...
            â€” the server's managed-host ``sandbox.daytona.image``
    ...
    def _daytona(self) -> daytona_sdk.Daytona:
        ...
        ``DAYTONA_TARGET`` from the process environment â€” the same
    ...
    def _resolve(self, sandbox_id: str) -> DaytonaSandbox:
        ...
        # preflight â€” ensure the missing-SDK error stays the friendly
        ...
        if handle is None:
            ...
            try:
                ...
                raise click.ClickException(
                    f"Daytona sandbox '{sandbox_id}' not found â€” it may have "
    ...
    def _resolve_sandbox_env(self) -> dict[str, str]:
        ...
        :returns: Name â†’ value mapping for ``env_vars`` at creation.
    ...
    def provision(self, name: str) -> str:
        ...
        click.echo(f"â–¸ Creating Daytona sandbox '{name}' from {resolved_ref}")
        try:
    ...
    def attach(self, sandbox_id: str) -> None:
        ...
        a stopped Daytona sandbox can be restarted â€” e.g. one created
        outside this flow whose idle auto-stop kicked in â€” so attach
        ...
        """
        click.echo(f"â–¸ Reusing existing Daytona sandbox '{sandbox_id}'")
        ...
        try:
            ...
            if handle.state != daytona.SandboxState.STARTED:
                click.echo(f"  â†’ starting sandbox (state: {handle.state})")
                handle.start()
        except daytona.DaytonaError as exc:
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        try:
            ...
            ui.console.print(
                f"  â†’ warning: could not disable idle auto-stop on "
            ...
            click.echo("  â†’ idle auto-stop disabled (sandbox lives until deleted)")
    ...
    def terminate(self, sandbox_id: str) -> None:
        ...
        longer exists is treated as success â€” the desired end state
        holds. A delete that races another state change (Daytona
        reports ``DaytonaConflictError: Sandbox state change in
        progress`` â€” observed live when two cleanup paths overlap) is
        ...
        # not an agent-meow dependency â€” pulling it in for a 3-iteration
