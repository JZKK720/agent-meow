"""
Provider-agnostic sandbox bootstrap for agent-meow hosts.

Composes a :class:`~?omnigent.onboarding.sandboxes.base.SandboxLauncher`
into the full host-bootstrap flow: build the agent-meow wheels locally,
...
single-use â€” laptop and sandbox can't both hold the same one. Logging
...
# â”€â”€ Wheel build â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def build_wheels(
    ...
    *,
    ...
    """
    Build the agent-meow wheels and pack them into a single tarball.
    ...
    click.echo(f"â–¸ Building agent-meow wheels â†’ {tgz_path}")
    env = os.environ.copy()
    ...
    click.echo(f"  â†’ packed {wheel_count} wheels at {tgz_path}")
...
def _uv_build_wheel(
    ...
    *,
    ...
    if result.returncode != 0:
        raise click.ClickException(f"`uv build` for {pkg} failed â€” see {build_log}")
...
# â”€â”€ Wheel install â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def ship_wheels(
    ...
    *,
    ...
    """
    Install agent-meow wheels into a sandbox.

    Performs three remote operations, in order: ship wheels.tgz â†’
    pip install (the launcher supplies the image-appropriate flags) â†’
    PATH-export persistence. Credentials are *not* shipped â€” the
...
# â”€â”€ App OAuth (minted inside the sandbox) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _read_login_url(stream: Iterable[str]) -> str | None:
    ...
    only evidence of why â€” swallowing it here would leave the user
    ...
    :returns: The authorize URL, or ``None`` when the stream ends
        without printing one â€” which is NOT necessarily an error:
...
def _probe_server(server_url: str) -> httpx.Response | None:
    ...
    :returns: The probe response, or ``None`` when the server is
        unreachable â€” the caller treats that as "shape unknown" and
...
class DerivedWorkspace:
    ...
    probes of an agent-meow server URL.
...
def derive_workspace(server_url: str) -> DerivedWorkspace | None:
    ...
    performs â€” but from the LOCAL machine. Two consumers: the in-sandbox
...
def login_app_oauth_in_sandbox(
    ...
    **inside the sandbox**, driving the browser step from the local
    ...
    Databricks Apps front their HTTP/WS endpoints with an OAuth edge â€”
    ...
    The sandbox ends up the sole holder of its OAuth grant â€” nothing is
    shipped from the laptop â€” which sidesteps CLI-version cache-format
    ...
    matter: the image's baked PAT must go â€” host-keyed credential
    resolution prefers a host-matching cfg credential, and the Apps
    edge 302s PATs, so it shadows the OAuth grant the login mints â€”
    ...
    :param server_url: agent-meow server URL to log in to, e.g.
    ...
    :param workspace: The workspace fronting *server_url*, from
        ...
        threads it down â€” to here for the cfg seed, and to the lakebox
        launcher for its local workspace pin). ``None`` means the
        server is not Databricks-fronted â€” the cfg seed is skipped and
    ...
    """
    if skip:
        click.echo("â–¸ Skipping the in-sandbox server login")
        return
    # Fail fast for providers that can't bridge the OAuth callback port
    # (e.g. Modal) â€” BEFORE validating flags or touching the sandbox, so
    ...
    if server_url is None:
        raise click.ClickException(
            "The in-sandbox login needs the server URL â€” pass --server, or --no-auth to skip."
        )

    click.echo(f"â–¸ Logging sandbox '{sandbox_id}' in to {server_url}")
    if workspace is not None:
        # Reset ~/.databrickscfg to exactly one [DEFAULT] entry shaped
        # like what `databricks auth login` itself writes (host +
        # auth_type + workspace_id) â€” see the docstring for why the
        ...
        click.echo(f"  â†’ seeding ~/.databrickscfg with workspace {workspace.host}")
...
def _complete_browser_login(
    ...
    â€” success vs. failure is then read from the exit code alone.
    ...
    if url is None:
        ...
        # browser step â€” the exit code tells which).
        ...
        click.echo("  â†’ cached workspace credentials accepted; no browser login needed")
        return
    ...
    with launcher.forward_local_port(sandbox_id, port):
        click.echo("  â†’ Opening the OAuth URL in your browser. If it doesn't open, visit:")
        click.echo(f"    {url}")
        click.echo(
            "  â†’ On a headless host (Arca)? Forward the callback port to your "
...
# â”€â”€ Host registration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def set_sandbox_host_name(launcher: SandboxLauncher, sandbox_id: str, host_name: str) -> None:
    ...
    The host's ``host_id`` is preserved across the edit â€” only the
    ...
    """
    click.echo(f"  â†’ setting host name to '{host_name}' in ~/.omnigent/config.yaml")
...
def connect_sandbox_host(
    ...
    *,
    ...
    The remote command holds a WebSocket open until interrupted â€”
    ...
    â€” it resolves credentials itself, via a stored
    ...
    primary-keyed on ``(owner, name)`` â€” collisions overwrite each
    other and (with existing conversations) FK-violate. Pick a unique
    name per sandbox to avoid the clash.

    :param launcher: The provider's launcher.
    :param sandbox_id: Target sandbox.
    :param server_url: agent-meow App URL the runner registers with.
    ...
    """
    click.echo(f"â–¸ Registering sandbox '{sandbox_id}' as a host with {server_url}")
    if host_name is not None:
        set_sandbox_host_name(launcher, sandbox_id, host_name)
    click.echo("  â†’ running `agent-meow host` in the sandbox (Ctrl-C to detach)")
...
# â”€â”€ High-level orchestrator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def bootstrap_sandbox_host(
    ...
    *,
    ...
    Six steps: provider preflight â†’ provision or attach sandbox â†’
    keep-alive â†’ build wheels â†’ ship wheels â†’ ``agent-meow login``
    inside the sandbox.
    ...
    :param sandbox_name: Label for a new sandbox (ignored when
        *sandbox_id* is set).
    :param server_url: agent-meow server URL the sandbox logs in to.
    ...
    :raises SandboxCapabilityError: Immediately (before any remote
        ...
        the OAuth callback port â€” pass ``skip_auth`` for such
    ...
    click.echo(f"  â†’ sandbox_id={sandbox_id}")
