...
- one row per option (selected â†’ ``â¯  <label>`` in bold accent, others
  normal weight; non-selectable sub-lines dim italic and indented beneath),
- a muted footer hint line (``â†‘/â†“ move  Â·  Enter select  Â·  Esc back``),
...
reusable version. The duplication is intentional for now â€” a future
...
def clear_screen() -> None:
    ...
    # the cursor â€” together they wipe the leaked subprocess output.
...
def _render_menu(
    ...
    *,
    ...
    A bold accent header, one row per option (selected â†’ bold accent with the
    ``â¯`` pointer, others normal weight aligned under it), an optional dim
    ...
    ``default: â€¦`` summary) with a blank line after â€” no ``â¯`` pointer, and
    â†‘/â†“ skip them so the cursor only lands on real choices.
    ...
    :param status: Optional transient status line (e.g. ``"âœ“ added X"``)
        rendered green above the title. Being part of the frame, it is
        erased with the frame on ``clear_on_exit`` â€” so a re-rendering loop
    ...
    if win_start > 0:
        render_console.print(Text.from_markup(f"       [{MUTED}]â†‘ {win_start} more[/]"))
    ...
    for i in range(win_start, win_end):
        ...
        if not selectable[i]:
            ...
            # "+N more" summary): indented, no pointer. â†‘/â†“ skip them. Their
            # styling follows the cursor â€” the label's own emphasis (e.g. a
            ...
            # real choice, or the menu ends) â€” so consecutive sub-lines stay
            # together and the gap separates one choice's block from the next.
            if i + 1 >= len(options) or selectable[i + 1]:
                render_console.print()
        elif i == selected:
            # Highlighted choice: bold accent with the â¯ pointer.
            last_choice = i
            render_console.print(Text.from_markup(f"    [bold {ACCENT}]â¯  {label}[/]"))
        else:
    ...
    if win_end < n_options:
        render_console.print(Text.from_markup(f"       [{MUTED}]â†“ {n_options - win_end} more[/]"))
    ...
    hint = (
        "â†‘/â†“ nav  Â·  Enter select  Â·  Esc exit"
        if compact
        else "â†‘/â†“ move  Â·  Enter select  Â·  Esc back"
    )
...
def _step_selectable(selectable: list[bool], start: int, step: int) -> int:
    ...
    Skips non-selectable (header/separator) rows so â†‘/â†“ glide over them.
...
def _select_fallback(
    ...
    *,
    ...
    # Map each printed number â†’ original options index; print headers as
    ...
    while True:
        ...
        # "q" is the fallback's abort, mirroring Esc on the TTY path â€” it
...
def select(
    ...
    *,
    ...
    On a TTY this draws the menu via raw termios (accent ``â¯`` pointer,
    dimmed others, footer hints) and redraws in place on â†‘/â†“. Enter
    ...
    â†‘/â†“ skip over (e.g. ``"Claude"`` / ``"Codex"`` labels above their
    ...
    :param selectable: Optional parallel ``bool`` mask; ``False`` marks a
        row as a non-selectable header/separator. ``None`` â†’ all rows
        selectable.
    :param clear_on_exit: When ``True`` (TTY only), erase the rendered menu
        frame on return instead of leaving it in the scrollback â€” so a
    ...
    :param compact: When ``True`` (TTY only), render the dense top-level
        ...
        the footer reads ``nav Â· select Â· Esc exit`` (Esc exits rather
    ...
    :returns: The chosen zero-based index into *options* (always a
        selectable row), or ``-1`` when the user aborts â€” Esc / Ctrl-C /
    ...
    try:
        ...
        # Cannot enter raw mode â€” degrade to the numbered fallback.
        return _select_fallback(title, options, default=default, selectable=mask)
    ...
    try:
        ...
        while True:
            ...
            if ch in (b"\x03", b"\x04"):
                # Ctrl-C / Ctrl-D â€” abort the menu.
                cancelled = True
                break
            if ch == b"\x1b":
...
def prompt_text(
    ...
    *,
    ...
    if received_hidden_input and value:
        ...
        console.print(f"  [{MUTED}]âœ“ received ({count} {unit})[/{MUTED}]")
