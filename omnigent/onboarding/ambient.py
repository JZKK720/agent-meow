...
discovers credentials a user already has â€” vendor API keys in the
environment, a logged-in ``claude`` / ``codex`` CLI, or a local Ollama
server â€” so the setup flow can offer them as one-tap choices instead of
...
so on macOS â€” and only when the file check comes up empty â€” Claude detection
...
# ``amazon-bedrock`` resolves auth from the AWS environment â€” none of them is
...
# Timeout (seconds) for the Ollama TCP probe â€” short so setup stays snappy
...
# ``family=None`` â€” their key is detected but no harness surface is
...
class DetectedProvider:
    ...
    :param name: The provider/source name, e.g. ``"anthropic"``,
        ...
        ``"codex"``, or ``"ollama"``.
    :param kind: How the credential authenticates â€” ``"key"`` (an API key
...
def codex_auth_has_credential(auth_path: Path) -> bool:
    ...
    object whose fields are *all* optional, so an empty ``{}`` â€” or a file left
    behind after a logout â€” parses cleanly while representing **no** usable
    ...
    - ``OPENAI_API_KEY`` â€” a non-empty string (``auth_mode: "apikey"``);
    - ``tokens.access_token`` or ``tokens.refresh_token`` â€” a non-empty string
      (``auth_mode: "chatgpt"``; a refresh token alone suffices because the
      CLI mints a fresh access token from it);
    - ``personal_access_token`` â€” a non-empty string (enterprise / external
      token integrations).

    The check is purely local (no network), so it cannot detect a
    *present-but-expired* OAuth access token â€” but the Codex CLI refreshes
    ...
    try:
        ...
        # Missing or unreadable file â€” no login.
        return False
    try:
        data = json.loads(raw)
    except json.JSONDecodeError:
        # Malformed JSON â€” treat as no usable login rather than crash.
        return False
...
def _provider_table_has_self_contained_auth(table: dict[str, object]) -> bool:
    ...
    provider from the config table alone â€” no ``auth.json`` login and no
    ...
    - ``[X.auth]`` â€” a token-printing command (``command`` + ``args``), the
      shape ``isaac configure codex`` writes (``jq`` reading a cached
      Databricks Model Serving token);
    - ``experimental_bearer_token`` â€” an inline static bearer token;
    - ``[X.aws]`` â€” AWS SigV4 request signing (Bedrock-style);
    - ``http_headers`` containing an ``Authorization`` header â€” a static
      auth header.

    Deliberately **excluded** (each with a reason):

    - ``env_key`` / ``env_http_headers`` â€” auth via an environment
      ...
      design first.
    - ``requires_openai_auth = true`` â€” the provider rides the ChatGPT
...
def codex_config_custom_provider(config_path: Path) -> CodexConfigProvider | None:
    ...
    nothing â€” this is the detection for that state.
    ...
    never validates that the auth command actually yields a live token â€”
    ...
    try:
        ...
        # Missing or unreadable file â€” nothing configured.
        return None
    try:
        config = tomllib.loads(raw.decode("utf-8"))
    except (tomllib.TOMLDecodeError, UnicodeDecodeError):
        # Malformed TOML â€” treat as not configured rather than crash setup.
        return None
    ...
    if not isinstance(table, dict):
        # The config selects a provider it never defines â€” codex itself
        # would fail; nothing usable to adopt.
        return None
    if table.get("requires_openai_auth") is True:
        # Rides the ChatGPT login â€” the auth.json subscription detection's
...
class CodexConfigTransport:
    ...
    fields a harness needs to actually talk to the provider â€” the ones
...
def codex_config_provider_transport(
    ...
    user's Databricks AI Gateway) needs the *transport* â€” where to send
    requests and how to authenticate â€” not just the id. This parses the named
    ...
    ``None`` â€” rather than raising â€” for every "can't resolve" case so a caller
...
def codex_config_detection() -> DetectedProvider | None:
    ...
    The single constructor for this detection â€” used by
...
def claude_auth_has_credential(creds_path: Path) -> bool:
    ...
    from ``refreshToken``, so "expired" is the normal steady state â€” gating on
    ...
    server-side *revocation* â€” only the harness's own login attempt can. Its job
    ...
    if isinstance(refresh_token, str) and refresh_token.strip():
        return True
    # No refresh token â€” the access token must still be unexpired to be usable.
...
def _claude_login_detected() -> bool:
    ...
    Keychain** on macOS â€” so the fast, no-subprocess file check
    ...
    runs ``claude auth status`` â€” the command that reads wherever the CLI
    ...
    :returns: ``True`` when a usable Claude subscription login is present â€” via
...
def detect_providers() -> list[DetectedProvider]:
    ...
    2. A logged-in Claude CLI â€” ``~/.claude/.credentials.json`` carries a
    ...
    3. A custom, auth-carrying model provider in ``~/.codex/config.toml``
       (see :func:`codex_config_custom_provider`) â€” e.g. the Databricks AI
    ...
    4. A logged-in Codex CLI (``~/.codex/auth.json`` exists *and* carries a
       usable credential â€” see :func:`codex_auth_has_credential`).
    ...
    # 1b. Claude Code on Vertex AI â€” the CLI reads three env vars for GCP auth.
    ...
    # 2. Claude CLI login. Like codex (below), existence alone is not enough â€”
    ...
    #    writes config.toml only â€” never auth.json â€” so the login check
    ...
    # 4. Codex CLI login. Existence alone is not enough â€” an empty or
