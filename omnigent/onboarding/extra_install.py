...
* **``uv tool``** â€” the package lives in an isolated tool environment that
  ...
  git-source install (``uv tool install git+â€¦``) instead reinstalls from that
  same source (``uv tool install --force "agent-meow[extra] @ git+â€¦"``) so the
  tool is not silently re-pulled from PyPI.
* **``uv`` (non-tool)** â€” ``uv pip install "agent-meow[extra]"`` targets the
  active virtualenv.
* **``pip`` / fallback** â€” ``<sys.executable> -m pip install "agent-meow[extra]"``
...
def _installed_vcs_url() -> str | None:
    ...
    the URL to the ``git+â€¦`` form pip/uv accept back as an install target (and
...
def extra_install_command(extra: str) -> list[str]:
    ...
    1. ``uv tool`` install, git source â†’ ``uv tool install --force
       "agent-meow[extra] @ git+â€¦"`` (keeps the tool on its original source)
    2. ``uv tool`` install, registry â†’ ``uv tool install --with ...
       agent-meow --force``
    3. ``uv`` on PATH (non-tool) â†’ ``uv pip install "agent-meow[extra]"``
    4. fallback â†’ ``<sys.executable> -m pip install "agent-meow[extra]"``
