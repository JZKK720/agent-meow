...
Like :mod:`~?omnigent.onboarding.goose_auth`, agent-meow manages **no** Hermes
...
``~/.hermes/config.yaml``. This module is a thin, read-only reporter â€” it
...
Detection reads ``~/.hermes/config.yaml`` directly â€” the same user config the
native bridge copies forward in
:func:`~?omnigent.hermes_native_bridge._load_user_hermes_config`. A fresh install
ships ``model.provider: auto`` (auto-detect from credentials â€” nothing picked
...
Hermes' own error â€” the daemon's launch gate (:mod:`harness_readiness`)
deliberately fails open â€” so this reporter gates only on the picked provider,
...
#: ``hermes model`` yet â€” treated as not-configured for the setup overview.
_AUTO_PROVIDER = "auto"
...
def _model_section() -> dict:
    ...
    document / ``model`` value â€” this is read-only reporting and must never
    raise.
    """
