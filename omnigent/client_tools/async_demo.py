"""
Async-dispatch demo tool set for ``agent-meow run --tools async_demo``.

Ships one tool â€” ``slow_compute`` â€” registered as a client-side
...
synthesizes a ``function_call(action_required)`` SSE event â€” the
...
The tool itself is deliberately boring (sleep + echo) â€” the
...
which ``agent-meow run`` calls when ``--tools async_demo`` is
passed.
"""
...
# is no longer a ``synchronous`` property in the schema â€” async
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    """
    Execute a client-side tool call from ``agent-meow run``.
    ...
    :raises KeyError: If ``name`` is not ``"slow_compute"`` â€”
    ...
    # the SDK) â€” the event loop stays free to handle the rest
