...
# Reserved parameter name â€” kept in sync with
...
def main() -> None:
    ...
    if not tool_name:
        _write_error("Request missing 'tool_name' field â€” runner cannot dispatch.")
        return
...
def _resolve_tool_function(module: ModuleType, tool_name: str) -> Any:
    ...
    depth â€” prevents accidentally invoking a non-tool function
    with the same name).
...
def _invoke_tool(target: Any, arguments: dict[str, Any]) -> Any:
    ...
    already in a subprocess; blocking is fine here â€” the parent
...
def _serialize_result(target: Any, result: Any) -> str:
    ...
    :param target: The decorated function â€” its
    ...
    if return_annotation is not None:
        try:
            from pydantic import TypeAdapter  # local import â€” heavy module
