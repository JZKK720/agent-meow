...
isolation. Communication uses the fd 3 pipe protocol â€” see
...
1. **Container** â€” ``docker run`` or ``podman run`` with network
   disabled. Used when ``sandbox.container_image`` is configured.
   The runtime is selected via ``sandbox.container_runtime``.
2. **srt + uv** â€” ``srt uv run --with ... -- python _runner.py``.
   Used when srt is on PATH, sandbox enabled, and tool has PEP 723
   inline deps.
3. **srt** â€” ``srt python _runner.py``. Used when srt is on PATH
   and sandbox enabled.
4. **uv** â€” ``uv run --with ... -- python _runner.py``. Used when
   tool has PEP 723 inline deps and uv is available.
5. **plain** â€” ``python _runner.py``. Default fallback.
"""
...
# (strings, ints, nested objects, arrays) â€” no specific type fits.
...
class LocalPythonTool(Tool):
    ...
    def __init__(
        ...
        # Live subprocesses from any in-flight ``invoke()`` â€” tracked
    ...
    def _build_command(self, *, state_root: str | None) -> list[str]:
        ...
        :param state_root: Per-call ToolState directory (or ``None``
            ...
            right path â€” passing via ``self`` would race between
    ...
    def _build_uv_command(self, base: list[str]) -> list[str]:
        ...
        :param base: The base command ``[sys.executable, _runner]``
            (unused â€” replaced with ``python`` for uv).
    ...
    def _prepend_srt(
        ...
        *,
        ...
        Stateless tools use plain ``srt -c`` â€” srt's default
        ...
        ``code_sandbox`` uses â€” that config restricts reads too,
    ...
    def _build_container_command(self) -> list[str]:
        ...
        documents a caller-enforced invariant â€” a fail-loud
        ...
        assert image is not None, (
            "_build_container_command called without a container_image â€” "
...
def _write_srt_settings_file(state_root: str) -> str:
    ...
    shape matches srt's JSON schema â€”
