...
generate. They ship as schema-only :class:`~?omnigent.tools.base.Tool`
subclasses â€” the base-class ``invoke`` fails loud if the AP-side path
ever reaches them.

- ``video_generate`` â†’ calls a video-generation gateway (Pixelle-Video
...
- ``video_get`` â†’ ``GET /v1/sessions/{id}/resources/videos/{video_id}``
