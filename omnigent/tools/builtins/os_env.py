...
The agent-meow path can't reuse those Session tool registrations
directly: its
...
# deployments â€” copying these here is the cost of layering.
...
class _OSEnvBackedTool(Tool):
    ...
    def invoke(self, arguments: str, ctx: ToolContext) -> str:
        ...
        independent â€” tool dispatches don't share async state
        across calls.
        ...
        # payload â€” bubbling out of the tool call would make the
...
class SysOsReadTool(_OSEnvBackedTool):
    """``sys_os_read`` â€” read text from a file in the OS env."""
...
class SysOsWriteTool(_OSEnvBackedTool):
    """``sys_os_write`` â€” write a full file in the OS env."""
...
class SysOsEditTool(_OSEnvBackedTool):
    """``sys_os_edit`` â€” exact-text edit a file in the OS env."""
...
class SysOsShellTool(_OSEnvBackedTool):
    """``sys_os_shell`` â€” run a shell command in the OS env."""
...
def build_os_env_tools(os_env: OSEnvironment) -> list[Tool]:
    ...
    pass â€” keeps the tool list in one place so adding a new
    ...
    :param os_env: The agent's :class:`OSEnvironment`. All four
        returned tools share this instance â€” they MUST share
