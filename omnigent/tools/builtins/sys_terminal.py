...
- ``sys_terminal_launch`` â€” start a configured tmux session.
- ``sys_terminal_send`` â€” type text + key chords.
- ``sys_terminal_read`` â€” capture pane state (with optional scrollback).
- ``sys_terminal_list`` â€” enumerate the conversation's terminals.
- ``sys_terminal_close`` â€” kill a session and remove it.
...
Per ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.3, these tools register
...
Cwd resolution at launch follows the Â§4.6 precedence list â€” see
...
class _CloseFailed(Exception):
    """
    Internal flag for ``SysTerminalCloseTool`` â€” surfacing a tmux
...
class _ResolvedInstance:
    ...
    :param lock: The per-instance ``threading.Lock`` from
        ...
        to serialize concurrent tmux ops on the same instance â€”
        without it, a ``send(text=X, keys="Enter")`` call can
        interleave its 2 tmux subprocess invocations with another
        thread's send. See ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§9.1.
    """
...
def _materialize_terminal_spec_for_launch(
    ...
    set but its ``cwd`` is ``None``/``.``/``./`` â€” landing the tmux
    session in AP's process cwd, NOT the Â§4.6-resolved workspace.
    ...
    framework-resolved Â§4.6 path always wins over inner defaults,
    ...
    :param resolved_cwd: The Â§4.6-resolved cwd. ``None`` means no
        tier matched (e.g. test contexts with no workspace) â€” fall
    ...
    if not isinstance(ts_os_env, OSEnvSpec):
        # ``inherit`` sentinel or ``None`` â€” the inner builder will
    ...
    if _has_meaningful_cwd(ts_os_env.cwd):
        # Spec already names a real cwd â€” leave it alone.
        return terminal_spec
...
def _synthesize_parent_os_env(
    ...
    Applies the Â§4.6 cwd-resolution result to the spec's os_env so
    the inner :func:`build_terminal_os_env_spec` inherits the
    correct cwd through normal channels â€” without tripping the
    ``allow_cwd_override`` gate. The gate is for LLM-supplied
    per-call overrides; the framework's own Â§4.6 resolution is
    ...
    :param resolved_cwd: The cwd from Â§4.6 resolution, or ``None``
        if no tier matched (e.g. test contexts with no workspace).
        When ``None``, the function leaves the os_env alone â€” the
...
# â”€â”€ JSON Schemas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Mirror the legacy inner Session ``sys_terminal_*`` schemas so
# agents in the wild see an unchanged tool surface under agent-meow mode.
...
class SysTerminalLaunchTool(Tool):
    ...
    Looks up the terminal spec by name, applies the Â§4.6 cwd
    ...
    :param spec: The agent spec â€” used to look up the terminal's
    ...
    def __init__(
        ...
        :param spec: The agent spec â€” used to look up the terminal's
            :class:`TerminalEnvSpec` by name and to read the
            agent-level ``os_env`` for the Â§4.6 cwd-resolution
            fallback.
        :param registry: The shared :class:`TerminalRegistry`
    ...
    def invoke(self, arguments: str, ctx: ToolContext) -> str:
        ...
        :param ctx: Server-side context. ``conversation_id`` and
            ...
            workspace anchors the Â§4.6 cwd default).
        ...
        if isinstance(validated, str):
            # Validation hit an error envelope â€” already JSON-encoded.
            return validated

        # Â§4.6 cwd resolution. Two synthesis points feed the result
        ...
        # the terminal declares its own â€” so we can't just patch
        ...
        #   ``os.getcwd()`` â€” AP's process cwd â€” instead of the
        #   Â§4.6-resolved workspace.
        #
        # Neither path uses the inner builder's ``cwd_override``
        # parameter â€” that one enforces ``allow_cwd_override``,
    ...
    def _spawn_and_format(
        ...
        :param ctx: Tool execution context â€” supplies
        ...
        :param parent_os_env: Â§4.6-resolved parent os_env, or
        ...
        :param was_running: Snapshot of whether the registry
            ...
            launch attempt â€” drives the ``status`` field.
        ...
        # runner's tool dispatcher (not here) â€” ``session_stream`` has no
        ...
        try:
            return asyncio.run(
                self._registry.launch(
                    ...
                    # allow_cwd_override above). The Â§4.6
    ...
    def _resolve_cwd(
        ...
        """
        Apply the Â§4.6 cwd-resolution precedence.
        ...
        3. ``spec.os_env.cwd`` if it's set to a meaningful path
           (not ``None``, ``""``, ``"."``, or ``"./"``).
        4. ``ctx.workspace`` â€” the per-task workspace agent-meow creates
...
class SysTerminalSendTool(Tool):
    """
    ``sys_terminal_send`` â€” send text and key chords to a running terminal.
...
class SysTerminalReadTool(Tool):
    """
    ``sys_terminal_read`` â€” capture the visible pane and scrollback.
...
class SysTerminalCloseTool(Tool):
    """
    ``sys_terminal_close`` â€” kill a session and remove it from the registry.
    ...
    def invoke(self, arguments: str, ctx: ToolContext) -> str:
        ...
        def _do_close() -> bool:
            try:
                ...
                # gone â€” surface but don't propagate.
