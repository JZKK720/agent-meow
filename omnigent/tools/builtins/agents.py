...
each proxies the agent-meow server's existing REST endpoints over
...
:class:`~?omnigent.tools.base.Tool` subclasses â€” the base-class
``invoke`` fails loud if the AP-side path ever reaches them.

- ``sys_agent_get`` â†’ ``GET /v1/sessions/{id}/agent`` (a global read of
...
- ``sys_agent_list`` â†’ ``GET /v1/agents`` + ``GET /v1/sessions`` + a
...
class SysAgentGetTool(Tool):
    ...
    Requires a ``session_id`` â€” there is no way to inspect an agent that
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "guardrail policies. Global read â€” any session you can "
...
class SysAgentDownloadTool(Tool):
    ...
    ``sys_os_shell`` operate on) and the tool returns the path â€”
    path-only by design. To inspect the config, extract the archive
    (``sys_os_shell``) then read ``config.yaml`` (``sys_os_read``).
    The download exists for inspecting or forking a config â€” launching
    ...
    def description(cls) -> str:
        ...
        return (
            "Download a session's agent bundle (.tar.gz) to local disk "
            "and return the path. Global read â€” any session you can "
            ...
            "agent's config â€” launching an existing agent needs no "
...
class SysAgentListTool(Tool):
    ...
    - **local configs**: agent config YAMLs authored locally with
      ``sys_os_write`` (e.g. following the ``build-agent-meow`` skill) â€” a
    ...
    ``sys_session_create(agent_id=...)`` â€” an already-registered agent
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "sys_session_create(agent_id=...) â€” an already-registered "
            ...
            "an agent's config. Global read â€” no parameters."
        )
