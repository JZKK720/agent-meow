...
- ``image_get`` â†’ ``GET /v1/sessions/{id}/resources/images/{image_id}`` (metadata)
- ``image_upload`` â†’ ``POST /v1/sessions/{id}/resources/images`` (multipart)
- ``image_edit`` â†’ ``PATCH /v1/sessions/{id}/resources/images/{image_id}/edit``
  (store-and-forward Fabric.js JSON)
- ``image_generate`` â†’ stub for future diffusion-model hook. Schema-only;
