...
subclasses â€” the base-class ``invoke`` fails loud if the AP-side path ever
reaches them.
...
- ``doc_generate`` â†’ LLM-driven: takes an outline/prompt and produces a
  markdown document. Implemented as a pure schema tool â€” the runner's tool
...
class DocExportTool(Tool):
    ...
    def description(cls) -> str:
        return (
            ...
            "format â€” html, png (screenshot), or pdf â€” using officecli's "
