"""Tools package â€” Tool ABC, ToolManager, decorator, and built-in tools.
...
``omnigent_client.tools`` â€” import them from there.
...
# Map exported name â†’ (submodule path, attribute name).
...
# them from there â€” they used to be re-exported here but the
...
def __getattr__(name: str) -> Any:
    ...
    which avoids triggering the ``ToolManager`` â†’ ``mcp`` import
    chain that conflicts with the upstream ``mcp`` pip package
    in subprocess environments â€” see ``list_builtin_tools.py``
