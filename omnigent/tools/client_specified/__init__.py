"""Client-specified tools â€” tools whose schemas are supplied by the API caller.
...
- ``ClientSideTool``: A :class:`~?omnigent.tools.base.Tool` that must
  never be executed server-side â€” its ``invoke()`` raises ``RuntimeError``.
- ``ClientSideToolSpec``: Configuration for one client-side tool (name
  and schema only â€” no callback URL or headers).
...
class ClientSideToolSpec:
    ...
    Execution is handled entirely by the API caller â€” the runtime
    ...
    :param schema: Standard OpenAI-format function tool object, e.g.
        ...
        boolean â€” the LLM sets it ``false`` per call to receive
...
class ClientSideTool(Tool):
    ...
    ``invoke()`` raises ``RuntimeError`` â€” client-side tools must never
    ...
    def invoke(self, arguments: str, ctx: ToolContext) -> str:
        """
        Raise ``RuntimeError`` â€” client-side tools must never be executed
        server-side.
        ...
        :raises RuntimeError: Always â€” indicates a workflow bug.
        """
        raise RuntimeError(
...
def parse_client_side_tool_spec(raw: dict[str, Any]) -> ClientSideToolSpec:
    ...
    boolean inside ``parameters.properties`` â€” see
