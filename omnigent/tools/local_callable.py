...
distinct from native agent-meow local tools, which live as
...
- :class:`LocalCallableTool` â€” one :class:`Tool` per registered
  ...
  (fallback). Invocation runs in-process â€” these tools come
  from the agent author's own code and are trusted to the same
  level as the agent runtime.
- :func:`load_local_callable_tools` â€” helper that
...
class LocalCallableTool(Tool):
    ...
    invocation. The resolved callable runs in-process â€” these
    ...
    def invoke(self, arguments: str, ctx: ToolContext) -> str:
        ...
        re-raised to the caller â€” the tool-dispatch layer wraps
        ...
        :param ctx: Server-side execution context (unused â€” the
...
def load_local_callable_tools(
    ...
    translator produced â€” entries whose ``language`` is
    ...
    :returns: One :class:`LocalCallableTool` per omnigent-style
        ...
        case for native agent-meow specs).
    """
    ...
    for info in local_tools:
        ...
        # the runner via the SQL Statement Execution API â€” they
