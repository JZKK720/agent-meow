...
# max_retries reconnections). 5 failures Ã— 3 reconnects each
...
def _resolve_databricks_token(profile: str) -> str:
    ...
    NOT cached here â€” token resolution happens once per
...
class McpServerDisabledError(Exception):
    ...
    immediately â€” the breaker will automatically allow a probe
...
class _CircuitBreaker:
    ...
    - **CLOSED**: Normal operation â€” calls proceed.
    - **OPEN**: Tripped â€” calls fail immediately with
      :class:`McpServerDisabledError`.
    - **HALF-OPEN**: Cooldown elapsed â€” one probe call allowed.
...
def _is_sse_endpoint(url: str) -> bool:
    ...
    routes these straight to the SSE client instead of trying â€” and
    hanging on â€” Streamable HTTP first.

    :param url: The configured MCP server URL.
    :returns: ``True`` for an ``â€¦/sse`` path, ``False`` otherwise.
    """
...
class McpServerConnection:
    ...
    # ``__aexit__`` â€” anyio's stdio_client / sse_client cancel
    ...
    async def connect(self) -> list[McpToolDef]:
        ...
        opened them â€” required by anyio's cancel-scope identity
        check.
    ...
    async def call_tool(
        ...
        reconnect retries), records the failure â€” tripping the
        ...
        if self._session is None:
            raise RuntimeError(
                ...
                f"session â€” call connect() before call_tool()"
            )
    ...
    async def _invoke_tool(
        self,
        name: str,
        arguments: dict[str, Any],  # JSON values â€” see call_tool
        session_id: str | None = None,
    ) -> str:
        """
        ...
        ``/mcp/execute`` â†’ agent-meow server) can surface the elicitation
        ...
        """
        if self._session is None:
            raise RuntimeError("MCP session not initialized â€” call connect() first")
        ...
        # â”€â”€ MRTR: detect InputRequiredResult â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def _reconnect(self) -> None:
        ...
        error. Does not re-discover tools â€” the tool list from
    ...
    async def _run_lifecycle(self) -> None:
        ...
        Runs the entire ``open transport â†’ initialize session â†’
        ...
        when ``_close_event`` is set, the block exits â€” and
        ...
        - If a steady-state failure occurs *after* ready, it
          is logged here â€” :meth:`close` already has the
        ...
        try:
            async with AsyncExitStack() as stack:
                ...
                # already-open anyio streams â€” that does not touch
                # the cancel scopes opened above.
                await close_event.wait()
            # ``async with`` exits HERE on this task â†’ cancel scopes
            # opened by stdio_client / sse_client / ClientSession are
            # torn down by the same task that entered them. âœ“
    ...
    def _check_cache(self) -> list[McpToolDef] | None:
        ...
        TTLCache handles expiry internally â€” ``get()`` returns
    ...
    async def _open_transport(
        ...
        """
        Open the MCP transport â€” HTTP (SSE) or stdio subprocess.
        ...
        :param stack: The lifecycle task's exit stack â€”
    ...
    async def _open_http_transport(
        ...
        """
        Open an HTTP MCP transport â€” Streamable HTTP or legacy SSE.
        ...
        if self.config.url is None:
            ...
            # validator.
            raise RuntimeError(
                f"MCP server {self.config.name!r} transport='http' but url is None â€” "
    ...
    def _resolve_http_headers(self) -> dict[str, str] | None:
        ...
        if self.config.databricks_profile is not None:
            ...
            # Explicit Authorization header wins â€” don't overwrite.
    ...
    async def _open_stdio_transport(
        ...
        """
        Open a stdio MCP transport â€” spawns the MCP server as a
        subprocess directly.
        ...
        baseline. Per-MCP sandboxing â€” if reintroduced â€” should
        ...
        if self.config.command is None:
            ...
            raise RuntimeError(
                ...
                "None â€” validator should have caught this"
            )
    ...
    async def _elicitation_handler(
        ...
        if session_id is None or self.elicitation_callback is None:
            _logger.warning(
                ...
                "session context or callback â€” declining",
                self.config.name,
            )
...
_PROBLEMATIC_SCHEMA_KEYWORDS = frozenset(
    {
        # $ref with sibling properties â€” OpenAI ignores siblings,
        # Anthropic rejects the combination.
        "$ref",
        # oneOf â€” OpenAI rejects in nested contexts (must use anyOf).
        "oneOf",
        # allOf with $ref â€” Anthropic rejects the combination.
        "allOf",
    }
)
...
def _warn_problematic_keywords(
    ...
    for keyword in sorted(found):
        _logger.warning(
            ...
            "providers reject or handle inconsistently â€” "
...
async def _call_tool_with_reconnect(
    ...
    for attempt in range(total_tries):
        try:
            ...
            if not _is_connection_error(exc):
                raise
            last_exc = exc
            # Last attempt â€” don't reconnect, just raise.
    ...
    # All attempts exhausted â€” re-raise the last connection error.
