...
def build_accept_content_from_schema(
    ...
    (booleans â†’ ``True``, enums â†’ ``"allow"`` or first option,
    properties with ``default`` â†’ the default). Returns ``None``
    when the schema has properties that require free-form user
    input (strings, numbers without defaults) â€” the caller should
    ...
    for key, prop in properties.items():
        if not isinstance(prop, dict):
            return None
        # Enum with oneOf â€” pick "allow" or the first const.
        ...
        if prop_type == "boolean":
            content[key] = True
            continue
        # Has a default â€” use it.
        if "default" in prop:
            content[key] = prop["default"]
            continue
        # Free-form input required â€” can't auto-fill.
        return None
    return content
