"""TUI→web forwarder for the ironclaw-native harness.

The ``omnigent ironclaw`` wrapper launches the real ``ironclaw`` TUI in a runner-owned
tmux pane, and :mod:`omnigent.ironclaw_native_bridge` injects web-UI messages into
...
This module is that missing mirror — the IronClaw analog of
:mod:`omnigent.goose_native_forwarder`. IronClaw stores all sessions in a single
SQLite database at ``$IRONCLAW_HOME/state.db`` (default ``~/.ironclaw/state.db``,
verified against the ironclaw-agent ``ironclaw_state.py`` schema): a ``sessions`` row
per session (``id`` TEXT, ``source``, ``cwd``, ``started_at`` REAL-seconds) and a
``messages`` row per turn (``id`` autoincrement, ``session_id`` FK, ``role``,
``content`` TEXT, ``active``).

Unlike goose-native, IronClaw auto-generates its ``sessions.id`` and gives no
...
at/after the recorded launch time, with a claim guard so two ironclaw-native sessions
...
``response_id`` per turn — ``ironclaw_turn_{opening-msg-id}`` shared across every row
...
``running``/``idle`` ``session.status`` edges for ironclaw-native (as for
...
deduped against a persisted posted-count (:mod:`omnigent.ironclaw_native_status`) so
...
from omnigent import ironclaw_native_status

_logger = logging.getLogger(__name__)

#: Seconds between store polls. IronClaw flushes a ``messages`` row per agentic step
...
#: IronClaw writing the ``sessions`` row once the TUI initializes.
_DISCOVERY_SKEW_S = 10.0

_STATE_FILE = "ironclaw_forwarder.json"

# A sibling session's persisted claim (naming the same ``ironclaw_session_id``)
...
def _warn_sqlite_once(context: str, exc: sqlite3.Error) -> None:
    ...
    _logger.warning("ironclaw forwarder sqlite error during %s: %s", context, exc)
...
# IronClaw injects skill content as a user message prefixed with this marker.
...
def _read_model_from_ironclaw_config(bridge_dir: Path) -> str | None:
    """Best-effort read of the model name from the per-session IRONCLAW_HOME config.

    Falls back to the user's ``~/.ironclaw/config.yaml`` if no per-session config
    exists. Returns ``None`` when the model cannot be determined.
    """
    candidates = [
        bridge_dir / "ironclaw_home" / "config.yaml",
        Path.home() / ".ironclaw" / "config.yaml",
    ]
...
class _IronclawUsageTracker:
    """Post ``external_session_usage`` events for a ironclaw-native session.

    IronClaw's SQLite ``state.db`` does not expose per-message token counts, so
    ...
    requires IronClaw to expose usage data in its state.db or session transcript.
    """
    ...
    async def flush(self) -> None:
        ...
        if self._model is None:
            self._model = await asyncio.to_thread(
                _read_model_from_ironclaw_config, self._bridge_dir
            )
        ...
        try:
            ...
            if resp.status_code < 400:
                ...
                _logger.warning(
                    "ironclaw usage tracker POST failed: status=%s", resp.status_code
                )
        except httpx.HTTPError:
            _logger.debug("ironclaw usage tracker POST failed", exc_info=True)


def _ironclaw_home() -> Path:
    """Return IronClaw's home dir for this process (``$IRONCLAW_HOME`` or ``~/.ironclaw``)."""
    raw = os.environ.get("IRONCLAW_HOME", "").strip()
    return Path(raw) if raw else Path.home() / ".ironclaw"


def default_state_db() -> Path:
    """Return IronClaw's SQLite session store path for this process.

    Resolves to ``$IRONCLAW_HOME/state.db`` (default ``~/.ironclaw/state.db``) the same
    way IronClaw's own ``get_ironclaw_home()`` does, so the forwarder reads the exact
    DB the native TUI writes. Overridable via ``IRONCLAW_STATE_DB`` (tests,
    non-standard installs).
    """
    override = os.environ.get("IRONCLAW_STATE_DB", "").strip()
    if override:
        return Path(override)
    return _ironclaw_home() / "state.db"


@dataclass
class _ForwardState:
    """Durable forwarder cursor, persisted to ``bridge_dir/ironclaw_forwarder.json``.

    :param ironclaw_session_id: The resolved IronClaw ``sessions.id`` being tailed, or
    ...
    :param active_turn_id: The per-turn ``response_id`` of the turn currently in
        flight (``ironclaw_turn_{opening-msg-id}``), or ``None`` between turns.
    ...
    ironclaw_session_id: str | None = None
    last_id: int = 0
...
def _write_state(bridge_dir: Path, state: _ForwardState) -> bool:
    ...
    try:
        ...
        tmp.write_text(
            json.dumps(
                {
                    "ironclaw_session_id": state.ironclaw_session_id,
        ...
        _logger.warning(
            "ironclaw forwarder could not persist state to %s", bridge_dir, exc_info=True
        )
        return False


def clear_ironclaw_bridge_state(bridge_dir: Path) -> None:
...
def _session_claimed_by_other(
    bridge_dir: Path, ironclaw_session_id: str, my_launch_s: float
) -> bool:
    """Whether another LIVE session is already mirroring *ironclaw_session_id*.

    Two ironclaw-native sessions launched in the same cwd can momentarily discover
    ...
    ``ironclaw_session_id`` with a heartbeat fresher than ``_CLAIM_FRESH_MS``. Ties
    ...
    :param ironclaw_session_id: The IronClaw session id this session would mirror.
    ...
    for sibling in root.iterdir():
        ...
        if other.ironclaw_session_id != ironclaw_session_id:
            continue
...
def _discover_session_id(
    ...
    *,
    ...
    """Return this terminal's IronClaw ``sessions.id``, or ``None`` if not yet present.

    IronClaw can't be told its session id in advance, so we bind the newest session
    ...
    :param db_path: The IronClaw ``state.db`` to read.
    ...
    :param excluded: IronClaw session ids already claimed by a live sibling.
    ...
    for sid, cwd in candidates:
        if isinstance(cwd, str) and cwd and _same_path(cwd, workspace):
            return sid
    # Fallback ONLY when IronClaw recorded no cwd at all for any candidate (older
    # builds / unusual backends): bind a lone candidate. We never bind a row whose
    # cwd is a *different* real dir — unlike cursor's md5-hashed dirs, IronClaw
...
def _discover_child_session(db_path: Path, parent_session_id: str) -> str | None:
    """Return the newest IronClaw session whose parent is *parent_session_id*.

    IronClaw auto-compresses by ending the current session and forking a CHILD
    (``sessions.parent_session_id`` points at the old id; present in ironclaw
...
def _message_to_items(
    ...
    response_id = f"ironclaw:{msg_id}"

    if role == "user":
        if not text:
            return []
        # IronClaw injects skill content as a user message — replace with
...
def _read_new_items(
    db_path: Path, ironclaw_session_id: str, last_id: int, agent_name: str
) -> list[_MirrorItem]:
...
def _annotate_turn_actions(
    ...
    A IronClaw turn is ``user -> (assistant+tool_calls -> tool)* ->
    ...
    - a ``user`` group **opens** a turn → mint ``ironclaw_turn_{msg_id}`` and emit a
    ...
    for msg_id, group_iter in groupby(items, key=lambda it: it.msg_id):
        ...
        if opens or (active_turn_id is None and (has_function_call or is_assistant)):
            active_turn_id = f"ironclaw_turn_{msg_id}"
            actions.append(
...
def _assistant_row_has_tool_calls(tool_calls: object) -> bool:
    ...
    IronClaw writes one ``messages`` row per agentic step (complete, append-only —
...
def _count_completed_turns(
    db_path: Path, ironclaw_session_id: str, max_id: int | None = None
) -> int:
    """Count completed turns for *ironclaw_session_id* (0 on unreadable/empty).
    ...
    counted regardless of the ``active`` flag: IronClaw soft-deletes on compaction
    ...
    params: tuple[object, ...] = (ironclaw_session_id,)
    if max_id is not None:
        query += " AND id <= ?"
        params = (ironclaw_session_id, max_id)
    try:
...
async def _post_external_session_status(
    ...
    *,
    ...
    emits only a web-spinner ``session.status`` edge for ironclaw-native and never
    wakes a parent, which is why this explicit post is required.

    When *response_id* is given (the turn's ``ironclaw_turn_{id}``), the edge carries
...
def _has_new_compaction(db_path: Path, ironclaw_session_id: str) -> bool:
    """Check if ironclaw has compacted messages for this session."""
    con = _connect_ro(db_path)
    if con is None:
        return False
    try:
...
async def _persist_ironclaw_compaction_item(
    client: httpx.AsyncClient,
    *,
    session_id: str,
    db_path: Path,
    ironclaw_session_id: str,
) -> None:
    ...
    data: dict[str, object] = {
        "summary": "[IronClaw compaction — context was compacted via /compress]",
...
async def forward_ironclaw_store_to_session(
    *,
    base_url: str,
    ...
    """Tail IronClaw's session store and mirror new messages into the AP session.

    Discovers this session's IronClaw ``sessions.id`` (newest row whose ``cwd``
    ...
    :param bridge_dir: The ironclaw-native bridge dir (holds the persisted cursor).
    :param agent_name: Agent label stamped on mirrored assistant items.
    :param workspace: The session's working directory (IronClaw's ``sessions.cwd``).
    :param launch_epoch_s: Wall-clock seconds when this terminal launched.
    :param db_path: IronClaw state DB; defaults to :func:`default_state_db`.
    ...
    ironclaw_session_id: str | None = persisted.ironclaw_session_id
    last_id = persisted.last_id if ironclaw_session_id is not None else 0
    ...
    # tailed ironclaw session changes (discovery, claim-yield, compaction re-pin).
    active_turn_id: str | None = (
        persisted.active_turn_id if ironclaw_session_id is not None else None
    )
    ...
    async with httpx.AsyncClient(
        ...
        usage_tracker = _IronclawUsageTracker(client, session_id, bridge_dir)
        compaction_persisted = False
        while True:
            try:
                if ironclaw_session_id is None:
                    ...
                    if resolved is not None and not await asyncio.to_thread(
                        ...
                        ironclaw_session_id = resolved
                        last_id = (
                            persisted.last_id if persisted.ironclaw_session_id == resolved else 0
                        )
                ...
                # knows which IronClaw session backs this conversation
                # (needed for fork/resume).
                if ironclaw_session_id is not None and not _external_id_synced:
                    try:
                        resp = await client.patch(
                            f"/v1/sessions/{session_id}",
                            json={"external_session_id": ironclaw_session_id},
                        )
                        resp.raise_for_status()
                        _external_id_synced = True
                    except httpx.HTTPError:
                        _logger.debug(
                            "ironclaw forwarder failed to PATCH external_session_id; "
                            "will retry next poll; session=%s",
                            session_id,
                            exc_info=True,
                        )
                if ironclaw_session_id is not None:
                    ...
                    if await asyncio.to_thread(
                        _session_claimed_by_other, bridge_dir, ironclaw_session_id, launch_epoch_s
                    ):
                        _logger.warning(
                            "ironclaw session %s already mirrored by another session; "
                            "pausing mirror for session=%s",
                            ironclaw_session_id,
                            session_id,
                        )
                        ironclaw_session_id = None
                        active_turn_id = None
                    else:
                        items = await asyncio.to_thread(
                            _read_new_items, db, ironclaw_session_id, last_id, agent_name
                        )
                        ...
                        for action in turn_actions:
                            if action.kind == "running":
                                if action.response_id is not None:
                                    ...
                                    try:
                                        ...
                                        _logger.debug(
                                            "ironclaw forwarder running-edge post failed; "
                            ...
                            _write_state(
                                bridge_dir,
                                _ForwardState(
                                    ironclaw_session_id=ironclaw_session_id,
                                    last_id=last_id,
                                    launch_epoch_s=launch_epoch_s,
                        ...
                        if not compaction_persisted and await asyncio.to_thread(
                            _has_new_compaction, db, ironclaw_session_id
                        ):
                            try:
                                await _persist_ironclaw_compaction_item(
                                    client,
                                    session_id=session_id,
                                    db_path=db,
                                    ironclaw_session_id=ironclaw_session_id,
                                ...
                                _logger.warning(
                                    "Failed to persist ironclaw compaction item for %s",
                                    session_id,
                                    exc_info=True,
                                )
                            # Compaction ends this IronClaw session and forks a
                            ...
                            try:
                                child = await asyncio.to_thread(
                                    _discover_child_session, db, ironclaw_session_id
                                )
                                if child is not None and not await asyncio.to_thread(
                                    _session_claimed_by_other,
                                    bridge_dir,
                                    child,
                                    launch_epoch_s,
                                ):
                                    ironclaw_session_id = child
                                    last_id = 0
                                    active_turn_id = None
                                    compaction_persisted = False
                                    _external_id_synced = False
                                    ...
                                    # ironclaw_session_id; the child restarts its
                                    ...
                                    await asyncio.to_thread(
                                        ironclaw_native_status.write_posted_count,
                                        bridge_dir,
                                        await asyncio.to_thread(
                                            _count_completed_turns, db, child
                                        ),
                                    )
                                    _write_state(
                                        bridge_dir,
                                ...
                                _logger.warning(
                                    "ironclaw forwarder failed to re-pin to child session "
                        ...
                        # not open this batch. IronClaw leaves the tmux pane quiet
                        ...
                        if active_turn_id is not None and not running_posted_this_batch:
                            try:
                                ...
                                _logger.debug(
                                    "ironclaw forwarder running re-assert failed; "
                        ...
                        completed_turns = await asyncio.to_thread(
                            _count_completed_turns, db, ironclaw_session_id, last_id
                        )
                        if completed_turns > await asyncio.to_thread(
                            ironclaw_native_status.read_posted_count, bridge_dir
                        ):
                ...
                _logger.exception(
                    "ironclaw forwarder poll failed; session=%s ironclaw_session=%s",
                    session_id,
                    ironclaw_session_id,
                )
...
async def supervise_ironclaw_forwarder(
    *,
    base_url: str,
    headers: dict[str, str],
    session_id: str,
    ...
    """Run :func:`forward_ironclaw_store_to_session` under a restart supervisor.
    ...
    while True:
        ...
        try:
            await forward_ironclaw_store_to_session(
                base_url=base_url,
                headers=headers,
                session_id=session_id,
            ...
            _logger.warning(
                "ironclaw forwarder returned unexpectedly; restarting; "
        ...
        if crash_exc is not None:
            _logger.error(
                "ironclaw forwarder crashed; restarting in %.1fs; session=%s bridge_dir=%s",
                backoff_s,
                session_id,
                bridge_dir,
                exc_info=crash_exc,
