...
This does NOT suppress Goose's gate â€” its cliclack prompt stays the source of
...
# The confirmation question â€” both phrasings contain "do you allow"; it is only
...
_CLICLACK_PREFIX_RE = re.compile(r"^[\sâ”‚â—†â—‡â—Šâ—‹â—â–²â–¶>|*-]+")
...
def goose_permission_elicitation_id(session_id: str, token: str) -> str:
    """Return the deterministic agent-meow elicitation id for a Goose prompt.

    *token* identifies one approval episode (a per-session counter), not the
    scraped content â€” the rendered tool context above the cliclack widget jitters
...
def parse_goose_approval_prompt(pane: str) -> GooseApprovalPrompt | None:
    ...
    if not (has_allow and has_deny):
        return None
    # "Always Allow" present â†’ Deny is the 3rd item (2 downs); else 2nd (1 down).
        ...
        while True:
            try:
                ...
                if prompt is not None:
                    ...
                    # NOT re-mint while the prompt stays up â€” the scraped tool
...
async def _run_one_approval(
    ...
    *,
    ...
    # DownÃ—N + Enter selects "Deny". A decline/cancel verdict both map to Deny.
