...
# â€” a broader, distinct concern that must not depend on this harness module.)
...
# credential â€” only the three onboarding-state booleans.
...
# onboarding â€” captured ground-truth from a real onboarded profile. Enterprise
# onboarding is a distinct flow agent-meow does not drive, so it stays ``False``.
...
def ensure_agy_onboarding_complete() -> None:
    ...
    already hold their exact boolean values â€” so a returning user's agy state is
    ...
    # is normalised on write â€” matching this module's ``read_bridge_state``, which
...
class AntigravityNativeBridgeState:
    ...
    .. note::
        ...
        capture at launch â€” the CLI terminal uses ``tmux_start_on_attach=True``
...
# â”€â”€ agent-meow MCP relay wiring (sys_* tools) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
# agy is otherwise tool-isolated from agent-meow: unlike claude/codex/cursor it
# wires no MCP relay, so the wrapped agy cannot reach any ``sys_*`` tool (spawn
# sub-agent sessions, drive agent-meow terminals, list agents/models, ``sys_os_*``).
# This section wires the SAME relay cursor #742 uses â€” the shared
# ``omnigent.claude_native_bridge serve-mcp`` stdio server â€” into agy.
#
...
# ``$HOME/.gemini/config/mcp_config.json`` â€” the *same* file the user's
...
#
# CHOSEN DESIGN â€” per-session ISOLATED GEMINI DIR. The runner keeps agy's real
...
# the OS keyring (verified against agy 1.0.12 â€” the binary's auth path is
...
# HOME-global ``mcp_config.json`` footgun (#1194) there â€” concurrent sessions
# would clobber one another's relay config. ``--gemini_dir`` resolves both at
# once: real HOME â‡’ keyring auth works on every platform; isolated gemini dir â‡’
...
# prompt blocking the call before agent-meow ever sees it.
_AGY_ENABLED_TOOLS = [
    "list_comments",
    "sys_add_policy",
    "sys_agent_download",
...
# missing credential only means agy re-auths in that session â€” never corruption
# of the real tree).
_AGY_SEED_FILES = (
...
def agy_gemini_dir(bridge_dir: Path) -> Path:
    ...
    :returns: Absolute ``.gemini`` directory that should receive agy's session
        config/state and the agent-meow MCP config.
    """
...
def build_mcp_config(
    ...
    *,
    ...
    """Build agy's ``mcp_config.json`` payload for the agent-meow relay server.
    ...
    ``env`` keys plus ``enabledTools``, agy's auto-approve allowlist â€” verified
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared agent-meow MCP relay requires.
...
def write_mcp_config(
    ...
    *,
    ...
    ``mcp_config.json`` for the agent-meow relay into the per-session isolated agy
    Gemini dir at ``<bridge_dir>/agy-home/.gemini/config/mcp_config.json`` â€” the
...
# unrelated internal proto field, NOT a settings key â€” it would be ignored).
...
def ensure_agy_feedback_survey_disabled(home: Path) -> None:
    ...
    ``<home>/.gemini/antigravity-cli/settings.json`` â€” set it ``false`` before
    ...
    ``enableTelemetry``, â€¦) are preserved; a missing file is created with just
    this key; a malformed / non-object file is left UNTOUCHED rather than
    clobbered. Best-effort â€” a read/write failure is logged and the launch
    ...
    try:
        ...
        # An EXISTING but unreadable file (perms, stale NFS, â€¦) must NOT be
        # recreated from scratch â€” that would drop the user's real settings.
        _logger.warning(
    ...
    if data.get(_AGY_FEEDBACK_SURVEY_SETTING) is False:
        return  # already disabled â€” avoid a needless rewrite
    ...
    # corruption but not lost updates. That window is self-limiting â€” the
...
def update_conversation_id(
    ...
    agent-meow session stays the same (e.g. the runner cold-start replacing the
    ...
    the write is SKIPPED and a WARNING is logged naming the dropped id â€” silently
...
# â”€â”€ Web-turn TUI delivery (tmux send-keys) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
# The executor types EVERY web/mobile turn into the agy TUI over the runner-owned
# tmux pane â€” never agy's connect-RPC ``SendAgentMessage``, which agy records as a
...
# corner/join glyphs (e.g. ``â•­â”€â”€â”€â”€â•®``) is still detected, while ordinary text
# (which carries non-box chars) never matches.
_AGY_BOX_GLYPHS = frozenset("â”€â”â•Œâ•â”„â”…â”ˆâ”‰â•­â•®â•°â•¯â”Œâ”â””â”˜â”œâ”¤â”¬â”´â”¼â”‚â•‘â•”â•—â•šâ•â•â• â•£â•¦â•©â•¬â•´â•µâ•¶â•·")
...
def _agy_separator_line(line: str) -> bool:
    ...
    Accepts a pure ``â”€`` rule (agy 1.0.14) and a box-decorated rule (corner/join
...
def _draft_in_input_region(pane: str, needle: str, baseline_region: str) -> bool:
    ...
    pre-paste baseline counts as present â€” so short messages are still render- and
...
def _agy_draft_candidate_lines(region: str) -> list[str]:
    ...
    contains a word like ``Generating`` or a footer phrase â€” otherwise a
    legitimate message would be filtered out and misread as "never rendered".
    Status/footer rows (idle/active/"Generating â€¦") carry no ``>`` prompt, so the
...
def _wait_for_agy_prompt_ready(socket_path: str, tmux_target: str, *, timeout_s: float) -> None:
    ...
    mean the input box is mounted and can take a paste â€” agy accepts a mid-turn
    paste and queues it as the next turn â€” so a mid-turn steer must NOT wait for
...
def _submit_and_verify(
    ...
    **Mid-turn steer.** When agy already shows :data:`_AGY_ACTIVE_MARKER`, a turn
    ...
    is sent and the function returns without re-sending or hard-failing â€” the
    ...
    for _ in range(_MAX_SUBMIT_ATTEMPTS):
        ...
        # now, every capture returns "" and the draft never clears â€” so fail
...
def inject_user_message_via_tui(
    ...
    *,
    ...
    running turn â€” agy accepts a mid-turn paste), clear any leftover draft (Home +
    ...
    # runs for EVERY message â€” including short ones with no stable needle â€” so a
...
def send_interaction_keys_via_tui(
    ...
    **in-process permission / question prompt**. When agy runs attended (the
    ...
    agent-meow web UI AND as agy's own numbered TUI prompt ("Do you want to
    proceed?", 1.Yes â€¦ 4.No). Delivering the verdict over
    ...
    for that interaction can stay open** â€” and the next typed turn then lands in
    that stale prompt's filter/amend buffer instead of starting a fresh turn
    (live-verified; see ``docs/claude/antigravity-rpc-spike-notes.md`` Â§"attended
    ...
    Each argument is a tmux ``send-keys`` key argument â€” a bare ``"1"`` / ``"4"``
    selects a numbered option, ``"Enter"`` confirms, ``"Escape"`` cancels â€” sent
