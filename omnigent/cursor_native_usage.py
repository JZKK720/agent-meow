...
cursor-agent surfaces per-turn token usage ONLY through its lifecycle hooks â€”
...
``external_session_usage`` event â€” the SAME server contract claude-native and
...
# --------------------------------------------------------------------------- #
# Hook-side recorder (stdlib only â€” keep fast; cursor blocks the turn on it).
# --------------------------------------------------------------------------- #
...
def normalize_hook_payload(payload: object) -> dict[str, object] | None:
    ...
    :returns: ``{"generation_id", "model", <token fields>}`` when the payload
        ...
        ``None`` (skip â€” nothing to bill for this turn).
    """
...
def _cli_record_usage(bridge_dir: Path) -> int:
    ...
    (:func:`~?omnigent.cursor_native_status.record_turn_end`) on EVERY firing â€” even
    a turn with no billable usage, which :func:`record_usage_payload` skips â€” so
    ...
    exits 0 â€” a usage/idle-capture failure must never block or fail the agent
    turn.
    """
    ...
    except Exception:  # noqa: BLE001 â€” never let usage/idle capture break the turn
...
class _UsageAccumulator:
    ...
    â€” cache reads included â€” is the correct cumulative input). Dedup by
...
async def forward_cursor_usage_to_session(
    *,
    ...
    the cumulative accumulator (deduped by ``generation_id``), and â€” when the
    totals advanced â€” POSTs an ``external_session_usage`` event. The accumulator
    ...
    A newly-observed usage line means cursor-agent fired its ``stop`` hook â€” a
    ...
    # â€” the safe direction. Seeding from persisted usage would permanently skip a
