...
- ``agent-meow resume <conv_id>`` â€” fetch the conversation, read its
  ``omnigent.wrapper`` label, and dispatch by runtime.
- ``agent-meow resume`` (no id) â€” open the cross-agent picker (see
...
``agent-meow run --resume`` invocation â€” the agentless ``run
...
def run_resume(
    *,
    ...
    Direct-id form (``target`` provided) is the common CUJ â€” the user
    ...
    if target is None:
        ...
        if target is None:
            ...
            # server). Treat as a clean exit â€” the user explicitly
...
def _pick_conversation_for_resume(
    *,
    ...
    empty-list both surface as ``None`` â€” the caller treats both as
    ...
    try:
        ...
        # rather than a raw httpx traceback â€” the user can act on
...
def _dispatch_by_runtime(
    *,
    ...
    the existing ``agent-meow run --resume`` invocation â€” the
