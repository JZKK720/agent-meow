"""Kimi Code hook commands for the native agent-meow wrapper.
...
- ``evaluate-policy`` â€” the ``PreToolUse`` deny-gate. Mirrors
  :func:`~?omnigent.claude_native_hook._main_evaluate_policy`: it converts the
  Kimi hook payload into an agent-meow ``EvaluationRequest`` (the snake-cased
  ...
  runs â€” agent-meow enforces its deny-policy without silencing the user's
  consent. Fails CLOSED (deny) when an already-governed session can't reach a
  verdict, matching the claude-native gate.

- ``permission-request`` â€” the interactive web-UI approval. Kimi fires
  ``PermissionRequest`` fire-and-forget (it does NOT read this hook's output â€”
  ...
  endpoint claude-native uses â€” the server publishes the approval card and
...
# claude-native, a TOOL_CALL ASK does NOT park here â€” kimi owns the ask via
...
# Long-poll budget for the web approval verdict â€” the human may take a while.
...
def _main_evaluate_policy(argv: list[str]) -> int:
    """Evaluate a kimi ``PreToolUse`` hook against agent-meow policies.
    ...
    CLOSED with a ``deny`` (this hook is the sole agent-meow enforcement point
    for kimi tool calls).

    :param argv: CLI argv after the ``evaluate-policy`` subcommand.
    :returns: Always ``0`` â€” verdicts are expressed via JSON, not exit codes.
    """
    ...
    if not session_id:
        return 0  # not a governed session â€” no opinion
    ...
    if eval_request is None:
        ...
        # relay path â€” no policy to evaluate here.
        return 0
...
def _main_permission_request(argv: list[str]) -> int:
    ...
    Kimi fires this hook **fire-and-forget** â€” it answers approval in its own
    TUI and does NOT read the hook's stdout â€” so we cannot return a decision it
    honors. Instead we drive an interactive web-UI approval and type the answer
    back into kimi's prompt:

    1. POST the gated tool to ``/v1/sessions/{id}/hooks/permission-request`` â€”
...
def _request_web_approval(
    ...
    :returns: ``"allow"`` / ``"deny"``, or ``None`` on timeout (server returns
        an empty 200), transport failure, or an unparseable verdict â€” all of
