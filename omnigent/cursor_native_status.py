...
"Workingâ€¦" spinner (a ``session.status`` edge) â€” which never wakes a parent
orchestrator.
...
loop tails it and POSTs an ``external_session_status: idle`` event â€” the SAME
...
Stdlib-only (no httpx) so importing it from the stop hook keeps the hook fast â€”
...
def record_turn_end(bridge_dir: Path, payload: object | None = None) -> None:
    ...
    Fires on EVERY completed turn â€” including turns with no billable token usage,
    which :func:`~?omnigent.cursor_native_usage.record_usage_payload` skips â€” so the
    ...
    :param payload: The cursor ``stop`` hook payload, if any â€” its
