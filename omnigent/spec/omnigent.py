...
- :func:`agent_spec_to_agent_def` â€” forward direction, consumed by
  ``OmnigentExecutor`` at executor-construction time.
- :func:`agent_def_to_agent_spec` â€” reverse direction, consumed by
  :func:`~?omnigent.spec.load` when handed an agent-meow YAML so
...
Design invariants (see designs/OMNIGENT_INTEGRATION.md Â§1 and Â§2):
...
- Tool callables are encoded as dotted import paths in ``AgentSpec``
  ...
  clean serializable native type â€” no opaque payloads.
...
# agent-meow concept), so the adapter writes ``1`` â€” the currently
# valid agent-meow schema version (see omnigent/spec/validator.py).
...
# each child as an independent task â€” no live parent session
# exists at runtime to walk.
_OS_ENV_INHERIT_SENTINEL = "inherit"

# agent-meow loader policy-type discriminators. Used to dispatch
...
# â”€â”€ Forward direction: AgentSpec â†’ AgentDef â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def agent_spec_to_agent_def(spec: AgentSpec) -> AgentDef:
    ...
    :returns: A populated :class:`~?omnigent.datamodel.AgentDef`
        ...
        dataclass defaults â€” those concepts are unsupported and
        rejected upstream.
    ...
    # For native agent-meow v1 specs ``executor.config`` is empty; infer the
    ...
    # ``AgentDef.name`` / ``AgentDef.prompt`` are ``str | None`` â€” pass
    ...
    # via ``--plugin-dir``. ``None`` when the spec has no skills â€”
...
def _reject_unsupported_concepts(spec: AgentSpec) -> None:
    ...
    # BEFORE the harness receives a turn â€” the
    ...
    # ``cancellable_function`` is an agent-meow concept â€” agent-meow'
    ...
    for tool in spec.local_tools:
        # Client-runtime tools carry no path â€” skip the
...
def _translate_tools_to_omnigent(spec: AgentSpec) -> dict[str, Tool]:
    """
    Build the ``AgentDef.tools`` dict from agent-meow' tool model.
    ...
    (``"tools/python/foo.py"``) are not supported â€” the YAML adapter
    ...
    for tool_info in spec.local_tools:
        ...
        # survives a forwardâ†’reverse pass through the spec layer.
        ...
        # â€” see ``designs/SERVER_HARNESS_CONTRACT.md`` Â§"Async
    ...
    # forward direction translated ``MCPTool`` â†’
...
def _sub_spec_to_agent_tool(sub: AgentSpec) -> AgentTool:
    ...
    # (native agent-meow v1 specs). Also forward os_env so sub-sessions get
...
def _resolve_dotted_attr(dotted_path: str, tool_name: str) -> Any:
    ...
    the resolved object to be callable â€” it's used by the tool-
    translation path where the resolved object may be either a
    function (â†’ wrap in :class:`FunctionTool`) OR a
    cancellable-runner instance (â†’ wrap in
    ...
    try:
        ...
        # names â€” what you get when the caller hands us a string
        # with no dots, e.g. ``"foo"`` â†’ ``rpartition`` yields
        ...
        raise OmnigentError(
            f"tool {tool_name!r}: cannot resolve {dotted_path!r} â€” "
...
# signatures â€” the ``...`` args-form is the standard "we don't care
...
def _resolve_dotted_callable(
    ...
    callability check â€” the only difference between the two is
...
# â”€â”€ Policy translation (agent-meow YAML dict â†’ agent-meow
# GuardrailsSpec) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    validation â€” so every field-level rule (phase/tool validity,
    action enum coercion, label schema constraints) lands in the
    shared parser rather than being reimplemented here.

    Returns ``None`` when all four inputs are empty / absent â€”
    the synthesized :class:`AgentSpec` then carries
    ``guardrails=None`` and the runtime builds a no-op
    :class:`PolicyEngine` (per POLICIES.md Â§10 zero-policy case).

    :param raw_policies: agent-meow YAML ``policies:`` mapping,
    ...
    :param raw_labels: agent-meow YAML top-level ``labels:`` map
        of ``{label_key: initial_value}``. ``None`` or empty means
        no labels â€” a schema-only label (``label_schema`` entry
        without a ``labels`` entry) is still allowed and gets
        ``initial: None``.
    :param raw_label_schema: agent-meow YAML top-level
        ``label_schema:`` map of
    ...
    :returns: A validated :class:`GuardrailsSpec`, or ``None``
        ...
        source YAML.
    :raises OmnigentError: On malformed entries â€” same error
    ...
    *,
    ...
    for policy_name, raw_entry in raw_policies.items():
        if not isinstance(raw_entry, dict):
            ...
            # malformed entries â€” it already produces a specific
...
def _translate_policy_entry_yaml(
    ...
    *,
    ...
    :param policy_name: YAML key under ``policies:``, e.g.
        ``"block_long_sleep"`` â€” included in error messages.
    :param raw_entry: Raw YAML mapping for this policy.
    :param parent_profile: Databricks profile from the parent
        agent's executor. Only consumed by the ``type: prompt``
        branch â€” function policies don't carry LLM configs so
...
def _translate_function_policy_yaml(
    ...
    - ``handler: path.to.fn`` â†’ ``function: path.to.fn``
    - ``handler: path.to.fn`` + ``factory_params: {...}`` â†’
      ``function: {path: path.to.fn, arguments: {...}}``
    - Other fields (``condition``, ``action``, ``set_labels``,
      ``ask_timeout``) pass through unchanged. ``on:`` is
      intentionally excluded â€” ``type: function`` policies no
    ...
    # agent-meow parser side. ``on:`` is intentionally omitted â€”
    ...
    if callable_path is None:
        ...
        # required" error â€” it already does so with a clear
    ...
    # The wrapper lives at load time â€” zero cost in the engine's
    # hot evaluate() loop.
...
def _resolve_profile_to_connection(profile: str) -> dict[str, str] | None:
    ...
    self-contained â€” agent-meow' LLM adapters take credentials
    ...
    gap by resolving profile â†’ connection once at load time.
    ...
    :returns: ``{"base_url": "<host>/serving-endpoints", "api_key":
        ...
        malformed (callers treat ``None`` as "skip injection" â€”
    ...
    # runtime details from the spec module â€” only this single
...
def _translate_prompt_policy_yaml(
    ...
    *,
    ...
    - ``executor: {model: X}`` â†’ ``llm: {model: X}``. The
    ...
    :param parent_profile: Databricks profile from the parent
        agent's ``executor.profile`` â€” usually set by the
        ...
        routes correctly. ``None`` means no profile â€” policies
        must carry their own credentials.
    :returns: Agent-plane-shaped dict with ``executor:`` renamed
        to ``llm:``.
    """
    del parent_profile  # No longer needed â€” prompt_policy uses event["llm_client"]
    # Translate type:prompt â†’ type:function backed by the
...
# â”€â”€ Reverse direction: AgentDef â†’ AgentSpec â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    :func:`~?omnigent.spec.load` when handed an agent-meow YAML
    ...
    :param raw_yaml: Optional raw YAML dict the ``agent_def`` was
        ...
        agent-meow' ``guardrails:`` block â€” the agent-meow
    ...
    # alternative â€” silently dropping it in favor of an inline
    # ``prompt:`` â€” was the bug from kasey_uhlenhuth's report.
    ...
    # â€” connection is usually resolved at runtime, not baked into the
    # translated spec).
    ...
    # harness: required``.
    #
    # We propagate the *effective* parent harness â€” i.e. the
    # harness after model-prefix auto-pick has run â€” not the raw
    ...
    # translation time â€” the agent-meow path spawns each child as an
    ...
    #       â†’ AgentSpec.local_tools (agent-meow treats every
    #         tool as cancellable, so no subtype distinction).
    #   AgentTool
    #       â†’ AgentSpec.sub_agents (one nested AgentSpec each)
    #         + ToolsConfig.agents (list of exposed names).
    #   MCPTool (stdio / subprocess)
    #       â†’ AgentSpec.mcp_servers (one native stdio-transport
    #         MCPServerConfig each; subprocess is spawned by
    #         agent-meow' ToolManager, srt-wrapped when
    #         available â€” see omnigent/tools/mcp.py).
    #   MCPTool (HTTP) â†’ same, but transport='http'.
        ...
        if isinstance(tool, FunctionTool | CancellableFunctionTool):
            ...
            sub_agents.append(
                _agent_tool_to_sub_spec(
                    ...
                    # sub-agents have a path to launch them â€” see
    ...
    # policies, so we skip â€” callers in that position are either
    ...
    # Per designs/OMNIGENT_TERMINAL_BRIDGE.md Â§6.1 step 2 + Â§3 non-goal
...
def _translate_skills_filter_from_yaml(
    ...
    NOT read this field â€” it's agent-meow spec config that
    ...
    :returns: ``"all"``, ``"none"``, or a non-empty
        ``list[str]``. Normalizes ``[]`` â†’ ``"none"`` to match
...
def _self_agent_tool_to_sub_spec(
    ...
    *,
    ...
    â€” without this, re-translating would re-enter this branch and
    ...
    the agent-meow runtime spawns when the parent's LLM dispatches
    ...
    at runtime through the standard sub-agent dispatch path â€”
    ...
    :attr:`AgentSpec.guardrails` block too â€” clones share the
...
def _agent_tool_to_sub_spec(
    ...
    *,
    ...
    :param parent_profile: Databricks profile carried on the
        ...
        when the inline AgentTool omits one â€” inline agents in
        ...
        empty field does.
    :param parent_harness: agent-meow harness carried on the
        parent agent's executor. Used as the sub-spec's harness
        when the inline AgentTool's ``executor:`` block is
        absent. Same pattern as *parent_profile* â€” YAMLs like
    ...
    :param parent_os_env: OS environment on the parent agent.
        ...
        inline AgentTool â€” legacy agent-meow resolves it from the
    ...
    :param raw_executor: The raw ``executor:`` dict for this inline
        ...
        fields the agent-meow :class:`~?omnigent.inner.datamodel.ExecutorSpec`
        datamodel does not expose â€” ``auth`` and ``use_responses`` â€”
    ...
    # (see ``omnigent/tools/manager.py:426`` â€” "if not
    ...
    # sub-agents more than one level deep (e.g. researcher â†’
...
def _resolve_inline_agent_tool_os_env(
    ...
    agent-meow' :class:`AgentTool.os_env` is
    ...
    resolution â€” we bake in the parent's os_env here so the
...
def _fail_on_unsupported_tool(
    ...
    :param agent_name: Enclosing agent name, for error messages,
        ...
        name â€” the error message then renders ``None`` verbatim.
    ...
    # supported â€” the caller dispatches them appropriately.
    ...
    if isinstance(tool, MCPTool):
        ...
        # :func:`_translate_mcp_tool_from_def` â€” except for the
        # ``databricks_server=<name>`` shape, which references a
        # Databricks-managed MCP endpoint that agent-meow'
        ...
        if tool.databricks_server is not None:
            raise OmnigentError(
                ...
                f"``databricks_server={tool.databricks_server!r}`` â€” "
...
def _translate_llm_from_def(
    ...
    *,
    ...
    :class:`~?omnigent.datamodel.ExecutorSpec`. agent-meow splits
...
def _translate_executor_from_def(
    ...
    *,
    ...
    harness can be inferred â€” the spec must explicitly declare a
    ...
    trickery). ``os_env`` is NOT carried here â€” callers populate
    :attr:`AgentSpec.os_env` directly.

    Harness-resolution precedence (first non-empty wins):

    1. ``oa_executor.harness`` â€” explicit declaration in the YAML's
       ``executor:`` block.
    2. *parent_harness* â€” the parent agent's harness, threaded
    ...
    3. :func:`_infer_harness_from_model` on the resolved model
       string â€” mirrors pure agent-meow' CLI auto-pick
       (``databricks-claude-*`` â†’ ``claude-sdk``, etc.) so YAMLs
       that declare only a model continue to work under agent-meow mode.
    4. Error â€” when all of the above yield an empty string and a
    ...
    :param parent_profile: Profile to fall back to when
        *oa_executor* has no profile of its own. Used by
        ...
        ``--profile`` flows into the child task's executor â€”
    ...
    :param parent_harness: Harness to fall back to when
        *oa_executor* has no harness of its own. Matches the
        profile fallback's semantics â€” inline-AgentTool sub-
    ...
    # to ``""`` at the top of the function â€” one normalization
    ...
    #   (1) explicit child ``executor.harness``  â€” already in
    #       ``harness`` above.
    #   (2) child's own model auto-picks â€” if the child declared
...
def _translate_mcp_tool_from_def(
    ...
    - ``tool.url`` populated â†’ ``transport="http"`` with optional
      ``headers`` (used by Databricks-profile MCPs that hit an
      SSE endpoint with a Bearer token).
    - ``tool.command`` populated â†’ ``transport="stdio"`` with
    ...
    raise OmnigentError(
        ...
        f"'command' â€” cannot translate to an MCPServerConfig",
        code=ErrorCode.INVALID_INPUT,
    )
...
def _translate_function_tool_from_def(
    ...
    if isinstance(tool, CancellableFunctionTool):
        raise OmnigentError(
            ...
            f'``designs/SERVER_HARNESS_CONTRACT.md`` Â§"Async work '
    ...
    # Client-runtime tools have no server-side callable â€” the SDK
    # consumer implements them at stream-start time. Skip path
    # recovery (would fail: there's nothing to import) and
    # introspection (no callable to inspect â€” the YAML
    ...
    # No server-side callable to resolve or introspect â€” the
    ...
    # block (the common case for plain Python callables â€” e.g.
...
def _recover_callable_path(
    ...
    was removed in step (c) â€” only plain callables are supported
