...
def _validate_agents_sdk_executor(
    ...
    ``llm.connection`` is allowed â€” the SDK
    ...
    if spec.compaction is not None:
        result.add(
            "compaction",
            "not supported when executor.type is 'agents_sdk' â€” SDK manages context internally",
        )
...
def _validate_llm(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    if spec.executor.model is not None and spec.llm.model != spec.executor.model:
        result.add(
            ...
            f"({spec.llm.model!r}) disagree â€” use executor.model only",
        )
...
def _validate_local_tools(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    reserved builtin names (POLICIES.md Â§15.8).
        ...
        if tool.runtime == ToolRuntime.CLIENT:
            if tool.path is not None:
                result.add(
                    ...
                    f"must NOT declare a 'callable:' â€” the SDK "
                    f"consumer provides the implementation at "
                    f"stream-start time",
                )
            if tool.parameters is None:
                # No callable to introspect â†’ the YAML
...
def _validate_sub_agents(
    ...
    top-level spec â€” its own ``executor.type`` determines which
    ...
    # Validate each sub-agent independently â€” its own executor.type
...
# â€” it doesn't install a namespace or SBPL, so egress rules would be
...
def _validate_os_env(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    but ``AgentSpec`` may also be constructed programmatically â€”
    ...
    - ``egress_rules`` requires a sandbox backend that hard-enforces
      ...
      policy â€” silently bypassing the operator's intent.
...
def _validate_agent_names(
    ...
    - Non-empty â€” empty strings have no meaningful identity.
    - No dots â€” reserved as the delimiter between parent and sub-agent
      in the ``model`` field (e.g. ``"root.child"``).
    - No slashes â€” reserved by litellm as the ``provider/model``
      separator; a slash in a name would silently mis-route LLM calls.
    - No whitespace â€” whitespace in a model identifier confuses most
      API clients and logging pipelines.

    Names are additionally checked against ``_RESERVED_AGENT_NAMES`` â€”
...
def _check_unique_sub_agent_names(
    ...
    loading â€” see designs/SUBAGENT.md.
