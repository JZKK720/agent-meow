"""agent-meow compatibility surface â€” bundled for surgical removal.

ðŸš¨ **TECH DEBT â€” REMOVE WHEN agent-meow COMPAT WORKSTREAM ENDS.**
...
When agent-meow is consolidated (phase 6 of the integration design),
deleting agent-meow support means:
...
# intentionally excluded â€” agent-meow has a native databricks
...
# â”€â”€ Validator: agent-meow executor branch â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ YAML detection + loading â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def diagnose_yaml_rejection(path: Path) -> str:
    ...
    if _OMNIGENT_DISCRIMINATOR_KEY in raw:
        return (
            ...
            "spec â€” agent-meow specs must live in a directory with a "
...
def load_omnigent_yaml(
    ...
    *,
    ...
    Pipeline: ``omnigent.loader.load_agent_def(path)`` â†’
    :func:`~?omnigent.spec.omnigent.agent_def_to_agent_spec` â†’
    ...
    :param enforce_handler_allowlist: Forwarded to
        :func:`~?omnigent.inner.loader.load_agent_def` â€” when ``True``,
    ...
    :param prune_invalid_sub_agents: When ``True``, sub-agents that
        ...
        See :func:`~?omnigent.spec.load` for the full rationale â€” this
    ...
    # booleans parse consistently â€” importing load_agent_def
    ...
    if not result.valid:
        ...
        # sibling "required when executor.type is 'agent-meow' â€” must be
        ...
        if any(
            ...
            message += (
                "\n\nNote: if this harness is valid on a newer agent-meow server, "
