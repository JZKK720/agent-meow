...
def _harness_family(harness: str | None) -> str | None:
    ...
    Collapses variant suffixes/prefixes â€” ``claude-sdk``,
    ``claude-native`` and ``native-claude`` all map to ``"claude"`` â€”
    ...
    # ``agents_sdk``) to the hyphen form before splitting â€” the in-process
...
class SkillSourceContext:
    ...
    :param roots: Host-discovery roots in priority order â€” the session
...
def resolve_harness_skills(ctx: SkillSourceContext, harness: str | None) -> list[SkillSpec]:
    ...
    :returns: Deduplicated skill specs (first occurrence wins).
        Skills marked ``user-invocable: false`` are excluded â€” they are
...
def _enabled_plugin_settings_files(ctx: SkillSourceContext) -> list[Path]:
    """
    Settings files carrying ``enabledPlugins``, weakestâ†’strongest.
    ...
    approximate Claude's precedence for the menu (last-wins):
        ...
        user (home) .json â†’ .local  <  bundle .json â†’ .local
            <  workspace .json â†’ .local

    â€” ``.local`` over ``.json`` within each scope, project (roots) over
    ...
    """
    files: list[Path] = []
    # ``reversed`` puts the primary workspace (roots[0]) last â†’ strongest.
...
def _managed_plugin_keys(ctx: SkillSourceContext) -> set[str]:
    ...
    precedence â€” it cannot be overridden by a user/project ``enabledPlugins``
    toggle â€” so a managed plugin is enabled even when it carries no
    ...
    for path in _enabled_plugin_settings_files(ctx):
        ...
        if isinstance(ep, dict):
            for key, value in ep.items():
                # Accept only a real JSON boolean â€” a string ``"false"`` is
...
def _plugin_install_paths(ctx: SkillSourceContext, enabled: set[str]) -> dict[str, Path]:
    """Map ``<plugin>@<marketplace>`` â†’ installPath for enabled+installed plugins."""
...
def _claude_plugin_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    skill's own (bare) name â€” matching how the filter names skills,
...
def codex_host_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    Reuses the Codex executor's own helpers â€” ``codex_skill_sources`` (the
    shared source-list builder) and ``select_codex_skill_dirs`` (the shared
    selector) â€” so the menu draws from the same roots and selection the
    ...
    so a present-but-unparseable skill is linked but not shown (correct â€” Codex
    ...
    directory, and the executor symlinks under that dir name â€” so when the
...
def cursor_host_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    ``name`` â€” the dir carries the namespace and is collision-safe across
    ...
    try:
        ...
        # the lenient codex/_discover_skills behavior) â€” log and yield nothing.
...
def pi_host_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    (the unregistered SDK/CLI harnesses â€” antigravity, qwen â€” which route a
...
# (antigravity, qwen, openai-agents, â€¦) falls through to _generic_host_skills
# in resolve_harness_skills â€” the unchanged pre-existing ~/.claude/skills walk,
