...
def materialize_bundle(source: Path, dest: Path) -> Path:
    ...
    Taking the file-vs-directory branch once â€” here â€” means every
...
def load(
    ...
    *,
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        untrusted agent-bundle upload path â€” set by
    ...
    :param prune_invalid_sub_agents: When ``True``, a sub-agent that
        ...
        agent must still validate â€” a genuine root-level error always
    ...
    if isinstance(source, bytes):
        ...
        # agent-meow single-file YAML dispatch â€” see
        ...
        if source.suffix.lower() in {".yaml", ".yml"}:
            ...
            # loading from a tarball" â€” the file's a YAML, not a
            # tarball. Diagnose the actual reason and surface it.
            reason = diagnose_yaml_rejection(source)
            raise OmnigentError(
                f"{source}: not a valid agent-meow YAML â€” {reason}",
                code=ErrorCode.INVALID_INPUT,
            )
        if dest is None:
            raise OmnigentError(
    ...
    # â€” when a bundle (directory, tarball, or raw bytes) resolves to
    ...
    if enforce_handler_allowlist:
        ...
        # engine build), so a post-parse scan is safe and sufficient â€”
...
def _prune_invalid_sub_agents(spec: AgentSpec) -> list[str]:
    ...
    *spec* in place: the failing sub-agent is removed from
    ...
    True)`` â€” see :func:`load` for when and why it is enabled (the
    ...
    for sa in spec.sub_agents:
        ...
        _logger.warning(
            ...
            "produced by a newer agent-meow server with a feature this client does "
            "not support (e.g. a harness it does not recognize) â€” upgrade this "
...
def _reject_unregistered_spec_policy_handlers(spec: AgentSpec) -> None:
    ...
    ``sub_agents`` â€” the ``config.yaml`` parser discovers child agents
