...
callables â†’ agent-meow ``(ctx, context)`` convention.
...
instead â€” same arity, totally different semantics. When a legacy
...
This module bridges the gap. The agent-meowâ†’agent-meow
...
``(ctx, context)`` convention) return unchanged â€” the shim
...
# callable â€” the first two positional parameters must be exactly
...
# ``(phase, content)`` (wrong order) is also treated as modern â€”
...
def build(
    ...
    *factory_kwargs*, then decides whether the resolved callable
    ...
    :param factory_kwargs: If the YAML declared
        ...
        *target* as a factory first â€” ``target(**factory_kwargs)``
    ...
    :param configured_phases: The YAML ``on:`` list (verbatim
        ...
        evaluation â€” matching the contract
        ...
        add the key â€” used only by tests that build the shim
...
def _has_legacy_signature(fn: Callable[..., Any]) -> bool:
    ...
    without ``__signature__``) return False â€” those are treated
...
def _coerce_legacy_return(result: Any) -> Any:
    ...
    - A :class:`PolicyResult`-shaped object (already handled by
      ``_coerce_to_policy_result`` via ``hasattr(raw, "action")``) â€” pass through.
    - A dict with ``"action"`` key (old format, e.g.
      ``{"action": "deny", "reason": "..."}``) â€” converted to the
      decision dict shape
      ``{"result": "DENY", "reason": "..."}``.
    - Anything else â€” pass through (the caller handles it).
...
def _v0_event_to_legacy_phase(event: dict[str, Any]) -> str:
    ...
    """
    # Inner system uses "request"/"response"/"tool_call"/"tool_result" â€”
...
def _v0_event_to_legacy_content(event: dict[str, Any]) -> Any:
    ...
    ``isinstance(content, dict)`` keep working â€” mirroring
...
def _legacy_content(ctx: EvaluationContext) -> Any:
    ...
    normalized shapes almost exactly â€” this helper bridges the
    ...
    - ``TOOL_RESULT``: agent-meow passes the raw tool output
      ...
      â€” when an MCP server emits a single JSON-formatted text
    ...
    if ctx.phase in (Phase.REQUEST, Phase.RESPONSE, Phase.TOOL_CALL):
        return ctx.content
    # TOOL_RESULT â€” JSON-parse the string so legacy callables
...
def _maybe_parse_json(value: Any) -> Any:
    ...
    :param value: The candidate string (or any other value â€”
...
def _legacy_context(
    ...
    *,
    ...
    "configured_phases": [...], "tool_name": "sleep"}`` â€” but
    ...
    extra context â€” the legacy callable reads the tool name
    ...
    :param configured_phases: The policy's YAML ``on:`` list,
        forwarded by :func:`build`. ``None`` omits the key â€”
