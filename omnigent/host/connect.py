...
_RUNNER_ENV_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        # CLI â†’ daemon â†’ local-server chain or `agent-meow run`/`connect` would
        ...
        # ``OMNIGENT_CLAUDE_SDK_NO_SANDBOX=1 agent-meow run â€¦`` had no
...
# both the standard ``OTEL_*`` vars and agent-meow's ``OMNIGENT_OTEL_*`` knobs
...
def _build_runner_env(
    ...
    *,
    ...
    :param base_env: Host process environment to filter, e.g.
        ``os.environ``.
    :param server_url: agent-meow server URL the runner connects back to, e.g.
...
class HostProcess:
    ...
    :param server_url: agent-meow server URL, e.g.
    ...
    async def _orphan_reaper_loop(self) -> None:
        ...
        (``start_new_session=True`` â€” ``omnigent.inner._proc.spawn_kwargs``),
    ...
    def _build_connect_headers(self) -> dict[str, str]:
        ...
        try:
            ...
            # which only the runner subprocess sets â€” without it the
...
def run_host_process(
    ...
    # Tell the user where logs land up front â€” `agent-meow host` used to run
    ...
    # that path is None in the background daemon (no setup_cli_logging) â€”
