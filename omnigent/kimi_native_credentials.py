"""Per-session ``KIMI_CODE_HOME`` builder that injects agent-meow hooks.
...
relative to the same home â€” there is no project-level merge for the ``hooks``
...
- symlinks every entry of the user's global home (oauth, credentials,
  sessions, â€¦) so login / providers / history keep working, and
- carries a ``config.toml`` that is the user's config text with two agent-meow
  ``[[hooks]]`` appended â€” a ``PreToolUse`` deny-gate and a ``PermissionRequest``
...
#: Env var Kimi Code reads to locate its data dir (config.toml + oauth + â€¦).
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    """Render the two agent-meow ``[[hooks]]`` entries as TOML text.

    Both hooks dispatch to :mod:`~?omnigent.kimi_native_hook` with the bridge
    dir baked into the command (no secrets on the command line â€” the hook reads
    ...
    # package and the hook dies on ``ImportError`` before it can POST â€” so the
    ...
    # No ``matcher`` â†’ matches every tool. Commands are TOML basic strings;
    ...
    # evaluate. Pin both to kimi's 600s ceiling â€” the longest the human may take
    # to answer the card â€” after which kimi's own TUI prompt stands.
    return (
        "\n"
        "# --- agent-meow native hooks (auto-generated; do not edit) ---\n"
        "[[hooks]]\n"
        'event = "PreToolUse"\n'
        f'command = "{pre}"\n'
        "timeout = 600\n"
...
def build_kimi_session_home(
    ...
    *,
    ...
    """Materialize a session-scoped ``KIMI_CODE_HOME`` with agent-meow hooks.
    ...
    the user's config plus the agent-meow hooks. Best-effort and idempotent:
