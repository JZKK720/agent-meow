"""Runtime package â€” public API for accessing runtime state.
...
def get_conversation_store() -> ConversationStore:
    ...
    if store is None:
        raise RuntimeError("runtime not initialized â€” call init() first")
    return store
...
def get_file_store() -> FileStore | None:
    ...
    optional â€” multimodal file_id resolution is simply skipped
...
def get_artifact_store() -> ArtifactStore | None:
    ...
    is optional â€” multimodal file_id resolution is simply skipped
...
def get_comment_store() -> CommentStore | None:
    ...
    optional â€” comment tools surface a clear error to the agent when
...
def get_policy_store() -> PolicyStore | None:
    ...
    optional â€” the policy engine falls back to spec-declared
...
def get_dispatch_capability(
    ...
    :returns: The :class:`_globals.DispatchCapability`, or
        ...
        different process â€” both surface to the caller as a
...
def unregister_dispatch_capability(parent_task_id: str) -> None:
    ...
    Idempotent â€” missing keys are silently ignored so the
...
def get_terminal_registry() -> TerminalRegistry:
    ...
    ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.2.
...
def set_runner_router(router: RunnerRouter | None) -> None:
    ...
    Production agent-meow servers use this instead of a process-wide runner
...
def get_harness_process_manager() -> HarnessProcessManager:
    ...
    :raises RuntimeError: If the lifespan startup hasn't run yet
        (e.g. workflow invoked before agent-meow boot completed, or in a
    ...
    if pm is None:
        raise RuntimeError(
            "HarnessProcessManager not initialized â€” agent-meow lifespan startup "
