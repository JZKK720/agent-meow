...
1. Surgical clearing â€” tool result bodies and binary content blocks
   outside the recent window are replaced with markers.
2. LLM summarization â€” a @step LLM call summarises all messages
   outside the recent window into a single summary pair.
3. Truncation â€” oldest messages are dropped when layers 1+2
...
_TOOL_RESULT_CLEARED = "[Previous tool result cleared â€” re-call tool if needed]"

# Marker written into cleared binary content block payloads.
_BINARY_CONTENT_CLEARED = (
    "[binary content removed for context management â€” use file_id to retrieve]"
)
...
class CompactionResult:
    ...
    :param total_tokens: Tiktoken estimate of the post-compaction
        ...
        budget checks â€” no additional tokenization pass needed.
...
def count_tokens(messages: list[dict[str, Any]], model: str) -> int:
    ...
    the authoritative count â€” tiktoken is ~85-95% accurate for
    ...
    try:
        ...
        # Unknown model â€” fall back to the most common encoding.
...
def _clear_tool_results(
    ...
    preserved â€” no orphaned tool calls are created. Only the
...
def _truncate_oldest(
    ...
    Preserves tool call pair integrity â€” never drops a
...
async def summarize_history(
    ...
    llm_client: Any,  # llms.Client â€” typed as Any to avoid circular import
    model: str,
    connection: dict[str, str] | None = None,
...
def compaction_to_history_items(
    ...
    message provides it. This avoids attribution confusion â€”
...
async def compact(
    ...
    *,
    ...
    1. **Layer 1** â€” Clear tool result bodies and binary content
       blocks outside the recent window (fast, no LLM call).
    2. **Layer 2** â€” LLM summarization of messages outside the
       recent window (slow).
    3. **Layer 3** â€” Truncate oldest messages (emergency fallback).

    The in-memory *history* list is never modified â€” only the
    *messages* copy passed to the LLM is compacted.

    :param messages: The messages list to compact. This is a copy
        â€” the original history is not modified.
    ...
    :param conversation_id: When set, ``response.compaction.in_progress``
        ...
        ``None`` for explicit ``/compact`` â€” sessions.py handles those
    ...
    if conversation_id:
        # Publish to session stream so REPL/web UI shows "Compactingâ€¦" indicator.
    ...
    if summary_metadata is not None:
        ...
        # Summary + recent still exceeds budget â€” fall through to Layer 3.
        working = compacted

    # --- Layer 3 ---
    _logger.warning(
        "Layer 3 truncation triggered for task %s â€” context still exceeds budget after layers 1+2",
        task_id,
    )
    truncated = _truncate_oldest(working, budget, model)
    l3_tokens = count_tokens(truncated, model)
    # Emit completed on the Layer 3 path so "Compactingâ€¦" spinners
    ...
    try:
        ...
        if _is_summary_auth_error(exc):
            ...
            _logger.error(
                ...
                "(%s) â€” the summarizer's credentials are missing or invalid, so "
            ...
            _logger.warning(
                "Layer 2 summarisation failed for task %s â€” falling back to Layer 3",
                task_id,
                exc_info=True,
            )
        if fail_on_error:
