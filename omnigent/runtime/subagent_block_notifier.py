...
class _WakeOutcome(enum.Enum):
    ...
    ``DELIVERED`` â€” the dispatch confirmed delivery; ``MOOT`` â€” the block
    resolved mid-flight so nothing (stale) was sent and the cleared
    debounce slot must be left alone; ``FAILED`` â€” every attempt failed,
...
class SubagentBlockNotifier:
    ...
    def observe(self, conversation_id: str, event: dict[str, Any]) -> None:
        ...
        # No id â†’ can't be matched by its resolve, so never arm.
        ...
        with self._lock:
            if elicitation_id in self._notified:
                return  # already armed for this block â€” debounce
    ...
    def _discard_inflight(self, future: concurrent.futures.Future[None]) -> None:
        ...
        if future.cancelled():
            return  # teardown via close() â€” expected, not an error
    ...
    async def handle_request(
        ...
        *agent* if the block is still unanswered (the debounce arm doubles
        as the cancellation token â€” :meth:`observe` clears it on resolve).
        ...
        with self._lock:
            if elicitation_id not in self._notified:
                return  # answered within the grace â€” the parent never needs to know
    ...
    async def _deliver_with_retry(
        ...
        *,
        ...
        the cleared slot alone. ``None`` (resolution notices â€” the block is
        ...
        :param notice: Pre-formatted ``[System: â€¦]`` notice text.
            ...
            if armed_id is not None:
                with self._lock:
                    if armed_id not in self._notified:
                        # Resolved during the grace/backoff â€” block's gone, the
...
def _format_block_notice(child: Conversation, event: dict[str, Any]) -> str:
    """
    Build the ``[System: â€¦]`` notice posted into the parent session.
    ...
    the parent to surface it â€” it does not prescribe a specific tool.
    ...
    return (
        ...
        "unanswered â€” surface the situation to the human and do not wait "
...
def _format_resolution_notice(child: Conversation) -> str:
    ...
    :returns: A one-line ``[System: â€¦]`` notice, e.g. ``"[System:
        ...
        resolved and it is continuing. â€¦]"``.
    """
    label = _child_label(child)
    return (
        f"[System: sub-agent {label}'s pending approval has been resolved and it "
        "is continuing. No action is needed on the earlier block notice â€” if you "
...
def _block_reason(event: dict[str, Any]) -> str | None:
    ...
    if len(message) > _REASON_MAX_CHARS:
        return message[: _REASON_MAX_CHARS - 1].rstrip() + "â€¦"
    return message
