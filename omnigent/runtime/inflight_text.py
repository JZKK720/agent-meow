...
Lets the agent-meow server replay the text streamed so far when a client
(re)connects mid-turn â€” fixing the bug where, for non-claude-native
...
``response.output_text.delta`` events â€” there is no
...
tail of deltas happened to arrive after it resubscribed â€” "a few
...
ends â€” on a terminal ``response.*`` event, on a terminal
...
Reasoning deltas are intentionally NOT tracked â€” reasoning is
...
the durable transcript. The index lives only in the agent-meow process, so it
does not survive an AP-server restart mid-turn â€” acceptable, because
...
``yield``/``await``*. On the single agent-meow event loop where the relay
...
# (cancels the turn â†’ ``session.status: idle``, no ``response.cancelled``),
...
# Per-conversation mapping of conversation_id â†’ in-flight turn text.
...
# Per-conversation, insertion-ordered mapping of message_id â†’ its
...
# its authoritative ``response.output_item.done`` commits â€” matched to the
...
# its committed item â€” at commit time that message isn't tracked yet, so
...
# with text that hasn't committed â€” see the session.status branch); the
...
# committed â€” dropped from ``_native_inflight`` and barred from re-entry.
...
# concern â€” the race window is a single forwarder poll).
_native_retired: dict[str, dict[str, None]] = {}

# Per-conversation multiset of committed-message text fingerprints awaiting
# their deltas, keyed by HASH â†’ list of monotonic buffered-at timestamps
# (one per occurrence). Populated when an ``output_item.done`` commits but
# NO in-flight message yet matches its text â€” i.e. the deltas raced behind
...
# count â€” we only need to retire one message per committed text, and
...
# arrive (e.g. a multi-text-block message â€” see :func:`record_publish`) so
...
# suppressed (a possible â€” not guaranteed â€” duplicate), never a crash.
_MAX_NATIVE_COMMITTED_PER_CONV = 256

# How long a buffered committed-text fingerprint stays a valid suppression
# match. The real commitâ†’delta race is ~1 forwarder poll (~0.25s); this sits
...
def _monotonic() -> float:
    ...
    tasks / pytest-xdist workers â€” see the project's no-global-singleton-
...
def _committed_message_text(item: dict[str, Any]) -> str | None:
    ...
    shape with only non-text blocks) â€” the caller then neither matches nor
...
def _match_committed_native_message(conversation_id: str, fingerprint: str) -> str | None:
    ...
    whose ``final`` chunk has arrived are candidates â€” before that
...
def _consume_committed_fingerprint(conversation_id: str, parts: list[str]) -> bool:
    ...
    :data:`_NATIVE_COMMITTED_TTL_S` seconds â€” i.e. this message's
    ...
    discarded and ``False`` is returned if nothing fresh remains â€” see
    ...
    if not timestamps:
        return False
    # Drop occurrences that have outlived the commitâ†’delta race window;
...
def _buffer_committed_fingerprint(conversation_id: str, fingerprint: str) -> None:
    ...
    raced behind the commit â€” the single-chunk case). The message's
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> bool:
    ...
    * ``response.created`` / ``response.in_progress`` â€” capture the
    ...
    * ``response.output_text.delta`` WITHOUT a ``message_id`` â€” append
      the delta to the current (response-scoped) turn's accumulated text.
    * ``response.output_text.delta`` WITH a ``message_id`` (native
      message-scoped streaming) â€” append to that message's own buffer in
    ...
    * ``response.output_item.done`` for a ``message`` item â€” it just
    ...
    * a terminal turn event (see :data:`_TERMINAL_EVENT_TYPES`) â€” drop
    ...
    * a ``session.status`` event whose status is terminal (see
      :data:`_TERMINAL_STATUS_VALUES`) â€” drop only the RESPONSE-SCOPED
      ...
      NOT dropped here â€” claude-native goes ``idle`` mid-turn while
    ...
    :returns: ``True`` when the event must be WITHHELD from the live
        fan-out â€” a claude-native ``output_text.delta`` for an already-
    ...
    if event_type == "response.created" or event_type == "response.in_progress":
        ...
        with _lock:
            ...
            if entry is None or entry.response_id != response_id:
                ...
                # Same turn â€” refresh the response object (e.g. the
    ...
    if event_type == "response.output_text.delta":
        ...
        if isinstance(message_id, str) and message_id:
            ...
            # (``delta="", final=true`` â€” pi-native's end-of-message signal,
            ...
            # messages were NEVER retired â€” ``snapshot_for`` then replayed
            ...
            with _lock:
                ...
                # live fan-out too â€” see :data:`_native_retired`.
                ...
                if message is None:
                    if not delta and not final:
                        ...
                        # carries neither text nor a completion signal â€”
                ...
                if final:
                    ...
                    # behind it â€” the single-chunk case), a fingerprint is
        ...
        if not delta:
            ...
            # completion signal â€” there is nothing to accumulate.
            return False
        with _lock:
            entry = _inflight.get(conversation_id)
            if entry is None:
    ...
    if event_type == "response.output_item.done":
        ...
        # (``final``) chunk is POSTed AFTER its commit â€” at this point that
    ...
    if event_type == "session.status":
        ...
        if isinstance(status, str) and status in _TERMINAL_STATUS_VALUES:
            with _lock:
                ...
                # store â€” clearing it here would lose exactly that text on
...
def discard(conversation_id: str) -> None:
    ...
    Called from the agent-meow relay's teardown (``_relay_runner_stream``'s
    ``finally``) so a relay that exits WITHOUT a terminal turn event â€”
    a runner death / tunnel drop mid-turn, a ``[DONE]`` with no
    preceding terminal, or a PATCH-rebind cancellation â€” does not strand
...
def reset_text(conversation_id: str) -> None:
    ...
    untouched â€” it has its own per-message commit eviction.
...
def reset_for_tests() -> None:
    ...
    callers â€” there is no legitimate runtime use case for wiping it.
    """
    with _lock:
        _inflight.clear()
