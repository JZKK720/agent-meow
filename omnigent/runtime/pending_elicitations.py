...
Lets the agent-meow server answer "which sessions have a pending approval
prompt?" without scanning per-session state or persisting elicitation
rows. The index is the *sidebar's* view of pending state â€” it lives
...
lifecycle: when the agent-meow process dies, both the index and every parked
...
UI's chat blocks on cold load â€” the SSE stream itself has no replay,
so an elicitation emitted before the user opened the chat would
otherwise render as nothing.

Limitations:

* In-memory only; multi-replica agent-meow deploys would each see their own
  slice. This matches the existing ``_harness_elicitation_registry``
  constraint â€” when a shared backplane is added for the registry,
  this index should be wired through the same backplane.
* Events emitted before the agent-meow server starts (e.g. between turns,
...
# Per-conversation mapping of outstanding elicitation_id â†’ original
...
# tracked event â€” must be cheap + non-blocking. ``None`` (runner, tests) skips it.
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> None:
    ...
    Acts on two event types and silently ignores every other type â€”
    the function sits on the hot publish path so unrelated events
    must pay only one dict-key lookup.

    * ``response.elicitation_request`` â€” add the elicitation id +
    ...
    * ``response.elicitation_resolved`` â€” drop the elicitation id
      ...
      agent-meow server picks it up via ``_relay_runner_stream`` without
      a separate out-of-band signal.

    Idempotent on both event types â€” adds use a dict assignment
    (re-publishing the same id overwrites the payload) and drops are
    no-ops for unknown ids.

    After updating the index, a registered observer (if any â€” see
...
def resolve(conversation_id: str, elicitation_id: str) -> None:
    ...
    accepted (regardless of which downstream awaiter â€” runner
    workflow or server-side ``_harness_elicitation_registry``
    Future â€” receives the verdict). Also called by
...
def pending_session_ids() -> list[str]:
    ...
    snapshot â€” that walk costs one ``list_conversations`` query per
...
def snapshot_for(conversation_id: str) -> list[dict[str, Any]]:
    ...
    opened the chat would never render â€” the SSE stream has no
    replay buffer.
...
def project_for_peek(event: dict[str, Any]) -> dict[str, Any]:
    ...
    only in this index â€” see the module docstring), so get_history must
    ...
    Used by both read paths â€” the in-process
...
def lookup(elicitation_id: str) -> tuple[str, dict[str, Any]] | None:
    ...
    requiring a database round-trip â€” the in-memory index already
...
def reset_for_tests() -> None:
    ...
    production callers â€” there is no legitimate use case for
