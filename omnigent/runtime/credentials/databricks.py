...
class WorkspaceCreds:
    ...
    :param host: The workspace host URL with no trailing slash,
        ...
        the BARE workspace host â€” callers append their own path
...
class _SectionAbsent(Exception):
    ...
    Distinguishing "section absent" (this exception â€” fail loud when
    ...
    ``[DEFAULT]`` points to â€” a completely different workspace.
    """
...
class _SectionPresentButInvalid(Exception):
    ...
    Distinguishing "section absent" (:class:`_SectionAbsent` â€” fail
    loud when a profile was explicitly named) from "section present
    but invalid" (this exception â€” also fail loud) prevents the
...
def _read_section(config: configparser.ConfigParser, section: str) -> WorkspaceCreds | None:
    ...
    - Section absent â†’ raises :class:`_SectionAbsent`.
    - Section present and complete â†’ returns :class:`WorkspaceCreds`.
    - Section present but missing ``host`` or ``token`` â†’ raises
      :class:`_SectionPresentButInvalid`.

    For ``DEFAULT``: absent or incomplete â†’ returns ``None`` (no
    ...
    if section == DEFAULT_SECTION:
        ...
        # caller's perspective â€” nothing to fall back to FROM, since
    ...
    # falls through to None â€” there's nowhere further to fall back.
    if section == DEFAULT_SECTION:
        return None
...
def _call_sdk_authenticate(profile: str | None) -> WorkspaceCreds | None:
    ...
    try:
        ...
        _logger.warning(
            "databricks-sdk is not importable: %s â€” OAuth profiles will be invisible "
    ...
    try:
        ...
        _logger.info(
            "databricks-sdk Config(profile=%r).authenticate() failed: %s â€” "
...
def _try_resolve_via_sdk(profile: str | None) -> WorkspaceCreds | None:
    ...
    the ONLY path that yields a usable token â€” the raw configparser
    ...
    the inner/ legacy behavior â€” exotic setups that predate the SDK's
...
def _try_resolve_from_cfg(profile: str | None, cfg_path: Path) -> WorkspaceCreds | None:
    ...
    2. If the named section is ABSENT, :class:`_SectionAbsent`
       propagates â€” the caller (:func:`resolve_databricks_workspace`)
    ...
    3. If the named section is PRESENT but missing required fields,
       :class:`_SectionPresentButInvalid` propagates â€” same fail-loud
       treatment.
...
def resolve_databricks_workspace(profile: str | None) -> WorkspaceCreds:
    ...
    # the SDK path (Config raises ValueError â†’ returns None) but then silently
    ...
    try:
        ...
        # file â€” fail loud rather than silently routing to [DEFAULT]
