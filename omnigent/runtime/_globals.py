...
# See Â§Step 5b in designs/SERVER_HARNESS_CONTRACT.md /
...
# existing direct-to-harness path. See ``designs/RUNNER.md`` Â§7
# Phase 5.
...
# context than the one that registered the entry. Process-local â€” a
...
def init(
    *,
    ...
    # ``designs/OMNIGENT_TERMINAL_BRIDGE.md`` Â§4.2 for the design.
    # Sandbox enforcement is per-terminal (declared on the spec's
    # ``TerminalEnvSpec.os_env.sandbox``), not a registry-wide
    # toggle â€” different terminals in the same agent can have
