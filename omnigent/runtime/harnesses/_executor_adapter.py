...
openai-agents-sdk) all use this adapter â€” they only differ in the
inner ``Executor`` they construct. The adapter handles:

- Per-conversation lazy executor construction (Layer 1 state on
  the adapter instance).
- Per-turn translation of agent-meow :class:`CreateResponseRequest` â†’
  inner :class:`Message` list + :class:`ExecutorConfig`.
- Per-turn translation of inner :class:`ExecutorEvent`s â†’
...
- Cancellation propagation (POST /cancel â†’ ctx.cancelled â†’
  inner ``Executor.interrupt_session``).
- Per-conversation cleanup on shutdown.

V1 limitations (documented in Â§Autonomous decisions):
...
# server-dispatched tool calls use â€” see Â§Sub-agent representation
...
#    REPL the ``âµ tool_name`` line interleaved with text rather
...
#    event against the inline observed event â€” single render,
#    no duplicates.
# 3. Skip the observed function_call_output emission on
#    ToolCallComplete â€” the dispatch's PATCH handler emits the
...
def _strip_mcp_tool_prefix(name: str) -> str:
    ...
    and persisted conversation items carry â€” kept in sync with
    ...
    â€” kept local here to avoid a cross-module import that
...
class ExecutorAdapter(HarnessApp):
    ...
    The per-harness wrap supplies an ``executor_factory`` â€”
    ...
    adapter instance â€” reused across turns for the conversation's
    ...
    :param session_key: Stable identifier the inner executor uses
        ...
        â€” production wraps may want to set it from the
    ...
    def __init__(
        ...
        # e.g. "Claude" â†’ "Claude wants to call **Bash**" or
        # "Cursor" â†’ "Cursor wants to use **Bash**".
        ...
        # the MCP handlers cached inside ``ClaudeSDKClient`` â€”
        # which closure-capture whatever ``_tool_executor`` was
        # set on the FIRST turn â€” always dispatch into the
        ...
        # already carried â€” that's what lets the agent-meow REPL dedupe
        ...
        # ``âµ tool_name`` twice plus an empty result panel for
        # the orphan call (the 2026-04-29 user-reported
        # duplicate-render + empty-box regression for
        # openai-agents â€” same shape as the 2026-04-28 claude-
        ...
        # executor, which has no ``_stable_tool_executor`` dispatch) â€” those are
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        ...
        # calls â€” which use ``self._session_key`` from this
        # adapter â€” return ``False`` because the cached client
        ...
        # a turn-1 closure forever â€” every later tool call would
        ...
        # attribute set is best-effort â€” executors that don't
        ...
        agent_span = None
        # Active tool span for correlating ToolCallRequest â†’ ToolCallComplete.
        ...
        # in-flight session â€” without this hook, AP-forwarded
        ...
        try:
            ...
            # spans) is tagged with session.id by the span processor â€” no
            ...
            _logger.info(
                "elicitation explicitly declined for response %s â€” aborting turn",
                ctx.response_id,
            )
            if tctx is not None and agent_span is not None:
                from omnigent.runtime.telemetry import record_cancellation
            ...
            # against an ``asyncio.Queue.get`` â€” cancelling is
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        client â€” dropping which is what actually stops the in-flight
        generation and forces the next turn to rebuild fresh â€” was only
    ...
    async def _watch_injections(self, ctx: TurnContext, executor: Executor) -> None:
        ...
        Best-effort throughout â€” a failed injection logs and
        ...
        :param executor: The inner executor to inject into.
            ...
            forwarding to actually deliver â€” executors that
        ...
        while True:
            ...
            if injection is None:
                ...
                # sentinel. Defensive â€” bail rather than spin.
                return
            text = _extract_user_text(injection.input)
            if not text:
                # Empty / malformed input â€” skip rather than
                ...
                continue
            if ctx.cancelled.is_set():
                # Turn interrupted â€” don't deliver into the dying session.
                return
            try:
    ...
    async def _stable_tool_executor(
        ...
        returns a clear error payload â€” letting the call park
        ...
        # in :func:`_bridge_one_dispatch` â€” that loses the dedup
        # but doesn't break the dispatch protocol.
        # ``_stable_tool_executor`` IS the SDK's MCP-server tool
        # callback â€” only invoked for MCP-routed tools. The
        ...
        # ToolCallComplete for these ids â€” dispatch_tool already emits their
        # function_call_output â€” while letting internally-run SDK tool
    ...
    async def _stable_elicitation_handler(
        ...
        so the elicitation always lands in the active turn's context â€”
        ...
        ctx = self._current_ctx
        if ctx is None:
            # Elicitation callback fired with no active turn â€” this is a
        ...
        if result.action == "decline":
            ...
            # via ctx.cancelled instead â€” the run_turn event loop checks this
    ...
    async def _stable_policy_evaluator(
        ...
        if ctx is None:
            ...
            # the runner's phase-aware default in _evaluate_policy_via_omnigent â€”
    ...
    def _translate_event(self, event: ExecutorEvent, ctx: TurnContext) -> None:
        ...
        executed the tool natively) â€” they surface as paired
        function_call + function_call_output items with
        ``status: "completed"`` per Â§Sub-agent representation.
        ...
        if isinstance(event, TextChunk):
            ...
            # inner SDK parses each tool_use block â€” that's what
            # gives the REPL a ``âµ tool_name`` line interleaved
            ...
            # against this observed event â€” keeping the inline
            ...
            # ``âµ tool_name`` twice (which is what 989bfde
            # mitigated by suppressing this branch entirely â€”
            ...
            # carry the bare form â€” kept consistent with
            ...
            # needs the same correlation â€” openai-agents
            ...
            # STRICTLY by call_id and discard empty ones â€” there is NO
            ...
            #
            # A tool routed through _stable_tool_executor â†’
            ...
            # ``_dispatched_call_ids``) â€” the scaffold is their single
            # output source.
            #
            # Tools the inner SDK ran ENTIRELY INTERNALLY â€” the
            # antigravity executor has no _stable_tool_executor
            # dispatch at all, and its ToolCallComplete is bridged
            # from the SDK's PostToolCall/OnToolError hooks â€” never go
            ...
            # Suppress two cases â€” each would otherwise put a useless or duplicate
            # function_call_output on the SSE stream:
            #   1. a DISPATCHED id â€” _stable_tool_executor already emitted the
            #      authoritative output via ctx.dispatch_tool (its single source);
            #      a second one here duplicates it and ghosts a "Waiting for
            #      output" card.
            #   2. an EMPTY call_id â€” downstream consumers pair STRICTLY by call_id
            ...
            if event.response is not None:
                ...
                # matches the streamed deltas â€” but that
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        1. :class:`~?omnigent.errors.OmnigentError` (incl.
           ...
           â€” already carries a semantic ``code`` string per the
           project's own classification, so use it verbatim.
        2. OpenAI SDK exceptions â€” surfaced by the inner OpenAI
        ...
        3. ``claude_agent_sdk`` exceptions â€” the Claude Code CLI
        ...
        # Unknown exception type â€” preserve the class name so
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    ...
    retryable allowlist verbatim â€” adding more codes here without
    ...
    ``None`` so the caller falls through to other classifiers â€”
    ...
    if isinstance(exception, openai.InternalServerError):
        ...
        # retrying â€” usually transient capacity issues. Matches
...
def _classify_claude_sdk_exception(exception: BaseException) -> str | None:
    ...
    retryable allowlist entry â€” ``CLINotFoundError`` (binary
    ...
    # in the SDK's hierarchy â€” match it FIRST and return ``None``
...
def _classify_httpx_exception(exception: BaseException) -> str | None:
    ...
    classifier â€” though httpx is currently a hard project
    ...
    # behave the same way for retry purposes â€” the connection
    ...
    # sending complete message body" â€” the canonical symptom of a
    # subprocess being SIGKILL'd mid-stream). Sits under
    # ``ProtocolError`` â†’ ``TransportError``, NOT under
    ...
    # as permanent â€” the L2 retry layer never fires for the exact
...
def _classify_anthropic_exception(exception: BaseException) -> str | None:
    ...
    allowlist doesn't match â€” silent demotion of retryable
    ...
    classifiers â€” matches the established pattern of the
    sibling classifiers in this module.

    Phase 3 of ``designs/RETRY_ACROSS_HARNESSES.md`` â€”
    ...
    if isinstance(exception, anthropic.InternalServerError):
        # 500-class server-side failures from Anthropic â€” same
...
def classify_inner_exception(exception: BaseException) -> str | None:
    ...
    classifier recognizes the exception â€” caller is expected
    ...
    none do â€” each lazy import only matches its own SDK's
    classes â€” but if Anthropic ever subclasses an httpx
...
def _normalize_tool_schemas(
    ...
    directly â€” they expect a flat shape. Without this
    ...
    for schema in schemas:
        ...
        if "name" in schema:
            ...
            flat.append(schema)
            continue
        # Chat-completions form â€” pull from the ``function``
...
async def _bridge_one_dispatch(
    ...
    *,
    ...
    :param call_id: Optional explicit ``call_id`` to use on the
        ...
        freshly-allocated uuid â€” the dispatch still works but the
    ...
    # Try to parse the output as JSON â€” if agent-meow serialized the
...
def _extract_user_text(
    ...
    :param input_value: The injection's ``input`` field â€” either
...
def _translate_input_to_messages(
    ...
    1. **Conversation-history shape** â€” a list whose entries are
    ...
    2. **Single-turn fallback shape** â€” a plain string or a
       ...
       Concatenated into a single user-role message â€” same
...
def _extract_role_keyed_messages(
    ...
    for item in input_value:
        ...
        # doesn't break the prompt â€” the prior user turn still
...
def _serialize_tool_result(event: ToolCallComplete) -> str:
    ...
    - A plain ``str`` â€” pass through unchanged.
    - A list of typed content blocks (``{"type": "text", "text":
      ...}``, image blocks, etc.) â€” join the ``text`` fields so
    ...
    - Anything else (dicts, ints, ``None``, â€¦) â€” best-effort
    ...
    - ``error`` is preferred when ``result`` is None and the
      status carries one.
    - Empty string is a last resort â€” the inner Executor
...
def _stringify_tool_payload(value: Any) -> str:
    ...
    See :func:`_serialize_tool_result` for the full rationale â€”
    ...
    if isinstance(value, list):
        ...
        # have no string projection here and are skipped â€” a
        ...
        # No usable text blocks â€” fall through to JSON encode so
