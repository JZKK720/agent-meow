...
- ``--conversation-id <id>``: the conversation this process serves.
  ...
  its in-memory state per Â§Harness in-memory state in the design
...
The runner is intentionally minimal â€” import, factory call, state
stash, uvicorn launch. All harness-specific behavior lives in the
per-harness ``create_app()`` factory.

See ``designs/SERVER_HARNESS_CONTRACT.md`` Â§Required harness
package shape and Â§Process management.
"""
...
# stuck streaming response blocks the process forever â€” the root
...
# thread. 1 s is responsive enough (parent crash â†’ runner exit
...
def _load_harness_app(harness: str, module_path: str, conversation_id: str) -> FastAPI:
    ...
    # state containers (per Â§Harness in-memory state) key off this.
...
def _start_parent_watchdog(parent_pid: int) -> threading.Thread:
    ...
    def _watch() -> None:
        while True:
            ...
            # shutdown â€” a hard exit that skips graceful terminal teardown and
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Windows). ``log_level`` keeps per-process noise low â€” see
