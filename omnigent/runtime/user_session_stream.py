...
the CLI, or one shared with the user) â€” that id was never in its watch-set.
...
into the sidebar â€” so a new session appears within a tick of being created, and
...
has no stream connected are simply dropped â€” that user's next page load fetches
...
def publish(user_key: str, event: dict[str, Any]) -> None:
    ...
    :param user_key: The target user's discovery key â€” the authenticated user
...
async def subscribe(user_key: str) -> AsyncIterator[dict[str, Any]]:
    ...
    from :func:`publish`. Live-tail only â€” events emitted before this call are
