...
:func:`inject_user_message` (tmux bracketed paste + Enter) â€” the cursor analog
...
#: when mirroring the user turn back to agent-meow so the copied history isn't
...
def wrap_fork_preamble(preamble: str, user_text: str) -> str:
    ...
    duplicated in the agent-meow timeline.
    ...
    real open/close pair â€” this keeps the forwarder's non-greedy strip
...
def build_mcp_config(
    ...
    *,
    ...
    """Build Cursor's ``.cursor/mcp.json`` for the agent-meow relay server.

    Cursor prompts for MCP tool approval before it sends ``tools/call`` to the
    server. agent-meow tools already route through the agent-meow ``/mcp`` proxy,
    ...
    hidden in-terminal approval prompt blocking the call before agent-meow ever
    sees it, while preserving agent-meow's own policy/elicitation gate.
    """
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config required by the shared agent-meow MCP bridge."""
    _ensure_dir(bridge_dir)
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped Cursor MCP config for agent-meow tools."""
...
def build_hooks_config(
    ...
    payload (on stdin) carrying that turn's token usage â€” the only surface where
...
def approve_mcp_server_for_workspace(workspace: Path) -> None:
    """Approve the workspace-scoped agent-meow MCP server in Cursor's state.
...
def enable_mcp_for_workspace(workspace: Path) -> None:
    """Ensure Cursor does not keep the agent-meow MCP disabled for this workspace."""
    disabled_path = (
...
def allow_mcp_tools_in_cli_config() -> None:
    """Allow agent-meow MCP tool calls in Cursor's CLI permission config."""
...
def _clear_composer(socket_path: str, tmux_target: str) -> None:
    ...
    to clear a line â€” only ``Backspace`` deletes. So jump to the end (``End``)
    ...
    for _ in range(_COMPOSER_CLEAR_MAX_ROUNDS):
        ...
        # Once a burst no longer changes the pane, the composer is empty â€”
...
def inject_model_command(
    ...
    *,
    ...
    persisted ``model_override`` â€” it has to be typed into the TUI. Typing
    ...
    selects exactly that model (e.g. ``gpt-5.2`` â†’ "GPT-5.2 Medium"). This is
    the cursor analog of claude-native's ``inject_slash_command('/model â€¦')``.
    ...
    :raises RuntimeError: If the tmux target is never advertised, the TUI has
        ...
        (an unavailable/typo'd id) â€” in which case the picker is dismissed and
    ...
    # "Models matching" header (â‰¥1 match) or "No matches", then settle so the
...
def _wait_for_pane_settle(socket_path: str, tmux_target: str, *, timeout_s: float) -> None:
    ...
    rather than raising â€” the clear that follows is a no-op on an empty composer.
    """
    deadline = time.monotonic() + timeout_s
    previous = _capture_pane(socket_path, tmux_target)
    while time.monotonic() < deadline:
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    Terminates ``cursor-agent`` and the pane outright â€” the analog of the
