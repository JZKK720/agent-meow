...
persists those with a random primary key and does NOT dedupe them â€” producers
...
def append_dead_letter(
    ...
    *proven-undelivered* records is layered on top (#1579) and reads the structured
    ...
    :param session_id: agent-meow conversation id the dropped event targeted,
        e.g. ``"conv_abc123"``.
    ...
    :param delivered_ambiguous: Whether the failure was ambiguous (request sent,
        ...
        NEVER replayed â€” a re-POST risks a duplicate (no server-side dedup).
...
# sent) â€” safe to retry. See :func:`post_may_have_been_delivered`.
...
def post_may_have_been_delivered(exc: httpx.HTTPError) -> bool:
    ...
    committed by the server despite the error â€” making a blind retry
    ...
    - ``HTTPStatusError``: the server responded with a status. The
      ...
      append). No duplicate risk â†’ safe to retry, so ``False``.
    - Connection-establishment / pool-acquire failures
      (:data:`_DELIVERY_SAFE_RETRY_ERRORS`): no bytes were sent â†’ not
      delivered â†’ safe to retry, so ``False``.
    - Any other transport error (read/write timeout, read/write error,
      ...
      response, so the server may have processed it â†’ ambiguous â†’
      ``True``.
...
async def post_external_session_status(
    ...
    *,
    ...
    :param background_task_count: Background tasks (shells) still running at the
        ...
        ``None`` omits the field (server leaves its sticky tally untouched) â€” the
    ...
    :param response_id: Optional id of the assistant turn this status edge
        ...
        SSE event so ap-web can drive the bubble's streaming lifecycle â€” that's
...
async def post_session_event_with_retry(
    *,
    ...
    (request sent, response lost) is NOT retried â€” a re-post would
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # error), proving the server is reachable â€” clear any stale
...
def _dead_letter_record_replayable(
    ...
    *,
    ...
    if http_status is None:
        ...
        # â€” proven undelivered, so safe to re-POST.
        return True
...
def _read_dead_letter_entries(path: Path) -> list[object] | None:
    ...
    :returns: Ordered entries â€” parsed ``dict`` records, or the raw ``str`` line
        for any line that failed to parse (kept verbatim so a rewrite never
        drops forensic data) â€” or ``None`` when the file does not exist.
    """
    if not path.exists():
        return None
    entries: list[object] = []
...
async def replay_dead_letters(
    ...
    *,
    ...
    bound are retained unchanged (deferred to a later startup) and logged â€” never
    ...
    if not any_replayable:
        # Nothing recoverable â€” leave the forensic files untouched.
        return 0
    ...
    for path, entries in loaded:
        ...
        for entry in entries:
            if not _dead_letter_record_replayable(
                ...
                # Forensic record (ambiguous / permanent-4xx / malformed) â€” keep as is.
            ...
            if over_records or over_deadline:
                # Out of budget â€” defer this and every later replayable record
