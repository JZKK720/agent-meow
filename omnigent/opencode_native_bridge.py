...
# ``/v1/sessions/{id}/policies/evaluate`` â€” the SAME endpoint + ``PHASE_*``
...
// REQUEST-phase (prompt-submit) and TOOL_RESULT-phase policies enforce â€” the
...
export const OmnigentPolicyPlugin = async () => ({
  ...
  "chat.message": async (_input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      ...
      // TUI ("Unexpected server error") â€” its middleware hardcodes that. We
...
def write_relay_bridge_config(bridge_dir: Path) -> None:
    ...
    only a token â€” no ``workspace`` key, so no ``sys_os_*`` tools are served
...
def user_opencode_auth_path() -> Path:
    ...
    data home â€” the per-session override is set only on the spawned server),
...
def user_opencode_config_path() -> Path | None:
    ...
    config home â€” the per-session override is set only on the spawned server),
...
def seed_opencode_auth(bridge_dir: Path) -> Path | None:
    ...
    that isolates session state â€” but it also hides the user's
    ...
    (best-effort, ``0600``) so the user's providers â€” and any pinned model that
    needs them â€” work. Refreshed on every spawn so re-logins propagate.
...
def update_model_override(bridge_dir: Path, model_override: str | None) -> bool:
    """
    Persist a new per-session model override (agent-meowâ†’opencode model switch).

    opencode has no session-level model setting â€” the model is a per-prompt
    field â€” so the executor reads ``model_override`` from this bridge state on
