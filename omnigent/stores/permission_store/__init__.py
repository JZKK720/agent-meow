"""Permission store â€” manages session-level access grants.
...
class PermissionStore(ABC):
    ...
    for sessions routes through this store â€” there are no ownership
    ...
    def list_for_sessions(self, conversation_ids: list[str]) -> dict[str, list[SessionPermission]]:
        ...
        Issues a single ``WHERE conversation_id IN (â€¦)`` query and
    ...
    def set_admin(self, user_id: str, is_admin: bool) -> None:
        ...
        ``True``. No-op if the user row does not exist â€” callers
    ...
    def check_access(
        ...
        delegation â€” callers are responsible for those.
    ...
    def resolve_access(
        ...
        or sub-agent parent delegation â€” those are resolution policy and
