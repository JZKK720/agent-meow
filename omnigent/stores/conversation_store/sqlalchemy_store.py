...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def get_conversation(self, conversation_id: str) -> Conversation | None:
        ...
        against ``conversation_labels`` â€” separate from the
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param model_override: When set, the fork's ``model_override`` is
            this value instead of the source's copied one â€” the
        ...
        with self._session() as session:
            ...
            # agent_id=NULL (the row doesn't exist yet â€” an autoflush would
            ...
            new_conv = SqlConversation(
                ...
                # An explicit override wins over the copied value â€” this is
            ...
            if creating_clone:
                ...
                # NULL) and rolls back with the fork on failure â€” never
    ...
    def switch_conversation_agent(
        ...
        *,
        ...
        with self._session() as session:
            ...
            # constraints: (1) ``conversations.agent_id`` â†’ ``agents.id`` is
            ...
            # ``session_id``. Hence: null agent_id â†’ delete old â†’ insert new â†’
            ...
            # must be DELETEd â€” ``_upsert_labels`` only inserts/updates.
