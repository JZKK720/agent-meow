...
# ``_PERMISSION_MAX_CONSECUTIVE_FAILURES`` â€” see :func:`_post_hook_with_reattach`.
...
# respawning harness/tool subprocesses) every â‰¤30s for 24h â€” the spin-loop half
...
def _env_int(name: str, default: int) -> int:
    ...
    A non-integer value must not crash the hook subprocess at import time â€”
...
def _env_float(name: str, default: float) -> float:
    ...
    *default* rather than crashing the hook at import time.
    ...
    # False â€” both silent footguns, so treat them as malformed.
...
# (5xx, or a connection that never established) â€” i.e. the #1782 spin. It is
...
# as intended and does NOT count â€” see :func:`_post_hook_with_reattach`. So a
...
# was ever begun). These are unambiguous hard failures â€” the server is down /
# unreachable, not holding a poll. Everything else under ``httpx.HTTPError``
# that is not a 4xx/5xx status (RemoteProtocolError, ReadError, ReadTimeout, â€¦)
...
# budget â€” an unresponsive agent-meow server would otherwise hang the banner.
...
# resolves it via the approve URL â€” so the client must wait as long as
...
def _post_hook_with_reattach(
    ...
    *hard* failures, classified by kind â€” NOT by wall-clock. This is the #1782
    fix: before it the retry deadline was ``_PERMISSION_TIMEOUT_S`` (a day), so
    a persistently down server re-POSTed â€” re-driving the turn and respawning
    harness/tool subprocesses â€” every â‰¤30s for 24h.

    Failure classification:

    * **Hard failure â†’ count toward the cap.** A 5xx, or a connection that
    ...
    * **Held-poll sever â†’ reset the counter.** An established connection that
      dropped mid-poll after being held â‰¥ the floor. That is a proxy severing
      an idle long-poll â€” the re-park mechanism working as intended â€” so it
    ...
    proxy severing a genuinely-parked human poll â€” both surface as an
    ...
    topology. The blast radius is limited â€” this loop only re-POSTs over HTTP
    ...
    subprocesses a re-driven turn does spawn â€” so the worst case is one hook
    ...
    :param reauth: Optional callable that re-mints fresh auth headers when the
        server bounces the POST to its OAuth login flow (Apps 302â†’``/oidc/``)
        or returns ``401`` â€” i.e. the one-shot ``ap_auth_headers`` token lapsed.
    ...
    :returns: The successful (2xx) response, or ``None`` when rejected
        or out of budget â€” callers fail-ask as before.
    """
    body = {
        **payload,
    ...
    # window â€” matches the read budget and the server's ask_timeout.
    ...
    while True:
        ...
        try:
            ...
            if exc.response.status_code < 500:
                ...
                return None
            # 5xx: server responded but is sick â€” a hard failure (the spin).
            ...
            # tell it from a down server â€” #1782 Polly review).
        ...
        if is_hard_failure:
            ...
            # A proxy severed a genuinely-held poll â€” the re-park mechanism
        ...
        if consecutive_hard_failures >= _PERMISSION_MAX_CONSECUTIVE_FAILURES:
            print(
                ...
                "(server unreachable/sick) â€” failing ask",
                file=sys.stderr,
            )
            return None
        ...
        if time.monotonic() >= deadline:
            print(
                ...
                f"({_PERMISSION_TIMEOUT_S:.0f}s) â€” failing ask",
                file=sys.stderr,
            )
            return None
...
def _main_ask_user_question(argv: list[str]) -> int:
    ...
    so ``AskUserQuestion`` questions are silently swallowed â€” the web UI never
    ...
    if behavior not in ("allow", "deny"):
        # Unexpected shape â€” return empty output so Claude falls back to TUI.
        return 0
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    default when no policy matches) emits no output â€” "no opinion" â€” so
    ...
    is already committed â€” PostToolUse hooks are observational).
    ...
    ``/policies/evaluate`` cannot yield a usable verdict â€” the server is
    unreachable, returns non-2xx, or returns an empty / malformed body â€”
    ...
    conditions that mean the session simply is not governed â€” no active
    session, no ``ap_server_url``, an unparseable hook payload, or an
    ``mcp__omnigent__*`` tool already gated on the relay path â€” still
    ...
    :param argv: CLI argv after the ``evaluate-policy`` subcommand,
        e.g. ``["--bridge-dir", "/tmp/x"]``.
    :returns: Process exit code. Always ``0`` â€” blocking verdicts
    ...
    if eval_request is None:
        # Unrecognized hook event â€” no policy to evaluate.
        return 0
    ...
    # every render â€” including right after an in-pane ``/model`` switch). This
    ...
    # fail closed â€” blocking a cheap-model (sonnet/haiku) session over budget,
