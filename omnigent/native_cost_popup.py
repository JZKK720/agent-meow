...
verdict to the agent-meow server's elicitation-resolve endpoint â€” the **same**
...
def _list_tmux_clients(socket_path: str, tmux_target: str) -> list[str]:
    ...
    when a client is attached. Harness-agnostic â€” used for both
...
def launch_cost_popup(
    ...
    *,
    ...
    session, so a user working in a native terminal â€” claude-native or
    codex-native â€” can answer the checkpoint. Each popup resolves the same
    ...
    When **no client is attached** it skips silently â€” there is nothing to
    ...
    :param config_file: AP-routing config the popup reads for
        ``ap_server_url`` + ``ap_auth_headers`` â€” the runner mints a fresh
...
def launch_blocked_notice(
    ...
    *,
    ...
    The DENY counterpart of :func:`launch_cost_popup` â€” no approve/decline, no
    ...
    :param policy_name: Deciding policy (popup header); ``None`` â†’ generic.
...
def _read_omnigent_routing(config_file: Path) -> dict[str, object]:
    """
    Read agent-meow base URL + auth headers from a harness routing-config file.
    ...
    :raises SystemExit: If the file is missing, unreadable, not JSON, or
        is missing a usable ``ap_server_url`` â€” there is no safe default
        for "where is the agent-meow server", so this fails loud (the popup just
    ...
    try:
        ...
        raise SystemExit(
            f"cost-approval popup: cannot read agent-meow config {config_file}: {exc}"
        ) from exc
    if not isinstance(raw, dict) or not isinstance(raw.get("ap_server_url"), str):
        raise SystemExit(
            f"cost-approval popup: agent-meow config {config_file} has no 'ap_server_url' string"
        )
    return raw
...
def _prompt_verdict(message: str, *, policy_name: str | None = None) -> str | None:
    ...
    :param policy_name: Name of the deciding policy, used as the modal
        ...
        hardcoded cost-budget label â€” this popup serves *any*
    ...
    """
    header = f"Policy approval required â€” {policy_name}" if policy_name else "Approval required"
    print(f"\n  âš   {header}\n")
    for line in message.splitlines() or [message]:
        print(f"  {line}")
    print(
        "\n  [y] approve and continue    [n] decline and stop session\n"
        "  (Esc / Ctrl-C to dismiss â€” you can still answer in the web UI)\n"
    )
    while True:
        try:
...
def _show_notice(message: str, *, policy_name: str | None = None) -> None:
    ...
    There is nothing to resolve â€” just show the reason and block until the user
    ...
    :param policy_name: Deciding policy, used as the header. ``None`` â†’ generic.
    """
    header = f"Blocked by policy â€” {policy_name}" if policy_name else "Blocked by policy"
    print(f"\n  â›”  {header}\n")
    for line in message.splitlines() or [message]:
        print(f"  {line}")
...
def _post_verdict(
    *,
    ...
    """
    POST the verdict to the agent-meow elicitation-resolve endpoint.

    Hits ``POST /v1/sessions/{id}/elicitations/{eid}/resolve`` with an
    MCP :class:`~?omnigent.server.schemas.ElicitationResult` body â€” the
    identical call the web ``ApprovalCard`` makes â€” so the cost
    ...
    :param ap_server_url: agent-meow server base URL, e.g.
        ``"http://127.0.0.1:8787"``.
    ...
    :param action: MCP action verb â€” ``"accept"`` or ``"decline"``.
    ...
    try:
        ...
        # 3xx) status, so a returned response is already a success â€” there is
    ...
    def _watch() -> None:
        while True:
            ...
            if not still_pending:
                # Resolved on another surface (web card) or timed out â€” close
...
def main(argv: list[str] | None = None) -> int:
    ...
    Reads agent-meow routing up front (so a missing/invalid config fails loud
    ...
    :returns: Process exit code â€” ``0`` on a delivered verdict or a
    ...
    parser.add_argument(
        ...
        "dismissal â€” no approve/decline, no server resolution.",
    )
    ...
    if args.notice:
        ...
        # reason and wait for the user to dismiss â€” no resolution to POST.
    ...
    if action is None:
        ...
        print("\n  Dismissed â€” answer in the web UI or it will time out.\n")
        return 0
    _post_verdict(
        ap_server_url=ap_server_url,
        auth_headers=auth_headers,
    ...
    print(
        "\n  Approved â€” continuing.\n"
        if action == "accept"
        else "\n  Declined â€” stopping the session.\n"
    )
    return 0
