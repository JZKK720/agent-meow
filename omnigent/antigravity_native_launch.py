...
pure functions and a frozen dataclass â€” no live agy calls are made here.

Key design points:

* **Auth inheritance** â€” agy shares ``~/.gemini`` with the user's interactive
...
* **Per-session identity is discovered, not assigned** â€” agy mints its own UUID
...
* **Workspace = the agy process cwd** â€” agy runs its tools in its own working
...
* **No usable env knobs** â€” agy also ignores ``ANTIGRAVITY_SIDECAR_WEB_PORT``
...
* **Auth is OAuth-only for the agy CLI** â€” empirically the CLI ignores
...
# exposes â€” its ``hooks.json`` PreToolUse hook does not fire on tool execution in
# 1.0.8 (verified; see docs/claude/antigravity-native-governance-design.md Â§2.3),
# so there is no per-tool / web-routed gate. agent-meow therefore maps its
# ``permission_mode`` onto this single flag.
_SKIP_PERMISSIONS_FLAG = "--dangerously-skip-permissions"
# agent-meow permission mode that maps to agy's all-or-nothing bypass.
...
def resolve_native_antigravity_launch(
    *,
    ...
    user's Google login from ``~/.gemini`` automatically â€” no seeding is
...
def should_skip_permissions(
    ...
    * ``permission_mode == "bypassPermissions"`` â€” the user/worker explicitly
    ...
    * the launch is **headless** â€” no interactive client will attach to answer
    ...
    default ``request-review`` prompts the user per tool â€” answered by the
    attached TTY (CLI) or, on the host-spawned web path, surfaced as a real-time
    agent-meow elicitation by the RPC read driver's interaction bridge (see
    :mod:`~?omnigent.antigravity_native_reader` /
    :mod:`~?omnigent.antigravity_native_interactions`). agy exposes no firing
    pre-tool hook, so agent-meow cannot pre-empt a tool before it runs; the
    ...
    .. warning:: The ``headless`` argument is currently derived from the
       ...
       attached" â€” otherwise a web-attended session would auto-bypass here. See
       the matching note on ``_launch_is_headless``.

    :param permission_mode: The session's effective agent-meow permission mode,
...
def build_agy_launch(
    ...
    * **Fresh session** (``resume=False``) â€” nothing is set for identity. agy
    ...
    * **Resume session** (``resume=True``) â€” ``--conversation <id>`` is
    ...
    **Permission mode â†’ agy flag.** ``--dangerously-skip-permissions`` is
    appended when :func:`should_skip_permissions` says so â€” i.e. when
    ...
    cannot be routed to the web UI or made per-tool (the genuine agent-meow policy
    gate for this harness is post-hoc/audit-only). The flag is not duplicated
    when *extra_args* already carries it.

    In both modes auth is inherited from ``~/.gemini`` â€” nothing is seeded â€”
