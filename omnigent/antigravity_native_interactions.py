...
agent-meow elicitation, waits for the human's verdict, and delivers it back to agy
via ``HandleCascadeUserInteraction`` â€” handling agy's **WAITING-interaction
timeout gotcha** end-to-end.

The gotcha (design ``docs/antigravity-native-rpc-core-design.md`` Â§2.1, memory
``agy-rpc-interaction-bridge``): a WAITING interaction **times out** server-side
(â†’ ``CORTEX_STEP_STATUS_ERROR``), after which agy **auto-retries with a fresh
WAITING step at a HIGHER ``stepIndex``**. agent-meow elicitations wait on a human
...
* The bridge **re-reads the freshest WAITING step at delivery time** and targets
  THAT step's ids â€” never the ones captured at detection.
...
* ``get_steps`` â€” re-reads the freshest trajectory steps (production wraps
  :func:`~?omnigent.antigravity_native_rpc.get_trajectory_steps`).
* ``request_elicitation`` â€” publishes the elicitation under a deterministic id
  and long-poll-awaits the user's result; returns ``None`` on timeout/cancel
  (production, wired in Task 11, POSTs the
  ``antigravity-elicitation-request`` hook and long-polls â€” mirrors codex's
  ``_handle_codex_elicitation_request``).
* ``deliver`` â€” defaults to :func:`_deliver_via_rpc`, which offloads the blocking
...
* ``inject_tui`` â€” defaults to :func:`_inject_via_tui`, which types the verdict's
  ...
  (live-verified â€” ``docs/claude/antigravity-rpc-spike-notes.md`` Â§"attended
...
timeout-retry step (new ``step_index``) yields a NEW id â†’ a fresh elicitation is
...
# longer accepting input â€” either a missing ``trajectoryId`` or, the case this
...
# Default bound on the detectâ†’elicitâ†’deliver loop. Each iteration surfaces one
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    worker thread â€” the same pattern :func:`_deliver_via_rpc` uses for the RPC â€”
    ...
    the backend step â€” the keystroke is the belt-and-braces TUI dismissal).
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    """
    Build the agent-meow elicitation id for one agy WAITING interaction.
    ...
    DIFFERENT id â€” surfacing a fresh elicitation for the retry rather than
...
async def _deliver_via_rpc(
    ...
    *,
    ...
    :func:`asyncio.to_thread` â€” the same pattern the read driver uses for
    ``get_trajectory_steps`` â€” and preserves its :class:`AntigravityRpcError`
...
def _freshest_waiting(
    ...
    *,
    ...
    (agy left timed-out ones behind and issued retries), the freshest â€” highest
    ``step_index`` â€” is the live one to deliver against. Only steps of the
    ...
    :param after_index: When set, only steps with ``step_index > after_index``
        are considered â€” used after a timeout to require a strictly NEWER step
...
async def bridge_interaction(
    ...
    *,
    ...
    Runs the tight detectâ†’elicitâ†’deliver loop that absorbs agy's WAITING-timeout
    gotcha (see the module docstring / design Â§2.1). Each iteration:
    ...
    2. If the verdict is ``None`` (the human timed out or cancelled), **returns
       without delivering** â€” agy's own WAITING timeout handles the dangling step;
       forcing a deny here is out of scope.
    3. **Re-reads the freshest WAITING step** and delivers against THAT step's
       ids (never the captured ones â€” they may be stale because agy timed out and
    ...
    :param inject_tui: Types the verdict's key sequence into the agy TUI pane to
        ...
        in parallel â€” so the keystroke is required to actually advance the
        terminal (#1200). A failure here is logged, not raised: the backend step
        is already answered, so the turn proceeds regardless.
    :param max_retries: Upper bound on detectâ†’deliver iterations.
    :returns: None. Never raises on the expected timeout/cancel/RPC-error paths â€”
    ...
    for attempt in range(max_retries):
        ...
        if result is None:
            ...
            _logger.info(
                ...
                "hook rejection â€” see reader warnings); not delivering "
        ...
        try:
            ...
            # (live-verified â€” see the module note / spike doc). Type the verdict's
            ...
            # raised â€” never undo a delivered verdict over a flaky pane.
            keys = to_tui_selection_keys(current["kind"], result, current["spec"])
            if keys:
                try:
                    await inject_tui(keys)
                except Exception as tui_exc:
                    # Best-effort TUI dismissal â€” the backend step is already
            ...
            if _INPUT_NOT_REGISTERED not in str(exc).lower():
                # A genuine shape/transport error â€” not the timed-out-step race.
