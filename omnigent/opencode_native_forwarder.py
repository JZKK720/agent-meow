...
``/v1/sessions/{id}/events`` â€” the same envelope the codex forwarder uses
(``external_conversation_item`` / ``external_session_status`` /
``external_output_text_delta``).

Design references: the SSE-event â†’ agent-meow-event translation table in
``designs/opencode-harness-and-unified-interface.md`` Â§A.9. The forwarder
...
# ``response.compaction.in_progress`` / ``â€¦completed`` SSE the web UI renders.
...
# Transient chain-of-thought delta â€” the reasoning analogue of the text delta
...
class OpenCodeNativeForwarder:
    ...
    :param policy_evaluator: Optional async policy resolver. Production
        ...
        ``omnigent.runner.app._build_opencode_policy_evaluator``) â€” the SAME
    ...
    :param default_decision: Decision used when no evaluator is provided or
        ...
        ``reject`` so an unconfigured or unreachable policy FAILS CLOSED â€” a
    ...
    def _response_id(self, message_id: str | None) -> str:
        ...
        is the natural per-turn key â€” fall back to the session id only when the
    ...
    async def _on_message_updated(self, event: OpenCodeEvent) -> None:
        """Handle ``message.updated`` â€” learn role; begin a turn for assistant.
    ...
    async def _on_part_updated(self, event: OpenCodeEvent) -> None:
        """Handle ``message.part.updated`` â€” text / tool / step-boundary parts."""
        ...
        if part_type == "text":
            ...
            # harnesses â€” mirrors codex-native). So the USER message must be
    ...
    async def _handle_tool_part(self, part: Mapping[str, Any]) -> None:
        ...
        ``pending`` â†’ ``running`` â†’ ``completed`` / ``error`` with ``input``
    ...
    async def _handle_reasoning_part(self, part: Mapping[str, Any]) -> None:
        ...
        (the codex-native reasoning contract). Reasoning is transient â€” not
        persisted as a chat item â€” so nothing is flushed on step end.
        """
        part_id = part.get("id")
        text = part.get("text")
        if not isinstance(part_id, str) or not isinstance(text, str):
    ...
    async def _handle_file_part(self, part: Mapping[str, Any]) -> None:
        """Mirror an opencode ``file`` part â€” images as image blocks, else a note.
        ...
        the data URI / URL â€” the same shape the inbound transport reads);
        ...
        if isinstance(mime, str) and mime.startswith("image/") and isinstance(url, str) and url:
            ...
            return
        # Non-image attachment â†’ a short text reference (text-flattened).
    ...
    async def _on_session_status(self, event: OpenCodeEvent) -> None:
        """Handle ``session.status`` â€” surface the running edge."""
    ...
    async def _on_session_idle(self, event: OpenCodeEvent) -> None:
        """Handle ``session.idle`` â€” finalize text, post usage, end the turn."""
        del event
        await self._flush_pending_text()
        await self._post_session_usage()
    ...
    def _record_assistant_usage(self, message_id: str, info: Mapping[str, Any]) -> None:
        ...
        streams) â€” :meth:`_post_session_usage` sums them into the cumulative.
        """
    ...
    async def _on_session_error(self, event: OpenCodeEvent) -> None:
        """Handle ``session.error`` â€” surface a failed (or re-auth) status edge.
    ...
    async def _on_compaction_started(self, event: OpenCodeEvent) -> None:
        ...
        "Compacting conversationâ€¦" marker while opencode summarizes the session
        server-side. The agent-meow server maps ``external_compaction_status``
        ``in_progress`` â†’ the ``response.compaction.in_progress`` SSE the web
    ...
    async def _on_compaction_ended(self, event: OpenCodeEvent) -> None:
        """Handle compaction completion â€” opencode finished compacting.
    ...
    async def _on_model_switched(self, event: OpenCodeEvent) -> None:
        """Handle ``session.next.model.switched`` â€” mirror a TUI /model switch.

        When the user switches model in the opencode TUI, reflect it to agent-meow
        (``external_model_change`` â†’ the session's ``model_override``) so the
    ...
    async def _on_permission_asked(self, event: OpenCodeEvent) -> None:
        """Handle ``permission.v2.asked`` â€” evaluate policy and reply."""
        ...
        if reply is None:
            ...
            # resolution was obtained â€” which must DENY, never auto-approve.
            reply = "reject"
        try:
    ...
    async def _resolve_permission(self, *, request_dict: Any) -> PolicyDecision:
        ...
        if self._policy_evaluator is None:
            # No policy gate wired â†’ fail closed (default ``reject``). A
...
# Event type â†’ bound handler-name lookup. Built once; ``handle_event``
...
_HANDLERS: dict[str, Callable[[OpenCodeNativeForwarder, OpenCodeEvent], Awaitable[None]]] = {
    ...
    # Context compaction lifecycle â†’ the web UI's compaction marker. Verified
