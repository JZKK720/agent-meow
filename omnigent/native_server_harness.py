...
deliberately thin and transport-agnostic â€” the same orchestration drives
...
- ``run_turn`` resolves the native session id from bridge state (briefly
  ...
  transport, and yields ``TurnComplete`` â€” streaming is the forwarder's
...
class NativeServerHarness(Executor):
    ...
    def supports_streaming(self) -> bool:
        """:returns: ``False`` â€” the runner-side forwarder emits output."""
        return False

    def handles_tools_internally(self) -> bool:
        """:returns: ``True`` â€” the native server runs its own tools."""
        return True
    ...
    async def run_turn(
        ...
        :param messages: Conversation history; the latest user message is
            delivered.
        :param tools: agent-meow tool schemas (ignored â€” native owns tools).
        :param system_prompt: Agent system prompt (ignored â€” set at
            session creation).
...
def _latest_user_prompt(
    ...
    :param build_prompt: Content â†’ prompt builder.
