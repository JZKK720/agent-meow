"""Filesystem bridge + tmux injection for the ironclaw-native terminal harness.

The runner launches the ``ironclaw`` TUI in a private tmux pane and records that
...
:func:`inject_user_message` (tmux bracketed paste + a single Enter) — the IronClaw
analog of the goose-native tmux bridge. This is what wires the web-UI chat box to
the running IronClaw TUI (and, since the web UI embeds that pane, the message shows
in both surfaces).

When Omnigent policies are configured the runner writes a per-session
``IRONCLAW_HOME`` with a ``pre_tool_call`` hook (via :func:`write_policy_hook_config`)
that evaluates tool calls against the Omnigent policy engine — the same hook used
by the headless ``ironclaw`` harness (:mod:`omnigent.inner.ironclaw_executor`). The
``IRONCLAW_HOME`` env var in :func:`build_ironclaw_native_spawn_env` points the TUI at
this per-session dir so the hook fires alongside IronClaw's own approval prompt.

The native TUI's own tool-approval prompt is surfaced to the web UI as a synced
approval card by the runner-side mirror
(:mod:`omnigent.ironclaw_native_permissions`), which reads the pane via
:func:`capture_ironclaw_pane` and answers it via :func:`send_ironclaw_pane_keys`.
"""
...
BRIDGE_DIR_ENV_VAR = "HARNESS_IRONCLAW_NATIVE_BRIDGE_DIR"

_BRIDGE_ROOT = (
    Path(os.environ.get("TMPDIR", "/tmp")) / f"omnigent-{stable_user_id()}" / "ironclaw-native"
)
...
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "omnigent-ironclaw-paste"
...
# IronClaw's prompt_toolkit TUI emits no fixed ready-prompt sentinel; readiness is
...
# On a NEW session, IronClaw blocks its prompt_toolkit input loop while it cold-
...
# confirm delivery against IronClaw's OWN store instead: an accepted turn writes a
# new ``messages`` row (IronClaw flushes a row per agentic step), so a new row
...
# How long to wait for IronClaw to persist a new ``messages`` row confirming it
...
def mint_ironclaw_session_id() -> str:
    """Generate a fresh IronClaw session id (UUID4 string)."""
...
def clone_ironclaw_session(
    ...
    *,
    ...
    """Clone an IronClaw session from *source_db* into *target_db* under a new id.

    Copies the entire source database (preserving whatever schema IronClaw uses)
    then remaps the session and message rows to the new id. This avoids
    hard-coding the schema — if IronClaw adds columns (e.g. ``parent_session_id``)
    the clone picks them up automatically.

    :param source_db: Path to the source IronClaw ``state.db``.
    :param target_db: Path to the target IronClaw ``state.db`` (created/overwritten).
    :param source_session_id: IronClaw session id in the source database.
    ...
    # IronClaw starts fresh rather than crashing on a broken state.db.
    try:
        ...
        _logger.warning(
            "Source ironclaw state.db at %s is unreadable; skipping clone",
            source_db,
        )
        return 0
    if row is None:
        _logger.warning(
            "Source ironclaw session %s not found in %s; skipping clone",
            source_session_id,
            source_db,
        )
        return 0
    ...
    # Use SQLite's backup API instead of shutil.copy2 — IronClaw uses WAL mode
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/omnigent-<uid>/ironclaw-native/<hash>``."""
    digest = hashlib.sha256(session_id.encode("utf-8")).hexdigest()[:32]
    return _BRIDGE_ROOT / digest


def bridge_root() -> Path:
    """Return the configured ironclaw-native bridge root."""
    return _BRIDGE_ROOT
...
def build_ironclaw_native_spawn_env(session_id: str) -> dict[str, str]:
    """Build the ``HARNESS_IRONCLAW_NATIVE_*`` env the harness executor reads.

    Publishes the per-session bridge dir so the
    :class:`~omnigent.inner.ironclaw_native_executor.IronclawNativeExecutor` can find
    the tmux target advertised by the runner. If a per-session ``IRONCLAW_HOME``
    was written by :func:`write_policy_hook_config`, the env includes
    ``IRONCLAW_HOME`` so the TUI picks up the policy hook.
    ...
    ironclaw_home = read_ironclaw_home(bridge_dir)
    if ironclaw_home is not None:
        env["IRONCLAW_HOME"] = str(ironclaw_home)
    return env


# Keys from the user's ``~/.ironclaw/config.yaml`` that the per-session
# IRONCLAW_HOME needs in order to authenticate with the inference provider.
...
_IRONCLAW_HOME_SUBDIR = "ironclaw_home"


def _load_user_ironclaw_config() -> dict:
    """Load inference-relevant keys from the user's ``~/.ironclaw/config.yaml``."""
    user_config = Path.home() / ".ironclaw" / "config.yaml"
    ...
    try:
        ...
        _logger.debug(
            "Failed to load user IronClaw config at %s", user_config, exc_info=True
        )
        return {}
...
def write_policy_hook_config(
    ...
    server_url: str,
    session_id: str,
    ironclaw_home: Path | None = None,
) -> Path:
    """Write per-session ``IRONCLAW_HOME`` with Omnigent policy hook and MCP server.

    Creates a ``config.yaml`` registering:

    1. A ``pre_tool_call`` shell hook that evaluates tool calls against the
       Omnigent policy engine (same hook the headless ``ironclaw`` harness uses).
    2. An ``mcp_servers.omnigent`` entry that launches the Omnigent MCP stdio
       ...
       ``web_fetch``, etc.) to the IronClaw model.

    Also copies the user's auth/env files so IronClaw can still authenticate with
    its inference provider. Shared by the ``ironclaw-native`` TUI path and the
    headless ``ironclaw`` executor. The credential-bearing IRONCLAW_HOME and the
    ...
    the runner and ``serve-mcp`` agree on, while ``ironclaw_home`` may be a
    private tempdir so the copied ``.env`` / ``auth.json`` never land on the
    predictable path. Defaults to ``bridge_dir/ironclaw_home`` when unset.

    :param bridge_dir: Per-session bridge dir (runner<->serve-mcp rendezvous).
    :param server_url: Omnigent server base URL.
    :param session_id: Omnigent session / conversation ID.
    :param ironclaw_home: Where to write the credential-bearing home. ``None``
        places it under ``bridge_dir``.
    :returns: The IRONCLAW_HOME path.
    """
    if ironclaw_home is None:
        ironclaw_home = bridge_dir / _IRONCLAW_HOME_SUBDIR
    _ensure_dir(bridge_dir)
    # Owner-only: the home holds the copied .env / auth.json credentials and the
    # token-bearing hook wrapper.
    _ensure_dir(ironclaw_home)

    hook_script_path = str(
        Path(__file__).resolve().parent / "inner" / "ironclaw_policy_hook.py"
    )
    ...
    wrapper = ironclaw_home / "omnigent-policy-hook.sh"
    ...
    user_cfg = _load_user_ironclaw_config()
    ...
    # Register the Omnigent MCP stdio server so IronClaw can call
    ...
    config_path = ironclaw_home / "config.yaml"
    config_path.write_text(json.dumps(config, indent=2) + "\n")

    # Copy user's .env (API keys).
    user_env = Path.home() / ".ironclaw" / ".env"
    if user_env.is_file():
        shutil.copy2(user_env, ironclaw_home / ".env")

    # Copy user's auth.json (provider credentials).
    user_auth = Path.home() / ".ironclaw" / "auth.json"
    if user_auth.is_file():
        shutil.copy2(user_auth, ironclaw_home / "auth.json")

    # Pre-populate the allowlist so IronClaw doesn't prompt for hook consent.
    allowlist_path = ironclaw_home / "shell-hooks-allowlist.json"
    allowlist_data = {
        "approvals": [
...
def inject_compress_command(bridge_dir: Path, *, timeout_s: float = 5.0) -> None:
    """Type ``/compress`` into the IronClaw TUI pane.

    IronClaw's ``/compress`` slash command compacts the conversation context,
...
def read_ironclaw_home(bridge_dir: Path) -> Path | None:
    """Return the per-session IRONCLAW_HOME if it was previously written.

    :param bridge_dir: Per-session bridge dir.
    :returns: The IRONCLAW_HOME path, or ``None`` if it doesn't exist.
    """
    ironclaw_home = bridge_dir / _IRONCLAW_HOME_SUBDIR
    if ironclaw_home.is_dir():
        return ironclaw_home
    return None
...
def write_tmux_target(
    ...
    *,
    ...
    """Advertise the tmux socket + target for the running IronClaw terminal."""
    _ensure_dir(bridge_dir)
...
def _wait_for_tmux_info(bridge_dir: Path, *, timeout_s: float) -> dict[str, str]:
    ...
    raise RuntimeError(
        f"ironclaw-native tmux target was not advertised within {timeout_s:.0f}s"
    )
...
def _settle_pane(socket_path: str, tmux_target: str, *, timeout_s: float) -> None:
    """Best-effort wait until the IronClaw input box is ready to receive a paste.

    IronClaw emits no fixed idle marker, so readiness is detected by the pane
...
def _state_db_path(bridge_dir: Path) -> Path | None:
    """Resolve the IronClaw ``state.db`` this session writes to, for delivery checks.

    Prefers the per-session ``IRONCLAW_HOME`` under *bridge_dir* (created by
    :func:`write_policy_hook_config` and passed to the TUI as ``IRONCLAW_HOME``),
    then ``$IRONCLAW_HOME``, then the default ``~/.ironclaw``. Returns the EXPECTED
    path even if the file does not exist yet — on a fresh session IronClaw creates
    ...
    :returns: The state DB path, or ``None`` when no plausible home is known (no
        per-session home, no ``$IRONCLAW_HOME``, no ``~/.ironclaw`` dir) — the caller
        then skips delivery confirmation and falls back to best-effort delivery.
    """
    home = bridge_dir / _IRONCLAW_HOME_SUBDIR
    if home.is_dir():
        return home / "state.db"
    env_home = os.environ.get("IRONCLAW_HOME", "").strip()
    if env_home:
        return Path(env_home) / "state.db"
    default_home = Path.home() / ".ironclaw"
...
def _max_message_id(db_path: Path) -> int:
    """Return ``MAX(messages.id)`` in the IronClaw ``state.db``, or ``0`` on error.
...
def _await_new_message(db_path: Path, baseline_id: int, timeout_s: float) -> bool:
    ...
    A new ``messages`` row is IronClaw's own record that it accepted the injected
...
def inject_user_message(
    ...
    *,
    ...
    """Deliver a web-UI user message into the IronClaw TUI via a tmux bracketed paste.
    ...
    On a NEW session IronClaw blocks input while cold-starting its MCP server, so
    ...
    later turn. To prevent that, when IronClaw's ``state.db`` is readable we confirm
    ...
    :raises RuntimeError: If the tmux target is never advertised, a tmux command
        fails, or IronClaw never confirms acceptance of the message.
    """
    if not content:
        raise RuntimeError("ironclaw-native injection requires non-empty content")
    info = _wait_for_tmux_info(bridge_dir, timeout_s=timeout_s)
    socket_path = info["socket_path"]
    ...
    if not _session_alive(socket_path, tmux_target):
        raise RuntimeError(
            "ironclaw terminal is no longer running (the TUI exited); restart the session"
        )
    needle = _submit_needle(content)
    # Snapshot the store high-water mark BEFORE delivery so a new row afterwards
    # is unambiguous proof IronClaw accepted this turn.
    ...
    if db_path is None:
        ...
        # preserving prior behavior for setups without a per-session IRONCLAW_HOME.
        return
    ...
    if _await_new_message(db_path, baseline_id or 0, _DELIVERY_CONFIRM_TIMEOUT_S):
        return
    raise RuntimeError(
        "ironclaw did not accept the message (the TUI may still be initializing); "
...
def inject_interrupt(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    """Cancel the in-flight IronClaw turn by sending ``C-c`` to the pane.

    IronClaw uses Ctrl+C to interrupt a running turn (double-press within 2s
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    """Hard-stop the IronClaw session by killing its tmux session.

    Terminates ``ironclaw`` and the pane outright — the analog of the user manually
...
def capture_ironclaw_pane(bridge_dir: Path) -> str | None:
    """Return the visible IronClaw pane text, or ``None`` if the TUI is not running.

    Used by the runner-side approval mirror
    (:mod:`omnigent.ironclaw_native_permissions`) to detect IronClaw's in-terminal
...
def send_ironclaw_pane_keys(bridge_dir: Path, *keys: str) -> None:
    """Send one or more keys to the IronClaw pane (tmux ``send-keys``).

    Used by the approval mirror to answer IronClaw's native prompt from a web
    ...
    info = read_tmux_info(bridge_dir)
    if info is None:
        raise RuntimeError("ironclaw-native tmux target not advertised")
