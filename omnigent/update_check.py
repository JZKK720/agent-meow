...
* **Dev clone** (``git clone â€¦ && uv sync`` / ``pip install -e .``): we
...
* **Installed wheel** (``uv tool install agent-meow``,
  ``pip install agent-meow``, ``pipx install``, Homebrew, etc.): no clone
  ...
  Repository API â€” :func:`fetch_latest_version`) and nag *only when a
  strictly newer release exists* â€” never merely because the install is
  ...
  *config files* (``uv.toml`` / ``pip.conf``); default pypi.org. So it
  works on corporate mirrors and air-gapped networks â€” even when the
  mirror is configured in a file rather than an env var â€” and stays
...
_DIST_NAME = "agent-meow"
# "Is a newer release out?" is answered against the *configured* package
# index via the Simple Repository API (PEP 503/691) â€” the universal
...
# uses a more forgiving timeout (and one retry) than the background refresh â€” a
...
class _CacheEntry:
    ...
    :param kind: Which detection path wrote this entry â€”
...
class _InstalledWheelInfo:
    ...
    :param detected_installer: The effective installer label we use
        for picking the upgrade command â€” same as ``installer`` for
...
def maybe_show_update_notice() -> None:
    ...
    / pipx). Safe to call unconditionally â€” silently returns when the
...
def _run_dev_clone_check(repo_root: Path) -> None:
    ...
    (the user ran ``git pull``). Any failure is swallowed â€” the check
    must never break the CLI.

    :param repo_root: Absolute path to the dev clone's Git repo root,
        e.g. ``Path("/Users/me/agent-meow")``.
    """
    try:
        cached = _read_cache()
        if cached is not None and cached.kind == "clone" and not _is_stale(cached):
            ...
            # ran ``git pull``), do a cheap local recount â€” no fetch.
        ...
        if result is None:
            # git unavailable or fetch/rev-list failed â€” record a
...
def _run_installed_wheel_check() -> None:
    ...
    newer release exists â€” and only once per new release, tracked via
    ...
    added latency â€” the failure mode of the old install-age nag.
    """
    try:
        info = _read_installed_wheel_info()
    except (OSError, ValueError, KeyError):
        # Metadata parsing failed â€” fail open.
        return
    if info is None:
        return
    if info.is_editable:
        # Editable install with no .git/ reachable â€” most likely a
...
def _resolve_index_url() -> str:
    ...
    1. index env vars (:data:`_INDEX_ENV_VARS`) â€” explicit
    ...
    ``~/.config/uv/uv.toml`` or ``pip.conf`` rather than an env var â€” the
...
def _index_from_pip_config() -> str:
    ...
    legacy ``~/.pip/pip.conf``) and common system locations â€” highest
    precedence first â€” returning the first ``[global]`` / ``[install]``
...
def fetch_latest_version(
    ...
    *,
    ...
    """Fetch the latest ``agent-meow`` release from the configured index.

    Queries the Simple Repository API of the resolved index
    (:func:`_resolve_index_url`) â€” PEP 691 JSON when the index serves it,
    ...
    for _ in range(max(1, attempts)):
        try:
            ...
            break  # got a response (any status) â€” don't retry a definitive reply
        except httpx.HTTPError:
            resp = None  # transient (timeout / connection reset) â€” retry if any left
...
def _spawn_background_refresh() -> None:
    ...
    Any spawn failure is swallowed â€” the worst case is simply that the
...
def _find_repo_root() -> Path | None:
    ...
    is exactly what a dev clone of this repo looks like::
        ...
        <repo>/.git/
        <repo>/pyproject.toml
        <repo>/omnigent/update_check.py    â† this file

    We do NOT walk further up the filesystem tree. The previous
    implementation did, which caused a real bug: when ``agent-meow``
    is installed via ``uv tool install`` at
    ``~/.local/share/uv/tools/omnigent/â€¦``, the unbounded walk-up
...
def _read_cache() -> _CacheEntry | None:
    ...
    try:
        ...
        return _CacheEntry(
            ...
            # the default ``"clone"`` â€” they were all clone caches.
...
def _local_rev_list_count(repo_root: Path) -> int | None:
    ...
    No ``git fetch`` â€” uses whatever ``origin/main`` ref is already
...
def _print_pypi_notice(current: str, latest: str) -> None:
    ...
    Informational only â€” it names the new release and points the user at
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    won't have it on disk â€” in which case the import fails and we
    ...
    try:
        ...
        # Don't trust it â€” fall back to other signals.
        return None
    return ts, sha
...
def _get_distribution() -> importlib.metadata.Distribution | None:
    ...
    :returns: The ``Distribution`` for the ``agent-meow`` package,
...
def _unredact_ssh_userinfo(vcs_url: str) -> str:
    ...
    no password â€” which for an SSH remote is just the login user â€” is
    ...
    the metadata, so we restore the canonical ``git`` â€” the only SSH
    user those hosts accept â€” which makes both the command we display
    ...
    :param vcs_url: The normalized ``<vcs>+<scheme>://â€¦`` reinstall URL,
    ...
    # ``user:****`` form hides a real password â€” leave it alone.
...
def _read_installed_wheel_info() -> _InstalledWheelInfo | None:
    ...
    * ``INSTALLER`` (PEP 376) â€” the installer that wrote the package
      (``"uv"``, ``"pip"``, ``"poetry"``, ...).
    * ``direct_url.json`` (PEP 610) â€” present for direct-URL installs
    ...
    * ``uv_cache.json`` (uv-specific) â€” carries the build/cache
    ...
    when ``uv_cache.json`` is absent â€” this works for any installer.
        ...
        if isinstance(data, dict):
            ...
            if isinstance(vcs_info, dict):
                ...
                if isinstance(url, str):
                    ...
                    # ``****`` when it wrote direct_url.json â€” otherwise
    ...
    # Tier 2: uv_cache.json â€” uv-only, but more accurate than mtime
    ...
    if install_time_epoch is None:
        # Fall back to the dist-info dir's mtime â€” set by the
...
def _looks_like_pipx_install() -> bool:
    ...
    pipx delegates to pip, so ``INSTALLER`` reads ``"pip"`` â€” but pipx
...
class _UpgradeSuggestion:
    ...
    :param command: The shell command (or prose) we display in the
        nag panel, e.g. ``"uv tool upgrade agent-meow"`` or
        ``"reinstall agent-meow from your original source"``.
        Always populated.
    :param runnable: ``True`` when ``command`` is a real shell
        invocation we can execute via ``subprocess.run`` â€”
...
def _pip_invocation() -> str:
    ...
    ``pip`` is resolved against ``PATH`` â€” which, in a shell where some
    *other* environment's ``pip`` shadows the one running ``omni`` (a
    ...
    copy of agent-meow while the running one stays put. ``<sys.executable>
...
def _build_upgrade_suggestion(
    ...
    if info.vcs_url:
        # VCS install â€” we know the exact source URL.
        if installer == "uv":
        ...
        # Unknown installer with a known URL â€” fall through to a
    ...
    # Registry install â€” no VCS URL recorded.
    if installer == "uv":
...
def upgrade_command_for_installed() -> _UpgradeSuggestion | None:
    ...
    upgrade command (``uv tool upgrade agent-meow``, ``pip install -U
    agent-meow``, etc.).
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    ``agent-meow`` and cached its metadata â€” re-reading in-process returns the
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    â€” pinned spec, cooldown, stale index cache, or a git ref that can't move
    the version â€” exits 0 without changing anything).

    The version is read via ``importlib.metadata`` (no ``agent-meow`` import,
    ...
    :returns: ``(version, commit_sha)`` of the now-installed distribution.
        ...
        failure, frozen interpreter with no ``sys.executable``, or â€” for the
        commit â€” a non-VCS install).
    """
    if not sys.executable:
        return None, None
    probe = (
...
def _split_vcs_url(vcs_url: str) -> tuple[str, str | None]:
    ...
    final ``/`` â€” i.e. past the repo path â€” is treated as one.
    ...
    url = url.split("#", 1)[0]
    at = url.rfind("@")
    if at > url.rfind("/"):  # '@' past the final path segment â†’ revision suffix
...
def _remote_git_head(vcs_url: str) -> str | None:
    ...
    :param vcs_url: A normalized VCS URL, e.g.
        ...
        ``"git+https://â€¦/agent-meow.git@main"``.
