...
server (e.g. ``ConnectError: No route to host``) â€” which is logged separately
...
def record_post_failure(event_type: str, error: BaseException) -> None:
    ...
    failure such as a connectivity outage â€” transient single failures that
...
def note_post_success() -> None:
    ...
    4xx/5xx â€” receiving a status proves the server was reachable, so the prior
