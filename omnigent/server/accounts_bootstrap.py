...
1. **Password flag** â€” ``--admin-password`` /
   ``OMNIGENT_ACCOUNTS_INIT_ADMIN_PASSWORD``. Headless / CI. Bootstrap
   creates the admin from it directly (this module).
2. **Terminal prompt** â€” ``agent-meow server`` on a TTY asks for a
   username + password before serving (``cli.py``).
3. **Web Create-admin form** â€” when neither of the above applied,
...
action â€” Account menu â†’ Change password, or admin Members â†’ Reset.
...
non-loopback (remote) deploys where the server's machine â‰  the
operator's.
"""
...
# Username regex â€” duplicated from ``routes/accounts_auth.py`` to
...
def resolve_admin_username() -> str:
    ...
    1. ``OMNIGENT_ACCOUNTS_INIT_ADMIN_USERNAME`` env var â€” explicit
    ...
    2. ``getpass.getuser()`` â€” the OS user running the server. On
       a laptop, this is the operator's own login (``"dhruv.gupta"``,
       ``"alice"``, etc.) so the CLI's auto-handoff JWT and the
       web UI's admin row are the same identity from the start â€”
       no separate "local" / "admin" split.
    3. Literal ``"admin"`` â€” fallback when (a) #1 isn't set, (b)
    ...
    except Exception:  # noqa: BLE001 â€” getpass can raise on weird shells / no pwd
...
def _sanitize_admin_username(candidate: str) -> str | None:
    ...
    Returns ``None`` when the value can't be used â€” the caller
...
class BootstrapResult:
    ...
    Returned to ``create_app`` â†’ lifespan startup so it can do the
    deferred parts that need uvicorn to have bound the port first â€”
    ...
    :param needs_setup: ``True`` when no admin exists yet and none was
        created this boot (no password was supplied) â€” so the first
...
def _mint_loopback_cli_token(
    ...
    *,
    ...
    boot's port) won't match a later spawn â€” and on a returning boot
    ...
    try:
        ...
        return True
    except Exception as exc:  # noqa: BLE001 â€” best-effort, must not block boot
        logger.warning(
            "accounts: failed to write loopback CLI token (%s) â€” "
...
def bootstrap_admin(
    ...
    *,
    ...
    - ``admin`` already exists â†’ no-op. Returns
      ``BootstrapResult(fresh_boot=False, ...)``.
    - ``admin`` missing, ``init_admin_password`` provided â†’
    ...
    - ``admin`` missing, no pre-seed â†’ mints a fresh random
    ...
    file's password is orphaned â€” recoverable by deleting the
    file and rebooting.

    :param account_store: The accounts-specific store backing user
        identity + tokens. Mutually exclusive with PermissionStore
        for accounts concerns â€” see ``omnigent/server/accounts_store.py``
    ...
    :param base_url: The server's public base URL â€” used for the
    ...
    if any(u.has_password for u in account_store.list_users()):
        ...
        # fresh token for THIS spawn's port â€” the daemon picks a new port
    ...
    # â”€â”€ No admin yet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    #      OMNIGENT_ACCOUNTS_INIT_ADMIN_PASSWORD â€” headless / CI;
    ...
    if not init_admin_password:
        ...
        if open_url is None and base_url is not None:
            print(
                f"\n  â†’ No admin yet. Open {base_url.rstrip('/')} to create the "
