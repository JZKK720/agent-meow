...
The split between OIDCConfig and AccountsConfig is intentional â€”
...
- ``ENABLED`` â€” required, must be truthy (``1`` / ``true`` /
  ``yes``). Enforced by :func:`create_auth_provider` â€” a
  ...
  flips to default.
- ``COOKIE_SECRET`` â€” required, 64+ hex chars. HMAC key for HS256
  session cookies. Generate with ``openssl rand -hex 32`` (or
  ``deploy/docker/bootstrap.sh`` mints one alongside POSTGRES_PASSWORD).
- ``SESSION_TTL_HOURS`` â€” optional, default 8. How long a
  ``/auth/login`` cookie stays valid.
- ``BASE_URL`` â€” required. The user-facing base URL of the
...
- ``INIT_ADMIN_PASSWORD`` â€” optional. When set on first boot,
...
- ``INVITE_TTL_HOURS`` â€” optional, default 72. Invite tokens
  expire after this window.
- ``MAGIC_TTL_MINUTES`` â€” optional, default 10. Magic-link
  tokens are intentionally very short-lived since they're used
  for an immediate CLIâ†’browser handoff.
"""
...
class AccountsConfig:
    ...
    def from_env() -> AccountsConfig:
        ...
        # â€” docker compose ${VAR:-} pattern forwards the var as set-to-"".
