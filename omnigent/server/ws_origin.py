"""WebSocket ``Origin`` enforcement for the agent-meow server.
...
agent-meow server and drive the agent, read session updates, or attach to a
terminal. In single-user **local mode** there is no cookie / proxy auth to
stop it â€” the server falls back to the reserved ``"local"`` user â€” so the
``Origin`` header is the only signal that distinguishes the user's own UI
from a hostile cross-origin page.

This module provides:

- :func:`origin_allowed` â€” the pure, protocol-neutral policy function
...
- :class:`WebSocketOriginMiddleware` â€” an ASGI middleware that applies the
...
- :data:`OMNIGENT_INTERNAL_WS_ORIGIN` â€” the sentinel ``Origin`` the
...
def origin_allowed(
    ...
    *,
    ...
    - In ``local_mode`` an ``Origin`` is allowed only when its hostname is
      a loopback host â€” this is the CSRF / CSWSH guard for the
...
class WebSocketOriginMiddleware:
    ...
    :func:`origin_allowed` â€” closing the connection before it
