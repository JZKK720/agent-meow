...
(``GET /v1/sessions/{id}/stream``) anywhere in a session *tree* â€” the
root conversation or any sub-agent conversation under it â€” so the web
...
* ``connect`` on stream-generator entry â€” broadcasts the full viewer
  list on the user's 0â†’1 connection edge or when their idle
  aggregate changes.
* ``disconnect`` in the generator's ``finally`` â€” on the user's last
  ...
  refreshes, and rootâ†”sub-agent navigation within one tree, so
  co-viewers' avatars don't flicker.
* ``snapshot`` â€” the current full viewer list as a
...
stamped as that stream's own conversation â€” clients guard incoming
...
ephemeral state â€” it dies with the process, and all streams (whose
...
class _Connection:
    ...
    :param conversation_id: The conversation whose stream this
        connection holds open â€” the tree's root itself or any
...
class _ViewerEntry:
    ...
    :param joined_at: ISO 8601 UTC timestamp of the user's 0â†’1
    ...
    :param connections: Open streams keyed by the server-minted
        ...
        ``{"a3f9â€¦": _Connection("conv_child1", False)}``. Empty
...
def snapshot(root_id: str, conversation_id: str) -> dict[str, Any]:
    ...
    :param conversation_id: The conversation whose stream this event
        is addressed to â€” the root itself or a sub-agent
    ...
    :returns: Event dict shaped like
        ``{"type": "session.presence", "conversation_id": â€¦,
        "viewers": [{"user_id": â€¦, "joined_at": â€¦, "idle": â€¦}]}``,
...
def _broadcast(root_id: str) -> None:
    ...
    One event per distinct conversation stream â€” the root stream
    ...
    has a registered viewer connection â€” each stamped with that
...
def connect(root_id: str, conversation_id: str, user_id: str, idle: bool) -> str:
    ...
    window â€” e.g. the ingress' ~5-minute stream cap, or navigating
    between agents of the same session â€” is invisible to co-viewers)
    ...
    :param conversation_id: The conversation whose stream was opened
        â€” the root itself or a sub-agent conversation under it,
        e.g. ``"conv_child1"``.
    :param user_id: The authenticated viewer identity,
        e.g. ``"alice@example.com"``. Callers must pre-filter the
        reserved single-user sentinel via ``attribution_user`` â€”
...
def disconnect(root_id: str, user_id: str, token: str) -> None:
    ...
    loop â€” every exit path (clean close, client disconnect,
    ...
    :param root_id: Root conversation of the session tree that was
        being viewed â€” the value passed to :func:`connect`.
...
def reset_for_tests() -> None:
    ...
    ``omnigent.server._elicitation_registry.reset_for_tests`` â€”
