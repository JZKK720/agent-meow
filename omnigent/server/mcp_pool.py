...
Provides :class:`ServerMcpPool` â€” an AP-server-owned MCP connection
...
from omnigent.runner.mcp_manager import (  # noqa: F401 â€” McpSchemasResult re-exported for callers
    McpSchemasResult,
    compute_spec_hash,
)
...
class ServerMcpPool:
    ...
    internally â€” it is the responsibility of callers to namespace names
            ...
            for td in server.tools:
                if not is_valid_tool_name(td.name):
                    _logger.warning(
                        ...
                        "(must match [a-zA-Z0-9_-]{1,256}) â€” skipping",
                        td.name,
                        server.config.name,
                    )
                    continue
    ...
    async def call_tool(
        ...
        if server.connection is None:
            raise RuntimeError(
                ...
                "connection â€” connect() may have been skipped"
            )
    ...
    async def shutdown_all(self) -> None:
        ...
        Called on AP-server shutdown. Best-effort â€” individual close
    ...
    async def _ensure_warm(
        ...
        async with self._lock:
            ...
            # Spec changed â†’ evict old connections and start fresh.
