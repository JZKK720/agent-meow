...
the *primary* admin mechanism for the ``oidc`` provider â€” OIDC has no
...
- **File only.** No env-var roster. The file is editable at runtime
  without a redeploy â€” in the bundled Docker stack it lives on the
  persistent ``/data`` volume next to ``admin-credentials``.
- **Additive promotion only.** A listed identity is promoted on login.
  Removing an identity from the file NEVER demotes them â€” demotion is a
...
A missing or unreadable file yields an empty roster (no error) â€” the
...
def resolve_data_dir() -> Path:
    ...
    :returns: The resolved data directory. Not created here â€” callers
...
class MtimeCachedIdentitySet:
    ...
    allowed-domains file â€” both are "one token per line, ``#``
    ...
    def _refresh(self) -> None:
        ...
        try:
            ...
            # Absent file â†’ empty set. Re-checked every call (cheap)
        ...
        try:
            ...
            # Unreadable (permissions, race with a writer) â€” fail open
...
class AdminList:
    ...
    The ``extra`` set is the canonical, declarative source â€” admins
    ...
    def is_admin(self, identity: str) -> bool:
        ...
        :param identity: User identifier â€” an email in OIDC mode, a
...
class AdminFlagStore(Protocol):
    ...
    satisfy this â€” they read and write the same ``users.is_admin``
