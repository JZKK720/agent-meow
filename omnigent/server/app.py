...
def _ensure_default_agents(
    ...
    _ensure_default_ironclaw_agent(agent_store, artifact_store, agent_cache)
...
def _build_ironclaw_native_bundle() -> bytes:
    """Build a gzipped tarball of the ironclaw-native-ui agent spec."""
    import tempfile

    from omnigent.ironclaw_native import _materialize_ironclaw_agent_spec
    from omnigent.spec import materialize_bundle
...
def _ensure_default_ironclaw_agent(
    agent_store: AgentStore,
    artifact_store: ArtifactStore,
    agent_cache: Any,
) -> None:
    """Register or refresh the ironclaw-native-ui agent."""
    _ensure_builtin_agent(
        agent_store,
        artifact_store,
        agent_cache,
        name=_IRONCLAW_NATIVE_AGENT_NAME,
        bundle_bytes=_build_ironclaw_native_bundle(),
    )
