...
[agent-meow Server API](API.md).

## Codex Goal

Codex-native sessions expose Codex app-server's persisted thread goal as
a live subresource. Goal state is not mirrored into agent-meow labels or
...
`status` is forwarded exactly as Codex reports it; agent-meow does not
