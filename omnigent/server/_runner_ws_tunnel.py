...
directly. Now the only serverâ†”runner transport is the
...
class _TunneledWSConn:
    ...
    async def __aenter__(self) -> _TunneledWSConn:
        # 4 random bytes â†’ 8 hex chars; plenty of entropy for a
    ...
    async def __aexit__(
        ...
        # attach cleanly. Swallow errors â€” the tunnel may already
    ...
    async def send(self, data: str | bytes) -> None:
        ...
        :param data: ``str`` â†’ ``ws.frame`` with utf-8 encoding (the
            JSON resize control frames). ``bytes`` â†’ ``ws.frame``
    ...
    async def recv(self) -> str | bytes:
        ...
        :raises ConnectionClosed: On runner-side close â€” including
