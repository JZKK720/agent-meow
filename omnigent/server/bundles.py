...
def _is_dotted_callable_path(path: str) -> bool:
    ...
    ``omnigent.runner.tool_dispatch._resolve_spec_callable``) â€” the
...
def _reject_uploaded_callable_tools(spec: AgentSpec) -> None:
    ...
    Recurses into sub-agents â€” each is a full :class:`AgentSpec` with its
    own ``local_tools`` â€” mirroring the handler-allowlist guard's sub-agent
...
def validate_agent_bundle(
    ...
    *,
    ...
    behalf â€” operator-authored specs resolve env at the client /
    ...
    if enforce_handler_allowlist:
        ...
        # absolute-cwd behavior â€” designs/SESSION_WORKSPACE_SELECTION.md.)
