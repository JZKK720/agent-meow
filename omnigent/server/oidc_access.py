"""OIDC admission policy â€” who may sign in via the configured IdP.
...
1. **Domain allowlist.** The email's domain is in the effective
   allowlist â€” the union of ``OMNIGENT_OIDC_ALLOWED_DOMAINS`` (env,
   ...
   authenticated email is admitted (the OSS default â€” a fresh deploy
...
3. **Individual invite.** The email was pre-authorized via an opt-in
   OIDC invite (``OMNIGENT_OIDC_ALLOW_INVITES`` â€” see
...
Conditions 2 and 3 are *additive bypasses* â€” they widen access, never
restrict it.
"""
...
class OidcAdmissionPolicy:
    ...
    :param admin_list: The file-backed admin roster â€” admins bypass the
        domain check.
    ...
    def is_admitted(self, email: str) -> bool:
        ...
        if not domains:
            return True  # no domain restriction configured â†’ admit all
