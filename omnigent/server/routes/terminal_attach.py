...
- **Server â†’ client**: every PTY read is forwarded as a *binary*
...
typed by anyone other than the owner would be acted on â€” and, for the
agent's TUI, persisted into conversation history â€” as if the owner typed
...
def create_terminal_attach_router(
    *,
    ...
    :param permission_store: Optional session permission store. When
        ...
        interactive (write) attach requires owner access â€” only the
...
async def _authorize_terminal_attach(
    ...
    *,
    ...
    attribution, so an interactive attach requires ``LEVEL_OWNER`` â€” only
