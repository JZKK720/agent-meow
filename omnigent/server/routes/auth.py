...
# provider's default invite window (72h) â€” long enough to share
...
def create_auth_router(
    ...
    :param admin_list: File-backed admin roster. Consulted on each
        callback to promote a listed email to admin (additive â€” see
    ...
    :param account_store: Invite-token persistence, required only when
        ...
        ``account_tokens`` table â€” the single-use invite token is
    ...
    # Admission policy: domain allowlist (env âˆª runtime-editable file)
    # with admin-list and (when enabled) invite bypasses. One place
    # decides who may sign in â€” see omnigent/server/oidc_access.py.
    ...
    # from redirect_uri). The __Host- prefix requires HTTPS â€” using
    ...
    async def login(request: Request) -> Response:
        ...
        # signed into the state cookie â€” prevents an open redirect on
        # the post-auth 302 in /callback.
        return_to = _sanitize_return_to(request.query_params.get("return_to"))
        # Optional CLI login ticket â€” threaded through the state
        # cookie so the callback can fulfill it.
        ticket = request.query_params.get("ticket")
        # Optional OIDC invite token â€” threaded through the signed state
    ...
    async def callback(request: Request) -> Response:
        ...
        # Admission control: domain allowlist (env âˆª file) plus the
        # admin-list / invite bypasses. An empty effective allowlist
        # means "no restriction" (admit any IdP user) â€” the OSS default.
        ...
        # and is OIDC's only path to admin â€” the IdP doesn't tell us
        ...
        if ticket_id and ticket_id in _cli_tickets:
            ...
            # Return a simple HTML page â€” the CLI is polling
        ...
        # Normal browser login â€” redirect back to the app.
    ...
    # â”€â”€ CLI login ticket endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def cli_poll(request: Request) -> Response:
        ...
        # Still pending â€” browser hasn't completed the flow yet.
        ...
        # Fulfilled â€” return the token and clean up.
        token = ticket.token
    ...
    # â”€â”€ Admin: read-only user list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def list_users(request: Request) -> Response:
        ...
        â€” same response shape, so the SPA's Members surface renders
        ...
        admin list as a direct fallback â€” matching the OIDC invite
        route above.
...
def _sanitize_return_to(raw: str | None) -> str:
    ...
    but does nothing for its *safety* â€” the value still originates with
    ...
    cross-origin). Anything else â€” absolute URLs, scheme-bearing values,
    or an empty/``None`` value â€” falls back to ``"/"``. Query strings
...
async def _resolve_github_email(
    ...
    as the sign-in identity â€” trusting it would let a user log in as an
    ...
    # Fetch email list â€” primary verified email is the identity.
    ...
    # Fail closed â€” the caller turns a ``None`` email into a 400.
    return None
...
def _claim_is_verified_true(value: object) -> bool:
    ...
    OpenID Connect Core Â§5.1 types ``email_verified`` as a boolean,
    but notes implementations may emit it as the *string* ``"true"``
    â€” so accept both. Everything else (``False``, ``"false"``,
