...
* ``GET /admin/harnesses`` â€” status of every built-in harness CLI.
* ``GET /admin/skills`` â€” every skill discoverable across bundles + host dirs.
* ``GET /admin/mcp-servers`` â€” every MCP server declared across all agents.
...
def _install_status(harness_id: str, spec: HarnessInstallSpec | None) -> str:
    ...
    SDK / in-process harnesses (``spec is None``) report ``"installed"`` â€”
...
def _harness_install_key(harness_id: str) -> str | None:
    ...
    Reuses :func:`required_cli_for_harness`'s resolution path indirectly â€”
...
def _harness_install_key_by_spec(spec: HarnessInstallSpec) -> str | None:
    ...
    ``"pi"`` / ``"cursor"`` / â€¦). Reverse-lookup by identity is fine â€” the
    ...
    """
    from omnigent.onboarding.harness_install import _all_harness_install
...
def _install_command(spec: HarnessInstallSpec | None) -> str | None:
    ...
    (cursor-agent, agy, kiro-cli, goose, hermes â€” ``package is None``) get
...
_BLOCK_SKILLS_HANDLER = "omnigent.policies.builtins.safety.block_skills"


def _build_harness_entries() -> list[dict[str, Any]]:
    """Build the harness admin catalog entries (sync â€” call via to_thread).
...
def create_admin_catalog_router(
    *,
    ...
    async def list_skills(request: Request) -> dict[str, Any]:
        ...
        #    ``AgentStore.list(limit, after, before, order)`` â€” pass limit
        ...
        for agent in builtins_page.data:
            try:
                ...
                # ``asyncio.to_thread`` forwards *args only â€” wrap in
            ...
            except Exception:  # noqa: BLE001 â€” unreadable bundle must not break the list
                continue
    ...
    async def list_mcp_servers(request: Request) -> dict[str, Any]:
        ...
        scope here â€” they're created on demand and already editable via the
