...
# assistant response outside the agent-meow task runtime and need to
...
# semantic item observed outside the agent-meow task runtime. Unlike a
...
# assistant text delta observed outside the agent-meow task runtime. The
...
# explicit ``session.interrupted`` edge observed outside the agent-meow
...
# ``.meta.json`` appears so the agent-meow server can mint a child Conversation
...
# forwarder converts each receiver thread into this event so agent-meow can mint a
...
# reload for sessions where no agent-meow task carries usage (claude-native).
...
# own id (claude-native ``elicit_claude_â€¦``, kimi-native ``elicit_kimi_â€¦``), so
...
# Maximum time agent-meow waits for its runner->AP SSE relay to observe the
...
# Validates every dict that crosses the APâ†’client SSE boundary on
...
# â”€â”€ WS /v1/sessions/updates tuning â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class _PendingPolicyAskWrites:
    ...
    ``approval`` event resolves with ``accept`` (POLICIES.md Â§7.2: a denied
...
def _native_ask_gate_lock(conversation_id: str, deciding_policy: str) -> asyncio.Lock:
    ...
    :param conversation_id: agent-meow conversation id whose ASK gate is being
...
def _signal_terminal_resolved_harness_elicitation(
    ...
    :param session_id: agent-meow conversation id whose forwarder mirrored the
...
def _accumulate_session_usage(
    ...
    **only when the turn is priced** (catalog pricing available or a
    ...
    if llm_model:
        ...
        # "priced âŸº key present" contract).
...
def _persist_native_cumulative_usage(
    ...
    Unlike the agent-meow relay path (:func:`_accumulate_session_usage`), which adds
    ...
    "priced âŸº key present" contract the relay path uses. ``policy_cost_usd``
    ...
    # label writes in ``cost_advisor`` â€” usage was the missing half.)
...
async def _persist_external_session_usage(
    ...
    # from the agent-meow relay's per-response accumulation. Persist this session's
...
async def _persist_external_model_change(
    ...
    agent-meow session: writes ``model_override`` so the value survives reload
...
async def _persist_external_reasoning_effort_change(
    ...
    Mirrors a native-terminal thinking-level change onto the agent-meow session.
...
async def _hold_native_ask_gate(
    ...
    *,
    ...
    if approved:
        # POLICIES.md Â§7.2: writes accumulated by the ASKing policy
...
async def _persist_external_assistant_message(
    ...
    """
    Persist and broadcast assistant text produced outside agent-meow tasks.

    The event is append-only conversation history. It intentionally
    bypasses the legacy persist path so mirroring a
    Claude terminal response does not create or steer an agent-meow
    agent task.
...
async def _persist_external_subagent_start(
    ...
    never POSTs to agent-meow to register them. The forwarder watches the
...
def _subagent_delivery_status(
    ...
    A claude-native session running as an agent-meow sub-agent relabels its
...
async def _persist_external_conversation_item(
    ...
    without starting or steering the placeholder agent-meow agent.
...
def _latest_assistant_text_from_store(
    ...
    helper lets agent-meow forward the durable assistant output with the
...
def _require_collaboration_mode_forward(
    ...
    app-server. Persisting the agent-meow label without a successful runner
...
def _drive_terminal_resolved_elicitation(session_id: str, persisted: ConversationItem) -> None:
    ...
    :param session_id: agent-meow conversation id the item was mirrored for,
        e.g. ``"conv_abc123"``.
...
async def _validate_session_workspace(
    *,
    ...
    if agent_cache is None:
        # Should never happen in production â€” the route factory
    ...
    # Authorize host ownership FIRST â€” before loading the agent spec or
    ...
    # Read the agent's os_env.cwd â€” None when the spec has no
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    if runner_id is not None and tunnel_registry is not None:
        ...
        # timeout still settles successfully â€” the host is bound, and
...
async def _maybe_relaunch_managed_sandbox(
    *,
    ...
    if await asyncio.to_thread(host_store.is_online, conv.host_id):
        ...
        # heartbeat) â€” the missing tunnel is likely a transient blip
...
async def _ensure_runner_session_initialized(
    ...
    also posts ``/v1/sessions`` on the same connection â€” whichever
    ...
    error = ErrorData(
        ...
        message=(
            ...
            # third-party host omits it.
                ...
                "the agent's harness is not configured on the selected host â€” run `agent-meow setup`"
            )
        ),
    )
    turn_id = generate_task_id()
...
async def _forward_native_terminal_message(
    ...
    """
    Forward one agent-meow web-chat message to the native terminal harness.
...
def _extract_claude_native_runner_failure(resp: httpx.Response) -> str | None:
    ...
    ``response.failed``. Claude-native agent-meow forwarding must treat that
...
async def _forward_session_change_to_runner(
    ...
    * Explicit ``compact`` â€” the caller inspects the returned status
      ...
      200) or the agent-meow server must run its own in-process compaction
    ...
    skipped â€” the persisted value on the agent-meow side is the authoritative
    ...
    in the agent-meow log â€” otherwise the POST succeeds at the httpx layer
...
async def _forward_event_to_runner(
    ...
    # â”€â”€ Server-side intelligent routing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _dispatch_session_event_to_runner(
    ...
    * **transcript-forwarded native + ``type == "message"``**: web-chat user
      ...
      server. The agent-meow would otherwise persist an AP-side copy AND
    ...
    * **All other cases**: persist the item AP-side (invariant I1:
    ...
    if body.type == "message" and _is_native_terminal_session(conv):
        ...
        # â”€â”€ Server-side routing for native terminal sessions â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        async with runner_client.stream(
            ...
            async for chunk in resp.aiter_text():
                ...
                while "\n\n" in buffer:
                    ...
                    # (the agent-meow subscriber generates its own heartbeats).
...
async def _ensure_runner_relay_ready(
    ...
    The runner stream has no replay buffer. For item events, agent-meow must
    subscribe to runner output before it forwards the input event; a
    fast harness can otherwise complete before agent-meow is listening, leaving
...
def _same_provider_family(a: Agent, b: Agent) -> bool:
    ...
    native transcript from agent-meow items instead).
...
def _agent_harness_id(agent: Agent) -> str | None:
    ...
    codex-native fork). ``None`` when the bundle can't be loaded â€” the
    ...
    except Exception:  # noqa: BLE001 â€” unloadable bundle â†’ skip the family guard
        return None
    from omnigent.harness_aliases import canonicalize_harness
...
# agent-meow items and relaunch the CLI with --resume, so prior turns reappear as
...
def _agent_carries_native_fork_history(agent: Agent) -> bool:
    ...
    session file that the runner rebuilds from the copied agent-meow items on
...
async def _evaluate_input_policy(
    ...
    *,
    ...
    # deciding policy's writes only on accept (POLICIES.md Â§7.2). Accept ->
...
async def _wake_parent_for_blocked_child(
    ...
    *outcome* is reported back so the notifier can release its per-block
    ...
    :param notice: The ``[System: â€¦]`` text to inject into the parent.
...
def configure_subagent_block_notifier(
    ...
    async def _wake_dispatch(parent_id: str, child: Conversation, notice: str) -> bool:
        ...
        :param notice: Pre-formatted ``[System: â€¦]`` text.
...
def _validate_terminal_launch_args(value: list[str] | None) -> list[str] | None:
    ...
    a caller to smuggle internal launch wiring (bridge dir, agent-meow URL,
...
def _require_cost_control_label_authority(
    *,
    ...
    :param tunnel_token: Value of the ``X-agent-meow-Runner-Tunnel-Token``
...
async def _create_session_from_existing_agent(
    ...
    # session â€” otherwise they can execute another user's private
...
# â”€â”€ MCP proxy helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The agent-meow server's role here is routing only â€”
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    if is_retry:
        # â”€â”€ Retry path: user has responded to the elicitation â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Fail-closed: re-evaluate TOOL_CALL policy on retry â”€â”€â”€â”€â”€â”€
        ...
        if retry_result.action == PolicyAction.ASK:
            ...
            # user approved (POLICIES.md Â§7.2: only on accept).
        ...
        # â”€â”€ First call: evaluate TOOL_CALL policy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Server-side sys_advise_models intercept â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Execute on the runner via WS tunnel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # tunnel the runner already opened to the agent-meow server at startup.
    runner_client = await _get_runner_client(session_id, runner_router)
    if runner_client is None:
        from omnigent.runtime import get_runner_client
    ...
    # â”€â”€ MRTR: external MCP server needs user input â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ TOOL_RESULT policy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def create_sessions_router(
    ...
    # â”€â”€ POST /sessions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ GET /sessions/projects â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #
    ...
    # â”€â”€ PUT /sessions/{session_id}/read-state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #
    ...
    # â”€â”€ GET /sessions/{session_id} â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ GET /sessions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ WS /sessions/updates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Codex-native goal controls â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ PATCH /sessions/{session_id} â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def update_session(
        ...
        if body.runner_id is not None:
            ...
            if body.runner_id == "":
                ...
                # (to runner)" step from Â§7 Flow: session creation.
        ...
        # Only on archiveâ†’true; unarchiving leaves it pruned (reads as seen).
        ...
        # dispatch â€” agent-meow server stays harness-agnostic; the runner
        ...
        if live_forward and (model_override is not None or clear_model):
            ...
            # whose history agent-meow writes. Gate on the wrapper label (NOT
    ...
    # â”€â”€ POST /sessions/{source_id}/fork â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def fork_session(
        ...
        if body.model_override is not None:
            ...
            # actually supplied â€” a normal fork with no override is
        ...
        # the copied agent-meow transcript on its own â€” mark the fork so the
        ...
        # copied agent-meow items. Cross-family: the source's native transcript
        # is the wrong format, so ALWAYS rebuild from the copied agent-meow
        # items (the converters consume agent-meow's normalized item shape, so
    ...
    # â”€â”€ POST /sessions/{session_id}/switch-agent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def switch_session_agent(
        ...
        # agent-meow items (a format-agnostic conversion). SDK targets replay
    ...
    # â”€â”€ POST /sessions/{session_id}/hooks/permission-request â”€â”€â”€â”€â”€
    ...
    async def claude_permission_request_hook(
        ...
        the same Bearer token used for every other agent-meow request. For
        ...
        :param session_id: agent-meow conversation id from the URL path.
    ...
    # â”€â”€ POST /sessions/{session_id}/policies/evaluate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ POST /sessions/{session_id}/hooks/codex-elicitation-request â”€
    ...
    # â”€â”€ POST /sessions/{session_id}/hooks/antigravity-elicitation-request â”€â”€
    ...
    # â”€â”€ POST /sessions/{session_id}/hooks/cursor-permission-request â”€
    ...
    async def cursor_permission_request_hook(
        ...
        :param session_id: agent-meow conversation id from the URL path.
        :returns: An ``ElicitationResult`` (``{"action": â€¦}``) on a web
    ...
    # â”€â”€ POST /sessions/{session_id}/hooks/native-permission-request â”€
    ...
    async def native_permission_request_hook(
        ...
        :param session_id: agent-meow conversation id from the URL path.
        :returns: An ``ElicitationResult`` (``{"action": â€¦}``) on a web verdict,
    ...
    # â”€â”€ GET /sessions/{session_id}/items â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ GET /sessions/{session_id}/child_sessions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ GET /sessions/{session_id}/resources â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Phase 1b: typed resource collections & terminal lifecycle â”€â”€
    ...
    async def create_session_terminal(
        ...
        or ``bridge_inject_dir`` â€” the ``agent-meow claude`` / ``codex``
    ...
    # â”€â”€ Phase 1c: session-scoped file endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Phase 3: environment filesystem proxy endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Phase 5: environment shell proxy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ POST /sessions/{session_id}/events â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def get_elicitation(
        ...
        :mod:`~?omnigent.runtime.pending_elicitations` index â€” no
    ...
    async def post_event(
        ...
        - ``"external_output_reasoning_delta"`` publishes a transient
          ...
          observed outside the agent-meow task runtime, without persisting an
          item or starting / steering a task.
        - ``"external_session_interrupted"`` publishes a
          ``session.interrupted`` event observed outside the agent-meow task
        ...
        # â”€â”€ Policy evaluation (path-agnostic) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if body.type == _COMPACT_TYPE:
            ...
            # The agent-meow server stays harness-agnostic: it runs its own
        ...
        if runner_client is None:
            ...
            if body.type == "message" and _is_native_terminal_session(conv):
                ...
                return {"queued": True, "item_id": item_id}
            # Raise so the agent-meow server doesn't persist an item the
    ...
    # â”€â”€ GET /sessions/{session_id}/stream â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def stream_session(
        ...
        exit paths â€” see :func:`_stream_live_events`.
    ...
    # â”€â”€ DELETE /sessions/{session_id} â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Permission management endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Agent sub-resource â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ POST /sessions/{session_id}/mcp â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def mcp_proxy(
        ...
        - Policy runs on the agent-meow server where the ConversationStore and
          label state live.
