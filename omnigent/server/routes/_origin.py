...
**not** help routes that legitimately accept ``multipart/form-data`` â€”
...
- a **missing** ``Origin`` is allowed. Modern browsers attach ``Origin``
  to every cross-site (and same-site state-changing) request â€” it is on
  the forbidden-header list, so page JS cannot strip or forge it â€” so an
...
- in single-user **local mode** (no cookie / proxy auth) a present
  ``Origin`` must be a loopback host â€” this is where the guard actually
...
def require_trusted_origin(request: Request) -> None:
    ...
    not a browser CSRF vector â€” this preserves backward compatibility for
    ...
    # soon â€” first-party clients (SDK, runner, the test harness) already
