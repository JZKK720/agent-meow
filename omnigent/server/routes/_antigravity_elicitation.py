...
Pure shape-mapping â€” no I/O, no RPC calls.  The bridge (Task 8) and
the endpoint (Task 9) handle all network interaction.

Two functions are exported:

* :func:`to_elicitation_params` â€” converts a :class:`PendingInteraction`
...
* :func:`to_interaction_payload` â€” converts the user's
...
def _agy_questions_to_ask_user_question(spec: object) -> list[dict[str, Any]]:
    ...
    * each agy ``questions[i]`` â†’ one Claude question with a synthetic string
    ...
    * agy ``is_multi_select`` â†’ Claude ``multiSelect`` (radios vs checkboxes);
    * each agy option's ``text`` â†’ the Claude option ``label`` (the human-
      readable choice the form displays and returns). The agy option ``id`` is
      NOT sent to the SPA â€” the form returns the chosen *label*, which
...
def to_interaction_payload(
    ...
    :param kind: Interaction kind â€” ``"ask_question"`` or ``"permission"``.
...
def _agy_ask_question_response(
    ...
    ``content`` map keyed by each question's synthetic id (its index) â€” the
    value being the selected option *label(s)* (single-select â†’ one string,
    multi-select â†’ list of strings), or free-form custom text. agy's
    ...
    :param spec: The ``askQuestion`` spec block; supplies the option idâ†”text map
...
def _agy_permission_response(result: ElicitationResult) -> dict[str, Any]:
    ...
    ``accept`` â†’ ``allow: True``; ``decline`` or ``cancel`` â†’ ``allow: False``.
...
# binary Approve/Reject (the non-1:1 mapping with options 2/3 â€” the "always
# allow" variants â€” is intentionally acceptable, #1200), so Approve drives the
...
def to_tui_selection_keys(
    ...
    stale prompt's buffer â€” #1200). So the bridge ALSO types the selection into
    the pane via
    ...
    * **permission** â€” Approve â†’ option ``"1"`` ("Yes"), Reject â†’ option ``"4"``
    ...
    * **ask_question** â€” type the selected option id(s) ("1".."N") then ``Enter``;
...
def _agy_ask_question_tui_keys(
    ...
    cancel â€” or any answer that resolves to no concrete option id â€” presses
    ``Escape`` to dismiss the prompt (the RPC ``askQuestion`` skip already tells
    the backend; the keystroke clears the TUI surface).

    :param result: The web-submitted elicitation verdict.
    :param spec: The ``askQuestion`` spec block (option idâ†”text map).
