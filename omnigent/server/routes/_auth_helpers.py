...
full BEGIN / SELECT / COMMIT round trip â€” load testing showed this
...
def attribution_user(user_id: str | None) -> str | None:
    ...
    Drops the reserved single-user ``"local"`` sentinel â€” it is not a
...
class SessionAccess:
    ...
    :param conversation: The conversation fetched while authorizing, ready
        ...
        (who bypass the conversation lookup) â€” callers fall back to their
...
def _require_access_and_level_sync(
    ...
    if conv.parent_conversation_id is None:
        ...
        # conversation, so reuse the rows already fetched â€” no extra reads.
    ...
    # Denied â€” distinguish "has some access but not enough" (403) from
