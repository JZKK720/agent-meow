...
same prefix for OIDC mode) â€” only one is wired by ``create_app``.
...
Plausible â€” see ``designs/oss-cuj/01-research-summary.md`` Â§2.2.1):

- Login / logout via username + password (one cookie set on success).
- Invite-only signup â€” an admin mints a single-use copyable URL.
- No SMTP required â€” invite + admin-initiated reset cover the
...
mints the token â€” here it's ``/auth/login`` validating a stored
...
# NIST SP 800-63B Â§5.1.1.2 ("memorized secrets shall be at least
# 8 characters"). We don't enforce composition rules (mix of
# uppercase / digits / etc.) â€” current NIST guidance is to drop
# composition rules in favor of length + denylist.
_MIN_PASSWORD_LENGTH = 8

# Username constraints. Lowercase letters, digits, hyphens, dots,
# underscores; 1-64 chars. Restrictive on purpose â€” narrow the
...
def _set_session_cookie(
    ...
    *,
    ...
    magic redeem) uses the same attributes â€” divergence here is
    ...
    # app is embedded in a cross-origin iframe â€” e.g. Hugging Face Spaces' preview
    # pane â€” because browsers won't send a Lax cookie in a third-party frame, so
    ...
    # "samesite=none; Secure" option gated behind an opt-in env var â€” deferred, as
...
def _validate_username(username: str) -> str | None:
    ...
    â€” the auth provider also rejects them at cookie validation
...
def create_accounts_auth_router(
    ...
    # â”€â”€ Login / logout / me â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def login(body: LoginRequest) -> Response:
        ...
        "no such user" and "wrong password" â€” same status code,
        ...
        # Always run a verify even on missing user â€” keeps the
        ...
        if needs_rehash(password_hash):
            ...
            except Exception as exc:  # noqa: BLE001 â€” never fail login on rehash
                _logger.warning(
        ...
        # Apply the file-backed admin list (additive â€” never demotes).
        ...
        # `user` cannot be None here â€” we just verified the password
    ...
    async def me(request: Request) -> Response:
        ...
        if user is None:
            # Cookie validated but user row gone â€” admin deleted them
    ...
    # â”€â”€ Invite + register (admin-issued, self-serve) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def register(body: RegisterRequest) -> Response:
        ...
        Ordering: validate format â†’ check name availability â†’ THEN
        redeem the invite. Reversing this would burn the (single-use)
        invite on every name collision â€” and an attacker holding a
        ...
        is consumed in that case â€” the user was genuinely racing
        ...
        if account_store.get_user(username) is not None:
            # Don't burn the invite â€” the user can retry with a
        ...
        if token is None:
            ...
            # â€” keeps the route opaque to brute-force token guessing.
        ...
        try:
            ...
            # invite â€” invite is now consumed, no recovery here.
            return JSONResponse(status_code=409, content={"error": "username already taken"})
    ...
    async def setup(body: SetupRequest) -> Response:
        ...
        Unauthenticated by necessity â€” there's no one to authenticate
        ...
        choosing a username + password in the browser â€” no container
        ...
        try:
            ...
            # is now done â€” surface the same terminal 409.
            return JSONResponse(status_code=409, content={"error": "setup already completed"})
        ...
        # (loopback only) â€” otherwise the in-flight `run` 401s until the next
    ...
    # â”€â”€ Magic link (CLI â†’ web UI handoff) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def magic_mint(request: Request) -> Response:
        ...
        browser session in as the same user â€” zero password
    ...
    async def magic_redeem(request: Request) -> Response:
        ...
        their password. Always returns a redirect â€” on failure,
    ...
    # â”€â”€ Members admin (list, delete, reset password) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def delete_user(request: Request, user_id: str) -> Response:
        ...
        - **Self-delete** â€” the calling admin would lock themselves
          out of their own session (cookie still valid but row
          gone; every subsequent /v1/me 401s).
        - **Last admin** â€” the deploy must always have at least one
        ...
        if target.is_admin:
            ...
            if not other_admins:
                return JSONResponse(
                    status_code=400,
                    content={
                        "error": "cannot delete the last admin â€” promote another "
    ...
    async def change_own_password(request: Request, body: ChangePasswordRequest) -> Response:
        ...
        if current_hash is None:
            # User exists (cookie valid) but has no password â€” they
...
# this will always raise InvalidPasswordError â€” we only run it
