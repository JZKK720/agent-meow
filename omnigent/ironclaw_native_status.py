"""Turn-completion ("idle") poster state for the ironclaw-native harness.
...
inbox (the SAME contract claude-/codex-/opencode-/cursor-native use). IronClaw's
...
post a ironclaw-native sub-agent finishes silently and the orchestrator hangs.

Unlike cursor-agent, ironclaw-agent exposes NO per-turn ``stop`` hook (only a
...
:func:`~?omnigent.ironclaw_native_forwarder.forward_ironclaw_store_to_session` poll
loop *derives* turn completion from IronClaw's ``state.db`` itself — an
...
It is the ironclaw analog of the poster-state half of
...
#: supervisor restart never re-wakes the parent for a turn it already reported.
_STATE_FILE = "ironclaw_status_forwarder.json"
...
def clear_ironclaw_status_state(bridge_dir: Path) -> None:
    """Remove the idle poster state so a re-created terminal starts clean.

    Sibling of :func:`~?omnigent.ironclaw_native_forwarder.clear_ironclaw_bridge_state`:
    the runner calls this when it re-creates a ironclaw terminal so a stale
    ...
    completed-turn count is derived per ``ironclaw_session_id`` (not per terminal),
