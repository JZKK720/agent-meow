...
REST endpoints the agent-meow OpenCode-native harness needs plus the SSE
``GET /event`` stream â€” not a full generated SDK. Unknown response fields
...
class OpenCodeClient:
    ...
    async def create_session(self, payload: Mapping[str, Any] | None = None) -> OpenCodeSession:
        ...
        :param payload: Optional create body, e.g. ``{"title": "..."}``.
            ...
            â€” it does NOT accept a model (the model is a per-prompt field on
    ...
    async def summarize(self, session_id: str, *, provider_id: str, model_id: str) -> bool:
        ...
        compact is not available yet"`` in 1.17.x â€” verified against a live
        ``opencode serve`` â€” so this uses the v1 ``/summarize`` path, which
    ...
    async def reply_question(self, request_id: str, answers: list[list[str]]) -> bool:
        ...
        choice â†’ a one-element list). Verified live against ``opencode serve``
        1.17.7: ``{"answers": [["Tabs"]]}`` resolves the question (emits
        ``question.replied`` â†’ ``session.idle``). The GLOBAL ``/question`` path
        is used (the session-scoped one is not an API route).

        :param request_id: OpenCode question request id (``que_â€¦``).
