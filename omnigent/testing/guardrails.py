...
throwaway resources â€” running under pytest, against a tmp/in-memory
SQLite DB, and not aimed at a known dev/prod host or port â€” *before*
...
def looks_like_test_db(db_uri: str) -> bool:
    ...
    :param db_uri: A SQLAlchemy-style URI, e.g. ``sqlite:///â€¦`` .
...
def _under_temp_dir(path: Path) -> bool:
    ...
    temp dirs (macOS ``/tmp`` â†’ ``/private/tmp``) still match.
    ...
    *,
    ...
    if warn_only or guardrails_disabled:
        if not warn_only and guardrails_disabled:
            _logger.warning(
                "%s escape hatch active (%s) â€” hard-fail suppressed",
                _WARN_PREFIX,
