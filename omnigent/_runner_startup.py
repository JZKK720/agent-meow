...
1. A progress indicator while waiting for the runner to come up,
   ...
   itself on success â€” once the runner is online there should be
...
``run`` / ``claude`` cold-start path â€” ``_ensure_backend`` (daemon
spawn + local-server boot) and ``_prepare_chat_session_via_daemon``
(agent upload + runner bring-up) â€” so the user sees plain-language
forward motion ("Starting upâ€¦", "Starting the local serverâ€¦",
"Connectingâ€¦", "Launching your agentâ€¦") instead of a silent gap.
"""
...
# intent so a future "Waiting for runner tunnel registrationâ€¦" label
# fails loud.
STARTUP_PHASE_STARTING = "Starting upâ€¦"
STARTUP_PHASE_LOCAL_SERVER = "Starting the local serverâ€¦"
STARTUP_PHASE_CONNECTING_REMOTE = "Connecting to the serverâ€¦"
STARTUP_PHASE_PREPARING_AGENT = "Preparing your agentâ€¦"
STARTUP_PHASE_CONNECTING = "Connectingâ€¦"
...
# a vaguer "Almost readyâ€¦".
STARTUP_PHASE_LAUNCHING_AGENT = "Launching your agentâ€¦"
...
class RunnerStartupProgress:
    ...
    the context manager and torn down on context exit â€” or earlier
    ...
    :param update: Set the current progress message, e.g.
        ``progress.update("Starting local runnerâ€¦")``.
    :param finish: Tear the renderer down now (idempotent). Calling it
        is optional â€” the context manager also tears down on exit â€” but
...
def runner_startup_progress(
    *,
    ...
    gets the same UX::
        ...
        with runner_startup_progress(initial_message="Startingâ€¦"):
            _poll_remote_runner(...)
    ...
    :param initial_message: First line shown when the context
        opens, e.g. ``"Starting local runnerâ€¦"``.
    ...
    if enabled:
        ...
        # one-shot output (``agent-meow run â€¦ -p "â€¦"``) stays clean.
        ...
        # REPL paints â€” no empty gap between steps; (2)
        # ``redirect_stdout/stderr=False`` leaves the process std streams
        # unwrapped (our startup spans are silent), so the hand-off to the
        # prompt-toolkit REPL â€” which manages stdout via ``patch_stdout``
        # and a CPR handshake â€” isn't disturbed by a stream proxy being
    ...
    # signal that something was happening â€” leaving it in the
...
def format_runner_log_tail(log_path: Path | None) -> str:
    ...
    lines into the terminal â€” those make the actual error summary
    hard to spot.
    ...
    :param log_path: Path to the captured runner log, e.g.
        ...
        ``None`` returns an empty string (no log was captured â€”
