"""TUIâ†’web forwarder for the qwen-native harness.
...
That covers the webâ†’TUI direction, but the *embedded terminal* is then the only
surface that reflects the agent's work â€” the agent-meow conversation view stays
empty because nothing mirrors the transcript back into the session.

This module is that missing mirror â€” the qwen analog of
...
- ``{"type":"assistant","message":{"role":"assistant","content":[{"type":"text"|
  "thinking"|"tool_use",...}]}}`` â€” only ``text`` blocks are mirrored.
- ``{"type":"control_request","request":{"subtype":"can_use_tool",...},
  "request_id":...}`` and the matching ``control_response`` â€” the permission
...
(``session_start``'s ``supported_events`` omits it â€” verified live, ``qwen``
...
success, ``failed`` otherwise) â€” the completion half of the web ``/compact`` â†’
...
# mapping (a ``dict`` used as an ordered set), not a ``set`` â€” ``list(set)`` is
...
def _write_state(bridge_dir: Path, state: _ForwardState) -> bool:
    ...
    try:
        ...
        # _DEDUP_WINDOW uuids â€” the ones a relaunch re-read is most likely to hit.
...
def _text_from_content(content: object) -> str:
    ...
    ``thinking`` and ``tool_use`` blocks are skipped â€” only user-facing prose is
...
# --- Compaction mirror (chat-recording tail â†’ external_compaction_status) ------

#: qwen's CompressionStatus enum (verified, qwen v0.18.2): 1 = COMPRESSED (success),
#: 2/3 = COMPRESSION_FAILED_*. 1 â†’ completed; anything else â†’ failed.
...
def _read_new_compaction_statuses(recording: Path, offset: int) -> tuple[list[str], int]:
    ...
    try:
        ...
        return [], offset  # recording not created yet â€” retry next poll
    if size < offset:
...
async def supervise_qwen_compaction_mirror(
    *,
    ...
    compactions that happen *after* launch are posted â€” a resumed session's
    ...
    mirror â€” the offset is in-memory, so a forwarder restart may miss a compaction
