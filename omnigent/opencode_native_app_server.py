...
is runner-internal â€” the web UI attaches to agent-meow terminal resources,
...
# OpenCode env vars that point the server at the user's GLOBAL config â€” they
    ...
    *,
    ...
    for key, value in os.environ.items():
        if key in _ENV_OPENCODE_CONFIG_DENYLIST:
            # Never inherit the parent's global OpenCode config â€” the
...
def client_for_state(
    *,
    ...
    â€” it only has the URL + auth secret from bridge state.
