...
class CodexNativeExecutor(Executor):
    ...
    def __init__(self, bridge_dir: Path | None = None) -> None:
        ...
        # injections race that read-decide-write â€” both can see "no
    ...
    async def run_turn(
        ...
        # decision/RPC/write below runs under the lock â€” re-reading state so
        # it's atomic with respect to a steer that landed during the wait.
        state = read_bridge_state(self._bridge_dir)
        if state is None:
            for _ in range(60):
                # Startup already failed; the runner recorded the cause â€” stop waiting.
            ...
            if state is None:
                ...
                try:
                    if state.active_turn_id is not None:
                        ...
                        # ``effort``), NOT ``turn/start`` â€” whose params are
...
def _model_effort_overrides(config: ExecutorConfig | None) -> dict[str, Any]:
    ...
    itself takes no model/effort â€” its params are input/context only â€” which
    ...
    thread keeps its launch-pinned model â€” so this is a no-op for
    ...
    try:
        ...
        # A bad effort must not sink the turn â€” drop it and keep Codex's
...
def _content_to_input_items(content: Any, bridge_dir: Path) -> list[dict[str, Any]]:
    ...
    ``{"type": "localImage", "path": ...}`` â€” sending the base64 data
