...
Drives GitHub Copilot via :mod:`copilot` (the ``github-copilot-sdk`` package) â€”
one persistent :class:`copilot.CopilotClient` + :class:`copilot.CopilotSession`
per agent-meow conversation, created once and reused turn to turn. Each
...
assistant text deltas â†’ :class:`TextChunk`, reasoning deltas â†’
:class:`ReasoningChunk`, tool execution â†’ :class:`ToolCallRequest` /
...
whose async ``handler`` routes back to the executor's ``_tool_executor`` â€” the
...
directly â€” no ``run_coroutine_threadsafe`` hop.

Native tools â€” Copilot's built-in ``create`` / ``view`` / ``edit`` / ``bash`` run
*inside* the SDK rather than through agent-meow's bridged-tool dispatch. They are
...
transcript â€” only the streamed narration is recorded â€” so an ``on:[tool_call]``
...
Auth: a **GitHub token** that carries Copilot access â€” a fine-grained PAT with
...
def _resolve_model(model: str | None) -> str | None:
    ...
    own auto-select (``None`` â†’ the SDK picks). ``None`` likewise lets the SDK
    choose.
    """
...
def _resolve_reasoning_effort(config: ExecutorConfig | None) -> str | None:
    ...
    but the Copilot SDK has no per-turn output-token cap â€” the only
    ``max_output_tokens`` lever is a model *capability* override folded into
    context-window math, not a generation limit â€” so it is intentionally not
    forwarded here.
    """
    if config is None:
        return None
...
def _tools_fingerprint(tools: list[ToolSpec]) -> str:
    ...
    invalidate the persistent session â€” otherwise removed tools stay callable
...
def _build_copilot_prompt(messages: list[Message], *, is_first_turn: bool) -> str:
    ...
    agent-meow system prompt is delivered separately (``system_message``), so on
...
def _encode_tool_result(result: Any) -> Any:  # type: ignore[explicit-any]
    ...
    surface it as a ``failure`` result so the Copilot model sees the failure â€”
...
class CopilotExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # handler is wired (single-process / test paths â†’ default approve).
    ...
    def handles_tools_internally(self) -> bool:
        ...
        # observed tool events â€” same contract as claude-sdk / cursor.
        return True
    ...
    def _make_tools(self, tools: list[ToolSpec]) -> list[Any]:  # type: ignore[explicit-any]
        """Build the SDK ``tools`` list from agent-meow ToolSpecs.

        Each tool's async ``handler`` is awaited by the SDK in its own event
        loop, so it bridges straight into ``_tool_executor`` â€” no thread hop.
        ``skip_permission=True`` because agent-meow's policy layer (and the
    ...
    def _make_handler(self, tool_name: str) -> Callable[[Any], Awaitable[Any]]:  # type: ignore[explicit-any]
        """Build an async ``handler`` that bridges a Copilot tool call to agent-meow.
        ...
        async def handler(invocation: Any) -> Any:  # type: ignore[explicit-any]
            ...
            try:
                ...
                raise
            except Exception as exc:  # noqa: BLE001 â€” surface as a tool failure
                return ToolResult(
    ...
    async def _on_permission_request(self, request: Any, _invocation: dict[str, str]) -> Any:  # type: ignore[explicit-any]
        ...
        if evaluator is not None:
            ...
            if getattr(verdict, "action", "") == "POLICY_ACTION_DENY":
                ...
                return PermissionDecisionReject(feedback=f"Denied by agent-meow policy: {reason}")

        # Stage 2 â€” user elicitation: surface an approval card.
        handler = self._elicitation_handler
        if handler is not None:
            approved = await handler(name, args)
            if not approved:
                return PermissionDecisionReject(feedback="Denied via agent-meow approval UI")
    ...
    async def _ensure_session(
        ...
        try:
            ...
            # path re-raises without terminating that subprocess â€” only
            ...
            # its built-in tools) and layers the agent-meow agent's system prompt
            # on top â€” a ``replace`` would strip the tool-use guidance the model
    ...
    async def run_turn(
        ...
        try:
            while True:
                ...
                if getter in done:
                    ...
                    getter.cancel()
                if send_task.done():
                    # The turn ended â€” drain whatever is still queued, then stop.
            ...
            raise
        except Exception as exc:  # noqa: BLE001 â€” the SDK turn failed mid-stream
            await self.close_session(session_key)
        ...
        # whenever the turn did NOT produce a successful final message â€” even if
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        # ``stop()`` from racing a live generation â€” which can orphan the CLI's
        ...
        # Always drop the session so the next turn starts a fresh one â€” mirrors
...
def _permission_policy_input(request: Any) -> tuple[str, dict[str, Any]]:  # type: ignore[explicit-any]
    ...
    except Exception:  # noqa: BLE001 â€” defensive: hand the policy a best-effort dict
        args = {}
...
def _event_data(event: Any) -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    except Exception:  # noqa: BLE001 â€” fall back to attribute access below
        payload = None
...
def _unwrap_tool_result(raw: Any) -> Any:  # type: ignore[explicit-any]
    ...
    result) rather than the wrapper â€” matching the bare-payload convention the
    peer executors use. Only a recognized wrapper shape is unwrapped, so an
    agent-meow-convention result (e.g. ``{"blocked": True}``) passes through
