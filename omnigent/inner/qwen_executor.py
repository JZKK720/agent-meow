...
class QwenExecutor(Executor):
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``session/request_permission`` â€” decide via agent-meow's TOOL_CALL
    ...
    def _text_from_blocks(
        ...
        *,
        ...
        :param emit_image_marker: When ``True``, append a ``[attached image:
            â€¦]`` marker for each ``input_image`` (used when qwen can't accept a
    ...
    async def run_turn(
        ...
        tools: list[Any],  # type: ignore[explicit-any]  # noqa: ARG002 â€” qwen runs its own tool registry; param required by the Executor interface
        system_prompt: str,
