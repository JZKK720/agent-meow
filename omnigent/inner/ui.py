"""Shared terminal-output styling for the agent-meow CLI.
...
Core rule â€” **stdout carries data, stderr carries decoration**:

* Machine-readable output (IDs, paths, config dumps, the ``version``
  string) goes to stdout via :data:`console`, so ``agent-meow â€¦ | cat``
  stays clean.
...
#: Brand accent â€” Otto's magenta-pink, shared with the mascot and banner.
...
# inside our messages â€” CLI output must be predictable. File is resolved
# lazily by rich, so these follow ``CliRunner`` / ``capsys`` stream swaps.
#: Console for stdout â€” data and primary output.
console = Console(theme=OMNIGENT_THEME, highlight=False)
#: Console for stderr â€” status, warnings, errors, and the brand banner.
...
def _emit(target: Console, glyph: str, style: str, message: str) -> None:
    ...
    :param glyph: Leading status glyph, e.g. ``"âœ“"``.
...
def success(message: str) -> None:
    """Print a ``âœ“`` success line (green) to stdout."""
    _emit(console, "âœ“", "omni.success", message)


def info(message: str) -> None:
    """Print a dim ``Â·`` informational line to stdout."""
    _emit(console, "Â·", "omni.muted", message)
...
def error(message: str) -> None:
    """Print a ``âœ—`` error (red) to stderr."""
    _emit(err_console, "âœ—", "omni.error", message)


# â”€â”€ Structure helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Brand banner â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def print_landing(
    *,
    ...
    :param gradient: Fade the wordmark magentaâ†’pink (default on for the
...
def print_brandmark(subtitle: str | None = None) -> None:
    """
    Print the compact one-line brandmark (``âœ¦ agent-meow``), TTY-gated.
