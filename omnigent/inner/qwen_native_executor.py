...
It does not launch ``qwen`` â€” the ``agent-meow qwen`` wrapper already launched the
...
file append â€” no settle-detection, paste-commit polling, or draft-clearing. See
...
class QwenNativeExecutor(Executor):
    ...
    async def _ensure_ready(self) -> None:
        ...
        for qwen's first ``system`` event on the events stream â€” emitted after the
        watcher is up â€” so the offset is taken on the still-empty input file.
        ...
        anyway â€” best-effort beats hanging the turn â€” but the warning makes the
        ...
        if ready:
            ...
            logger.warning(
                "qwen-native readiness gate timed out for %s; submitting anyway â€” "
    ...
    def supports_streaming(self) -> bool:
        """:returns: ``False`` â€” output is shown by the embedded terminal, not this executor."""
        return False

    def supports_live_message_queue(self) -> bool:
        """:returns: ``True`` â€” messages can be injected mid-turn (steering)."""
        return True
...
def _content_to_text(content: Any, bridge_dir: Path) -> str:
    ...
    (``[Attached: <path>]``) so qwen can open them with its tools â€” otherwise
