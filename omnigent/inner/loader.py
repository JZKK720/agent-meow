...
# tree (tools, policies, executor, os_env, sandbox â€” each with variant-typed
...
def load_agent_def(
    ...
    *,
    ...
    treated as inline text â€” there is no anchor for path
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        **calls** it. This is the guard for the untrusted
        ...
        custom handlers â€” the operator already has code execution, so
...
def _reject_unregistered_policy_handlers(data: YamlData) -> None:
    ...
    scanned â€” they are a separate surface and are not invoked at parse
...
def _read_contained_file(root: Path, value: str) -> str | None:
    ...
    *value* as inline text.
    ...
    try:
        ...
        # Path too long or invalid characters â€” fall through to inline text.
        pass
    return None
...
def _resolve_instructions(
    ...
    so omnigent-flavored YAMLs and native agent-meow YAMLs treat the
    field identically:
    ...
    - File-path-shaped value that doesn't resolve: silently
      treated as inline text. Matches native agent-meow behavior so users
    ...
    :param instructions_root: Directory to anchor relative path
        ...
        every value as inline text â€” used when the loader has no
...
def _parse_agent_def(
    ...
    *,
    ...
    # text right here (file path â†’ contents, or inline string).
    # Translator code downstream only sees the resolved string
    # in ``agent.instructions`` and the raw user-supplied
    # ``agent.prompt`` â€” it doesn't have to re-walk the path.
    ...
    # â€” egress_rules stay on the policy object but an override to
...
def _parse_tool(name: str, data: str | YamlData) -> Tool:
    if isinstance(data, str):
        ...
        if data == "self":
            ...
            # :class:`SelfAgentTool` â€” the translator materializes
    ...
    if tool_type == "mcp":
        ...
        # form â€” it matches the executor auth shape.
            ...
            for conflicting in (
                ...
                if conflicting in data:
                    raise ValueError(
                        ...
                        f"cloned from the parent â€” declare a regular "
    ...
    if tool_type == "skill":
        # ``content`` is an alias for ``path`` â€” either is accepted. ``None``
...
def _parse_policy(name: str, data: YamlData | str | bool | None) -> Policy:
    ...
    # Default ``on`` for function policies is all four phases â€” the callable
...
def _parse_os_env_spec(data: YamlData | str | bool | None) -> OSEnvSpec | None:
    ...
    # the sandbox on the legacy path" bug â€” an operator who used
    # ``load_agent_def`` would otherwise get past parse time with
    # a spec that ``create_os_environment`` later rejects (or, worse,
    # accepts with weaker semantics than the agent-meow parser would have
    # allowed).
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    if raw_type is None:
        ...
        # (same behavior as the agent-meow YAML parser, kept in sync so legacy
        # and agent-meow loaders agree on what an "untyped" sandbox block means).
    ...
    # Mirror the agent-meow parser's hard reject of ``egress_rules`` paired with
    ...
    # MITM proxy is never wired up â€” the rules would sit in the spec
    ...
    # bundle/config.yaml path can never drift â€” a duplicated parser here
