...
agent-meow as the system of record for tools, policies, and session history.
agent-meow tools are exposed as SDK FunctionTools whose callbacks route back
through the Session's tool registry.

Conversation state is delegated to the SDK's session layer. Each agent-meow
Session gets its own SDK SQLiteSession keyed by the agent-meow session id.
"""
...
# Replay items persisted to the SDK Session â€” heterogeneous Responses-API
...
# an optional import at type-check time â€” the executor only constructs
...
# to the agent-meow tool executor.
...
def _normalize_responses_items_for_chat(
    ...
    list.  Non-message items (function_call, function_call_output, â€¦) pass
    through unchanged.
...
def _normalize_content_blocks_for_chat(
    ...
    agent-meow-only metadata (for example ``filename``) from known Responses
    ...
    ``filename``.  That metadata is useful in agent-meow, but the OpenAI API
    ...
    Completions ``file`` content type, which expects **plain base64** â€” not a
    ...
    for block in content:
        ...
        if block_type == "input_file":
            ...
            # Preserve inline images; strip agent-meow-only metadata.
...
def _copy_known_keys(block: dict[str, Any], keys: tuple[str, ...]) -> dict[str, Any]:
    ...
    agent-meow content blocks can carry UI/history metadata such as
...
class _SDKSession(Protocol):
    """Structural view of ``agents.memory.Session`` â€” the four async
...
class _AgentsSDK(Protocol):
    ...
    the caller touches â€” kept deliberately loose (``Any`` factories in a
...
class _RunState(Protocol):
    """Structural view of ``agents.run_state.RunState`` â€” only the three
...
class _ToolCallRawItem(Protocol):
    ...
    corresponding field â€” ``None`` on items where it isn't set.
    """
...
def _get_openai_async_client(
    ...
    :param api_key: Direct OpenAI-compatible API key, e.g.
        ...
        ``AsyncOpenAI(api_key=â€¦)`` directly, bypassing all profile and
        env-var lookups. Intended for specs that declare
        ``executor.auth: {type: api_key, api_key: â€¦}``.
    ...
    :param model: The model name that will be used. When set to a
        ...
        credential fallback are skipped â€” the caller must supply
    ...
    # Explicit spec-level api_key (executor.auth: {type: api_key, â€¦}).
    ...
    # can be dropped anywhere on the daemon â†’ runner â†’ harness propagation
    ...
    # Non-Databricks model with no OpenAI credentials â€” fail loudly rather
    ...
    # No profile, no env â€” final fallback via ambient Databricks credentials.
    try:
...
class _AgentsSessionState:
    ...
    :param stream_consumer_task: The asyncio.Task running
        ...
        â€” the SDK's own ``except CancelledError`` then calls
        ...
        flag but does NOT close the network connection â€” events
...
# already been ``model_dump``-ed to dicts, or primitive JSON values â€” so
...
class RawToolItemParts:
    ...
    The SDK hands tool calls back in one of two shapes â€” a dict (e.g. the
    Responses-API input-item TypedDict) or a pydantic model such as
    ``ResponseFunctionToolCall`` â€” and this dataclass is the single
...
class _ReasoningBlockFilterStream:
    ...
    list input â€” ``delta`` must be ``str``.
    ...
    async def __anext__(self) -> Any:  # type: ignore[explicit-any]
        ...
        for choice in chunk.choices:
            if isinstance(getattr(choice.delta, "content", None), list):
                # Bypass Pydantic's __setattr__ validation â€” the field type
...
def _is_empty_turn(
    ...
    narrower gate (``output_tokens == 0``) on top of this one â€” see the
...
class OpenAIAgentsSDKExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param api_key: Direct OpenAI-compatible API key, e.g.
            ...
            declares ``executor.auth: {type: api_key, api_key: â€¦}``.
        ...
        :param model: Optional constructor-level model default. Acts
            as the spec-level default â€” applied when a per-turn
        ...
        :param base_url_override: Override the OpenAI-compatible base URL
            ...
            the agent-meow workflow layer). Required whenever ``gateway_host`` is set.
        ...
        if _base.startswith("http"):
            try:
                ...
                class _SafeCompactionSession(OpenAIResponsesCompactionSession):
                    ...
                    failure should not kill the turn â€” the session continues
                    without compaction.
                    """
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        task lands ``CancelledError`` exactly there â€” and the
        ...
        unwinds â€” finally stopping the network bytes that were
        producing 1,500+ post-cancel deltas in the original bug.

        Calling ``active.cancel(mode="immediate")`` on the result
        directly does *not* close the network stream â€” it only
        ...
        # the SDK's stream_events()'s queue.get() â€” see the class
        ...
        *,
        ...
        if len(delta_messages) == 1 and delta_messages[0].get("role") == "user":
            ...
            if isinstance(content, list):
                ...
                # content type, which expects plain base64 â€” not the data:
    ...
    async def _consume_stream_into_queue(
        ...
        the SDK's ``stream_events`` (agents/result.py:721) â€” and
        ...
        network connection â€” events keep streaming for 15+
        seconds.
        ...
        try:
            ...
            # Always push the sentinel â€” even on CancelledError /
            # exception â€” so the consumer's ``await queue.get()``
    ...
    async def run_turn(
        ...
        # (HARNESS_OPENAI_AGENTS_MODEL â†’ self._model_override).
        model = (
            cfg.model
        ...
        # (no text, no tool calls), so retrying never double-emits â€” we
        ...
        for attempt in range(_EMPTY_TURN_MAX_ATTEMPTS):
            ...
            # the producer's ``put_nowait`` never blocks â€” keeping the
            ...
            try:
                ...
                # before â€” these used to fire from inside the inline
                ...
                # CancelledError â€” state.interrupt_requested is True,
                ...
                if isinstance(exc, DatabricksAuthError) or (
                    ...
                    # When exc IS the DatabricksAuthError, use str(exc) â€” it
                ...
                # pump task is still alive â€” cancel it here so it
            ...
            if attempt + 1 < _EMPTY_TURN_MAX_ATTEMPTS:
                ...
                # the retry re-runs from an identical state â€” the empty
        ...
        # falls through to the silent ``TurnComplete("")`` below â€” exactly
        ...
        # output) â€” the best proxy for how full the context window
                ...
                if getattr(item, "type", None) == "compaction_item":
                    ...
                    yield CompactionComplete(
                        summary=(
                            ...
                            " â€” context was automatically compacted]"
                        ),
