...
experimental betas are disabled â€” the request builder gates ``display``
...
``display="summarized"`` re-injected; every other request â€” and every
response, including SSE streams â€” is forwarded verbatim and unbuffered.
...
# ``thinking.display="omitted"`` â€” Opus 4.7+ (e.g.
...
# Hop-by-hop headers (RFC 9110 Â§7.6.1) must not be forwarded by an
...
# ASGI callable types â€” uvicorn's protocol is untyped dicts.
...
class ClaudeGatewayShim:
    ...
    async def start(self) -> None:
        ...
        Idempotent â€” subsequent calls return immediately once the
        server is up.
    ...
    async def _start_locked(self) -> None:
        ...
        config = uvicorn.Config(
            self._asgi_app,
            host="127.0.0.1",
            port=0,  # ephemeral â€” the bound port is read back below
            log_level="warning",
        ...
        logger.info(
            "Claude gateway shim listening on %s â†’ %s",
            self.base_url,
