...
so the agy TUI and the agent-meow web mirror share ONE conversation in both
directions. The prior headless RPC path delivered onto a separate
``StartCascade`` cascade the TUI never showed â€” so the agy TUI never echoed web
...
* does NOT stream (``supports_streaming() -> False``) â€” the read driver posts the
  assistant message;
...
* supports a live message queue (``supports_live_message_queue() -> True``) â€” a
...
(design Â§10.4): (1) echo agy's CURRENT model from the latest ``USER_INPUT`` step's
...
``GetAvailableModels``. The agent-meow ``ExecutorConfig.model``/``reasoning_effort``
stay informational on this write path â€” agy's own model selection determines the
...
absolute path (``[Attached: <path>]``) so agy can open it with its Read tool â€”
...
# the user was on for that turn (the tier-1 model-echo source, design Â§10.4).
...
class AntigravityNativeExecutor(Executor):
    ...
    def supports_streaming(self) -> bool:
        """:returns: ``False`` â€” assistant output is emitted by the RPC read driver."""
        return False

    def supports_live_message_queue(self) -> bool:
        """:returns: ``True`` â€” a mid-turn web message is delivered over the same turn-send RPC."""
        return True
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        .. note:: **Scope â€” RUNNING cascades only (live-verified, C3).**
           ``CancelCascadeSteps`` stops an in-flight (generating) cascade â€” the
           ...
           **NO-OP on a step that is WAITING for a user interaction**
           ...
           (:mod:`~?omnigent.antigravity_native_interactions`), NOT here â€” this
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from agent-meow. Ignored; native agy owns its
            own tool surface.
        ...
        :param config: Per-turn executor config. Only ``reasoning_effort`` is
            ...
            effort is informational â€” agy's model selection determines the actual
    ...
    async def _deliver(self, text: str) -> str | None:
        ...
        into the agy TUI pane over tmux (bracketed paste + Enter â€” see
        ...
        never showed â€” splitting the TUI and the web mirror into two cascades
        ...
        discovers/binds â€” see :mod:`~?omnigent.antigravity_native_reader`).
...
def _latest_requested_model(steps: list[dict[str, object]]) -> str | None:
    ...
    Tier-1 of the per-turn model resolution (design Â§10.4): scans the trajectory
    ...
    as a STRING at ``userInput.userConfig.plannerConfig.planModel`` â€” the same
...
def _recommended_model(catalog: dict[str, object]) -> str | None:
    ...
    Tier-2 of the per-turn model resolution (design Â§10.4): picks the entry agy
...
def _content_to_text(content: EnqueuedContent, bridge_dir: Path) -> str:
    ...
    (``[Attached: <path>]``) so agy can open them with its Read tool â€” otherwise
    web-UI attachments are silently dropped. Mirrors cursor-native.

    :param content: Message content â€” a string, a list of content blocks like
