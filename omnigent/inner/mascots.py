"""Static terminal mascot art for agent-meow startup banners."""
...
# Otto the starfish â€” a compact Braille (U+28xx) silhouette of a
...
# ``omnigent.cli``. Otto's starfish magenta-pink â€” the agent-meow brand accent.
...
def random_mascot_color() -> str:
    ...
    :returns: Hex color string for the agent-meow accent,
        e.g. ``"#F43BA6"``.
    """
...
def random_mascot_lines() -> list[str]:
    ...
    mascot API, but the TUI now uses the single static agent-meow mascot.
