...
It does not launch ``kimi`` â€” the ``agent-meow kimi`` wrapper already
...
kimi_executor.KimiExecutor`, which shells ``kimi -p â€¦ --output-format
...
class KimiNativeExecutor(Executor):
    ...
    def __init__(self, bridge_dir: Path | None = None) -> None:
        ...
        # paste + Enter) â€” without the lock their keystrokes interleave.
...
def _content_to_text(content: Any, bridge_dir: Path) -> str:
    ...
    (``[Attached: <path>]``) so kimi can open them with its Read tool â€”
