"""Executor that bridges agent-meow web-chat turns into the native IronClaw TUI.

It does not launch ``ironclaw`` — the ``agent-meow ironclaw`` wrapper already launched
the interactive ``ironclaw`` TUI in the session terminal. Each web-UI turn injects
the latest user message into that same tmux pane (bracketed paste + Enter), so the
message appears in the running IronClaw TUI (and, since the web UI embeds the pane,
...
from omnigent.ironclaw_native_bridge import (
    BRIDGE_DIR_ENV_VAR,
    inject_interrupt,
    inject_user_message,
)
...
class IronclawNativeExecutor(Executor):
    """Harness-side executor for ``agent-meow ironclaw`` web-UI turns.

    Injects each web-UI message into the running IronClaw TUI's tmux pane. Does not
    ...
    async def enqueue_session_message(self, session_key: str, content: Any) -> bool:
        """Inject a live steering message into the IronClaw terminal."""
        del session_key
    ...
    async def run_turn(
        ...
        """Inject the latest web-UI user message into the IronClaw TUI pane."""
        del tools, system_prompt, config
        text = _latest_user_text(messages, self._bridge_dir)
        if not text:
            yield ExecutorError(message="ironclaw native turn had no user text to send")
            return
        try:
            async with self._inject_lock:
                await asyncio.to_thread(inject_user_message, self._bridge_dir, content=text)
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        """Cancel the in-flight IronClaw turn by sending Escape to the TUI pane.
...
def _bridge_dir_from_env() -> Path:
    """Resolve the ironclaw-native bridge dir from the harness spawn env."""
    raw = os.environ.get(BRIDGE_DIR_ENV_VAR, "").strip()
    if not raw:
        raise RuntimeError(f"{BRIDGE_DIR_ENV_VAR} is required for the ironclaw-native harness")
    return Path(raw)
...
def _content_to_text(content: Any, bridge_dir: Path) -> str:
    """Normalize executor content into text the IronClaw TUI receives.
    ...
    (``[Attached: <path>]``) so IronClaw can open them with its tools — otherwise
