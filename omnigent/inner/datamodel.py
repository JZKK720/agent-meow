"""Core data model types for agent-meow, which are all exposed to agents and in configs."""
...
class Message:
    ...
    :param content: Message body â€” plain text or provider-specific
...
class CredentialSourceSpec:
    ...
    handed to the sandbox verbatim â€” only a synthetic placeholder is.
...
class CredentialProxyEntry:
    ...
    **Opt-in: env injection.** Some clients refuse to issue a request
    when they don't see a credential locally â€” most notably ``gh``,
    ...
    The placeholder is non-secret â€” the real secret still never enters
    the sandbox.
    ...
    :param inject_env: Opt-in environment-variable names set to a
        ...
        pure swap-on-access â€” nothing is injected and the proxy attaches
...
class OSEnvSandboxSpec:
    ...
    # NOT expanded â€” see ``_resolve_root`` for the rationale).
    #
    ...
    #    ``~/.config/gcloud/`` etc. â€” those dotdirs are masked just
    ...
    # list means "mask every dotfile" â€” there is no implicit default
    ...
    #   and tunes the limit explicitly. Fail-Loud â€” the right pick for
    #   untrusted source trees where an unmasked dotfile is a leak.
    # - ``"unlimited"``: ignore the cap and walk the entire tree â€”
    ...
    #   ``.ssh/id_*``) sits past the cap â€” e.g. checked into a
    #   ``node_modules`` subtree, or planted by a compromised tool
    #   call earlier in the session â€” the sandbox will NOT mask it.
    ...
    #   malicious workspace bomb the resolver â€” a tarbomb with
    ...
    # ``GITHUB_TOKEN``, ``OPENAI_API_KEY``, etc. â€” passing all of that
    ...
    # (e.g. ``"GET api.github.com/repos/org/**"``). Default deny â€”
    ...
    # routable â€” RFC1918 private ranges, loopback, link-local, IPv6
    ...
    # routed only inside the tenant â€” currently Azure WireServer
    ...
    # to reach internal corp services on private IPs â€” auditable
...
class TerminalEnvSpec:
    ...
    :param terminal_transport: How the web UI attaches to this terminal:
        ...
        native scrollback + selection â€” the default) or ``"pty"`` (the legacy
...
class AgentDef:
    ...
    # sys_session_share â€” the SOLE enabler of that tool (independent of
    # spawn / declared agents, and unrelated to server-API / CLI
    # sharing). Raw YAML string from ``agent_session_sharing:`` â€” "none"
    ...
    # ``--plugin-dir`` mechanism. Set by the AgentSpec â†’ AgentDef
    ...
    # option; other harnesses ignore it. Defaults to ``"all"`` â€”
