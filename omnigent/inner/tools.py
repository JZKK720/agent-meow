"""Tool type hierarchy for agent-meow."""
...
# User-supplied Python callables bound via YAML (``callable: mod.func``) â€”
...
class CancellableRunner(Protocol):
    ...
    ``cancel(reason)`` method â€” callers must check at runtime because older
    runners may omit it.
    """
...
class Tool:
    ...
    :param output_schema: Optional JSON-Schema for the tool's return
        value. Advisory only â€” not enforced.
...
class CancellableFunctionTool(Tool):
    ...
    the target is a runner instance until resolve-time â€” the session
...
class AgentTool(Tool):
    ...
    :param tools: Sub-agent's tool registry â€” may include
...
class SelfAgentTool(Tool):
    ...
    :class:`AgentDef` and re-running the translation â€” same model,
    ...
    needed â€” the entire sub-agent configuration comes from the
...
class HandoffTool(Tool):
    ...
    :param history_filter: Optional callable applied to the history
        before handoff â€” signature is opaque to the framework.
