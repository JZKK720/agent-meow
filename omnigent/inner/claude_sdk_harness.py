...
than per-spec data on the request body. agent-meow sets these env vars
...
- ``HARNESS_CLAUDE_SDK_RETRY_POLICY``: JSON-encoded
  ...
  executor's default ``RetryPolicy()`` applies â€” matches AP's
...
- ``HARNESS_CLAUDE_SDK_SKILLS_FILTER``: JSON-encoded
  ...
  ``ClaudeAgentOptions(skills=[], setting_sources=[])`` â€” the
...
# Set by the agent-meow workflow layer when executor.auth: {type: api_key, â€¦} is
# declared.  Passed into ClaudeSDKExecutor.api_key_helper so it reaches
# settings.apiKeyHelper at turn time (not read from os.environ â€” the
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    so the SDK still exposes the natives â€” leaving them off by
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing â€” agent-meow omits the env var when ``llm.retry`` matches
    ...
    retry payload shouldn't prevent the harness from booting â€”
...
def _resolve_skills_filter() -> str | list[str]:
    ...
    to ``"all"`` â€” the SDK's default behavior of loading every
...
def _build_claude_sdk_executor() -> Executor:
    ...
    :raises OSError: If ``HARNESS_CLAUDE_SDK_GATEWAY`` is set
        but credentials are missing â€” the inner executor's
