...
1. **Hidden entries** â€” any file/directory whose basename starts with
   ``.`` and is NOT in the spec's ``cwd_allow_hidden`` allowlist.
2. **Escaping symlinks** â€” any symlink (at any depth) whose target
...
- ``cwd_hidden_scan_overflow`` chooses behaviour when the cap is hit:
  ...
  - ``"error"``: raise :class:`OSError` with an actionable message
    naming both spec keys the user can tune. Fail-Loud â€” the right
...
get walked when they're on ``allow_hidden`` â€” otherwise they're
...
exception alike) names the directories the walk did not finish â€”
...
the deprioritized ones â€” so an operator can tell at a glance that, for
...
# their basename is on ``allow_hidden`` â€” otherwise they are masked and
...
class MaskedEntry:
    ...
    :param path: Absolute, resolved-without-strict path to the entry.
        Backend emitters use this verbatim â€” ``bwrap`` as the mount
...
def scan_cwd_mask_entries(
    ...
    *,
    ...
    :data:`_DEFAULT_DEPRIORITIZED_DIRS` â€” ``node_modules``, ``.venv``,
    ``.mypy_cache``, ``.codex-tmp``) are deferred and walked only after
    every other directory has been processed, so the cap budget masks
    the real project tree first. They are still walked â€” and their
    dotfiles still masked â€” when budget remains; they are simply last
    ...
    :param safe_roots: Paths the sandbox already exposes (typically
        ...
        is responsible for assembling this list â€” bwrap and Seatbelt
    ...
    # tripped â€” reported as "partially scanned". ``None`` until then.
    partial_dir: Path | None = None

    while (stack or deferred) and not truncated:
        if not stack:
            # Primary tier drained â€” promote the deprioritized subtrees
        ...
        try:
            ...
            # Unreadable directory â€” skip without masking. The backend
        ...
        for child in children:
            ...
            if should_mask:
                ...
                if key in seen:
                    continue
                seen.add(key)
                # ``is_dir`` follows symlinks by default â€” matches what
            ...
            # Not masked â€” recurse only into real directories so a
...
def _handle_scan_overflow(
    *,
    ...
    Builds the shared overflow message â€” which names the unfinished
    directories via :func:`_summarize_unfinished_dirs` â€” and then either
    ...
    # ``"warn"`` and ``"unlimited"`` â€” fail-soft with an obvious log
    ...
    logger.critical(
        ...
        "cap are READABLE by the sandboxed helper â€” including "
