...
- :func:`prepare_credential_proxy_runtime` â€” parent-side: resolve
  secrets, mint placeholders for ``inject_env`` entries, and build the
  helper env updates plus the proxy rewrite rules.
- :class:`CredentialRewriteRule` â€” the host-scoped real-credential
...
def prepare_credential_proxy_runtime(
    ...
    *,
    ...
    Pure swap-on-access entries mint nothing â€” the proxy attaches the
