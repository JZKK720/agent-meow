...
agent reuse, and a streaming :meth:`run_turn` mapping SDK events onto agent-meow
...
.. note::
   ...
   harness only runs on hosts with glibc â‰³ 2.36; older hosts surface an
   :class:`ExecutorError`.
...
# back through the Session registry, so the in-SDK agent reaches agent-meow's
...
def _ensure_antigravity_sdk() -> ModuleType:
    ...
    :raises ImportError: If ``google-antigravity`` isn't installed â€” surfaced
...
def _latest_user_text(messages: list[Message]) -> str:
    ...
    :param messages: The agent-meow turn message list (role / content dicts),
...
def _content_to_text(content: Any) -> str:  # type: ignore[explicit-any]
    ...
    :param content: The message ``content`` field â€” a ``str``, a ``list`` of
...
def _render_prior_history(prior: list[Message]) -> str:
    ...
    ``Conversation`` accumulates history only from steps it streams â€” it
    exposes no API to inject prior turns, and ``send()`` triggers a model
    turn â€” so the prior turns ride into the single ``send()`` of the next
    ...
    :param prior: The conversation messages preceding the latest user turn
        (``messages[:-1]``), as agent-meow role/content dicts.
    :returns: A transcript prefix like ``"Conversation so far:\\nuser: â€¦\\n
        assistant: â€¦"``, or ``""`` when no prior user/assistant text exists.
    """
    lines: list[str] = []
...
class _AntigravitySessionState:
    ...
    :param conversation: The agent's live ``Conversation`` (from
        ``agent.conversation``) â€” source of the ``Step`` stream and the
    ...
    :param agent_signature: ``(model, system_prompt, tool_signature)`` key; a
        ...
        conversation â€” fine since mid-session model switches are rare.
...
class AntigravityExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # invocations through this to reach agent-meow's tools under policy.
    ...
    def handles_tools_internally(self) -> bool:
        ...
        # Session must not re-execute on ToolCallRequest â€” they're informational.
        return True
    ...
    def _session_key(self, messages: list[Message]) -> str:
        ...
        :param messages: The agent-meow turn message list; the last entry's
    ...
    def _tool_signature(tools: list[ToolSpec]) -> str:
        """Stable cache key for a tool set (names only â€” enough to detect change).

        :param tools: agent-meow tool specs for the turn.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        :param session_key: The agent-meow session id whose agent to release.
        """
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        :meth:`_ensure_agent`'s rebuild path â€” the same teardown the
        error / signature-change path uses â€” which closes the stale agent and
        ...
        :param session_key: The agent-meow session id to interrupt.
        ...
        # Drop the cached signature (not the agent itself â€” _ensure_agent needs
    ...
    async def run_turn(
        ...
        request / tool completion) and a terminal :class:`TurnComplete` â€”
        or :class:`TurnCancelled` on interrupt, :class:`ExecutorError` on
        failure.

        :param messages: The conversation messages for this turn.
        :param tools: agent-meow tool specs exposed to the agent as callables.
        ...
        # A FRESH agent (new session, or a model/system-prompt/tools rebuild â€”
        ...
        # prefix â€” the SDK exposes no history-injection API and ``send()``
    ...
    # â”€â”€ SDK touchpoints (isolated; duck-typed; verified against v0.1.x) â”€â”€
    ...
    def _complete_pending_from_step(
        ...
        present here wasn't completed by it â€” emit a :class:`ToolCallComplete`
    ...
    async def _ensure_agent(
        ...
        *,
        ...
        :param session_key: The agent-meow session id.
        :param model: The resolved model id to pin, e.g. ``"gemini-3.5-flash"``.
        :param system_prompt: The agent's system instructions.
        :param tools: agent-meow tool specs to expose to the agent.
        :returns: ``(state, created)`` â€” the session's
        ...
        # by ``close()`` / ``close_session()`` for teardown â€” otherwise that
    ...
    async def _open_agent(
        ...
        *,
        ...
        agent's async context. agent-meow's tools are exposed as callables
        ...
        :param system_prompt: The agent's system instructions.
        :param tools: agent-meow tool specs to expose as callables.
    ...
    def _build_post_tool_hook(
        ...
        class _OmnigentToolCompleteHook(antigravity.hooks.PostToolCallHook):  # type: ignore[misc, name-defined]
            ...
            async def run(self, context: SDKHookContext, data: SDKToolResult) -> None:  # noqa: ARG002 â€” context unused; required by hook signature
                ...
                # fallback already completed it â€” skip to avoid a double emit.
    ...
    def _build_sdk_tools(self, tools: list[ToolSpec]) -> list[SDKTool]:
        """Build SDK tools (plain callables) from agent-meow tool specs.
        ...
        the agent reaches agent-meow's tool registry under policy. Returns ``[]``
        when there are no tools or no executor bridge yet (the agent then runs
        with its native + MCP tools only).

        :param tools: agent-meow tool specs (``name`` / ``description`` /
            ``parameters``).
    ...
    def _make_tool_callable(self, tool_name: str, description: str) -> _ToolCallable:
        ...
        :param tool_name: The agent-meow tool name, e.g. ``"sys_shell"``.
    ...
    def _extract_usage(meta: SDKUsage) -> _StrAnyDict | None:
        """Map an SDK ``UsageMetadata`` to agent-meow's usage dict shape.
    ...
    async def _close_agent(agent: SDKAgent) -> None:
        ...
        if closer is not None:
            ...
            except Exception:  # noqa: BLE001 â€” agent teardown is best-effort
