...
- **Fail-loud bind**: If ``asyncio.start_server`` fails (e.g. port
  ...
  on the port â€” a same-user-process MITM risk on macOS where
...
it the WEAKEST secret in the system â€” strictly weaker than the
...
def start_relay(
    ...
    MITM â€” without it, a port-bind race would result in the
    ...
    :raises OSError: If the relay cannot bind to the port. This is
        intentionally fail-loud â€” silently absorbing this error
...
async def _serve(
    sock: object,  # socket.socket â€” typed loosely to avoid an import in the signature.
    ...
    async def _handle(
        ...
        try:
            ...
            except Exception:  # noqa: BLE001 â€” connection teardown is best-effort
                pass
            finally:
                for w in (client_writer, ux_writer):
                    try:  # noqa: SIM105
                        w.close()
                    except Exception:  # noqa: BLE001 â€” close is best-effort
                        pass
        except Exception:
            logger.exception("Egress relay handler crashed")
            # ``close()`` is best-effort â€” the connection may already be torn down
