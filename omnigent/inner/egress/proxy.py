...
# this is a control byte and is rejected in the inner request line â€” see
...
def _parse_http_headers(headers_raw: bytes) -> Message:
    ...
    so the standard library's email parser â€” the same machinery
    :mod:`http.client` uses under the hood â€” tokenizes header blocks
...
class _AuthRewriteResult:
    ...
    :param headers: The header block to forward upstream â€” rewritten
...
# â€” the cloud vendor just picked a public-looking IP and routes it
...
_CLOUD_TRAP_NETWORKS: tuple[ipaddress.IPv4Network | ipaddress.IPv6Network, ...] = (
    ...
    # network â€” used for boot-time guest agent communication, host
...
class EgressProxy:
    ...
    :param upstream_ca_bundle: Optional path to a CA bundle for
        ...
        write â€” the controller passes the immutable bundle under
    ...
    :param block_private_destinations: When ``True`` (the default),
        ...
        which currently lists Azure WireServer ``168.63.129.16`` â€”
        ...
        agents that legitimately reach intranet endpoints â€” wired
    ...
    :param auth_token: When set, every inbound proxy connection MUST
        ...
        â€” Linux ``bwrap`` is already protected by its own network
    ...
    :param credential_rewrites: Optional host-scoped real-credential
        ...
        ``Authorization`` header â€” nothing credential-shaped ever enters
    ...
    def __init__(
        ...
        *,
        ...
        # context here â€” sourced from the host-only bundle the agent
        # cannot write â€” removes the per-request read of an
        ...
        #   injects the real credential. Exactly one rule per host â€” the
    ...
    def _client_connected(
        ...
        """Server accept callback â€” create and track the handler task.
        ...
        self-registering handler would have â€” a handler whose task is
    ...
    async def _handle_client(
        ...
        try:
            ...
            # upstream connect â€” a peer that can't prove it's our
            ...
            if not self._check_proxy_auth(headers):
                logger.warning(
                    "REJECT-AUTH %s %s â€” missing or invalid Proxy-Authorization",
                    method,
                    target,
                )
            ...
            except Exception:  # noqa: BLE001 â€” client close is best-effort
                pass
    ...
    async def _handle_connect(
        ...
        """Handle CONNECT â€” TLS MITM, inspect inner HTTP, enforce rules."""
        ...
        # differential â€” leaks data to the attacker's authoritative
        ...
        # policy" via the response â€” same oracle hygiene as
        # ``_send_proxy_auth_required``.
        if not is_dns_safe_host(host):
            logger.warning(
                "REJECT-INVALID-HOST CONNECT %r â€” host contains "
        ...
        #      our post-tunnel check fires â€” the rejection then
        ...
        # its 30 s timeout and the request silently died â€” surfacing
        ...
        try:
            ...
            # parsed (method, path) below â€” that makes the upstream
            ...
            # ``0xa0`` â€” so a control-byte filter alone would be
            ...
            if any(ord(ch) < _MIN_PRINTABLE_BYTE for ch in inner_line):
                logger.warning(
                    "REJECT-CONTROL-CHAR CONNECT %s â€” inner request line contains a control byte",
                    host,
                )
            ...
            except Exception:  # noqa: BLE001 â€” TLS close is best-effort
                pass
    ...
    async def _forward_https(
        ...
        # a cafile here â€” that re-read of a (potentially sandbox-writable)
        ...
        # would otherwise perform â€” the rebinding window this guard
        ...
        if rewrite.error is not None:
            logger.warning(
                "BLOCKED-CREDENTIAL %s https://%s%s â€” %s", method, host, path, rewrite.error
            )
            await self._send_forbidden(client_writer, rewrite.error)
            return
        # Single-shot the upstream so ``_relay_response`` gets a prompt EOF
        # instead of blocking on a keep-alive socket â€” and so pipelining
        ...
        except Exception as exc:  # noqa: BLE001 â€” upstream connect failure maps to 502
        ...
        try:
            ...
            if bytes_relayed == 0:
                ...
                # Empty reply from server`` â€” indistinguishable from a
            ...
            except Exception:  # noqa: BLE001 â€” upstream close is best-effort
                pass
    ...
    async def _handle_http(
        ...
        # connection silently â€” a malformed-URL request shouldn't
        ...
        try:
            ...
            logger.warning("REJECT-MALFORMED-URL %s %r â€” urlparse: %s", method, url, exc)
            await self._send_forbidden(writer, "host contains forbidden character")
            return
        ...
        # ``_handle_connect`` â€” see the comment there for the full
        ...
        if not is_dns_safe_host(host):
            logger.warning(
                "REJECT-INVALID-HOST %s %r â€” host contains characters "
        ...
        if rewrite.error is not None:
            logger.warning(
                "BLOCKED-CREDENTIAL %s http://%s%s â€” %s", method, host, path, rewrite.error
            )
        ...
        # None) to avoid a second, independent DNS lookup â€” the
    ...
    async def _assert_destination_allowed(self, host: str, port: int) -> str | None:
        ...
        RFC 6598 (``100.64.0.0/10``) â€” notably Alibaba Cloud's IMDS
        at ``100.100.100.200`` â€” in addition to the usual private,
        ...
        â€” currently just Azure WireServer ``168.63.129.16``. These
        ...
        **Fail closed + pin the IP (DNS-rebinding defense).** This
        ...
        try:
            ...
            raise PermissionError(
                f"host {host!r} DNS resolution failed ({exc}) â€” blocked "
            ...
            for trap_net in _CLOUD_TRAP_NETWORKS:
                if addr.version == trap_net.version and addr in trap_net:
                    raise PermissionError(
                        ...
                        f"endpoint {ip_str} ({trap_net}) â€” blocked "
        ...
        if pinned_ip is None:
            ...
            raise PermissionError(
                ...
                "â€” blocked because egress_allow_private_destinations is False"
            )
        return pinned_ip
    ...
    async def _relay_response(
        ...
        means the upstream produced no response at all â€” EOF on the
    ...
    async def _send_forbidden(writer: asyncio.StreamWriter, message: str) -> None:
        ...
        try:
            writer.write(resp)
            await writer.drain()
        except Exception:  # noqa: BLE001 â€” response write is best-effort
            pass
    ...
    def _rewrite_authorization(self, *, host: str, headers_raw: bytes) -> _AuthRewriteResult:
        ...
        - **Placeholder swap (opt-in).** When the request's
          ...
          ``403`` â€” the cross-host leak guard for a compromised sandbox.
        ...
        if existing:
            ...
            for value in existing:
                ...
                if synthetic is None:
                    # A real/foreign Authorization the client set itself â€”
                ...
                if rule is None or rule.host != host_key:
                    ...
                    # recognise for this host. Refuse rather than forward â€”
            ...
            changed = True
        if not changed:
            # Nothing matched â€” forward the client's bytes untouched rather
    ...
    async def _send_proxy_auth_required(writer: asyncio.StreamWriter) -> None:
        ...
        userinfo component. Body is intentionally generic â€” leaking
    ...
    def _check_proxy_auth(self, headers_raw: bytes) -> bool:
        ...
        - When :attr:`_auth_token` is ``None``, all requests pass (the
          proxy was constructed without auth â€” relied on by tests and
    ...
    def _strip_proxy_auth(headers_raw: bytes) -> bytes:
        ...
        removed. The header is hop-by-hop (RFC 7235 Â§4) so MUST NOT
        be forwarded upstream, but we don't rely on the upstream
        server to honor that â€” we drop it ourselves so a logged
    ...
    def _force_connection_close(headers_raw: bytes) -> bytes:
        ...
        and ``Keep-Alive`` are all hop-by-hop (RFC 7230 Â§6.1), so dropping
        ...
        # (RFC 7230 Â§6.1) before we substitute our own single directive.
        del msg["Connection"]
