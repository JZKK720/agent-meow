"""Startup banner builder for the agent-meow REPL and onboarding wizard.

Migrated from the legacy ``omnigent.inner.cli`` module during the
``--no-sessions-api`` sunset so that the agent-meow REPL and onboarding wizard can
...
_CLI_WELCOME_HINT_LINE = "ctrl-c: cancel Â· ctrl-d: quit"


def _ansi_truecolor_fg(hex_rgb: str) -> str:
    """``#RRGGBB`` â†’ ANSI 24-bit foreground; invalid input falls back to gray."""
...
def _display_width(text: str) -> int:
    ...
    SELECTOR-16 (U+FE0F) â€” e.g. the cli-config ``âš™ï¸`` glyph â€” as the two
    ...
    :param text: The string to measure, e.g. ``"âš™ï¸ my-gateway"``.
...
class BannerLine:
    ...
    :param text: The row's text. Width is measured with
        ...
        ``ðŸ§± Databricks`` credential label) pad and align correctly.
...
def startup_banner_strings(
    ...
    *,
    ...
    (*agent_label*) plus any number of dim info rows to the right â€”
    ...
    :param info_lines: Explicit dim info rows shown beneath the title,
        ...
        BannerLine("claude-sonnet-4-6 Â· Subscription", dim=True),
    ...
    # Pad with blank rows so every mascot row has a content row to sit on â€”
    ...
    def _content_line(i: int, row: BannerLine) -> RenderedLines:
        ...
        ansi = (
            f"{_ansi_truecolor_fg(art_color)}â”‚{_RESET} "
            f"{ansi_content}"
            f" {_ansi_truecolor_fg(art_color)}â”‚{_RESET}"
        )
