...
1. :data:`~?omnigent.inner._seccomp.BASELINE_DENYLIST_SYSCALLS` â€” the
...
3. ``clone3`` denied with ``ENOSYS`` (not ``EPERM``) â€” the
...
4. Argument-filtered ``socket(domain, ...)``: an allowlist of
   ...
   socket family is denied with ``EPERM``. This is future-proof â€”
...
# Read-only directory binds for the multi-file /etc subtrees â€” linker
...
# ``clone()`` would create a fresh namespace â€” the canonical user-
...
class BwrapSandboxBackend(SandboxBackend):
    ...
    def resolve(self, spec: OSEnvSpec, cwd: Path) -> SandboxPolicy:
        ...
        - The bwrap binary must be on ``PATH`` or the resolver fails
          loud with an install hint â€” no silent fallback.
        ...
        # the design plan â€” agents that need an editable project tree
    ...
    def wrap_launcher_argv(
        ...
        :param chdir: Optional separate target for ``--chdir``. When
            ...
            set â€” typically to the per-helper scratch tmpdir for
            ``OSEnvSpec.start_in_scratch`` â€” the helper starts there
        ...
        :param target: Absolute path to the binary that the launcher
            ...
            its directory chain is bind-mounted into the namespace â€”
        ...
        # the sandbox even when it lives outside the default mounts â€”
        ...
        # target via subprocess.run â€” without this bind the target's
        ...
        # Additional write roots â€” typically the per-helper scratch
        ...
        # write root â€” the instance ``private_dir`` â€” so the helper can
        ...
        # host (which manages the server) keeps the real socket â€” the
        ...
        # under the system temp dir â€” the parent's
        ...
        # when egress rules are active (hard enforcement â€” the helper
    ...
    def activate(self, policy: SandboxPolicy) -> None:
        ...
        namespaces), but we re-set it here for defense in depth â€” the
        ...
        if policy.egress_relay_port is not None and policy.egress_socket_path is not None:
            ...
            # so port collisions with host processes are impossible â€”
...
def _bwrap_extra_seccomp_rules() -> list[SeccompRule]:
    ...
    1. ``clone(flags, ...)`` denied when any ``CLONE_NEW*`` bit is set
       ...
       ``fork()`` and ``pthread_create()`` keep working â€” they don't
       set ``CLONE_NEW*``.
    2. ``clone3`` denied with ``ENOSYS``. ``struct clone_args`` lives
       ...
       on ``ENOSYS`` â€” returning ``EPERM`` propagates up and breaks
    ...
    3. ``socket(domain, ...)`` denied for all families NOT in
       ...
       deny ``AF_UNSPEC`` (0), deny families 3â€“9 (gap between
       ``AF_INET`` and ``AF_INET6``), and deny ``>= 11`` via
       ``SCMP_CMP_GE`` â€” catching all current and future families
...
def _set_no_new_privs() -> None:
    ...
    Idempotent â€” bwrap already does this for the helper, but applying
...
def _resolve_root(cwd: Path, root: str) -> Path:
    ...
    sandbox-widening lever â€” an attacker who can influence
    ...
    :returns: An absolute, normalized :class:`Path` (without strict
        existence check â€” the caller may bind a path that doesn't
        exist yet).
    """
    if "$" in root:
        _LOGGER.warning(
...
def _ensure_executable_visible(argv: list[str], cwd: Path) -> list[str]:
    ...
    Bwrap exec's ``argv[0]`` literally â€” it does NOT follow symlinks on
    the host before exec â€” so every name in the path the kernel will
...
def _is_within(path: Path, root: Path, *, resolve: bool = True) -> bool:
    ...
    roots (e.g. ``./.venv/bin/python`` â†’ ``/usr/bin/python3.12``)
    count as "within" the safe root for safety checks.

    Set *resolve* to ``False`` for path-string-prefix checks where
    symlink dereferencing would mask the literal path's location â€”
...
def _bwrap_safe_roots(
    ...
    policy's read/write roots, AND â€” when *argv* is supplied â€” the
    ...
    ``--bind /dev/null`` mask over it â€” making the helper unspawnable
...
def _dotfile_and_symlink_mask_args(
    ...
    - **Directories** are masked with ``--tmpfs <path>`` â€” the agent
      sees an empty in-memory directory.
    - **Files / symlinks / sockets** are masked with
      ``--bind-try /dev/null <path>`` â€” the agent sees what looks like
    ...
    Roots that live under ``cwd`` are skipped â€” the cwd pass already
    ...
    # didn't already cover (skip roots that live under cwd â€” those
    ...
    for entry in entries:
        ...
        # target is safe â€” persistent host dotfiles still exist and are
        # still masked.
