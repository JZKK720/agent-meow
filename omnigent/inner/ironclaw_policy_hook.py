"""IronClaw ``pre_tool_call`` shell hook for agent-meow policy enforcement.

Registered as a ``pre_tool_call`` hook in the per-session
``IRONCLAW_HOME/config.yaml`` written by :func:`write_policy_hook_config`
in :mod:`omnigent.ironclaw_native_bridge`.

IronClaw pipes a JSON payload to stdin before each tool execution::
...
The hook evaluates ``PHASE_TOOL_CALL`` policy via the agent-meow server.
...
def main() -> None:
    ...
    # card whose long-poll hangs. IronClaw's own tools lack the prefix and stay gated.
    ...
    try:
        ...
        resp, api_error = post_evaluate_with_retry(
            ...
            hook_label="ironclaw pre_tool_call",
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["reason"] = f"Tool '{tool_name}' denied by agent-meow policy: {reason}"
        else:
            out["reason"] = f"Tool '{tool_name}' denied by agent-meow policy"
