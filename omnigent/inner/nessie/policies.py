...
``designs/NESSIE.md`` "Layer 1 â€” enforcement".
"""
...
def read_only_os(
    *,
    ...
    ``tee``) â€” this policy does not gate that, and command parsing cannot
    reliably catch it. For a hard guarantee (e.g. reviewing untrusted input),
    run the agent sandboxed â€” ``os_env.sandbox.type: linux_bwrap`` (Linux) /
    ``darwin_seatbelt`` (macOS) binds cwd read-only â€” and treat this policy as
