"""Qwen-native tool-approval mirror (TUI â†’ web elicitation).

The ``qwen`` TUI gates its own tool calls (shell, file write, â€¦) with an
in-terminal approval prompt. Unlike cursor-native â€” where the prompt lives only
in the TUI's in-memory state and has to be scraped off the pane â€” qwen exposes a
...
So to surface those approvals in the agent-meow web UI (so a user on the Chat tab
...
3. on the verdict, answer qwen by appending a ``confirmation_response`` to
   ``--input-file`` (``allowed=True`` on accept, ``False`` on decline/cancel) â€”
   no keystrokes,
...
class QwenApprovalRequest:
    ...
    :param request_id: qwen's ``request_id`` for the control request â€” the
...
class _ControlEvent:
    ...
    :param request_id: The control request's id (correlates request â†” response).
...
def qwen_permission_elicitation_id(session_id: str, request_id: str) -> str:
    """Return the deterministic agent-meow elicitation id for a qwen control request.

    qwen's ``request_id`` is already unique per pending tool call, so it keys the
    elicitation directly â€” stable across polls and recomputable for the
    loser-release path.
    """
...
def _control_response_request_id(event: dict[str, object]) -> str | None:
    ...
    by an external ``confirmation_response`` â€” either way it marks the request
...
def _read_new_control_events(events_file: Path, offset: int) -> tuple[list[_ControlEvent], int]:
    ...
    truncated/recreated file (``size < offset`` â†’ rewind to 0), consumes only
...
async def supervise_qwen_approval_mirror(
    *,
    ...
    requests are parked â€” history was already resolved, and re-parking it would
                ...
                for ev in events:
                    if ev.kind == "request":
                        ...
                        if isinstance(task, asyncio.Task) and not task.done():
                            ...
                            # answered â†’ release the parked card.
...
async def _run_one_approval(
    ...
    *,
    ...
    if not response.content:
        # Empty 2xx â†’ resolved elsewhere (TUI answered) or timeout: no response.
        return
    try:
