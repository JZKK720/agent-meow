"""
:class:`FunctionPolicy` â€” policy backed by a Python callable.
...
- ``function: myorg.policies.rate_limit`` â€” the resolved
  dotted path IS the evaluator, called directly.
- ``function: {path: ..., arguments: {...}}`` â€” the resolved
...
# Distinguishing form handled at call time â€” the adapter wraps
...
class FunctionPolicy(Policy):
    """
    A policy driven by a Python callable (POLICIES.md Â§9.1).
    ...
    async def evaluate(
        ...
        context: dict[str, Any],  # noqa: ARG002 â€” engine contract; callable uses event.context instead
    ) -> PolicyResult:
        """
        ...
        5. Lets any exception bubble up â€” the engine wraps it
        ...
        :param context: Read-only engine context bundle
            ...
            callable â€” use ``event.context`` instead.
...
def _build_event(ctx: EvaluationContext) -> dict[str, Any]:
    ...
    event: dict[str, Any] = {
        ...
        "context": {
            ...
            # empty dict otherwise (that policy treats it as $0 â†’ never trips).
            ...
            # injected by the engine. ``None`` when undeterminable â€” cost
...
def resolve_function_policy(spec: FunctionPolicySpec) -> FunctionPolicy:
    ...
    :raises ValueError: If ``spec.function`` is absent (the
        parser should have rejected this â€” fail loud here
    ...
    # factory â€” call it to produce the evaluator. Direct callables
    ...
    if func_ref.arguments is not None:
        ...
        # Factory with all-default params â€” invoke to get evaluator.
        callable_obj = target()
    else:
        callable_obj = target
    ...
    # function.py â†’ _omnigent_legacy_shim â†’ policies.types â†’
    # policies.__init__ â†’ function.py (partially initialised).
...
def _resolve_dotted_path(path: str) -> Any:
    ...
    with no attribute â€” not useful in practice, so we reject.
...
def _callable_arity(fn: _PolicyCallable) -> int:
    ...
    ``**kwargs`` count as 0 here â€” policies that want them
...
def _has_no_required_params(fn: _PolicyCallable) -> bool:
    ...
    have only keyword-with-default params â€” no required positional
...
def _coerce_to_policy_result(raw: Any, *, spec_name: str) -> PolicyResult:
    ...
    - ``None`` â€” treated as ALLOW (defensive fallback; callables should
      return an explicit ALLOW decision rather than relying on this).
    - :class:`PolicyResult` â€” returned as-is.
    ...
    - Anything else â†’ :class:`TypeError` with a clear
    ...
    """
    if raw is None:
        # Defensive fallback â€” callables should return explicit ALLOW.
    ...
    if hasattr(raw, "action"):
        ...
        # ``action``/``reason``/``set_labels`` structurally â€”
...
def _coerce_state_updates(
    ...
    *,
    ...
    1. ``None`` â†’ ``None`` (no updates).
    2. ``list[dict]`` â€” each dict has ``key``, ``action``,
       and optional ``value``. This is the canonical form.
    3. ``dict[str, Any]`` â€” legacy flat key-value map, converted
       to a list of ``SET`` operations for backward compat.
    4. ``list[StateUpdate]`` â€” already coerced (from a
    ...
    if raw is None:
        return None
    if isinstance(raw, dict):
        # Legacy compat: {"call_count": 5} â†’ [SET("call_count", 5)]
