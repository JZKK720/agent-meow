...
These classes are pure evaluators â€” they hold no mutable state
...
class Policy(ABC):
    ...
    async def evaluate(
        ...
        :param ctx: Current evaluation context â€” phase,
        ...
        :param context: Read-only context bundle from the
            engine â€” labels snapshot, conversation_id, and
            ...
            with agent-meow' signature (see POLICIES.md Â§9.1).
        :returns: The policy's single-policy
            :class:`PolicyResult` (``deciding_policy`` left
            ``None`` â€” engine fills it on composed results).
        """

    def reset_turn(self) -> None:  # noqa: B027 â€” intentional no-op default (see docstring)
        """
        ...
        accumulators â€” e.g. the ``max_tool_calls_per_turn``
        rate-limit factory in
        ``examples/_shared/rate_limit_policy.py`` â€” override
        ...
        The runtime calls this once per "turn" â€” defined as one
        user prompt â†’ terminal assistant response cycle, which
