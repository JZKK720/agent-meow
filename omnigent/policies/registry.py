"""Policy registry â€” discovers and serves built-in policy metadata.
...
def load_registry(
    ...
    logged and skipped â€” a broken module should not prevent
...
def is_registered_handler(handler: str) -> bool:
    ...
    The registry â€” built-in policy modules plus any admin-configured
    ``policy_modules`` â€” is the single allowlist of policy handlers a
    ...
    exist" â€” this keeps the check correct without requiring every caller
...
def validate_factory_params(
    ...
    entry = _registry_by_handler.get(handler)
    if entry is None:
        # Not in registry â€” custom policy, skip validation.
        return None

    if entry.kind == "callable":
        # Direct callable â€” must not receive factory_params.
        if factory_params:
            return (
    ...
    # kind == "factory" â€” validate against params_schema.
    schema = entry.params_schema
    if schema is None:
        # Factory with no declared schema â€” accept anything.
        return None
