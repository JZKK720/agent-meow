...
- **MCP tool calls** â€” both the official per-operation GitHub MCP server
  (``get_file_contents`` / ``create_or_update_file`` / ``push_files`` /
  ``create_pull_request`` / ``merge_pull_request`` â€¦) and the layered
...
- **``git`` / ``gh`` shell commands** dispatched through ``sys_os_shell``
  ...
  never touch the remote (``git status`` / ``git commit`` / ``git diff`` â€¦)
...
- When a *shell* command is a gated read/write but its target repo (or, for a
  ...
  command text â€” e.g. ``git push origin main``, where ``origin`` is a local
  remote alias the policy cannot resolve to a repo â€” the decision is **ASK**
...
classified as read or write fails closed (DENY) â€” we will not let an
...
# prefix â€” both legal DNS-label characters) does NOT match and leak a foreign repo
...
# Arg keys (MCP) carrying a *destination* branch / ref â€” the branch a write
# lands on. Excludes ``head`` / ``headRefName``: a PR's head is its source
# branch, not a write target, so gating it would wrongly block ``feature â†’
# main`` PRs.
...
def _repo_from_url(value: str) -> str:
    ...
    ``a/b`` string as a repo â€” it matches only when the value actually
...
def _normalize_repos(values: list[str] | None) -> set[str]:
    ...
    :param values: Repo IDs and/or GitHub URLs. ``None`` â†’ empty list.
...
def _normalize_branch(value: str) -> str:
    ...
    for ref_prefix in ("refs/heads/", "heads/"):
        if value.startswith(ref_prefix):
            ...
            break
    # A cross-fork PR head is ``owner:branch`` â€” keep only the branch.
    if ":" in value:
...
def _normalize_branches(values: list[str] | None) -> set[str]:
    ...
    :param values: Branch names / refs. ``None`` â†’ empty list.
...
# â”€â”€ MCP tool classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# Canonical (post-``github_`` strip) names that touch no repo and are always
# safe â€” discovery / planning helpers exposed by the wrapper servers.
...
# missing branch means the repo default, which we cannot confirm â†’ fail to the
...
# â”€â”€ Shell command classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# gh top-level command groups â†’ the set of actions under each that write.
...
# gh groups that are GitHub-aware but never touch a specific repo's contents â€”
...
class _ShellOp:
    ...
    :param kind: Access kind â€” ``"read"``, ``"write"``, ``"ignore"`` (local /
        non-repo command, no gating), or ``"unparseable"`` (a git/gh write we
        could not tokenize â€” fail to ASK).
    ...
    :param branch_targeted: Whether this op writes content to a specific branch
        ...
        content (``gh issue create``, ``gh pr merge`` â€¦).
...
def _classify_git(tokens: list[str]) -> _ShellOp | None:
    ...
    :returns: A :class:`_ShellOp` for remote read/write subcommands, or ``None``
        for local-only git commands (status / commit / diff / branch / â€¦),
    ...
    if sub in _GIT_WRITE_SUBCMDS:
        # ``git push [flags] [<remote>] [<refspec>...]`` â€” first non-flag token
...
def _classify_gh(tokens: list[str]) -> _ShellOp | None:
    ...
    # (reads are the safer default â€” they are only gated when read_all is off).
    return _ShellOp(
...
def _classify_gh_api(rest: list[str], repo: str | None) -> _ShellOp:
    ...
    (``-X POST`` / ``--method PATCH`` / â€¦) or the presence of field flags
...
def _classify_shell_command(command: str, _depth: int = 0) -> list[_ShellOp]:
    ...
    Commands wrapped in a shell interpreter (``bash -c "git push â€¦"``) or
...
# â”€â”€ The policy factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def github_policy(
    *,
    ...
    :param read_repos: Repos readable when ``read_all`` is ``False``, as
        ``owner/repo`` or GitHub URLs, e.g. ``["octo/hello"]``. ``None`` â†’ none.
    :param write_repos: Repos the agent may write to, as ``owner/repo`` or URLs.
        ``None`` â†’ no repo is writable.
