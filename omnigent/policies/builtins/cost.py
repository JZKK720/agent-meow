...
# cannot enforce a budget it cannot measure â€” asking rather than denying lets
...
def _usage_is_unpriced(usage: dict[str, Any]) -> bool:
    ...
    ``total_cost_usd`` is absent â€” meaning no turn was ever priced by the
    ...
    The first turn on an unpriced model still runs â€” the gate only has
    post-turn data at check time â€” but every subsequent turn is denied.
    ...
    *,
    ...
    if block_all:
        ...
        return (
            ...
            f"the user verbatim, then stop and wait for them â€” do not silently re-run the tool "
...
def _model_blocked_over_budget(
    ...
    *,
    ...
    factory) every model is blocked â€” the budget is a true hard stop.
    ...
    :param block_all: When ``True``, return ``True`` for every model
        (``expensive_models=[]`` semantics â€” all models blocked).
...
class _ExpensiveModelConfig:
    ...
    :param hard_cap_enabled: Whether the hard over-budget DENY gate is
        active. Always ``True`` â€” including when the caller passes an
    ...
    :param block_all_models: When ``True`` (set only when the caller
        passes ``expensive_models=[]``) every model â€” not just a named
        expensive tier â€” is blocked once the hard limit is reached.
...
def _resolve_expensive_models(expensive_models: list[str] | None) -> _ExpensiveModelConfig:
    ...
    - ``None`` or ``[]`` â†’ **all models blocked** once over budget
    ...
    - a non-empty list â†’ those tokens (lowercased), matched literally
      ...
      on (downgrade gate â€” cheaper models still allowed over budget).
...
def cost_budget(
    ...
    while the session is still on an expensive model â€” telling the user
    ...
    :param ask_thresholds_usd: Optional soft warning checkpoints in USD,
        ...
        Order does not matter â€” they are sorted internally.
...
def user_daily_cost_budget(
    ...
    :param expensive_models: Optional case-insensitive substring tokens
        ...
        (the default) or ``[]`` makes the hard cap a true hard stop â€”
