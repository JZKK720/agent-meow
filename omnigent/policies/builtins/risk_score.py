...
- **Accrue risk** as the agent takes risky actions. Two sources:
  - *Tool calls* â€” each call to a configured tool adds points
    (``tool_points``), e.g. a ``web_search`` adds 10. This source needs no
    special support from any tool, so it works out of the box.
  - *Tool results* â€” a result carrying a sensitive data-classification label
    ...
    *any* MCP server that annotates its results with a classification field â€”
...
"start a session at higher risk" for specific users without writing Python â€”
...
tool when they are equal *or* the raw name ends with ``"__" + name`` â€” so
...
# real results are only a few levels deep. Truncating is fail-safe here â€” an
...
class _RiskCfg:
    ...
    :param tool_points: Canonical tool name â†’ points added on each call, e.g.
        ``{"web_search": 10}``.
    :param sensitive_labels: Lower-cased classification label â†’ points added
    ...
    :param escalate_action: Verdict for a guarded tool over threshold â€”
        ``"ASK"`` or ``"DENY"``.
    :param initial_scores_by_actor: ``context.actor.run_as`` email â†’ starting
...
def _points_for_tool(raw_tool: str, tool_points: dict[str, int]) -> int:
    ...
    :param tool_points: Canonical tool name â†’ points mapping.
    :returns: Total points (0 when no configured name matches). When several
        configured names match the same raw tool, their points sum â€” configure
...
def _collect_labels(payload: Any, label_keys: tuple[str, ...], max_depth: int) -> set[str]:  # type: ignore[explicit-any]
    ...
    risk (fail-safe â€” the score never drops).

    :param payload: Parsed ``tool_result`` payload â€” dict, list, or scalar, e.g.
...
def _current_score(event: PolicyEvent, cfg: _RiskCfg) -> int:
    ...
    :returns: The effective integer score (â‰¥ the actor's offset).
    """
...
# â”€â”€ Phase handlers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _decide_tool_call(event: PolicyEvent, cfg: _RiskCfg) -> PolicyResponse | None:
    ...
    ``threshold`` escalates (ASK/DENY) and does **not** score â€” the escalation
        ...
        if score >= cfg.threshold:
            ...
            return cast(
                PolicyResponse,
                {
                    ...
                    "reason": (
                        f"{cfg.reason} Session risk score {score} â‰¥ threshold "
...
def risk_score_policy(
    *,
    ...
    :param sensitive_labels: Data-classification label â†’ points added when a
    ...
    :param initial_scores_by_actor: ``context.actor.run_as`` email â†’ starting
        ...
        all-users offset is intentionally omitted â€” it would be identical to
        lowering ``threshold``.)
