...
Python. Each callable follows the :class:`PolicyEvent` â†’
...
# policy verdict â€” but under its own names, distinct from the
...
# forwarder â€” live-verified against 1.17.7). opencode collapses write/edit/patch
...
# â”€â”€ OS tool approval â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def ask_on_os_tools(event: PolicyEvent) -> PolicyResponse:
    ...
    - **Claude Code native tools** (``Bash``, ``Read``, ``Write``,
      ``Edit``, ``Glob``, ``Grep``) â€” surfaced via the
      ``PreToolUse`` hook contract.
    - **Codex native tools** â€” uses the same ``PreToolUse`` hook
      contract with the same tool names (e.g. ``Bash``).
    - **Cursor SDK native tools** (``Shell``) â€” surfaced via the
    ...
    - **Pi native tools** (``read``, ``bash``, ``write``, ``edit``)
      â€” surfaced via the pi ``tool_call`` extension hook. Lowercase
      and distinct from the Claude/Codex casing.
    - **Hermes Agent tools** (``terminal``, ``execute_code``,
      ``read_file``, ``write_file``, ``search_files``) â€” surfaced
      via the ``pre_tool_call`` shell hook.
    - **opencode native tools** (``bash``, ``edit``, ``read``,
      ``grep``, ``glob``) â€” opencode's permission CATEGORIES, surfaced
      via the SSE forwarder's ``permission.asked`` â†’ policy-evaluate
...
# â”€â”€ Policy tool approval â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def ask_on_add_policy(event: PolicyEvent) -> PolicyResponse:
    ...
    every ``sys_add_policy`` call parks for approval â€” the user
...
# â”€â”€ Skill blocking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def block_skills(blocked: list[str]) -> PolicyCallable:
    ...
    1. **AP runner tools** â€” ``load_skill`` and ``read_skill_file`` tool
       calls dispatched by non-native (SDK) harnesses.
    2. **Native ``Skill`` tool** â€” Claude Code's built-in ``Skill`` tool,
       ...
       harnesses â€” there is no ``load_skill`` runner tool in native
       mode.
    3. **Slash commands** â€” ``/skill-name`` commands submitted by the
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        # â”€â”€ Path 1 & 2: tool_call interception â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if event_type == "tool_call":
            ...
            # Fired via PreToolUse hook â†’ agent-meow /policies/evaluate.
        ...
        # â”€â”€ Path 3: request phase for slash-command skill loads â”€â”€â”€â”€â”€â”€â”€
        ...
        if event_type == "request":
            ...
            if text.startswith("/"):
                ...
                # an empty list â€” guard against IndexError.
...
# â”€â”€ Sandbox enforcement â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# ignores it â€” prevents injection of unsupported fields.
...
def enforce_sandbox(
    ...
    """
    # Build the override dict â€” only include keys the admin explicitly set.
...
# â”€â”€ PII detection on LLM requests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def deny_pii_in_llm_request(
    ...
    Only fires on ``llm_request`` events â€” all other phases pass
    through with ALLOW.
    ...
    :param action: The verdict to emit on match â€” ``"DENY"`` or
    ...
    if pii_types:
        ...
        # None or empty list â†’ all categories enabled.
...
POLICY_REGISTRY: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": "Asks for user approval before any file or shell tool call â€” "
