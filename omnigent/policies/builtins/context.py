...
the goal is not fewer tokens *used*, but fewer tokens *wasted* â€”
...
# â”€â”€ detect_task_switch â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
Guidelines:
- CONTINUATION: the latest message follows naturally from prior work â€” a
...
- When in doubt, prefer CONTINUATION â€” false positives (blocking a legitimate
...
def detect_task_switch(
    *,
    ...
    recommending a fresh session â€” not compaction or summarisation.  The
    ...
    path only â€” the ``ASK`` path cannot write state on decline, so the
    ...
    .. note::
        ``action="DENY"`` is not a security control â€” user messages are
    ...
    if normalised_action not in {"DENY", "ASK"}:
        _log.warning(
            "detect_task_switch: unknown action %r â€” defaulting to ASK",
            action,
        )
    ...
    async def evaluate(event: PolicyEvent) -> PolicyResponse | None:
        ...
        # Not enough prior turns yet â€” record and pass through.
        if len(history) < min_turns:
            _log.debug(
                "detect_task_switch: history_len=%d < min_turns=%d â€” accumulating",
                len(history),
                min_turns,
            )
            return {
        ...
        # â”€â”€ Classify â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        llm_client = event.get("llm_client")
        if llm_client is None:
            _log.warning(
                "detect_task_switch: no llm_client â€” server has no llm: config. Abstaining."
            )
            return None
        ...
        except Exception:  # noqa: BLE001 â€” fail-open
        ...
        if verdict == "TASK_SWITCH":
            _log.info("detect_task_switch: TASK_SWITCH detected â€” action=%s", normalised_action)
            ...
            # applied if the user approves â€” on decline the window stays
        ...
        # Unrecognised verdict â€” fail open.
        return None
