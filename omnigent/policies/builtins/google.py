...
- :func:`gdrive_policy`   â€” Drive / Docs / Sheets / Slides files
  (read-allowlist, create gating, write-only-to-created, comments).
- :func:`gmail_policy`    â€” Gmail messages / drafts / send
  (default: read + draft, but **don't send** and don't modify).
- :func:`gcalendar_policy` â€” Calendar events
  (default: read-only).
...
across turns via the engine's persisted ``session_state`` (not closure state â€”
...
.. important::
    ...
    **Scope â€” these policies enforce ONLY at the Google MCP tool-call
...
# state-updates work â€” see ``engine._apply_one``). Public: they surface in the
...
# may re-emit (``document_id``) â€” the Databricks Google MCP filters create
...
# â”€â”€ Drive / Docs / Sheets / Slides tool sets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# Canonical-name prefixes the Drive policy owns (unrecognized â†’ fail closed).
_DRIVE_OWNED_PREFIXES: tuple[str, ...] = ("docs_", "drive_", "sheets_", "slides_")

# â”€â”€ Bell-LaPadula "no write-down" constants â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
#
# We model a simple two-level lattice â€” *confidential* vs. everything else â€” from
...
# the "simple security property" (no read-up) â€” the agent must be able to read
...
# â”€â”€ Gmail tool sets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Calendar tool sets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _normalize_file_refs(values: list[str] | None) -> set[str]:
    ...
    :param values: File IDs and/or Google URLs. ``None`` â†’ empty list.
...
def _extract_created_ids(  # type: ignore[explicit-any]
    ...
    more permitted IDs, so truncating is safe â€” fail closed).

    :param payload: Parsed ``tool_result`` payload â€” dict, list, or scalar,
        e.g. ``{"documentId": "1New"}``.
...
# â”€â”€ Google Drive / Docs / Sheets / Slides â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class _DriveCfg:
    ...
    :param write_down_action: Verdict on a write-down violation â€” ``"DENY"``
...
def _read_confidential_update(event: PolicyEvent, cfg: _DriveCfg) -> list[StateUpdateEntry]:
    ...
    if session_state.get(READ_CONFIDENTIAL_STATE_KEY):
        return []  # already latched â€” nothing to update
...
def _check_no_write_down(
    ...
    if not session_state.get(READ_CONFIDENTIAL_STATE_KEY):
        return None  # no confidential data read yet â€” no containment
...
def _decide_drive_tool_call(
    ...
    if canonical in _DRIVE_WRITE_TOOLS:
        ...
        # requires it to be created this session or in ``write_files`` â€” so a
...
def _record_drive_result(
    ...
    - Created-file tracking (:func:`_record_created_drive`) â€” always on.
    - Confidential-read latch (:func:`_read_confidential_update`) â€” only when
...
def gdrive_policy(
    *,
    ...
    :param confidential_files: File IDs / Google URLs that form the confidential
        ...
        confidentiality rule â€” the "*-property", i.e. **no write down** â€” on top
        ...
        listed files â€” writing to a confidential file still requires it to be
...
def gmail_policy(
    *,
    ...
    :param allow_send: Whether the agent may send mail. Default ``False`` â€” the
...
# â”€â”€ Google Calendar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Registry (one list, three entries â€” one per policy) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
POLICY_REGISTRY: list[dict[str, Any]] = [  # type: ignore[explicit-any]
    ...
    },
    {
        ...
        "description": (
            ...
            "read-only â€” allows reading events but blocks creating, updating, and "
            "deleting them."
        ),
        "params_schema": {
            "type": "object",
