...
- **Directory changes** (``block_cd``) â€” ``cd`` / ``chdir`` / ``pushd`` /
  ``popd`` shell built-ins, plus ``git -C <path> â€¦`` (which runs git as if in
...
- **Worktree switches** (``block_worktree``) â€” ``git worktree add`` /
  ...
  (``list`` / ``prune`` / ``lock`` / ``unlock`` / ``repair``) are not gated â€”
...
split on chaining operators (``cd /x && â€¦``), wrapper / env prefixes are
stripped (``sudo`` / ``env`` / ``VAR=x``), and shell-interpreter / ``eval``
wrappers (``bash -c "cd /x"``) are unwrapped and re-parsed â€” so the gate
...
# tree â€” i.e. that "switch" worktrees. ``list`` / ``prune`` / ``lock`` /
...
def _classify_cd(tokens: list[str]) -> _DirOp:
    ...
    :returns: A ``cd`` :class:`_DirOp`. ``target`` is the first non-flag
        ...
        directory (``popd``, bare ``cd`` â†’ home, ``cd -`` â†’ previous dir).
    """
    cmd = tokens[0]
...
def block_working_dir_changes(
    *,
    ...
    :param action: What a gated command yields â€” ``"deny"`` (block it) or
    ...
    :raises ValueError: If *action* is not ``"deny"`` / ``"ask"``, or if both
        *block_cd* and *block_worktree* are ``False`` (the policy could then
        never fire â€” fail loud at spec load rather than ship a no-op gate).
    """
