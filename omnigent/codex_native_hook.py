"""Codex Code hook entrypoint for native agent-meow policy enforcement.
...
the agent-meow policy schema is shared with the Claude-native hook via
...
def main(argv: list[str] | None = None) -> int:
    ...
    :returns: Process exit code. Always ``0`` â€” blocking verdicts are
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    ``decision: "block"`` for UserPromptSubmit â€” the request-phase gate
    ...
    yield a usable verdict â€” server unreachable, non-2xx, or an empty /
    malformed body â€” a ``PreToolUse`` (``PHASE_TOOL_CALL``) call fails
    CLOSED with a ``deny`` (this hook is the sole enforcement point for
    native tools), while ``UserPromptSubmit`` and ``PostToolUse`` fail
    OPEN. Conditions that mean the session simply is not governed â€” no
    ...
    are never blocked. The complementary fail-loud guard â€” asserting the
    hook is actually registered and trusted â€” lives at session startup in
    ...
    if config is None:
        # No agent-meow server configured for this session â€” nothing to enforce.
        return 0
    ...
    # against the user's CURRENT selection. Reading it here â€” synchronously,
    # the instant the tool call is gated â€” is race-free, unlike relying on the
    ...
    # model-switch surface (terminal /model only â€” no web picker).
