...
def map_step_to_events(
    ...
    *,
    ...
    ``external_output_text_delta``. The user turn IS mirrored here (see below) â€”
    nothing else commits it on the TUI-inject write path.

    Mapping:

    * ``CORTEX_STEP_TYPE_USER_INPUT`` â†’ one ``message`` item (role user)
      ...
      this commits exactly once. An empty user turn â†’ ``[]``.
