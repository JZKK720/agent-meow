...
``agent-meow sandbox â€¦`` bootstraps an agent-meow host inside a sandbox
from one of the registered providers (``--provider``) so that sessions
on it are reachable from the server-hosted UI, TUI, and ``agent-meow
resume``. Provider availability is build-dependent â€” the Databricks
Lakebox launcher ships only internally â€” so cli.py registers this group
...
def _omnigent_repo_root() -> Path:
    """
    Locate the agent-meow checkout that hosts the three packages
    we build wheels for (``sdks/python-client``, ``sdks/ui``, ``.``).

    Strategy:

    1. Walk up from the current working directory looking for a parent
       that contains both ``sdks/python-client`` and ``agent-meow``.
       This succeeds when the user runs ``agent-meow sandbox â€¦`` from
       inside a checkout.
    ...
    for candidate in candidates:
        if (candidate / "sdks" / "python-client").is_dir() and (candidate / "agent-meow").is_dir():
            return candidate
    raise click.ClickException(
        "Could not locate the agent-meow repo root from "
...
def _resolve_repo_root(repo_root: Path | None) -> Path:
    ...
    if not (resolved / "sdks" / "python-client").is_dir():
        raise click.ClickException(
            ...
            "point it at an agent-meow checkout."
        )
    return resolved
...
def _require_cli_bootstrap(launcher: SandboxLauncher) -> None:
    ...
    if not launcher.supports_cli_bootstrap:
        raise click.ClickException(
            ...
            "sessions only â€” create one with "
...
def _normalize_server_url(server_url: str) -> str:
    ...
    if not normalized.startswith(("http://", "https://")):
        raise click.ClickException(
            ...
            f"https://{normalized.lstrip('/')} â€” got {server_url!r}."
        )
    return normalized
...
def _print_ready_banner(provider: str, sandbox_id: str, server_url: str) -> None:
    ...
    click.echo(
        f"  agent-meow sandbox connect --provider {provider} --sandbox-id {sandbox_id} "
        f"--server {server_url}\n"
    )
...
def sandbox() -> None:
    ...
    Subcommands:
      ...
      connect  Register the sandbox as a host with your server (runs
               `agent-meow host` in the sandbox).

    \b
    Provider notes:
      modal    Sandboxes live at most 24 hours (platform cap). Needs
               `pip install 'agent-meow[modal]'` + `modal token new`.
      daytona  No lifetime cap (idle auto-stop is disabled). Needs
               `pip install 'agent-meow[daytona]'` + DAYTONA_API_KEY.
...
@click.option(
    ...
    # its slow provisioning + per-sandbox OAuth dance) â€” hidden from --help
...
@click.option(
    ...
    help=(
        ...
        "inference as `agent-meow login`), and the bootstrap finishes "
        "by logging the sandbox in to it (`agent-meow login` inside the "
        "sandbox â€” one browser step)."
    ),
)
@click.option(
    "--repo-root",
    ...
    default=None,
    help="Path to the agent-meow checkout.",
)
@click.option(
    "--no-auth",
    "skip_auth",
    is_flag=True,
    default=False,
    # Databricks-flow option â€” hidden from --help pending removal.
    hidden=True,
    help=(
...
def sandbox_create(
    ...
    unauthenticated probe ``agent-meow login`` uses), and for lakebox
    the sandbox is created IN that workspace â€” so the sandbox always
    ...
    sandbox in to the server (``agent-meow login`` runs inside the
    sandbox; the browser step is driven from this machine). Sandboxes
    are disposable â€” when your code changes, just create a new one.

    After this finishes, run ``agent-meow sandbox connect`` to register
    ...
    # the browser's callback port â€” others skip it automatically, no
...
# The auth flow is Databricks-specific (in-sandbox server login) â€”
...
@click.option(
    ...
    help=(
        ...
        "`agent-meow login` infers the fronting Databricks workspace "
...
def sandbox_auth(
    ...
    """
    Run the server login inside the sandbox (``agent-meow login``).
    ...
    ``agent-meow sandbox create --sandbox-id`` because it skips wheel
    build / ship / pip install â€” it only re-authenticates.
    """
...
def sandbox_connect(
    ...
    Runs ``agent-meow host --server <url>`` inside the sandbox â€” the
    host resolves its own credentials (a stored ``agent-meow login``
    ...
    WebSocket open until interrupted â€” Ctrl-C tears down the
    foreground transport and the remote process.

    Pass ``--host-name <label>`` when registering multiple sandboxes â€”
    ...
    # there) â€” the local `lakebox ssh` transport must resolve through
