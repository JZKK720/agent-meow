...
- :func:`startup_theme_picker` â€” interactive arrow-key menu shown on
...
# â”€â”€ Shared preview builders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _build_dark_preview(width: int = 52) -> Panel:
    ...
    body = Text.from_markup(
        ...
        f'  [{DARK_THEME.muted}]âµ read_file(path="src/main.py")[/{DARK_THEME.muted}]\n'
        f"  [{DARK_THEME.success}]âœ“ tool completed[/{DARK_THEME.success}]  "
        f"[{DARK_THEME.warning}]âš  retry[/{DARK_THEME.warning}]  "
        f"[{DARK_THEME.error}]âœ— error[/{DARK_THEME.error}]\n"
        f"  [{DARK_THEME.reasoning_style}]thinking step-by-stepâ€¦[/{DARK_THEME.reasoning_style}]"
    )
    return Panel(
        body,
...
def _build_light_preview(width: int = 52) -> Panel:
    ...
    body = Text.from_markup(
        ...
        f"Hello, I can help with that.\n"
        f'  [{LIGHT_THEME.muted}]âµ read_file(path="src/main.py")[/{LIGHT_THEME.muted}]\n'
        f"  [{LIGHT_THEME.success}]âœ“ tool completed[/{LIGHT_THEME.success}]  "
        f"[{LIGHT_THEME.warning}]âš  retry[/{LIGHT_THEME.warning}]  "
        f"[{LIGHT_THEME.error}]âœ— error[/{LIGHT_THEME.error}]\n"
        f"  [{LIGHT_THEME.reasoning_style}]thinking step-by-stepâ€¦[/{LIGHT_THEME.reasoning_style}]"
    )
    return Panel(
        body,
...
# â”€â”€ Startup picker rendering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _render_theme_picker(
    ...
    *,
    ...
    for i, (_name, label) in enumerate(_ITEMS):
        if i == selected:
            console.print(Text.from_markup(f"    [bold {_ACCENT}]â¯ {label}[/]"))
        else:
    ...
    console.print(
        Text.from_markup(
            f"  [{_MUTED}]â†‘/â†“ navigate  Â·  Enter confirm  Â·  Esc accept current[/{_MUTED}]"
        ),
    )
...
# â”€â”€ OSC 11 background detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _parse_osc11_response(response: str) -> Literal["dark", "light"] | None:
    ...
    rgb_start = idx + 4
    # Find end â€” terminated by ESC, BEL, or end of string.
    ...
    try:
        # Components can be 2 or 4 hex digits; normalize to 0â€“1 range.
        values = []
        for part in parts:
            part = part.strip()
            val = int(part, 16)
...
# â”€â”€ Startup picker (raw termios) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def startup_theme_picker(
    *,
    ...
    if not sys.stdin.isatty():
        # Non-interactive â€” use detected or default to light.
    ...
    try:
        ...
        # Can't enter raw mode â€” fall back to detected/default.
        theme = DARK_THEME if detected == "dark" else LIGHT_THEME
        update_user_config(theme=theme.name)
        return theme
...
# â”€â”€ REPL /theme integration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def build_theme_confirmation(
    theme: TerminalTheme,
) -> Text:
    """Build the ``â¯ <theme> mode (saved)`` confirmation line.

    :param theme: The theme that was just applied.
    :returns: A :class:`rich.text.Text` for ``host.output()``.
    """
    return Text.from_markup(
        f"  [{_ACCENT}]â¯[/{_ACCENT}] [bold]{theme.name}[/bold] [{_MUTED}]mode (saved)[/{_MUTED}]"
    )
