"""SSE-to-UI debug tooling â€” event tape, pipeline counters, and JSONL logger.
...
# ~400 bytes/entry â€” enough to hold a full multi-turn conversation
...
# â”€â”€ Stage enumeration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


class Stage:
    """Pipeline stage identifiers for :class:`TapeEntry`.

    Not an ``enum.Enum`` because we never iterate or reverse-lookup â€”
...
def _snapshot_event(event: object) -> dict[str, object] | None:
    ...
    1. Pydantic ``model_dump()`` â€” server-side SSE events are
       :class:`BaseModel` subclasses.
    2. :func:`dataclasses.asdict` â€” SDK events are dataclasses.
    3. ``vars()`` â€” fallback for plain objects with ``__dict__``.
    ...
    if model_dump is not None and callable(model_dump):
        ...
        except Exception:  # noqa: BLE001 â€” best-effort snapshot
            pass
...
# â”€â”€ TapeEntry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


@dataclass
class TapeEntry:
    """One event's journey through the SSE â†’ Block â†’ Render pipeline.
    ...
    :param stage_reached: Furthest :class:`Stage` this event reached,
        e.g. ``"rendered"``.
    :param path: Which REPL code path handled the event â€”
...
# â”€â”€ PipelineCounters â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ EventTape â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class EventTape:
    ...
    def record_raw(self, event: object, *, path: str = "sessions") -> TapeEntry:
        ...
        :param path: Which code path â€” ``"sessions"`` or
            ``"blockstream"``.
...
# â”€â”€ Overlay builders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# The Ctrl+E overlay uses the two-pane Overlay mode:
# * **Sidebar** (``targets_builder``): one row per tape entry â€” shows
...
def build_tape_targets(tape: EventTape) -> list[_OverlayTargetLike]:
    """Build the sidebar target list â€” one per tape entry.
...
def build_tape_detail(
    ...
    1. **Header** â€” event index, type, timestamp, delta.
    2. **Pipeline journey** â€” stage reached, translation result,
       formatter output, render status.
    3. **Raw JSON payload** â€” the event's fields as pretty-printed
       JSON, enabling full inspection of what the server sent.
    4. **Summary footer** â€” pipeline-wide counters.

    :param tape: The :class:`EventTape` holding buffered entries.
    :param target_key: The ``key`` from the selected
        :class:`OverlayTarget` â€” a stringified index into the
    ...
    # â”€â”€ 2. Pipeline journey â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    delta_str = f"+{entry.delta_ms:.1f}ms" if entry.delta_ms > 0 else "+0ms"
    gap_flag = "  âš  GAP" if entry.delta_ms >= _GAP_THRESHOLD_MS else ""
    ...
    if entry.sdk_translation is not None:
        ...
        parts.append(
            Text.from_markup(
                f"    {entry.raw_event_type} â†’ [{tx_color}]{entry.sdk_translation}[/{tx_color}]"
            )
        )
    else:
    ...
    # Rendered output â€” show what the TUI actually displays.
    ...
    if entry.formatted_items:
        ...
        if rendered_preview:
            for rl in rendered_preview.splitlines():
                parts.append(Text.from_markup(f"    [{muted}]â”‚[/{muted}] {_escape_markup(rl)}"))
        else:
        ...
        parts.append(
            Text.from_markup(
                f"    [{muted}](rendered via _render_history_item â€” "
        ...
        parts.append(Text.from_markup(f"    [red]âœ— not rendered â€” {reason}[/red]"))
    parts.append(Text(""))

    # â”€â”€ 3. Raw JSON payload â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    parts.append(Text.from_markup(f"  [{accent}]Raw Event Payload[/{accent}]"))
    if entry.raw_payload is not None:
        # Use generous limits for the detail panel â€” this is the
    ...
    # â”€â”€ 4. Summary footer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _render_formatted_items(items: list[object]) -> str:
    ...
    * ``StreamingText`` â†’ raw ``.text`` content (streaming delta).
    * ``StreamReplace`` / ``StreamLive`` â†’ render the inner
      ``.renderable`` through a Rich Console to produce the final
      visual output (markdown, panels, etc.).
    * Other ``RenderableType`` â†’ render through Rich Console.
    ...
    for item in items:
        # StreamingText: raw text delta â€” show as-is.
        ...
        try:
            ...
            if rendered.strip():
                lines.append(rendered)
        except Exception:  # noqa: BLE001 â€” best-effort preview
...
def _format_payload_detail(payload: dict[str, object]) -> str:
    ...
    this version allows more lines and wider values â€” the detail
...
class _OverlayTargetLike:
    ...
    Avoids importing the SDK at module scope â€” the real
    ...
    :param icon: Stage icon, e.g. ``"ðŸŸ¢"`` for rendered.
    """
...
def _stage_color(stage: str) -> str:
    ...
    :returns: A Rich color string â€” green for rendered, yellow for
...
def _stage_icon(stage: str) -> str:
    ...
    :param stage: One of the :class:`Stage` constants.
    :returns: A colored circle emoji â€” green for rendered, yellow
...
def _truncate_deep(obj: object, max_chars: int = _PAYLOAD_VALUE_MAX_CHARS) -> object:
    ...
    if isinstance(obj, str):
        return obj[:max_chars] + "â€¦" if len(obj) > max_chars else obj
...
# â”€â”€ Item summarizer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ JSONL logger â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
