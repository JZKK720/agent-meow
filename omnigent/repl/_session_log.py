"""
JSON dump of an agent-meow conversation â€” the agent-meow mode port of the
...
was tied to the in-memory :class:`Session` machinery â€” agent-meow
...
no compat â€” and it's a more honest dump of what's actually
persisted.
...
ancestor â€” shouldn't happen in practice) emits a stub node with
...
# Schema version â€” bump only on breaking shape changes. Readers MAY
...
def default_log_path(conversation_id: str, log_dir: Path | None = None) -> Path:
    ...
    with any path separators stripped â€” matching the legacy slug
...
async def write_session_log(
    ...
    *,
    ...
    in ``omnigent/repl/_repl.py`` â€” same parser the Ctrl+O debug
    ...
    :param client: A connected :class:`OmnigentClient`. The
        REPL's existing client is fine â€” this helper does not
...
async def _build_node_async(
    ...
    :param visited: Set of session ids already in the tree â€”
    ...
    if conversation_id in visited:
        # Already in the tree â€” break the cycle and emit a stub so
...
def _extract_child_conversation_ids(items: list[dict[str, object]]) -> list[str]:
    ...
    overlay so the parsing rules stay in one place â€” both shapes
    ...
    Same item, two equally-valid serializations â€” the walker
...
def write_session_log_from_store(
    ...
    *,
    ...
    Used by the one-shot ``agent-meow run <yaml> -p "â€¦" --log`` path
    where the in-process ASGI app doesn't have a connected
    :class:`OmnigentClient` â€” the run goes through raw httpx +
...
def _fetch_all_items_sync(conv_store: Any, conversation_id: str) -> list[dict[str, object]]:
    ...
    self-contained â€” a reader doesn't need any of agent-meow'
