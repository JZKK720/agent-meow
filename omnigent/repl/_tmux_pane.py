...
# :func:`register_pane` is a no-op â€” no pane options written, no
# split-window bindings wrapped â€” and the REPL behaves as if it
...
# wrappers in a running tmux server are not removed by this flag â€”
...
class SplitBinding:
    ...
    :param direction: One of ``'v'`` (vertical split), ``'h'``
        (horizontal split), or ``'w'`` (new window/tab) â€” the flag
    ...
    :param original_command: The full command string from the
        ...
        "#{pane_current_path}"'`` â€” re-emitted verbatim in the
...
def _resolve_omnigent_argv() -> list[str]:
    ...
    1. ``sys.argv[0]`` is path-shaped (contains ``/``) â€” abspath
       it. Covers ``./.venv/bin/agent-meow â€¦`` and absolute-path
       invocations.
    2. ``sys.argv[0]`` is a bare name like ``"agent-meow"`` â€” try
    ...
    3. Fallback â€” ``[sys.executable, "-m", "omnigent.cli"]``.
    ...
    # ``cli.py`` itself â€” not a directly callable executable. Don't
    # try to exec it as a binary; round-trip through python -m again
    # so the recursive case (split â†’ picker â†’ new REPL â†’ which
    ...
    if argv0 and ("/" in argv0 or os.sep in argv0):
        # Path-shaped â€” make absolute. ``abspath`` handles relative
...
def _user_args_after_launcher(launch_argv: list[str]) -> list[str]:
    ...
    - ``[<omnigent-binary>, run, â€¦]`` (PATH-resolved console script)
    - ``[<python>, -m, omnigent.cli, run, â€¦]`` (python-m fallback)
    - ``[<python>, -m, omnigent.cli, -m, omnigent.cli, run, â€¦]``
    ...
    :returns: The user's portion of the argv. Empty list when
        no recognized subcommand is found (degenerate input â€”
...
def _parse_bind_line(line: str) -> tuple[str, list[str]] | None:
    """
    Parse a tmux ``bind-key â€¦`` line into ``(key, command-tokens)``.
...
def _classify(cmd_tokens: list[str]) -> str | None:
    ...
    Only top-level bindings qualify â€” lambda blocks, chained
    ...
    if cmd != "split-window":
        return None
    # ``-h`` flag â†’ horizontal split (panes side by side).
    # Default and ``-v`` â†’ vertical split (panes stacked). Match
...
# user's original command from the false branch â€” without this,
...
def _unwrap_existing_wrapper(cmd_tokens: list[str]) -> list[str] | None:
    ...
    branch is the user's original (untouched) command â€” we
...
def _discover_split_bindings() -> list[SplitBinding]:
    ...
    for line in _list_prefix_keys():
        ...
        # place â€” the live regression that prompted this code path.
...
def _wrap_binding(binding: SplitBinding, omnigent_argv: list[str]) -> None:
    ...
    True branch (focused pane is an agent-meow â€” the
    ...
    format-string evaluation â€” no shell process is spawned for
    ...
    inner_argv = [
        *omnigent_argv,
        ...
        # shell token â€” tmux substitutes it before the shell
...
def register_pane(
    *,
    ...
    pane_id = _tmux_pane_id()
    if pane_id is None:
        _LOGGER.warning("$TMUX is set but $TMUX_PANE is not â€” skipping agent-meow pane integration")
        return
    ...
    # ``agent-meow`` lives â€” so a bare ``agent-meow`` exits 127.
    ...
    # ladder (abspath â†’ ``shutil.which`` â†’ ``python -m`` fallback).
    omnigent_argv = _resolve_omnigent_argv()

    # Normalize the launcher prefix in *launch_argv* â€” replacing
...
def update_conv_id(conv_id: str) -> None:
    ...
    Safe to call when not in tmux â€” degenerates to a no-op.
