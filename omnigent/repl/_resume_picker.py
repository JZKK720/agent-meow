...
- :func:`pick_conversation` â€” pure picker. Caller already has the
  conversation list (e.g. fetched via SDK or store).
- :func:`pick_conversation_from_sdk` â€” convenience that drives the
  SDK fetch + filter for the chat REPL path.

The picker writes to ``stderr`` rather than ``stdout`` so a parent
process piping the agent's stdout stream still gets clean output â€”
...
# Wrapper label sentinel â€” single source of truth in
...
_METADATA_SEPARATOR = " Â· "

# Role symbols shared with the rest of the REPL surface so the
# preview row reads as the same visual language as the chat.
# ``â¯`` mirrors :meth:`omnigent_ui_sdk.RichBlockFormatter.\
# user_message`'s prefix; ``â—†`` is the SDK formatter's default
# assistant glyph.
_USER_GLYPH = "â¯"
_ASSISTANT_GLYPH = "â—†"
...
class _Preview:
    ...
    :param role: ``"user"`` or ``"assistant"`` â€” drives the
    ...
    :param text: The latest message's plain-text content,
        ...
        :data:`_PREVIEW_DISPLAY_CHARS` with a trailing ``â€¦``
...
class _ConversationRow(Protocol):
    ...
    satisfy this without inheriting from it â€” the Protocol exists
    ...
    :param labels: Session-scoped labels (read for the Runtime
        metadata badge â€” the ``omnigent.wrapper`` key identifies
...
def pick_conversation(
    ...
    *,
    ...
    :param previews: Optional ``{conversation_id: _Preview | None}``
        map showing the latest message in each conversation â€”
    ...
    :param show_workspace: When ``True``, render workspace metadata
        ...
        the current cwd are flagged with a ``â†ªâ€¯cd`` suffix so the
...
def _append_prompt_toolkit_header(
    ...
    fragments.extend(
        [
            ...
            ("class:muted", f"  Â· page {page_no}/{page_count}, "),
    ...
    *,
    ...
    if show_workspace:
        ...
        if workspace is not None:
            ...
            if requires_cd:
                fragments.append(("class:accent", "  â†ª\xa0cd"))
    fragments.extend(
        [
            ("class:muted", _METADATA_SEPARATOR),
...
def _append_prompt_toolkit_preview(
    ...
    """
    if preview is None:
        fragments.append(("class:muted", "    â€¦\n"))
        return
...
async def pick_conversation_by_wrapper_label_from_sdk(
    ...
    *,
    ...
    record â€” agent-id filtering can't be used. List every session the
...
async def pick_conversation_cross_agent_from_sdk(
    ...
    *,
    ...
    previews = await _collect_previews_async(client, convos)
    # Header label is intentionally generic â€” "resume" describes the
...
def pick_conversation_from_store(
    ...
    # stays decoupled from the SqlAlchemy implementations â€” only
...
def _page_header_text(
    ...
    return Text.from_markup(
        f"[{_ACCENT}]Resume[/]  [{_MUTED}]Â·[/]  [bold]{_escape_for_markup(agent_name)}[/]"
        f"  [{_MUTED}]Â· page {page_no}/{page_count}, "
...
def _print_page_footer(console: Any) -> None:
    ...
    console.print(
        Text.from_markup(
            f"  [{_MUTED}]Keys:[/]  "
            f"[bold {_ACCENT}]â†‘[/]/[bold {_ACCENT}]â†“[/] move  "
            f"[{_MUTED}]Â·[/]  "
...
def _print_empty(agent_name: str, out: IO[str]) -> None:
    ...
    the substring ``"No prior conversations"`` â€” preserve it
    exactly.
...
def _print_invalid(out: IO[str]) -> None:
    ...
    Tests assert on the substring ``"Invalid selection."`` â€”
    preserve it exactly.
...
def _render_preview_cell(preview: _Preview | None) -> Any:
    ...
    role: ``â¯`` for user, ``â—†`` for assistant. Glyphs are styled
    in accent color; the text body is muted so the list item's
    title stays the eye's first stop. ``None`` (no preview
    available â€” fetch failed, conversation is empty, or it
    fell past the prefetch cap) renders as a single muted ``â€¦``
...
def _render_workspace_cell(row: _ConversationRow, *, current_cwd: Path) -> Any | None:
    ...
    *row.id* via the matching native state module.
    ...
    and appends a ``â†ª cd`` flag in accent color if it differs from
    *current_cwd* (resolved). When state is absent -- legacy session
    ...
    The flag uses ``â†ª`` ("return / leftwards arrow with hook") +
    ...
    return Text.from_markup(
        f"[{_MUTED}]{_escape_for_markup(working_directory)}[/]  [{_ACCENT}]â†ª\xa0cd[/]"
    )
...
async def _collect_previews_async(
    ...
    to stall while it warms up â€” the cap covers typical use
    ...
    async def fetch_one(conv: _ConversationRow) -> tuple[str, _Preview | None]:
        ...
        except Exception:  # noqa: BLE001 â€” preview is best-effort, swallow per-conv errors
            return conv.id, None
...
def _collect_previews_sync(
    ...
    Sequential rather than parallel â€” same ``conv_store`` instance
    ...
    latency, we move to a thread-pool fan-out â€” but only with
...
def _extract_text_from_content_blocks(content: Any) -> str:
    ...
    :data:`_PREVIEW_DISPLAY_CHARS` with a trailing ``â€¦`` when the
    original was longer. Image / file blocks are dropped â€” the
    ...
    # single tidy preview line â€” newlines and tabs become single
    ...
    if len(compact) <= _PREVIEW_DISPLAY_CHARS:
        return compact
    return compact[: _PREVIEW_DISPLAY_CHARS - 1] + "â€¦"
