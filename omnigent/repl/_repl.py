"""Rich-based REPL for agent-meow â€” built on the UI SDK framework.
    ...
    *,
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, ElicitationResolvedEvent):
            if approval_state.pending:
                ...
                host.output(
                    Text.from_markup(
                        f"   [{fmt.muted}]â€º resolved via approval page[/{fmt.muted}]",
                    ),
                )
            if tape_entry is not None:
                _maybe_log_tape_entry(tape_entry)
    ...
    async def on_input(text: str, attachments: list[PendingAttachment] | None = None) -> None:
        ...
        if field_input_state.pending:
            ...
            host.output(
                Text(f"   â€º {field_name}: {text}", style=fmt.muted),
            )
        ...
        if approval_state.pending:
            ...
            host.output(
                Text.from_markup(
                    f"   [{fmt.muted}]â€º {verdict_label}[/{fmt.muted}]",
                ),
            )
...
def _build_model_readout_lines(
    ...
    if cred is None:
        ...
        if model_override is not None:
            ...
            lines.append(
                "no model configured â€” run `agent-meow setup --no-internal-beta` to add one"
            )
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    :param client: agent-meow HTTP client â€” used to re-walk the
...
async def _build_debug_overview(
    ...
    *,
    ...
    # pair â€” matching the agent-meow event view. When an output
...
def _coerce_arguments_dict(raw: object) -> dict[str, object]:
    ...
    :returns: Parsed arguments dict, e.g. ``{"file_path": "/x.py"}``.
        ...
        the caller renders ``âµ name()`` instead of raising.
    """
