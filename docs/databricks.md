...
A production deployment guide for running agent-meow agents on Databricks
...
agent-meow's fine standalone with any OTLP backend and any LLM
...
> (Beta) is a fully managed service: Databricks operates the agent-meow
> server for you, already wired to workspace identity, Foundation
> Models, AI Gateway, and MLflow Tracing. You enable the **agent-meow**
...
This guide assumes you're new to both agent-meow and Databricks. Each
...
#1050](https://github.com/JZKK720/agent-meow/pull/1050)) to MLflow
...
![agent-meow on Databricks architecture](images/databricks/architecture.png)

The boundary stays sharp. agent-meow itself remains a standalone Apache
...
3. **Python 3.11+** locally with `uv` installed (the agent-meow project
   standard).
...
guide, this is the fastest path. It runs agent-meow locally (not on
Apps) but wires up Foundation Models + Gateway + tracing to the
workspace.

```bash
pip install 'agent-meow[tracing]' openai

export DATABRICKS_HOST=https://<your-workspace>.cloud.databricks.com
export DATABRICKS_TOKEN=<personal-access-token>

# Point agent-meow at Mosaic AI Foundation Models as the LLM provider
export OPENAI_BASE_URL=$DATABRICKS_HOST/serving-endpoints
export OPENAI_API_KEY=$DATABRICKS_TOKEN

# Point agent-meow's OTel exporter at the MLflow OTLP receiver in UC
...
Databricks workspace. agent-meow's server is an HTTP app, so it fits
...
The `deploy/databricks/` directory in the agent-meow repository contains
...
#   uv run python deploy/databricks/deploy.py --app-name agent-meow --profile <profile> ...
...
agent-meow's harnesses (`claude_sdk`, `openai_agents_sdk`, `pi`, and
...
Then any agent-meow agent spec that points at a `databricks-` model
...
agent-meow doesn't need to know it's calling a Gateway. The Gateway
endpoint speaks the OpenAI API, so the same `OPENAI_BASE_URL` knob
that points agent-meow at Foundation Models points it at the Gateway.
...
The OpenAI-style chat completions URL (what agent-meow uses) is:
...
### Point agent-meow at the Gateway endpoint
...
Then in the agent-meow agent spec, use the Gateway endpoint name as the
model:
...
then called it via the same OpenAI SDK pattern agent-meow uses. The
...
resp = client.chat.completions.create(
    model='agent-meow-docs-test-gateway',   # the Gateway endpoint, not the backing model
...
(`agent-meow-docs-test-gateway`) is what agent-meow calls. Swapping the
...
| Claude Max / ChatGPT Plus / Cursor Pro (OAuth subscription) | No | agent-meow session metadata only; LLM calls invisible | No — billed to the individual's consumer account |
...
| Capability | Raw provider call from agent-meow | Through AI Gateway |
|---|---|---|
...
agent-meow emits OpenTelemetry spans for every agent turn, LLM call, and
...
in [PR #1050](https://github.com/JZKK720/agent-meow/pull/1050). Any
...
agent-meow's `telemetry.init()` auto-detects the OTLP endpoint and wires
...
`mlflow.start_span` API agent-meow's `TracingContext` wraps) against the
...
Experiment:   id=3163592711242134 path=/Users/.../agent-meow-databricks-docs-verification
...
agent-meow does not capture message bodies into traces by default. Set:
...
The four sections above cover the V1 integration. The agent-meow +
Databricks story extends further. Planned follow-ups, each as a
separate PR:

- **Unity Catalog functions as agent tools.** A `databricks-tools`
  optional extra in agent-meow that exposes UC functions as
...
- **Mosaic AI Agent Evaluation integration.** Sample production
  traces from agent-meow for the managed Mosaic AI Agent Evaluation
  pipeline.
...
Track these on the [agent-meow issues
list](https://github.com/JZKK720/agent-meow/issues) under the
`databricks` label.
...
fresh Lakebase project per agent-meow app rather than sharing one.
...
observability series shipped in PRs [#1050](https://github.com/JZKK720/agent-meow/pull/1050),
[#1068](https://github.com/JZKK720/agent-meow/pull/1068),
[#1070](https://github.com/JZKK720/agent-meow/pull/1070),
[#1071](https://github.com/JZKK720/agent-meow/pull/1071),
[#1072](https://github.com/JZKK720/agent-meow/pull/1072), and
[#1083](https://github.com/JZKK720/agent-meow/pull/1083).
...
- External Model (Gateway) endpoint created, called, and torn down:
  `agent-meow-docs-test-gateway` proxied to `databricks-claude-sonnet-4`
...
ping @debu-sinha on the agent-meow Slack channel.
