...
```bash
meow server --config server_config.yaml
```
...
policies:
  ...
  github_access:
    type: function
    handler: agent_meow.policies.builtins.github.github_policy
    factory_params:
      write_repos:
        - myorg/my-repo
      write_branches:
...
daily_budget:
  type: function
  handler: agent_meow.policies.builtins.cost.user_daily_cost_budget
  factory_params:
    max_cost_usd: 25.00
...
policies:
  deny_trivial_opus:
    type: function
    handler: agent_meow.policies.builtins.routing.deny_trivial_to_expensive_model
    factory_params:
      expensive_models:
