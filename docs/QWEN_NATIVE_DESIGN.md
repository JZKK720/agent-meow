...
that embeds the **live interactive `qwen` TUI** in the agent-meow web UI, instead of
...
This lets agent-meow inject turns atomically *and* recover two things the other
...
```mermaid
flowchart TD
    subgraph CLI["agent-meow qwen (CLI wrapper)"]
      W[qwen_native.py<br/>daemon bind · terminal-ready poll · tmux attach]
    end

    subgraph Runner["agent-meow Runner"]
...
```
   ...
   └──────────────────────────────────────────────────────────────────┘
            ...
            │
   ┌──────────────────────────────────────────────────────────────────┐
   │   agent-meow qwen  (CLI wrapper, qwen_native.py)                     │
...
- **Control** — instead of racy `tmux send-keys`, agent-meow writes JSONL to the **IN
...
| **Tool permission gating** | **surrendered to vendor** | **agent-meow gates** via `can_use_tool` → policy → `confirmation_response` |
...
- **Permission gating works** — the headline win. `can_use_tool` control requests
  let agent-meow run its TOOL_CALL policy + human elicitation and answer with
...
| Still needs tmux for display | **Inherent** | Any *embedded live terminal* needs a real terminal — unavoidable for the native-TUI goal. Upside: because we already parse the full JSON event stream, a future **pure-web rendering mode** (agent-meow renders qwen with no tmux) becomes possible as a separate option — but that is explicitly out of scope here. |

## Files to add (mirrors goose-native; bridge is file-based, not send-keys)

| New file | Role |
|---|---|
| `agent_meow/inner/qwen_native_executor.py` | `submit` via input-file; `supports_streaming=False`, `supports_live_message_queue=True` |
| `agent_meow/inner/qwen_native_harness.py` | `create_app()` factory |
| `agent_meow/qwen_native_bridge.py` | input-file append (`submit` / `confirmation_response`), tmux.json, `inject_interrupt` (Escape), `kill_session`, `build_qwen_native_spawn_env` |
| `agent_meow/qwen_native_forwarder.py` | tail `--json-file`, mirror transcript, drive the permission gate |
| `agent_meow/qwen_native.py` | `agent-meow qwen` wrapper (clone `goose_native.py`) |

## Registration touch-points (one-liners, beside the goose entries)

- `agent_meow/runtime/harnesses/__init__.py` → `"qwen-native": "agent_meow.inner.qwen_native_harness"`
- `agent_meow/harness_aliases.py` → add `qwen-native` + `native-qwen` to `NATIVE_HARNESSES`
- `agent_meow/native_coding_agents.py` + `agent_meow/_wrapper_labels.py` → `QWEN_NATIVE_*`
- `agent_meow/onboarding/harness_install.py` → `_HARNESS_NAME_TO_KEY` → existing `QWEN_KEY`
- `agent_meow/runner/app.py` → `_auto_create_qwen_terminal` + the ~7 goose-native dispatch sites
- `agent_meow/cli.py` → `agent-meow qwen` command
...
- **Persisted id (the convention).** The qwen session id is recorded on the
  agent-meow session via `external_session_id` (`_persist_qwen_external_session_id`
  → `PATCH /v1/sessions/{id}`), read back from the snapshot on the next launch
  (`launch_config.external_session_id`), and stamped as
  `agent_meow.fork.source_external_session_id` so a fork carries history — exactly
