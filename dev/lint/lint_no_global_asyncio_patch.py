...
When test code writes::
    ...
    with patch("agent_meow.tools.mcp.asyncio.sleep", new_callable=AsyncMock):
        ...

``unittest.mock``'s dotted-path resolver walks:

1. ``import agent_meow.tools.mcp``
2. ``getattr(agent_meow.tools.mcp, "asyncio")`` -> returns the
...
def _attribute_ends_in_asyncio(node: ast.expr) -> bool:
    ...
    e.g. matches ``mcp_mod.asyncio`` and ``agent_meow.tools.mcp.asyncio``.
    """
