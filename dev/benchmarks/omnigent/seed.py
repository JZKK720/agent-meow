...
# Allow ``uv run <path>`` (no package context) to import agent_meow + siblings.
...
# FTS5 mirror row written per item on SQLite (must match agent_meow.db.utils
...
# (a few thousand rows) and inserted in one shot each. 100k keeps a 5000Ã—200
...
def seed(
    ...
    *,
    ...
    """Seed *sessions* sessions Ã— *items_per_session* items into *db_uri*.
    ...
    print(f"seed: created {n} sessions Ã— {items_per_session} items ({want})")
    return n
