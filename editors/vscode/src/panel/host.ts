/**
 * Shared render helper for the agent-meow editor panel.
 *
 * The agent-meow UI renders in the editor-beside `WebviewPanel`
...
/** Render the agent-meow iframe host into a webview. Sets `webview.html`. */
export function renderInto(webview: vscode.Webview, opts: RenderIntoOptions): void {
  ...
  opts.log?.(
    `[agent-meow] iframe rendered (origin=${opts.target.origin}, nonce=${nonce.slice(0, 8)}...)`,
  );
}
...
<body>
  <p>Resolving agent-meow server…</p>
</body>
</html>`;
}
