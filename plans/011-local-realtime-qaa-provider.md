...
**Date:** 2026-08-06  
**Status:** Draft  
**Effort:** L (large — new provider + model integration)  
**Risk:** Medium — requires patching QAA npm package, running 3 local models
...
**The standalone WebSocket server approach is simpler** — it doesn't require patching QAA's provider registry, just changing `SPEECH_TO_SPEECH_REALTIME_URL` in config.env.

### Step 5: Register the provider in QAA (if using provider file approach)

If using the provider file approach (Step 4 option 1):

1. Add to `realtime-provider-catalog.mjs`:
```js
const PROVIDERS = Object.freeze({
  dashscope: ...,
  'speech-to-speech': ...,
  'local-realtime': Object.freeze({
    key: 'local-realtime',
    label: 'Local Realtime (Qwen3-ASR + Hermes + Qwen3-TTS)',
    aliases: Object.freeze(['local']),
  }),
})
```

2. Add to `registry.mjs`:
```js
import { localRealtimeProvider } from './local-realtime.mjs'
const providers = new Map([
  ['dashscope', dashscopeProvider],
  ['speech-to-speech', s2sProvider],
  ['local-realtime', localRealtimeProvider],
])
```

3. Set in config.env:
```
QWEN_AUDIO_REALTIME_PROVIDER=local-realtime
```
...
3. QAA's existing `speech-to-speech` provider now connects to our local-realtime server instead of S2S.

This is simpler because:
- No QAA provider registry patching needed
- QAA sees it as just another S2S endpoint
- The VoicePanel already shows the S2S provider button
- We can keep the old S2S server as a fallback
...
| File | Change |
|---|---|
| `scripts/start-voice-stack.ps1` | Add startup for local-realtime servers |
| `~/.config/qwaudio/config.env` | Change S2S URL to local-realtime server |
...
- If Qwen3-TTS is too slow on first load (>30s), fall back to Kokoro for TTS
...
- If the OpenAI Realtime API protocol is too complex to implement in the orchestrator, use the existing S2S server with `--stt qwen3-asr --tts qwen3-tts` flags (if S2S supports them)
...
- Hermes ACP connection stays the same — the LLM path is unchanged from current S2S setup
