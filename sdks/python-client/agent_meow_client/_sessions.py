...
``Session`` class exported from :mod:`agent_meow_client._session` is
...
this module's ``Session`` from :mod:`agent_meow_client.__init__` —
...
_log = logging.getLogger("agent_meow_client.sessions")
...
class Session:
    ...
    Note: distinct from :class:`agent_meow_client._session.Session`
    (re-exported as ``agent_meow_client.Session``), which is a
...
class SessionsNamespace:
    ...
    async def subtree_busy(
        ...
        *,
        ...
        :func:`agent_meow_client.child_summary_busy` predicate —the same "busy"
