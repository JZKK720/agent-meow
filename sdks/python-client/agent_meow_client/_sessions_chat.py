...
A higher-level wrapper over :class:`agent_meow_client._sessions.SessionsNamespace`
that mirrors enough of :class:`agent_meow_client._session.Session`'s
...
:class:`agent_meow_client._session.Session` synthesizes a multi-turn
...
:class:`agent_meow_client._session.Session` and renaming would break
...
from ._errors import AgentMeowError
...
class SessionToolCallInfo:
    ...
    :class:`agent_meow_client._tool_handler.ToolCallInfo` — that
...
# :mod:`agent_meow_client._session`).
...
class SessionsChat:
    ...
    Usage mirrors :class:`agent_meow_client._session.Session` for
    ...
    session is an HTTP call. Use :meth:`AgentMeowClient.sessions_chat`
    ...
    :param namespace: The :class:`SessionsNamespace` this helper
        ...
        :class:`AgentMeowClient`; the chat helper does not own
    ...
    async def refresh(self) -> Session:
        ...
        :raises AgentMeowError: If the session no longer exists
    ...
    async def send(
        ...
        *,
        ...
        :raises AgentMeowError: If the underlying ``post_event`` or
        ...
        try:
            ...
            while True:
                ...
                if isinstance(event, SessionStatusEvent) and event.status == "failed":
                    ...
                    raise AgentMeowError(message, code=code)
                try:
        ...
        * a key in ``tool_callables`` that does not name a
        ...
        if self._tool_callables and self._agent_tools_getter is None:
            raise RuntimeError(
                ...
                "AgentMeowClient.sessions_chat which wires it "
    ...
    async def cancel(self) -> None:
        ...
        :raises AgentMeowError: If the session does not exist
            (404).
        """
    ...
    async def query(
        ...
        *,
        ...
        :raises AgentMeowError: If the underlying HTTP calls fail.
    ...
    async def await_turn(self, *, timeout: float | None = 1200.0) -> QueryResult:
        ...
        :raises AgentMeowError: Propagated from :meth:`stream` if the
    ...
    async def _build_content(
        ...
        :meth:`agent_meow_client._session.Session._build_input_with_files`
...
class _FilesUploader(Protocol):
    ...
    :meth:`agent_meow_client._files.FilesNamespace.upload`. Defined
...
class _FilesGetter(Protocol):
    ...
    Matches the signature of
    :meth:`agent_meow_client._files.FilesNamespace.get`.
    """
