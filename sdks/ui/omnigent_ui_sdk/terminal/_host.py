...
# boilerplate on ~20 functions 鈥?disabling ARG001 file-wide is the right
# granularity because the genuine "dead arg" risk this rule catches does not
# apply when the signature is externally mandated.
"""TerminalHost 鈥?manages terminal I/O with a pinned input bar.
...
# Eight steps from empty circle to full circle 鈥?same Unicode Geometric Shapes
# block characters used by the ContextUsageBar widget in tests/scripts/tui.py.
_RING_CHARS = ("鈼?, "鈼?, "鈼?, "鈼?, "鈼?, "鈼?, "鈼?, "鈼?, "鈼?)
...
# from the inline ``鈫揱` menu (sentinel + chosen session id). ``host.run``
# decodes the id and invokes ``on_subagent_select`` between prompt
# iterations 鈥?the safe context for switching sessions + re-rendering.
_SUBAGENT_SELECT_SENTINEL: str = "\x00__omnigent_ui_sdk.subagent_select__\x00"

# Braille-dot spinner frames for the "thinking鈥? indicator and
...
_SPINNER_FRAMES: tuple[str, ...] = ("鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?)
# Spinner tick interval while streaming 鈥?100 ms is fast enough
...
# finished sub-agents stay in the ``鈫揱` selector indefinitely (web parity 鈥?
...
# Max rows shown at once in the inline ``鈫揱` sub-agents menu. Longer
...
class _SubagentNode:
    ...
    agent: str | None = None
    title: str | None = None
    # ``current_task_status``: launching / in_progress / completed / failed / 鈥?
    # Running-ness is tracked via ``done_at`` (terminal-vs-not), not a live
    # ``busy`` flag 鈥?see :meth:`TerminalHost.active_subagent_count`.
    ...
    # Whether the child session is closed to new user input 鈥?derived from its
...
class OverlayAction:
    ...
    sidebar target 鈥?e.g. "press ``o`` to attach to this
    ...
    ``trigger`` 鈥?collisions raise at ``add_overlay`` time.
    ...
    :param label: Short label rendered in the footer hint,
        ...
        terse 鈥?the footer is one line.
...
class OverlayTarget:
    ...
    :param key: Stable identifier, e.g. ``"main"`` or
        ``"conv_abc123"``. Opaque to the host 鈥?the builder uses
    ...
    :param icon: Optional single glyph prefix for the sidebar
        row, e.g. ``"馃"`` for the main target, ``"馃懢"`` for
...
class Overlay:
    ...
    The host does not know anything about the overlay's meaning 鈥?
    ...
    the one-correct-path rule) 鈥?it's an explicit API choice:
    ...
    * **Sidebar + content** (``targets_builder`` is supplied):
    ...
    :param builder: Async callable invoked each time the overlay
        ...
        :func:`rich.text.Text.from_markup` per line 鈥?NOT a
        ...
        idiom every shipped overlay builder uses 鈥?match it so
    ...
    :param sidebar_width: Column width (in characters) for the
        ...
        鈥?matches the omnigent debug panel and fits the
...
# rendering (e.g. ``"c-c"`` 鈫?``"^C"``) 鈥?the rule for everything
...
def _compute_sidebar_scroll_offset(
    ...
    *selected_index* inside the visible window.
    ...
    - Selection above the viewport (``selected_index <
      current_offset``) 鈫?scroll up so the selection lands on
      the first visible row.
    - Selection past the viewport (``selected_index >=
      current_offset + visible_height``) 鈫?scroll down so the
      selection lands on the last visible row.
    - Selection already inside the window 鈫?no change
    ...
    the offset back to 0 鈥?exactly what the user expects after
...
def _abbreviate_key(key: str) -> str:
    ...
    Keeps the rendering consistent 鈥?``"escape"`` always shows as
...
def _build_close_hint(overlay: Overlay) -> str:
    ...
    *trigger* with consistent short-form abbreviations.
    ...
    鈥?keys joined with ``/`` and ``"close"`` appended as the
    affordance label.
...
def _extract_file_paths(text: str) -> list[PendingAttachment]:
    ...
    Only checks whitespace-separated tokens 鈥?no shell parsing
...
# UNDER this ceiling 鈥?anything beyond can't be reached because the
...
# harmless (same key 鈫?same value) but it's wasteful and
...
def _install_csi_u_sequences() -> None:
    ...
    The Kitty Keyboard Protocol 鈥?supported by Kitty, WezTerm,
    Ghostty, recent xterm, and iTerm2 with "Report modifiers
    using CSI u" enabled 鈥?encodes keystrokes like Ctrl+C as
    ...
    3. Special keys (Escape, Backspace, Delete, ``Ctrl+M``,
       Shift+Enter 鈫?``F20``, focus-in/out markers).

    Idempotent 鈥?guarded by :data:`_CSI_U_INSTALLED` so
    ...
    The user just doesn't get the protocol awareness 鈥?same
    ...
    try:
        ...
        # Prompt-toolkit's API changed under us 鈥?degrade to
        # no-op so the host still constructs.
        _CSI_U_INSTALLED = True
        return

    # Shift+Enter 鈫?F20. Legacy maps F20 to "insert newline" so
    ...
    # for the F20 key event to actually do something 鈥?without
    ...
    # Modified Backspace under the Kitty keyboard protocol 鈫?delete the
    ...
    ANSI_SEQUENCES["\x1b[127;3u"] = Keys.ControlW  # Option/Alt+Backspace 鈫?word delete
    ANSI_SEQUENCES["\x1b[127;5u"] = Keys.ControlW  # Ctrl+Backspace 鈫?word delete
    ...
    # Option/Alt+Enter and Ctrl+Enter 鈫?insert a newline. F20 is the key the
    ...
    ANSI_SEQUENCES["\x1b[13;3u"] = Keys.F20  # Option/Alt+Enter 鈫?newline
    ANSI_SEQUENCES["\x1b[13;5u"] = Keys.F20  # Ctrl+Enter 鈫?newline
    ANSI_SEQUENCES["\x1b[9u"] = Keys.ControlI  # plain Tab via CSI-u
    # Shift+Tab 鈫?back-tab. Unregistered it leaked "[9;2u"; the overlay
    # navigation already uses BackTab, so decode it consistently here.
    ANSI_SEQUENCES["\x1b[9;2u"] = Keys.BackTab  # Shift+Tab 鈫?back-tab
...
class TerminalHost:
    ...
    :param completer: Optional prompt-toolkit
        ...
        The host stays generic 鈥?callers decide what to complete.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # recent xterm) work 鈥?without this, Ctrl+letter
        ...
        # Toolbar label. ``None`` = model not known yet 鈥?the
        ...
        # Toolbar hint list. Joined with `` 路 `` separators in
        # ``build_toolbar``. Callers should pass the same list
        # they hand to ``welcome(hints=...)`` so the bar's
        # hints match the welcome panel 鈥?drift here means
        # users see "/help help 路 Ctrl+O debug 路 ..." up top
        # but only "esc cancel 路 ctrl+c exit" at the bottom.
        ...
        # ``鈫揱` menu). Keyed by child session id; the tree is reconstructed
        ...
        # Inline ``鈫揱` sub-agents menu state. Open is toggled by Down (on an
        ...
        # Stored as ``None`` when no confirmation is pending 鈥?the
        ...
        # on each :meth:`prompt_async` call 鈥?it reuses the same
        # ``KeyBindings`` object 鈥?so mutating this set between
        ...
        def _on_escape(event: object) -> None:
            # Esc first dismisses the inline 鈫?sub-agents menu if it's open,
        ...
        # Multi-line input bindings 鈥?multiline=True is set on
        ...
        #   ``Esc`` then ``Enter``).
        # - ``c-j`` (Ctrl+J) works in every terminal 鈥?Ctrl+J
        ...
        def _accept_history_search(event: KeyPressEvent) -> None:
            ...
            # input buffer 鈥?don't submit the prompt.
        ...
        def _on_enter(event: KeyPressEvent) -> None:
            # When the inline 鈫?menu is open, Enter picks the highlighted
        ...
        # 鈹�鈹� Inline 鈫?sub-agents menu navigation 鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�
        ...
        def _can_open_subagent_menu() -> bool:
            # Openable whenever ANY sub-agent node exists 鈥?active OR finished.
        ...
        # 鈹�鈹� Left-arrow: back to the top-level session 鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�
        # While viewing a sub-agent, Left (on an empty input) jumps straight
        # back to the top-level "main" session 鈥?available any time you're
        ...
        #
        # - Input buffer has text 鈫?clear the buffer (reset the
        ...
        # - Input is empty AND no exit hint pending 鈫?arm the
        #   hint: set the deadline, invalidate the app so the
        #   toolbar rerenders with "Press Ctrl+C again to exit".
        # - Input is empty AND hint still in its window 鈫?raise
        ...
        def _on_ctrl_c(event: KeyPressEvent) -> None:
            ...
            if deadline is not None and now < deadline:
                # Second press within the window 鈥?exit. Use
                ...
                return
            # Empty input, no pending hint (or expired) 鈥?arm one.
        ...
        # No keyboard binding for ``on_help`` 鈥?users invoke it
        ...
        #   layer 鈥?binding ``c-h`` causes ``on_help`` to fire on
        ...
        # below 鈥?there's only one ``Output`` per host.
        self._output = _output
        self._window_title = window_title

        # ``completer=None`` and ``complete_while_typing=True`` are
        # both prompt-toolkit defaults: no completer 鈫?no popup; if
        ...
        # children 鈥?the buffer window absorbs it all, pushing the
        ...
        if isinstance(_root, HSplit):
            # (1) TOP alignment 鈥?remaining space goes to padding,
            #     not to the buffer.
            _root.align = VerticalAlign.TOP

            # (2) Find and cap the buffer Window height.
            #     Use get_children() 鈥?the standard Container API 鈥?
            ...
            _sep = Window(
                FormattedTextControl(lambda: FormattedText([("class:bar", "鈹�" * _term_width())])),
                height=1,
            ...
            # Inline 鈫?sub-agents menu, BELOW the toolbar so the navigable
            # list sits beneath the whole chat interface. Shown only while
            # the menu is open with rows; sized to its content (鈮?row cap +
        ...
        return PTStyle.from_dict(
            {
                ...
                # Settled (done / warm-idle) sub-agent rows in the 鈫?selector
                # are de-emphasized vs live ones 鈥?mirroring the web Agents
                # rail's SETTLED_STATE dimming 鈥?but kept in the list.
                ...
                # the magenta ``鈹�鈹�鈹�鈹�`` accent fragments into magenta
    ...
    def set_model_name(self, model_name: str) -> None:
        ...
        Used when the session's bound agent changes mid-run 鈥?e.g. an
        in-place agent switch made from another client 鈥?so the bottom
    ...
    def add_overlay(self, overlay: Overlay) -> None:
        ...
        Triggers are unique per host 鈥?registering two overlays
        ...
        # binding silently does the wrong thing 鈥?a close key
    ...
    def _try_set_window_title(self) -> None:
        ...
        if self._window_title is None:
            return
        # Broad swallow matches the legacy CLI's bare ``except`` 鈥?
    ...
    def _try_clear_window_title(self) -> None:
        ...
        effort for the same reason 鈥?a terminal that ignored the
    ...
    async def _drain_pending_handlers(self) -> None:
        ...
        ``prompt_async`` raises ``EOFError`` 鈥?which prompt_toolkit
    ...
    async def __aexit__(self, *exc: object) -> None:
        # Restore stderr first 鈥?everything below (drain, cancel)
        ...
        # the drain 鈥?cancel fast so the error surfaces without
    ...
    def _handle_paste_text(self, raw: str) -> str:
        ...
        :param raw: Bracketed-paste payload from prompt-toolkit,
            e.g. ``"line1\\r\\nline2\\r\\n"``.
        :returns: Text to insert 鈥?normalized paste, placeholder
    ...
    def _expand_pasted_blocks(self, text: str) -> str:
        ...
        content is dropped 鈥?safer than guessing the splice point.
    ...
    async def run(self, handler: Callable[..., Awaitable[None]]) -> None:
        ...
        async def _toolbar_ticker() -> None:
            ...
            - **Streaming**: tick every
              ...
              the Braille spinner in the ``鉅?working`` line
              and the ``state: running 鉅筦` badge animate
              smoothly.
            ...
            - **Sub-agents active**: same cadence, so the
              ``state: N agents running 鉅筦` badge keeps animating
            ...
            - **Idle**: 500 ms 鈥?nothing to animate, just keep
        ...
        try:
            ...
            # never knows to send CSI-u encoded sequences 鈥?e.g.
            ...
            with patch_stdout(raw=True):
                while True:
                    ...
                    # Overlay trigger 鈥?``add_overlay`` wired the
                    ...
                    # the main prompt 鈥?otherwise Ctrl+C inside
                    ...
                    # Inline 鈫?sub-agents menu selection 鈥?Enter on a menu
            ...
            # Pop Kitty Keyboard Protocol 鈥?restore previous mode.
    ...
    async def _show_overlay(self, overlay: Overlay) -> None:
        ...
        an alternate layout. A standalone fullscreen app 鈥?run
        sequentially, not concurrently 鈥?keeps the two layouts
        ...
        # ``None`` OR returns empty, we render single-pane mode 鈥?
        ...
        # invisibly walks off the bottom of the viewport 鈥?exactly
        ...
        # content pane. "top" 鈥?row 0 of the visible window (less
        # pager default, matches how all generic scroll bindings
        # leave the cursor). "bottom" 鈥?the row that renders the
        ...
        # Search state 鈥?vim / less-style ``/`` incremental
        ...
        # ``invalidate`` when nothing changed 鈥?that's what kills
        ...
        async def _rebuild_content(
            *,
            ...
            2. Computes a content signature and early-returns
               (no ``invalidate`` 鈫?no repaint) when it matches
            ...
            3. Only resets scroll when the target actually changed
               ...
               ``reset_scroll=True`` 鈥?the default for Tab).
            ...
            # Drop stale result 鈥?a newer rebuild has started
        ...
        async def _refresh_loop() -> None:
            ...
            500 ms cadence 鈥?tight enough that users see turn
            ...
            with the interval bumped from 50 ms 鈫?500 ms because
        ...
        def _highlight_matches(line: str, needle: str) -> str:
            ...
            *line* with a yellow-background + black-foreground
            ANSI span (``\\x1b[30;43m`` 鈥?``\\x1b[39;49m``).
            ...
            and background (``\\x1b[49m``) independently 鈥?using
            ...
            :param line: One content line 鈥?may already contain
        ...
        def _get_visible_ansi() -> ANSI:
            ...
            reverse-video 鈥?the "yellow highlight" users expect
        ...
        def _get_cursor_position() -> Point:
            ...
            The default position (0, 0) 鈥?i.e. the top-left of
            the currently visible slice 鈥?is what prompt-toolkit
            ...
            and the total content length 鈥?we can't hardcode a
            ...
            # pane 鈥?capped by view_height when content overflows
        ...
        def _sidebar_visible_height() -> int:
            ...
            below 鈥?count them in lock-step:
        ...
        def _ensure_selection_visible() -> None:
            ...
            selection into view 鈥?selection above viewport scrolls
        ...
        def _get_sidebar() -> ANSI:
            ...
            The selected row gets a ``鈻竊` prefix + bold style; the
            ...
            list longer than the terminal 鈥?without this, lists
            ...
            # eats one of the visible slots 鈥?acceptable
            ...
            for window_idx, t in enumerate(window):
                ...
                # actual rendering will misalign rows by 卤1
                # column. wcswidth is the source of truth for
                # both this padding AND prompt-toolkit's
                # :class:`Window` layer that contains us 鈥?so
                ...
                # Wide emoji like ``馃`` / ``馃懢`` / ``馃悮``;
                # AVOID Neutral-classified ones like ``馃枼`` /
                # ``鈱╜` that wcswidth says is 1 cell but
        ...
        # user is typing a search 鈥?no separate footer Window,
        ...
        # search 鈥?close to the close hint so the eye picks them
        # up alongside other "things you can press right now."
        # When no actions are registered the segment collapses
        # away entirely.
        action_hint = (
            " 路 ".join(f"{a.key} {a.label}" for a in overlay.actions)
        ...
        if has_sidebar:
            ...
            idle_footer = " 路 ".join(segments)
        else:
            idle_footer = "鈫戔啌 gg/G scroll 路 pgup/pgdn page 路 / search 路 " + close_hint
        ...
        def _find_matches(query: str) -> list[int]:
            ...
            query returns an empty list 鈥?the caller decides
        ...
        def _build_footer() -> str:
            ...
            if not search_query[0]:
                return f"/{search_query[0]}_  (type to search 鈥?Enter commit 路 Esc cancel)"
            if hits:
                current = _current_match_index(hits) + 1
                return f"/{search_query[0]}  ({current}/{len(hits)})  路 Enter commit 路 Esc cancel"
            return f"/{search_query[0]}  (no matches)  路 Esc cancel"
        ...
        if overlay.title:
            ...
            layout_children.append(
                Window(height=1, char="鈹�", style="class:bar"),
            )
        ...
        if has_sidebar:
            ...
            separator_window = Window(width=1, char="鈹?, style="class:bar")
            layout_children.append(
        ...
        # overlay 鈥?including re-pressing the trigger so the
        ...
        # Actions only make sense with a sidebar 鈥?without
        ...
        # supports multi-key sequences natively 鈥?binding the
        ...
        # 鈹�鈹� Vim / less-style incremental search 鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�
        ...
        # search has been committed 鈥?otherwise they fall
        ...
        # the new scroll position (incremental search 鈥?same
        ...
        # query and the pane is showing highlights 鈥?Esc clears
        ...
        def _search_char(event: KeyPressEvent) -> None:
            ...
            # an escape sequence 鈥?skip those so arrow keys
        ...
        # nested Application. Run to completion 鈥?returns when
        # one of the close keys calls ``event.app.exit()``.
        try:
            await app.run_async()
        finally:
            # Stop the periodic refresh no matter how the overlay
            # closed 鈥?clean exit (Esc), Ctrl+C exception, builder
    ...
    def output(self, item: FormattedItem | None) -> None:
        ...
        - ``StreamReplace``: atomic clear-streamed-region + render
          ...
          formatter for per-paragraph Markdown re-rendering 鈥?the
        ...
        if isinstance(item, StreamingText):
            ...
            # line is full-width with consistent indent 鈥?no jagged
            ...
            while _display_width(self._text_buffer) >= available:
                ...
                # ``_print_text_line`` 鈥?see ``_should_stream_more``
        ...
        # Non-streaming output 鈥?reset streamed and live line counters.
    ...
    def _print_text_line(self, text: str) -> None:
        ...
        becomes a silent no-op. The line content is NOT lost 鈥?the
        ...
        line 鈥?preventing the scrollback-duplicate render.
        """
        if not self._should_stream_more():
            return
        if not text.strip():
    ...
    def _should_stream_more(self) -> bool:
        ...
        them 鈥?the original cause of the 2026-04-28 duplicate-render
        ...
        ``_paragraph_buffer`` holds the full text 鈥?no content is
    ...
    def clear_streamed_text(self) -> None:
        ...
        panel 鈥?exactly the "raw text shows up alongside the rendered
    ...
    def _replace_live_region(
        ...
        *,
        ...
        to the viewport ceiling 鈥?the same limit
        ...
        :param commit: If ``True``, the content is committed
            permanently 鈥?``_live_line_count`` and
            ...
            region 鈥?``_live_line_count`` tracks its rendered
        ...
        if commit:
            # Committed content is permanent 鈥?no viewport cap.
    ...
    def start_timer(self) -> None:
        ...
        no-op. Two call sites fire start_timer 鈥?``on_input()``
    ...
    # ------------------------------------------------------------------
    # Sub-agent tree 鈥?live status for the main-interface badge + 鈫?menu.
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def upsert_subagent(
        ...
        *,
        ...
        # levels) hardcodes it to ``None`` 鈥?letting that null overwrite the
        ...
        # cancelled, or a durable failure detail) 鈥?NOT a merely-not-busy loop.
    ...
    def seed_subagent_tree(
        ...
        selector regardless of whether it has finished 鈥?finished sub-agents
        ...
        an unsent (still-active) node that's momentarily absent 鈥?a poll racing
        a just-spawned child 鈥?is kept.
        ...
        for sid in list(self._subagents):
            ...
            if node.done_at is None:
                continue
            # Terminal AND gone from the server snapshot 鈫?truly removed. Drop
    ...
    def _subagent_visible(self, node: _SubagentNode, now: float) -> bool:
        """Whether a node shows in the 鈫?selector tree.

        Always ``True`` 鈥?finished / warm sub-agents are retained in the
        ...
        snapshot reconciliation (the server stops listing them 鈥?see
    ...
    def _subagent_active(self, node: _SubagentNode, now: float) -> bool:
        ...
        This is the running-ness signal (NOT visibility 鈥?see
        ...
        a non-terminal ``current_task_status`` (``in_progress`` / ``queued`` 鈥?
        ...
        running 鈥?matching the web ``Idle`` state.
        """
    ...
    def subagent_tree(self) -> list[tuple[_SubagentNode, int]]:
        ...
        past the linger window are hidden (but kept 鈥?see
        ...
        # Surface any visible node not reachable from the root 鈥?an orphan
        # whose parent isn't tracked (or was hidden), or a node stranded in a
        # parent cycle 鈥?at depth 1 so no running sub-agent is silently lost.
        for node in visible:
    ...
    def has_active_subagents(self) -> bool:
        ...
        raw ``busy`` flag 鈥?see :meth:`active_subagent_count`. NOTE: this is
        running-ness, NOT selector availability; use :meth:`has_any_subagents`
        to gate the 鈫?menu so finished children remain reachable."""
        return self.active_subagent_count() > 0

    def has_any_subagents(self) -> bool:
        """True when ANY sub-agent node exists 鈥?active OR finished. Gates the
        鈫?selector + its toolbar hint, which stay available after children
    ...
    def clear_subagents(self) -> None:
        """Drop the whole tree 鈥?called when the REPL changes which top-level
    ...
    def is_subagent_chattable(self, session_id: str) -> bool:
        ...
        co-drive parity). A closed child is view-only 鈥?a ``message`` to it
        returns 409 CONFLICT 鈥?and unknown ids (e.g. the ``main`` row, a stale
    ...
    def _is_inside_subagent(self) -> bool:
        """True when the active session is a sub-agent below the tree root 鈥?
    ...
    def _subagent_status_label(self, node: _SubagentNode) -> str:
        ...
        1. ``pending_elicitations > 0`` 鈫?``Needs response`` (outranks busy: a
           sub-agent parked on an approval needs the user, not a generic badge).
        2. ``current_task_status == "launching"`` 鈫?``Launching``.
        3. ``busy`` 鈫?``Working``.
        4. a durable failure detail 鈫?``Failed``.
        5. ``current_task_status`` is ``failed`` / ``cancelled`` / ``completed``
           鈫?``Failed`` / ``Cancelled`` / ``Done``; any other non-null status is
           shown raw.
        6. otherwise (loop idle, no task) 鈫?``Idle`` (warm 鈥?still chattable;
    ...
    def subagent_menu_rows(self) -> list[tuple[str, str]]:
        """Return ``(session_id, label)`` rows for the ``鈫揱` sub-agents menu.

        Pre-order, depth-indented so the parent鈫抍hild hierarchy is visible.
        Each label carries the indent, a 鉁?鉁?glyph for finished nodes, the
        ...
        for node, depth in self.subagent_tree():
            ...
            label = f"{indent}{glyph}{name}  路 {status}"
            rows.append((node.session_id, label))
        return rows

    # ------------------------------------------------------------------
    # Inline ``鈫揱` sub-agents menu 鈥?a navigable list below the toolbar.
    # ------------------------------------------------------------------
    ...
    def _active_subagent_menu_index(self) -> int:
        ...
        pre-select that sub-agent's row (so ``鈫揱` then ``Enter`` is a no-op and
    ...
    def _open_subagent_menu(self) -> None:
        ...
        ANY node exists 鈥?active OR finished 鈥?so finished children stay
    ...
    def _subagent_row_settled(self, session_id: str) -> bool:
        """Whether a selector row is in a settled (dimmed) state 鈥?a finished
    ...
    def build_subagent_menu(self) -> FormattedText:
        ...
        return FormattedText([*rows, ("class:model-name", " 鈫戔啌 select 路 鈴?open 路 esc close")])

    def _subagent_menu_line_count(self) -> int:
        """Number of terminal lines the inline menu occupies (rows + hint),
        or 0 when closed 鈥?drives the menu Window's height."""
        rows = self._render_subagent_menu_fragments()
    ...
    async def _read_input(self) -> str | None:
        ...
        # called) 鈥?now the top-of-input line does too.
        ...
        # multiline mode" behavior to submit instead 鈥?newline
    ...
    def _spinner_frame(self) -> str:
        ...
        invalidation-driven 鈥?every render reads the same frame
    ...
    def build_prompt(self) -> FormattedText:
        ...
        # Auto-close the inline 鈫?menu when its list has drained 鈥?it renders
        ...
        for i, att in enumerate(self._pending_attachments):
            ...
            if att.is_image:
                ...
                parts.append(("class:prompt-marker", " 馃搸 "))
                parts.append(("class:model-name", f"{rel}\n"))
    ...
    def build_toolbar(self) -> FormattedText:
        ...
        Layout: ``{model 路 state-badge} 鈥?hints``. The state
        badge reads ``state: running 鉅筦` (with animated Braille
        spinner) while a handler task is running, and
        ``state: sleeping`` while idle 鈥?matching the format
        ...
        if self._stream_start is not None:
            ...
            status = f"streaming鈥?{elapsed:.0f}s"
            state_badge = f"state: running {self._spinner_frame()}"
        elif self.is_busy:
            status = "streaming鈥?
            state_badge = f"state: running {self._spinner_frame()}"
        elif n_sub > 0:
            # The top-level agent is idle but sub-agents are still working 鈥?
        ...
        # + separator so the toolbar just reads ``status 路 hints``
        # instead of a floating ``None`` or a dangling ``路``.
        parts = f" {self._model} 路 {status} " if self._model else f" {status} "
        ...
        if deadline is not None:
            ...
            # entries joined with `` 路 `` and padded with one
            ...
            if self.has_any_subagents():
                ...
                hint_items.insert(insert_at, "鈫?agents")
            if self._is_inside_subagent():
                hint_items.append("鈫?back")
            hints = " " + " 路 ".join(hint_items) + " "
        ...
        if self.pipeline_counters is not None:
            counter_segment = f" 鈹?{self.pipeline_counters.toolbar_text()} "
        # Context-window ring indicator. Shows a single Unicode arc
        # character (鈼嬧棓鈼戔棔鈼? followed by a compact percentage, matching
        ...
        segments.append(("class:bar", "鈹�" * bar_right))
