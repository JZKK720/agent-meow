...
async def test_iter_sse_events_skips_malformed_frame() -> None:
    ...
    # not fatal â€” the good events before and after it still yield, so a turn
...
async def test_client_create_and_submit_request_shapes() -> None:
    create = respx.post("http://agent_meow.test/v1/sessions").mock(
        return_value=httpx.Response(201, json={"id": "conv_1"})
    )
    submit = respx.post("http://agent_meow.test/v1/sessions/conv_1/events").mock(
...
async def test_check_health_probes_health_endpoint() -> None:
    health = respx.get("http://agent_meow.test/health").mock(
...
async def test_validate_returns_agents_and_online_hosts() -> None:
    respx.get("http://agent_meow.test/health").mock(
        return_value=httpx.Response(200, json={"status": "ok"})
    )
    respx.get("http://agent_meow.test/v1/agents").mock(
        return_value=httpx.Response(200, json={"data": [{"id": "ag_1", "name": "Helper"}]})
    )
    respx.get("http://agent_meow.test/v1/hosts").mock(
...
async def test_validate_raises_auth_required_on_401() -> None:
    ...
    respx.get("http://agent_meow.test/v1/agents").mock(return_value=httpx.Response(401))
    client = OmnigentClient("http://agent_meow.test")
...
async def test_validate_raises_auth_required_on_proxy_redirect() -> None:
    ...
    # The bot must treat that as auth-required (â†’ start enrollment), not as a
...
async def test_get_host_home_derives_home_from_filesystem_listing() -> None:
    respx.get("http://agent_meow.test/v1/hosts/host_1/filesystem").mock(
        return_value=httpx.Response(
            200,
            json={
                "object": "list",
...
async def test_client_pool_reuses_client_per_server() -> None:
    ...
    try:
        first = await pool.get("http://agent_meow.test/")
        again = await pool.get("http://agent_meow.test")
...
async def test_launch_runner_on_explicit_host() -> None:
    launch = respx.post("http://agent_meow.test/v1/hosts/host_1/runners").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched"})
    )
    respx.get("http://agent_meow.test/v1/runners/runner_launched/status").mock(
...
async def test_launch_runner_picks_random_online_host_when_unspecified() -> None:
    ...
    launch = respx.post("http://agent_meow.test/v1/hosts/host_online/runners").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched"})
    )
    respx.get("http://agent_meow.test/v1/runners/runner_launched/status").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched", "online": True})
...
async def test_launch_runner_raises_host_unavailable_when_runner_never_online() -> None:
    ...
    respx.get("http://agent_meow.test/v1/runners/runner_x/status").mock(
        return_value=httpx.Response(200, json={"online": False})
    )
    client = OmnigentClient("http://agent_meow.test", runner_launch_timeout_seconds=0.01)
...
async def test_run_turn_streams_across_multiple_responses_until_id_terminal() -> None:
    ...
    respx.post("http://agent_meow.test/v1/sessions/conv_1/events").mock(
        return_value=httpx.Response(200, json={})
    )
    client = OmnigentClient("http://agent_meow.test")
...
async def test_run_turn_ignores_stale_idle_when_resuming_idle_session() -> None:
    ...
    # (idle=false) replays the session's CURRENT status first â€” a stale id-less
    # `idle` â€” which arrives BEFORE the just-submitted message's `running` edge.
...
async def test_resolve_elicitation_posts_accept() -> None:
    route = respx.post(
        "http://agent_meow.test/v1/sessions/conv_1/elicitations/elicit_1/resolve"
...
async def test_get_session_activity_maps_server_state() -> None:
    ...
    client = OmnigentClient("http://agent_meow.test")
    try:
        route = respx.get("http://agent_meow.test/v1/sessions/conv_1")
...
async def test_stream_session_events_classifies_mid_stream_drop() -> None:
    ...
    with respx.mock:
        ...
        try:
            ...
            # classified â€” mirroring how ``_run_turn_once`` consumes the stream.
            async with client.stream_session_events("conv_1") as events:
...
async def test_run_turn_reconnects_on_mid_stream_drop_without_resubmit() -> None:
    ...
    # Session still running after the drop â†’ reconnect (not a clean stop).
    ...
    # The message was submitted exactly once â€” the reconnect did not start a
    # second turn.
...
async def test_run_turn_raises_stream_interrupted_when_reconnect_exhausted(
    ...
    # turn running â†’ give up as StreamInterruptedError (a non-alarming "lost the
...
async def test_run_turn_survives_many_drops_that_each_make_progress(
    ...
    # proxy caps. Each leg streams a NEW delta before dropping â€” genuine progress
    # â€” so the consecutive-reconnect budget resets and the turn is NOT abandoned.
    monkeypatch.setattr(omnigent_module, "_STREAM_RECONNECT_BACKOFF_S", 0.0)
    ...
    # Every leg's new delta was forwarded and the turn completed â€” not abandoned
