...
class _ViewUpdateAck:
    ...
    background task where there is no live ``ack`` â€?only the modal's
    ``view_id`` â€?so this shim turns the same call into a ``views_update``.
    """
...
class SetupFlow:
    ...
    operator â€?never entered by a user), so setup no longer asks for a URL.
    ...
    async def _handle_config_command(self, ack: Any, command: dict[str, Any], client: Any) -> None:
        ...
        # server â€?connectivity is validated immediately, login is folded in
    ...
    async def _handle_logout(self, *, team_id: str, user_id: str, client: Any) -> None:
        """Handle ``/omnigent logout`` â€?full reset for the user.

        Revokes every delegated token the user holds and clears all their
        saved settings (agent/host/workspace plus threadâ†’session mappings),
        ...
        if dm_channel:
            ...
            await client.chat_postMessage(
                ...
                text=(
                    ...
                    "cleared â€?run `/omnigent` to set up again."
                ),
            )
        else:
        ...
        *,
        ...
        if in_channel:
            await client.chat_postEphemeral(
                ...
                user=user_id,
                thread_ts=thread_ts,
                text="Let's get you set up â€?check your DM with me to configure agent_meow.",
            )
    ...
    async def _open_connecting_modal(self, client: Any, trigger_id: str) -> str | None:
        """Open the initial "connectingâ€? modal and return its ``view_id``.
    ...
    async def _begin_setup(self, client: Any, *, team_id: str, user_id: str, view_id: str) -> None:
        ...
        try:
            ...
            # the moment the user approves â€?no DM, no re-running /agent_meow.
    ...
    async def _advance_to_select(
        ...
        if not validated.agents:
            # Not a form-validation error (there's no live submission here â€?
    ...
    async def _begin_in_modal_login(
        ...
        *,
        ...
        async def _on_success() -> None:
            ...
            # can fail if the user already closed the modal â€?log, don't crash
    ...
    return [
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "âš™ï¸ Set up Omnigent"},
                    "style": "primary",
...
def connecting_modal() -> dict[str, Any]:
    ...
    # views_update. No submit â€?it just shows progress.
    return {
        "type": "modal",
        ...
        "blocks": [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": "Connecting to Omnigentâ€?},
            }
        ],
    }
...
def no_agents_modal(server_url: str) -> dict[str, Any]:
    # Shown when the connected server exposes no agents to choose from â€?setup
...
def login_waiting_modal(server_url: str, verification_url: str, user_code: str) -> dict[str, Any]:
    ...
    # to the agent/host picker via views_update â€?no DM, no re-running the
    ...
    return {
        ...
        "blocks": [
            ...
            },
            {
                "type": "context",
                "elements": [
                    {"type": "mrkdwn", "text": "Waiting for approvalâ€?keep this window open."}
                ],
            },
        ],
    }
...
def _plain(text: str) -> str:
    ...
    return text if len(text) <= 75 else text[:74] + "â€?
...
def _dm_channel_id(opened: Any) -> str | None:
    ...
    # plain dict â€?but it proxies ``.get``/``[]`` to the underlying payload, so
