...
event arrive as normal events â€?no polling.
...
class PendingElicitation:
    ...
    The pushed ``response.elicitation_resolved`` â€?or the resolver itself â€?
    ...
    # posted (external answer) â€?decides the card's outcome label.
...
class ElicitationController:
    ...
    async def handle_action(self, *, elicitation_id: str, verdict: Verdict) -> bool:
        ...
        Returns whether a live waiter received it â€?``False`` means the request
    ...
    async def reject_non_owner_click(
        ...
        try:
            await client.chat_postEphemeral(
                ...
                text=(
                    "This request belongs to whoever started the thread â€?only they "
    ...
    async def start(
        ...
        typed input), it posts a web-UI link and returns â€?no card, no resolver;
    ...
    async def _resolve_verdict(
        ...
        if verdict is None:
            # Nobody answered in time â€?decline so the server park releases, and
    ...
    async def on_resolved(
        ...
        Fired when the server pushes ``response.elicitation_resolved`` â€?for our
    ...
    def _outcome(pending: PendingElicitation) -> ElicitationOutcome:
        ...
        if verdict is None:
            # No Slack verdict was posted â€?answered elsewhere (web UI/other
            # client). We don't know which way it went â€?neutral label.
