...
# is actually on screen â€?on the first flushed delta, or after the finalizing
# stop() for a buffered answer â€?so the thread never shows an empty gap between
# the placeholder vanishing and the reply appearing.
_ACK_TEXT = "_Working on itâ€¦_"
...
# Shown when the server rejects the request as unauthenticated â€?the user's
# delegated login is missing or expired (e.g. the bot restarted and in-memory
# tokens were lost). They re-authenticate by running /agent_meow.
_AUTH_REQUIRED_TEXT = (
...
def _turn_error_text(exc: BaseException, server_url: str) -> str | None:
    ...
    if isinstance(exc, HarnessNotConfiguredError):
        # The server's message is curated, actionable guidance for this code â€?
...
class SlackOmnigentService:
    def __init__(
        ...
        *,
        ...
        # target â€?any server_url persisted on an older config/session row is
        ...
        # deflection notices) â€?keeps message formatting out of this class.
        ...
        # task, finalize) â€?keeps this class to routing + turn lifecycle.
        ...
        # any await, so two racing messages can't both pass) â€?necessary because
    ...
    async def handle_message(
        ...
        *,
        ...
        if not _is_direct_message(event):
            ...
            # arrives as an app_mention event). Plain messages â€?even a reply in
            # a thread that already has a session, and even one that mentions the
            # bot (app_mention handles that copy) â€?are human discussion and must
        ...
        # get â€?strip the mention (if any) and treat it like any other DM rather
    ...
    async def _route_turn(
        ...
        *,
        ...
        if not requester:
            # No authenticated Slack user on the event â€?we can't attribute the
        ...
        # this process â†?deflect. This is distinct from the server-activity check
        ...
        try:
            ...
            if record is not None:
                ...
                # to the session. Tell that user â€?privately â€?why nothing
                ...
                activity = await agent_meow.get_session_activity(record.session_id)
            ...
            # Release the reservation unless a turn was spawned â€?the spawned
    ...
    async def _run_turn(self, turn: SlackTurn) -> None:
        ...
        # Acknowledge now â€?AFTER any session-config summary â€?so a new thread
        # reads metadata â†?"Working on itâ€? â†?answer. The create + runner launch
        ...
        baseline = await agent_meow.latest_assistant_message(session_id)
        ...
        if reply.needs_fallback_text():
            ...
            latest = await agent_meow.latest_assistant_message(session_id)
            if (
                latest is not None
                and latest != baseline
    ...
    async def _ensure_session(self, turn: SlackTurn, omnigent: OmnigentClient) -> str | None:
        ...
        try:
            session_id = await agent_meow.create_session(turn.agent_id, turn.title)
            runner_id = await agent_meow.launch_runner(
            ...
            # must still report rather than strand the thread on "Working on itâ€?.
        ...
        try:
            info = await agent_meow.get_session_info(session_id)
    ...
    async def _stream_turn(
        ...
        mrkdwn conversion or msg_too_long handling here â€?just event routing.
        ...
        state = _StreamState()
        try:
            async for event in agent_meow.run_turn(
    ...
    async def _dispatch_stream_event(
        ...
        if elicitation is not None:
            ...
            # card and spawn a background resolver â€?WITHOUT blocking this loop.
        ...
        if todos is not None:
            # The first plan post is a new out-of-band message â†?seal before it;
...
async def _session_title(
    ...
    a missing scope) â€?the title is cosmetic and must never block session start.
    """
