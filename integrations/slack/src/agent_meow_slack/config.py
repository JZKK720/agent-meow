...
class Settings(BaseSettings):
    ...
    slack_bot_token: str = Field(validation_alias="agent_meow_slack_BOT_TOKEN")
    slack_app_token: str = Field(validation_alias="agent_meow_slack_APP_TOKEN")
    ...
    database_path: Path = Field(
        default_factory=lambda: _local_data_dir() / "agent_meow_slack.sqlite3",
        validation_alias="agent_meow_slack_DATABASE_PATH",
    )
    ...
    token_encryption_key: str | None = Field(
        default=None,
        validation_alias="agent_meow_slack_TOKEN_ENCRYPTION_KEY",
    )
    ...
    # event. Set agent_meow_slack_SERVER_AUTH=databricks and register a custom U2M
    ...
    server_auth_mode: ServerAuthMode = Field(
        default="auto",
        validation_alias="agent_meow_slack_SERVER_AUTH",
    )
    ...
    databricks_workspace_host: str | None = Field(
        ...
        validation_alias="agent_meow_slack_DATABRICKS_WORKSPACE_HOST",
    )
    ...
    databricks_oauth_client_id: str | None = Field(
        default=None,
        validation_alias="agent_meow_slack_DATABRICKS_CLIENT_ID",
    )
    databricks_oauth_client_secret: str | None = Field(
        default=None,
        validation_alias="agent_meow_slack_DATABRICKS_CLIENT_SECRET",
    )
    ...
    databricks_state_secret: str | None = Field(
        default=None,
        validation_alias="agent_meow_slack_DATABRICKS_STATE_SECRET",
    )
    ...
    databricks_oauth_scopes: str = Field(
        default="all-apis",
        validation_alias="agent_meow_slack_DATABRICKS_SCOPES",
    )
    ...
    databricks_webauth_base_url: str | None = Field(
        default=None,
        validation_alias="agent_meow_slack_WEBAUTH_BASE_URL",
    )
    ...
    databricks_webauth_port: int = Field(
        ...
        validation_alias="agent_meow_slack_WEBAUTH_PORT",
    )
    ...
    def _check_databricks_config(self) -> Settings:
        ...
        missing = [
            name
            for name, value in (
                ("agent_meow_slack_DATABRICKS_CLIENT_ID", self.databricks_oauth_client_id),
                ("agent_meow_slack_DATABRICKS_CLIENT_SECRET", self.databricks_oauth_client_secret),
                ("agent_meow_slack_DATABRICKS_STATE_SECRET", self.databricks_state_secret),
                # workspace_host defaults to DATABRICKS_HOST (injected on the
                # platform); still required for a laptop run where it's unset.
                ("agent_meow_slack_DATABRICKS_WORKSPACE_HOST", self.databricks_workspace_host),
            )
            if not value
        ]
        if missing:
            raise ValueError(
                "agent_meow_slack_SERVER_AUTH=databricks requires " + ", ".join(missing)
            )
        ...
        if host.startswith("http://") and not _is_loopback_url(host):
            raise ValueError(
                "agent_meow_slack_DATABRICKS_WORKSPACE_HOST must use https:// "
        ...
        if base.startswith("http://") and not _is_loopback_url(base):
            raise ValueError(
                "agent_meow_slack_WEBAUTH_BASE_URL must use https:// "
        ...
        if len(self.databricks_state_secret or "") < _MIN_STATE_SECRET_LEN:
            raise ValueError(
                f"agent_meow_slack_DATABRICKS_STATE_SECRET must be at least "
