...
# Check the agent version (should be v3 now with the use_responses fix)
$agents = & $curl -s http://127.0.0.1:6767/v1/agents
if ($agents -match '"name":"hermes-gateway","version":(\d+)') { Write-Output "hermes-gateway agent version: $($Matches[1])" }
